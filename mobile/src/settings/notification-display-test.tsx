...
export function NotificationDisplayTest({ onTroubleshoot }: { onTroubleshoot: () => void }) {
  ...
  const run = async () => {
    ...
    try {
      ...
      for (const { client } of connected) {
        ...
        if (result?.accepted) {
          setMessage('Accepted by Dorka’s push service. Check for the notification.')
          return
        }
        ...
        throw new Error(
          result?.reason === 'rate_limited'
            ...
            : 'Could not send through Dorka’s push service. Try again.'
        )
      }
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.detail}>Send a test through Dorka’s push service.</Text>
      <Pressable
