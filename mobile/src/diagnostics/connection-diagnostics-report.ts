...
export function buildConnectionDiagnosticsReport(args: {
  ...
  const lines: string[] = []
  lines.push('Dorka Mobile connection diagnostics')
  lines.push(`Generated: ${new Date(now).toISOString()}`)
  lines.push(`App: Dorka Mobile ${args.appVersion} · ${args.platform}`)
  const desktopAppVersion = normalizeHostAppVersion(args.desktopAppVersion)
  lines.push(`Host Dorka version: ${desktopAppVersion ?? 'unknown'}`)
