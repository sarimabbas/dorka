...
describe('buildConnectionDiagnosticsReport', () => {
  it('summarizes a failing Tailscale host with its log', () => {
    ...
    expect(report).toContain('App: Dorka Mobile 0.0.29 · ios 26.5.1')
    expect(report).toContain('Host Dorka version: 1.4.191')
  ...
  it('marks never-connected sessions and empty logs', () => {
    ...
    expect(report).toContain('Host Dorka version: unknown')
  ...
  it('explains the most likely cause and redacts credentials before copying', () => {
    ...
    expect(report).toContain(
      'Likely cause: Relay service was temporarily unavailable and asked Dorka to retry in 30s.'
    )
    expect(report).toContain('Path: active=tailscale; recovery=relay')
    expect(report).toContain('Next step: Keep Dorka open; recovery should retry automatically.')
