...
describe('resolveHostEndpointEdit', () => {
  it.each([
    ...
    ['secure path-routed proxy', 'wss://desk.example.com/dorka'],
    ['secure query-routed proxy', 'wss://desk.example.com/dorka?route=runtime'],
  ...
  it('discriminates invalid input without a candidate endpoint', () => {
    expect(
      resolveHostEndpointEdit('wss://desk.example.com/dorka', 'https://desk.example.com')
    ).toEqual({
      kind: 'invalid',
  ...
  it('stays stable through repeated name-only edits', () => {
    const stored = 'wss://desk.example.com/dorka?route=runtime'
