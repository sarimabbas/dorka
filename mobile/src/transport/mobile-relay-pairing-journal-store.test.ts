...
const JOURNAL_PRESENCE_KEY = 'dorka:pairing-keychain-presence:dorka.mobile-relay.pairing-journal.v1'
const offer = {
  v: 2,
...
describe('mobile relay pairing journal store', () => {
  ...
  it('retries the journal under a distinct alias before relay pairing connects (#6600)', async () => {
    ...
    secureStore.setItemAsync.mockImplementation(
      async (_key: string, value: string, options?: { keychainService?: string }) => {
        if (options?.keychainService === undefined) {
          throw new Error(
            "Could not encrypt the value for key 'dorka.mobile-relay.pairing-journal.v1' under keychain 'key_v1'. Caused by: unknown"
          )
        }
        secretRaw = value
      }
    ...
    expect(secureStore.setItemAsync).toHaveBeenLastCalledWith(
      'dorka.mobile-relay.pairing-journal.v1',
      expect.any(String),
      expect.objectContaining({ keychainService: 'dorka.pairing.v1' })
    )
  })
