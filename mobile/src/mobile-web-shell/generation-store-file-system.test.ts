...
describe('generationDirectoryPath', () => {
  it('hands the native view the absolute path both loaders demand', () => {
    ...
    expect(generationDirectoryPath('file:///data/user/0/com.stably.dorka.mobile/cache/mw')).toBe(
      '/data/user/0/com.stably.dorka.mobile/cache/mw'
    )
  })

  it('decodes what a uri escaped and a path spells literally', () => {
    expect(generationDirectoryPath('file:///var/Dorka%20Mobile/mobile-web')).toBe(
      '/var/Dorka Mobile/mobile-web'
    )
  })
