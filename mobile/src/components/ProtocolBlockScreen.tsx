...
const IOS_APP_STORE_URL = 'itms-apps://apps.apple.com/app/dorka-ide/id6766130217'
...
function blockTitle(remedy: BlockRemedy): string {
  switch (remedy) {
    case 'update-mobile':
      return 'Update Dorka Mobile'
    case 'update-desktop':
      return 'Update Dorka on your computer'
    case 'refresh-bundle':
...
function blockBody(verdict: BlockedVerdict, remedy: BlockRemedy, storeName: string): string {
  ...
  if (verdict.reason === 'mobile-too-old') {
    return `This desktop needs a newer Dorka Mobile app. Update Dorka Mobile from ${storeName}, then try this host again.`
  }
  if (verdict.reason === 'bundle-unavailable') {
    return 'This paired desktop app does not include the mobile workspace yet. Update Dorka on your computer, then try this host again.'
  }
  if (remedy === 'update-mobile') {
    return `This desktop's mobile workspace needs a newer Dorka Mobile app. Update Dorka Mobile from ${storeName}, then try this host again.`
  }
