import { z } from 'zod'
import type { SetupRunPolicy } from '../../../src/shared/dorka-yaml-hook-types'
 ...
 * handler always answers it, `null` when no dorka.yaml parsed. Nullable rather than optional so the
 ...
 * there for a binary or unparsable dorka.yaml. `setupRunPolicy` is a closed enum with the call
 ...
 * (src/shared/dorka-yaml-hook-types.ts:1), which is what `getEffectiveSetupRunPolicy` answers, and
...
const trustedDorkaHookApprovalSchema = z.looseObject({
  contentHash: z.string(),
  approvedAt: z.number()
})

const trustedDorkaHookRepoSchema = z.looseObject({
  all: salvagedOptional('all', z.looseObject({ approvedAt: z.number() })),
  setup: salvagedOptional('setup', trustedDorkaHookApprovalSchema),
  archive: salvagedOptional('archive', trustedDorkaHookApprovalSchema),
  issueCommand: salvagedOptional('issueCommand', trustedDorkaHookApprovalSchema),
  vmRecipe: salvagedOptional('vmRecipe', trustedDorkaHookApprovalSchema)
})
 ...
 * Main cast the result and read `?.trustedDorkaHooks ?? {}`, which tolerated a reply of any shape,
 * so anything that is not an object decodes as absent and reaches that `?? {}` exactly as before —
 * use-new-workspace-runtime-context.ts:80 takes `ui.value?.trustedDorkaHooks ?? {}`. The record
...
export const newWorkspaceUiTrustSchema = z
  .looseObject({
    ui: salvagedOptional(
      ...
      z
        .looseObject({
          trustedDorkaHooks: salvagedOptional(
            'trustedDorkaHooks',
            salvagingRecord(z.string(), trustedDorkaHookRepoSchema)
          )
        })
        .nullable()
    )
