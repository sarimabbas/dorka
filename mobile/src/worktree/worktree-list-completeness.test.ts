...
// Same-named repos on different hosts (a local "dorka" and a runtime "dorka")
...
describe('every worktree stays visible regardless of repo.list contents', () => {
  it('shows worktrees whose repoId is missing from repo.list', () => {
    ...
    const worktrees = [
      worktree('dorka-local', 'a'),
      worktree('dorka-local', 'b'),
      worktree('dorka-runtime', 'c')
    ]
    const repoIdsByName = new Map([['dorka-runtime', 'dorka-runtime']])
  ...
  it('shows every worktree when the same repo name maps to two host-specific ids', () => {
    // The exact trigger: a local "dorka" and a runtime "dorka". repo.list returns
    // both under the same displayName; a name-keyed map keeps only the last id.
    const worktrees = [
      worktree('dorka-local', 'local-1', 'dorka'),
      worktree('dorka-local', 'local-2', 'dorka'),
      worktree('dorka-runtime', 'runtime-1', 'dorka')
    ]
    // Name-keyed map collapses "dorka" to a single id, as the host screen builds it.
    const repoIdsByName = new Map([['dorka', 'dorka-runtime']])
