...
public class DorkaNotificationDismissalSubscriber: ExpoAppDelegateSubscriber {
  public func application(
    _ application: UIApplication,
    didReceiveRemoteNotification userInfo: [AnyHashable: Any],
    fetchCompletionHandler completionHandler: @escaping (UIBackgroundFetchResult) -> Void
  ) {
    guard let payload = userInfo["dorka"] as? [String: Any],
    ...
    center.getDeliveredNotifications { notifications in
      let ids = notifications.compactMap { notification -> String? in
        guard let data = notification.request.content.userInfo["dorka"] as? [String: Any],
