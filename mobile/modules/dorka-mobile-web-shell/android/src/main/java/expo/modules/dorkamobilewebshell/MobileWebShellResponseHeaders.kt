...
internal fun mobileWebShellResponseHeaders(path: String, byteCount: Int): Map<String, String> {
  ...
  if (path == "/") {
    ...
    // `https://<sha256(sessionId) first 32 hex>.dorka-mobile-web.invalid/`, so a referrer carries a
    // stable per-session handle rather than the id itself; iOS serves
    // `dorka-mobile-web://<sessionId>/` and carries the id verbatim. Either is something
