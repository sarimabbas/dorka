...
private let networkBlockIdentifier = "dev.dorka.mobile-web-shell.network-block-v1"
...
/// Installs `window.dorkaBridge`, the whole page-facing surface: `postMessage(json)` and an
...
private let bridgeInstaller = """
  (function(){
  var sink=window.webkit.messageHandlers.dorkaBridge;
  var handler=null;
  var bridge={};
  Object.defineProperty(bridge,'postMessage',{value:function(json){
  if(typeof json!=='string'){throw new TypeError('dorkaBridge.postMessage expects a string')}
  ...
  Object.defineProperty(globalThis,'dorkaBridge',{value:bridge,configurable:false,writable:false,enumerable:true});
  })();
  """
...
private let bridgeDeliver = """
  globalThis.dorkaBridge.__deliver(m)
  """
...
private final class MobileWebShellSchemeHandler: NSObject, WKURLSchemeHandler {
  ...
  private let readQueue = DispatchQueue(label: "dev.dorka.mobile-web-shell.read")
...
private final class MobileWebShellBridgeReceiver: NSObject, WKScriptMessageHandler {
  weak var view: DorkaMobileWebShellView?
...
final class DorkaMobileWebShellView: ExpoView, WKNavigationDelegate, WKUIDelegate {
