...
Vendored Expo module for capturing and playing PCM audio data in the Dorka mobile app (iOS and Android).
...
```
npm i @dorka/expo-two-way-audio
```
...
1. Request permissions for recording audio
   ...
   ```JSX
   import {useMicrophonePermissions} from "@dorka/expo-two-way-audio";
...
Some audio features of expo-two-way-audio like Acoustic Echo Cancelling, noise reduction or microphone modes (iOS) don't work on simulator. Run the Dorka mobile app on a real device to test these features.
