#!/usr/bin/env node
/**
 * Start Dorka Mobile server and load it in the iOS emulator.
...
const DORKA_CLI = process.env.DORKA_CLI || 'dorka'
...
// Execute dorka CLI command
async function dorka(args, options = {}) {
  const { stdout, stderr } = await execFileAsync(DORKA_CLI, args, {
...
async function getWorktree() {
  ...
  try {
    const { stdout } = await dorka(['worktree', 'current', '--json'])
...
async function attachEmulator(worktree, device, runtime) {
  ...
  try {
    await dorka(['emulator', 'attach', device.udid, '--worktree', worktree, '--focus', '--json'], {
      cwd: worktree,
...
function devClientUrlForMetroUrl(url) {
  return `exp+dorka-mobile://expo-development-client/?url=${encodeURIComponent(url)}`
}
...
async function startMetro(worktree) {
  ...
  return new Promise((resolve, reject) => {
    ...
    rl.on('line', (line) => {
      ...
      const urlMatch = line.match(/exp\+dorka-mobile:\/\/expo-development-client\/\?url=([^\s]+)/)
...
async function openPairingUrlInSimulator(pairingUrl, deviceUdid, runtime, worktree) {
  ...
  await dorka(['emulator', 'tap', '0.5', '0.56', '--worktree', worktree, '--json'], {
    cwd: worktree,
    env: runtime?.env || process.env,
    timeout: 30000
  })
...
async function takeScreenshot(
  deviceUdid,
  outputPath = path.join(os.tmpdir(), 'dorka-mobile-ios.png')
) {
...
// Main function
async function main() {
  log(colors.bright + 'Starting Dorka Mobile in Emulator\n' + colors.reset)
  ...
  try {
    ...
    // Why: emulator helpers are worktree-scoped in Dorka; attach is idempotent
