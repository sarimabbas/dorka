...
export async function startHeadlessPairingRuntime({
  ...
  const runDir = mkdtempSync(path.join(os.tmpdir(), 'dorka-mobile-run.'))
  ...
  const child = spawn(
    ...
    ['serve', '--mobile-pairing', '--pairing-address', pairingAddress, '--json'],
    {
      ...
      env: {
        ...process.env,
        DORKA_E2E_USER_DATA_DIR: userData,
        DORKA_E2E_HOME_DIR: homeDir,
        HOME: homeDir,
        USERPROFILE: homeDir
      },
...
export async function registerWorktreeForPairingRuntime(runtime, worktree, tools) {
  ...
  await tools.dorka(['repo', 'add', '--path', worktree, '--json'], {
    cwd: worktree,
    env: runtime.env,
    timeout: 60000
  })
...
async function waitForPairingRuntime({ child, userData, pairingAddress, logSuccess }) {
  ...
  const runtimeResult = (pairingUrl) => ({
    ...
    env: {
      ...process.env,
      DORKA_USER_DATA_PATH: userData,
      // Why: `dorka-dev` derives its own profile and ignores DORKA_USER_DATA_PATH, so
      // without this an DORKA_CLI=dorka-dev run would address the dev profile instead
      // of this disposable runtime. Plain `dorka` ignores it.
      DORKA_DEV_USER_DATA_PATH: userData
    },
    stop
  })
