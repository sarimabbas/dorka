...
const USER_DATA =
  process.env.DORKA_USER_DATA ?? `${process.env.HOME}/Library/Application Support/dorka-dev`
...
const devices = readJson<Array<{ token: string }>>(join(USER_DATA, 'dorka-devices.json'))
const token = devices[0]?.token
const keypair = readJson<{ publicKeyB64: string }>(join(USER_DATA, 'dorka-e2ee-keypair.json'))
