...
#
# Everything is driven with xcrun simctl rather than `dorka emulator`, because the
...
#     > /tmp/dorka-emulator-boot.log 2>&1 &
#
...
LOG="${3:-/tmp/dorka-emulator-boot.log}"
BUNDLE_ID=com.stably.dorka.mobile
# Long enough for the tiered backoff to reach its 30s/60s tail.
ESCALATE_SECONDS=200

APP=$(pgrep -f "CoreSimulator.*Dorka.app/Dorka" | head -1)
...
T0=$(date +%s)
step "[5] user returns to Dorka  <-- t0, desktop still down"
