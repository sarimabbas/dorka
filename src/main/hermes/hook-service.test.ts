...
describe('HermesHookService', () => {
  ...
  beforeEach(() => {
    ...
    homeDir = mkdtempSync(join(tmpdir(), 'dorka-hermes-hooks-'))
  ...
  it('preserves other enabled plugins and removes Dorka from disabled list', () => {
    writeFileSync(
  ...
  it('registered plugin hooks post normalized JSON to Dorka', async () => {
    ...
    const received = new Promise<Record<string, unknown>>((resolve, reject) => {
      const server = createServer((req, res) => {
        ...
        req.on('end', () => {
          try {
            ...
            expect(req.headers['x-dorka-agent-hook-token']).toBe('token-1')
            res.writeHead(204)
            res.end()
      ...
      server.listen(0, '127.0.0.1', () => {
        ...
        const script = [
          ...
          `spec = importlib.util.spec_from_file_location("dorka_status", ${JSON.stringify(initPath)})`,
