export const HERMES_PLUGIN_NAME = 'dorka-status'
export const HERMES_PLUGIN_MARKER = 'Managed by Dorka. Do not edit; changes may be overwritten.'
...
export function getPluginManifest(): string {
  return [
    ...
    'version: 1.0.0',
    'description: "Reports Hermes Agent lifecycle events to Dorka."',
    'author: "Dorka"',
    'kind: standalone',
    'provides_hooks:',
...
def _endpoint_env() -> dict[str, str]:
    env = dict(os.environ)
    endpoint = env.get("DORKA_AGENT_HOOK_ENDPOINT", "")
...
def _post_to_dorka(payload: dict[str, Any]) -> None:
    env = _endpoint_env()
    port = env.get("DORKA_AGENT_HOOK_PORT", "")
    token = env.get("DORKA_AGENT_HOOK_TOKEN", "")
    pane_key = env.get("DORKA_PANE_KEY", "")
    if not port or not token or not pane_key:
        return
    body = {
        "paneKey": pane_key,
        "launchToken": env.get("DORKA_AGENT_LAUNCH_TOKEN", ""),
        "tabId": env.get("DORKA_TAB_ID", ""),
        "worktreeId": env.get("DORKA_WORKTREE_ID", ""),
        "env": env.get("DORKA_AGENT_HOOK_ENV", ""),
        "version": env.get("DORKA_AGENT_HOOK_VERSION", ""),
        "payload": payload,
    }
...
def _make_hook(event_name: str) -> Callable[..., None]:
    def _hook(**kwargs: Any) -> None:
        _post_to_dorka(_payload_for_event(event_name, kwargs))
