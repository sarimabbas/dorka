/**
 * The single `.zshenv` Dorka writes for every transport: local PTY, daemon/SSH,
 * and relay.
 *
 * Dorka needs to run code AFTER the user's own zsh startup files. The old shape
 * bought that by keeping ZDOTDIR pointed at Dorka's wrapper dir for the whole of
 ...
 * unconditionally, so history landed inside Dorka's own dir (#11044); zsh's
 ...
 * Dorka's work to a `precmd` hook that runs at the first prompt — after
...
export type ZshWrapperRestoreSpec = {
  /** Dorka's agent-teams shim dir back onto PATH. */
  agentTeamsPath: boolean
  /** Remote CLI bin dir onto PATH — relay hosts only. */
  remoteCliBinDir: boolean
  /** Dorka's runtime CODEX_HOME. */
  codexHome: boolean
  /** The `codex()` wrapper that runs Dorka's launch preflight. */
  codexLaunchPreflight: boolean
}

export type ZshStartupHookSpec = {
  /** First line of the generated file, e.g. `# Dorka zsh shell-ready wrapper`. */
  headerLabel: string
...
const AGENT_TEAMS_PATH_RESTORE_BLOCK = `__dorka_restore_agent_teams_path() {
...
const OPENCODE_CONFIG_DIR_RESTORE = `[[ -n "\${DORKA_OPENCODE_CONFIG_DIR:-}" ]] && export OPENCODE_CONFIG_DIR="\${DORKA_OPENCODE_CONFIG_DIR}"`
const MIMOCODE_HOME_RESTORE = `[[ -n "\${DORKA_MIMOCODE_HOME:-}" ]] && export MIMOCODE_HOME="\${DORKA_MIMOCODE_HOME}"`
const REMOTE_CLI_BIN_DIR_RESTORE = `[[ -n "\${DORKA_REMOTE_CLI_BIN_DIR:-}" ]] && case ":$PATH:" in *:"\${DORKA_REMOTE_CLI_BIN_DIR}":*) ;; *) export PATH="\${DORKA_REMOTE_CLI_BIN_DIR}:$PATH" ;; esac`
const CODEX_HOME_RESTORE = `# Why: Codex must keep using Dorka's runtime CODEX_HOME after rc files.
 ...
 */
const ZSH_OSC133_FUNCTION_BLOCK = `__dorka_osc133_precmd() {
  local exit_code=$?
...
/**
 * Everything Dorka owns that must run after the user's config, in one function
...
function buildDeferredInit(spec: ZshStartupHookSpec): string {
  ...
  // the unwrapped pane it was — no stray Dorka name in `precmd_functions`.
