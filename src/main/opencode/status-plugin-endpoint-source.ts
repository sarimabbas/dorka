export function getStatusPluginEndpointSource(): string[] {
  return [
    ...
    "// OpenCode's Node process (not Dorka's) and has no access to server.ts's",
    ...
    '// which reads the endpoint file. The file only changes on Dorka restart',
    ...
    '      // writeEndpointFile on the Dorka side) allocates a fresh inode on',
    ...
    '        // digits in the key for forward-compat with future DORKA_AGENT_HOOK_*',
    ...
    '      console.warn("[dorka-hook] failed to parse endpoint file:", err.message);',
    '    }',
    '    return null;',
    '  }',
    '}',
    ...
    '  // frozen when OpenCode was fork()ed — stale after an Dorka restart. The',
    '  // file is rewritten on every Dorka start(), so sourcing it per post lets',
    '  // a long-running OpenCode session reach the current server. Falls back',
    '  // to process.env when the file is absent (first-run / pre-endpoint-file / Dorka',
