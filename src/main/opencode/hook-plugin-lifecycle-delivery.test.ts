...
describe('OpenCode plugin lifecycle delivery', () => {
  ...
  beforeEach(() => {
    tempDir = mkdtempSync(join(tmpdir(), 'dorka-opencode-lifecycle-plugin-'))
    posts = []
    savedEnv = {}
  ...
  async function loadHooksWithSession(session: SessionClientFixture): Promise<PluginHooks> {
    ...
    return module.DorkaOpenCodeStatusPlugin({ client: { session } })
  }
  ...
  it('falls back per coordinate when the endpoint file is partial or malformed', async () => {
    ...
    writeFileSync(
      endpointPath,
      'not-an-assignment\nDORKA_AGENT_HOOK_TOKEN=file-token\nBROKEN LINE\n'
    )
    process.env.DORKA_AGENT_HOOK_ENDPOINT = endpointPath
    ...
    expect(new Headers(init?.headers).get('X-Dorka-Agent-Hook-Token')).toBe('file-token')
  })

  it('warns once for an unreadable endpoint without exposing hook credentials', async () => {
    process.env.DORKA_AGENT_HOOK_ENDPOINT = tempDir
    process.env.DORKA_AGENT_HOOK_TOKEN = 'fallback-secret-token'
  ...
  it('reasserts Busy through the refreshed endpoint after a live text delta', async () => {
    ...
    globalThis.fetch = vi.fn(async (url: RequestInfo | URL, init?: RequestInit) => {
      ...
      deliveries.push({
        url: String(url),
        token: new Headers(init?.headers).get('X-Dorka-Agent-Hook-Token')
      })
      return new Response(null, { status: 204 })
    }) as typeof globalThis.fetch
    const handler = await loadHandler()
    ...
    await handler({ event: status('busy') })
    process.env.DORKA_AGENT_HOOK_PORT = '56789'
    process.env.DORKA_AGENT_HOOK_TOKEN = 'refreshed-token'
    await handler({ event: delta('still working') })
