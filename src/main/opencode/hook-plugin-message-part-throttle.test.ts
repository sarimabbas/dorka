 ...
 * coalesced and capped before POSTing to Dorka's agent-hook server. The
 * un-throttled plugin re-posted the full accumulated reply per streamed
 * append — O(n²) bytes per turn — which saturated Dorka's main + renderer
...
const ENV_KEYS = ['DORKA_PANE_KEY', 'DORKA_AGENT_HOOK_PORT', 'DORKA_AGENT_HOOK_TOKEN'] as const
...
describe('OpenCode plugin MessagePart throttling', () => {
  ...
  beforeEach(() => {
    tempDir = mkdtempSync(join(tmpdir(), 'dorka-opencode-plugin-test-'))
    posts = []
    savedEnv = {}
  ...
  async function loadPluginEventHandler(): Promise<PluginEventHandler> {
    ...
    const module = (await import(pathToFileURL(pluginPath).href)) as {
      DorkaOpenCodeStatusPlugin: (ctx: unknown) => Promise<{ event: PluginEventHandler }>
    }
    const client = {
      session: {
    ...
    const hooks = await module.DorkaOpenCodeStatusPlugin({ client })
    return hooks.event
  }
