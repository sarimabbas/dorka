...
describe('ModelManager Windows model path handling', () => {
  beforeEach(() => {
    ...
    appGetPathMock.mockImplementation(() => join(tmpdir(), 'dorka-speech-models-test'))
  })
  ...
  it('uses an ASCII cache path when the Windows default user data path has non-ASCII characters', () => {
    ...
    try {
      ...
      const userDataDir = join(dir, '用户', 'Dorka')
      ...
      expect(manager.getModelsDir()).toContain(join(programDataDir, 'Dorka', 'speech-models'))
