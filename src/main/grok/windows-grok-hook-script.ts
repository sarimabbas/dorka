...
const WINDOWS_GROK_HOOK_POST_COMMAND = buildWindowsAgentHookPostCommand('grok', [
  ...
  '  --data-urlencode "grokHome=%DORKA_GROK_HOME%" ^'
])
 ...
 * is unset (the default outside an Dorka-managed terminal), length/trailing
...
export function buildWindowsGrokHookScript(): string {
  return [
    ...
    ...buildWindowsHookEnvironmentGuardLines(),
    'set "DORKA_GROK_HOME="',
    'if not defined GROK_HOME goto :dorka_grok_home_ready',
    `if not "%GROK_HOME:~${GROK_HOME_ENVELOPE_MAX_LENGTH},1%"=="" goto :dorka_grok_home_ready`,
    ...
    'set "DORKA_GROK_HOME=%GROK_HOME:"=%"',
    'if not defined DORKA_GROK_HOME goto :dorka_grok_home_ready',
    'if "%DORKA_GROK_HOME:~-1%"=="\\" set "DORKA_GROK_HOME=%DORKA_GROK_HOME%."',
    // Why: the trailing-backslash safety sentinel counts toward the relay envelope.
    `if not "%DORKA_GROK_HOME:~${GROK_HOME_ENVELOPE_MAX_LENGTH},1%"=="" set "DORKA_GROK_HOME="`,
    ':dorka_grok_home_ready',
