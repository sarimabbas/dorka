...
function readInstalledConfig(home: string): InstalledConfig {
  return JSON.parse(
    readFileSync(join(home, '.grok', 'hooks', 'dorka-status.json'), 'utf8')
  ) as InstalledConfig
}
...
describe('Windows Grok managed hook launch shape', () => {
  ...
  describe('registered command, from a synthetic Windows path', () => {
    const scriptPath = 'C:\\Users\\dev\\.dorka\\agent-hooks\\grok-hook.cmd'
    ...
    it('still wraps a path cmd.exe would split or expand (#6078)', () => {
      const spaced = 'C:\\Users\\Jane Doe\\.dorka\\agent-hooks\\grok-hook.cmd'
  ...
  describe('installed config', () => {
    ...
    beforeEach(() => {
      home = mkdtempSync(join(tmpdir(), 'dorka-grok-launcher-'))
    ...
    it.skipIf(process.platform !== 'win32')(
      ...
      () => {
        const scriptPath = join(home, '.dorka', 'agent-hooks', 'grok-hook.cmd')
    ...
    it.skipIf(process.platform !== 'win32')(
      ...
      () => {
        ...
        const configPath = join(home, '.grok', 'hooks', 'dorka-status.json')
        ...
        // Why: sweeping stale Dorka entries must not touch hooks the user wrote.
    ...
    it('falls back to the encoded launcher when the profile path is not cmd-safe (#6078)', () => {
      const spaceHome = mkdtempSync(join(tmpdir(), 'dorka grok spaced '))
