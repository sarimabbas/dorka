...
export function getEffectiveHooksFromConfig(
  ...
  // Why: committed `dorka.yaml` and local Settings can coexist; the source policy decides which is authoritative.
  return {
    scripts: {
      ...(setup ? { setup } : {}),
      ...(archive ? { archive } : {})
...
export function getDefaultTabCommandTrustContent(hooks: DorkaHooks | null): string {
...
export function getDefaultTabsLaunch(
  ...
  // Why: local-only repos may use shared tab titles/colors but must not run the committed dorka.yaml commands.
