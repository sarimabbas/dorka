...
// Why: kept in sync with pty/wsl-dorka-env.ts, which path-translates the same keys for WSL.
// DORKA_WORKSPACE_NAME is a display name and the credential-guard policy is an enum — never paths.
...
export function getSetupEnvVars(repo: Repo, worktreePath: string): Record<string, string> {
  return {
    DORKA_ROOT_PATH: repo.path,
    DORKA_WORKTREE_PATH: worktreePath,
    DORKA_WORKSPACE_NAME: getRuntimePathBasename(worktreePath),
