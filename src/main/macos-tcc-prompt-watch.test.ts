...
describe('parseTccPromptEvent', () => {
  it('parses a real captured AUTHREQ_PROMPTING line', () => {
    expect(parseTccPromptEvent(REAL_PROMPT_LINE)).toEqual({
      ...
      accessingIdentifier: 'com.dorka.tccprobe.shapecapture',
      responsibleIdentifier: 'com.dorka.tccprobe.shapecapture',
  ...
  it('separates the accessing binary from the responsible app', () => {
    const event = parseTccPromptEvent(DORKA_APPDATA_LINE)
    // The whole point of #9756: the dialog says Dorka, but node did the access.
    expect(event?.responsibleIdentifier).toBe('com.stablyai.dorka')
...
describe('isDorkaAttributedPrompt', () => {
  it('accepts the app and detached terminal helper across Dorka build identities', () => {
    for (const id of [
      'com.stablyai.dorka',
      'com.stablyai.dorka.helper',
      'com.stablyai.dorka.dev',
      'com.stablyai.dorka.dev.helper',
      'com.stablyai.dorka.local',
      'com.stablyai.dorka.local.helper'
    ]) {
      expect(
        isDorkaAttributedPrompt({
  ...
  it('rejects unrelated services even when Dorka is responsible', () => {
    expect(
      isDorkaAttributedPrompt({
        service: 'kTCCServiceMicrophone',
        accessingIdentifier: 'dorka',
        responsibleIdentifier: 'com.stablyai.dorka'
      })
    ).toBe(false)
  })
})
...
describe('MacosTccPromptWatch', () => {
  ...
  it('reports only Dorka-attributed dialogs from a live stream', async () => {
    ...
    stdout.write(`${DORKA_APPDATA_LINE}\n`)
