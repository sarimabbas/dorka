...
function makeRepo(): string {
  const root = mkdtempSync(join(tmpdir(), 'dorka-vm-recipe-runner-'))
  tmpRoots.push(root)
  return root
}
...
describe('runEphemeralVmRecipeStart', () => {
    ...
    async ({ checkoutMode, expected }) => {
      ...
      writeFileSync(
        ...
        [
          ...
          '  schemaVersion: Number(process.env.DORKA_RECIPE_RESULT_SCHEMA_VERSION),',
          '  ...(process.env.DORKA_RECIPE_RESULT_SCHEMA_VERSION === "2"',
  ...
  it('runs a recipe from the repo root and parses its JSON result', async () => {
    ...
    writeFileSync(
      ...
      [
        ...
        'console.error(`instance:${process.env.DORKA_VM_INSTANCE_ID}`)',
        'console.log(JSON.stringify({',
        '  schemaVersion: 1,',
        `  pairingCode: ${JSON.stringify(makePairingCode())},`,
        "  projectRoot: '/workspace/repo',",
        '  userData: { providerResourceId: process.env.DORKA_VM_INSTANCE_ID }',
        '}))'
      ].join('\n')
    )
    ...
    if (result.ok) {
      ...
      expect(result.context.instanceId).toMatch(/^dorka-/)
  ...
  it('returns a parse failure when stdout is not the recipe result contract', async () => {
    ...
    const result = await runEphemeralVmRecipeStart({
      ...
      context: { instanceId: 'dorka-test-instance' }
    })
    ...
    expect(result).toMatchObject({
      ...
      signal: null,
      context: {
        instanceId: 'dorka-test-instance',
...
describe('runEphemeralVmRecipeCleanup', () => {
  it('builds a copyable cleanup payload and command', () => {
    const repoPath = makeRepo()
    const recipe: DorkaVmRecipe = {
      id: 'cloud-sandbox',
      name: 'Cloud Sandbox',
      create: 'unused',
      destroy: './scripts/dorka-vm/destroy.sh'
    }
    ...
    ).toContain('| ./scripts/dorka-vm/destroy.sh')
  })
  ...
  it('passes cleanup context and recipe result on stdin', async () => {
    ...
    writeFileSync(
      ...
      [
        ...
        '    envMode: process.env.DORKA_VM_MODE,',
        '    envWorkspace: process.env.DORKA_WORKSPACE_NAME',
        '  }))',
        '})'
      ].join('\n')
    )
  ...
  it('passes suspend context and recipe result on stdin', async () => {
    ...
    writeFileSync(
      ...
      [
        ...
        "process.stdin.on('end', () => {",
        '  const payload = JSON.parse(input)',
        '  console.log(JSON.stringify({ mode: payload.mode, envMode: process.env.DORKA_VM_MODE }))',
        '})'
      ].join('\n')
    )
