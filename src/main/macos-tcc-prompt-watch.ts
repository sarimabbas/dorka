...
/**
 * Counts the macOS TCC consent dialogs that name Dorka as the responsible
 ...
 * Dorka and puts Dorka's name on the dialog, so users read it as Dorka snooping.
 *
...
const DORKA_RESPONSIBLE_IDENTIFIERS = new Set([
  'com.stablyai.dorka',
...
/** True when this dialog is one macOS raised in Dorka's name for a watched file-access service. */
export function isDorkaAttributedPrompt(event: TccPromptEvent): boolean {
  return (
    DORKA_RESPONSIBLE_IDENTIFIERS.has(event.responsibleIdentifier) &&
...
/**
 * Watches for Dorka-attributed TCC dialogs. macOS-only; `start()` is a no-op
...
export class MacosTccPromptWatch {
  ...
  private handleLine(line: string): void {
    ...
    if (!event || !isDorkaAttributedPrompt(event)) {
      return
    }
