...
const DORKA_DOMAIN = 'com.stablyai.dorka'
...
function createHost(overrides: HostOverrides = {}): {
  ...
  const host: PressAndHoldHost = {
    platform: 'darwin',
    resolveBundleIdentifier: () => DORKA_DOMAIN,
    readRecord: () => stored,
    writeRecord: (record) => {
      stored = record
      records.push(record)
...
function terminalRecord(decision: PressAndHoldDecision): PressAndHoldRecord {
  return { version: 1, decision, domain: DORKA_DOMAIN, decidedAt: '2026-01-01T00:00:00.000Z' }
}
...
describe('ensureMacPressAndHoldDefault', () => {
  it('writes the opt-out when the domain has no explicit value', () => {
    ...
    expect(writes).toEqual([{ domain: DORKA_DOMAIN, value: false }])
  ...
  describe('domains we do not own', () => {
    ...
    it('accepts Dorka and its channel-scoped bundles, and nothing else', () => {
      expect(isDorkaPreferencesDomain('com.stablyai.dorka')).toBe(true)
      expect(isDorkaPreferencesDomain('com.stablyai.dorka.dev')).toBe(true)
      expect(isDorkaPreferencesDomain('com.github.Electron')).toBe(false)
      // Why: a prefix test without the dot would accept a lookalike bundle id.
      expect(isDorkaPreferencesDomain('com.stablyai.dorkafake')).toBe(false)
    })
  })
  ...
  describe('failures leave the decision open', () => {
    ...
    it('retries on the next launch after a failed write', () => {
      ...
      expect(retry.writes).toEqual([{ domain: DORKA_DOMAIN, value: false }])
    })
  })
...
describe('readBundleIdentifierFromExecutablePath', () => {
  ...
  function bundleWithPlist(body: string): string {
    const root = mkdtempSync(join(tmpdir(), 'dorka-press-hold-'))
    roots.push(root)
    mkdirSync(join(root, 'Dorka.app', 'Contents', 'MacOS'), { recursive: true })
    writeFileSync(join(root, 'Dorka.app', 'Contents', 'Info.plist'), body)
    return join(root, 'Dorka.app', 'Contents', 'MacOS', 'Dorka')
  }

  it('reads CFBundleIdentifier from the plist beside the executable', () => {
    const exe = bundleWithPlist(
      '<plist><dict>\n<key>CFBundleName</key>\n<string>Dorka</string>\n' +
        '<key>CFBundleIdentifier</key>\n\t<string>com.stablyai.dorka</string>\n</dict></plist>'
    )

    expect(readBundleIdentifierFromExecutablePath(exe)).toBe('com.stablyai.dorka')
  })
...
describe('startup wiring', () => {
  ...
  it('runs before app.whenReady(), which is the last point AppKit could still see it', () => {
    ...
    // Why after initDataPath: the record lives beside dorka-data.json, and the canonical userData
