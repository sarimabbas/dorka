...
describe.runIf(RUN_REAL_WINDOWS)('real Windows skill workspace installation', () => {
  ...
  beforeAll(async () => {
    root = await mkdtemp(join(tmpdir(), 'dorka-windows-skill-integration-'))
    ...
    stateDirectory = join(root, 'dorka state')
    ...
    await git(['config', 'user.name', 'Dorka Test'], repository)
    await git(['config', 'user.email', 'dorka@example.invalid'], repository)
