...
describe('skillUpdateFailedNames', () => {
  it('treats a convergent copy that is now current as landed', () => {
    expect(
      skillUpdateFailedNames(['dorka-cli'], [placement('dorka-cli', 'current')], noLocks)
    ).toEqual([])
  })

  it('reports a copy the run left outdated', () => {
    expect(
      skillUpdateFailedNames(['dorka-cli'], [placement('dorka-cli', 'outdated')], noLocks)
    ).toEqual(['dorka-cli'])
  })
  ...
  it('reports a half-written bundle instead of reading it as success', () => {
    ...
    expect(
      skillUpdateFailedNames(['dorka-cli'], [placement('dorka-cli', 'unrecognized')], noLocks)
    ).toEqual(['dorka-cli'])
  })

  it('reports an unreadable copy', () => {
    expect(
      skillUpdateFailedNames(['dorka-cli'], [placement('dorka-cli', 'inaccessible')], noLocks)
    ).toEqual(['dorka-cli'])
  })

  it('reports a skill the run removed outright', () => {
    expect(skillUpdateFailedNames(['dorka-cli'], [], noLocks)).toEqual(['dorka-cli'])
  })
  ...
  it('accepts a revision newer than this build ships', () => {
    ...
    expect(
      skillUpdateFailedNames(['dorka-cli'], [placement('dorka-cli', 'newer-known')], noLocks)
    ).toEqual([])
  })
  ...
  it('ignores placements the update command never writes to', () => {
    expect(
      skillUpdateFailedNames(
        ['dorka-cli'],
        [placement('dorka-cli', 'current'), placement('dorka-cli', 'outdated', 'plugin-cache')],
        noLocks
      )
    ).toEqual([])
  })
  ...
  it('fails the name when any convergent alias was left behind', () => {
    expect(
      skillUpdateFailedNames(
        ['dorka-cli'],
        [placement('dorka-cli', 'current'), placement('dorka-cli', 'outdated', 'provider-alias')],
        noLocks
      )
    ).toEqual(['dorka-cli'])
  })
  ...
  it('judges each requested name independently', () => {
    expect(
      skillUpdateFailedNames(
        ['dorka-cli', 'orchestration'],
        [placement('dorka-cli', 'current'), placement('orchestration', 'outdated')],
        noLocks
      )
  ...
  it('treats unrecognized content whose tree sha matches the lock as landed', () => {
    ...
    expect(
      skillUpdateFailedNames(
        ['dorka-cli'],
        [placement('dorka-cli', 'unrecognized', 'canonical-copy', 'ahead-of-bundle')],
        new Map([['dorka-cli', 'ahead-of-bundle']])
      )
    ).toEqual([])
  })
  ...
  it('still reports unrecognized content whose bytes do not match the lock', () => {
    expect(
      skillUpdateFailedNames(
        ['dorka-cli'],
        [placement('dorka-cli', 'unrecognized', 'canonical-copy', 'half-written-bytes')],
        new Map([['dorka-cli', 'ahead-of-bundle']])
      )
    ).toEqual(['dorka-cli'])
  })
  ...
  it('never forgives an outdated copy, even at the lock hash', () => {
    ...
    expect(
      skillUpdateFailedNames(
        ['dorka-cli'],
        [placement('dorka-cli', 'outdated', 'canonical-copy', 'locked-revision')],
        new Map([['dorka-cli', 'locked-revision']])
      )
    ).toEqual(['dorka-cli'])
  })
  ...
  it('does not let a lock-matching canonical copy excuse a degraded alias', () => {
    expect(
      skillUpdateFailedNames(
        ...
        [
          placement('dorka-cli', 'unrecognized', 'canonical-copy', 'ahead-of-bundle'),
          placement('dorka-cli', 'inaccessible', 'provider-alias')
        ],
        new Map([['dorka-cli', 'ahead-of-bundle']])
      )
    ).toEqual(['dorka-cli'])
...
describe('skillUpdateFailedNames over a real inventory', () => {
  ...
  async function postCutFixture(): Promise<{ homeDir: string; installedTreeSha: string }> {
    const root = await mkdtemp(join(tmpdir(), 'dorka-skill-outcome-'))
    temporaryDirectories.push(root)
    const homeDir = join(root, 'home')
    const skillDir = join(homeDir, '.agents', 'skills', 'dorka-cli')
    ...
    const current = await readFile(join(repoRoot, 'skills', 'dorka-cli', 'SKILL.md'))
    await writeFile(
  ...
  it('accepts post-cut source content when the lock records exactly those bytes', async () => {
    ...
    const canonical = inventory.installations.filter(
      (entry) => entry.name === 'dorka-cli' && entry.topology === 'canonical-copy'
    )
    expect(canonical).toHaveLength(1)
    expect(canonical[0].status).toBe('newer-known')
    expect(canonical[0].installedReleaseRevision).toBeNull()

    expect(skillUpdateFailedNames(['dorka-cli'], inventory.installations, locks)).toEqual([])
  })
