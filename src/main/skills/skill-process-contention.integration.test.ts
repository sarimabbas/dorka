...
const RUN_REAL_PROCESS = process.env.DORKA_REAL_PROCESS_SKILL_TEST === '1'
...
function startChild(input: {
  ...
  const child = spawn(
    ...
    ],
    {
      ...
      env: {
        ...process.env,
        DORKA_REAL_PROCESS_SKILL_TEST: '0',
        DORKA_SKILL_CONTENTION_CHILD: '1',
        DORKA_SKILL_CONTENTION_ROOT: input.root,
        DORKA_SKILL_CONTENTION_ARCHIVE: input.archivePath,
        DORKA_SKILL_CONTENTION_ROLE: input.role,
        DORKA_SKILL_CONTENTION_READY: input.readyPath,
        DORKA_SKILL_CONTENTION_RELEASE: input.releasePath,
        DORKA_SKILL_CONTENTION_RESULT: input.resultPath
      },
...
describe.runIf(RUN_REAL_PROCESS)('skill multi-process contention', () => {
  it('fails busy without residue, then converges after the owner releases', async () => {
    const root = await mkdtemp(join(tmpdir(), 'dorka-skill-process-contention-'))
    roots.push(root)
