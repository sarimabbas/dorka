...
function userDataPath(): string {
  const path = mkdtempSync(join(tmpdir(), 'dorka-skill-cloud-service-'))
...
describe('SkillCloudService bearer links', () => {
  it('resolves and grants downloads without an Dorka session', async () => {
  ...
  it('uses the development auth token without opening a profile session', async () => {
    vi.stubEnv('DORKA_CLOUD_AUTH_TOKEN', 'desktop-e2e-token')
    const requests: RequestInit[] = []
    vi.stubGlobal(
      'fetch',
  ...
  it('rejects the development auth token in packaged builds', async () => {
    ...
    await expect(
      new SkillCloudService(userDataPath()).listOwnedShares({ apiUrl: 'https://share.ondorka.dev' })
