...
async function interruptedUpdate(phase: SkillInstallJournalV1['phase']): Promise<{
  ...
  const root = await mkdtemp(join(tmpdir(), 'dorka-skill-recovery-test-'))
  roots.push(root)
  ...
  const extractionPath = join(destinationRoot, '.dorka-skill-extract-recovery')
  const stagingPath = join(destinationRoot, '.recovery-skill.dorka-staging-recovery')
  const backupPath = join(destinationRoot, '.recovery-skill.dorka-backup-recovery')
