...
describe.runIf(RUN_REAL_WSL)('real WSL POSIX skill semantics', () => {
  ...
  beforeAll(async () => {
    localRoot = await mkdtemp(join(tmpdir(), 'dorka-wsl-posix-semantics-'))
    guestRoot = await runWsl('mktemp', '-d', '/tmp/dorka-skill-posix.XXXXXX')
    if (!guestRoot.startsWith('/tmp/dorka-skill-posix.')) {
  ...
  it('preserves case and applies owner-private executable modes', async () => {
    ...
    const result = await installSharedSkill({
      ...
      scope: 'global',
      homeDirectory,
      dorkaStateDirectory: join(localRoot, 'state'),
      detectedProviders: [],
  ...
  it('installs only selected bundle skills into the distro-owned home', async () => {
    ...
    const result = await installSkillBundle({
      ...
      scope: 'global',
      homeDirectory,
      dorkaStateDirectory: join(localRoot, 'bundle-state'),
      detectedProviders: [],
