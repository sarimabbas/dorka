...
const PROCESS_DOWNLOAD_ROOT_PREFIX = '.dorka-skill-download-process-'
      ...
      await Promise.all(
        entries.map(async (entry) => {
          const match = entry.isDirectory()
            ? entry.name.match(/^\.dorka-skill-download-process-(\d+)-/)
            : null
...
async function downloadSkillPackageGrantUnobserved(
  ...
  try {
    ...
    const temporaryDirectory = await mkdtemp(join(processRoot, '.dorka-skill-download-'))
