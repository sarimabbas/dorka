...
async function installedFixture(): Promise<{
  ...
  const root = await mkdtemp(join(tmpdir(), 'dorka-skill-remove-recovery-test-'))
  roots.push(root)
...
describe('skill removal recovery', () => {
  it('rejects a journal-owned move outside its recorded provider roots', async () => {
    ...
    const backupPath = join(dirname(sourcePath), `.${basename(sourcePath)}.dorka-remove-backup-x`)
  ...
  it('preserves a moved backup whose bytes changed before recovery', async () => {
    ...
    const backupPath = join(
      ...
      `.${basename(fixture.canonicalPath)}.dorka-remove-backup-x`
    )
