...
describe.runIf(RUN_REAL_WSL)('real WSL skill install transactions', () => {
  ...
  beforeAll(async () => {
    localRoot = await mkdtemp(join(tmpdir(), 'dorka-wsl-skill-integration-'))
    guestRoot = await runWsl('mktemp', '-d', '/tmp/dorka-skill-integration.XXXXXX')
    if (!guestRoot.startsWith('/tmp/dorka-skill-integration.')) {
  ...
  function installInput(
    ...
    return {
      ...
      dorkaStateDirectory: join(localRoot, `state-${scope}`),
  ...
  it('installs, recovers, updates, conflicts, and removes on the guest filesystem', async () => {
    ...
    const removeInput = {
      ...
      homeDirectory,
      dorkaStateDirectory: join(localRoot, 'state-global'),
      detectedProviders: ['codex', 'claude'],
      filesystem
    }
    ...
    expect(
      (
        await removeSharedSkill({
          ...
          homeDirectory,
          workspaceDirectory,
          dorkaStateDirectory: join(localRoot, 'state-workspace'),
          detectedProviders: ['codex', 'claude'],
          filesystem
        })
      ).status
  ...
  it('installs and removes within a Windows-backed WSL workspace', async () => {
    ...
    const input = {
      ...
      workspaceDirectory: windowsWorkspace,
      dorkaStateDirectory: join(localRoot, 'state-drvfs')
    }
