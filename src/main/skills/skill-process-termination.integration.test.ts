  ...
  const child = spawn(
    ...
    ],
    {
      ...
      env: {
        ...
        DORKA_SKILL_PROCESS_CHILD: '1',
        DORKA_SKILL_CRASH_ROOT: root,
        DORKA_SKILL_CRASH_MARKER: marker,
        DORKA_SKILL_CRASH_OPERATION: crashCase.operation,
        DORKA_SKILL_CRASH_PHASE: crashCase.phase,
        DORKA_SKILL_CRASH_BOUNDARY: crashCase.boundary
      },
...
describe.runIf(RUN_REAL_PROCESS)('skill process termination recovery', () => {
  it.each(cases)(
    ...
    async (crashCase) => {
      const root = await mkdtemp(join(tmpdir(), 'dorka-skill-process-crash-'))
      roots.push(root)
