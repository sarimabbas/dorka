...
const globallyUpdatableNames = new Set(['computer-use', 'dorka-cli', 'orchestration'])
...
describe('skill freshness name-scoped update eligibility', () => {
  ...
  it.each([
    ...
    (status, topology) => {
      ...
      expect(
        eligible([
          placement('dorka-cli'),
          placement('dorka-cli', { id: `outlier-${status}-${topology}`, status, topology })
        ])
      ).toEqual(['dorka-cli'])
    }
  )
  ...
  it.each(['unrecognized', 'inaccessible', 'newer-known'] as const)(
    ...
    (status) => {
      ...
      expect(
        eligible([
          placement('dorka-cli', { id: 'blocked-canonical', status }),
          placement('dorka-cli', {
            id: 'dorka-cli-claude',
            rootId: 'home-claude',
            topology: 'provider-alias',
    ...
    expect(
      eligible([
        ...
        placement('dorka-cli', {
          id: 'dorka-cli-gemini',
          rootId: 'home-gemini',
          unresolvedPath: '/home/.gemini/skills/dorka-cli',
          resolvedPath: '/home/.gemini/skills/dorka-cli',
  ...
  it('does not offer an official canonical copy missing from the updater lock (#10791)', () => {
    expect(eligibleSkillUpdateNames([placement('dorka-cli')], new Set())).toEqual([])
  })

  it('builds only an explicit, deterministic global command', () => {
    expect(buildTargetedSkillUpdateCommand(['orchestration', 'dorka-cli', 'dorka-cli'])).toBe(
      'npx skills update dorka-cli orchestration --global'
    )
    expect(buildTargetedSkillUpdateCommand([])).toBeNull()
    expect(buildTargetedSkillUpdateCommand(['dorka-cli;echo unsafe'])).toBeNull()
  })
})
