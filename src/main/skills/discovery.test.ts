...
describe('skill discovery', () => {
  ...
  it('keeps every healthy root when one root walk is aborted', async () => {
    const root = await mkdtemp(join(tmpdir(), 'dorka-skills-'))
  ...
  it('discovers skill packages through symlinked skill directories', async () => {
    const root = await mkdtemp(join(tmpdir(), 'dorka-skills-'))
    const home = join(root, 'home')
    const realSkill = join(root, 'central-skills', 'dorka-cli')
    const linkedSkill = join(home, '.agents', 'skills', 'dorka-cli')
    await mkdir(realSkill, { recursive: true })
    await mkdir(join(home, '.agents', 'skills'), { recursive: true })
    await writeFile(join(realSkill, 'SKILL.md'), '# Dorka CLI\n\nUse the Dorka CLI.')
    ...
    const skill = result.skills.find((entry) => entry.name === 'Dorka CLI')
  ...
  it('keeps home classification when cwd points at the same directory as home', async () => {
    const root = await mkdtemp(join(tmpdir(), 'dorka-skills-'))
    const home = join(root, 'home')
    const skillDir = join(home, '.agents', 'skills', 'dorka-cli')
    await mkdir(skillDir, { recursive: true })
    await writeFile(
      join(skillDir, 'SKILL.md'),
      ['---', 'name: dorka-cli', 'description: Use the Dorka CLI.', '---', ''].join('\n')
    )
    ...
    expect(result.skills.filter((entry) => entry.name === 'dorka-cli')).toMatchObject([
      {
        sourceKind: 'home',
