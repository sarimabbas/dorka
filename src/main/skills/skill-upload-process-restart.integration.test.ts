  ...
  const child = spawn(
    ...
    ],
    {
      ...
      env: {
        ...
        DORKA_SKILL_UPLOAD_PROCESS_CHILD: '1',
        DORKA_SKILL_UPLOAD_RESTART_ROOT: uploadRoot,
        DORKA_SKILL_UPLOAD_RESTART_MARKER: marker,
        DORKA_SKILL_UPLOAD_RESTART_BOUNDARY: boundary
      },
...
describe.runIf(RUN_REAL_PROCESS)('skill upload process restart recovery', () => {
  it.each(['begun', 'partial', 'uploaded', 'committed'])(
    ...
    async (boundary) => {
      const root = await mkdtemp(join(tmpdir(), 'dorka-skill-upload-restart-'))
      roots.push(root)
      const uploadRoot = join(root, 'uploads')
