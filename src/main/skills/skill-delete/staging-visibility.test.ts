...
async function stagedRoot(stagedName: string): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), 'dorka-staging-visibility-'))
  roots.push(root)
...
describe('isSkillStagingEntryName', () => {
  it.each([
    '.demo.dorka-skill-delete-1234',
    '.demo.dorka-remove-backup-1234',
    '.demo.dorka-placement-backup-1234',
    '.demo.dorka-placement-staging-1234'
  ])('matches the %s convention', (name) => {
    expect(isSkillStagingEntryName(name)).toBe(true)
  })

  it.each(['demo', '.hidden-skill', '..cache', '.dorka'])('leaves %s alone', (name) => {
...
describe('native walker', () => {
  it.each([
    ...
    '.demo.dorka-remove-backup-id',
    '.demo.dorka-placement-backup-id'
