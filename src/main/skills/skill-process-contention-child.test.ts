...
const CHILD = process.env.DORKA_SKILL_CONTENTION_CHILD === '1'
...
describe.runIf(CHILD)('skill process contention child', () => {
  it('runs one transaction process', async () => {
    const root = process.env.DORKA_SKILL_CONTENTION_ROOT
    const archivePath = process.env.DORKA_SKILL_CONTENTION_ARCHIVE
    const resultPath = process.env.DORKA_SKILL_CONTENTION_RESULT
    const role = process.env.DORKA_SKILL_CONTENTION_ROLE
    ...
    const result = await installLocalSkillPackage(
      ...
      },
      {
        onJournalTransition: async (phase, boundary) => {
          ...
          const readyPath = process.env.DORKA_SKILL_CONTENTION_READY
          const releasePath = process.env.DORKA_SKILL_CONTENTION_RELEASE
