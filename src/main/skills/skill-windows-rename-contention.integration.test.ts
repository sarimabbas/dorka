...
const RUN_REAL_WINDOWS =
  process.platform === 'win32' && process.env.DORKA_REAL_WINDOWS_SKILL_TEST === '1'
const roots: string[] = []
const lockers: ChildProcessWithoutNullStreams[] = []

const LOCK_SCRIPT = [
  '$stream = [System.IO.File]::Open($env:DORKA_SKILL_LOCK_PATH, [System.IO.FileMode]::Open,',
  '  [System.IO.FileAccess]::Read, [System.IO.FileShare]::None)',
  "[Console]::Out.WriteLine('LOCKED')",
  'Start-Sleep -Milliseconds ([int]$env:DORKA_SKILL_LOCK_DURATION_MS)',
  '$stream.Dispose()'
].join('\n')
...
function holdFile(path: string, durationMs: number) {
  const child = spawn(
    ...
    ['-NoProfile', '-NonInteractive', '-Command', LOCK_SCRIPT],
    {
      ...
      env: {
        ...process.env,
        DORKA_SKILL_LOCK_PATH: path,
        DORKA_SKILL_LOCK_DURATION_MS: String(durationMs)
      }
    }
  )
  lockers.push(child)
...
async function fixture(label: string) {
  const root = await mkdtemp(join(tmpdir(), `dorka-windows-rename-${label}-`))
  roots.push(root)
