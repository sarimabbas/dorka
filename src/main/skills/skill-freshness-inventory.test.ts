...
// Why: hashed with real git, not Dorka's tree-sha port — the port validating
...
async function gitTreeShaOf(directory: string): Promise<string> {
  const gitDir = await mkdtemp(join(tmpdir(), 'dorka-skill-hash-'))
...
async function fixture() {
  const root = await mkdtemp(join(tmpdir(), 'dorka-skill-inventory-'))
  ...
  await Promise.all([
        ...
        `${JSON.stringify({
          ...
          skills: {
            'dorka-cli': {
              ...
              skillPath: 'skills/dorka-cli/SKILL.md',
    ...
    writeFile(
      ...
      `${JSON.stringify({ schemaVersion: 1, skills: { 'dorka-cli': snapshots } }, null, 2)}\n`
    ),
    writeFile(
      ...
      `${JSON.stringify(
        {
          ...
          releases: [
            ...
            { appVersion: '2.0.0', skills: { 'dorka-cli': 2 } },
            { appVersion: '3.0.0', skills: { 'dorka-cli': 3 } }
          ]
        },
        null,
        2
  ...
  const writeSkill = async (rootPath: string, markdown: string): Promise<string> => {
    const directory = join(rootPath, 'dorka-cli')
...
describe('read-only skill freshness inventory', () => {
  it('offers an exact older official name only when all global placements are safe', async () => {
    ...
    expect(inventory.eligibleUpdateNames).toEqual(['dorka-cli'])
  })
  ...
  it('trusts the updater lock for canonical bytes the bundle has never seen (#11220 scan half)', async () => {
    ...
    expect(getSkillFreshnessDisplayStatus(inventory, 'dorka-cli')).toBe('up-to-date')
  })
  ...
  it('scopes the lock hash to the current bundle, not every path a revision ever shipped', async () => {
    ...
    registry.skills['dorka-cli'][0].files.push(legacy)
  ...
  it('still flags canonical bytes that do not match what the lock says was installed', async () => {
    ...
    expect(getSkillFreshnessDisplayStatus(inventory, 'dorka-cli')).toBe('needs-attention')
  })
  ...
  it.runIf(process.platform !== 'win32')(
    ...
    async () => {
      ...
      await symlink(canonical, join(claudeRoot, 'dorka-cli'))
    ...
    async () => {
      ...
      const repos = await Promise.all(
        ['one', 'two'].map(async (id) => {
          ...
          await symlink(shared, join(root, 'dorka-cli'))
  ...
  it('keeps an unreadable foreign-home placement visible without withholding the update', async () => {
    const test = await fixture()
    await test.writeSkill(join(test.homeDir, '.agents', 'skills'), test.oldMarkdown)
    const inaccessiblePath = join(test.homeDir, '.codex', 'skills', 'dorka-cli')
  ...
  it('does not lose an inaccessible known repository placement', async () => {
    ...
    const inaccessiblePath = join(repoPath, '.agents', 'skills', 'dorka-cli')
  ...
  it('reads a plugin-cache copy with untouched official files as current', async () => {
    ...
    // of an edit, so the bytes Dorka owns decide alone — here and in every scope, not just
    ...
    await writeFile(join(withSidecarRoot, 'README.md'), 'Neighbouring file Dorka never shipped\n')
  ...
  it('does not classify an empty plugin-cache directory as a skill', async () => {
    const test = await fixture()
    await test.writeSkill(join(test.homeDir, '.agents', 'skills'), test.currentMarkdown)
    const emptyRoot = join(test.homeDir, '.codex', 'plugins', 'cache', 'vendor', 'dorka-cli')
  ...
  it('classifies exact current bytes as current when a later snapshot reuses them', async () => {
    ...
    const registry = JSON.parse(await readFile(registryPath, 'utf8'))
    registry.skills['dorka-cli'].push(snapshot(4, test.currentMarkdown))
    await writeFile(registryPath, `${JSON.stringify(registry, null, 2)}\n`)
    await test.writeSkill(join(test.homeDir, '.agents', 'skills'), test.currentMarkdown)
