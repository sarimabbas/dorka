...
import menuBarIconPath from '../../../resources/tray/dorka-menu-barTemplate.png?asset&asarUnpack'
import menuBarIconRetinaPath from '../../../resources/tray/dorka-menu-barTemplate@2x.png?asset&asarUnpack'
...
export type SystemTrayOptions = {
  ...
  /** Quit Dorka for real (caller must set the quitting latch before quitting). */
  onQuit: () => void
}
...
function baseTooltip(): string {
  ...
  return devIndicator.label ? `Dorka DEV (${devIndicator.label})` : 'Dorka DEV'
}
      ...
      try {
        ...
        tray.setToolTip(
          devIndicator
            ...
            : translateMain('tray.activityWaiting', 'Dorka - activity waiting')
        )
        return
      } catch (error) {
...
export function createSystemTray(opts: SystemTrayOptions): Tray | null {
  ...
  const menu = Menu.buildFromTemplate([
    ...(devIndicator
      ...
      : []),
    {
      label: translateMain('tray.openDorka', 'Open Dorka'),
