...
const PROMPT_MARK = 'DORKAHIST> '
const WORKTREE_HASH = 'deadbeefdeadbeef'
const MARKER = 'echo dorka-worktree-scoped-history'
...
describe('fish keeps per-worktree history under the session Dorka names', () => {
  ...
  itWithFish(
    ...
    async () => {
      ...
      home = mkdtempSync(path.join(tmpdir(), 'dorka-fish-history-'))
      ...
      term.write(`${MARKER}\r`)
      expect(await waitUntil(() => rendered.includes('dorka-worktree-scoped-history'), 5_000)).toBe(
        true
      )
