...
describe('quoteForPosixShell', () => {
  it('wraps plain values in single quotes', async () => {
    ...
    expect(quoteForPosixShell('/tmp/dorka.deb')).toBe("'/tmp/dorka.deb'")
  })

  it('quotes spaces', async () => {
    const { quoteForPosixShell } = await loadCommandModule()
    expect(quoteForPosixShell('/tmp/Dorka Setup.deb')).toBe("'/tmp/Dorka Setup.deb'")
  })
...
describe('buildLinuxPackageInstallCommand', () => {
  it('refuses a non-absolute package path before looking for anything', async () => {
    ...
    for (const packagePath of ['dorka.deb', './dorka.deb', '--force-all', '-i']) {
  ...
  it('reports no-sudo when sudo is absent', async () => {
    install('/usr/bin/apt')
    const { buildLinuxPackageInstallCommand } = await loadCommandModule()
    expect(buildLinuxPackageInstallCommand('deb', '/tmp/dorka.deb')).toEqual({
      ok: false,
      reason: 'no-sudo'
    })
  })
  ...
  it('builds an apt command', async () => {
    ...
    expect(buildLinuxPackageInstallCommand('deb', '/tmp/dorka.deb')).toEqual({
      ok: true,
      command: "/usr/bin/sudo /usr/bin/apt install -- '/tmp/dorka.deb'"
    })
  })
  ...
  it('falls back to dpkg', async () => {
    ...
    expect(buildLinuxPackageInstallCommand('deb', '/tmp/dorka.deb')).toEqual({
      ok: true,
      command: "/usr/bin/sudo /usr/bin/dpkg -i -- '/tmp/dorka.deb'"
    })
  })
  ...
  it('builds a zypper command', async () => {
    ...
    install('/usr/bin/rpm')
    const { buildLinuxPackageInstallCommand } = await loadCommandModule()
    expect(buildLinuxPackageInstallCommand('rpm', '/tmp/dorka.rpm')).toEqual({
      ok: true,
      command:
        "/usr/bin/sudo /usr/bin/zypper --no-refresh install --allow-unsigned-rpm -f '/tmp/dorka.rpm'"
    })
  })
  ...
  it('prefers dnf over yum and rpm', async () => {
    ...
    expect(buildLinuxPackageInstallCommand('rpm', '/tmp/dorka.rpm')).toEqual({
      ok: true,
      command: "/usr/bin/sudo /usr/bin/dnf install --nogpgcheck '/tmp/dorka.rpm'"
    })
  })
  ...
  it('falls back to yum', async () => {
    ...
    expect(buildLinuxPackageInstallCommand('rpm', '/tmp/dorka.rpm')).toEqual({
      ok: true,
      command: "/usr/bin/sudo /usr/bin/yum install --nogpgcheck '/tmp/dorka.rpm'"
    })
  })
  ...
  it('falls back to rpm', async () => {
    ...
    expect(buildLinuxPackageInstallCommand('rpm', '/tmp/dorka.rpm')).toEqual({
      ok: true,
      command: "/usr/bin/sudo /sbin/rpm -Uvh '/tmp/dorka.rpm'"
    })
  })
  ...
  it('never adds an unattended-confirmation flag', async () => {
    ...
    install('/usr/bin/apt')
    const { buildLinuxPackageInstallCommand } = await loadCommandModule()
    const result = buildLinuxPackageInstallCommand('deb', '/tmp/dorka.deb')
