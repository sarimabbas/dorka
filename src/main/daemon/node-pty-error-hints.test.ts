...
const PTY_ALLOCATION_HINT = [
  ...
  '',
  'Dorka requires a pty device to launch a new terminal. This error is usually due to having too many terminal windows or terminal sessions open, either in Dorka or another program.',
  '',
...
describe('node-pty diagnostic error hints', () => {
  it('parses the native step and errno without dropping the original message', () => {
    ...
    expect(addNodePtyRecoveryHint(message)).toBe(
      `Daemon's node-pty install is gone (worktree deleted?). Restart Dorka. ${message}`
    )
  })
  ...
  it('hints local wrapped spawn errors from pty allocation failures', () => {
    const message =
      'Failed to spawn shell "/bin/zsh": node-pty: open_slave failed: EMFILE (errno 24, Too many open files) - slave=\'/dev/ttys003\' (shell: /bin/zsh, cwd: /tmp, arch: arm64, platform: darwin 25.0.0, dorka: 1.4.178). If this persists, please file an issue.'
