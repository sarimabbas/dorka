...
const DORKA_USER_DATA_PATH_ENV = 'DORKA_USER_DATA_PATH'

function getShellReadyWrapperBaseDir(): string {
  const userDataPath = process.env[DORKA_USER_DATA_PATH_ENV]
  ...
  // DORKA_USER_DATA_PATH, and daemon startup must not fail before the parent can
  ...
  return join(userDataPath || tmpdir(), userDataPath ? 'shell-wrappers' : 'dorka-shell-wrappers')
}
...
// DORKA_USER_DATA_PATH is ever re-pointed mid-process.
...
export function resolvePtyShellPath(env: Record<string, string>): string {
  if (process.platform === 'win32') {
    return env.DORKA_TERMINAL_WINDOWS_SHELL || 'powershell.exe'
  }
