...
describe('createPtySubprocess', () => {
  ...
  it('derives shell-ready launch config from the resolved fallback shell', async () => {
    ...
    const platform = Object.getOwnPropertyDescriptor(process, 'platform')
    const previousShell = process.env.SHELL
    const previousFeatures = process.env.DORKA_SHELL_FEATURES
    const previousZdotdir = process.env.ZDOTDIR
    const warn = vi.spyOn(console, 'warn').mockImplementation(() => {})
    Object.defineProperty(process, 'platform', { configurable: true, value: 'linux' })
    delete process.env.SHELL
    // Why: the test runner itself can execute inside an Dorka-wrapped shell
    ...
    try {
      ...
      // ZDOTDIR and DORKA_SHELL_FEATURES; /bin/sh must spawn without them.
      expect(spawnOptions.env.ZDOTDIR).toBeUndefined()
      expect(spawnOptions.env.DORKA_SHELL_FEATURES).toBeUndefined()
    ...
    try {
      await expect(
        createPtySubprocess({
          ...
          cols: 80,
          rows: 24,
          cwd: '/definitely-missing-dorka-cwd'
        })
      ).rejects.toThrow(/definitely-missing-dorka-cwd/)
    } finally {
      if (platform) {
        Object.defineProperty(process, 'platform', platform)
      }
  ...
  it('combines HOMEDRIVE and HOMEPATH for Windows default cwd', async () => {
    ...
    process.env.HOMEPATH = '\\Users\\dorka'
    ...
    expect(spawnMock).toHaveBeenCalledWith(
      expect.any(String),
      expect.any(Array),
      expect.objectContaining({ cwd: 'D:\\Users\\dorka' })
    )
  })
})
