...
function daemonEnvironmentDiagSuffix(): string {
  const dorka = process.env.DORKA_APP_VERSION?.trim() || '0.0.0-dev'
  const systemVersion =
    (process as NodeJS.Process & { getSystemVersion?: () => string }).getSystemVersion?.() ||
    release()
  return ` (dorka: ${dorka}, arch: ${process.arch}, platform: ${process.platform} ${systemVersion})`
}
...
function formatMissingDaemonPathError(kind: 'helper' | 'cwd', path: string): DaemonProtocolError {
  ...
  return new DaemonProtocolError(
    `Daemon's ${missingTarget} is gone (worktree deleted?). Restart Dorka. node-pty: ${step} failed: ENOENT (errno 2, No such file or directory) - ${detailName}='${path}'${daemonEnvironmentDiagSuffix()}`
  )
}
...
function repairDaemonCwd(): string | null {
  const candidates = [process.env.DORKA_USER_DATA_PATH]
  try {
...
export function runPtySpawnHealthProbe(): Promise<void> {
  const cwd = isExistingDirectory(process.env.DORKA_USER_DATA_PATH)
