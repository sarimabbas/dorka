...
describe('createPtySubprocess', () => {
  ...
  it('uses shell wrapper when managed env must survive shell startup', async () => {
    ...
    try {
      await createPtySubprocess({
        ...
        env: {
          SHELL: '/bin/zsh',
          DORKA_OPENCODE_CONFIG_DIR: '/tmp/dorka-opencode-config'
        }
      })
    } finally {
      if (platform) {
    ...
    expect(lastCall[2].env.DORKA_SHELL_FEATURES).not.toContain('ready')
  })
  ...
  it('uses shell wrapper when OpenCode config must survive shell startup', async () => {
    ...
    try {
      await createPtySubprocess({
        ...
        env: {
          SHELL: '/bin/zsh',
          OPENCODE_CONFIG_DIR: '/tmp/dorka-opencode-overlay',
  ...
  it('uses shell wrapper when MiMo home must survive shell startup', async () => {
    ...
    try {
      await createPtySubprocess({
        ...
        env: {
          SHELL: '/bin/zsh',
          MIMOCODE_HOME: '/tmp/dorka-mimocode-overlay',
  ...
  it('uses shell wrapper when typed OMP commands need the status extension', async () => {
    ...
    try {
      await createPtySubprocess({
        ...
        env: {
          SHELL: '/bin/zsh',
          DORKA_OMP_STATUS_EXTENSION: '/tmp/.omp/agent/extensions/dorka-agent-status.ts'
        }
      })
    } finally {
      if (platform) {
  ...
  it('uses shell wrapper when Codex home must survive shell startup', async () => {
    ...
    try {
      await createPtySubprocess({
        ...
        env: {
          SHELL: '/bin/zsh',
          CODEX_HOME: '/tmp/dorka-codex-home',
          DORKA_CODEX_HOME: '/tmp/dorka-codex-home'
        }
      })
    } finally {
      if (platform) {
    ...
    try {
      await createPtySubprocess({
        ...
        env: {
          ...
          DORKA_AGENT_TEAMS_SHIM_DIR: '/tmp/dorka-agent-teams-bin'
        }
      })
    } finally {
      if (platform) {
  ...
  it('enables readiness and shell identity for plain Codex startup', async () => {
    ...
    expect(lastCall[1]).toEqual(['-l'])
    expect(lastCall[2].env.ZDOTDIR).toMatch(ZSH_SHELL_READY_DIR)
    expect(lastCall[2].env.DORKA_SHELL_FEATURES).toContain('ready')
    expect(lastCall[2].env.DORKA_SHELL_FEATURES).toContain('identity')
  })
  ...
  it('deletes daemon-owned Codex overlay pairs when the private marker is requested', async () => {
    ...
    process.env.DORKA_CODEX_HOME = '/daemon/managed/codex-home'
  ...
  it('strips an inherited per-account self-contained CODEX_HOME overlay in a nested Dorka (#5370)', async () => {
    const proc = mockPtyProcess()
    spawnMock.mockReturnValue(proc)
    const previousCodexHome = process.env.CODEX_HOME
    const previousDorkaCodexHome = process.env.DORKA_CODEX_HOME
    // A per-account home is injected as CODEX_HOME === DORKA_CODEX_HOME, so the
    // nested-Dorka strip must clear it exactly as it does the shared mirror.
    const perAccountHome = '/daemon/managed/codex-accounts/019f0000-aaaa/home'
    process.env.CODEX_HOME = perAccountHome
    process.env.DORKA_CODEX_HOME = perAccountHome
  ...
  it('preserves a daemon-owned custom Codex home while deleting a stale private marker', async () => {
    ...
    process.env.DORKA_CODEX_HOME = '/daemon/stale/managed-home'
