...
export function getDaemonEntryPath(): string {
  ...
  // Why asar and not isPackaged: dorkad is a packaged non-Electron host whose bundle root holds
  // dorkad.js and daemon-entry.js side by side with no asar to redirect (see parcel-watcher-entry-path.ts).
...
// Why: pass a log-file arg so field failures are diagnosable, but honor the DORKA_DIAGNOSTICS_DISABLED privacy switch.
export function daemonLogArgs(): string[] {
  const disabled = (process.env.DORKA_DIAGNOSTICS_DISABLED ?? '').trim().toLowerCase()
