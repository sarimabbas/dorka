/**
 * #15192, activation-order hypothesis: "the Dorka unicode provider is not reached
 ...
 * two cells under xterm's Unicode 6 tables, its Unicode 11 tables, and Dorka's
...
const DORKA_UNICODE_VERSION = 'dorka-11-zwj'
...
function openWithUnicode11AddonLoaded(): {
  ...
  activateDorkaTerminalUnicodeProvider(terminal as never)
  const unicode = (
...
describe('Hangul cell width agreement (#15192)', () => {
  it('reaches the Dorka provider, not the v11 fallback, on a live terminal', () => {
    const { terminal, unicode } = openWithUnicode11AddonLoaded()
    expect(unicode.versions).toContain(DORKA_UNICODE_VERSION)
    expect(unicode.activeVersion).toBe(DORKA_UNICODE_VERSION)
    terminal.dispose()
  })

  it('budgets every precomposed syllable at two cells under v6, v11 and Dorka', () => {
    const { terminal, unicode } = openWithUnicode11AddonLoaded()
    const disagreeing: { codepoint: number; detail: string }[] = []
    for (const version of ['6', '11', DORKA_UNICODE_VERSION]) {
