...
// `app`, so dorkad's daemon launch path can resolve without Electron in the graph.
const hostApp = {
  isPackaged: true,
  userDataPath: '',
  appPath: '',
...
function buildInstallFixture(root: string): void {
  ...
  writeFileSync(join(root, 'Dorka.exe'), 'exe-bytes')
...
beforeEach(() => {
  ...
  setProcessProp('execPath', join(installDir, 'Dorka.exe'))
...
describe('buildDaemonHostManifest', () => {
  it('mirrors the win-unpacked layout: exe + data blobs + resources tree, no GPU DLLs', () => {
    ...
    expect(byDest.get('Dorka.exe')?.kind).toBe('file')
    const exeOp = ops.find((op) => op.sourcePath === 'C:\\app\\Dorka.exe')
    expect(exeOp?.destRel).toBe('Dorka.exe')
...
describe('materializeRelocatedDaemonHost', () => {
  it('copies the tree, writes the marker, and returns mirrored fork paths', () => {
    ...
    const dest = join(localAppDataDir, 'Dorka', 'daemon-host', '9.9.9')
    expect(result?.execPath).toBe(join(dest, 'Dorka.exe'))
  ...
  it('copies the exe verbatim: same file name and same bytes as the install-dir exe', () => {
    const result = materializeRelocatedDaemonHost()
    const sourceExe = join(installDir, 'Dorka.exe')
  ...
  it('tracks a differently-named app exe rather than pinning an image name of its own', () => {
    ...
    renameSync(join(installDir, 'Dorka.exe'), join(installDir, 'Dorka Nightly.exe'))
    setProcessProp('execPath', join(installDir, 'Dorka Nightly.exe'))
    const result = materializeRelocatedDaemonHost()
    const dest = join(localAppDataDir, 'Dorka', 'daemon-host', '9.9.9')
    expect(result?.execPath).toBe(join(dest, 'Dorka Nightly.exe'))
    expect(existsSync(join(dest, 'dorka-terminal-daemon.exe'))).toBe(false)
    // Re-resolution must agree with materialization or the fork would target a missing exe.
    expect(getRelocatedDaemonHost()?.execPath).toBe(join(dest, 'Dorka Nightly.exe'))
  })
  ...
  it('fails open on a missing required input, leaving no dest or staging dir', () => {
    ...
    const hostRoot = join(localAppDataDir, 'Dorka', 'daemon-host')
  ...
  it('returns null off win32', () => {
    ...
    expect(existsSync(join(localAppDataDir, 'Dorka', 'daemon-host'))).toBe(false)
  })

  it('does nothing for a packaged host with no asar root (dorkad on win32)', () => {
    // dorkad answers isPackaged() true — it is a shipped build — but it is plain Node: no
...
describe('getRelocatedDaemonHost', () => {
  it('returns null when the marker version does not match the current version', () => {
    ...
    writeFileSync(join(dest, 'Dorka.exe'), 'exe')
...
describe('pruneOldDaemonHosts', () => {
  it('removes unpinned non-current version dirs, keeping current and pinned', () => {
    const root = join(localAppDataDir, 'Dorka', 'daemon-host')
  ...
  it('reclaims nothing for a packaged host with no asar root (dorkad on win32)', () => {
    const root = join(localAppDataDir, 'Dorka', 'daemon-host')
    mkdirSync(join(root, '1.0.0'), { recursive: true })
    hostApp.appPath = join(installDir, 'resources', 'app')
    installHostApp()
