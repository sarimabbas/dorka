...
describe('createPtySubprocess', () => {
  ...
  it('ignores the PowerShell implementation setting for cmd.exe on Windows', async () => {
    ...
    try {
      await createPtySubprocess({
        ...
        terminalWindowsPowerShellImplementation: 'pwsh.exe',
        env: { DORKA_CODEX_LAUNCH_PREFLIGHT: CODEX_LAUNCH_PREFLIGHT }
      })
    } finally {
      if (platform) {
        Object.defineProperty(process, 'platform', platform)
    ...
    expect(spawnMock).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        env: expect.objectContaining({ DORKA_CODEX_LAUNCH_PREFLIGHT_CMD_QUOTE: '"' })
      })
    )
  })
    ...
    try {
      await expect(
        createPtySubprocess({
          ...
          cols: 80,
          rows: 24,
          cwd: 'C:\\definitely-missing-dorka-cwd',
          shellOverride: 'powershell.exe'
        })
      ).rejects.toThrow(/Working directory "C:\\definitely-missing-dorka-cwd" does not exist/)
    } finally {
      if (platform) {
        Object.defineProperty(process, 'platform', platform)
      }
  ...
  it('adds shell and cwd context when node-pty reports File not found on Windows', async () => {
    ...
    const previousVersion = process.env.DORKA_APP_VERSION
    process.env.DORKA_APP_VERSION = '1.4.178-test'
    ...
    try {
      await expect(
        ...
        /Daemon failed to spawn shell "not-a-real-shell\.exe" with cwd ".+": File not found:.*dorka: 1\.4\.178-test/
      )
    } finally {
      if (previousVersion === undefined) {
        delete process.env.DORKA_APP_VERSION
      } else {
        process.env.DORKA_APP_VERSION = previousVersion
      }
      if (platform) {
        Object.defineProperty(process, 'platform', platform)
      }
