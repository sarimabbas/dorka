...
describePosix('daemon shell-ready bash wrapper', () => {
  ...
  it('emits OSC 133 C/D markers in the daemon bash wrapper', async () => {
    ...
    expect(bashRc).toContain('__dorka_append_prompt_command "__dorka_osc133_epilogue"')
    ...
    expect(bashRc.lastIndexOf("trap '__dorka_osc133_preexec' DEBUG")).toBeGreaterThan(
      bashRc.indexOf('__dorka_append_prompt_command "__dorka_osc133_epilogue"')
    )
  ...
  itWithBash(
    ...
    async () => {
      const { getDaemonBashShellReadyRcfileContent } = await importFreshDaemonBashRcfile()
      // Minimal bash-preexec imitation: re-arms its own DEBUG trap from PROMPT_COMMAND at first prompt, silencing Dorka's trap.
      writeFileSync(
        join(userDataPath, '.bash_profile'),
        [
          'preexec_functions=()',
  ...
  itWithBash(
    'dispatches a non-empty preexec_functions against the real command, not Dorka hooks',
    async () => {
      const { getDaemonBashShellReadyRcfileContent } = await importFreshDaemonBashRcfile()
      ...
      expect(output).not.toContain('USER_PREEXEC:__dorka_osc133')
