...
describe('createPtySubprocess', () => {
  ...
  it('expands variables in PATH before spawning a Windows shell', async () => {
    ...
    expect(spawnMock.mock.calls.at(-1)?.[2].env.PATH).toBe(
      'C:\\Users\\dorka\\AppData\\Local\\agy\\bin;C:\\Windows'
    )
  })

  it('does not inherit parent Dorka pane identity when caller omits pane env', async () => {
    const proc = mockPtyProcess()
    spawnMock.mockReturnValue(proc)
    const saved = {
      DORKA_PANE_KEY: process.env.DORKA_PANE_KEY,
      DORKA_TAB_ID: process.env.DORKA_TAB_ID,
      DORKA_WORKTREE_ID: process.env.DORKA_WORKTREE_ID
    }
    process.env.DORKA_PANE_KEY = 'parent-tab:parent-leaf'
    process.env.DORKA_TAB_ID = 'parent-tab'
    process.env.DORKA_WORKTREE_ID = 'parent-worktree'
  ...
  it('preserves explicit child Dorka pane identity over parent env', async () => {
    const proc = mockPtyProcess()
    spawnMock.mockReturnValue(proc)
    const saved = {
      DORKA_PANE_KEY: process.env.DORKA_PANE_KEY,
    ...
    try {
      await createPtySubprocess({
        ...
        rows: 24,
        env: {
          DORKA_PANE_KEY: 'child-tab:child-leaf',
          DORKA_TAB_ID: 'child-tab',
          DORKA_WORKTREE_ID: 'child-worktree'
        }
      })
    } finally {
      for (const [key, value] of Object.entries(saved)) {
    ...
    const env = spawnMock.mock.calls.at(-1)?.[2].env
    expect(env.DORKA_PANE_KEY).toBe('child-tab:child-leaf')
    expect(env.DORKA_TAB_ID).toBe('child-tab')
    expect(env.DORKA_WORKTREE_ID).toBe('child-worktree')
  })
  ...
  it.each([
    ...
    ['drops an inherited Dorka fish_history', undefined, undefined],
    ['keeps the session this spawn injected', 'dorka_c0ffee', 'dorka_c0ffee'],
    ['keeps a caller-supplied value', 'mine', 'mine']
  ])('%s', async (_name, requested, expected) => {
    spawnMock.mockReturnValue(mockPtyProcess())
    const saved = process.env.fish_history
    process.env.fish_history = 'dorka_abc123'
  ...
  it.each([
    // HISTFILE is exported too, so a daemon started from an Dorka pane would hand
    // every session the launching worktree's history file.
    ['drops an inherited Dorka HISTFILE', undefined, undefined],
    [
  ...
  it.each([
    // DORKA_HISTFILE is exported into every pane, so a daemon started from an
    // Dorka pane inherits one. Left in place it BOTH re-scopes the pane to
    ...
    ['drops an inherited Dorka DORKA_HISTFILE', undefined, undefined],
    [
      'keeps the path this spawn injected',
      '/fake/userData/terminal-history/00112233445566aa/zsh_history',
      '/fake/userData/terminal-history/00112233445566aa/zsh_history'
    ...
    spawnMock.mockReturnValue(mockPtyProcess())
    const saved = process.env.DORKA_HISTFILE
    process.env.DORKA_HISTFILE = '/fake/userData/terminal-history/aabbccddeeff0011/zsh_history'
    ...
    try {
      await createPtySubprocess({
        ...
        ...(requested === undefined ? {} : { env: { DORKA_HISTFILE: requested } })
      })
    } finally {
      if (saved === undefined) {
        delete process.env.DORKA_HISTFILE
      } else {
        process.env.DORKA_HISTFILE = saved
      }
    }

    const env = spawnMock.mock.calls.at(-1)?.[2].env
    expect(env.DORKA_HISTFILE).toBe(expected)
    // The wrapping consequence: no inherited value may point a pane at Dorka's
    // ZDOTDIR that the client scoped no history for.
    expect(env.DORKA_SHELL_FEATURES).toBe(expected === undefined ? undefined : 'history')
  })
  ...
  it('does not inherit legacy attribution state from a pre-upgrade daemon', async () => {
    ...
    process.env.PATH = `/tmp/dorka-terminal-attribution/posix${delimiter}/usr/bin`
  ...
  it('does not inherit NODE_ENV from the daemon process env', async () => {
    // Why: a dev-mode Dorka forks the daemon with NODE_ENV=development; leaking
    // Dorka's build mode into user shells breaks `next build` and Vitest.
    const proc = mockPtyProcess()
    spawnMock.mockReturnValue(proc)
  ...
  it('does not inherit AppImage runtime env into daemon PTY shells', async () => {
    ...
    process.env.APPIMAGE = '/data/apps/dorka.appimage'
    process.env.APPDIR = '/tmp/.mount_dorka123'
    process.env.ARGV0 = '/data/apps/dorka.appimage'
    process.env.OWD = '/home/user/project'
    process.env.APPIMAGE_LIBRARY_PATH = '/tmp/.mount_dorka123/usr/lib'
    process.env.PATH = ['/tmp/.mount_dorka123', '/tmp/.mount_dorka123/usr/sbin', '/usr/bin'].join(
      delimiter
    )
    process.env.LD_LIBRARY_PATH = ['/tmp/.mount_dorka123/usr/lib', '/opt/audio/lib'].join(delimiter)
  ...
  it('does not inherit parent agent hook endpoint for development hook env', async () => {
    const proc = mockPtyProcess()
    spawnMock.mockReturnValue(proc)
    const previousEndpoint = process.env.DORKA_AGENT_HOOK_ENDPOINT
    process.env.DORKA_AGENT_HOOK_ENDPOINT = '/tmp/stale-endpoint.env'
    ...
    try {
      await createPtySubprocess({
        ...
        rows: 24,
        env: {
          DORKA_AGENT_HOOK_ENV: 'development',
      ...
      if (previousEndpoint === undefined) {
        ...
        process.env.DORKA_AGENT_HOOK_ENDPOINT = previousEndpoint
      }
    }

    const env = spawnMock.mock.calls.at(-1)?.[2].env
    expect(env.DORKA_AGENT_HOOK_ENDPOINT).toBeUndefined()
    expect(env.DORKA_AGENT_HOOK_ENV).toBe('development')
    expect(env.DORKA_AGENT_HOOK_PORT).toBe('1234')
    expect(env.DORKA_AGENT_HOOK_TOKEN).toBe('token')
  })
  ...
  it('preserves explicit development agent hook endpoint files', async () => {
    ...
    try {
      await createPtySubprocess({
        ...
        env: {
          ...
          DORKA_AGENT_HOOK_TOKEN: 'token',
          DORKA_AGENT_HOOK_VERSION: '1',
          DORKA_AGENT_HOOK_ENDPOINT: '/tmp/fresh-endpoint.env'
        }
      })
    } finally {
      if (previousEndpoint === undefined) {
    ...
    const env = spawnMock.mock.calls.at(-1)?.[2].env
    expect(env.DORKA_AGENT_HOOK_ENDPOINT).toBe('/tmp/fresh-endpoint.env')
    expect(env.DORKA_AGENT_HOOK_ENV).toBe('development')
    expect(env.DORKA_AGENT_HOOK_PORT).toBe('1234')
    expect(env.DORKA_AGENT_HOOK_TOKEN).toBe('token')
  })
  ...
  it('honors explicit terminal env overrides after deleting requested defaults', async () => {
    ...
    await createPtySubprocess({
      ...
      env: {
        ...
        PATH: '/tmp/dorka-agent-teams-bin:/usr/bin',
        DORKA_AGENT_TEAMS_TEAM_ID: 'team-test'
      },
    ...
    expect(lastCall[2].env.PATH.split(':')[0]).toBe('/tmp/dorka-agent-teams-bin')
  ...
  it('collapses its own env merge onto the requested Windows `Path` spelling', async () => {
    ...
    expect(env.Path.split(':')[0]).toBe('/tmp/dorka-agent-teams-bin')
  })
