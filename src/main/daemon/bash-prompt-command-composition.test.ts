...
function runInteractiveBash(
  ...
  const result = spawnSync(
    ...
    ['-lc', '"$BASH" --noprofile --rcfile "$1" -i 2>&1', 'bash', rcfile],
    {
      input,
      encoding: 'utf8',
      env: { ...process.env, HOME: tempHome, DORKA_SHELL_FEATURES: 'ready', TERM: 'xterm' },
      timeout: 5000
    }
  )
...
`
  ...
  itWithBash('keeps a scalar ending in an odd backslash isolated from Dorka hooks', () => {
...
`
    ...
    expect(output).not.toContain('PROMPT_BACKSLASH:<__dorka_')
    expectLifecycle(output)
  })
...
`
    ...
    expect(output).not.toContain('PROMPT_ARRAY_BACKSLASH:<__dorka_')
    expectLifecycle(output)
  })
...
`
  ...
  itWithBash('normalizes hooks without a DEBUG-trap command substitution', () => {
    ...
    expect(output).not.toContain('PROMPT_DEBUG:<(( __dorka_exit_code == 0 ))>')
    expect(output).not.toContain('PROMPT_DEBUG:<__dorka_restore_prompt_status')
    expectLifecycle(output)
  })
  ...
  itWithBash('preserves DEBUG visibility for inherited prompt commands without functrace', () => {
    ...
    expect(output).not.toContain('PROMPT_DEBUG:<(( __dorka_exit_code == 0 ))>')
    expect(output).not.toContain('PROMPT_DEBUG:<__dorka_restore_prompt_status')
    expect(output).not.toContain('PROMPT_DEBUG:<eval "$__dorka_prompt_part">')
    expectLifecycle(output)
  })
  ...
  itWithBash('does not emit command lifecycle for an empty prompt', () => {
    ...
    expect(output).not.toContain('PROMPT_DEBUG:<__dorka_prompt_status=')
    expect(output).not.toContain('PROMPT_DEBUG:<__dorka_prompt_had_functrace="">')
    expect(output).not.toContain('PROMPT_DEBUG:<__dorka_outer_debug_trap_spec=')
  })
  ...
  itWithBash('does not recurse when a user trap installs Dorka preexec', () => {
    const profile = [
      `trap 'trap '\\''__dorka_osc133_preexec'\\'' DEBUG 2>/dev/null; printf "PRIVATE_TRAP\\n"' DEBUG`,
  ...
  itWithBash('does not introduce RETURN traps for inherited prompt commands', () => {
    ...
    expect(output).not.toContain('PROMPT_RETURN:<__dorka_run_prompt_command_array>')
    expectLifecycle(output)
  })
  ...
  itWithBash('hides legacy dispatcher commands from a chained DEBUG trap', () => {
    ...
    const output = runInteractiveBash(
      profile,
      tempHome,
      'echo __dorka_osc133_probe\nfalse\nexit 0\n'
    )
    const commands = [...output.matchAll(/PROMPT_PREEXEC:<([^>]+)>/g)].map((match) => match[1])

    expect(commands).toEqual(['echo __dorka_osc133_probe', 'false', 'exit 0'])
    expectLifecycle(output)
  })
  ...
  itWithBash.each([
    ['status capture', '__dorka_prompt_status=$?'],
    ['trap capture', '__dorka_outer_debug_trap_spec="$(trap -p DEBUG)"'],
    ['trap re-arm', 'trap "__dorka_osc133_preexec" DEBUG']
