...
import { buildBareDorkaCliScript } from './linux-bare-dorka-dispatcher'
import { quoteShell } from './cli-install-path-format'

const SHIM_DIR_NAME = 'linux-dorka-cli-shim'

export type LinuxTerminalDorkaCliShimOptions = {
  userDataPath: string
...
// Why: on Linux the CLI installs as `dorka-ide` so it never shadows the GNOME
// Dorka screen reader at /usr/bin/dorka — but agent-facing surfaces (skills,
// dispatch preambles, CLI hints) all invoke bare `dorka`, so on stock Ubuntu an
// agent inside an Dorka terminal would launch the screen reader instead
// (stablyai/orca#7904). Prepending this userData-scoped shim dir to managed-PTY
// PATH makes bare `dorka` resolve to the Dorka CLI inside Dorka terminals only,
// leaving the user's own shells (and their screen reader) untouched.
export function ensureLinuxTerminalDorkaCliShimDir(
  options: LinuxTerminalDorkaCliShimOptions
): string | null {
  const resourcesPath =
...
fail() {
  printf 'Dorka CLI is unavailable; reopen Dorka or register the CLI again.\\n' >&2
  exit 1
}
...
function ensureShimForLauncher(userDataPath: string, launcherPath: string): string | null {
  const script = buildBareDorkaCliScript(launcherPath)
