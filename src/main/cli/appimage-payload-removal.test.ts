...
async function makePayloadTree(): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), 'dorka-payload-removal-'))
...
describe('removeExtractedAppImagePayload', () => {
  ...
  it('restores the previous asar setting after a failure', async () => {
    ...
    await expect(
      removeExtractedAppImagePayload(join(tmpdir(), 'dorka-missing', 'nested', '\0invalid'))
    ).rejects.toThrow()
  ...
  it('is a no-op for a path that does not exist', async () => {
    await expect(
      removeExtractedAppImagePayload(join(tmpdir(), 'dorka-payload-removal-absent'))
