...
describe('CliInstaller', () => {
  ...
  it('creates a windows wrapper and updates the user PATH', async () => {
    ...
    const installPath = join(fixture.root, 'Programs', 'Dorka', 'bin', 'dorka.cmd')
    let userPath = 'C:\\Windows\\System32'
    const installer = new CliInstaller({
      platform: 'win32',
      isPackaged: false,
      userDataPath: fixture.userDataPath,
      execPath: 'C:\\Users\\me\\AppData\\Local\\Dorka\\Dorka.exe',
    ...
    expect(userPath).toContain(join(fixture.root, 'Programs', 'Dorka', 'bin'))

    const wrapperContent = await readFile(installPath, 'utf8')
    expect(wrapperContent).toContain('DORKA_LAUNCHER=')
    expect(wrapperContent).toContain('dorka.cmd')
    const launcherContent = await readFile(installed.launcherPath as string, 'utf8')
    expect(launcherContent).toContain(`set "DORKA_USER_DATA_PATH=${fixture.userDataPath}"`)
    expect(launcherContent).toContain('set "DORKA_APP_EXECUTABLE=%ELECTRON%"')

    const removed = await installer.remove()
    expect(removed.state).toBe('not_installed')
    expect(userPath).not.toContain(join(fixture.root, 'Programs', 'Dorka', 'bin'))
  })
  ...
  it('skips the Windows PATH write when removing an absent entry', async () => {
    ...
    const installer = new CliInstaller({
      ...
      execPath: 'C:\\Users\\me\\AppData\\Local\\Dorka\\Dorka.exe',
      appPath: fixture.appPath,
      commandPathOverride: join(fixture.root, 'Programs', 'Dorka', 'bin', 'dorka.cmd'),
      userPathReader: async () => userPathRead('C:\\Windows\\System32'),
      userPathWriter
    })
  ...
  it('matches expandable Windows PATH entries case-insensitively without rewriting them', async () => {
    const fixture = await makeFixture()
    const installPath = join(fixture.root, 'Local App Data', 'Dorka', 'bin', 'dorka.cmd')
    const userPathWriter = vi.fn()
    const installer = new CliInstaller({
      platform: 'win32',
      isPackaged: false,
      ...
      userPathReader: async () => userPathRead('%localappdata%\\Dorka\\bin\\', true),
      userPathWriter
    })
  ...
  it('does not expand environment variables stored in a REG_SZ Windows PATH', async () => {
    ...
    const installer = new CliInstaller({
      ...
      commandPathOverride: installPath,
      windowsEnvironment: { LOCALAPPDATA: join(fixture.root, 'Local App Data') },
      userPathReader: async () => userPathRead('%LOCALAPPDATA%\\Dorka\\bin'),
      userPathWriter
    })

    await installer.install()

    expect(userPathWriter).toHaveBeenCalledWith(`%LOCALAPPDATA%\\Dorka\\bin;${pathDirectory}`)
  })
  ...
  it('resolves custom-install packaged Windows command path from resourcesPath', async () => {
    ...
    const resourcesPath = join(fixture.root, 'D Custom Dorka', 'resources')
    await mkdir(join(resourcesPath, 'bin'), { recursive: true })
    await writeFile(join(resourcesPath, 'bin', 'dorka.exe'), 'native launcher', 'utf8')
    ...
    const installer = new CliInstaller({
      ...
      localAppDataPath,
      userDataPath: fixture.userDataPath,
      execPath: join(fixture.root, 'D Custom Dorka', 'Dorka.exe'),
      appPath: fixture.appPath,
    ...
    expect(status.commandPath).toBe(join(resourcesPath, 'bin', 'dorka.exe'))
  })
  ...
  it('keeps a bundled Windows launcher installed when the user PATH read is unknown', async () => {
    ...
    const bundledLauncher = join(resourcesPath, 'bin', 'dorka.exe')
    ...
    const installer = new CliInstaller({
      ...
      resourcesPath,
      userDataPath: fixture.userDataPath,
      execPath: join(fixture.root, 'Dorka.exe'),
      appPath: fixture.appPath,
      userPathReader: async () => ({
        state: 'unknown',
        detail: 'Dorka could not read the Windows user PATH registry value.'
