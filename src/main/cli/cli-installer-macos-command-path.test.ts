...
describe('CliInstaller', () => {
  ...
  it.skipIf(process.platform === 'win32')(
    'falls back to ~/.local/bin/dorka on macOS when /usr/local/bin does not exist',
    async () => {
      ...
      const absentUsrLocalBin = join(fixture.root, 'usr', 'local', 'bin', 'dorka')
      ...
      expect(status.commandPath).toBe(join(homePath, '.local', 'bin', 'dorka'))
      expect(status.state).toBe('not_installed')
      expect(status.supported).toBe(true)

      const installed = await installer.install()
      expect(installed.state).toBe('installed')
      expect(installed.commandPath).toBe(join(homePath, '.local', 'bin', 'dorka'))
  ...
  it.skipIf(process.platform === 'win32')(
    'uses /usr/local/bin/dorka on macOS when /usr/local/bin exists',
    async () => {
      const fixture = await makeFixture()
      const resourcesPath = await createPackagedMacLauncher(fixture.root)
      ...
      const installPath = join(usrLocalBin, 'dorka')
      const installer = new CliInstaller({
        platform: 'darwin',
        isPackaged: true,
        resourcesPath,
  ...
  // Why: users can have a managed Dorka command in ~/.local/bin even when
  // /usr/local/bin exists; Settings must follow the shell-visible command.
  it.skipIf(process.platform === 'win32')(
    'uses an existing managed macOS dorka command from the shell PATH before /usr/local/bin',
    async () => {
      const fixture = await makeFixture()
      const homePath = join(fixture.root, 'home')
      const resourcesPath = await createPackagedMacLauncher(fixture.root)
      ...
      const defaultInstallPath = join(usrLocalBin, 'dorka')
      const userInstallPath = join(userLocalBin, 'dorka')
  ...
  it.skipIf(process.platform === 'win32')(
    'skips a broken managed macOS dorka symlink before /usr/local/bin',
    async () => {
      const fixture = await makeFixture()
      const homePath = join(fixture.root, 'home')
      const resourcesPath = await createPackagedMacLauncher(fixture.root)
  ...
  // ~/.local/bin/dorka must not steal status from /usr/local/bin/dorka.
  it.skipIf(process.platform === 'win32')(
    'keeps the default macOS command when a managed dorka appears later on PATH',
    async () => {
      const fixture = await makeFixture()
      const homePath = join(fixture.root, 'home')
      const resourcesPath = await createPackagedMacLauncher(fixture.root)
  ...
  // is still the shell-visible Dorka command until the default path is installed.
  it.skipIf(process.platform === 'win32')(
    'uses a later managed macOS dorka command when the default command is missing',
    async () => {
      const fixture = await makeFixture()
      const homePath = join(fixture.root, 'home')
      const resourcesPath = await createPackagedMacLauncher(fixture.root)
  ...
  // Why: bash/zsh skip non-executable PATH entries even at Dorka's configured
  // install slot, then keep looking for a runnable command later in PATH.
  it.skipIf(process.platform === 'win32')(
    'uses a later managed macOS dorka command when the default command is not executable',
    async () => {
      const fixture = await makeFixture()
      const homePath = join(fixture.root, 'home')
      const resourcesPath = await createPackagedMacLauncher(fixture.root)
      ...
      const installed = await installer.install()
      expect(installed.commandPath).toBe(userInstallPath)
      await expect(readFile(defaultInstallPath, 'utf8')).resolves.toContain('other-dorka')
    }
  )

  // Why: a non-Dorka command after an empty default install slot can be shadowed
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      expect(installed.state).toBe('installed')
      await expect(readlink(defaultInstallPath)).resolves.toBe(launcherPath)
      await expect(readFile(userInstallPath, 'utf8')).resolves.toContain('other-dorka')
    }
  )

  // Why: an off-PATH ~/.local/bin/dorka must not hijack CLI registration and
  // leave the shell-visible /usr/local/bin command missing.
  it.skipIf(process.platform === 'win32')(
    'ignores managed macOS dorka commands that are not visible on the shell PATH',
    async () => {
      const fixture = await makeFixture()
      const homePath = join(fixture.root, 'home')
      const resourcesPath = await createPackagedMacLauncher(fixture.root)
  ...
  it.skipIf(process.platform === 'win32')(
    'reports a conflict for an unmanaged macOS dorka that shadows the install path',
    async () => {
      const fixture = await makeFixture()
      const homePath = join(fixture.root, 'home')
      const resourcesPath = await createPackagedMacLauncher(fixture.root)
  ...
  it.skipIf(process.platform === 'win32')(
    'skips a non-executable unmanaged macOS dorka before the install path',
    async () => {
      const fixture = await makeFixture()
      const homePath = join(fixture.root, 'home')
      const resourcesPath = await createPackagedMacLauncher(fixture.root)
  ...
  // Why: when macCommandPath falls back to ~/.local/bin/dorka on arm64, commandName
  // must still be 'dorka' (not 'dorka-ide' which is Linux-only).
  it.skipIf(process.platform === 'win32')(
    'reports commandName as dorka (not dorka-ide) when falling back to ~/.local/bin on macOS',
    async () => {
      const fixture = await makeFixture()
      const homePath = join(fixture.root, 'home')
      const resourcesPath = await createPackagedMacLauncher(fixture.root)
      ...
      const status = await installer.getStatus()
      expect(status.commandName).toBe('dorka')
    }
  )
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      expect(s1.commandPath).toBe(join(homePath, '.local', 'bin', 'dorka'))
    }
  )

  // Why: the arm64 fallback must apply for packaged builds, not just dev launchers.
  it.skipIf(process.platform === 'win32')(
    'resolves to ~/.local/bin/dorka on arm64 even when isPackaged is true',
    async () => {
      const fixture = await makeFixture()
      const homePath = join(fixture.root, 'home')
      const absentUsrLocalBin = join(fixture.root, 'usr', 'local', 'bin', 'dorka')
      const resourcesPath = join(fixture.root, 'resources')
      const bundledLauncher = join(resourcesPath, 'bin', 'dorka')
