...
import { installLinuxBareDorkaDispatcher } from './linux-bare-dorka-dispatcher'
...
async function makeFixture(): Promise<{ homePath: string; resourcesPath: string }> {
  const root = await mkdtemp(join(tmpdir(), 'dorka-bare-dispatcher-'))
  created.push(root)
  const resourcesPath = join(root, 'resources')
  // The bundled dorka-ide launcher must exist for the dispatcher to be written.
  await mkdir(join(resourcesPath, 'bin'), { recursive: true })
  await writeFile(join(resourcesPath, 'bin', 'dorka-ide'), '#!/usr/bin/env bash\n', 'utf8')
...
describe('installLinuxBareDorkaDispatcher', () => {
  it('uses the mounted bundled launcher when only APPDIR is inherited', async () => {
    ...
    const result = await installLinuxBareDorkaDispatcher({ resourcesPath, homePath })

    expect(result.state).toBe('installed')
    expect(result.target).toBe(join(resourcesPath, 'bin', 'dorka-ide'))
  })

  it('writes an executable bare-dorka dispatcher that execs the bundled dorka-ide launcher', async () => {
    const { homePath, resourcesPath } = await makeFixture()

    const result = await installLinuxBareDorkaDispatcher({
      resourcesPath,
      homePath,
      appImagePath: null
    })

    const expectedTarget = join(resourcesPath, 'bin', 'dorka-ide')
    expect(result.state).toBe('installed')
    expect(result.target).toBe(expectedTarget)
    expect(result.dispatcherPath).toBe(join(homePath, '.local', 'bin', 'dorka'))
  ...
  it('is idempotent — a second install rewrites its own dispatcher without throwing', async () => {
    ...
    const first = await installLinuxBareDorkaDispatcher({
      resourcesPath,
      homePath,
      appImagePath: null
    })
    const second = await installLinuxBareDorkaDispatcher({
      resourcesPath,
      homePath,
      appImagePath: null
    })
  ...
  it('publishes when the home filesystem does not support hard links', async () => {
    ...
    await expect(readFile(result.dispatcherPath, 'utf8')).resolves.toContain(
      '# dorka-serve-bare-dorka-dispatcher'
    )
  })

  it('restores a displaced foreign dispatcher when hard links are unsupported', async () => {
    const { homePath, resourcesPath } = await makeFixture()
    await installLinuxBareDorkaDispatcher({ resourcesPath, homePath, appImagePath: null })
  ...
  it('quotes a resources path containing spaces so the exec line cannot be split', async () => {
    const root = await mkdtemp(join(tmpdir(), 'dorka-bare-dispatcher-space-'))
    created.push(root)
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      const { homePath, resourcesPath } = await makeFixture()
      const appImagePath = join(homePath, 'Dorka.AppImage')
      ...
      const result = await installLinuxBareDorkaDispatcher({
        ...
        appImageExtractRunner: async (_appImagePath, cwd) => {
          ...
          await writeFile(join(launcherDir, 'dorka-ide'), '', { encoding: 'utf8', mode: 0o755 })
        }
      })
  ...
  it('skips (does not clobber) a user-owned dorka already at ~/.local/bin', async () => {
    const { homePath, resourcesPath } = await makeFixture()
    const dispatcherPath = join(homePath, '.local', 'bin', 'dorka')
    const appImagePath = join(homePath, 'Dorka.AppImage')
  ...
  it('preserves a foreign dispatcher created while AppImage extraction is in flight', async () => {
    ...
    const installation = installLinuxBareDorkaDispatcher({
      resourcesPath,
      homePath,
      appImagePath,
      appImageCacheRootPath: cacheRootPath,
  ...
  it('prunes old owner generations without touching a sibling namespace', async () => {
    ...
    await installLinuxBareDorkaDispatcher(options)
    ...
    const installation = installLinuxBareDorkaDispatcher(options)
  ...
  it('skips when the bundled dorka-ide launcher is missing from the build', async () => {
    const root = await mkdtemp(join(tmpdir(), 'dorka-bare-dispatcher-nolauncher-'))
    created.push(root)
...
async function writePayload(cwd: string): Promise<void> {
  ...
  await writeFile(join(launcherDirectory, 'dorka-ide'), '#!/usr/bin/env bash\n', { mode: 0o755 })
}
