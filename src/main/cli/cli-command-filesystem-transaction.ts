...
export async function inspectStableCommand(
  ...
  throw new Error(`The command at ${commandPath} changed while Dorka inspected it.`)
}
...
export async function quarantineCommandPath(commandPath: string): Promise<CommandQuarantine> {
  ...
  const directoryPath = join(commandDirectory, `.dorka-cli-${process.pid}-${randomUUID()}`)
...
export function buildMacPrivilegedSymlinkTransaction(
  ...
  const transactionDirectory = join(commandDirectory, `.dorka-cli-${process.pid}-${randomUUID()}`)
