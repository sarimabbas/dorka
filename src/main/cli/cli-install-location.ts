...
export abstract class CliInstallLocation {
  ...
  protected get commandName(): string {
    ...
    // Why: packaged Linux uses `dorka-ide` to avoid shadowing GNOME Dorka's /usr/bin/dorka.
    return this.platform === 'linux' ? LINUX_CLI_COMMAND_NAME : 'dorka'
  }
  ...
  constructor(options: CliInstallerOptions = {}) {
    ...
    this.commandPathOverride =
      options.commandPathOverride ?? process.env.DORKA_CLI_INSTALL_PATH ?? null
    ...
    this.macCommandPath = existsSync(dirname(candidateMacPath))
      ? candidateMacPath
      : join(this.homePath, '.local', 'bin', 'dorka')
    ...
    for (const commandPath of this.getPathCommandCandidates(defaultCommandPath)) {
      ...
      if (status.state !== 'not_installed') {
        if (reachedDefaultCommandPath && !isDefaultCommandPath && status.state === 'conflict') {
          // Why: a non-Dorka command after an empty default slot can be shadowed by installing there; no user file replaced.
          continue
        }
  ...
  protected resolveCommandPath(): string | null {
    ...
    if (!this.isPackaged) {
      ...
      if (this.platform === 'win32') {
        return join(this.localAppDataPath, 'Programs', 'Dorka Dev', 'bin', `${DEV_COMMAND_NAME}.cmd`)
      }
    }
    ...
    if (this.platform === 'linux') {
      ...
      // Why `dorka-ide`: GNOME Dorka ships /usr/bin/dorka, so avoid shadowing that screen reader.
