...
// our own file idempotently but never clobber a user's own ~/.local/bin/dorka.
const DISPATCHER_MARKER = '# dorka-serve-bare-dorka-dispatcher'

export type LinuxBareDorkaDispatcherOptions = {
  /** Packaged app resources root; the bundled `dorka-ide` launcher lives under it. */
  ...
  /** Test seam — defaults to $XDG_CACHE_HOME/dorka/appimage. */
...
export type LinuxBareDorkaDispatcherState =
  | 'installed'
  | 'skipped-foreign'
  | 'skipped-launcher-missing'

export type LinuxBareDorkaDispatcherResult = {
  state: LinuxBareDorkaDispatcherState
  dispatcherPath: string
  /** The bundled `dorka-ide` launcher the dispatcher execs. */
  target: string | null
}

// Why: on Linux the CLI installs as `dorka-ide`, not bare `dorka`, to avoid
// shadowing GNOME Dorka's /usr/bin/dorka. But the Claude Team launcher typed into
// the initial managed terminal invokes the literal `dorka claude-teams`, so a
// headless serve box needs a bare-`dorka` dispatcher on the managed-terminal PATH
...
// never reclaims it.
export async function installLinuxBareDorkaDispatcher(
  options: LinuxBareDorkaDispatcherOptions
): Promise<LinuxBareDorkaDispatcherResult> {
  const dispatcherPath = join(options.homePath ?? homedir(), '.local', 'bin', 'dorka')
  ...
  const installed = await publishDispatcher(
    dispatcherPath,
    insertDispatcherMarker(buildBareDorkaCliScript(launcher))
  )
  return installed
...
/** Bare-`dorka` script that execs the one Linux CLI launcher. */
export function buildBareDorkaCliScript(launcherPath: string): string {
...
async function publishDispatcher(dispatcherPath: string, content: string): Promise<boolean> {
  ...
  const temporaryPath = join(directoryPath, `.dorka-dispatcher-${process.pid}-${randomUUID()}`)
  ...
  try {
    ...
    const displacedPath = join(
      directoryPath,
      `.dorka-preserved-dispatcher-${process.pid}-${randomUUID()}`
    )
    try {
