const MANAGED_MARKER = '# Dorka managed WSL CLI launcher'
const BRIDGE_MANAGED_MARKER = '# Dorka managed WSL CLI PowerShell bridge'

export function buildWslLauncher(
  windowsLauncherPath: string,
  bridgePath = '${XDG_DATA_HOME:-$HOME/.local/share}/dorka/dorka-wsl-bridge.ps1'
): string {
  const encodedTarget = Buffer.from(windowsLauncherPath, 'utf8').toString('base64')
  return `#!/usr/bin/env bash
set -euo pipefail
${MANAGED_MARKER}
# DORKA_WIN_LAUNCHER_B64=${encodedTarget}
DORKA_WIN_LAUNCHER=${quoteShell(windowsLauncherPath)}
DORKA_BRIDGE_PS1=${quoteShell(bridgePath)}
...
DORKA_WSL_CWD=$(pwd -P 2>/dev/null) || {
  DORKA_WSL_CWD=/
  cd /
}
DORKA_BRIDGE_PS1_WIN=$(wslpath -w "$DORKA_BRIDGE_PS1")
DORKA_WSL_CWD_WIN=$(wslpath -w "$DORKA_WSL_CWD")
exec "$DORKA_POWERSHELL" -NoProfile -ExecutionPolicy Bypass -File "$DORKA_BRIDGE_PS1_WIN" "$DORKA_WIN_LAUNCHER" -WslCwd "$DORKA_WSL_CWD_WIN" "$@"
`
}
...
try {
  ...
  if ($args.Count -lt 1) {
    throw 'Invalid Dorka WSL CLI bridge invocation.'
  }
  [string]$DorkaLauncher = $args[0]
  [string]$WslCwd = ''
  ...
  if ([string]::IsNullOrEmpty($WslCwd)) {
    Remove-Item Env:DORKA_CLI_CWD -ErrorAction SilentlyContinue
  } else {
    $env:DORKA_CLI_CWD = $WslCwd
  }
  $LauncherDirectory = Split-Path -Parent $DorkaLauncher
  ...
  $StartInfo.FileName = $DorkaLauncher
  ...
  if ($null -eq $Process) {
    throw 'Unable to start the Dorka Windows CLI launcher.'
  }
  $Process.WaitForExit()
...
export function getBridgePathFromCommandPath(commandPath: string): string {
  ...
  // share one WSL bridge under ~/.local/share/dorka.
  return `${commandPath.replace(/\/\.local\/bin\/(?:dorka|dorka-ide)$/, '/.local/share/dorka')}/dorka-wsl-bridge.ps1`
}
...
export function buildRegistrationLockPrelude(commandPath: string): string {
  ...
  // Why: the per-distro queue only serializes one Dorka process; flock covers
  ...
  return [
    ...
    `  exec 9>${quoteShell(`${lockDir}/.dorka-wsl-cli.lock`)}`,
    '  flock -x -w 30 9',
    'fi'
  ].join('\n')
}
...
export function buildManagedLegacyRemoveCommand(quotedLegacyCommandPath: string): string {
  // Why: remove only the Dorka-managed pre-rename wrapper; user-owned `dorka`
...
export function buildSafeRemoveCommand(commandPath: string, legacyCommandPath?: string): string {
  ...
  return [
    ...
    // Why: leaving a managed legacy `dorka` behind lets startup reconciliation
...
export function parseManagedLauncherTarget(content: string): string | null {
  const encoded = content.match(/^# DORKA_WIN_LAUNCHER_B64=([A-Za-z0-9+/=]+)$/m)?.[1]
  if (encoded) {
    try {
  ...
  const legacyTarget = content.match(/^DORKA_WIN_LAUNCHER='((?:[^']|'"'"')*)'$/m)?.[1]
