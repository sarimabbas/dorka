...
describe.skipIf(process.platform === 'win32')('AppImage CLI removal', () => {
  it.each([false, true])(
    ...
    async (removeCommandFirst) => {
      const root = await mkdtemp(join(tmpdir(), 'dorka-appimage-cli-remove-'))
      created.push(root)
      const appImagePath = join(root, 'Dorka.AppImage')
      const cacheRootPath = join(root, 'cache')
      const commandPath = join(root, 'home', '.local', 'bin', 'dorka-ide')
      ...
      const installer = new CliInstaller({
        ...
        resourcesPath,
        execPath: join(root, 'mount', 'dorka-ide'),
        ...
        appImageExtractRunner: async (_path, cwd) => {
          ...
          await writeFile(
            join(payloadDirectory, 'dorka-ide'),
            '#!/usr/bin/env bash\nprintf installed',
            {
              mode: 0o755
            }
  ...
  it('does not remove a sibling AppImage registration or payload', async () => {
    const root = await mkdtemp(join(tmpdir(), 'dorka-appimage-cli-siblings-'))
    created.push(root)
    const cacheRootPath = join(root, 'cache')
    const commandPath = join(root, 'home', '.local', 'bin', 'dorka-ide')
    const resourcesPath = join(root, 'mount', 'resources')
    const firstAppImagePath = join(root, 'Dorka-stable.AppImage')
    const secondAppImagePath = join(root, 'Dorka-nightly.AppImage')
    await mkdir(join(resourcesPath, 'bin'), { recursive: true })
    await Promise.all([
    ...
    const installerOptions = (appImagePath: string, content: string): CliInstallerOptions => ({
      ...
      appImageExtractRunner: async (_path, cwd) => {
        const payloadDirectory = join(cwd, 'squashfs-root', 'resources', 'bin')
        await mkdir(payloadDirectory, { recursive: true })
        await writeFile(join(payloadDirectory, 'dorka-ide'), content, { mode: 0o755 })
      }
    })
