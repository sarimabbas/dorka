...
async function makeFixture(): Promise<{ userDataPath: string; resourcesPath: string }> {
  const root = await mkdtemp(join(tmpdir(), 'dorka-terminal-cli-shim-'))
  created.push(root)
  const resourcesPath = join(root, 'resources')
  // The bundled dorka-ide launcher must exist for the shim to be written.
  mkdirSync(join(resourcesPath, 'bin'), { recursive: true })
  writeFileSync(join(resourcesPath, 'bin', 'dorka-ide'), '#!/usr/bin/env bash\n', 'utf8')
...
describe('ensureLinuxTerminalDorkaCliShimDir', () => {
  it('uses the mounted bundled launcher when only APPDIR is inherited', async () => {
    ...
    const shimDir = ensureLinuxTerminalDorkaCliShimDir({ userDataPath, resourcesPath })

    expect(shimDir).toBe(join(userDataPath, 'linux-dorka-cli-shim'))
    expect(readFileSync(join(shimDir!, 'dorka'), 'utf8')).toContain(
      `exec '${join(resourcesPath, 'bin', 'dorka-ide')}' "$@"`
    )
  })

  it('writes an executable bare-dorka shim that execs the bundled dorka-ide launcher', async () => {
    const { userDataPath, resourcesPath } = await makeFixture()

    const shimDir = ensureLinuxTerminalDorkaCliShimDir({
      userDataPath,
      resourcesPath,
      appImagePath: null
    })
    ...
    expect(shimDir).toBe(join(userDataPath, 'linux-dorka-cli-shim'))
    const content = readFileSync(join(shimDir!, 'dorka'), 'utf8')
    // Single-quoted so a resources path with shell metacharacters can't break out.
    expect(content).toContain(`exec '${join(resourcesPath, 'bin', 'dorka-ide')}' "$@"`)
    const mode = statSync(join(shimDir!, 'dorka')).mode & 0o777
  ...
  it('reuses the shim path and re-asserts its exec bit', async () => {
    ...
    const first = ensureLinuxTerminalDorkaCliShimDir(options)
    expect(first).not.toBeNull()
    const shimPath = join(first!, 'dorka')
    chmodSync(shimPath, 0o644)

    const second = ensureLinuxTerminalDorkaCliShimDir(options)
    expect(second).toBe(first)
    expect(statSync(shimPath).mode & 0o111).not.toBe(0)

    const root = await mkdtemp(join(tmpdir(), 'dorka-terminal-cli-shim-2-'))
    created.push(root)
    const otherUserData = join(root, 'user-data')
    mkdirSync(join(otherUserData, 'linux-dorka-cli-shim'), { recursive: true })
    writeFileSync(join(otherUserData, 'linux-dorka-cli-shim', 'dorka'), 'stale contents', 'utf8')
    chmodSync(join(otherUserData, 'linux-dorka-cli-shim', 'dorka'), 0o644)

    const healed = ensureLinuxTerminalDorkaCliShimDir({
      userDataPath: otherUserData,
      resourcesPath,
      appImagePath: null
    })
    expect(healed).not.toBeNull()
    const healedPath = join(healed!, 'dorka')
    expect(readFileSync(healedPath, 'utf8')).toContain('dorka-ide')
  ...
  it.skipIf(!canFenceAppImageRuntime)(
    ...
    async () => {
      const { userDataPath, resourcesPath } = await makeFixture()
      const appImagePath = join(userDataPath, 'Dorka.AppImage')
      await mkdir(userDataPath, { recursive: true })
      await writeFile(appImagePath, '#!/usr/bin/env bash\n', { encoding: 'utf8', mode: 0o755 })
      const cacheRootPath = join(userDataPath, 'cache')
      const liveLauncherPath = join(resourcesPath, 'bin', 'dorka-ide')
      ...
      const shimPath = join(shimDir!, 'dorka')
  ...
  it.skipIf(!canFenceAppImageRuntime)(
    ...
    async () => {
      ...
      await writeFile(appImagePath, '#!/usr/bin/env bash\n', { encoding: 'utf8', mode: 0o755 })
      const cacheRootPath = join(userDataPath, 'cache')
      const firstLauncher = join(resourcesPath, 'bin', 'dorka-ide')
      ...
      const shimDir = ensureLinuxTerminalDorkaCliShimDir(options)
      const shimPath = join(shimDir!, 'dorka')
      const originalShim = readFileSync(shimPath, 'utf8')

      const nextResourcesPath = join(userDataPath, 'next-mount', 'resources')
      const nextLauncher = join(nextResourcesPath, 'bin', 'dorka-ide')
      ...
      expect(
        ensureLinuxTerminalDorkaCliShimDir({ ...options, resourcesPath: nextResourcesPath })
      ).toBe(shimDir)
  ...
  it.skipIf(!canFenceAppImageRuntime)(
    ...
    async () => {
      ...
      await mkdir(userDataPath, { recursive: true })
      await writeFile(appImagePath, '#!/usr/bin/env bash\n', { mode: 0o755 })
      const liveLauncher = join(resourcesPath, 'bin', 'dorka-ide')
  ...
  it('returns null (and does not memoize) when the bundled launcher is missing', async () => {
    const root = await mkdtemp(join(tmpdir(), 'dorka-terminal-cli-shim-missing-'))
    created.push(root)
    const userDataPath = join(root, 'user-data')

    const missing = ensureLinuxTerminalDorkaCliShimDir({
      userDataPath,
    ...
    mkdirSync(join(resourcesPath, 'bin'), { recursive: true })
    writeFileSync(join(resourcesPath, 'bin', 'dorka-ide'), '#!/usr/bin/env bash\n', 'utf8')
    const recovered = ensureLinuxTerminalDorkaCliShimDir({
      userDataPath,
      resourcesPath,
      appImagePath: null
    })
    expect(recovered).toBe(join(userDataPath, 'linux-dorka-cli-shim'))
  })
})
