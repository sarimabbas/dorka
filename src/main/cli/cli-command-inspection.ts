...
// previous packaged Dorka and is ours to reclaim; anything else stays a conflict.
const PACKAGED_LINUX_LAUNCHER_DIRECTORIES = ['/opt/Dorka', '/opt/dorka-ide', '/opt/dorka']
...
export class CliCommandInspection extends CliInstallLocation {
    ...
    try {
      ...
      if (!stats.isSymbolicLink()) {
        if (stats.isFile()) {
          ...
          if (managedTarget) {
            return this.buildStatus({
              ...
              detail: `${commandPath} contains an older Dorka launcher.`
            })
          }
        }
        ...
        return this.buildStatus({
          ...
          state: 'conflict',
          currentTarget: null,
          detail: `${commandPath} exists but is not an Dorka symlink.`
        })
      }
      ...
      return this.buildStatus({
        ...
        detail: isInstalled
          ...
          : isManagedStaleTarget
            ? `${commandPath} points to an older Dorka launcher.`
            : `${commandPath} points to a non-Dorka launcher.`
      })
    } catch (error) {
      if (isMissingError(error)) {
        return this.buildStatus({
          ...
          state: 'not_installed',
          currentTarget: null,
          detail: `Register ${commandPath} to use Dorka from the terminal.`
        })
      }
      throw error
    }
  ...
  protected isManagedSymlinkTarget(resolvedTarget: string, launcherPath: string): boolean {
    ...
    if (this.platform === 'darwin') {
      // Why: reclaim symlinks to an older Dorka.app launcher, but never replace arbitrary user-owned symlinks.
  ...
  /** A launcher inside a packaged Linux install tree, left behind by a deb/rpm Dorka. */
  ...
  protected isSiblingDevLauncherTarget(
    ...
    // Why: dev builds generate launchers under the sibling `*-dev` profile; packaged Dorka must reclaim that command.
    return (
    ...
    try {
      ...
      if (!stats.isFile()) {
        return this.buildStatus({
          ...
          state: 'conflict',
          currentTarget: null,
          detail: `${commandPath} exists but is not an Dorka launcher script.`
        })
      }
      ...
      if (isMissingError(error)) {
        return this.buildStatus({
          ...
          state: 'not_installed',
          currentTarget: null,
          detail: `Register ${commandPath} to use Dorka from Command Prompt or PowerShell.`
        })
      }
      throw error
    }
