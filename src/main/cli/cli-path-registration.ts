...
export class CliPathRegistration extends CliCommandInstallation {
  ...
  protected withPathInfo(
    ...
    if (
      ...
      return {
        ...
        state: 'not_installed',
        currentTarget: null,
        detail: `Register ${status.commandPath} to use Dorka from Command Prompt or PowerShell.`
      }
    }
    ...
    if (pathConfigured === null) {
      return {
        ...
        detail:
          pathProbe.detail ??
          'The Dorka launcher exists, but Dorka could not check your Windows user PATH.'
      }
    }
  ...
  protected async writeWindowsUserPathEntry(
    ...
    try {
      ...
      const guidance =
        action === 'add'
          ? `Add this folder to your PATH manually: ${pathDirectory}. Or run Dorka as an administrator and try again.`
          : `Remove this folder from your PATH manually: ${pathDirectory}. Or run Dorka as an administrator and try again.`
      throw new Error(
