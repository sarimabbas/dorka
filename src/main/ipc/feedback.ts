...
const FEEDBACK_API_URL = 'https://www.ondorka.dev/v1/feedback'
...
// which Dorka build and which OS the feedback came from. The main process is
...
function feedbackRequestBodyInit(body: FeedbackSubmitBody): Pick<RequestInit, 'body' | 'headers'> {
  ...
  if (body.diagnosticBundle) {
    ...
    formData.append(
      ...
      `dorka-diagnostics-${body.diagnosticBundle.bundleSubmissionId}.ndjson`
    )
  }
...
export async function submitFeedback(
  ...
  try {
    ...
    // Why: api.ondorka.dev serves a different product, so transient failures
