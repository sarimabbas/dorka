...
describe('registerPtyHandlers', () => {
  ...
  it('spawns a plain POSIX login shell and queues startup commands for the live session', async () => {
    ...
    const originalDorkaOrigZdotdir = process.env.DORKA_ORIG_ZDOTDIR
    ...
    try {
      ...
      // config dir to hand back and Dorka must not invent one — the wrapper
      // leaves ZDOTDIR unset, exactly as an unwrapped login zsh would.
      expect(options.env.DORKA_ORIG_ZDOTDIR).toBeUndefined()
    } finally {
      Object.defineProperty(process, 'platform', {
        configurable: true,
        value: originalPlatform
      ...
      if (originalDorkaOrigZdotdir === undefined) {
        delete process.env.DORKA_ORIG_ZDOTDIR
      } else {
        process.env.DORKA_ORIG_ZDOTDIR = originalDorkaOrigZdotdir
      }
