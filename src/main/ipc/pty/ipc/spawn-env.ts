...
export async function assemblePtyIpcSpawnEnv(ctx: PtyIpcSpawnState): Promise<void> {
  ...
  const baseEnvWithAuth = ctx.claudeAuth
    ...
    : sshSourceEnv
  const spawnPaneKey = baseEnvWithAuth?.DORKA_PANE_KEY
  ...
  ctx.requestedAgentTeamsPath = ctx.baseEnv?.DORKA_AGENT_TEAMS_TEAM_ID
  ...
  if (ctx.baseEnv && ctx.stablePaneKey && canForwardPaneEnv) {
    ctx.baseEnv.DORKA_PANE_KEY = ctx.stablePaneKey
    if (typeof args.tabId === 'string') {
      ctx.baseEnv.DORKA_TAB_ID = args.tabId
    } else if (!args.connectionId) {
      delete ctx.baseEnv.DORKA_TAB_ID
    }
    if (typeof args.worktreeId === 'string') {
      ctx.baseEnv.DORKA_WORKTREE_ID = args.worktreeId
    } else if (!args.connectionId) {
      delete ctx.baseEnv.DORKA_WORKTREE_ID
    }
  } else if (ctx.baseEnv) {
    // Why: DORKA_PANE_KEY crosses into shells/hook registries; only a key proven to match this spawn's tab+leaf may cross the IPC boundary.
    delete ctx.baseEnv.DORKA_PANE_KEY
    delete ctx.baseEnv.DORKA_TAB_ID
    delete ctx.baseEnv.DORKA_WORKTREE_ID
    delete ctx.baseEnv.DORKA_AGENT_LAUNCH_TOKEN
  }
  ctx.validatedPaneKey = ctx.stablePaneKey
  // Why: SSH can strip DORKA_PANE_KEY when remote hooks are off; IPC tab/leaf metadata still names the pane.
