...
import { prependDorkaCliDirToChildPath } from '../../../cli/dorka-cli-child-path'
...
import { stripInheritedDorkaCodexHomeOverride } from './codex-home'
import {
...
export function buildPtyHostEnv(
  ...
  if (opts.agentStatusHooksEnabled) {
    // Why: OPENCODE_CONFIG_DIR is a single path, not a colon-list; mirror the user's value into an overlay so their plugins and Dorka's status plugin coexist. See docs/opencode-config-dir-collision.md.
    const openCodeStatusService =
      openCodeAgent === 'opencode2' ? openCode2HookService : openCodeHookService
    baseEnv.DORKA_OPENCODE_AGENT = openCodeAgent
    ...
    if (baseEnv.OPENCODE_CONFIG_DIR) {
      ...
      baseEnv.DORKA_OPENCODE_CONFIG_DIR = baseEnv.OPENCODE_CONFIG_DIR
      if (preexistingOpenCodeConfigDir) {
        // Why: nested Dorka terminals inherit the overlay as OPENCODE_CONFIG_DIR; keep the real source so overlays don't mirror overlays.
        baseEnv.DORKA_OPENCODE_SOURCE_CONFIG_DIR = preexistingOpenCodeConfigDir
      } else {
        delete baseEnv.DORKA_OPENCODE_SOURCE_CONFIG_DIR
      }
    }
    if (isMimoLaunchCommand(launchCommandHint)) {
      ...
      if (baseEnv.MIMOCODE_HOME) {
        baseEnv.DORKA_MIMOCODE_HOME = baseEnv.MIMOCODE_HOME
        if (preexistingMimocodeHome) {
          baseEnv.DORKA_MIMOCODE_SOURCE_HOME = preexistingMimocodeHome
        } else {
          delete baseEnv.DORKA_MIMOCODE_SOURCE_HOME
        }
      }
    }
  } else {
    restoreOrStripOverlayEnv(baseEnv, {
      ...
      overlay: 'DORKA_OPENCODE_CONFIG_DIR',
      source: 'DORKA_OPENCODE_SOURCE_CONFIG_DIR'
    })
    restoreOrStripOverlayEnv(baseEnv, {
      primary: 'MIMOCODE_HOME',
      overlay: 'DORKA_MIMOCODE_HOME',
      source: 'DORKA_MIMOCODE_SOURCE_HOME'
    })
  }
  ...
  if (opts.agentStatusHooksEnabled) {
    ...
    if (opts.isWsl === true) {
      ...
      if (guestEndpoint) {
        baseEnv.DORKA_AGENT_HOOK_ENDPOINT = guestEndpoint
      }
      ...
      if (opencodeOverlayDir) {
        ...
        baseEnv.DORKA_OPENCODE_CONFIG_DIR = opencodeOverlayDir
        delete baseEnv.DORKA_OPENCODE_SOURCE_CONFIG_DIR
      } else {
        ...
        delete baseEnv.DORKA_OPENCODE_CONFIG_DIR
        delete baseEnv.DORKA_OPENCODE_SOURCE_CONFIG_DIR
      }
    }
  }

  // Why: PI_CODING_AGENT_DIR is the user's config/session root; install only Dorka-owned extension files, don't override it.
  if (opts.agentStatusHooksEnabled) {
    ...
    restoreOrStripOverlayEnv(baseEnv, {
      ...
      overlay: 'DORKA_PI_CODING_AGENT_DIR',
      source: 'DORKA_PI_SOURCE_AGENT_DIR'
    })
    restoreOrStripOverlayEnv(baseEnv, {
      primary: 'PI_CODING_AGENT_DIR',
      overlay: 'DORKA_OMP_CODING_AGENT_DIR',
      source: 'DORKA_OMP_SOURCE_AGENT_DIR'
    })
  ...
  if (opts.isWsl && opts.agentStatusHooksEnabled) {
    ...
    if (explicitPiAgentKind === 'pi') {
      ...
      if (guestPiDir) {
        baseEnv.DORKA_PI_SOURCE_AGENT_DIR = guestPiDir
      }
      ...
      if (guestOmpExtension) {
        baseEnv.DORKA_OMP_STATUS_EXTENSION = guestOmpExtension
      }
    }
  }

  // Why: keep the Codex home override PTY-scoped so dev/prod Dorkas don't share hooks through ~/.codex.
  if (opts.skipCodexHomeEnv) {
    delete baseEnv.CODEX_HOME
    delete baseEnv.DORKA_CODEX_HOME
    delete baseEnv.DORKA_CODEX_LAUNCH_PREFLIGHT
    ...
    baseEnv.DORKA_CODEX_HOME = opts.selectedCodexHomePath
    ...
    if (preflightCommand) {
      baseEnv.DORKA_CODEX_LAUNCH_PREFLIGHT = preflightCommand
    } else {
      delete baseEnv.DORKA_CODEX_LAUNCH_PREFLIGHT
    }
  } else if (opts.stripInheritedDorkaCodexHome) {
    stripInheritedDorkaCodexHomeOverride(baseEnv)
    delete baseEnv.DORKA_CODEX_LAUNCH_PREFLIGHT
  } else {
    delete baseEnv.DORKA_CODEX_LAUNCH_PREFLIGHT
  }

  // Why: WSL shells need the managed userData root for shell-ready wrappers; dev-mode terminals need the same export so `dorka` targets the live dev instance.
  if (opts.isWsl) {
    baseEnv.DORKA_USER_DATA_PATH = opts.userDataPath
    // Why: managed WSL registration uses `dorka-ide`; exposing that literal scopes agent guidance to WSL without a bare-dorka shim.
    baseEnv.DORKA_CLI_COMMAND = opts.isPackaged ? 'dorka-ide' : 'dorka-dev'
  } else {
    if (!opts.isPackaged) {
      baseEnv.DORKA_USER_DATA_PATH ??= opts.userDataPath
    }
    delete baseEnv.DORKA_CLI_COMMAND
  }
  prependDorkaCliDirToChildPath(baseEnv, {
    isPackaged: opts.isPackaged,
    userDataPath: opts.userDataPath,
    resourcesPath: opts.resourcesPath
  })
  ...
  if (
    ...
    const cliCommand = opts.isWsl ? (opts.isPackaged ? 'dorka-ide' : 'dorka-dev') : 'dorka'
