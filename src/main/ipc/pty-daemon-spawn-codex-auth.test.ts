  ...
  describe('spawn environment', () => {
    describe('daemon-active provider (parity with LocalPtyProvider)', () => {
      ...
      it('mirrors a user-provided OPENCODE_CONFIG_DIR into a source-scoped overlay on the daemon path', async () => {
        ...
        expect(env.DORKA_OPENCODE_SOURCE_CONFIG_DIR).toBe('/user/custom/opencode')
      })
      it('uses source OpenCode config env instead of remirroring a parent overlay', async () => {
        const env = await daemonSpawnAndGetEnv({
          ...
          DORKA_OPENCODE_SOURCE_CONFIG_DIR: '/user/custom/opencode'
        })
        expect(openCodeBuildPtyEnvMock).toHaveBeenCalledWith(
          expect.any(String),
          '/user/custom/opencode'
      ...
      it('installs Pi managed extensions without redirecting homes on the daemon path', async () => {
        ...
        expect(env.DORKA_PI_SOURCE_AGENT_DIR).toBe('/user/.pi/agent')
        expect(env.DORKA_OMP_CODING_AGENT_DIR).toBeUndefined()
        expect(env.DORKA_OMP_STATUS_EXTENSION).toBe(expectedOmpStatusExtension)
      })
      ...
      it('threads command: "omp" through to piBuildPtyEnv on the daemon path with OMP status metadata', async () => {
        ...
        expect(env.DORKA_OMP_STATUS_EXTENSION).toBe(
          '/user/.omp/agent/extensions/dorka-agent-status.ts'
        )
        expect(env.DORKA_OMP_SOURCE_AGENT_DIR).toBe('/user/.omp/agent')
        expect(env.DORKA_PI_CODING_AGENT_DIR).toBeUndefined()
        expect(env.DORKA_PI_SOURCE_AGENT_DIR).toBeUndefined()
      })
