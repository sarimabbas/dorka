...
describe('diagnostics IPC handlers', () => {
  beforeEach(() => {
    ...
    delete (globalThis as { DORKA_BUILD_IDENTITY?: unknown }).DORKA_BUILD_IDENTITY
    delete (globalThis as { DORKA_DIAGNOSTICS_TOKEN_URL?: unknown }).DORKA_DIAGNOSTICS_TOKEN_URL
    process.env.DORKA_DIAGNOSTICS_TOKEN_URL = 'https://diagnostics.example.com/diagnostics/token'
  ...
  it('pins official builds to the compile-time diagnostics endpoint', async () => {
    ...
    const globalOverrides = globalThis as {
      DORKA_BUILD_IDENTITY?: 'stable'
      DORKA_DIAGNOSTICS_TOKEN_URL?: string
    }
    globalOverrides.DORKA_BUILD_IDENTITY = 'stable'
    globalOverrides.DORKA_DIAGNOSTICS_TOKEN_URL = 'https://official.example.com/diagnostics/token'
    process.env.DORKA_DIAGNOSTICS_TOKEN_URL = 'https://attacker.example.com/diagnostics/token'
    collectDiagnosticBundleMock.mockReturnValue(bundle)
    readFileSyncMock.mockReturnValue(bundle.payload)
    const collect = handlers.get('diagnostics:collectBundle')!
    const openPreview = handlers.get('diagnostics:openBundlePreview')!
