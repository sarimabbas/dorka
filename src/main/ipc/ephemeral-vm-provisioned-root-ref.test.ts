...
beforeEach(() => {
  handlers.clear()
  userDataPath = mkdtempSync(join(tmpdir(), 'dorka-vm-ref-user-data-'))
  repoPath = mkdtempSync(join(tmpdir(), 'dorka-vm-ref-repo-'))
...
function writeRecipe(path: string, createCountPath: string, createEnvPath?: string): void {
  ...
  writeFileSync(
    ...
    [
      ...
      ...(createEnvPath
        ? [
            `require('node:fs').writeFileSync(${JSON.stringify(createEnvPath)}, JSON.stringify({ref:process.env.DORKA_REPO_REF,refHead:process.env.DORKA_REPO_REF_HEAD,repoUrl:process.env.DORKA_REPO_URL}))`
          ]
        : []),
