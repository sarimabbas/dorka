...
describe('local filesystem watcher canonical root paths', () => {
  ...
  it('reports symlink-resolved event paths under the subscribed root', async () => {
    const worktreePath = resolve('/tmp/dorka-link')
    const payload = await emitAndCapture(worktreePath, resolve('/private/tmp/dorka-real'), [
      { path: resolve('/private/tmp/dorka-real/src/agent-edit.ts'), type: 'update' }
    ])
    ...
    expect(payload.events).toEqual([
      {
        kind: 'update',
        absolutePath: resolve('/tmp/dorka-link/src/agent-edit.ts'),
        isDirectory: false
      }
    ])
  })
  ...
  it.runIf(process.platform === 'darwin' || process.platform === 'win32')(
    ...
    async () => {
      const worktreePath = resolve('/tmp/dorka-case/repo')
      const payload = await emitAndCapture(worktreePath, worktreePath, [
        { path: resolve('/tmp/dorka-case/Repo/src/agent-edit.ts'), type: 'update' }
      ])

      expect(payload.events[0]!.absolutePath).toBe(resolve('/tmp/dorka-case/repo/src/agent-edit.ts'))
    }
  )

  it('leaves already-matching event paths untouched', async () => {
    const worktreePath = resolve('/tmp/dorka-plain')
    const eventPath = resolve('/tmp/dorka-plain/src/agent-edit.ts')
    const payload = await emitAndCapture(worktreePath, worktreePath, [
