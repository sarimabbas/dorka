...
const runLiveBenchmark = isFederatedReadBenchmarkEnabled(process.env.DORKA_FEDERATED_READ_BENCH)
...
describe.runIf(runLiveBenchmark)('federated read RPC transport benchmark', () => {
  it('compares one-shot and shared-control latency on one saved runtime', async () => {
    const userDataPath = process.env.DORKA_RUNTIME_USER_DATA_PATH
    const environmentName = process.env.DORKA_RUNTIME_ENVIRONMENT
    if (!userDataPath || !environmentName) {
      throw new Error('Set DORKA_RUNTIME_USER_DATA_PATH and DORKA_RUNTIME_ENVIRONMENT.')
    }
    const store = RuntimeEnvironmentStoreSchema.parse(
      JSON.parse(readFileSync(join(userDataPath, 'dorka-environments.json'), 'utf8'))
    )
