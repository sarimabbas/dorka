...
describe('registerWorktreeHandlers', () => {
  ...
  it('force-removes a legacy Dorka-created orphaned worktree directory after Git tracking is gone', async () => {
    const parentDir = await mkdtemp(join(tmpdir(), 'dorka-ipc-orphan-'))
  ...
  it('prompts for force before removing an Dorka-created orphaned worktree directory', async () => {
    const parentDir = await mkdtemp(join(tmpdir(), 'dorka-ipc-orphan-'))
    const repoPath = join(parentDir, 'repo')
    const orphanPath = join(parentDir, 'orphan')
    const adminWorktreePath = join(repoPath, '.git', 'worktrees', 'orphan')
    ...
    store.getWorktreeMeta.mockReturnValue(
      makeWorktreeMeta({ dorkaCreatedAt: Date.now(), dorkaCreationSource: 'runtime' })
    )
  ...
  it('prompts then force-removes an Dorka-created unregistered leftover directory with no git marker', async () => {
    const parentDir = await mkdtemp(join(tmpdir(), 'dorka-ipc-leftover-'))
    const repoPath = join(parentDir, 'repo')
  ...
  it('rejects an Dorka-created unregistered local directory with a git directory', async () => {
    const parentDir = await mkdtemp(join(tmpdir(), 'dorka-ipc-standalone-'))
    const repoPath = join(parentDir, 'repo')
  ...
  it('does not inspect or delete a local path when SSH orphan cleanup has no filesystem provider', async () => {
    const localPath = await mkdtemp(join(tmpdir(), 'dorka-ipc-ssh-missing-fs-'))
    const repo = {
    ...
    store.getRepo.mockReturnValue(repo)
    store.getWorktreeMeta.mockReturnValue(
      makeWorktreeMeta({ dorkaCreatedAt: Date.now(), dorkaCreationSource: 'ssh' })
    )
