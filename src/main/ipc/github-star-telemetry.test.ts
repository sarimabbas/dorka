...
describe('registerGitHubHandlers', () => {
  ...
  it('emits app_starred_dorka once after a successful star with cohort context', async () => {
    starDorkaMock.mockResolvedValue(true)
    getCohortAtEmitMock.mockReturnValue({ nth_repo_added: 3 })

    registerGitHubHandlers(store as never, stats as never)

    await expect(handlers['gh:starDorka'](null, 'settings')).resolves.toBe(true)

    expect(starDorkaMock).toHaveBeenCalledTimes(1)
    expect(getCohortAtEmitMock).toHaveBeenCalledTimes(1)
    expect(trackMock).toHaveBeenCalledTimes(1)
    expect(trackMock).toHaveBeenCalledWith('app_starred_dorka', {
      source: 'settings',
      nth_repo_added: 3
    })
  })
  ...
  it('accepts every app star source for success telemetry', async () => {
    ...
    for (const source of [
      ...
      await expect(handlers['gh:starDorka'](null, source)).resolves.toBe(true)
    }
  ...
  it('does not emit app_starred_dorka when the star action returns false', async () => {
    starDorkaMock.mockResolvedValue(false)

    registerGitHubHandlers(store as never, stats as never)

    await expect(handlers['gh:starDorka'](null, 'landing')).resolves.toBe(false)
  ...
  it('does not emit app_starred_dorka when the star action throws', async () => {
    starDorkaMock.mockRejectedValue(new Error('gh failed'))

    registerGitHubHandlers(store as never, stats as never)

    await expect(handlers['gh:starDorka'](null, 'star_nag')).rejects.toThrow('gh failed')
  ...
  it('preserves star result but skips telemetry for an invalid IPC source', async () => {
    ...
    await expect(handlers['gh:starDorka'](null, 'github_website')).resolves.toBe(true)
