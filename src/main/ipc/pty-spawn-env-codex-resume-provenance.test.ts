...
describe('registerPtyHandlers', () => {
  ...
  describe('spawn environment', () => {
    describe('unverifiable Codex resume provenance', () => {
      ...
      posixOnlyIt(
        'launches plain codex when a REAL rollout sits under a home Dorka no longer trusts',
        async () => {
      ...
      it('strips the resume argv from the sequenced startup command too', async () => {
        ...
        await spawnCodexResume(ORIGIN_ROLLOUT, {
          env: { DORKA_SEQUENCED_STARTUP_COMMAND: `codex 'resume' '${RESUME_SESSION_ID}'` }
        })

        const spawnOptions = daemonSpawn.mock.calls.at(-1)![0]
        expect(spawnOptions.env.DORKA_SEQUENCED_STARTUP_COMMAND).toBe('codex')
      ...
      posixOnlyIt(
        ...
        async () => {
          ...
          const env = spawnMock.mock.calls.at(-1)![2].env as Record<string, string>
          expect(env.DORKA_SEQUENCED_STARTUP_COMMAND).toBe('codex')
        }
      )
      posixOnlyIt(
        ...
        async () => {
          ...
          const env = spawnMock.mock.calls.at(-1)![2].env as Record<string, string>
          expect(env.DORKA_SEQUENCED_STARTUP_COMMAND).toBe(sequenced)
        }
      )
