...
describe('migrateForkRemoteRefspecsWithExec', () => {
  it('narrows a wide-refspec remote to the known branch and deletes the strays left by the old wide fetch', async () => {
    ...
    const exec = makeExec({
      urlByRemote: { [FORK_REMOTE]: 'git@github.com:contributor/dorka.git\n' },
      fetchByRemote: { [FORK_REMOTE]: ['+refs/heads/*:refs/remotes/pr-contributor-dorka/*'] },
      trackingRefsByRemote
    })
  ...
  it('unions branches from multiple worktrees sharing the same fork remote', async () => {
    ...
    expect(addedRefspecs).toEqual(
      expect.arrayContaining([
        '+refs/heads/branch-a*:refs/remotes/pr-contributor-dorka/branch-a*',
        '+refs/heads/branch-b*:refs/remotes/pr-contributor-dorka/branch-b*'
      ])
    )
  })
  ...
  it('clears the fetch refspec of a wide pr-* remote with zero worktree-metadata trace at all', async () => {
    const ORPHAN_REMOTE = 'pr-ghost-dorka'
    const trackingRefsByRemote = { [ORPHAN_REMOTE]: ['some-branch', 'another-branch'] }
    const exec = makeExec({
      remoteNames: [ORPHAN_REMOTE],
      urlByRemote: { [ORPHAN_REMOTE]: 'git@github.com:ghost/dorka.git\n' },
      fetchByRemote: { [ORPHAN_REMOTE]: ['+refs/heads/*:refs/remotes/pr-ghost-dorka/*'] },
      trackingRefsByRemote
    })
  ...
  it('leaves a zero-provenance pr-* remote alone if its refspec is not the stock wide default', async () => {
    const CUSTOM_REMOTE = 'pr-custom-dorka'
    const exec = makeExec({
      remoteNames: [CUSTOM_REMOTE],
      urlByRemote: { [CUSTOM_REMOTE]: 'git@github.com:custom/dorka.git\n' },
      fetchByRemote: {
        [CUSTOM_REMOTE]: ['+refs/heads/some-branch:refs/remotes/pr-custom-dorka/some-branch']
      }
    })
  ...
  it('also narrows branches only referenced by surviving branch.*.remote config (no metadata left)', async () => {
    ...
    const migrated = await migrateForkRemoteRefspecsWithExec(
      REPO_PATH,
      REPO_ID,
      // Only proof of Dorka provenance; the branch itself comes from local config.
    ...
    expect(addedRefspecs).toEqual(
      expect.arrayContaining([
        '+refs/heads/contributor/preserved*:refs/remotes/pr-contributor-dorka/contributor/preserved*'
      ])
    )
  })
})
