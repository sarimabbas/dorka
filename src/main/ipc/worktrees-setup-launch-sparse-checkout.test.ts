...
describe('registerWorktreeHandlers', () => {
  ...
  it('returns a setup launch payload when setup should run', async () => {
    ...
    expect(result).toMatchObject({
      ...
      setup: {
        runnerScriptPath: '/workspace/repo/.git/dorka/setup-runner.sh',
        envVars: {
  ...
  it('launches setup even when primary and worktree dorka.yaml scripts diverge', async () => {
    // Why: benign dorka.yaml divergence must not disable setup (regression from #1280 content-equality gate); repo trust already gates execution.
    listWorktreesMock.mockResolvedValue(createdWorktreeList)
