...
describe('registerPtyHandlers', () => {
  ...
  describe('spawn environment', () => {
    ...
    it('injects the OpenCode hook env into Dorka terminal PTYs', async () => {
      ...
      expect(env.DORKA_OPENCODE_HOOK_PORT).toBe('4567')
      expect(env.DORKA_OPENCODE_HOOK_TOKEN).toBe('opencode-token')
      expect(env.DORKA_OPENCODE_PTY_ID).toBe('test-pty')
      expect(env.OPENCODE_CONFIG_DIR).toEqual(expect.any(String))
      expect(env.DORKA_OPENCODE_CONFIG_DIR).toBe(env.OPENCODE_CONFIG_DIR)
    })
    it('mirrors the original OpenCode source dir when launched from an Dorka overlay shell', async () => {
      const env = await spawnAndGetEnv({
        OPENCODE_CONFIG_DIR: '/tmp/parent-dorka-opencode-overlay',
        DORKA_OPENCODE_SOURCE_CONFIG_DIR: '/tmp/user-opencode-config'
      })
      expect(openCodeBuildPtyEnvMock).toHaveBeenCalledWith(
        expect.any(String),
        '/tmp/user-opencode-config'
      )
      expect(env.OPENCODE_CONFIG_DIR).toBe('/tmp/dorka-opencode-overlay')
      expect(env.DORKA_OPENCODE_CONFIG_DIR).toBe('/tmp/dorka-opencode-overlay')
      expect(env.DORKA_OPENCODE_SOURCE_CONFIG_DIR).toBe('/tmp/user-opencode-config')
    })
    it('does not treat inherited Dorka OpenCode config as user config without a source dir', async () => {
      const env = await spawnAndGetEnv({
        OPENCODE_CONFIG_DIR: '/tmp/parent-dorka-opencode-overlay',
        DORKA_OPENCODE_CONFIG_DIR: '/tmp/parent-dorka-opencode-overlay'
      })

      expect(openCodeBuildPtyEnvMock).toHaveBeenCalledWith(expect.any(String), undefined)
      expect(env.OPENCODE_CONFIG_DIR).toBe('/tmp/dorka-opencode-config')
      expect(env.DORKA_OPENCODE_CONFIG_DIR).toBe('/tmp/dorka-opencode-config')
      expect(env.DORKA_OPENCODE_SOURCE_CONFIG_DIR).toBeUndefined()
    })
    it('restores user OpenCode config when agent status hooks are disabled in a nested Dorka shell', async () => {
      const env = await spawnAndGetEnv(
        {
          OPENCODE_CONFIG_DIR: '/tmp/parent-dorka-opencode-overlay',
          DORKA_OPENCODE_CONFIG_DIR: '/tmp/parent-dorka-opencode-overlay',
      ...
      expect(env.DORKA_OPENCODE_CONFIG_DIR).toBeUndefined()
      expect(env.DORKA_OPENCODE_SOURCE_CONFIG_DIR).toBeUndefined()
    })
    ...
    it('injects MiMo overlay env only when launch command is mimo', async () => {
      ...
      expect(env.MIMOCODE_HOME).toBe('/tmp/dorka-mimocode-shared')
      expect(env.DORKA_MIMOCODE_HOME).toBe('/tmp/dorka-mimocode-shared')
      expect(env.DORKA_MIMOCODE_SOURCE_HOME).toBeUndefined()
    })
    ...
    it('restores user MiMo home when agent status hooks are disabled in a nested Dorka shell', async () => {
      const env = await spawnAndGetEnv(
        {
          MIMOCODE_HOME: '/tmp/parent-dorka-mimocode-overlay',
          DORKA_MIMOCODE_HOME: '/tmp/parent-dorka-mimocode-overlay',
          DORKA_MIMOCODE_SOURCE_HOME: '/tmp/user-mimocode-home'
        },
        undefined,
        undefined,
        () => ({ agentStatusHooksEnabled: false }),
      ...
      expect(env.DORKA_MIMOCODE_HOME).toBeUndefined()
      expect(env.DORKA_MIMOCODE_SOURCE_HOME).toBeUndefined()
    })
    posixOnlyIt(
      'reproduces issue #1534: GUI-launched Dorka mirrors zshrc-only OpenCode config',
      async () => {
        ...
        const env = await spawnAndGetEnv(undefined, {
          ...
          DORKA_OPENCODE_SOURCE_CONFIG_DIR: undefined
        })
        ...
        expect(env.OPENCODE_CONFIG_DIR).toBe('/tmp/dorka-opencode-overlay')
        expect(env.DORKA_OPENCODE_CONFIG_DIR).toBe('/tmp/dorka-opencode-overlay')
        expect(env.DORKA_OPENCODE_SOURCE_CONFIG_DIR).toBe('/home/pim/company/opencode-config')
        expect(env.OPENCODE_CONFIG_DIR).not.toBe(env.DORKA_OPENCODE_SOURCE_CONFIG_DIR)
      }
    )
    it('installs Pi managed extensions without redirecting Dorka terminal PTY homes', async () => {
      ...
      expect(env.DORKA_PI_CODING_AGENT_DIR).toBeUndefined()
      expect(env.DORKA_PI_SOURCE_AGENT_DIR).toBe('/tmp/user-pi-agent')
      expect(env.DORKA_OMP_CODING_AGENT_DIR).toBeUndefined()
      expect(env.DORKA_OMP_STATUS_EXTENSION).toBe(
        '/tmp/dorka-user-data/omp-managed-status-extension/dorka-agent-status.ts'
      )
      expect(env.DORKA_OMP_SOURCE_AGENT_DIR).toBeUndefined()
    })
    it('does not materialize a missing Pi home when another agent mentions Pi', async () => {
      ...
      expect(env.DORKA_PI_SOURCE_AGENT_DIR).toBeUndefined()
    })
    it('materializes Pi home for an explicit Pi launch through a custom command', async () => {
      ...
      expect(env.DORKA_PI_SOURCE_AGENT_DIR).toBe('/tmp/default-pi-agent')
    })
    it.each([
      'omp',
      ...
      // Why: OMP launches emit DORKA_OMP_* shadow vars, not Pi-named ones; only PI_CODING_AGENT_DIR stays (OMP's own binary reads it).
      const env = await spawnAndGetEnv(
        undefined,
      ...
      expect(env.DORKA_OMP_CODING_AGENT_DIR).toBeUndefined()
      expect(env.DORKA_OMP_STATUS_EXTENSION).toBe(
        '/tmp/user-omp-agent/extensions/dorka-agent-status.ts'
      )
      expect(env.DORKA_OMP_SOURCE_AGENT_DIR).toBe('/tmp/user-omp-agent')
    ...
    it('installs Prime status into its independent agent dir on explicit launch', async () => {
      ...
      expect(env.PI_CODING_AGENT_DIR).toBe('/tmp/user-pi-agent')
      expect(env.DORKA_PRIME_AGENT_SOURCE_AGENT_DIR).toBe('/tmp/user-prime-agent')
      expect(env.DORKA_PI_SOURCE_AGENT_DIR).toBeUndefined()
      expect(env.DORKA_OMP_SOURCE_AGENT_DIR).toBeUndefined()
    })
    ...
    it('mirrors the original Pi source dir when launched from an Dorka overlay shell', async () => {
      const env = await spawnAndGetEnv({
        PI_CODING_AGENT_DIR: '/tmp/parent-dorka-pi-overlay',
        DORKA_PI_SOURCE_AGENT_DIR: '/tmp/user-pi-agent'
      })
      expect(piBuildPtyEnvMock).toHaveBeenCalledWith(
        expect.any(String),
        '/tmp/user-pi-agent',
      ...
      expect(env.PI_CODING_AGENT_DIR).toBe('/tmp/parent-dorka-pi-overlay')
      expect(env.DORKA_PI_CODING_AGENT_DIR).toBeUndefined()
      expect(env.DORKA_PI_SOURCE_AGENT_DIR).toBe('/tmp/user-pi-agent')
    })
  })
