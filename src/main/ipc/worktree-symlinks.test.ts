...
describe('createWorktreeSymlinks', () => {
  ...
  beforeEach(() => {
    root = mkdtempSync(join(tmpdir(), 'dorka-symlinks-'))
  ...
  it('does not overwrite a file target that appears before APFS clone-copy is published', async () => {
    ...
    expect(existsSync(join(worktree, '.dorka-apfs-clone-file-race'))).toBe(false)
    expect(warn).not.toHaveBeenCalled()
    expect(error).not.toHaveBeenCalled()
  })
...
describe('createWorktreeSharedPaths', () => {
  ...
  beforeEach(() => {
    root = mkdtempSync(join(tmpdir(), 'dorka-sharedpaths-'))
    primary = join(root, 'primary')
    worktree = join(root, 'worktree')
    mkdirSync(primary, { recursive: true })
    mkdirSync(worktree, { recursive: true })
...
describe('createWorktreeCopiedPaths', () => {
  ...
  beforeEach(() => {
    root = mkdtempSync(join(tmpdir(), 'dorka-copiedpaths-'))
    primary = join(root, 'primary')
    worktree = join(root, 'worktree')
    mkdirSync(primary, { recursive: true })
    mkdirSync(worktree, { recursive: true })
...
describe('removeWorktreeSymlinks', () => {
  ...
  beforeEach(() => {
    root = mkdtempSync(join(tmpdir(), 'dorka-unlink-'))
    primary = join(root, 'primary')
    worktree = join(root, 'worktree')
    mkdirSync(primary, { recursive: true })
    mkdirSync(worktree, { recursive: true })
