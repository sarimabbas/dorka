...
describe('registerPtyHandlers', () => {
  ...
  describe('spawn environment', () => {
    it('does not use an inherited Pi overlay source for an OMP launch', async () => {
      ...
      expect(env.DORKA_OMP_SOURCE_AGENT_DIR).toBe('/tmp/default-omp-agent')
    ...
    it('does not use an inherited OMP overlay source for an explicit Pi launch', async () => {
      const env = await spawnAndGetEnv(
        {
          PI_CODING_AGENT_DIR: '/tmp/parent-dorka-omp-overlay',
          DORKA_OMP_CODING_AGENT_DIR: '/tmp/parent-dorka-omp-overlay',
          DORKA_OMP_SOURCE_AGENT_DIR: '/tmp/user-omp-agent'
        },
        undefined,
        undefined,
        undefined,
    ...
    it('restores user Pi config when agent status hooks are disabled in a nested Dorka shell', async () => {
      const env = await spawnAndGetEnv(
        {
          PI_CODING_AGENT_DIR: '/tmp/parent-dorka-pi-overlay',
          DORKA_PI_CODING_AGENT_DIR: '/tmp/parent-dorka-pi-overlay',
      ...
      const env = await spawnAndGetEnv(
        {
          ...
          DORKA_PRIME_AGENT_SOURCE_AGENT_DIR: '/tmp/user-prime-agent'
        },
        undefined,
        undefined,
        () => ({ agentStatusHooksEnabled: false })
    ...
    posixOnlyIt(
      ...
      async () => {
        ...
        expect(env.DORKA_PI_CODING_AGENT_DIR).toBeUndefined()
        expect(env.DORKA_PI_SOURCE_AGENT_DIR).toBe('/home/tester/.config/pi-agent')
      }
    )
    it('injects the agent hook receiver env into Dorka terminal PTYs', async () => {
      const env = await spawnAndGetEnv()
    ...
    it('strips stale inherited hook receiver env before injecting this runtime', async () => {
      ...
      expect(env.DORKA_AGENT_HOOK_PORT).toBe('5678')
      expect(env.DORKA_AGENT_HOOK_TOKEN).toBe('agent-token')
      expect(env.DORKA_AGENT_HOOK_ENV).toBeUndefined()
      expect(env.DORKA_AGENT_HOOK_VERSION).toBeUndefined()
      expect(env.DORKA_AGENT_HOOK_ENDPOINT).toBeUndefined()
      expect(env.DORKA_CLAUDE_AGENT_STATUS_SETTINGS).toBeUndefined()
    })
    it('does not leak inherited hook receiver env if the hook server is unavailable', async () => {
      ...
      expect(env.DORKA_AGENT_HOOK_TOKEN).toBeUndefined()
      expect(env.DORKA_AGENT_HOOK_ENV).toBeUndefined()
      expect(env.DORKA_AGENT_HOOK_VERSION).toBeUndefined()
      expect(env.DORKA_AGENT_HOOK_ENDPOINT).toBeUndefined()
      expect(env.DORKA_CLAUDE_AGENT_STATUS_SETTINGS).toBeUndefined()
    })
    it('overrides ambient CODEX_HOME with the Dorka-managed home for system default', async () => {
      const env = await spawnAndGetEnv(
        undefined,
    ...
    it('waits for managed Codex auth before spawning a local PTY', async () => {
      ...
      expect(spawnMock.mock.calls.at(-1)?.[2].env).toMatchObject({
        ...
        DORKA_CODEX_HOME: TEST_CODEX_HOME
      })
    })
    ...
    it('strips a nested-Dorka override for system default when the real-home flag is ON', async () => {
      const env = await spawnAndGetEnv(
    ...
    it('lets the resolver keep a per-spawn custom CODEX_HOME on the managed lane', async () => {
      ...
      const env = await spawnAndGetEnv(
        ...
        { CODEX_HOME: undefined, DORKA_CODEX_HOME: undefined },
        resolveHome,
        () => ({ codexSystemDefaultRealHomeEnabled: true }) as never
      )
