...
describe('the PATH fallback, run by a real POSIX shell', () => {
  ...
  itPosix('finds an nvm-installed CLI a degraded probe would have missed', async () => {
    ...
    const home = mkdtempSync(join(tmpdir(), 'dorka-wsl-cmd-'))
    try {
      const bin = join(home, '.nvm/versions/node/v20.1.0/bin')
      mkdirSync(bin, { recursive: true })
      writeFileSync(join(bin, 'dorka-fake-cli'), '#!/bin/sh\necho cli-ok\n')
      chmodSync(join(bin, 'dorka-fake-cli'), 0o755)
      ...
      await runPreflightCommandInWsl({ distro: 'Ubuntu' }, 'command -v dorka-fake-cli', 5000)
      ...
      expect(String(execFileSync('/bin/sh', ['-c', script], options))).toContain(
        '.nvm/versions/node/v20.1.0/bin/dorka-fake-cli'
      )
    } finally {
  ...
  itPosix('appends rather than prepends, so a resolved PATH still wins', async () => {
    const home = mkdtempSync(join(tmpdir(), 'dorka-wsl-cmd-'))
    const real = mkdtempSync(join(tmpdir(), 'dorka-wsl-real-'))
    try {
      for (const [dir, body] of [
        ...
        writeFileSync(join(dir, 'dorka-fake-cli'), `#!/bin/sh\necho ${body}\n`)
        chmodSync(join(dir, 'dorka-fake-cli'), 0o755)
      }
      runWslProcessMock.mockResolvedValue({
        environmentResolved: true,
        code: 0,
      ...
      await runPreflightCommandInWsl({ distro: 'Ubuntu' }, 'dorka-fake-cli', 5000)
      const script = String(runWslProcessMock.mock.calls.at(-1)?.[0].script)
      const options: ExecFileSyncOptions = {
        encoding: 'utf8',
