...
describe('usage provider IPC handlers', () => {
  it('registers every provider route and forwards query arguments', () => {
    ...
    call('claudeUsage', 'getSnapshot', { scope: 'dorka', range: '30d', limit: 7 })
    call('claudeUsage', 'getSummary', { scope: 'all', range: '7d' })
    call('claudeUsage', 'getDaily', { scope: 'dorka', range: '90d' })
    call('claudeUsage', 'getBreakdown', { scope: 'all', range: 'all', kind: 'model' })
    call('claudeUsage', 'getRecentSessions', { scope: 'dorka', range: '30d', limit: 4 })
    ...
    expect(claudeUsage.getSnapshot).toHaveBeenCalledWith('dorka', '30d', 7)
    expect(claudeUsage.getSummary).toHaveBeenCalledWith('all', '7d')
    expect(claudeUsage.getDaily).toHaveBeenCalledWith('dorka', '90d')
    expect(claudeUsage.getBreakdown).toHaveBeenCalledWith('all', 'all', 'model')
    expect(claudeUsage.getRecentSessions).toHaveBeenCalledWith('dorka', '30d', 4)
  })
})
