...
describe('registerPtyHandlers', () => {
  ...
  describe('spawn environment', () => {
    describe('daemon-active provider (parity with LocalPtyProvider)', () => {
      ...
      it('strips DORKA_PANE_KEY/TAB_ID/WORKTREE_ID from SSH spawn env when remote agent hooks are disabled', async () => {
        ...
        const prevFlag = process.env.DORKA_FEATURE_REMOTE_AGENT_HOOKS
        ...
        try {
          ...
          if (prevFlag === undefined) {
            ...
            process.env.DORKA_FEATURE_REMOTE_AGENT_HOOKS = prevFlag
          }
        }
      })
      it('forwards DORKA_PANE_KEY/TAB_ID/WORKTREE_ID over SSH by default', async () => {
        const sshSpawn = vi.fn(async (_opts: { env: Record<string, string> }) => ({
          id: 'ssh-pty'
        }))
        registerSshPtyProvider('ssh-1', {
        ...
        try {
          ...
          const env = sshSpawn.mock.calls.at(-1)![0].env
          expect(env.DORKA_PANE_KEY).toBe(paneKey)
          expect(env.DORKA_TAB_ID).toBe('tab-2')
          expect(env.DORKA_WORKTREE_ID).toBe('wt-2')
