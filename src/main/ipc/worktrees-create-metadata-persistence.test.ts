...
describe('registerWorktreeHandlers', () => {
  ...
  it('strips Dorka provenance fields from renderer metadata updates', () => {
  ...
  it('defers the fork-PR remote during local create and persists the target unmaterialized', async () => {
    ...
    expect(gitExecFileAsyncMock).not.toHaveBeenCalledWith(
      [
        ...
        '--no-tags',
        'pr-prateek-dorka',
        'git@github.com:prateek/dorka.git'
      ],
      { cwd: '/workspace/repo' }
    )
    expect(gitExecFileAsyncMock).not.toHaveBeenCalledWith(
      [
        'fetch',
        'pr-prateek-dorka',
        '+refs/heads/prateek/fix-sidebar-agents-toggle*:refs/remotes/pr-prateek-dorka/prateek/fix-sidebar-agents-toggle*'
      ],
      { cwd: '/workspace/repo' }
    )
    ...
    expect(gitExecFileAsyncMock).not.toHaveBeenCalledWith(
      [
        'branch',
        '--set-upstream-to',
        'pr-prateek-dorka/prateek/fix-sidebar-agents-toggle',
        'improve-dashboard'
      ],
  ...
  // Was "keeps the Dorka-created marker ...": create used to inherit the marker while
  ...
  it('does not claim the Dorka-created marker at create when a sibling worktree minted the fork remote', async () => {
    listWorktreesMock.mockResolvedValue([
      {
        path: '/workspace/improve-dashboard',
        head: 'abc123',
    ...
    gitExecFileAsyncMock.mockImplementation(async (args: string[]) => {
      if (args[0] === 'remote' && args.length === 1) {
        return { stdout: 'pr-contributor-dorka\n', stderr: '' }
      }
      if (args[0] === 'remote' && args[1] === 'get-url') {
        return { stdout: 'https://github.com/contributor/dorka.git\n', stderr: '' }
      }
  ...
  it('threads explicit origin preference into dual-remote PR head resolution', async () => {
    ...
    expect(gitExecFileAsyncMock).toHaveBeenCalledWith(
      [
        ...
        '--no-tags',
        'origin',
        `+refs/pull/1738/head:refs/dorka/pull/${ORIGIN_HEAD_COMPONENT}/1738`
      ],
