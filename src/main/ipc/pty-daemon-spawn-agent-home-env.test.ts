  ...
  describe('spawn environment', () => {
    describe('daemon-active provider (parity with LocalPtyProvider)', () => {
      ...
      it('keeps the authoritative home for runtime-created daemon resumes', async () => {
        ...
        expect(spawnOptions.env.DORKA_CODEX_HOME).toBe(systemHome)
      ...
      it('skips host Codex home when a daemon-backed Windows spawn targets a WSL cwd', async () => {
        ...
        try {
          const spawnOptions = await daemonSpawnAndGetOptions(
            {},
            () => 'C:\\Users\\test\\AppData\\Roaming\\Dorka\\codex-runtime-home\\home',
            undefined,
            {
              CODEX_HOME: 'C:\\Users\\test\\AppData\\Roaming\\Dorka\\codex-runtime-home\\home',
              DORKA_CODEX_HOME: 'C:\\Users\\test\\AppData\\Roaming\\Dorka\\codex-runtime-home\\home'
            },
            {
          ...
          expect(env.DORKA_CODEX_HOME).toBeUndefined()
          expect(spawnOptions.envToDelete).toEqual(
            expect.arrayContaining(['CODEX_HOME', 'DORKA_CODEX_HOME'])
          )
        } finally {
          Object.defineProperty(process, 'platform', {
            configurable: true,
      ...
      it('skips host Codex home when a daemon-backed Windows spawn uses a WSL shell override', async () => {
        ...
        try {
          ...
          expect(spawnOptions.env.DORKA_CODEX_HOME).toBeUndefined()
          expect(spawnOptions.envToDelete).toEqual(
            expect.arrayContaining(['CODEX_HOME', 'DORKA_CODEX_HOME'])
          )
        } finally {
      ...
      it('does not install or inject a Prime extension for an explicit WSL launch', async () => {
        await withWin32Platform(async () => {
          ...
          expect(env.DORKA_PRIME_AGENT_SOURCE_AGENT_DIR).toBeUndefined()
          expect(env.DORKA_PRIME_AGENT_STATUS_EXTENSION).toBeUndefined()
          expect(env.DORKA_WSL_HOOK_INSTANCE).toBeUndefined()
      ...
      it('points OPENCODE_CONFIG_DIR at the guest overlay when the WSL relay reports it', async () => {
        const guestDir = '/home/jin/.dorka-relay/opencode-overlays/abc'
        const spy = vi.spyOn(wslHookRelayManager, 'getOpenCodeOverlayDir').mockReturnValue(guestDir)
        try {
          await withWin32Platform(async () => {
            const env = await daemonSpawnAndGetEnv(
              { DORKA_OPENCODE_SOURCE_CONFIG_DIR: '/home/jin/.config/opencode' },
              undefined,
              undefined,
              undefined,
              { shellOverride: 'wsl.exe' }
            ...
            expect(env.DORKA_OPENCODE_CONFIG_DIR).toBe(guestDir)
      ...
      it('strips the daemon-inherited Dorka-owned CODEX_HOME for real-home routing', async () => {
        const spawnOptions = await daemonSpawnAndGetOptions(
          {},
          () => null,
          () => ({ codexSystemDefaultRealHomeEnabled: true }) as never,
          { CODEX_HOME: '/managed/home', DORKA_CODEX_HOME: '/managed/home' }
        )
        expect(spawnOptions.env.CODEX_HOME).toBeUndefined()
        expect(spawnOptions.env.DORKA_CODEX_HOME).toBeUndefined()
        expect(spawnOptions.envToDelete).toEqual(expect.arrayContaining(['DORKA_CODEX_HOME']))
      ...
      it('preserves a daemon-inherited user CODEX_HOME for real-home routing', async () => {
        const spawnOptions = await daemonSpawnAndGetOptions(
          ...
          () => null,
          () => ({ codexSystemDefaultRealHomeEnabled: true }) as never,
          { CODEX_HOME: '/home/me/.config/codex', DORKA_CODEX_HOME: undefined }
        )
        expect(spawnOptions.envToDelete).toEqual(expect.arrayContaining(['DORKA_CODEX_HOME']))
      ...
      it('prepends the bare-dorka CLI shim dir to PATH for packaged Linux spawns', async () => {
        const originalPlatform = process.platform
        Object.defineProperty(process, 'platform', {
          configurable: true,
          value: 'linux'
        ...
        try {
          ...
          const shimDir = join('/tmp/dorka-user-data', 'linux-dorka-cli-shim')
          // Why: bare `dorka` must resolve to the Dorka CLI before /usr/bin/dorka (the GNOME screen reader) in Dorka terminals (#7904).
      ...
      it('prepends the bundled CLI dir to PATH for packaged macOS spawns', async () => {
        ...
        Object.defineProperty(process, 'resourcesPath', {
          configurable: true,
          value: '/tmp/dorka-resources'
        })
        try {
          const env = await daemonSpawnAndGetEnv({ PATH: '/usr/bin' })
          expect(env.PATH.split(delimiter)[0]).toBe(join('/tmp/dorka-resources', 'bin'))
        } finally {
      ...
      it('deletes stale Claude scoped settings env from daemon-hosted PTYs', async () => {
        ...
        expect(spawnOptions.env.DORKA_CLAUDE_AGENT_STATUS_SETTINGS).toBeUndefined()
        expect(spawnOptions.envToDelete).toEqual(
          expect.arrayContaining(['DORKA_CLAUDE_AGENT_STATUS_SETTINGS'])
        )
        expect(spawnOptions.env.DORKA_AGENT_HOOK_PORT).toBe('5678')
        expect(spawnOptions.env.DORKA_AGENT_HOOK_TOKEN).toBe('agent-token')
      })
      it('asks surviving pre-upgrade daemons to delete legacy attribution env', async () => {
        ...
        expect(spawnOptions.envToDelete).not.toContain('DORKA_REAL_GIT')
        expect(spawnOptions.envToDelete).not.toContain('DORKA_REAL_GH')
      })
      it('deletes stale Claude scoped settings env from runtime-created daemon PTYs', async () => {
        ...
        process.env.DORKA_CLAUDE_AGENT_STATUS_SETTINGS =
          '/tmp/dorka/agent-hooks/claude-agent-status-settings.json'
        handlers.clear()
      ...
      it('strips inherited Claude child-session stamps from runtime-created PTYs', async () => {
        // Why: the runtime controller is the `dorka` CLI / automation spawn path and
