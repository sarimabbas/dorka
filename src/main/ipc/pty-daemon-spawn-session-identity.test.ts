  ...
  describe('spawn environment', () => {
    describe('daemon-active provider (parity with LocalPtyProvider)', () => {
      ...
      it('falls back to process.env.PI_CODING_AGENT_DIR when baseEnv lacks it on the daemon path', async () => {
        ...
        expect(env.DORKA_PI_SOURCE_AGENT_DIR).toBe('/ambient/pi/agent')
      })
      it('does not mutate the caller-provided args.env on the daemon path', async () => {
        // Why: the handler clones baseEnv so IPC-provided env stays pristine; a regression would leak Dorka host env back into the renderer's reused copy.
        const daemonSpawn = setupDaemonAdapter()
        ...
        expect(spawnEnv.DORKA_AGENT_HOOK_PORT).toBe('5678')
      ...
      it('does NOT inject host-local env on SSH spawns (connectionId set)', async () => {
        ...
        await handlers.get('pty:spawn')!(null, {
          cols: 80,
          rows: 24,
          env: { FOO: 'bar', DORKA_PANE_KEY: makePaneKey('tab-1', leafId) },
          connectionId: 'ssh-1',
          worktreeId: 'wt-1',
          tabId: 'tab-1',
          leafId
        ...
        await handlers.get('pty:spawn')!(null, {
          cols: 80,
          rows: 24,
          env: { DORKA_PANE_KEY: 'tab-1:pane:1' },
          connectionId: 'ssh-1',
          worktreeId: 'wt-1',
          tabId: 'tab-1',
          leafId: 'pane:1'
        ...
        expect(legacySpawnOptions?.env.DORKA_PANE_KEY).toBeUndefined()
