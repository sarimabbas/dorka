...
export function installMainThreadHangWatchdog(options: {
  ...
  if (!app.isPackaged && process.env.DORKA_HANG_WATCHDOG_FORCE !== '1') {
    return null
  }
  ...
  const workerData: HangWatchdogWorkerData = {
    ...
    timeoutMs: positiveTiming(process.env.DORKA_HANG_WATCHDOG_TIMEOUT_MS, HANG_WATCHDOG_TIMEOUT_MS),
    checkIntervalMs: positiveTiming(
      process.env.DORKA_HANG_WATCHDOG_CHECK_INTERVAL_MS,
      HANG_WATCHDOG_CHECK_INTERVAL_MS
    )
  }
  let worker: Worker
  try {
    ...
    worker = new Worker(workerPath, {
      name: 'dorka-main-thread-hang-watchdog',
      workerData
    })
  } catch (error) {
