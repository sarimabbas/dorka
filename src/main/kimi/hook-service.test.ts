...
// homedir()/.dorka, and getKimiHome() honors KIMI_CODE_HOME. Point both at a
// temp dir so the local install/remove cycle never touches the real ~/.dorka or
...
beforeEach(() => {
  home = mkdtempSync(join(tmpdir(), 'dorka-kimi-hook-'))
...
const scriptPath = (): string => join(home, '.dorka', 'agent-hooks', 'kimi-hook.sh')
