...
// bounded orphan recovery all agree on what counts as an Dorka-written hook.
...
function getManagedScript(target: 'local' | 'posix' = 'local'): string {
  ...
  const endpointRefreshAndGuard = [
    // Why: refresh PORT/TOKEN/ENV/VERSION from the current Dorka install so a PTY
    // that survived an Dorka restart still reaches the live listener. See
  ...
  return [
    ...
    'printf \'%s\' "$payload" | curl -sS -X POST "http://127.0.0.1:${DORKA_AGENT_HOOK_PORT}/hook/kimi" \\',
...
export class KimiHookService {
  ...
  // Why: install Dorka's managed Kimi hooks on a remote box over SFTP, mirroring
