...
export const DORKA_REPO = 'stablyai/orca'
...
/**
 * Check if the authenticated user has starred the Dorka repo.
 * Returns true if starred, false if not, null if unable to determine (gh unavailable).
 */
export function checkDorkaStarred(): Promise<boolean | null> {
  ...
  inFlightStarCheck ??= runDorkaStarredCheck().finally(() => {
...
export function __resetDorkaStarCheckForTests(): void {
  inFlightStarCheck = null
}

async function runDorkaStarredCheck(): Promise<boolean | null> {
  await acquire()
  try {
    const { stdout, stderr } = await ghExecFileAsync(
      ['api', '--include', `user/starred/${DORKA_REPO}`],
...
/**
 * Star the Dorka repo for the authenticated user.
 */
export async function starDorka(): Promise<boolean> {
  await acquire()
  try {
    await ghExecFileAsync(['api', '-X', 'PUT', `user/starred/${DORKA_REPO}`], {
      encoding: 'utf-8',
