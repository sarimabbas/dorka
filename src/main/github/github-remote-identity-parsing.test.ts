...
describe('parseGitHubRemoteIdentity', () => {
  it('parses a plain github.com https remote', () => {
    expect(parseGitHubRemoteIdentity('https://github.com/team/dorka.git')).toEqual({
      host: 'github.com',
      owner: 'team',
      repo: 'dorka'
    })
  ...
  it('parses an SCP-style github.com remote', () => {
    expect(parseGitHubRemoteIdentity('git@github.com:team/dorka.git')).toEqual({
      host: 'github.com',
      owner: 'team',
      repo: 'dorka'
    })
  ...
  it('preserves a custom port on a GHES https remote', () => {
    ...
    expect(parseGitHubRemoteIdentity('https://ghe.acme.com:8443/team/dorka.git')).toEqual({
  ...
  it('preserves a custom port on a GHES http remote', () => {
    expect(parseGitHubRemoteIdentity('http://ghe.acme.com:8080/team/dorka.git')).toEqual({
  ...
  it('drops the default https port so github.com stays bare', () => {
    ...
    expect(parseGitHubRemoteIdentity('https://github.com:443/team/dorka.git')?.host).toBe(
      'github.com'
    )
  })

  it('drops the default https port on a GHES host', () => {
    expect(parseGitHubRemoteIdentity('https://ghe.acme.com:443/team/dorka.git')?.host).toBe(
      'ghe.acme.com'
    )
  })
  ...
  it('normalizes ssh.github.com:443 (SSH-over-HTTPS) to github.com without a port', () => {
    ...
    expect(parseGitHubRemoteIdentity('ssh://git@ssh.github.com:443/team/dorka.git')).toEqual({
      host: 'github.com',
      owner: 'team',
      repo: 'dorka'
    })
  ...
  it('drops a custom ssh transport port on a GHES ssh remote', () => {
    ...
    expect(parseGitHubRemoteIdentity('ssh://git@ghe.acme.com:2222/team/dorka.git')?.host).toBe(
      'ghe.acme.com'
    )
  })

  it('drops the transport port for git+ssh and git remotes', () => {
    expect(parseGitHubRemoteIdentity('git+ssh://git@ghe.acme.com:2222/team/dorka.git')?.host).toBe(
      'ghe.acme.com'
    )
    expect(parseGitHubRemoteIdentity('git://ghe.acme.com:9418/team/dorka.git')?.host).toBe(
      'ghe.acme.com'
    )
  })

  it('lowercases the host while keeping the port', () => {
    expect(parseGitHubRemoteIdentity('https://GHE.Acme.COM:8443/team/dorka.git')?.host).toBe(
      'ghe.acme.com:8443'
    )
  })
...
describe('parseGitHubOwnerRepo', () => {
  it('returns owner/repo for github.com', () => {
    expect(parseGitHubOwnerRepo('https://github.com/team/dorka.git')).toEqual({
      owner: 'team',
      repo: 'dorka'
    })
  })
  ...
  it('returns null for a custom-port GHES remote (not github.com)', () => {
    ...
    expect(parseGitHubOwnerRepo('https://ghe.acme.com:8443/team/dorka.git')).toBeNull()
  })

  it('still recognizes github.com even with an explicit default port', () => {
    expect(parseGitHubOwnerRepo('https://github.com:443/team/dorka.git')).toEqual({
      owner: 'team',
      repo: 'dorka'
    })
  })
  ...
  it('returns null for an SSH Host alias remote without HostName resolution', () => {
    expect(parseGitHubOwnerRepo('git@github-work:team/dorka.git')).toBeNull()
    expect(parseGitHubOwnerRepo('git@github.com-work:team/dorka.git')).toBeNull()
    expect(parseGitHubOwnerRepo('ssh://git@github-work/team/dorka.git')).toBeNull()
  })
})

describe('SSH Host alias identity (#10284)', () => {
  it('detects SCP and ssh:// remotes as SSH transport', () => {
    expect(remoteUrlUsesSshTransport('git@github-work:team/dorka.git')).toBe(true)
    expect(remoteUrlUsesSshTransport('ssh://git@github-work/team/dorka.git')).toBe(true)
    expect(remoteUrlUsesSshTransport('git+ssh://git@github-work/team/dorka.git')).toBe(true)
    expect(remoteUrlUsesSshTransport('https://github.com/team/dorka.git')).toBe(false)
  })

  it('exposes Host aliases that need ssh -G expansion', () => {
    expect(gitHubSshConfigHostAlias('git@github-work:team/dorka.git')).toBe('github-work')
    expect(gitHubSshConfigHostAlias('git@github.com-work:team/dorka.git')).toBe('github.com-work')
    expect(gitHubSshConfigHostAlias('ssh://git@github-work/team/dorka.git')).toBe('github-work')
    expect(gitHubSshConfigHostAlias('git@github.com:team/dorka.git')).toBeNull()
    expect(gitHubSshConfigHostAlias('https://github.com/team/dorka.git')).toBeNull()
  })

  it('preserves SSH Host alias case for OpenSSH Host matching', () => {
    expect(gitHubSshConfigHostAlias('git@GitHub-Work:team/dorka.git')).toBe('GitHub-Work')
    expect(gitHubSshConfigHostAlias('ssh://git@GitHub-Work/team/dorka.git')).toBe('GitHub-Work')
    expect(gitHubSshConfigHostAlias('git+ssh://git@GitHub-Work/team/dorka.git')).toBe('GitHub-Work')
  })

  it('returns owner/repo when resolved HostName is github.com', () => {
    expect(
      parseGitHubOwnerRepoWithResolvedSshHostname('git@github-work:team/dorka.git', 'github.com')
  ...
  it('keeps owner/repo for literal github.com even if resolved host is unused', () => {
    expect(
      parseGitHubOwnerRepoWithResolvedSshHostname('git@github.com:team/dorka.git', null)
    ).toEqual({ owner: 'team', repo: 'dorka' })
  })

  it('returns null when resolved HostName is a non-GitHub forge', () => {
    expect(
      parseGitHubOwnerRepoWithResolvedSshHostname('git@gitlab-work:team/dorka.git', 'gitlab.com')
    ).toBeNull()
  })
  ...
  it('returns null when SSH resolution is missing', () => {
    expect(
      parseGitHubOwnerRepoWithResolvedSshHostname('git@github-work:team/dorka.git', null)
    ).toBeNull()
    expect(
      parseGitHubOwnerRepoWithResolvedSshHostname('git@github-work:team/dorka.git', '   ')
    ).toBeNull()
  })
