...
describe('github owner/repo resolution', () => {
  ...
  it('parses GitHub Enterprise host identity', () => {
    expect(parseGitHubRemoteIdentity('https://ghe.acme.internal/acme/dorka.git')).toEqual({
      host: 'ghe.acme.internal',
      owner: 'acme',
      repo: 'dorka'
    })
    expect(parseGitHubRemoteIdentity('git@ghe.acme.internal:acme/dorka.git')).toEqual({
      host: 'ghe.acme.internal',
      owner: 'acme',
      repo: 'dorka'
    })
    expect(parseGitHubOwnerRepo('https://ghe.acme.internal/acme/dorka.git')).toBeNull()
  })

  it('prefers upstream for PR owner/repo resolution (#7331)', async () => {
    mockGitRemoteCommands({
      origin: 'git@github.com:fork/dorka.git\n',
      upstream: 'git@github.com:stablyai/orca.git\n'
    })

    await expect(getOwnerRepo('/repo')).resolves.toEqual({ owner: 'stablyai', repo: 'dorka' })
  ...
  it('prefers upstream for issue owner/repo resolution', async () => {
    ...
    await expect(getIssueOwnerRepo('/repo')).resolves.toEqual({ owner: 'stablyai', repo: 'dorka' })
    expect(gitRemoteGetUrlCalls('upstream')).toHaveLength(1)
  })
  ...
  it('falls back to origin when upstream is present but non-GitHub', async () => {
    ...
    await expect(getIssueOwnerRepo('/repo')).resolves.toEqual({ owner: 'fork', repo: 'dorka' })
    expect(gitRemoteGetUrlCalls('upstream')).toHaveLength(1)
    expect(gitRemoteGetUrlCalls('origin')).toHaveLength(1)
  })
  ...
  it('does not mix origin and upstream cache entries for the same repo path', async () => {
    gitExecFileAsyncMock
      .mockResolvedValueOnce({ stdout: 'git@github.com:fork/dorka.git\n' })
  ...
  it('resolves SSH repo remotes through the registered SSH git provider', async () => {
    ...
    await expect(getOwnerRepo('/home/user/dorka', 'openclaw-2')).resolves.toEqual({
      owner: 'stablyai',
      repo: 'dorka'
    })
  ...
  it('keeps local and SSH owner/repo cache entries separate for the same path', async () => {
    const sshProvider = {
      exec: vi.fn(async (args: string[]) => {
        ...
        return { stdout: 'git@github.com:remote/dorka.git\n', stderr: '' }
      })
    }
    mockGitRemoteCommands({ origin: 'git@github.com:local/dorka.git\n' })
    getSshGitProviderMock.mockReturnValue(sshProvider)
      ...
      async (args: string[], options: { wslDistro?: string } = {}) => {
        ...
        return {
          stdout: options.wslDistro
            ? 'git@github.com:wsl/dorka.git\n'
            : 'git@github.com:host/dorka.git\n'
        }
      }
    )

    await expect(getOwnerRepo('/repo')).resolves.toEqual({ owner: 'host', repo: 'dorka' })
  ...
  it('resolves PR candidates as upstream then origin and de-dupes matching slugs', async () => {
    mockGitRemoteCommands({
      origin: 'git@github.com:acme/dorka.git\n',
      upstream: 'git@github.com:Acme/Dorka.git\n'
    })

    await expect(resolvePRRepositoryCandidates('/repo')).resolves.toEqual({
      candidates: [{ owner: 'Acme', repo: 'Dorka' }],
      headRepo: { owner: 'acme', repo: 'dorka' }
    })
    expect(gitRemoteGetUrlCalls('upstream')).toHaveLength(1)
  })

  it('does not spawn git remote get-url upstream for origin-only PR candidates', async () => {
    mockGitRemoteCommands({ origin: 'git@github.com:fork/dorka.git\n' })

    await expect(resolvePRRepositoryCandidates('/repo')).resolves.toEqual({
      candidates: [{ owner: 'fork', repo: 'dorka' }],
  ...
  it('ignores non-GitHub upstream while keeping origin as the head repo', async () => {
    mockGitRemoteCommands({
      origin: 'git@github.com:fork/dorka.git\n',
      upstream: 'git@example.com:Acme/Dorka.git\n'
    })
  ...
  it('expires cached remote owner/repo entries after the TTL', async () => {
    ...
    try {
      gitExecFileAsyncMock
        .mockResolvedValueOnce({ stdout: 'git@github.com:old/dorka.git\n' })
        .mockResolvedValueOnce({ stdout: 'git@github.com:new/dorka.git\n' })
  ...
  it('keeps local missing-remote probes cached beyond the short positive TTL', async () => {
    const repoPath = await mkdtemp(join(tmpdir(), 'dorka-gh-utils-'))
...
describe('resolveIssueSource', () => {
  ...
  it("'auto' + upstream exists → upstream, fellBack=false", async () => {
    ...
    await expect(resolveIssueSource('/repo', 'auto')).resolves.toEqual({
      source: { owner: 'stablyai', repo: 'dorka' },
      fellBack: false
    })
    expect(gitRemoteGetUrlCalls('upstream')).toHaveLength(1)
  })
  ...
  it("'auto' + no github upstream → origin, fellBack=false", async () => {
    mockGitRemoteCommands({
      origin: 'git@github.com:solo/dorka.git\n',
      upstream: 'git@example.com:stablyai/orca.git\n'
    })

    await expect(resolveIssueSource('/repo', 'auto')).resolves.toEqual({
      source: { owner: 'solo', repo: 'dorka' },
      fellBack: false
    })
  })
  ...
  it("'upstream' + no upstream remote → origin, fellBack=true", async () => {
    ...
    gitExecFileAsyncMock
      ...
      .mockResolvedValueOnce({ stdout: 'git@github.com:solo/dorka.git\n' })
  ...
  it("'origin' + upstream exists → origin (ignores upstream), fellBack=false", async () => {
    ...
    gitExecFileAsyncMock.mockResolvedValueOnce({
      stdout: 'git@github.com:fork/dorka.git\n'
    })

    await expect(resolveIssueSource('/repo', 'origin')).resolves.toEqual({
      source: { owner: 'fork', repo: 'dorka' },
      fellBack: false
    })
    expect(gitExecFileAsyncMock).toHaveBeenCalledTimes(1)
    expect(gitExecFileAsyncMock).toHaveBeenCalledWith(['remote', 'get-url', 'origin'], {
  ...
  it("'origin' + no upstream → origin, fellBack=false", async () => {
    gitExecFileAsyncMock.mockResolvedValueOnce({
      stdout: 'git@github.com:solo/dorka.git\n'
    })
...
describe('gh error classification', () => {
  ...
  it('classifies "has disabled issues" stderr as issues_disabled', () => {
    const stderr =
      "Command failed: gh issue list --limit 36 --json number,title,state --repo brennanb2025/dorka --state open\nthe 'brennanb2025/dorka' repository has disabled issues"
