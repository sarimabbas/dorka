...
describe('GitHub PR local runtime routing', () => {
  ...
  it('routes PR details and mutations through the selected WSL distro', async () => {
    ...
    ghExecFileAsyncMock.mockImplementation(async (args: string[]) => {
      const endpoint = args.find((arg) => arg.startsWith('repos/acme/dorka/')) ?? ''
      ...
      if (args.length === 2 && endpoint === 'repos/acme/dorka/pulls/7') {
        return {
          stdout: JSON.stringify({
            number: 7,
  ...
  it('never falls through to the default gh host for an unresolved SSH repository', async () => {
    const legacyRepo = { owner: 'team', repo: 'dorka' }
  ...
  it('preserves a ported GHES host in SSH-backed review reads and mutations', async () => {
    ...
    ghExecFileAsyncMock.mockImplementation(async (args: string[]) => {
      const endpoint = args.find((arg) => arg.startsWith('repos/team/dorka/')) ?? ''
      const query = args.find((arg) => arg.startsWith('query=')) ?? ''
      ...
      if (args[0] === 'pr' && args[1] === 'view') {
        ...
        return {
          stdout: JSON.stringify({
            ...
            title: 'Enterprise PR',
            state: 'OPEN',
            url: 'https://github.acme-corp.com:8443/team/dorka/pull/7',
            labels: [],
            ...
            check_runs: [
              {
                ...
                status: 'completed',
                conclusion: 'failure',
                details_url: 'https://github.acme-corp.com:8443/team/dorka/actions/runs/77/job/88'
              }
            ]
          })
        }
      ...
      if (args.length === 2 && endpoint === 'repos/team/dorka/pulls/7') {
        return {
          stdout: JSON.stringify({
            number: 7,
    ...
    await expect(
      getWorkItemByOwnerRepo('/remote/repo', { owner: 'team', repo: 'dorka' }, 7, 'pr', 'ssh-1')
    ).resolves.toMatchObject({ number: 7, title: 'Enterprise PR' })
    await expect(
    ...
    expect(prViewCall?.[0]).toEqual(expect.arrayContaining(['--repo', 'team/dorka']))
    ...
    expect(
      prCalls.every(
        ([args]) => args.includes('--repo') && args[args.indexOf('--repo') + 1] === 'team/dorka'
      )
    ).toBe(true)
