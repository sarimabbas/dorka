...
describe('canCleanupUnregisteredDorkaLeftoverDirectory', () => {
  const repo = { path: '/repos/main' }
  const ownedMeta = { dorkaCreatedAt: 1, dorkaCreationSource: 'runtime' as const }
  const baseArgs = {
    meta: ownedMeta,
    worktreePath: '/workspaces/dorka-owned',
    runtimeWorktreePath: '/workspaces/dorka-owned',
    repo,
  ...
  it('rejects unregistered existing targets that are files or symlinks', async () => {
    ...
    await expect(
      canCleanupUnregisteredDorkaLeftoverDirectory({
        ...baseArgs,
        statPath: makeStatPath(['/workspaces/dorka-owned']),
        isGitRepository
      })
    ).resolves.toBe(false)
    await expect(
      canCleanupUnregisteredDorkaLeftoverDirectory({
        ...baseArgs,
        statPath: async (path) => {
          if (path === '/workspaces/dorka-owned') {
            return { type: 'symlink' }
          }
          throw missingPath(path)
        },
  ...
  it('rejects unregistered leftover directories with a .git marker', async () => {
    ...
    await expect(
      canCleanupUnregisteredDorkaLeftoverDirectory({
        ...baseArgs,
        statPath: makeStatPath(['/workspaces/dorka-owned/.git'], ['/workspaces/dorka-owned']),
        isGitRepository
      })
    ).resolves.toBe(false)
  ...
  it('rejects no-marker cleanup when only the Dorka path shape matches', async () => {
    const isGitRepository = vi.fn().mockResolvedValue(false)
  ...
  it('rejects unregistered leftover directories that still answer git status', async () => {
    ...
    expect(isGitRepository).toHaveBeenCalledWith('/workspaces/dorka-owned')
  })
    ...
    await expect(
      canCleanupUnregisteredDorkaLeftoverDirectory({
        ...
        registeredWorktrees: [
          ...
          makeGitWorktree('/workspaces/dorka-owned/child')
        ],
        statPath: makeStatPath([], ['/workspaces/dorka-owned']),
        isGitRepository: vi.fn().mockResolvedValue(false)
      })
    ).rejects.toThrow(
      'Refusing to delete worktree because it contains another registered worktree: /workspaces/dorka-owned/child'
    )
  })
  ...
  it('uses runtime paths for filesystem proof and original paths for nested worktree checks', async () => {
    const statPath = vi.fn(async (path: string) => {
      if (path === '/mnt/c/workspaces/dorka-owned') {
        return { type: 'directory' }
      }
      throw missingPath(path)
    })
    ...
    await expect(
      canCleanupUnregisteredDorkaLeftoverDirectory({
        ...baseArgs,
        worktreePath: 'C:\\workspaces\\dorka-owned',
        runtimeWorktreePath: '/mnt/c/workspaces/dorka-owned',
        repo: { path: 'C:\\repos\\main' },
        runtimeRepoPath: '/mnt/c/repos/main',
        registeredWorktrees: [
          makeGitWorktree('C:\\repos\\main', true),
          makeGitWorktree('C:\\workspaces\\dorka-owned-sibling')
        ],
        statPath,
        isGitRepository
      })
    ...
    expect(statPath).toHaveBeenCalledWith('/mnt/c/workspaces/dorka-owned')
    expect(statPath).toHaveBeenCalledWith('/mnt/c/workspaces/dorka-owned/.git')
    expect(statPath).not.toHaveBeenCalledWith('C:\\workspaces\\dorka-owned')
    expect(isGitRepository).toHaveBeenCalledWith('/mnt/c/workspaces/dorka-owned')
  })
  ...
  it('rejects translated-runtime cleanup when original path contains a registered child', async () => {
    await expect(
      canCleanupUnregisteredDorkaLeftoverDirectory({
        ...
        registeredWorktrees: [
          makeGitWorktree('C:\\repos\\main', true),
          makeGitWorktree('C:\\workspaces\\dorka-owned\\child')
        ],
        statPath: makeStatPath([], ['/mnt/c/workspaces/dorka-owned']),
        isGitRepository: vi.fn().mockResolvedValue(false)
      })
    ).rejects.toThrow(
      'Refusing to delete worktree because it contains another registered worktree: C:\\workspaces\\dorka-owned\\child'
    )
  })
})
...
describe('canSafelyRemoveOrphanedWorktreeDirectory on an execution host', () => {
  ...
  it('refuses the leftover-directory cleanup while the host home is unanswered', async () => {
    const leftoverArgs = {
      meta: { dorkaCreatedAt: 1, dorkaCreationSource: 'ssh' as const },
