...
export async function installCopilotHooksRemote(
  ...
  const remoteConfigPath = `${home}/.copilot/hooks/dorka.json`
  const remoteScriptPath = `${home}/.dorka/agent-hooks/copilot-hook.sh`
  ...
  try {
    ...
    if (!config) {
      return {
        ...
        detail: 'Could not parse remote Copilot hooks/dorka.json'
      }
    }
    ...
    for (const eventName of COPILOT_EVENTS) {
      ...
      nextHooks[eventName] = [
        ...
        getRemoteManagedHookDefinition(
          wrapPosixHookCommand(remoteScriptPath, { DORKA_COPILOT_HOOK_EVENT: eventName })
        )
      ]
    }
    ...
    // Why: SSH remotes use POSIX scripts regardless of Dorka's local OS. Write
    // the script before hooks/dorka.json so a partial install cannot point
