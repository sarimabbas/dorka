...
beforeEach(() => {
  vi.resetModules()
  testState.userData = mkdtempSync(join(tmpdir(), 'dorka-codex-e-migration-'))
  testState.home = mkdtempSync(join(tmpdir(), 'dorka-codex-e-home-'))
  for (const key of [
    'DORKA_USER_DATA_PATH',
    'DORKA_DISABLE_CODEX_TRUST_RPC',
    'CODEX_HOME',
    'DORKA_CODEX_HOME'
  ]) {
...
describe('CodexRuntimeHomeService per-account takeover composition', () => {
  ...
  it('does not expose an untrusted persisted home through rollout discovery', async () => {
    ...
    writeFileSync(join(outsideHome, '.dorka-managed-home'), 'account-1\n', 'utf-8')
    writeFileSync(
