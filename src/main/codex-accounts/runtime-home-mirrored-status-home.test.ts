...
beforeEach(() => {
  vi.resetModules()
  testState.userData = mkdtempSync(join(tmpdir(), 'dorka-codex-status-home-ud-'))
  testState.home = mkdtempSync(join(tmpdir(), 'dorka-codex-status-home-'))
  ...
  for (const key of ['DORKA_USER_DATA_PATH', 'CODEX_HOME', 'DORKA_CODEX_HOME']) {
    previousEnv[key] = process.env[key]
    delete process.env[key]
  }
  process.env.DORKA_USER_DATA_PATH = testState.userData
...
function createManagedAccount(id: string): CodexManagedAccount {
  ...
  writeFileSync(join(home, '.dorka-managed-home'), `${id}\n`, 'utf-8')
...
describe('CodexRuntimeHomeService.getMirroredHostHomePathForStatus', () => {
  ...
  it('returns the shared runtime home when a custom CODEX_HOME keeps the mirror lane', async () => {
    ...
    const { CodexRuntimeHomeService } = await import('./runtime-home-service')
    const { getDorkaManagedCodexHomePath } = await import('../codex/codex-home-paths')
    const service = new CodexRuntimeHomeService(createStore([], null) as never)
    ...
    expect(service.getMirroredHostHomePathForStatus()).toEqual({
      kind: 'ready',
      homePath: getDorkaManagedCodexHomePath()
    })
  })
})
