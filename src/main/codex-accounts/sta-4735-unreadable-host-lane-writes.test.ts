...
describe('STA-4735 an unreadable runtime auth.json must not be written over', () => {
  ...
  it('refuses the write instead of overwriting a token it could not read', async () => {
    ...
    const STALE = createCodexAuthJson('user@example.com', 'acct-1', 'refresh-stale-dorka-copy')
    ...
    // below replaced a freshly rotated refresh token with Dorka's stale copy.
