...
export class CodexConfigMirror {
  ...
  readForManagedHome(managedHomePath: string): CanonicalCodexConfig | null {
    ...
    const managedRootMarker = '/.local/share/dorka/codex-accounts/'
  ...
  assertOAuthAccountAddAllowed(canonicalConfig: CanonicalCodexConfig | null): void {
    ...
    throw new Error(
      `Dorka cannot add a Codex OAuth account while ~/.codex/config.toml pins the custom provider ${JSON.stringify(modelProvider)}. Keep using the system-default account for this provider, or remove model_provider (or set it to "openai") before adding an OAuth account. Dorka left your config unchanged.`
    )
  }
