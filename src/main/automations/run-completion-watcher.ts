...
export function describeStrandedAutomationRun(run: AutomationRun): string {
  if (run.status === 'dispatching') {
    return 'Dorka stopped before this run reported that its agent started.'
  }
  return 'Dorka lost the terminal for this run before it reported completion.'
}
...
function describeObservationError(error: unknown): string {
  ...
  return 'Dorka stopped watching this run before it reported completion.'
}
