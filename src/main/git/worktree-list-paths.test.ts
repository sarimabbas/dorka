...
async function createRepoWithNewlineWorktree(): Promise<{
  ...
  const root = await mkdtemp(path.join(tmpdir(), 'dorka-worktree-paths-'))
  tempRoots.push(root)
...
async function createRepoWithLockedDeletedWorktree(): Promise<{
  ...
  const root = await mkdtemp(path.join(tmpdir(), 'dorka-worktree-locked-delete-'))
  tempRoots.push(root)
  const repoPath = path.join(root, 'repo')
  ...
  git(repoPath, ['worktree', 'lock', '--reason', 'dorka locked stale repro', requestedWorktreePath])
...
async function createRepoWithPrunableWorktree(): Promise<{
  ...
  const root = await mkdtemp(path.join(tmpdir(), 'dorka-worktree-prunable-'))
  tempRoots.push(root)
  const repoPath = path.join(root, 'repo')
