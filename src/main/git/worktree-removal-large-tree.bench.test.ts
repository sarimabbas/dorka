...
// Opt in (it builds ~100k files): DORKA_WORKTREE_REMOVAL_BENCH=1 pnpm exec vitest run \
...
const describeBench = process.env.DORKA_WORKTREE_REMOVAL_BENCH ? describe : describe.skip
...
describeBench('worktree removal on a large checkout', () => {
  ...
  beforeAll(async () => {
    scratchDir = await mkdtemp(join(tmpdir(), 'dorka-worktree-removal-bench-'))
