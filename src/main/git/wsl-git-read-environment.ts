...
function probeWslGitReadEnvironment(distro: string): Promise<ProbeOutcome> {
  const probeCommand = [
    '_dorka_git_path=$(command -v git 2>/dev/null || true)',
    'case "$_dorka_git_path" in /*) [ -x "$_dorka_git_path" ] || exit 127 ;; *) exit 127 ;; esac',
    'if [ -n "${XDG_CONFIG_HOME:-}" ] || [ -n "${LD_LIBRARY_PATH:-}" ] || env | grep -q \'^GIT_\'; then exit 78; fi',
    `printf '%s\\0%s\\0%s' "$PATH" "$_dorka_git_path" "$HOME"`
  ].join('\n')
