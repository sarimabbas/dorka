...
describe('isGitRepo', () => {
  ...
  beforeEach(() => {
    tmpDir = mkdtempSync(path.join(tmpdir(), 'dorka-repo-detect-'))
  })
  ...
  it('rejects a .git file that points at a missing gitdir when git cannot be run', () => {
    ...
    mkdirSync(fakeRepo)
    writeFileSync(path.join(fakeRepo, '.git'), 'gitdir: /missing/dorka/gitdir')
...
describe('getLinkedWorktreeMainRepoRoot', () => {
  ...
  beforeEach(() => {
    tmpDir = mkdtempSync(path.join(tmpdir(), 'dorka-linked-worktree-'))
  })
  ...
  function initRepoWithCommit(repoRoot: string): void {
    ...
    git(repoRoot, ['config', 'user.email', 'test@dorka.test'])
    git(repoRoot, ['config', 'user.name', 'Dorka Test'])
