...
async function createStubGit(root: string): Promise<string> {
  ...
  await writeFile(
    ...
    `#!/bin/sh
set -eu
live="$DORKA_STUB_STATE_DIR/$DORKA_STUB_ID.live"
: > "$live"
trap 'rm -f "$live"' EXIT HUP INT TERM
sleep "$(awk "BEGIN { print $DORKA_STUB_SLEEP_MS / 1000 }")"
printf 'stub:%s\\n' "$*"
`
  )
...
async function measureStorm(mode: StormMeasurement['mode']): Promise<StormMeasurement> {
  ...
  const root = await mkdtemp(path.join(tmpdir(), `dorka-git-storm-${mode}-`))
  tempRoots.push(root)
  ...
  const background = Array.from({ length: 60 }, (_, index) =>
    gitExecFileAsync(['status', '--porcelain=v2', `storm-${index}`], {
      ...
      env: {
        ...baseEnv,
        DORKA_STUB_ID: `background-${index}`,
        DORKA_STUB_SLEEP_MS: index % 10 === 0 ? '5000' : '200',
        DORKA_STUB_STATE_DIR: stateDir
      },
            ...
            () => {
              void (async () => {
                ...
                await gitExecFileAsync(['rev-parse', commandLabel], {
                  ...
                  env: {
                    ...baseEnv,
                    DORKA_STUB_ID: `interactive-${index}`,
                    DORKA_STUB_SLEEP_MS: String(Math.max(10, concurrentAtInjection * 12)),
                    DORKA_STUB_STATE_DIR: stateDir
                  },
