...
// duplicates disk; `dorka.yaml` names the ones every worktree should share instead.
...
/** Every path Dorka may have symlinked into a worktree: the per-user Worktree
 *  Shared Paths setting plus the repo's `dorka.yaml` shared directories.
 *
...
/** Resolve `worktree.sharedDirectories` from the repo-root `dorka.yaml` to
