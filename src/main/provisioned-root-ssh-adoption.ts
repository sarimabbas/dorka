...
export async function adoptProvisionedRootSshCheckout(args: {
  ...
  if (gitWorktree.branch !== `refs/heads/${requestedBranch}`) {
    throw new Error("The recipe projectRoot is not checked out on Dorka's requested branch.")
  }
  ...
  if (request.expectedRefHead && gitWorktree.head !== request.expectedRefHead) {
    throw new Error("The recipe projectRoot was not created from Dorka's requested ref.")
  }
...
function buildProvisionedRootMeta(
  ...
  return {
    ...
    dorkaCreationWorkspaceLayout: getWorktreeCreationLayout(repo, store.getSettings()),
