...
const DISTRO_ROOT = '\\\\wsl.localhost\\Ubuntu\\home\\ada\\dorka\\workspaces'
...
describe('retirement discovery on WSL', () => {
  ...
  it('reports a gate refusal as incomplete but still returns what the host side could read', async () => {
    ...
    const hostHome = await mkdtemp(join(tmpdir(), 'dorka-wsl-host-home-'))
    try {
      await mkdir(
        join(
          hostHome,
  ...
  it('accepts a UNC ENOENT as absence once the parent directory still lists', async () => {
    ...
    const missing = new Set([
      DISTRO_ROOT,
      '\\\\wsl.localhost\\Ubuntu\\home\\ada\\dorka',
  ...
  it('keeps a deleted WSL workspace name spent, so the next create cannot reuse its cwd', async () => {
    ...
    const distroHome = await mkdtemp(join(tmpdir(), 'dorka-wsl-distro-home-'))
    const workspaceRoot = await mkdtemp(join(tmpdir(), 'dorka-wsl-workspaces-'))
    try {
  ...
  it('reaches distro discovery from the real backfill, not only when called directly', async () => {
    ...
    const distroHome = await mkdtemp(join(tmpdir(), 'dorka-wsl-backfill-home-'))
    try {
      await mkdir(join(distroHome, '.claude', 'projects', `-home-ada-dorka-workspaces-${FIRST}`), {
        recursive: true
      })
