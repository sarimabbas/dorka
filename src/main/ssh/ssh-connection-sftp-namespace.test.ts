...
vi.mock('./ssh-system-fallback', () => ({
  getDorkaControlSocketPath: vi.fn().mockReturnValue(null),
...
const RELAY_DIR = '.dorka-remote/relay-0.1.0+hash'
...
describe('SshConnection SFTP namespace resolution', () => {
  ...
  beforeEach(() => {
    ...
    localDir = realpathSync(mkdtempSync(join(tmpdir(), 'dorka-relay-')))
