/**
 * dorkad's garbage collection, and the half of §06 falsifier 1 that says who owns it.
 *
 * **Each model GCs only its own namespace, permanently.** dorkad removes `dorkad-<v>/`
 ...
 * to keep intact.
 *
 * On top of the ownership rule, dorkad pins three directories that are idle-looking but
...
import { dorkadGcPinnedDirNames, type DorkadActivationRecord } from './dorkad-activation-record'
import {
...
export async function gcOldDorkadVersions(options: DorkadGcOptions): Promise<void> {
  await gcOldRemoteInstallVersions(
    ...
    options.host,
    {
      pinnedDirNames: dorkadGcPinnedDirNames(options.record, options.liveDaemonVersion),
      isDirLive: async (dir) => {
        try {
          const probe = await execCommand(
            options.conn,
            dorkadLivenessProbeCommand(options.host, dir),
            {
              wrapCommand: options.host.commandDialect !== 'powershell',
              signal: options.signal
            }
          )
          return dorkadLivenessBlocksGc(parseDorkadLiveness(probe))
        } catch {
