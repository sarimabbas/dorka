...
const roots: string[] = []
const configuredPowerShell = process.env.DORKA_POWERSHELL_EXECUTABLE
...
function createPool(): string {
  const root = mkdtempSync(join(tmpdir(), 'dorka-upload-stage-pool-'))
  roots.push(root)
...
function createStage(
  ...
  const marker = join(stage, '.dorka-upload-owner')
  ...
  (_label, host) => {
    ...
    it('promotes only the post-claim owned payload and removes its fixed stage', () => {
      ...
      const stage = parseReservedRelayUploadStage(
        ...
        pool,
        owner,
        `noise\n__DORKA_UPLOAD_STAGE_SLOT__${owner}:slot-0\n`
      )
...
describe('POSIX ownership race fencing', () => {
  it('restores a replacement directory and preserves the original moved aside before claim', () => {
    ...
    const prefix = [
      ...
      'mv() {',
      'if [ "$raced" -eq 0 ]; then',
      'raced=1; command mv "$1" "$1.original"; mkdir "$1"; mkdir "$1/payload"; cp "$1.original/.dorka-upload-owner" "$1/.dorka-upload-owner"; cp "$1.original/.dorka-upload-identity" "$1/.dorka-upload-identity"; touch -t 202001010000 "$1/.dorka-upload-owner"; printf foreign > "$1/foreign";',
      'fi;',
      'command mv "$@";',
      '}'
    ].join('\n')
    ...
    expect(existsSync(join(pool, 'slot-0.original', '.dorka-upload-owner'))).toBe(true)
    expect(existsSync(join(destination, 'relay.js'))).toBe(false)
  })
  ...
  () => {
    it('restores an old same-owner replacement whose persisted file ID does not match', () => {
      ...
      const prefix = [
        ...
        '[System.IO.File]::WriteAllText((Join-Path $newPath ".dorka-upload-owner"), [System.IO.File]::ReadAllText((Join-Path $originalPath ".dorka-upload-owner")))',
        '[System.IO.File]::WriteAllText((Join-Path $newPath ".dorka-upload-identity"), [System.IO.File]::ReadAllText((Join-Path $originalPath ".dorka-upload-identity")))',
        '(Get-Item -LiteralPath (Join-Path $newPath ".dorka-upload-owner") -Force).LastWriteTimeUtc = [DateTime]::UtcNow.AddHours(-2)',
