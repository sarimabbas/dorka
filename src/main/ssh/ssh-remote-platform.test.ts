...
describe('joinRemotePath', () => {
  it('joins POSIX remote paths', () => {
    expect(joinRemotePath(getRemoteHostPlatform('linux-x64'), '/home/me', '.dorka-remote')).toBe(
      '/home/me/.dorka-remote'
    )
  })

  it('normalizes and joins Windows remote paths with forward slashes for SFTP and Node', () => {
    expect(
      joinRemotePath(getRemoteHostPlatform('win32-x64'), 'C:\\Users\\me', '.dorka-remote', 'relay')
    ).toBe('C:/Users/me/.dorka-remote/relay')
  })
})
...
describe('detectRemoteHostPlatform', () => {
  it('uses uname when the remote is POSIX', async () => {
    vi.mocked(execCommand).mockResolvedValueOnce('__DORKA_REMOTE_PLATFORM__ Darwin arm64')
  ...
  it('falls back to PowerShell when uname is unavailable on Windows', async () => {
    ...
    expect(script).toContain('Write-Output ("`n__DORKA_REMOTE_PLATFORM__ Windows " + $arch)')
  })
})
