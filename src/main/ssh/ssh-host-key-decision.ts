...
export type HostKeyDecision = {
  ...
  disagreeingSource?: 'dorka-store' | 'known-hosts'
...
export function decideHostKey(input: HostKeyDecisionInput): HostKeyDecision {
  ...
  if (
    ...
    return {
      ...
      disagreeingSource: fromKnownHosts ? 'known-hosts' : 'dorka-store',
      reason: rejection(
        displayHost,
  ...
  // escape hatch that is an environment variable, unreachable when Dorka is launched from the Dock.
