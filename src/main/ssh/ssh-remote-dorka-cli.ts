...
import {
  ...
  type RemoteDorkaCliRequest,
  type RemoteDorkaCliResult
...
export type { RemoteDorkaCliRequest, RemoteDorkaCliResult } from './ssh-remote-cli-host-passthrough'
...
const HOST_INTERACTIVE_COMMANDS: Record<string, string> = {
  serve:
    'dorka serve starts a foreground headless Dorka server and cannot run through the SSH relay bridge. Run it directly on the machine that should host Dorka.',
  'claude-teams':
    'dorka claude-teams starts an interactive Claude Code session and cannot run through the SSH relay bridge. Run it in a terminal on the Dorka host machine.',
  'agent-teams-tmux':
    'dorka agent-teams-tmux is a tmux pane shim for the Dorka host machine and cannot run through the SSH relay bridge.',
  'account add':
    'dorka account add runs an interactive agent login and cannot run through the buffered SSH relay bridge. Run it directly in a terminal on the Dorka host machine.'
}

export async function runRemoteDorkaCli(
  ...
  if (command === 'orchestration check' || command === 'orchestration ask') {
    ...
    return await runLegacyRemoteDorkaCli(
      runtime,
      request,
      parsed,
      json,
  ...
  try {
    return await runHostDorkaCliPassthrough(request, passthroughOptions)
  } catch (err) {
  ...
  return await runLegacyRemoteDorkaCli(runtime, request, parsed, json, passthroughFailure)
}

async function runLegacyRemoteDorkaCli(
  runtime: DorkaRuntimeService,
  request: RemoteDorkaCliRequest,
...
async function dispatchRemoteCli(
  ...
  runtimeAuthority: RemoteDorkaCliRequest['runtimeAuthority']
): Promise<RpcResponse> {
  const command = parsed.commandPath.join(' ')
  ...
  switch (command) {
    case 'status': {
      ...
      const cliStatus: CliStatusResult = {
        ...
        // Why: this answers for the Dorka host the caller reached over SSH, not for the caller's
    ...
    case 'orchestration send': {
      ...
      return await call(
        ...
        'orchestration.send',
        {
          ...
          senderPaneKey: env.DORKA_PANE_KEY || undefined
        },
        {
    ...
    default:
      ...
      throw new Error(
        `Unsupported SSH Dorka CLI command: ${command} (full Dorka CLI bridge unavailable: ${passthroughFailureReason})`
      )
  }
}
