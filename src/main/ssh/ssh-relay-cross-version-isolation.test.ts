...
describe('cross-version isolation', () => {
  ...
  it('a v2 deploy never references the v1 install dir or v1 socket path', async () => {
    ...
    //   v1 dir = ~/.dorka-remote/relay-0.1.0+111111111111/  (live daemon, listening)
    //   v2 dir = ~/.dorka-remote/relay-0.1.0+222222222222/  (does not yet exist)
    // The v2 client has fullVersion='0.1.0+222222222222' (from the fs mock above).
    //
    mockExec.mockImplementation((_conn, command) => {
      if (command.includes('__DORKA_UPLOAD_STAGE_SLOT__')) {
        return Promise.resolve(
          '__DORKA_UPLOAD_STAGE_SLOT__.sftp-namespace-00000000000000000000000000000000:slot-0'
        )
      }
      if (command.includes('__DORKA_UPLOAD_STAGE_PROMOTION__')) {
        return Promise.resolve(
      ...
      if (command.includes('DORKA-NPTY-PROBE-OK')) {
        return Promise.resolve('DORKA-NPTY-PROBE-OK\n')
      }
      ...
      if (command.includes('__DORKA_RELAY_GC_FIND_STATUS__')) {
