...
const SHELL_RELAY_DIR = `/var/services/homes/alice/.dorka-remote/relay-${VERSION}`

describe('relayRemoteDirSegments', () => {
  it('builds the two segments every relay path shares', () => {
    expect(relayRemoteDirSegments(VERSION, 'posix')).toEqual(['.dorka-remote', `relay-${VERSION}`])
  })
...
describe('computeRemoteRelayDir agreement', () => {
  ...
  it('applies Windows segment rules on the windows flavor', () => {
    expect(computeRemoteRelayDir('C:\\Users\\u', VERSION, 'windows')).toBe(
      `C:/Users/u/.dorka-remote/relay-${VERSION}`
    )
...
describe('createRelayInstallNamespace', () => {
  it('mints an unguessable 128-bit marker name per install', () => {
    ...
    expect(first.homeRelativeRelayDir).toBe(`.dorka-remote/relay-${VERSION}`)
  })
})
...
describe('relaySftpNamespaceMapping', () => {
  ...
  it('maps the bundle directory itself when no file name is given', () => {
    ...
    expect(mapping.homeRelativePath).toBe(`.dorka-remote/relay-${VERSION}`)
    expect(mapping.homeRelativeNamespaceRoot).toBe(`.dorka-remote/relay-${VERSION}`)
    expect(mapping.homeRelativeProbePath).toBe(
      `.dorka-remote/relay-${VERSION}/.install-lock/${namespace.markerFileName}`
    )
  ...
  it('maps a file inside the bundle directory', () => {
    ...
    expect(mapping.homeRelativePath).toBe(`.dorka-remote/relay-${VERSION}/package.json`)
  })
...
describe('relay upload stage namespace', () => {
  const stageSuffix = `.dorka-remote/relay-${VERSION}.upload-123e4567-e89b-12d3-a456-426614174000`
