...
vi.mock('./dorkad-local-build-hash', () => ({
  computeLocalDorkadBuildHash: () => 'abc123def4567890'
}))

import { execCommand } from './ssh-relay-deploy-helpers'
import { acquireInstallLock } from './ssh-relay-install-lock'
import { uploadRelayDirectory, writeRelayFile } from './ssh-relay-install-transfers'
import { deployDorkad, type DorkadDeployOptions } from './dorkad-remote-deploy'
import { emptyDorkadActivationRecord, withActivatedVersion } from './dorkad-activation-record'
...
function scriptHost(script: HostScript): void {
  mockExec.mockImplementation(async (_conn, command: string) => {
    ...
    if (text.startsWith('cat ') && text.includes('dorkad-active.json')) {
      return script.activationRecord
    }
    if (text.includes('.dorkad-readiness') && text.startsWith('cat ')) {
...
function options(overrides: Partial<DorkadDeployOptions> = {}): DorkadDeployOptions {
  return {
    conn: {} as SshConnection,
    host: getRemoteHostPlatform('linux-x64'),
    remoteHome: '/home/u',
    localDorkadDir: '/local/out/dorkad',
...
const ACTIVE_OLD = JSON.stringify(
  withActivatedVersion(emptyDorkadActivationRecord(), OLD_VERSION, null, new Date(0))
)

describe('deployDorkad', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })

  it('installs under the dorkad namespace, not the relay one', async () => {
    ...
    scriptHost(script)
    await deployDorkad(options())
    expect(vi.mocked(acquireInstallLock).mock.calls[0][1]).toBe(
      `/home/u/.dorka-remote/dorkad-${NEW_VERSION}`
    )
    expect(vi.mocked(uploadRelayDirectory).mock.calls[0][2]).toContain(`dorkad-${NEW_VERSION}`)
  })
  ...
  it('installs but does not activate when terminals are running', async () => {
    ...
    scriptHost(script)
    const result = await deployDorkad(
  ...
  it('does not write the activation record when the candidate fails its health gate', async () => {
    ...
    scriptHost(script)
    const result = await deployDorkad(options())
    expect(result).toMatchObject({ code: 'dorkad_activation_daemon_degraded' })
    expect(
      vi
        .mocked(writeRelayFile)
        .mock.calls.some((call) => String(call[2]).endsWith('dorkad-active.json'))
    ).toBe(false)
  })
  ...
  it('puts the previous version back after a rejected candidate, rather than leaving the host down', async () => {
    ...
    expect(result.outcome === 'installed-not-activated' && result.reason).toContain(
      `dorkad ${OLD_VERSION} was restarted and is serving again`
    )
  })
  ...
  it('refuses to activate when a different build answered the port', async () => {
    ...
    scriptHost(script)
    const result = await deployDorkad(options())
    expect(result).toMatchObject({ code: 'dorkad_activation_build_mismatch' })
  })
  ...
  it('refuses to treat an unreadable activation record as an empty one', async () => {
    ...
    scriptHost(script)
    await expect(deployDorkad(options())).rejects.toThrow('activation record')
