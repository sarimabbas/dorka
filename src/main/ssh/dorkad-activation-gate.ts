/**
 * Whether a freshly launched dorkad has earned the right to become the active one.
 *
 * The failure this exists to prevent is the one `docs/design/shipping-dorkad.html` names
 * throughout: a deployment that reports success because a port opened. dorkad answers RPC
...
export type DorkadActivationRejectCode =
  | 'dorkad_activation_no_readiness'
  | 'dorkad_activation_no_health'
  | 'dorkad_activation_build_mismatch'
  | 'dorkad_activation_not_listening'
  | 'dorkad_activation_daemon_absent'
  | 'dorkad_activation_daemon_degraded'
  | 'dorkad_activation_pty_self_test_failed'
  | 'dorkad_activation_no_persistent_terminals'

export type DorkadActivationVerdict =
  | {
      decision: 'activate'
      /**
  ...
  | { decision: 'reject'; code: DorkadActivationRejectCode; reason: string }

export type DorkadActivationExpectation = {
  /** sha256(dorkad.js).slice(0,16) computed from the bytes this client just uploaded. */
  buildHash: string
 ...
 */
export function evaluateDorkadActivation(
  readiness: ServeReadiness | null,
  expected: DorkadActivationExpectation
): DorkadActivationVerdict {
  if (!readiness) {
    return {
      decision: 'reject',
      code: 'dorkad_activation_no_readiness',
      reason:
        'The candidate dorkad never published an `dorka_server_ready` line. It may have exited, ' +
  ...
  if (!health) {
    return {
      decision: 'reject',
      code: 'dorkad_activation_no_health',
      reason:
  ...
  // Why identity first: a stale dorkad already holding the port would answer readiness and
  ...
  if (health.buildHash !== expected.buildHash) {
    return {
      decision: 'reject',
      code: 'dorkad_activation_build_mismatch',
      reason:
  ...
  if (!readiness.boundEndpoint) {
    return {
      decision: 'reject',
      code: 'dorkad_activation_not_listening',
      reason:
  ...
  if (daemon.state === 'absent') {
    return {
      decision: 'reject',
      code: 'dorkad_activation_daemon_absent',
      reason:
        'The candidate has no terminal daemon. Every terminal on this host would run in the ' +
        'dorkad process and die with it, which is the exact regression the daemon exists to ' +
  ...
  if (daemon.state === 'degraded') {
    return {
      decision: 'reject',
      code: 'dorkad_activation_daemon_degraded',
      reason:
  ...
  if (health.buildVersion !== expected.fullVersion) {
    // Not a rejection: the hash already proved identity, and DORKA_VERSION is whatever the
    ...
    warnings.push(
      `The candidate reports version ${health.buildVersion} but was installed as ` +
        `${expected.fullVersion}; check DORKA_VERSION in the launch command.`
    )
  }
