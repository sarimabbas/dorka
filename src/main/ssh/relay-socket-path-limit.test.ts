...
describe('remote unix socket path limit', () => {
  it('uses the per-OS sun_path budget and ignores Windows named pipes', () => {
    ...
    expect(remoteSocketPathFitsLimit(WINDOWS, `\\\\.\\pipe\\dorka-relay-${'a'.repeat(400)}`)).toBe(
      true
    )
  })
  ...
  it('accepts only the marker line as the short directory', () => {
    ...
    expect(
      parseShortRelaySocketDir(
        `Welcome to Ubuntu\nDORKA-RELAY-SHORT-SOCKET-DIR /tmp/.dorka-relay-1000/${segment}\n`,
        segment
      )
    ).toBe(`/tmp/.dorka-relay-1000/${segment}`)
    expect(parseShortRelaySocketDir('mkdir: permission denied\n', segment)).toBeNull()
    expect(
      parseShortRelaySocketDir(`DORKA-RELAY-SHORT-SOCKET-DIR /etc/${segment}\n`, segment)
    ).toBeNull()
    // A directory belonging to another build must not be adopted as this build's.
    expect(
      parseShortRelaySocketDir(
        `DORKA-RELAY-SHORT-SOCKET-DIR /tmp/.dorka-relay-1000/${shortRelayVersionSegment('relay-9.9.9+other')}\n`,
        segment
      )
    ).toBeNull()
  })
...
describe('relay launch with a long remote $HOME', () => {
  ...
  it('keeps the launched socket path inside the remote sun_path limit', async () => {
    ...
    vi.mocked(execCommand)
      ...
      .mockResolvedValueOnce(
        `DORKA-RELAY-SHORT-SOCKET-DIR ${SHORT_RELAY_SOCKET_DIR_PREFIX}1000/${shortRelayVersionSegment(RELAY_VERSION_DIR_NAME)}`
      )
    ...
    // 45-byte $HOME + `/.dorka-remote/relay-0.1.0+8d4e15ad63eb` + `/relay-<hash16>.sock` = 110 bytes.
    ...
    // build's version segment sits above it, so the next Dorka release binds a path of
  ...
  it('sweeps superseded relays under the short base too, but never the live one', () => {
    ...
    const script = supersededRelayEndpointListCommand({
      ...
      currentRelayDir: `${LONG_HOME}/.dorka-remote/${RELAY_VERSION_DIR_NAME}`,
  ...
  it('leaves the socket in the versioned relay dir when it already fits', async () => {
    ...
    expect(launchedSockPath(conn)).toBe(
      '/home/user/.dorka-remote/relay-0.1.0+8d4e15ad63eb/relay.sock'
    )
  })
