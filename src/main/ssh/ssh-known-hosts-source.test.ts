...
async function createRoot(): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), 'dorka-known-hosts-'))
  roots.push(root)
  return root
}
...
describe('resolveKnownHostsLookupHost', () => {
  ...
  // INVERTED from 'uses the resolved hostname, never the Dorka label'. That test encoded an
  // assumption verified false against OpenSSH 10.2p1: `ssh -G` echoes its own argument back as
  // `hostname` when no Host block matches, so for a manual target `resolved.hostname` IS the Dorka
  ...
  it('uses the dialed host, because a resolved hostname can just echo the label', async () => {
    const resolved = resolvedConfig({ hostname: 'my-dorka-label' })
  ...
  it('still prefers an explicit HostKeyAlias over the dialed host', async () => {
    const resolved = resolvedConfig({ hostname: 'my-dorka-label', hostKeyAlias: 'bastion' })
...
describe('a known_hosts path containing a space', () => {
  ...
  beforeEach(async () => {
    dir = await mkdtemp(join(tmpdir(), 'dorka known hosts '))
  })
  ...
  it('leaves a genuine multi-file list alone', async () => {
    ...
    const resolved = await resolveKnownHostsFiles({
      userKnownHostsFiles: [real, '/tmp/dorka-does-not-exist-known-hosts'],
      globalKnownHostsFiles: []
    } as never)

    expect(resolved).toEqual([real, '/tmp/dorka-does-not-exist-known-hosts'])
  })

  it('leaves a single unresolvable path alone rather than inventing one', async () => {
    const resolved = await resolveKnownHostsFiles({
      userKnownHostsFiles: ['/tmp/dorka-absent-known-hosts'],
      globalKnownHostsFiles: []
    } as never)

    expect(resolved).toEqual(['/tmp/dorka-absent-known-hosts'])
  })
})
