/**
 * Which hosts the dorkad launch/lifecycle path actually supports, declared rather than
...
export class DorkadRemoteLaunchUnsupportedError extends Error {
  readonly code = 'dorkad_remote_launch_unsupported_host'
  constructor(hostLabel: string) {
    super(
      `Deploying dorkad to a ${hostLabel} host is not implemented. The install transaction is ` +
    ...
    this.name = 'DorkadRemoteLaunchUnsupportedError'
  }
}

export function assertPosixDorkadHost(host: RemoteHostPlatform): void {
  if (isWindowsRemoteHost(host)) {
    throw new DorkadRemoteLaunchUnsupportedError('Windows')
  }
}

/** PID of the launched dorkad, written into its own version dir at launch. */
export const DORKAD_PID_FILENAME = '.dorkad-pid'
...
export function posixProcessAliveShellFunction(): string {
  return (
    'dorkad_alive() { kill -0 "$1" 2>/dev/null || return 1; ' +
