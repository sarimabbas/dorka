...
import { parseDorkadStopOutcome, stopDorkadCommand } from './dorkad-remote-process-control'
import {
...
beforeEach(() => {
  root = mkdtempSync(join(tmpdir(), 'dorkad-shell-'))
  dataDir = join(root, '.dorka')
  snapshotDir = join(root, 'snapshots', 'pre-0.2.0+bb01-1000')
  versionDir = join(root, '.dorka-remote', 'dorkad-0.2.0+bb01')
  mkdirSync(join(dataDir, 'profiles', 'p1'), { recursive: true })
  mkdirSync(join(dataDir, 'daemon'), { recursive: true })
  mkdirSync(versionDir, { recursive: true })
  writeFileSync(join(dataDir, 'dorka-profile-index.json'), '{"v":"before"}')
  writeFileSync(join(dataDir, 'profiles', 'p1', 'dorka-data.json'), '{"repos":"before"}')
...
describe('state snapshot commands, run for real', () => {
  it('captures, then restores state the newer build overwrote', () => {
    expect(
      parseDorkadSnapshotCapture(sh(captureDorkadStateSnapshotCommand(host, dataDir, snapshotDir)))
    ).toBe('captured')
    expect(sh(probeDorkadStateSnapshotCommand(host, snapshotDir)).trim()).toBe('PRESENT')

    // The new version migrates the store and adds a file of its own.
    writeFileSync(join(dataDir, 'dorka-profile-index.json'), '{"v":"migrated"}')
    writeFileSync(join(dataDir, 'profiles', 'p1', 'new-build-only.json'), '{}')

    expect(
      parseDorkadSnapshotRestore(sh(restoreDorkadStateSnapshotCommand(host, dataDir, snapshotDir)))
    ).toBe('restored')
    expect(readFileSync(join(dataDir, 'dorka-profile-index.json'), 'utf8')).toBe('{"v":"before"}')
  ...
  it('leaves the live daemon runtime dir untouched through capture and restore', () => {
    sh(captureDorkadStateSnapshotCommand(host, dataDir, snapshotDir))
    ...
    sh(restoreDorkadStateSnapshotCommand(host, dataDir, snapshotDir))
  ...
  it('reports MISSING rather than claiming a restore it did not perform', () => {
    expect(
      parseDorkadSnapshotRestore(
        sh(restoreDorkadStateSnapshotCommand(host, dataDir, join(root, 'nope')))
      )
    ).toBe('missing')
  })
  ...
  it('survives a data root whose path contains a quote and a space', () => {
    ...
    writeFileSync(join(nasty, 'dorka-profile-index.json'), '{"v":"quoted"}')
    expect(
      parseDorkadSnapshotCapture(sh(captureDorkadStateSnapshotCommand(host, nasty, snapshotDir)))
    ).toBe('captured')
    writeFileSync(join(nasty, 'dorka-profile-index.json'), '{"v":"changed"}')
    expect(
      parseDorkadSnapshotRestore(sh(restoreDorkadStateSnapshotCommand(host, nasty, snapshotDir)))
    ).toBe('restored')
    expect(readFileSync(join(nasty, 'dorka-profile-index.json'), 'utf8')).toBe('{"v":"quoted"}')
  })
})

describe('liveness and stop commands, run for real', () => {
  it('reports UNKNOWN with no pid file, and DEAD for a pid that has exited', () => {
    expect(parseDorkadLiveness(sh(dorkadLivenessProbeCommand(host, versionDir)))).toBe('UNKNOWN')
    writeFileSync(join(versionDir, DORKAD_PID_FILENAME), 'not-a-pid')
    expect(parseDorkadLiveness(sh(dorkadLivenessProbeCommand(host, versionDir)))).toBe('UNKNOWN')
    ...
    expect(parseDorkadLiveness(sh(dorkadLivenessProbeCommand(host, versionDir)))).toBe('DEAD')
  })

  it('reports LIVE for a running process and stops it with SIGTERM', async () => {
    const child = spawn('/bin/sh', ['-c', 'sleep 30'], { stdio: 'ignore' })
    try {
      writeFileSync(join(versionDir, DORKAD_PID_FILENAME), String(child.pid))
      expect(parseDorkadLiveness(sh(dorkadLivenessProbeCommand(host, versionDir)))).toBe('LIVE')

      const exited = new Promise<NodeJS.Signals | null>((resolve) =>
        child.once('exit', (_code, signal) => resolve(signal))
      )
      expect(
        parseDorkadStopOutcome(sh(stopDorkadCommand(host, versionDir, { waitSeconds: 10 })))
      ).toBe('stopped')
