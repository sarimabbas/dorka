...
function scriptHost(log: string[], overrides: { restore?: string } = {}): void {
  mockExec.mockImplementation(async (_conn, command: string) => {
    ...
    if (text.startsWith('cat ') && text.includes('.dorkad-readiness')) {
...
function options(overrides: Partial<DorkadRollbackOptions> = {}): DorkadRollbackOptions {
  return {
...
describe('rollbackDorkad', () => {
  ...
  it('stops, restores state, then starts the target — in that order', async () => {
    const log: string[] = []
    scriptHost(log)
    const result = await rollbackDorkad(options())
    expect(result).toMatchObject({ outcome: 'rolled-back', target: TARGET })
    // Restoring under a running dorkad would replace the store beneath a process holding it;
  ...
  it('refuses before touching anything when terminals started after activation', async () => {
    const log: string[] = []
    scriptHost(log)
    const result = await rollbackDorkad(
  ...
  it('refuses when the snapshot is gone from the host', async () => {
    ...
    const result = await rollbackDorkad(options())
    expect(result).toMatchObject({ outcome: 'refused', code: 'dorkad_rollback_snapshot_missing' })
    expect(log).toEqual([])
  })
  ...
  it('does not start the old build when the restore failed', async () => {
    ...
    const result = await rollbackDorkad(options())
    expect(result).toMatchObject({ outcome: 'failed', code: 'dorkad_rollback_restore_failed' })
  ...
  it('leaves the record naming the newer version when the target fails to come up', async () => {
    ...
    const result = await rollbackDorkad(options())
    expect(result).toMatchObject({ outcome: 'failed', code: 'dorkad_activation_no_readiness' })
