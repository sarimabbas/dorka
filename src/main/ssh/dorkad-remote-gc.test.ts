...
import { gcOldDorkadVersions } from './dorkad-remote-gc'
import { emptyDorkadActivationRecord } from './dorkad-activation-record'
...
function scriptHost(options: {
  ...
  mockExec.mockImplementation(async (_conn, command: string) => {
    ...
    if (command.includes('.dorkad-pid')) {
...
describe('dorkad GC', () => {
  ...
  it('scopes its remote listing to the dorkad namespace', async () => {
    const removed: string[] = []
    scriptHost({ listing: [], removed })
    await gcOldDorkadVersions({
      conn,
      host,
      remoteHome: '/home/u',
      currentDirAbsPath: '/home/u/.dorka-remote/dorkad-0.2.0+bb',
      record: emptyDorkadActivationRecord()
    })
    const listCommand = mockExec.mock.calls
      .map((call) => String(call[1]))
      .find((c) => c.includes('find'))
    expect(listCommand).toContain("-name 'dorkad-*'")
  ...
  it('leaves relay directories alone even when the host hands them to it', async () => {
    ...
    const removed: string[] = []
    scriptHost({
      listing: ['relay-0.1.0+aa', 'relay-0.0.9+ff', 'dorkad-0.1.0+aa'],
      removed
    })
    await gcOldDorkadVersions({
      conn,
    ...
    expect(removed).toEqual(['dorkad-0.1.0+aa'])
  })

  it('never removes the active or the previous version', async () => {
    const removed: string[] = []
    scriptHost({
      listing: ['dorkad-0.1.0+01d', 'dorkad-0.2.0+9ee0', 'dorkad-0.3.0+cc0'],
      removed
    })
    await gcOldDorkadVersions({
      conn,
      host,
      remoteHome: '/home/u',
      currentDirAbsPath: '/home/u/.dorka-remote/dorkad-0.3.0+cc0',
      record: {
    ...
    expect(removed).toEqual(['dorkad-0.1.0+01d'])
  })

  it('never removes the version a live daemon was forked from', async () => {
    const removed: string[] = []
    scriptHost({ listing: ['dorkad-0.1.0+01d', 'dorkad-0.0.9+01de'], removed })
    await gcOldDorkadVersions({
      conn,
      host,
      remoteHome: '/home/u',
      currentDirAbsPath: '/home/u/.dorka-remote/dorkad-0.3.0+cc0',
      record: { ...emptyDorkadActivationRecord(), active: '0.3.0+cc0' },
      liveDaemonVersion: '0.1.0+01d'
    })
    expect(removed).toEqual(['dorkad-0.0.9+01de'])
  })

  it('treats an unanswerable liveness probe as in use', async () => {
    const removed: string[] = []
    scriptHost({
      listing: ['dorkad-0.1.0+bb0', 'dorkad-0.0.9+dead'],
      liveness: { 'dorkad-0.1.0+bb0': 'UNKNOWN', 'dorkad-0.0.9+dead': 'DEAD' },
      removed
    })
    await gcOldDorkadVersions({
      conn,
    ...
    expect(removed).toEqual(['dorkad-0.0.9+dead'])
  })
})
