/**
 * The two things Dorka installs into `~/.dorka-remote/`, and the rules that keep them from
 * touching each other.
 *
 * `docs/design/shipping-dorkad.html` §06 settles that on-disk coexistence is permanent: the
 * relay is the dumb execution host for SSH-target users, dorkad is the peer for paired
 * environments, and no plan item retires either. So `relay-<version>/` and `dorkad-<version>/`
 ...
 * deleting a live dorkad tree, so the ownership rule is asserted here rather than left to
...
import {
  ...
  DORKAD_INSTALL_COMPLETE_FILENAME,
  DORKAD_VERSION_FILENAME
} from '../../shared/dorkad-artifacts'

export type RemoteInstallModelId = 'relay' | 'dorkad'
...
export const RELAY_INSTALL_MODEL: RemoteInstallModel = {
  id: 'relay',
  dirPrefix: 'relay',
  nativeDepsPackageName: 'dorka-relay',
...
export const DORKAD_INSTALL_MODEL: RemoteInstallModel = {
  id: 'dorkad',
  dirPrefix: 'dorkad',
  nativeDepsPackageName: 'dorka-dorkad',
  versionFilename: DORKAD_VERSION_FILENAME,
  installCompleteFilename: DORKAD_INSTALL_COMPLETE_FILENAME,
  // Why the parameter is ignored: dorkad's forked children are the same three .js files on
  // every host. The Windows-only console-list agent patch is a relay/node-pty concern.
  requiredArtifacts: () => dorkadArtifactFilenames()
}
...
/**
 * Which model owns a directory found in `~/.dorka-remote/`, or null for anything neither
 * model created.
 *
 * This is the answer to §06 falsifier 1's first half: **the model that created a directory
 * owns it, and nothing else may delete it.** A relay GC pass that saw `dorkad-0.1.0+abc`
...
/** Group a raw `~/.dorka-remote/` listing by owning model, for diagnostics and the client's choice. */
export function inventoryRemoteInstallDirs(dirNames: readonly string[]): RemoteInstallInventory {
  const inventory: RemoteInstallInventory = { relay: [], dorkad: [], unknown: [] }
