/**
 * When an update may restart dorkad, and when going back is still sound.
 *
 * Two constraints shape everything here.
 *
 * **The daemon must outlive the restart.** dorkad forks the terminal daemon and deliberately
 * does not kill it on stop (`dorkad-daemon-supervision.ts` uses `disconnectDaemon`, never
 ...
 *
 * **The state root is shared across versions.** `~/.dorka/` (or `$DORKA_USER_DATA`) is outside
 * every version dir, and Dorka's persisted state carries no schema version — migrations run
 ...
 */
import type { DorkadActivationRecord } from './dorkad-activation-record'

export type DorkadTerminalCensus = {
  /**
...
export type DorkadUpdateDecision =
  ...
  | { action: 'defer'; code: DorkadUpdateDeferCode; reason: string }

export type DorkadUpdateDeferCode =
  | 'dorkad_update_terminals_running'
  | 'dorkad_update_terminal_census_unavailable'

/**
 * Decide whether to restart dorkad onto `candidateVersion`.
 *
 * Deferring on live terminals is a deliberate choice, not caution. The restart itself is
 * non-destructive, but it leaves the host running a NEW dorkad against an OLD daemon until
...
export function planDorkadUpdate(input: {
  ...
  if (liveSessions > 0 && !input.force) {
    return {
      action: 'defer',
      code: 'dorkad_update_terminals_running',
      reason:
        ...
        `dorkad ${input.candidateVersion} against a daemon forked from ` +
...
export type DorkadRollbackSafety =
  | { safety: 'clean'; target: string; notes: string[] }
  | { safety: 'lossy'; target: string; discards: string[] }
  | { safety: 'unsafe'; code: DorkadRollbackUnsafeCode; reason: string }

export type DorkadRollbackUnsafeCode =
  | 'dorkad_rollback_no_target'
  | 'dorkad_rollback_snapshot_missing'
  | 'dorkad_rollback_orphans_live_terminals'
  | 'dorkad_rollback_census_unavailable'
 ...
 */
export function assessDorkadRollback(input: {
  record: DorkadActivationRecord
...
}): DorkadRollbackSafety {
  const target = input.record.previous
  if (!target) {
    return {
      safety: 'unsafe',
      code: 'dorkad_rollback_no_target',
      reason:
        'This host has no previous dorkad version recorded, so there is nothing to roll back ' +
  ...
  if (!input.record.snapshot || !input.snapshotPresent) {
    return {
      safety: 'unsafe',
      code: 'dorkad_rollback_snapshot_missing',
      reason:
        `The pre-activation state snapshot for ${input.record.active ?? 'the active version'} ` +
        'is gone, and Dorka state carries no schema version that could prove the older build ' +
  ...
  if (startedSinceActivation === null) {
    return {
      safety: 'unsafe',
      code: 'dorkad_rollback_census_unavailable',
      reason:
