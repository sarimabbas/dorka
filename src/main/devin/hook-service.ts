...
import {
  ...
  readConfigFromDorkaOverlapDetail,
...
function getManagedScript(target: 'local' | 'posix' = 'local'): string {
  if (target === 'local' && process.platform === 'win32') {
    return [
      '@echo off',
      'setlocal',
      // Why: endpoint file holds the live port/token; a PTY that outlives an Dorka restart carries stale env, so `call` it to refresh (else PTY env).
  ...
  return [
    ...
    'printf \'%s\' "$payload" | curl -sS -X POST "http://127.0.0.1:${DORKA_AGENT_HOOK_PORT}/hook/devin" \\',
...
export class DevinHookService {
  ...
  getStatus(): AgentHookInstallStatus {
    ...
    return {
      ...
      configPath,
      managedHooksPresent,
      detail: mergeHookInstallDetail(detail, readConfigFromDorkaOverlapDetail(config))
    }
  }
  ...
  async installRemote(sftp: SFTPWrapper, remoteHome: string): Promise<AgentHookInstallStatus> {
    ...
    try {
      ...
      // Why: SSH remotes use POSIX `.sh` hooks even when Dorka runs on Windows; never derive remote script syntax from local OS.
