...
describe('PowerShell OSC 133 bootstrap', () => {
  it('wraps prompt/readline without bypassing profiles or execution policy', () => {
    ...
    expect(script).toContain('DORKA_OPENCODE_CONFIG_DIR')
    expect(script).toContain('DORKA_MIMOCODE_HOME')
    expect(script).not.toContain('DORKA_PI_CODING_AGENT_DIR')
    expect(script).not.toContain('DORKA_OMP_CODING_AGENT_DIR')
    expect(script).toContain('DORKA_OMP_STATUS_EXTENSION')
    expect(script).toContain('function Global:omp')
    expect(script).toContain('--extension $env:DORKA_OMP_STATUS_EXTENSION')
    expect(script).toContain('DORKA_CODEX_HOME')
    expect(script).toContain('DORKA_CODEX_LAUNCH_PREFLIGHT')
    expect(script).toContain('function Global:codex')
    expect(script).not.toContain('$Global:__DorkaCodexExecutable')
    ...
    const codexHomeRestore = script.indexOf('if ($env:DORKA_CODEX_HOME)')
