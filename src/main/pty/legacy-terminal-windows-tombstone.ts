...
set "dorka_real=%DORKA_REAL___DORKA_UPPER_COMMAND__%"
set "dorka_wrapper_dir=%~dp0"
set "dorka_legacy_wrapper_dir=%DORKA_ATTRIBUTION_SHIM_DIR%"
set "dorka_clean_path="
...
set "dorka_legacy_norm="
if defined dorka_legacy_wrapper_dir call :dorka_normalize_legacy_dir
...
if not defined PATH goto :dorka_path_walked
...
rem below ever saw it, and the cwd would then be searched for __DORKA_COMMAND__. Proven on
...
for %%P in ("%PATH:;=" "%") do (
  set "dorka_entry=%%~P"
  call :dorka_append_path
)
:dorka_path_walked
set "PATH=%dorka_clean_path%"
set "DORKA_ENABLE_GIT_ATTRIBUTION="
set "DORKA_GIT_COMMIT_TRAILER="
set "DORKA_GH_PR_FOOTER="
set "DORKA_GH_ISSUE_FOOTER="
set "DORKA_ATTRIBUTION_SHIM_DIR="
set "DORKA_ATTRIBUTION_BYPASS="
set "DORKA_REAL_GIT="
set "DORKA_REAL_GH="
if defined dorka_real for %%G in ("%dorka_real%") do if /I "%%~dpG"=="%~dp0" set "dorka_real="
rem Why: a captured path may be relative, and both "if exist" and the invocation resolve it
rem against the current directory, so an inherited .\__DORKA_COMMAND__.exe would run from the repo.
set "dorka_probe=%dorka_real%"
if defined dorka_real call :dorka_check_rooted
if defined dorka_real if not defined dorka_rooted set "dorka_real="
rem Why: clear a captured path that no longer exists, or the PATH walk below is skipped.
if defined dorka_real if not exist "%dorka_real%" set "dorka_real="
if defined dorka_real goto run
rem Why: an unqualified Windows command lookup searches the current directory before PATH, so a
rem repository-local __DORKA_COMMAND__.exe would win. Walk the cleaned PATH ourselves instead.
if not defined dorka_clean_path goto :dorka_candidates_walked
for %%P in ("%dorka_clean_path:;=" "%") do (
  set "dorka_entry=%%~P"
  call :dorka_try_candidate
)
:dorka_candidates_walked
if not defined dorka_real (
  echo Dorka compatibility wrapper could not locate __DORKA_COMMAND__ on PATH. 1>&2
  exit /b 127
)
:run
"%dorka_real%" %*
...
:dorka_normalize_legacy_dir
rem Why the rooted test first: full-path expansion resolves a relative value against the current
rem directory, so a
rem relative DORKA_ATTRIBUTION_SHIM_DIR would let the cwd decide which PATH entry counts as the
...
set "dorka_probe=%dorka_legacy_wrapper_dir%"
call :dorka_check_rooted
if not defined dorka_rooted exit /b
for %%G in ("%dorka_legacy_wrapper_dir%") do set "dorka_legacy_norm=%%~fG"
rem Why: full-path expansion preserves a trailing separator; normalize before comparing.
if "%dorka_legacy_norm:~-1%."=="%dorka_sep%." set "dorka_legacy_norm=%dorka_legacy_norm:~0,-1%"
exit /b
...
rem Why dorka_probe rather than an argument: see the CALL re-expansion note above.
set "dorka_rooted="
rem Why: an unset probe would leave the substring syntax below unexpanded and mangle the line.
if not defined dorka_probe exit /b
if "%dorka_probe:~0,2%"=="\\" set "dorka_rooted=1"
if "%dorka_probe:~1,2%"==":\" set "dorka_rooted=1"
if "%dorka_probe:~1,2%"==":/" set "dorka_rooted=1"
exit /b

:dorka_try_candidate
if defined dorka_real exit /b
if not defined dorka_entry exit /b
rem Why: a relative entry resolves against the current directory, same exposure as an empty one.
set "dorka_probe=%dorka_entry%"
call :dorka_check_rooted
if not defined dorka_rooted exit /b
for %%G in ("%dorka_entry%") do set "dorka_candidate_dir=%%~fG"
...
if "%dorka_candidate_dir:~-1%."=="%dorka_sep%." set "dorka_candidate_dir=%dorka_candidate_dir:~0,-1%"
...
if /I "%dorka_candidate_dir%\"=="%dorka_wrapper_dir%" exit /b
if exist "%dorka_candidate_dir%\__DORKA_COMMAND__.exe" set "dorka_real=%dorka_candidate_dir%\__DORKA_COMMAND__.exe"
if not defined dorka_real if exist "%dorka_candidate_dir%\__DORKA_COMMAND__.cmd" set "dorka_real=%dorka_candidate_dir%\__DORKA_COMMAND__.cmd"
if not defined dorka_real if exist "%dorka_candidate_dir%\__DORKA_COMMAND__.bat" set "dorka_real=%dorka_candidate_dir%\__DORKA_COMMAND__.bat"
exit /b
...
call :dorka_check_rooted
if not defined dorka_rooted exit /b
for %%G in ("%dorka_entry%") do set "dorka_path_entry_dir=%%~fG"
rem Why: full-path expansion preserves a trailing separator; normalize before comparing.
if "%dorka_path_entry_dir:~-1%."=="%dorka_sep%." set "dorka_path_entry_dir=%dorka_path_entry_dir:~0,-1%"
set "dorka_path_entry_dir=%dorka_path_entry_dir%\"
if /I "%dorka_path_entry_dir%"=="%dorka_wrapper_dir%" exit /b
set "dorka_skip_entry="
if defined dorka_legacy_wrapper_dir call :dorka_reject_legacy_dir
if defined dorka_skip_entry exit /b
if defined dorka_clean_path (set "dorka_clean_path=%dorka_clean_path%;%dorka_entry%") else set "dorka_clean_path=%dorka_entry%"
exit /b

:dorka_reject_legacy_dir
rem Why: an unrooted legacy dir leaves this unset, and comparing against a bare separator could
rem only ever misfire.
if not defined dorka_legacy_norm exit /b
if /I "%dorka_path_entry_dir%"=="%dorka_legacy_norm%\" set "dorka_skip_entry=1"
exit /b
`

const POWERSHELL_PASSTHROUGH_WRAPPER = String.raw`$ErrorActionPreference = 'Stop'
$commandName = '__DORKA_COMMAND__'
$realCommand = [Environment]::GetEnvironmentVariable('DORKA_REAL___DORKA_UPPER_COMMAND__')
$wrapperDir = Split-Path -Parent $MyInvocation.MyCommand.Path
$legacyWrapperDir = $env:DORKA_ATTRIBUTION_SHIM_DIR
...
'DORKA_ENABLE_GIT_ATTRIBUTION', 'DORKA_GIT_COMMIT_TRAILER', 'DORKA_GH_PR_FOOTER', 'DORKA_GH_ISSUE_FOOTER', 'DORKA_ATTRIBUTION_SHIM_DIR', 'DORKA_REAL_GIT', 'DORKA_REAL_GH', 'DORKA_ATTRIBUTION_BYPASS' | ForEach-Object { Remove-Item "Env:$_" -ErrorAction SilentlyContinue }
...
if (-not $realCommand) {
  [Console]::Error.WriteLine("Dorka compatibility wrapper could not locate $commandName on PATH.")
  exit 127
}
& $realCommand @args
exit $LASTEXITCODE
...
function renderWindowsWrapper(template: string, command: string, upperCommand: string): string {
  return template
    .replaceAll('__DORKA_UPPER_COMMAND__', upperCommand)
    .replaceAll('__DORKA_COMMAND__', command)
}
