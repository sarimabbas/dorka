...
// Why: Dorka launches bash as a login shell (see local-pty-shell-ready.ts
...
// .bashrc. Scanning .bashrc would mirror values the live Dorka bash never sees.
...
function shellStartupFiles(
  ...
  if (!shell) {
    // Why: Dorka's POSIX default shell is /bin/zsh when $SHELL is unset.
  ...
  // Dorka's zsh/bash/fish shell-ready startup files, so scanning those files
 ...
 *
 * Why: GUI-launched Dorka does not inherit interactive shell exports, but the
