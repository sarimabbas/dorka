...
const POSIX_TOMBSTONE = String.raw`#!__DORKA_INTERPRETER__
set -u

command_name="__DORKA_COMMAND__"
...
legacy_wrapper_dir="${SHELL_DOLLAR}{DORKA_ATTRIBUTION_SHIM_DIR:-}"
...
filter_path() {
  ...
  while true; do
    ...
    if [[ "$entry" != /* ]]; then
      ...
      # directory, so a relative DORKA_ATTRIBUTION_SHIM_DIR let the cwd decide which PATH entry
...
filter_path
unset DORKA_ENABLE_GIT_ATTRIBUTION DORKA_GIT_COMMIT_TRAILER DORKA_GH_PR_FOOTER
unset DORKA_GH_ISSUE_FOOTER DORKA_ATTRIBUTION_SHIM_DIR DORKA_REAL_GIT DORKA_REAL_GH DORKA_ATTRIBUTION_BYPASS
...
if [[ -z "$real_command" ]]; then
  printf 'Dorka compatibility wrapper could not locate %s on PATH.\n' "$command_name" >&2
  exit 127
fi
...
export function renderLegacyTerminalPosixTombstone(
  ...
  return POSIX_TOMBSTONE.replaceAll('__DORKA_INTERPRETER__', interpreter).replaceAll(
    '__DORKA_COMMAND__',
    command
  )
}
