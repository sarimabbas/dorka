...
describe('legacy terminal shim neutralization', () => {
  ...
  const makeUserDataDir = (): string => {
    const userData = mkdtempSync(join(tmpdir(), 'dorka-legacy-shim-'))
  ...
  it('atomically replaces the legacy command paths with executable tombstones', () => {
    ...
    const legacyRoot = join(userData, 'dorka-terminal-attribution')
    ...
    for (const path of [
      ...
      expect(readFileSync(path, 'utf8')).not.toContain('Co-authored-by: Dorka')
  ...
  itOnPosix('resolves the interpreter from absolute PATH entries only', () => {
    ...
    const cwdRelName = `.dorka-interp-${process.pid}`
  ...
  itOnPosix('resolves its own directory even when CDPATH is set', () => {
    ...
    const userData = makeUserDataDir()
    const posixDir = join(userData, 'dorka-terminal-attribution', 'posix')
    ...
    // so the fixture must mirror `dorka-terminal-attribution/posix`, not just its first segment.
    mkdirSync(join(cdpathDir, 'dorka-terminal-attribution', 'posix'), { recursive: true })
    writeFileSync(join(posixDir, 'git'), 'legacy attribution wrapper')
    ...
    const run = spawnSync('/bin/bash', ['dorka-terminal-attribution/posix/git', '--version'], {
      cwd: userData,
  ...
  itOnPosix('rejects a distinct legacy shim directory named by the environment', () => {
    // Why: DORKA_ATTRIBUTION_SHIM_DIR can name a *different* directory than the wrapper's own (an
    ...
    const run = spawnSync(join(posixDir, 'git'), ['--version'], {
      env: {
        ...process.env,
        DORKA_ATTRIBUTION_SHIM_DIR: legacyDir,
  ...
  itOnPosix('keeps the POSIX wrapper on a host where no absolute bash resolves', async () => {
    ...
    try {
      ...
      expect(git.split('\n')[0]).toBe('#!/bin/bash')
      expect(git).toContain('Dorka compatibility wrapper could not locate')
  ...
  it('strips a captured shim directory spelled with repeated separators', () => {
    ...
    const posixEnv = {
      ...
      DORKA_ATTRIBUTION_SHIM_DIR: '/custom/elsewhere'
    }
    ...
    const windowsEnv = {
      ...
      DORKA_ATTRIBUTION_SHIM_DIR: 'C:\\Custom\\Else'
    }
  ...
  itOnPosix(
    ...
    () => {
      ...
      // relative DORKA_ATTRIBUTION_SHIM_DIR let the cwd decide which PATH entry counted as the legacy
      ...
      const outputs = [userData, join(userData, 'dorka-terminal-attribution')].map((cwd) => {
        const run = spawnSync(join(posixDir, 'git'), ['--version'], {
          cwd,
          env: {
            ...process.env,
            DORKA_ATTRIBUTION_SHIM_DIR: 'safe-bin',
  ...
  it('keeps a POSIX directory whose name ends in a backslash', () => {
    ...
    const env = {
      PATH: '/tmp/captured\\',
      DORKA_ATTRIBUTION_SHIM_DIR: '/tmp/captured'
    }
    ...
    const windowsEnv = { Path: 'C:\\captured\\', DORKA_ATTRIBUTION_SHIM_DIR: 'C:\\captured' }
    stripLegacyTerminalShimEnv(windowsEnv, 'win32')
  ...
  itOnPosix('hands the child the cleaned PATH, not the inherited one', () => {
    ...
    expect(run.stdout).not.toContain('dorka-terminal-attribution')
  ...
  itOnPosix('does not let the cwd supply the script interpreter', async () => {
    ...
    const userData = makeUserDataDir()
    const shimDir = join(userData, 'dorka-terminal-attribution', 'posix')
    const realBin = join(userData, 'real-bin')
    const hostile = join(userData, 'hostile')
  ...
  it('removes every Windows PATH occurrence of both captured wrapper directories', () => {
    const userData = makeUserDataDir()
    const win32Dir = join(userData, 'dorka-terminal-attribution', 'win32')
    mkdirSync(win32Dir, { recursive: true })

    neutralizeLegacyTerminalShimDir(userData)

    const cmd = readFileSync(join(win32Dir, 'git.cmd'), 'utf8')
    const cmdCapture = 'set "dorka_legacy_wrapper_dir=%DORKA_ATTRIBUTION_SHIM_DIR%"'
    expectOrdered(cmd, cmdCapture, 'set "DORKA_ATTRIBUTION_SHIM_DIR="')
    expect(cmd).toContain('for %%P in ("%PATH:;=" "%") do (')
    expect(cmd).toContain('if /I "%dorka_path_entry_dir%"=="%dorka_wrapper_dir%" exit /b')
    expect(cmd).toContain('if defined dorka_legacy_wrapper_dir call :dorka_reject_legacy_dir')
    // Why: `call :label && ...` is not valid cmd; the flag variable is what makes it work.
    expect(cmd).toContain('if defined dorka_skip_entry exit /b')
    expect(cmd).not.toContain('call :dorka_reject_legacy_dir &&')
    expect(cmd).toContain('if "%dorka_path_entry_dir:~-1%."=="%dorka_sep%."')
  ...
  it('leaves an install that never ran the shim untouched', () => {
    ...
    expect(existsSync(join(userData, 'dorka-terminal-attribution'))).toBe(false)
  })
  ...
  itOnPosix('keeps a real Bash command hash working with trailing PATH separators', async () => {
    ...
    const child = spawn('bash', ['--noprofile', '--norc'], {
      ...
      env: {
        ...
        DORKA_ENABLE_GIT_ATTRIBUTION: '1',
        DORKA_GIT_COMMIT_TRAILER: 'Co-authored-by: Dorka <help@stably.ai>',
        DORKA_ATTRIBUTION_SHIM_DIR: ''
      },
    ...
    const ready = waitForOutput(child.stdout, '__DORKA_HASH_READY__\n')
    child.stdin.write(`hash -p ${quoteBash(shimGit)} git\nprintf '__DORKA_HASH_READY__\\n'\n`)
    ...
    expect(stdout).not.toContain('Co-authored-by: Dorka')
    expect(stderr).toBe('fixture stderr\n')
  })

  it('drops inherited shim env and its PATH entry without touching real entries', () => {
    const env: Record<string, string> = {
      PATH: `/home/u/.dorka/dorka-terminal-attribution/posix:/usr/local/bin:/usr/bin`,
      DORKA_ENABLE_GIT_ATTRIBUTION: '1',
      DORKA_GIT_COMMIT_TRAILER: 'Co-authored-by: Dorka <help@stably.ai>',
      DORKA_GH_PR_FOOTER: 'footer',
      DORKA_GH_ISSUE_FOOTER: 'footer',
      DORKA_ATTRIBUTION_SHIM_DIR: '/home/u/.dorka/dorka-terminal-attribution/posix',
      DORKA_REAL_GIT: '/usr/bin/git',
      DORKA_REAL_GH: '/usr/bin/gh',
      HOME: '/home/u'
    }
  ...
  it('uses the captured POSIX shim directory literally when it contains a colon', () => {
    const shimDir = '/tmp/dorka:user/dorka-terminal-attribution/posix'
    const env: Record<string, string> = {
      PATH: `/usr/local/bin:${shimDir}:/usr/bin`,
      DORKA_ATTRIBUTION_SHIM_DIR: shimDir
    }
  ...
  it('treats legacy Windows environment keys case-insensitively', () => {
    const shimDir = 'C:\\Users\\dorka;user\\dorka-terminal-attribution\\win32'
    const env: Record<string, string> = {
      Path: `${shimDir};C:\\Windows\\System32`,
      dorka_attribution_shim_dir: shimDir,
      Dorka_Enable_Git_Attribution: '1',
      dorka_real_git: 'C:\\Git\\git.exe'
    }
  ...
  it('strips legacy entries from every Windows PATH spelling', () => {
    const env: Record<string, string> = {
      PATH: 'C:\\Dorka\\dorka-terminal-attribution\\win32',
      Path: 'C:\\Dorka\\dorka-terminal-attribution\\win32;C:\\Windows\\System32'
    }
  ...
  it('strips legacy shim entries that carry a trailing separator', () => {
    // Why: without normalizing the trailing separator the entry does not match, so Dorka's own
    // scrub leaves the legacy shim directory on the spawned PATH and the wrapper stays reachable.
    const posix: Record<string, string> = {
      PATH: '/home/u/.dorka/dorka-terminal-attribution/posix/:/usr/bin'
    }
    stripLegacyTerminalShimEnv(posix, 'linux')
    expect(posix.PATH).toBe('/usr/bin')
    ...
    const win: Record<string, string> = {
      Path: 'C:\\Users\\u\\dorka-terminal-attribution\\win32\\;C:\\Windows'
    }
    stripLegacyTerminalShimEnv(win, 'win32')
    expect(win.Path).toBe('C:\\Windows')
  })
  ...
  it('keeps neighbouring directories that merely share the name prefix', () => {
    const env: Record<string, string> = {
      PATH: '/opt/dorka-terminal-attribution:/opt/dorka-terminal-attribution/custom-tools:/home/u/dorka-terminal-attribution-notes/bin:/usr/bin'
    }

    stripLegacyTerminalShimEnv(env, 'linux')

    expect(env.PATH).toBe(
      '/opt/dorka-terminal-attribution:/opt/dorka-terminal-attribution/custom-tools:/home/u/dorka-terminal-attribution-notes/bin:/usr/bin'
    )
  })
