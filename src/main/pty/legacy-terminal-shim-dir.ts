...
const LEGACY_TERMINAL_ATTRIBUTION_ENABLE_ENV_KEY = 'DORKA_ENABLE_GIT_ATTRIBUTION'
const LEGACY_TERMINAL_ATTRIBUTION_BYPASS_ENV_KEY = 'DORKA_ATTRIBUTION_BYPASS'

const LEGACY_SHIM_ROOT_DIR = 'dorka-terminal-attribution'
...
export const LEGACY_TERMINAL_SHIM_ENV_KEYS = [
  'DORKA_ENABLE_GIT_ATTRIBUTION',
  'DORKA_GIT_COMMIT_TRAILER',
  'DORKA_GH_PR_FOOTER',
  'DORKA_GH_ISSUE_FOOTER',
...
function writeFileAtomically(filePath: string, contents: string, mode: number): void {
  const temporaryPath = `${filePath}.dorka-neutralizing-${process.pid}`
  try {
...
export function isLegacyTerminalShimPathEntry(entry: string): boolean {
  // Why `windows` unconditionally here: this classifier only ever matches Dorka's own
  // `dorka-terminal-attribution/{posix,win32}` layout, and a Windows PATH can reach it through the
...
export function stripLegacyTerminalShimEnv(
  ...
  const shimDirKey = 'DORKA_ATTRIBUTION_SHIM_DIR'.toLowerCase()
  const explicitShimDirs = Object.entries(env)
    .filter(([key]) =>
      windows ? key.toLowerCase() === shimDirKey : key === 'DORKA_ATTRIBUTION_SHIM_DIR'
    )
    .map(([, value]) => value)
    .filter(Boolean)
