...
describe('removeAppImageRuntimeEnv', () => {
  ...
  it('removes AppImage runtime identity and mount paths from Linux terminal env', () => {
    ...
    const env = {
      APPIMAGE: '/data/apps/dorka.appimage',
      APPDIR: '/tmp/.mount_dorka123',
      ARGV0: '/data/apps/dorka.appimage',
      OWD: '/home/user',
      APPIMAGE_LIBRARY_PATH: '/tmp/.mount_dorka123/usr/lib',
      PATH: ['/tmp/.mount_dorka123', '/tmp/.mount_dorka123/usr/sbin', '/usr/bin'].join(delimiter),
      LD_LIBRARY_PATH: ['/tmp/.mount_dorka123/usr/lib', '/opt/audio/lib'].join(delimiter),
      HOME: '/home/user'
    }
  ...
  it('leaves AppImage-looking env untouched outside Linux', () => {
    ...
    const env = {
      ...
      APPDIR: '/tmp/.mount_dorka123',
      ARGV0: '/data/apps/dorka.appimage',
      PATH: ['/tmp/.mount_dorka123/usr/bin', '/usr/bin'].join(delimiter)
    }
