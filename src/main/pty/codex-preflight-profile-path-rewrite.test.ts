...
function buildFixture(options: { aliasCodex?: boolean } = {}): Fixture {
  const root = mkdtempSync(join(tmpdir(), 'dorka-codex-profile-path-'))
  roots.push(root)
  ...
  // The CLI Dorka ships, at the absolute path Dorka controls.
  writeStub(getBundledLauncherPath(process.platform, resourcesPath) as string, intendedMarker)
  // The impostor a user's own bin directory could hold under every CLI name Dorka uses.
  for (const name of ['dorka', 'dorka-ide', 'dorka-dev']) {
...
function launchCodexThroughRcfile(fixture: Fixture, preflightValue: string): void {
  ...
  // Why the second command: the rcfile defines __dorka_osc133_preexec *below* the
  ...
  const command = `codex --version; declare -F __dorka_osc133_preexec >/dev/null && printf ran > ${JSON.stringify(fixture.postSnippetMarker)}; :`
  execFileSync('/bin/bash', ['--rcfile', rcfilePath, '-i', '-c', command], {
    ...
    env: {
      ...
      TERM: 'dumb',
      SHELL: '/bin/bash',
      // Why no DORKA_SHELL_FEATURES: absent means no features, so the rcfile
      // emits neither the identity nor the readiness marker into stdout.
      DORKA_CODEX_HOME: join(fixture.root, 'codex-home'),
      DORKA_CODEX_LAUNCH_PREFLIGHT: preflightValue
    }
  })
}
...
describe.skipIf(!bashAvailable)('Codex preflight under a profile-rewritten PATH', () => {
  it('sources the user profile before emitting the codex wrapper', () => {
    ...
    expect(rcfile.indexOf('source "$HOME/.bash_profile"')).toBeLessThan(
      rcfile.indexOf('DORKA_CODEX_LAUNCH_PREFLIGHT')
    )
  })
  ...
  it('skips an unqualified preflight value while still launching codex', () => {
    ...
    launchCodexThroughRcfile(fixture, 'dorka')
