...
describe('legacy terminal Windows tombstone text', () => {
  ...
  const makeUserDataDir = (): string => {
    const userData = mkdtempSync(join(tmpdir(), 'dorka-legacy-shim-win-'))
  ...
  it('rejects stale Windows real-command paths inside the wrapper directory', () => {
    ...
    expect(cmd).toContain('if defined dorka_real if not exist "%dorka_real%" set "dorka_real="')
    expectOrdered(cmd, 'if not exist "%dorka_real%"', ':dorka_try_candidate')
  ...
  it('resolves Windows fallbacks against PATH only, never the current directory', () => {
    ...
    for (const command of ['git', 'gh'] as const) {
      ...
      expect(cmd).toContain('for %%P in ("%dorka_clean_path:;=" "%") do (')
      expect(cmd).toContain(`if exist "%dorka_candidate_dir%\\${command}.exe"`)
      ...
      expect(cmd).toContain('set "dorka_sep=\\"')
      ...
      // Why: a captured DORKA_REAL_* may be relative, and both the existence test and the
      // invocation resolve it against the cwd.
      expect(cmd).toContain('if defined dorka_real call :dorka_check_rooted')
      expect(cmd).toContain('if defined dorka_real if not defined dorka_rooted set "dorka_real="')
      ...
      // Why the exact block, not an ordering pin: the first ':dorka_append_path' in the file is
      // the top-level CALL, and the first 'if not defined dorka_rooted' belongs to a different
      // subroutine, so an ordering pin stayed green with this subroutine's guard deleted.
      expect(cmd).toContain(':dorka_append_path\r\nif not defined dorka_entry exit /b\r\n')
      expect(cmd).toContain(
      ...
      expect(cmd).not.toContain('call :dorka_check_rooted "')
      expect(cmd).not.toContain('call :dorka_try_candidate "')
      expect(cmd).not.toContain('call :dorka_append_path "')
      expect(cmd).not.toContain('%~1')
      expect(cmd).toContain('set "dorka_probe=%dorka_entry%"\r\ncall :dorka_check_rooted\r\n')
      // Why: dorka_sep is compared against before the legacy dir is normalized; defining it later
      // made that strip silently no-op and left the legacy dir on PATH.
      expectOrdered(cmd, 'set "dorka_sep=', 'if "%dorka_legacy_norm:~-1%."')
      // Why: %dorka_sep% expands before the line is parsed, so `=="%dorka_sep%"` becomes `=="\"` —
      // the literal trap the variable exists to avoid. Both sides must carry the trailing dot.
      expect(cmd).not.toContain('=="%dorka_sep%"')
      ...
      expectOrdered(
        cmd,
        'call :dorka_check_rooted\r\nif not defined dorka_rooted exit /b',
        'for %%G in ("%dorka_entry%") do set "dorka_candidate_dir='
      )
      expect(cmd).toContain('if not defined dorka_rooted exit /b')
      expect(cmd).toContain('if "%dorka_candidate_dir:~-1%."=="%dorka_sep%."')
      expect(cmd).not.toContain(':\\#=#%')
      ...
      expect(cmd).toContain('if /I "%dorka_candidate_dir%\\"=="%dorka_wrapper_dir%" exit /b')
      ...
      expect(cmd).toContain('if "%dorka_probe:~1,2%"==":\\" set "dorka_rooted=1"')
      expect(cmd).toContain('if "%dorka_probe:~0,2%"=="\\\\" set "dorka_rooted=1"')
      ...
      expect(cmd).toContain('if not defined dorka_entry exit /b')
      ...
      expect(cmd).toContain('if "%dorka_legacy_norm:~-1%."=="%dorka_sep%."')
      ...
      expect(cmd).toContain('if defined dorka_legacy_wrapper_dir call :dorka_normalize_legacy_dir')
      expect(cmd).not.toContain('if defined dorka_legacy_wrapper_dir for ')
  ...
  it('guards both cmd PATH walks so an empty variable cannot break parsing', () => {
    ...
    for (const command of ['git', 'gh'] as const) {
      const cmd = readFileSync(join(win32Dir, `${command}.cmd`), 'utf8')
      expect(cmd).toContain('if not defined PATH goto :dorka_path_walked')
      expect(cmd).toContain('if not defined dorka_clean_path goto :dorka_candidates_walked')
      for (const [guard, loop] of [
        ['if not defined PATH goto :dorka_path_walked', 'for %%P in ("%PATH:;='],
        [
          'if not defined dorka_clean_path goto :dorka_candidates_walked',
          'for %%P in ("%dorka_clean_path:;='
        ]
      ] as const) {
