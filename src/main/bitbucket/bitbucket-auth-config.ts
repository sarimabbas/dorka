...
export function getEnvAuthConfig(): BitbucketAuthConfig {
  return {
    baseUrl: envValue('DORKA_BITBUCKET_API_BASE_URL') ?? DEFAULT_API_BASE_URL,
    accessToken: envValue('DORKA_BITBUCKET_ACCESS_TOKEN'),
    email: envValue('DORKA_BITBUCKET_EMAIL'),
    apiToken: envValue('DORKA_BITBUCKET_API_TOKEN')
  }
}
