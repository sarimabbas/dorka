...
describe('terminal-history', () => {
  ...
  describe('injectHistoryEnv', () => {
    ...
    it('records the history DIRECTORY from the SPAWN env, not this process env', () => {
      ...
      // Dorka was launched with a different XDG_DATA_HOME than the shells it spawns.
    ...
    it('drops an DORKA_HISTFILE inherited from a parent Dorka PTY', () => {
      // Why: an Dorka terminal opened from inside another Dorka terminal inherits
      // it, and the zsh wrapper would then re-export the PARENT worktree's
      // history path here. Credit: caught by @innocarpe in #11146.
      const env: Record<string, string> = {
        DORKA_HISTFILE: ['', 'other', 'wt', 'zsh_history'].join(sep)
      }

      injectHistoryEnv(env, 'repo-1::/path/wt', '/bin/zsh', '/path/wt')

      expect(env.DORKA_HISTFILE).toBe(env.HISTFILE)
      expect(env.DORKA_HISTFILE).not.toContain('other')
    })

    it('drops an inherited DORKA_HISTFILE even when it injects nothing', () => {
      ...
      expect(env.DORKA_HISTFILE).toBeUndefined()
    })
    ...
    ])('drops a %s HISTFILE inherited from a parent Dorka pane', (_kind, inherited) => {
      // HISTFILE stays EXPORTED once the wrapper restores it, so an Dorka launched
    ...
    it.each([
      ...
      // Dorka only ever mints absolute paths, so the same shape relative to the
      // user's cwd is theirs.
      [
        'a relative path of Dorka’s shape',
        ['terminal-history', OTHER_WORKTREE_HASH, 'zsh_history'].join(sep)
      ]
    ])('preserves %s the user set as HISTFILE', (_kind, histFile) => {
      // Only a path Dorka minted is droppable; everything else is the user's.
    ...
    ])('replaces a %s fish_history inherited from a parent Dorka', (_kind, inherited) => {
      // fish EXPORTS fish_history, so an Dorka launched from a fish pane hands the
  ...
  describe('injectWslFishHistoryEnv', () => {
    ...
    it('replaces an inherited Dorka fish_history with this worktree session', () => {
      const env: Record<string, string> = {
        fish_history: fishHistorySessionName(hashWorktreeId('repo-1::/path/other-wt'))
      }
  ...
  describe('updateHistoryEnvForFallback', () => {
    ...
    it('swaps an injected fish session for the fallback shell HISTFILE', () => {
      const env: Record<string, string> = { fish_history: 'dorka_abc123' }
      updateHistoryEnvForFallback(env, '/bin/bash', {
        shell: 'fish',
        histFile: null,
        fishSession: 'dorka_abc123',
        historyDir: '/fake/userData/terminal-history/abc123'
      })
      expect(env.fish_history).toBeUndefined()
      expect(env.HISTFILE).toBe('/fake/userData/terminal-history/abc123/bash_history')
  ...
  describe('deleteWorktreeHistoryDir', () => {
    ...
    it('ignores a fishSession that does not belong to this history directory', async () => {
      ...
      lstatSyncMock.mockReturnValue({ isFile: () => true })
      readFileSyncMock.mockReturnValue(
        JSON.stringify({ worktreeId, fishSession: 'dorka_deadbeefdeadbeef' })
      )

      deleteWorktreeHistoryDir('repo-1::/path/wt')

      expect(rmSyncMock).not.toHaveBeenCalledWith(expect.stringContaining('dorka_deadbeefdeadbeef'))
      await flushPendingWorktreeHistoryDeletions()
    })
  ...
  describe('WSL path conversion', () => {
    it('converts HISTFILE to Linux path for WSL cwd', () => {
      ...
      try {
        ...
        toLinuxPathMock.mockReturnValue(
          '/mnt/c/Users/user/AppData/Roaming/Dorka/terminal-history-wsl/Ubuntu/abc123/bash_history'
        )
        mkdirSyncMock.mockReturnValue(undefined)
        existsSyncMock.mockReturnValue(true)
    ...
    it('uses the project WSL distro hint when cwd is a Windows path', () => {
      ...
      try {
        ...
        mkdirSyncMock.mockReturnValue(undefined)
        existsSyncMock.mockReturnValue(true)
        getPathMock.mockReturnValue('C:\\Users\\alice\\AppData\\Roaming\\Dorka')
