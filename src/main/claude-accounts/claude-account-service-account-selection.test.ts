...
const CLAUDE_SERVICE_TEST_ROOT = join(tmpdir(), 'dorka-claude-service-selection-test')
...
describe('ClaudeAccountService credential capture', () => {
    ...
    let settings = {
      claudeManagedAccounts: [
        ...
        },
        {
          ...
          wslDistro: 'Ubuntu',
          wslLinuxAuthPath: '/home/jin/.local/share/dorka/claude-accounts/wsl-account/auth',
          authMethod: 'subscription-oauth',
          organizationUuid: null,
          organizationName: null,
          createdAt: 1,
  ...
  it('removes a WSL account without clearing the Windows active account', async () => {
    ...
    mkdirSync(hostAuthPath, { recursive: true })
    mkdirSync(wslAuthPath, { recursive: true })
    writeFileSync(join(wslAuthPath, '.dorka-managed-claude-auth'), 'wsl-account\n', 'utf-8')
    let settings = {
      claudeManagedAccounts: [
        {
          id: 'host-account',
