 ...
 */
const POWERSHELL_OSC133_BOOTSTRAP = `# Dorka OSC 133 shell integration for PowerShell.
...
if ($env:DORKA_OPENCODE_CONFIG_DIR) { $env:OPENCODE_CONFIG_DIR = $env:DORKA_OPENCODE_CONFIG_DIR }
if ($env:DORKA_MIMOCODE_HOME) { $env:MIMOCODE_HOME = $env:DORKA_MIMOCODE_HOME }
...
if ($ExecutionContext.SessionState.LanguageMode -eq "FullLanguage" -and
    ((-not (Test-Path variable:global:__DorkaOsc133State)) -or
     $null -eq $Global:__DorkaOsc133State.OriginalPrompt)) {
...
${getPowerShellCodexShellLaunchPreflight()}
    ...
    function Global:prompt {
        ...
        if ($Global:__DorkaOsc133State.HasSeenPrompt) {
            $result += "$($Global:__DorkaOsc133State.Esc)]133;D;$fakeExitCode$($Global:__DorkaOsc133State.Bel)"
        }
        $Global:__DorkaOsc133State.HasSeenPrompt = $true

        $result += "$($Global:__DorkaOsc133State.Esc)]133;A$($Global:__DorkaOsc133State.Bel)"
        ...
        $result += $Global:__DorkaOsc133State.OriginalPrompt.Invoke()
        $result += "$($Global:__DorkaOsc133State.Esc)]133;B$($Global:__DorkaOsc133State.Bel)"
        $result
    }

    if ($Global:__DorkaOsc133State.HasPSReadLine -and
        $null -ne $Global:__DorkaOsc133State.OriginalReadLine) {
        function Global:PSConsoleHostReadLine {
            $commandLine = $Global:__DorkaOsc133State.OriginalReadLine.Invoke()
            [Console]::Write("$($Global:__DorkaOsc133State.Esc)]133;C$($Global:__DorkaOsc133State.Bel)")
            return $commandLine
        }
    }
}
