...
function buildAndApplyMenu(options: RegisterAppMenuOptions): void {
  ...
  const featureTourItem: Electron.MenuItemConstructorOptions = {
    label: translateMain('menu.exploreDorka', 'Explore Dorka'),
    click: (_menuItem, window) => onOpenFeatureTour(window)
  }

  const setupGuideItem: Electron.MenuItemConstructorOptions = {
    label: translateMain('menu.gettingStarted', 'Getting Started with Dorka'),
  ...
  // redundant "Dorka" entry with roles that don't apply, so we omit it there
    ...
    submenu: [
      ...
      }),
      {
        ...
        click: () => {
          ...
          // control, so raw Electron paste cannot know which Dorka surface owns it.
  ...
  const appearanceSubmenu: Electron.MenuItemConstructorOptions = {
    ...
    submenu: [
      ...
      {
        label: translateMain('menu.showMobileButton', 'Show Dorka Mobile Button'),
        type: 'checkbox',
