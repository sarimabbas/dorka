...
const WSL_RATE_LIMIT_PTY_CWD_DIR = 'dorka-rate-limit-pty-cwd'
...
function resolveUserDataRoot(userDataPath?: string | null): string {
  const root = userDataPath?.trim() || process.env.DORKA_USER_DATA_PATH?.trim()
  if (root && !isRootLikePath(root)) {
    return root
  }
  return join(tmpdir(), 'dorka-rate-limit-pty')
}
...
export function getHiddenRateLimitWslCwdSetupCommands(): string[] {
  return [
    `dorka_rate_limit_cwd="\${TMPDIR:-/tmp}/${WSL_RATE_LIMIT_PTY_CWD_DIR}"`,
    'mkdir -p "$dorka_rate_limit_cwd"',
    'cd "$dorka_rate_limit_cwd"'
  ]
}
