...
describe('CodexUsageStore', () => {
  ...
  it('reports no data for Dorka scope when only non-Dorka Codex usage exists', async () => {
  ...
  it('uses only Dorka-scoped models when projecting mixed-scope sessions', async () => {
    const store = createStoreWithState({
      sessions: [
        {
          sessionId: 'session-1',
    ...
    const breakdown = await store.getBreakdown('dorka', '30d', 'model')
    const recentSessions = await store.getRecentSessions('dorka', '30d', 10)
