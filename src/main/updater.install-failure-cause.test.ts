...
const DEB_ELEVATION_ERROR =
  'Error: Command failed: /usr/bin/pkexec --disable-internal-agent "/bin/bash" "-c" "dpkg -i \'/home/u/.cache/dorka-updater/pending/dorka-ide_1.4.163_amd64.deb\'"\npkexec must be setuid root'
...
const WINDOWS_SIGNATURE_MISMATCH_ERROR =
  'New version 1.4.163 is not signed by the application owner: publisherNames: Dorka, Inc.'
 ...
 * re-dispatches it through the 'error' event *synchronously* inside `quitAndInstall()`. Dorka
...
describe('quitAndInstall failure carries the updater cause', () => {
  ...
  it('redacts the cause the same way the retained-package card does', async () => {
    ...
    expect(message).toContain("<home>/.cache/dorka-updater/pending/dorka.deb'")
  })
