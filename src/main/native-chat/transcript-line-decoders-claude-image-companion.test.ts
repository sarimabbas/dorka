...
const COMPANION_ROW = JSON.stringify({
  ...
  message: {
    role: 'user',
    content: [
      { type: 'text', text: '[Image: source: /tmp/dorka-paste-a.png]' },
      { type: 'text', text: '[Image: source: /tmp/dorka-paste-b.png]' }
    ]
  }
})
...
describe('Claude pasted-image companion row', () => {
  ...
  it('folds a multi-image companion into the prompt as image refs', () => {
    ...
    expect(folded[0]!.blocks.filter((b) => b.type === 'image-ref')).toEqual([
      { type: 'image-ref', path: '/tmp/dorka-paste-a.png' },
      { type: 'image-ref', path: '/tmp/dorka-paste-b.png' }
    ])
