...
const MANAGED_HOME = '/tmp/dorka-user-data/codex-runtime-home/home'
...
vi.mock('../codex/codex-home-paths', () => ({
  getDorkaManagedCodexHomePath: vi.fn(() => MANAGED_HOME),
  resolveDorkaManagedCodexHomePath: vi.fn(() => MANAGED_HOME)
}))
...
beforeEach(() => {
  ...
  vi.mocked(getDorkaManagedCodexHomePath).mockClear()
  scanned.dirs = []
})
...
describe('codex sessions roots', () => {
  it('scans the managed home without materializing it', async () => {
    ...
    expect(vi.mocked(getDorkaManagedCodexHomePath)).not.toHaveBeenCalled()
  })
})
