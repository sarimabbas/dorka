...
// Two Dorka-spawned codex processes refreshing the same home concurrently can
// consume one rotation twice and permanently invalidate the stored credential,
// so Dorka's own spawns (quota probes, commit-message runs) serialize per home.
...
export function resolveCodexHomeProcessLockKeyForSpawnEnv(
  ...
  if (wslDistro) {
    ...
    return normalizeRuntimePathForComparison(
      `//wsl$/${wslDistro}${codexHome ?? '/.dorka-default-codex-home'}`
    )
  }
