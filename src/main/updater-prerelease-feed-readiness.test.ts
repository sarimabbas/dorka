...
describe('fetchNewerReleaseTagsWithReadiness', () => {
  ...
  it('requires every asset referenced by the manifest files list to be reachable', async () => {
    netFetchMock.mockImplementation((url: string, init?: { method?: string }) => {
          ...
          text: () =>
            Promise.resolve(
              [
                ...
                '  - url: dorka-windows-setup.exe',
                '    sha512: test',
                `  - url: Dorka-${version}-mac.zip`,
                '    sha512: test',
                `path: Dorka-${version}-mac.zip`
              ].join('\n')
            )
        })
      }
      ...
      if (init?.method === 'HEAD') {
        ...
        const unavailable = latest && url.endsWith('/Dorka-1.4.28-mac.zip')
        const missing = latest && url.endsWith('/dorka-windows-setup.exe')
        return Promise.resolve({
  ...
  it('treats an explicit asset 404 as not-ready when another asset is unavailable', async () => {
    netFetchMock.mockImplementation((url: string, init?: { method?: string }) => {
      ...
      if (init?.method === 'HEAD') {
        const isWindowsAsset = url.endsWith('/dorka-windows-setup.exe')
        return Promise.resolve({
          ok: false,
  ...
  it('accepts absolute manifest asset URLs without rewriting them to release asset paths', async () => {
          ...
          text: () =>
            Promise.resolve(
              [
                'version: 1.4.27',
                'files:',
                '  - url: https://downloads.example.com/Dorka-1.4.27-arm64-mac.zip',
                '    sha512: test'
              ].join('\n')
            )
        })
    ...
    expect(assetUrls).toEqual(['https://downloads.example.com/Dorka-1.4.27-arm64-mac.zip'])
  })
