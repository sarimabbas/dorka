...
export function finalizeWindowsLocalPtySpawnEnvironment(args: {
  ...
  if (pathWin32.basename(plan.shellPath).toLowerCase() === 'wsl.exe') {
    if (codexHomeWslInfo) {
      ...
      // Why: Dorka's Codex home is host-local; WSL Codex must use its Linux-side ~/.codex, not a Windows path.
      delete env.CODEX_HOME
      delete env.DORKA_CODEX_HOME
    ...
    // Why: WSL Codex homes are Linux paths Windows can't use; also drop DORKA_CODEX_HOME (shell-ready restores CODEX_HOME from it).
    delete env.CODEX_HOME
    delete env.DORKA_CODEX_HOME
  }

  const shellBasename = pathWin32.basename(plan.shellPath).toLowerCase()
  const codexLaunchPreflightCommand = env.DORKA_CODEX_LAUNCH_PREFLIGHT
  if (
