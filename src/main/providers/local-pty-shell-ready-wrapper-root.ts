...
function getShellReadyWrapperBaseDir(): string {
  // Why: bundled into the daemon fork (no electron), so read DORKA_USER_DATA_PATH rather than electron's userData; main and the fork both set it to the same path.
  // Why a truthiness test rather than `??`: a set-but-empty DORKA_USER_DATA_PATH
  // would leave a relative base dir, so wrapper trees would be written under
  // whatever the process cwd happens to be.
  const userDataPath = process.env.DORKA_USER_DATA_PATH
...
// self-invalidates if DORKA_USER_DATA_PATH is ever re-pointed mid-process.
