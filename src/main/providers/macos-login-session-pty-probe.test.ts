  ...
  it('runs login under expect-owned PTY and requires its marker plus a clean exit', async () => {
    ...
    execFileMock.mockImplementation(
      (_file: string, _args: string[], _options: unknown, callback: ExecFileCallback) => {
        callback(null, '^D\b\bDORKA_LOGIN_PREFLIGHT_OK', '')
    ...
    expect(execFileMock).toHaveBeenCalledWith(
      ...
      [
        ...
        expect.stringContaining(
          'spawn -noecho /usr/bin/login -flpq $env(DORKA_LOGIN_PROBE_USERNAME)'
        )
      ],
      expect.objectContaining({
        cwd: '/Users/ada',
        env: expect.objectContaining({ DORKA_LOGIN_PROBE_USERNAME: 'ada' }),
        killSignal: 'SIGKILL',
        maxBuffer: 1_024,
