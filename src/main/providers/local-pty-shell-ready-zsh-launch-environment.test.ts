...
describePosix('live zsh subprocess tests', () => {
  describeIfZsh('terminal emulator edge cases', () => {
    ...
    beforeEach(async () => {
      testHome = mkdtempSync(join(tmpdir(), 'dorka-term-'))
      userDataPath = mkdtempSync(join(tmpdir(), 'dorka-term-userdata-'))
    ...
    it('discovers ZDOTDIR when launched inside tmux', async () => {
      ...
      delete cleanEnv.DORKA_ORIG_ZDOTDIR
    ...
    it('re-discovers ZDOTDIR despite stale DORKA_ORIG_ZDOTDIR from previous session', async () => {
      ...
      const previousDorkaZdotdir = process.env.DORKA_ORIG_ZDOTDIR
      process.env.DORKA_ORIG_ZDOTDIR = '/opt/dorka-old/shell-ready/zsh' // stale wrapper path
      ...
      try {
        ...
        const cleanEnv: Record<string, string | undefined> = {
          ...process.env,
          HOME: testHome,
          DORKA_ORIG_ZDOTDIR: '/opt/dorka-old/shell-ready/zsh'
        }
        delete cleanEnv.ZDOTDIR
        cleanEnv.ZDOTDIR = config.env.ZDOTDIR
        ...
        if (previousDorkaZdotdir === undefined) {
          delete process.env.DORKA_ORIG_ZDOTDIR
        } else {
          process.env.DORKA_ORIG_ZDOTDIR = previousDorkaZdotdir
        }
      }
    })

    it('prioritizes fresh discovery over inherited DORKA_ORIG_ZDOTDIR', async () => {
      ...
      process.env.DORKA_ORIG_ZDOTDIR = oldZdotdir
    ...
    it('sources launch-time ZDOTDIR .zshenv when it is explicitly inherited', async () => {
      ...
      try {
        const { getShellReadyLaunchConfig } = await importFreshLocalPtyShellReady()
        const config = getShellReadyLaunchConfig('/bin/zsh')
        // One channel now does what DORKA_ORIG_ZDOTDIR and DORKA_ZSHENV_SOURCE_DIR
        // split between them: the wrapper hands this back as ZDOTDIR, and zsh
        // reads .zshenv through it.
        expect(config.env.DORKA_ORIG_ZDOTDIR).toBe(inheritedZdotdir)
