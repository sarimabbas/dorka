...
describe('LocalPtyProvider', () => {
  ...
  describe('spawn', () => {
    ...
    it.each([
      // fish EXPORTS fish_history, so an Dorka launched from a fish pane hands every
      // pane the LAUNCHING worktree's session — even with isolation off (STA-4682).
      ['an inherited Dorka session', 'dorka_abc123', undefined],
    ...
    it.each([
      // HISTFILE is exported, so an Dorka launched from a pane in another worktree
      // hands every pane that worktree's history file — isolation off included.
      [
        'an inherited Dorka path',
    ...
    it('does not inherit NODE_ENV from the Dorka process env', async () => {
    ...
    it('honors explicit terminal env overrides after deleting requested defaults', async () => {
      provider.configure({
        buildSpawnEnv: (_id, env) => {
          env.TERM_PROGRAM = 'Dorka'
          env.DORKA_STALE_TEST_ENV = '/tmp/dorka-stale'
          env.PATH = `/tmp/dorka-stale:${env.PATH ?? ''}`
          return env
        }
      })
      ...
      expect(spawnCall[2].env.PATH.split(':')[0]).toBe('/tmp/dorka-agent-teams-bin')
      expect(spawnCall[2].env.TERM_PROGRAM).toBeUndefined()
      expect(spawnCall[2].env.DORKA_STALE_TEST_ENV).toBeUndefined()
    })
    ...
    it('does not re-promote a legacy attribution path for Agent Teams', async () => {
      await provider.spawn({
        ...
        rows: 24,
        env: {
          PATH: '/tmp/dorka-terminal-attribution/posix:/usr/bin',
          DORKA_AGENT_TEAMS_TEAM_ID: 'team-test'
        }
      })
    ...
    it('does not inherit AppImage runtime env into Linux PTY shells', async () => {
      ...
      process.env.LD_LIBRARY_PATH = ['/tmp/.mount_dorka123/usr/lib', '/opt/audio/lib'].join(
        delimiter
      )
    ...
    it('uses shell wrapper when MiMo home must survive shell startup', async () => {
      provider.configure({
        buildSpawnEnv: (_id, env) => {
          env.MIMOCODE_HOME = '/tmp/dorka-mimocode-overlay'
          env.DORKA_MIMOCODE_HOME = '/tmp/dorka-mimocode-overlay'
          return env
        }
      })
      ...
      expect(spawnCall[2].env.DORKA_SHELL_FEATURES).not.toContain('ready')
    })
    ...
    it('promotes the agent-teams shim onto the Windows `Path` spelling', async () => {
      ...
      expect(spawnEnv.Path.split(':')[0]).toBe('/tmp/dorka-agent-teams-bin')
    })
    ...
    it('does not inherit parent Dorka pane identity when caller omits pane env', async () => {
      ...
      const spawnCall = spawnMock.mock.calls.at(-1)!
      expect(spawnCall[2].env.DORKA_PANE_KEY).toBeUndefined()
      expect(spawnCall[2].env.DORKA_TAB_ID).toBeUndefined()
      expect(spawnCall[2].env.DORKA_WORKTREE_ID).toBeUndefined()
    })
    ...
    it('preserves explicit child Dorka pane identity over parent env', async () => {
      ...
      const spawnCall = spawnMock.mock.calls.at(-1)!
      expect(spawnCall[2].env.DORKA_PANE_KEY).toBe('child-tab:child-leaf')
      expect(spawnCall[2].env.DORKA_TAB_ID).toBe('child-tab')
      expect(spawnCall[2].env.DORKA_WORKTREE_ID).toBe('child-worktree')
    })
  })
})
