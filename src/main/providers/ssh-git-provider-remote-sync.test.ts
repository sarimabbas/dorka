...
describe('SshGitProvider', () => {
  ...
  it('syncForkDefaultBranch sends git.forkSync request', async () => {
    ...
    const expectedUpstream = { owner: 'stablyai', repo: 'dorka' }
  ...
  it('fetchGitLabMergeRequestHead sends the durable-ref git.fetchGitLabMergeRequestHeadRef request', async () => {
    mux.request.mockResolvedValueOnce({
      localRef: 'refs/dorka/merge-requests/origin-abc/42'
    })
    ...
    expect(localRef).toBe('refs/dorka/merge-requests/origin-abc/42')
  })
  ...
  it('fetchGitHubPullRequestHead sends git.fetchGitHubPullRequestHead request', async () => {
    mux.request.mockResolvedValueOnce({ localRef: 'refs/dorka/pull/origin-abc/42' })
    ...
    expect(localRef).toBe('refs/dorka/pull/origin-abc/42')
  })
  ...
  it('fetchGitHubPullRequestHead maps old relays to the reconnect message', async () => {
    ...
    await expect(
      ...
      'This SSH host is running an older Dorka relay that cannot fetch pull request heads. Reconnect to deploy the latest relay, then try again.'
    )
  })
