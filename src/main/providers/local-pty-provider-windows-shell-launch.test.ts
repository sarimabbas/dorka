...
describe('LocalPtyProvider', () => {
  ...
  describe('spawn', () => {
    it('does not pass a Windows Codex home into WSL terminals', async () => {
      ...
      provider.configure({
        buildSpawnEnv: (_id, env) => {
          ...
          env.DORKA_CODEX_HOME = 'C:\\Users\\jin\\.codex'
          return env
        }
      })
      ...
      expect(spawnCall[2].env.DORKA_CODEX_HOME).toBeUndefined()
    })
    ...
    it('translates a WSL managed Codex home before launching a WSL terminal', async () => {
      ...
      const spawnCall = spawnMock.mock.calls.at(-1)!
      expect(spawnCall[0]).toBe('wsl.exe')
      expect(spawnCall[2].env.CODEX_HOME).toBe('/home/jin/.local/share/dorka/codex-accounts/a/home')
      expect(spawnCall[2].env.DORKA_CODEX_HOME).toBe(
        '/home/jin/.local/share/dorka/codex-accounts/a/home'
      )
      expect(spawnCall[2].env.WSLENV).toContain('CODEX_HOME')
      expect(spawnCall[2].env.WSLENV).toContain('DORKA_CODEX_HOME')
    })
    ...
    it('uses the preferred WSL distro for Windows cwd WSL terminals', async () => {
      ...
      expect(spawnCall[1][5]).toContain('exec "$_dorka_wsl_shell" -l')
    ...
    it('marks Dorka terminal handle for WSL import when buildSpawnEnv opts in', async () => {
      Object.defineProperty(process, 'platform', { configurable: true, value: 'win32' })
      ...
      provider.configure({
        buildSpawnEnv: (_id, env, ctx) => {
          env.DORKA_TERMINAL_HANDLE = 'term_wsl'
          if (ctx?.isWsl) {
            env.WSLENV = 'DORKA_TERMINAL_HANDLE/u'
          }
          return env
        }
      })
      ...
      try {
        await provider.spawn({
          ...
          rows: 24,
          cwd: '\\\\wsl.localhost\\Ubuntu\\home\\jin\\repo',
          env: { DORKA_HERMES_STARTUP_QUERY: 'line one\nline two' }
        })
      } finally {
    ...
    it('resolves the Git Bash default shell and preserves the requested cwd', async () => {
      ...
      provider.configure({
        ...
        buildSpawnEnv: (_id, env) => ({
          ...env,
          DORKA_CODEX_LAUNCH_PREFLIGHT: CODEX_LAUNCH_PREFLIGHT
        })
      })
    ...
    it('runs the Codex preflight once in the cmd.exe startup chain', async () => {
      ...
      expect(spawnMock).toHaveBeenCalledWith(
        ...
        [
          '/K',
          'chcp 65001 > nul & if defined DORKA_CODEX_LAUNCH_PREFLIGHT call %DORKA_CODEX_LAUNCH_PREFLIGHT_CMD_QUOTE%%DORKA_CODEX_LAUNCH_PREFLIGHT%%DORKA_CODEX_LAUNCH_PREFLIGHT_CMD_QUOTE% agent hooks prepare-codex > nul 2>&1'
        ],
        expect.objectContaining({
          env: expect.objectContaining({
            DORKA_CODEX_LAUNCH_PREFLIGHT: CODEX_LAUNCH_PREFLIGHT,
            DORKA_CODEX_LAUNCH_PREFLIGHT_CMD_QUOTE: '"'
          })
        })
      )
    })
