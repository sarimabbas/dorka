/**
 * Agent/tool preflight detection. Split out of `ipc/preflight.ts` so the Dorka
 ...
 * Refresh — handles the "installed a new CLI, Dorka doesn't see it yet" case
...
async function executePreflightCheck(
  ...
  if (force) {
    ...
    // runs `glab auth login` for a self-hosted host after Dorka starts would
