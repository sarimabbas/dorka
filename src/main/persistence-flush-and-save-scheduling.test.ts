...
describe('Store', () => {
  ...
  it('snapshots the cache at flush and seeds the next Store from the sidecar', async () => {
    ...
    store.flush()
    expect(existsSync(join(testState.dir, 'dorka-github-cache.json'))).toBe(true)
  ...
  it('keeps GitHub cache sidecars scoped to explicit profile data files', async () => {
    ...
    const profileADataFile = join(profileADir, 'dorka-data.json')
    const profileBDataFile = join(profileBDir, 'dorka-data.json')
