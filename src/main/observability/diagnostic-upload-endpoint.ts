...
export function resolveDiagnosticBuildTokenEndpoint(): string | null {
  const endpoint =
    typeof DORKA_DIAGNOSTICS_TOKEN_URL !== 'undefined'
      ? DORKA_DIAGNOSTICS_TOKEN_URL
      : ((globalThis as { DORKA_DIAGNOSTICS_TOKEN_URL?: string | null })
          .DORKA_DIAGNOSTICS_TOKEN_URL ?? null)
...
export function resolveDiagnosticTokenEndpoint(): string | null {
  ...
  // cannot redirect uploads that the UI labels as going to Dorka support.
  if (resolveDiagnosticBuildIdentity()) {
    return buildEndpoint
  }
  const fromEnv = process.env.DORKA_DIAGNOSTICS_TOKEN_URL
...
export function resolveDiagnosticDorkaChannel(): 'stable' | 'rc' | 'dev' {
