...
//   DORKA_TELEMETRY_DISABLED=1 → identical to DO_NOT_TRACK for this lane.
//   DORKA_DIAGNOSTICS_DISABLED=1 → ALSO disable local file writes. The escape
...
export function resolveObservabilityConsent(): ObservabilityConsent {
  ...
  const dorkaDisabled = envOn('DORKA_TELEMETRY_DISABLED')
  const diagnosticsDisabled = envOn('DORKA_DIAGNOSTICS_DISABLED')
  const ci = inCI()
  ...
  if (diagnosticsDisabled) {
    return {
      localFileEnabled: false,
      bundleEnabled: false,
      disabledReason: 'dorka_diagnostics_disabled'
    }
  }
  if (dnt || dorkaDisabled) {
    // Local file remains active — DNT is a *network* signal, and the local
    // file never leaves the machine.
    return {
      localFileEnabled: true,
      bundleEnabled: false,
      disabledReason: dnt ? 'do_not_track' : 'dorka_telemetry_disabled'
    }
  }
...
export function initObservability(): ObservabilityConsent {
  ...
  if (!c.localFileEnabled) {
    // Disabled at the CI / DORKA_DIAGNOSTICS_DISABLED level — leave the
 ...
 *  `platform` / `arch` / `osRelease` / `dorkaChannel` inputs come from main
...
export function collectDiagnosticBundle(
  meta: Pick<
    ...
    'appVersion' | 'platform' | 'arch' | 'osRelease' | 'dorkaChannel' | 'lookbackMinutes'
  >
): CollectedBundle {
