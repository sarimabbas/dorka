...
// Why: the gate decision Dorka is allowed to emit — "allow" would auto-approve every observed tool call.
...
describe('AntigravityHookService', () => {
  ...
  beforeEach(() => {
    homeDir = mkdtempSync(join(tmpdir(), 'dorka-antigravity-home-'))
  ...
  it('installs Antigravity global hooks.json bundle and managed script', () => {
    ...
    expect(Object.keys(config['dorka-status']).sort()).toEqual(
      ['PostInvocation', 'PostToolUse', 'PreInvocation', 'PreToolUse', 'Stop'].sort()
    )
    expect(config['dorka-status'].PreToolUse[0].matcher).toBe('*')
    expect(config['dorka-status'].PreToolUse[0].hooks?.[0]?.command).toContain(
      ANTIGRAVITY_PRE_TOOL_USE_COMMAND
    )
    expect(config['dorka-status'].PostToolUse[0].matcher).toBe('*')
    expect(config['dorka-status'].PreInvocation[0].command).toContain(
      ANTIGRAVITY_PRE_INVOCATION_COMMAND
    )
    if (process.platform === 'win32') {
      expect(config['dorka-status'].PreInvocation[0].command).not.toContain(
        'DORKA_ANTIGRAVITY_EVENT'
      )
    } else {
      expect(config['dorka-status'].PreInvocation[0].command).toContain(
        "DORKA_ANTIGRAVITY_EVENT='PreInvocation'"
      )
      expect(config['dorka-status'].Stop[0].command).toContain("DORKA_ANTIGRAVITY_EVENT='Stop'")
    }

    const script = readFileSync(
      join(homeDir, '.dorka', 'agent-hooks', ANTIGRAVITY_SCRIPT_FILE_NAME),
      'utf8'
    )
    ...
    if (process.platform === 'win32') {
      ...
      expect(script).toContain('hook_event_name=%DORKA_ANTIGRAVITY_EVENT%')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      const result = spawnSync(
        '/bin/sh',
        [join(homeDir, '.dorka', 'agent-hooks', 'antigravity-hook.sh')],
        {
          env: {
            ...process.env,
            DORKA_ANTIGRAVITY_EVENT: 'PreToolUse',
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      new AntigravityHookService().install()
      rmSync(join(homeDir, '.dorka', 'agent-hooks'), { recursive: true, force: true })

      const config = JSON.parse(
        readFileSync(join(homeDir, '.gemini', 'config', 'hooks.json'), 'utf8')
      ) as { 'dorka-status': Record<string, { hooks?: { command: string }[] }[]> }
      const command = config['dorka-status'].PreToolUse[0].hooks?.[0]?.command
      ...
      // Why: hooks.json lives in ~/.gemini and outlives ~/.dorka, so a swept script must not deny every tool call.
      expect(result.status).toBe(0)
      expect(result.stdout).toBe(`${PRE_TOOL_USE_DECISION}\n`)
    }
  )
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      const config = JSON.parse(
        ...
        'dorka-status': Record<string, { command?: string; hooks?: { command: string }[] }[]>
      }
      const postToolUse = config['dorka-status'].PostToolUse[0].hooks?.[0]?.command
      const preInvocation = config['dorka-status'].PreInvocation[0].command
  ...
  it('installs Windows event wrappers without nested cmd quoting and replaces stale PreToolUse hooks', () => {
    withPlatform('win32', () => {
              ...
              PreToolUse: [
                {
                  ...
                  hooks: [
                    {
                      type: 'command',
                      command: `cmd /d /s /c "set "DORKA_ANTIGRAVITY_EVENT=PreToolUse" && call "${staleScriptPath}""`
                    }
                  ]
                }
              ]
      ...
      expect(config['dorka-status'].PreToolUse).toHaveLength(1)
      ...
      for (const [eventName, wrapperFileName] of Object.entries(expectedWrappers)) {
        const definition = config['dorka-status'][eventName][0]
        ...
        expect(command).not.toContain('DORKA_ANTIGRAVITY_EVENT')

        const wrapper = readFileSync(
          join(homeDir, '.dorka', 'agent-hooks', wrapperFileName),
          'utf8'
        )
        expect(wrapper).toContain(`set "DORKA_ANTIGRAVITY_EVENT=${eventName}"`)
        expect(wrapper).toContain('call "%DORKA_ANTIGRAVITY_CORE%"')
      ...
      const script = readFileSync(
        join(homeDir, '.dorka', 'agent-hooks', 'antigravity-hook.cmd'),
        'utf8'
      )
      expect(script).toContain('/hook/antigravity')
      expect(script).not.toContain('powershell.exe')
  ...
  it('preserves user-authored hook bundles and entries in Dorka bundle', () => {
    const configPath = join(homeDir, '.gemini', 'config', 'hooks.json')
    mkdirSync(dirname(configPath), { recursive: true })
    writeFileSync(
      configPath,
      `${JSON.stringify(
        {
          ...
          'dorka-status': {
            PreInvocation: [{ type: 'command', command: '/usr/local/bin/dorka-extra' }]
          }
        },
        null,
        2
    ...
    const config = JSON.parse(readFileSync(configPath, 'utf8')) as {
      ...
      'dorka-status': { PreInvocation: { command: string }[] }
    }
    expect(config['user-hook'].PreInvocation[0].command).toBe('/usr/local/bin/user-hook')
    const commands = config['dorka-status'].PreInvocation.map((entry) => entry.command)
    expect(commands).toContain('/usr/local/bin/dorka-extra')
  ...
  it('removes stale managed Antigravity hook entries from retired events', () => {
    ...
    expect(config['dorka-status'].OldEvent).toBeUndefined()
    ...
    expect(preToolCommands[0]).toContain(
      join(homeDir, '.dorka', 'agent-hooks', ANTIGRAVITY_PRE_TOOL_USE_COMMAND)
    )
    expect(preToolCommands[0]).not.toContain('/tmp/old/agent-hooks/antigravity-hook.sh')
    const commands = config['dorka-status'].PostToolUse.flatMap((definition) =>
      (definition.hooks ?? []).map((hook) => hook.command)
    )
    expect(commands).toHaveLength(1)
    expect(commands[0]).toContain(
      join(homeDir, '.dorka', 'agent-hooks', ANTIGRAVITY_POST_TOOL_USE_COMMAND)
    )
  })
})
