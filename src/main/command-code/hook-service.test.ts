...
describe('CommandCodeHookService', () => {
  ...
  beforeEach(() => {
    homeDir = mkdtempSync(join(tmpdir(), 'dorka-command-code-home-'))
  ...
  it('installs the managed command for Command Code status events', () => {
    ...
    if (process.platform !== 'win32') {
      expect(config.hooks.PreToolUse[0].hooks[0].command).toContain(join(homeDir, '.dorka'))
    }
    if (process.platform !== 'win32') {
  ...
  it.skipIf(process.platform !== 'win32')(
    ...
    () => {
      const spaceHome = join(tmpdir(), 'dorka command-code home with spaces')
  ...
  it('installs a hook script that can recover the endpoint when Command Code strips token env', () => {
    ...
    const script = readFileSync(join(homeDir, '.dorka', 'agent-hooks', scriptFileName), 'utf8')

    if (process.platform === 'win32') {
      expect(script).toContain('sourceEndpointByPort')
      expect(script).toContain('dorka-dev\\agent-hooks')
      expect(script).toContain('set DORKA_AGENT_HOOK_PORT=')
    } else {
      ...
      expect(script).toContain('__dorka_read_ancestor_var')
      expect(script).toContain('__dorka_fill_from_endpoint_file')
      expect(script).toContain('[ "$__dorka_endpoint_port" != "$DORKA_AGENT_HOOK_PORT" ]')
      expect(script).toContain('DORKA_PANE_KEY')
      expect(script).toContain('DORKA_AGENT_LAUNCH_TOKEN')
      expect(script).toContain('dorka-dev/agent-hooks')
  ...
  itPosix('does not let stale endpoint files clobber recovered connection fields', async () => {
    ...
    const server = createServer((req, res) => {
      ...
      req.on('end', () => {
        requests.push({
          body,
          token: req.headers['x-dorka-agent-hook-token']
        })
        res.statusCode = 204
        res.end()
      })
    ...
    try {
      ...
      const scriptPath = join(homeDir, '.dorka', 'agent-hooks', 'command-code-hook.sh')
      const child = spawn('/bin/sh', [scriptPath], {
        env: {
          ...process.env,
          HOME: homeDir,
          DORKA_AGENT_HOOK_ENDPOINT: staleEndpointPath,
          DORKA_AGENT_HOOK_PORT: String(address.port),
          DORKA_AGENT_HOOK_TOKEN: 'current-token',
          DORKA_PANE_KEY: 'tab:leaf',
          DORKA_TAB_ID: 'tab',
          DORKA_WORKTREE_ID: 'worktree',
          DORKA_AGENT_HOOK_ENV: 'development',
          DORKA_AGENT_HOOK_VERSION: '1'
        },
