...
describe('fish history session naming', () => {
  it('mints a session name that is a valid fish variable value', () => {
    ...
    expect(SESSION_A).toBe('dorka_a1b2c3d4e5f60718')
  ...
  it.each([
    ...
    ['dorka_', false],
    ['fish_history', false],
    ['dorka_../../etc/passwd', false],
    ['dorka_DEADBEEF', false],
    ['', false],
    [undefined, false],
    [42, false]
...
describe('fish history deletion', () => {
  ...
  beforeEach(() => {
    root = mkdtempSync(join(tmpdir(), 'dorka-fish-history-'))
...
describe('orphaned fish history sweep', () => {
  ...
  beforeEach(() => {
    root = mkdtempSync(join(tmpdir(), 'dorka-fish-sweep-'))
    fishDir = join(root, 'fish')
    mkdirSync(fishDir, { recursive: true })
  })
  ...
  it("never touches the user's own history files", () => {
    // The whole safety of the sweep rests on the dorka_<hex>_ prefix.
    const untouched = ['fish_history', 'work_history', 'dorka_history', 'dorka_nothex_history']
 ...
 * any desktop Dorka on the same machine, whose live set knows nothing of those
...
describe('relay fish history naming', () => {
  ...
  beforeEach(() => {
    relayRoot = mkdtempSync(join(tmpdir(), 'dorka-fish-relay-'))
  })
