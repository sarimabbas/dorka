...
const MANAGED_CODEX_HOME = 'C:\\Dorka Managed\\codex-runtime-home'
...
for (const shell of WINDOWS_POWERSHELLS) {
  describe.runIf(isAvailable(shell))(`${shell} managed home bootstrap`, () => {
    it.each(['FullLanguage', 'ConstrainedLanguage'] as const)(
      ...
      (languageMode) => {
        const cwd = mkdtempSync(join(tmpdir(), 'dorka-powershell-clm-'))
        try {
          expect(runBootstrap(shell, languageMode, cwd)).toContain(
            `mode=${languageMode};codexHome=${MANAGED_CODEX_HOME};dorkaHome=${MANAGED_CODEX_HOME};startupCount=2;cwd=${cwd}`
          )
        } finally {
...
function runBootstrap(
  ...
  const launch = resolveWindowsShellLaunchArgs(
    ...
    undefined,
    '$env:DORKA_TEST_STARTUP_COUNT = 1 + [int]$env:DORKA_TEST_STARTUP_COUNT'
  )
  ...
  return execFileSync(
    ...
    ['-NoLogo', '-NoProfile', '-NonInteractive', '-EncodedCommand', harness],
    {
      ...
      env: {
        ...
        DORKA_CODEX_HOME: MANAGED_CODEX_HOME,
        DORKA_TEST_BOOTSTRAP: encodedCommand,
        DORKA_TEST_LANGUAGE_MODE: languageMode
      },
      windowsHide: true
    }
  )
...
$initialState.LanguageMode = $env:DORKA_TEST_LANGUAGE_MODE
...
$bootstrap = [Text.Encoding]::Unicode.GetString(
  [Convert]::FromBase64String($env:DORKA_TEST_BOOTSTRAP)
)
...
$runner.Commands.Clear()
$runner.AddScript(
  '"mode=$($ExecutionContext.SessionState.LanguageMode);codexHome=$env:CODEX_HOME;dorkaHome=$env:DORKA_CODEX_HOME;startupCount=$env:DORKA_TEST_STARTUP_COUNT;cwd=$($PWD.Path)"'
).Invoke()
$runner.Dispose()
$runspace.Dispose()
`)
