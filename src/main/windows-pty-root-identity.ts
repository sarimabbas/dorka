 ...
 * Dorka descendant also reads `own`. It bounds the blast radius of a bad
 ...
 * usually belongs to an unrelated process whose chain never passes through Dorka,
 ...
 * `own`. That is not remote during teardown, when Dorka is itself the process
...
export function classifyWindowsTreeKillTarget(
  ...
  ownerPid: number,
  ownChromiumPids: ReadonlySet<number> = readDorkaChromiumProcessPids()
  ...
  // `taskkill /T /F` on ourselves would take Dorka and every pane down with it.
  ...
  // hands teardown a licence to taskkill /T /F Dorka's own UI (#10680).
...
export async function verifyWindowsTreeKillTarget(
  ...
  return classifyWindowsTreeKillTarget(
    ...
    deps.ownChromiumPids ?? readDorkaChromiumProcessPids()
  )
}
