export function getPiAgentStatusWslCurlSourceLines(): string[] {
  return [
    ...
    '  const windowsPort = process.env.DORKA_AGENT_HOOK_PORT',
    '  const windowsToken = process.env.DORKA_AGENT_HOOK_TOKEN',
    "  if (!curlPath || !windowsPort || !windowsToken) throw new Error('Dorka WSL hook bridge unavailable')",
    ...
    "        '-H', `X-Dorka-Agent-Hook-Token: ${windowsToken}`,",
    "        '--data-binary', '@-',",
    '        windowsUrl',
    '      ],',
    ...
    "      reject(new Error('Dorka WSL hook delivery timed out'))",
    '    }, 11000)',
    ...
    '      clearTimeout(timer)',
    "      if (code === 0) resolve(); else reject(new Error('Dorka WSL hook exit ' + code))",
    '    })',
