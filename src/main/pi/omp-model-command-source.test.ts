...
describe('OMP model command', () => {
  it('switches through OMP API and reports the actual selected model', async () => {
    ...
    await harness.commands['dorka-model'].handler('openai/test-model', commandContext())
  ...
  it('does not claim a switch for an unavailable model or rejected API call', async () => {
    ...
    await harness.commands['dorka-model'].handler('missing/model', context)
    ...
    await harness.commands['dorka-model'].handler('openai/test-model', context)
  ...
  it('does not register for Pi or an inherited child owner', () => {
    ...
    expect(
      createAgentStatusExtensionHarness({ kind: 'omp', env: { DORKA_PI_STATUS_OWNED: '123' } })
        .commands
    ).toEqual({})
  })
})
