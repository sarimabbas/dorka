...
export const MAIN_THREAD_DIAGNOSTICS_ENV = 'DORKA_MAIN_THREAD_DIAGNOSTICS'
...
export function classifySubprocessCommand(command: string, args: readonly string[]): string {
  ...
  const rest = [...args]
  if (binary === 'wsl') {
    // Dorka spawns guest commands with `--exec`; `--`/`-e` still appear on
    // wsl.exe processes started outside Dorka, so unwrap either separator.
    while (rest.length > 0) {
 ...
 * No-op unless DORKA_MAIN_THREAD_DIAGNOSTICS=1.
 */
