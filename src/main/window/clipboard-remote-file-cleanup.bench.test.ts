...
const BENCH_ENABLED = process.env.DORKA_CLIPBOARD_CLEANUP_BENCH === '1'
const FOREIGN_ENTRY_COUNT = Number(process.env.DORKA_CLIPBOARD_CLEANUP_BENCH_ENTRIES ?? 100_000)
const WARMUP_RUNS = 3
const SAMPLE_RUNS_PER_ARM = 24
const FIXTURE_PREFIX = 'dorka-clipboard-cleanup-bench-'
...
async function scanSharedTempRoot(root: string): Promise<void> {
  ...
  try {
    for await (const entry of directory) {
      if (entry.isDirectory() && entry.name.startsWith('dorka-clipboard-file-')) {
