...
} from '../dorka-profiles/profile-storage-paths'
import { getDorkaProfileIndexPath, readProfileIndex } from '../dorka-profiles/profile-index-store'

/**
 * Worktree ids owned by Dorka profiles OTHER than the running one.
 *
...
export function getOtherProfileWorktreeIdsForHistoryGc(userDataPath = getProfileUserDataPath()): {
  ...
  const index = readProfileIndex(getDorkaProfileIndexPath(userDataPath))
  if (!index) {
  ...
  for (const profile of index.profiles) {
    ...
    const collected = readProfileWorktreeIds(getDorkaProfileDataFile(profile.id, userDataPath))
    if (!collected) {
