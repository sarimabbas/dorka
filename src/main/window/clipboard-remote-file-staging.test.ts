...
const STAGING_ROOT = join(TEMP_ROOT, `dorka-clipboard-files${UID_SUFFIX}`)
...
const LEGACY_EXPIRED = `dorka-clipboard-file-${EXPIRED_TRANSFER}`
const LEGACY_FRESH = `dorka-clipboard-file-${FRESH_TRANSFER}`
...
describe('legacy remote clipboard staging compatibility', () => {
  ...
  it('removes only exact expired legacy children encountered inside the bound', async () => {
    opendirMock.mockResolvedValue(
      openedDirectory([
        directoryEntry('dorka-clipboard-file-lookalike'),
