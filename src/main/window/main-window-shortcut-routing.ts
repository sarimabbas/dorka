...
export function installMainWindowShortcutRouting(args: {
  ...
  const dispatchResolvedWindowShortcutAction = (
    ...
    const capturedTerminalActionId =
      ...
      focusedShortcutContext.terminalShortcutPolicy === 'dorka-first' &&
  ...
  mainWindow.webContents.on('before-input-event', (event, input) => {
    ...
    if (isMacAppPasteInput(input)) {
      // Why: chat/terminal panes hold focus without native editable controls, so route Cmd+V through Dorka's paste ownership.
      event.preventDefault()
