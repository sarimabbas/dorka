  ...
  while (!deps.isQuitting()) {
    ...
    const recoveryDetail = stalled
      ? translateMain(
          ...
          'Dorka reloaded the window after a crash, but it never finished loading.'
        )
      : translateMain(
          'rendererRecovery.crashLoopDetail',
          'Dorka tried to recover {{recoveryCount}} times in a row without success.',
    ...
    const causeDetail = diagnosis
      ...
      : translateMain(
          ...
          'This is often a graphics-driver or installation problem. Reload to try again, or quit and relaunch Dorka.'
        )
    const { response } = await deps.showMessageBox({
      ...
      cancelId: 0,
      title: translateMain('rendererRecovery.title', 'Dorka keeps failing to load'),
      message: stalled
        ? translateMain(
