 ...
 * lives in `dorka-runtime-get-worktree-ps.ts` — a `@ts-nocheck` file, so neither the
...
describe('structured Claude auth policy wiring', () => {
  ...
  // DorkaRuntime chain (measured), so the wiring itself is pinned by source and the
  ...
  it('passes a settings-derived Claude auth policy to the host installer', () => {
    const source = readFileSync(join(__dirname, 'dorka-runtime-get-worktree-ps.ts'), 'utf8')
  ...
  describe('installing without one', () => {
    ...
    it('refuses loudly rather than defaulting to a guess', async () => {
      stateDirectory = await mkdtemp(join(tmpdir(), 'dorka-auth-policy-wiring-'))
