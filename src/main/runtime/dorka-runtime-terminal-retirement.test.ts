...
function syncSplit(runtime: DorkaRuntimeService, snapshot = makeSplitSnapshot()): void {
...
describe('DorkaRuntimeService terminal surface retirement', () => {
  ...
  it('rejects one stale shared-PTY surface without removing its live sibling', () => {
    ...
    const runtime = new DorkaRuntimeService(runtimeStore({ getWorkspaceSession: () => session }))
    runtime.attachWindow(1)
