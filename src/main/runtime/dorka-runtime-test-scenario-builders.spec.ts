import * as mocks from './dorka-runtime-test-mocks.spec'
import type { Mock } from 'vitest'

const { HEADLESS_RUNTIME_WINDOW_ID, DorkaRuntimeService, electronMocks } = mocks
const { getBrowserHostLeaseRegistry, getDefaultWorkspaceSession, getRuntimeBrowserPageRegistry } =
  mocks
const { gitRunner, vi } = mocks

import * as fixtures from './dorka-runtime-test-fixtures.spec'
...
type RuntimeService = InstanceType<typeof DorkaRuntimeService>
type TestMock = Mock
...
// Why (#10333): `dorka serve` publishes a ready graph under
...
function createWorktreeRemovalRuntime(runtimeStore: unknown = store): RuntimeService {
  ...
  return new DorkaRuntimeService(runtimeStore as never, undefined, {
