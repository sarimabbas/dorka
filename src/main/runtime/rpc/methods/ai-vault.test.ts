...
describe('aiVault.listSessions handler + shared cache', () => {
  ...
  it('forwards codex-home through the real DorkaRuntimeService construction path', async () => {
    // Why: the dispatcher test above seeds the cache module directly, so it would
    // still pass if DorkaRuntimeService stopped forwarding the codex-home source.
