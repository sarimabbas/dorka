...
function precommitRefusal(error: unknown): AgentSessionWireRefusal {
  ...
  if (code) {
    return { code, message: 'Dorka cannot open a structured agent chat for this workspace.' }
  }
  ...
  return {
    ...
    message: `Dorka could not prepare a structured agent chat for this workspace: ${message}`
  }
}
