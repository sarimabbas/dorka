...
describe('orchestration federated worker output', () => {
  ...
  beforeEach(() => {
    ...
    workerDbDirectory = mkdtempSync(join(tmpdir(), 'dorka-federated-output-db-'))
  ...
  function configureWorkerRuntime(runtime: DorkaRuntimeService): void {
  ...
  it('reads the exact transcript on the worker server without leaking its path home', async () => {
    const dispatchId = await startRemoteWorker()
    const directory = await mkdtemp(join(tmpdir(), 'dorka-federated-worker-output-'))
