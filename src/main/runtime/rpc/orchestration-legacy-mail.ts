  ...
  const currentDelivery =
    principal.role === 'coordinator' && db.hasPendingCurrentDelivery(principal.run_id)
      ? {
          ...
          checkCommand: `${params.compatibilityCliCommand ?? 'dorka'} orchestration check --run ${principal.run_id}`,
          ackCommand: `${params.compatibilityCliCommand ?? 'dorka'} orchestration check --run ${principal.run_id} --ack <delivery-id>`
        }
      : undefined
  return {
    runId: principal.run_id,
