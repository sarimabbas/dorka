...
function stubStaleHandleRuntime(overrides: Partial<DorkaRuntimeService> = {}): DorkaRuntimeService {
  return {
...
async function expectStale(method: string, params: unknown, mutators: string[]): Promise<void> {
  ...
  const runtime = stubStaleHandleRuntime(spies as Partial<DorkaRuntimeService>)
