  ...
  it('preserves proxy path prefixes in web client URLs', async () => {
    ...
    try {
      const offer = server.createPairingOffer({
        address: 'wss://runtime.example.com/dorka',
        name: 'Proxy test'
      })
      expect(offer.available).toBe(true)
      if (offer.available) {
        expect(offer.webClientUrl).toContain('https://runtime.example.com/dorka/web-index.html')
      }
    } finally {
      await server.stop()
    }
  ...
  it('formats pairing-address overrides for IPv6 and host-port tunnel endpoints', async () => {
    ...
    try {
      ...
      if (fullUrl.available) {
        expect(fullUrl.endpoint).toBe('wss://runtime.example.com/dorka')
      }
    } finally {
      await server.stop()
    }
