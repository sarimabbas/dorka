...
async function launchOpenCodePane(options: {
  ...
  attestAgentHookCompatibilityAuthority?: DorkaRuntimeServiceDeps['attestAgentHookCompatibilityAuthority']
  ...
  const paneKey = env.DORKA_PANE_KEY as string
  const launchToken = env.DORKA_AGENT_LAUNCH_TOKEN as string
...
type DorkaRuntimeServiceDeps = NonNullable<ConstructorParameters<typeof DorkaRuntimeService>[2]>
...
describe('OpenCode finished-session launch authority (STA-4557)', () => {
  ...
  it('stops attesting the finished session token for the reused pane', async () => {
    ...
    const post = (payload: Record<string, unknown>): Promise<Response> =>
      fetch(`http://127.0.0.1:${hookEnv.DORKA_AGENT_HOOK_PORT}/hook/opencode`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-Dorka-Agent-Hook-Token': hookEnv.DORKA_AGENT_HOOK_TOKEN
        },
        body: JSON.stringify({
    ...
    // Every later process in this shell inherits DORKA_AGENT_LAUNCH_TOKEN from the PTY env,
  ...
  it('does not rehydrate a finished session token as restored authority after a restart', async () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'dorka-sta4557-'))
    ...
    await fetch(`http://127.0.0.1:${hookEnv.DORKA_AGENT_HOOK_PORT}/hook/opencode`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'X-Dorka-Agent-Hook-Token': hookEnv.DORKA_AGENT_HOOK_TOKEN
    ...
    // After a restart the PTY survives with DORKA_AGENT_LAUNCH_TOKEN still in its env and
