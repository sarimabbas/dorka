...
import type { DorkaRuntimeService as DorkaRuntimeServiceConstructor } from '../dorka-runtime'
...
export function acknowledgeAgentPromptSubmit(
  runtime: InstanceType<typeof DorkaRuntimeServiceConstructor>,
  ptyId: string,
  data: string
): void {
  if (data === '\r') {
...
vi.mock('../../hooks', () => ({
  ...
  parseDorkaYaml: vi.fn().mockReturnValue(null)
}))
