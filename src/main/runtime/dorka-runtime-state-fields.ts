...
import { DorkaRuntimeWithLinearCommands } from './dorka-runtime-linear-commands'
...
export class DorkaRuntimeWithStateFields extends DorkaRuntimeWithLinearCommands {
  ...
  constructor(
    ...
    deps?: {
      ...
      // runs under `dorka serve`, so remote/SSH hosts would silently drop
      ...
      // differs per host (scanner child on the desktop, in-process on dorkad), and on dorkad
    ...
    // even on headless `dorka serve` hosts where registerCoreHandlers never runs.
