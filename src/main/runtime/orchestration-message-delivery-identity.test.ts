...
async function driveToLiveIdle(runtime: DorkaRuntimeService): Promise<void> {
...
async function runBuiltCli(
  ...
  const child = spawn(process.execPath, [CLI_PATH, ...args], {
    env: {
      ...
      DORKA_TERMINAL_HANDLE: TERMINAL_HANDLE,
...
describe('STA-4325 message and delivery identity', () => {
  ...
  it('keeps pointer counts, filters, message IDs, and one fixed Delivery aligned through ack', async () => {
    vi.useFakeTimers()
    const { db } = createDatabase('dorka-sta-4325-identity-')
  ...
  it('replays one outstanding Delivery across restart and wakes a filtered waiter once', async () => {
    vi.useFakeTimers()
    const fixture = createDatabase('dorka-sta-4325-restart-')
  ...
  it('routes the complete direct backlog before rebinding forgets its old handle', () => {
    const fixture = createDatabase('dorka-sta-4325-rebind-backlog-')
  ...
  it('repairs committed mail sent to a forgotten handle after restart', async () => {
    const fixture = createDatabase('dorka-sta-4325-late-old-handle-')
    const run = fixture.db.createRun({
  ...
  itIfCliBuilt(
    ...
    async () => {
      const userDataPath = mkdtempSync(join(tmpdir(), 'dorka-sta-4325-cli-'))
