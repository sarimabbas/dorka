...
import { DorkaRuntimeWithRunCreateMobileSessionTerminal } from './dorka-runtime-run-create-mobile-session-terminal'
...
export class DorkaRuntimeWithResolveMobileSessionTerminalCommand extends DorkaRuntimeWithRunCreateMobileSessionTerminal {
  protected async resolveMobileSessionTerminalCommand(
    ...
    const startupPlan = buildAgentStartupPlan({
      ...resolveAgentStartupPlanInputs({
        ...
        // Why: SSH runs the CLI through the relay shim (plain `dorka`), so the Linux-only `dorka-ide` rename must not apply.
