...
describe('formatMessageBanner', () => {
  ...
  it('includes reply hint with message ID', () => {
    ...
    expect(banner).toContain(
      '[Reply: dorka orchestration reply --id msg_xyz789 --from term_coord --body "..."]'
    )
  })
  ...
  it('lets the CLI resolve the live sender for Run and Dispatch addresses', () => {
    for (const to_handle of ['run:run_test', 'dispatch:dispatch_test']) {
      ...
      expect(banner).toContain('[Reply: dorka orchestration reply --id msg_test1 --body "..."]')
  ...
  it('renders only attested actions for legacy compatibility authority', () => {
    const banner = formatMessageBanner(makeMessage({ id: 'msg_legacy' }), {
      ...
      supportedActionHints: [
        'dorka orchestration reply --id msg_legacy --from term_coord --body "..."'
      ]
    })

    expect(banner).toContain('[LEGACY COMPATIBILITY]')
    expect(banner).toContain(
      '[Supported action: dorka orchestration reply --id msg_legacy --from term_coord --body "..."]'
    )
    expect(banner).not.toContain('[Reply:')
  ...
  it('warns that a bounded legacy recovery replay may already have been seen', () => {
    const banner = formatMessageBanner(makeMessage(), {
      ...
      supportedActionHints: ['dorka orchestration check --ack delivery_legacy']
    })
    ...
    expect(banner).toContain('[Supported action: dorka orchestration check --ack delivery_legacy]')
    expect(banner).not.toContain('[Reply:')
  })
...
describe('formatMessagePointer', () => {
  it('formats a singular pointer without message content', () => {
    expect(formatMessagePointer(1, 'run:run_1')).toBe(
      '\nYou have 1 orchestration message. Run `dorka orchestration check --run run_1`.\n'
    )
  })
  ...
  it('uses the terminal-resolved CLI command', () => {
    expect(formatMessagePointer(1, 'run:run_wsl', 'dorka-ide')).toContain(
      '`dorka-ide orchestration check --run run_wsl`'
    )
    expect(formatMessagePointer(1, 'run:run_dev', 'dorka-dev')).toContain(
      '`dorka-dev orchestration check --run run_dev`'
    )
  })
})
