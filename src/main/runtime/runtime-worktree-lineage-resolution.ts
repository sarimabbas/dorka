...
export class WorktreeIdRequiresFullPathError extends Error {
  ...
  constructor() {
    super(
      'Worktree id selectors must use the full <repo-id>::<path> value. Use the id from `dorka worktree list --json`, or target by path:<path>, branch:<branch>, or issue:<number>.'
    )
  }
}
  ...
  if (input.envParentWorkspace) {
    try {
      ...
      warnings.push({
        ...
        message: 'Worktree created, but Dorka could not validate the environment parent context.',
  ...
  if (input.callerTerminalHandle) {
    try {
      ...
      warnings.push({
        code: 'LINEAGE_PARENT_CONTEXT_MISSING',
        message:
          'Worktree created, but Dorka could not validate the caller terminal as a parent context.',
  ...
  if (input.cwdParentWorktree) {
    try {
      ...
      warnings.push({
        code: 'LINEAGE_PARENT_CONTEXT_MISSING',
        message:
          'Worktree created, but Dorka could not validate the current directory as a parent context.',
      ...
      warnings: [
        {
          ...
          message: 'Worktree created, but Dorka could not prove which parent context caused it.',
          details: {
