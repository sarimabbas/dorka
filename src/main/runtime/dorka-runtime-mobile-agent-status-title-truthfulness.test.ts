...
async function createRuntime(rows: AgentStatusIpcPayload[] = []): Promise<DorkaRuntimeService> {
...
function publishRendererWorkingPane(runtime: DorkaRuntimeService, paneTitle: string): void {
...
function publishRendererReleasedPane(runtime: DorkaRuntimeService, title: string): void {
  runtime.attachWindow(1)
  runtime.syncWindowGraph(1, {
    tabs: [
      {
