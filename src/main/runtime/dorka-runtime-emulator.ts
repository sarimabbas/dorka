...
// Why: dedicated file for "one surface" separation (emulator), parallel to dorka-runtime-browser.ts. Keeps DorkaRuntimeService focused; emulator routing easy to scan. No max-lines disable (split further if grows; per AGENTS + plan Phase 3).
...
export class RuntimeEmulatorCommands {
  ...
  // devices/AVDs) for the cross-platform `dorka emulator devices` command.
