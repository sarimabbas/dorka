...
import { DorkaRuntimeService, electronMocks, makePaneKey } from '../dorka-runtime-test-mocks.spec'
import {
  HEADLESS_LEAF_ID,
...
describe('DorkaRuntimeService', () => {
  it('creates visible terminal sessions without asking the renderer to focus a tab', async () => {
    ...
    expect(spawnedEnv.DORKA_WORKTREE_ID).toBe(TEST_WORKTREE_ID)
    expect(spawnedEnv.DORKA_AGENT_LAUNCH_TOKEN).toMatch(UUID_RE)
  ...
  it('retires inherited launch authority when the agent command exits', async () => {
    ...
    const evidence = {
      ...
      paneKey: spawnEnv.DORKA_PANE_KEY,
      launchToken: spawnEnv.DORKA_AGENT_LAUNCH_TOKEN
    }
    ...
    expect(retireAuthority).toHaveBeenCalledWith(spawnEnv.DORKA_PANE_KEY)
