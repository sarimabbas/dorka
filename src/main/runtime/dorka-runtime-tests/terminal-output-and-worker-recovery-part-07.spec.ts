...
import { DorkaRuntimeService, getDefaultWorkspaceSession } from '../dorka-runtime-test-mocks.spec'
import {
  HEADLESS_LEAF_ID,
...
describe('DorkaRuntimeService', () => {
  it('keeps current-generation tab and leaf identity across a host restart', async () => {
    ...
    const makeRuntime = (): DorkaRuntimeService => {
  ...
  it('uses topology CAS before a client can claim a still-orphaned PTY', async () => {
    ...
    const { runtimeStore } = makeRuntimeStoreWithWorkspaceSession(session)
    const runtime = new DorkaRuntimeService({ ...runtimeStore, flushOrThrow: vi.fn() } as never)
    runtime.setPtyController({
      write: () => true,
      kill: () => true,
      getForegroundProcess: async () => null,
  ...
  it('rejects connection mismatch and reused handles while allowing a WSL-owned orphan', async () => {
    const makeRuntime = (): DorkaRuntimeService => {
      ...
      return new DorkaRuntimeService({ ...runtimeStore, flushOrThrow: vi.fn() } as never)
    }
    ...
    await withPlatform('win32', async () => {
      const makeWslRuntime = (reportedWslDistro?: string | null): DorkaRuntimeService => {
        const { runtimeStore } = makeRuntimeStoreWithWorkspaceSession({
          ...getDefaultWorkspaceSession(),
          tabsByWorktree: { [TEST_WORKTREE_ID]: [] }
        })
        const wsl = new DorkaRuntimeService({
          ...runtimeStore,
          flushOrThrow: vi.fn(),
          getProjects: () => [
            {
