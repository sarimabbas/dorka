...
import { store } from '../dorka-runtime-test-fixtures.spec'
...
describe('DorkaRuntimeService', () => {
  it('deduplicates runtime repo paths with Windows/UNC comparison semantics', async () => {
    ...
    const runtime = new DorkaRuntimeService(uncStore as never)
  ...
  it('browses runtime server directories before projects are added', async () => {
    const tempRoot = await mkdtemp(join(tmpdir(), 'dorka-runtime-browse-'))
    try {
  ...
  it('defaults runtime addRepo badgeColor to DEFAULT_REPO_BADGE_COLOR', async () => {
    ...
    const runtime = new DorkaRuntimeService(colorStore as never)
  ...
  it('sets up an existing folder on a fresh runtime after importing the repo project', async () => {
    const tempRoot = await mkdtemp(join(tmpdir(), 'dorka-runtime-project-setup-'))
    const repos: Record<string, unknown>[] = []
    getRepoUpstreamMock.mockResolvedValueOnce({ owner: 'stablyai', repo: 'dorka' })
    const runtimeStore = {
      ...store,
  ...
  it('sets up a project whose identity exists only on the requesting host', async () => {
    const tempRoot = await mkdtemp(join(tmpdir(), 'dorka-runtime-cross-host-project-'))
    const repos: Record<string, unknown>[] = []
    ...
    try {
      execFileSync('git', ['init'], { cwd: tempRoot, stdio: 'ignore' })
      const result = await runtime.setupProjectExistingFolder({
        projectId: 'github:github.acme.test/acme/dorka',
      ...
      expect(result.project).toMatchObject({
        id: 'github:github.acme.test/acme/dorka',
        providerIdentity: {
          provider: 'github',
          owner: 'acme',
          repo: 'dorka',
  ...
  it('rolls back a new runtime repo when project alignment fails', async () => {
    const tempRoot = await mkdtemp(join(tmpdir(), 'dorka-runtime-project-rollback-'))
    const repos: Record<string, unknown>[] = []
    getRepoUpstreamMock.mockResolvedValueOnce(null)
    const runtimeStore = {
      ...store,
    ...
    try {
      ...
      await expect(
        runtime.setupProjectExistingFolder({
          projectId: 'git:git.example.test/acme/dorka',
          hostId: 'runtime:env-1',
          path: tempRoot,
          kind: 'git'
        })
  ...
  it('rolls back a newly cloned repo when project alignment fails', async () => {
    ...
    await expect(
      runtime.setupProjectClone({
        projectId: 'git:git.example.test/acme/dorka',
        hostId: 'runtime:env-1',
        url: 'https://git.example.test/acme/dorka.git',
        destination: '/tmp'
      })
    ).rejects.toThrow('Imported folder does not match the selected project identity.')
  ...
  it('keeps existing-folder imports split by runtime host on the same normalized path', async () => {
    const tempRoot = await mkdtemp(join(tmpdir(), 'dorka-runtime-project-host-'))
    const repos: Record<string, unknown>[] = []
    getRepoUpstreamMock.mockResolvedValue({ owner: 'stablyai', repo: 'dorka' })
    const runtimeStore = {
      ...store,
      getRepos: () => [...repos] as never,
      addRepo: (repo: Record<string, unknown>) => {
  ...
  it('keeps project clone setup on the cloned host-qualified repo', async () => {
    const destination = await mkdtemp(join(tmpdir(), 'dorka-runtime-project-clone-'))
    const clonePath = join(destination, 'dorka')
    ...
    try {
      const result = await runtime.setupProjectClone({
        projectId: 'github:stablyai/orca',
        hostId: 'runtime:env-1',
        url: 'https://example.com/dorka.git',
        destination
      })
  ...
  it('never sets an SSH-hosted project up on the local machine', async () => {
    ...
    const destination = await mkdtemp(join(tmpdir(), 'dorka-runtime-ssh-guard-'))
    const existingFolder = join(destination, 'dorka')
