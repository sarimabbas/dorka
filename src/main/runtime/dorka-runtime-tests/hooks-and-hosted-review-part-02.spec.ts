...
describe('DorkaRuntimeService', () => {
  ...
  it('rejects hosted review worktree selectors outside the selected repo', async () => {
    ...
    const runtime = new DorkaRuntimeService(multiRepoStore as never)
  ...
  it('routes local WSL project hosted review flows through runtime git options', async () => {
    ...
    const runtime = new DorkaRuntimeService(wslStore as never)
    getHostedReviewForBranchMock.mockResolvedValueOnce({
      provider: 'github',
      number: 76,
      title: 'Feature WSL',
      state: 'open',
      url: 'https://github.com/acme/dorka/pull/76',
      status: 'success',
    ...
    createHostedReviewMock.mockResolvedValueOnce({
      ok: true,
      number: 77,
      url: 'https://github.com/acme/dorka/pull/77'
    })
