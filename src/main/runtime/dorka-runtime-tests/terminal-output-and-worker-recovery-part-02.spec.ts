...
import type { WorkspaceSessionState } from '../dorka-runtime-test-mocks.spec'
import {
  HEADLESS_LEAF_ID,
...
import { publishLegacyWorkerReveal } from '../dorka-runtime-test-scenario-builders.spec'

describe('DorkaRuntimeService', () => {
  it('recovers exported DORKA_TERMINAL_HANDLE from discovered live PTY sessions', async () => {
  ...
  it('reveals one exact live legacy worker without stealing focus', async () => {
    ...
    const runtime = new DorkaRuntimeService({ ...runtimeStore, flushOrThrow } as never, undefined, {
