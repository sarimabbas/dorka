 ...
 * in `dorka-runtime-get-worktree-ps.ts`, which is `@ts-nocheck`, so a renamed dependency there stays
 * green in typecheck while `dorka worktree ps` and mobile's poll would list nothing.
 */
...
describe('worktree ps reads structured sessions from the agent-status store', () => {
  it('lists a host-held structured session with no terminal behind it', async () => {
    ...
    const { worktrees } = await new DorkaRuntimeService(store, undefined, {
