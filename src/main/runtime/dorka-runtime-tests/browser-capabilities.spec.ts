...
import type { RuntimeMobileSessionTabsResult } from '../dorka-runtime-test-mocks.spec'
import {
  TEST_REPO_ID,
  TEST_WINDOW_ID,
  TEST_WORKTREE_ID,
...
} from '../dorka-runtime-test-scenario-builders.spec'
...
describe('DorkaRuntimeService', () => {
  ...
  it('advertises the browser identity capability only where an identity store exists', () => {
    ...
    initializeBrowserIdentityModeStore(mkdtempSync(join(tmpdir(), 'dorka-identity-capability-')))
    try {
  ...
  it('reports the driver as missing instead of telling a configured operator to configure it', () => {
    ...
    expect(degradation?.message).not.toMatch(/set DORKA_BROWSER_EXECUTABLE/)
  })
  ...
  it('keeps the degradation wire-safe for peers that predate structured causes', () => {
    ...
    expect(degradation?.message).toBe(
      'DORKA_BROWSER_EXECUTABLE points at a path that does not exist. (/nope/chromium)'
    )
  })
