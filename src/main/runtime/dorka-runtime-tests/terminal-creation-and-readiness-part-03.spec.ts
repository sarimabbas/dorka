...
import {
  ...
  makeFolderWorkspace,
  store
} from '../dorka-runtime-test-fixtures.spec'

describe('DorkaRuntimeService', () => {
  it('does not use the local Windows shell setting for remote Windows bare agent creates', async () => {
    ...
    const runtime = new DorkaRuntimeService(remoteStore as never)
  ...
  it('matches canonical bare agent commands when a command override is configured', async () => {
    ...
    const runtime = new DorkaRuntimeService(runtimeStore)
    runtime.setPtyController({
      spawn,
      write: () => true,
      kill: () => true,
  ...
  it('keeps non-bare agent command terminal creates unchanged', async () => {
    ...
    expect(spawnCall?.env?.DORKA_AGENT_LAUNCH_TOKEN).toBeUndefined()
  })
  ...
  it('injects runtime hook receiver env into terminal sessions', async () => {
    ...
    await runtime.createTerminal(`path:${TEST_WORKTREE_PATH}`, {
      ...
      env: {
        ...
        DORKA_AGENT_HOOK_TRANSPORT: 'stale-transport',
    ...
    expect(spawnCall?.env).toEqual(
      expect.objectContaining({
        ...
        DORKA_AGENT_HOOK_ENV: 'remote',
        DORKA_AGENT_HOOK_VERSION: '1',
        DORKA_PANE_KEY: expect.any(String),
        DORKA_TAB_ID: expect.any(String),
        DORKA_WORKTREE_ID: TEST_WORKTREE_ID
      })
    )
    expect(spawnCall?.env?.DORKA_AGENT_HOOK_ENDPOINT).toBeUndefined()
    expect(spawnCall?.env?.DORKA_AGENT_HOOK_TRANSPORT).toBeUndefined()
    expect(spawnCall?.envToDelete).toEqual(['CODEX_HOME', 'DORKA_CODEX_HOME'])
  })
  ...
  it.each([
    ...
    const folderPath = await mkdtemp(join(tmpdir(), 'dorka-runtime-folder-workspace-'))
    ...
    expect(spawnedEnv.DORKA_WORKSPACE_ID).toBe(TEST_FOLDER_WORKSPACE_KEY)
    expect(spawnedEnv.DORKA_PROJECT_GROUP_ID).toBe(TEST_FOLDER_PROJECT_GROUP_ID)
    expect(spawnedEnv.DORKA_WORKSPACE_ROOT).toBe(folderPath)
    expect(spawnedEnv.DORKA_WORKTREE_ID).toBe(TEST_FOLDER_WORKSPACE_KEY)
  })
  ...
  it.each([
    ...
    expect(spawnCall?.env?.DORKA_WORKTREE_ID).toBe(FLOATING_TERMINAL_WORKTREE_ID)
    expect(spawnCall?.env?.DORKA_WORKSPACE_ID).toBeUndefined()
    expect(spawnCall?.env?.DORKA_PROJECT_GROUP_ID).toBeUndefined()
    expect(spawnCall?.env?.DORKA_WORKSPACE_ROOT).toBeUndefined()
  })

  it('rejects folder workspace terminal creation when the backing path is missing', async () => {
    const missingPath = join(tmpdir(), `dorka-missing-folder-workspace-${randomUUID()}`)
    const spawn = vi.fn().mockResolvedValue({ id: 'pty-folder' })
  ...
  it('rejects folder workspace folderPath updates when the new path is missing', async () => {
    const missingPath = join(tmpdir(), `dorka-missing-folder-update-${randomUUID()}`)
    const folderWorkspace = makeFolderWorkspace()
    const runtimeStore = {
      ...createFolderWorkspaceRuntimeStore(folderWorkspace),
      updateFolderWorkspace: vi.fn()
    }
    const runtime = new DorkaRuntimeService(runtimeStore as never)
  ...
  it('reveals Claude Agent Teams launches with the rewritten launch config', async () => {
    ...
    const spawnedEnv = spawnCall?.env ?? {}
    const spawnedLeafId = spawnedEnv.DORKA_PANE_KEY.slice(`${spawnedEnv.DORKA_TAB_ID}:`.length)
    expect(revealTerminalSession).toHaveBeenCalledWith(TEST_WORKTREE_ID, {
      ...
      launchToken: spawnedEnv.DORKA_AGENT_LAUNCH_TOKEN,
      launchAgent: 'claude',
      activate: false,
      tabId: spawnedEnv.DORKA_TAB_ID,
