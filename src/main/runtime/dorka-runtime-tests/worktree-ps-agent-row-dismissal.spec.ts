 ...
 * row the user dismissed on the desktop stayed in `dorka worktree ps` and on the phone until the
...
function wiredRuntime(incarnationId?: string): {
  ...
  const runtime = new DorkaRuntimeService(store, undefined, statusWiring.deps)
...
function emitWorkingStatus(runtime: DorkaRuntimeService, sequence: number): void {
  runtime.onPtyData(
    'dismiss-pty',
