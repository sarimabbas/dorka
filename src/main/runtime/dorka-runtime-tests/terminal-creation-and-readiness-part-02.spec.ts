...
describe('DorkaRuntimeService', () => {
  ...
  it('applies Settings agent defaults to bare agent command terminal creates', async () => {
    ...
    expect(spawnCall?.env?.DORKA_AGENT_LAUNCH_TOKEN).toMatch(UUID_RE)
  ...
  // the CLI Dorka can host (issue #11926).
  ...
  it.each([
    { platform: 'win32' as const, expected: 'dorka.cmd claude-teams' },
    { platform: 'linux' as const, expected: 'dorka-ide claude-teams' },
    { platform: 'darwin' as const, expected: 'dorka claude-teams' }
  ])(
  ...
  it('resolves a startupAgent in a repo-less folder workspace', async () => {
    const folderPath = await mkdtemp(join(tmpdir(), 'dorka-runtime-folder-startup-agent-'))
    const spawn = vi.fn().mockResolvedValue({ id: 'pty-bg' })
