...
import { createRuntime } from '../dorka-runtime-test-fixtures.spec'
...
describe('DorkaRuntimeService mobile web bundle capability', () => {
  afterEach(() => {
    resetBundledMobileWebBundleCacheForTests()
  })

  // The mixed-version guarantee: a dev tree or an `dorka serve` install that never built
  ...
  it('omits the mobile web bundle capability where the install carries no bundle', () => {
    withInstallRoot('dorka-mobile-web-absent-', (install) => {
  ...
  it('advertises the mobile web bundle capability where the install carries one', () => {
    withInstallRoot('dorka-mobile-web-present-', (install) => {
  ...
  it('omits the capability where the bundle manifest does not parse', () => {
    withInstallRoot('dorka-mobile-web-unparseable-', (install) => {
