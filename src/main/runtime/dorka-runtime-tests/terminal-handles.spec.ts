...
import { DorkaRuntimeService, makePaneKey } from '../dorka-runtime-test-mocks.spec'
import type {
  RuntimeClientEvent,
...
describe('DorkaRuntimeService', () => {
  ...
  it('drops a stale leaf when a woken agent PTY is re-keyed to a new leaf on renderer reload', async () => {
    ...
    const tabId = 'tab-1'
    // Why: the agent's pre-allocated DORKA_TERMINAL_HANDLE gives its PTY a handleByPtyId entry — the condition the reload preservation loop keys on.
