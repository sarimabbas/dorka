...
describe('DorkaRuntimeService', () => {
  ...
  it('rejects SSH lineage updates when Dorka worktree identity is missing', async () => {
    const remoteRepo = {
      id: 'remote-repo',
      path: '/home/user/repo',
      displayName: 'remote',
  ...
  it('rejects local lineage updates when Dorka worktree identity is missing', async () => {
    const tempRoot = await mkdtemp(join(tmpdir(), 'dorka-runtime-lineage-'))
    ...
    try {
      ...
      await expect(lstat(join(childPath, '.dorka'))).rejects.toThrow()
      await expect(lstat(join(parentPath, '.dorka'))).rejects.toThrow()
      expect(setWorktreeLineage).not.toHaveBeenCalled()
    } finally {
