...
import { UUID_RE, expectStablePaneKeyEnv, store } from '../dorka-runtime-test-fixtures.spec'
...
describe('DorkaRuntimeService', () => {
  it('returns a setup launch payload for CLI-created worktrees when hooks are explicitly enabled', async () => {
    ...
    vi.mocked(createSetupRunnerScript).mockReturnValue({
      ...
      envVars: {
        ...
        DORKA_WORKTREE_PATH: '/tmp/workspaces/runtime-hook-test'
      }
    })
  ...
  it('passes setup payloads through when explicitly activating CLI-created worktrees', async () => {
    ...
    vi.mocked(createSetupRunnerScript).mockReturnValue({
      ...
      envVars: {
        DORKA_ROOT_PATH: '/tmp/repo',
        DORKA_WORKTREE_PATH: '/tmp/workspaces/runtime-hook-activate'
      }
    })
    vi.mocked(listWorktrees).mockResolvedValueOnce([
      {
  ...
  it('passes the selected Windows setup shell into runtime runner generation', async () => {
    ...
    vi.mocked(createSetupRunnerScript).mockReturnValue({
      ...
      envVars: {
        ...
        DORKA_WORKTREE_PATH: 'C:\\workspaces\\runtime-hook-activate'
      }
    })
    vi.mocked(listWorktrees).mockResolvedValueOnce([
      {
  ...
  it('follows normal setup policy for CLI-created worktrees without activating them', async () => {
    ...
    vi.mocked(createSetupRunnerScript).mockReturnValue({
      ...
      envVars: {
        DORKA_ROOT_PATH: '/tmp/repo',
        DORKA_WORKTREE_PATH: '/tmp/workspaces/runtime-hook-skip'
      }
    })
    ...
    expect(spawn).toHaveBeenNthCalledWith(
      ...
      expect.objectContaining({
        ...
        // Why: createTerminal stamps DORKA_PANE_KEY/TAB_ID/WORKTREE_ID so hook-based agent status can attribute events to a stable pane.
        env: expect.objectContaining({
          DORKA_ROOT_PATH: '/tmp/repo',
          DORKA_WORKTREE_PATH: '/tmp/workspaces/runtime-hook-skip',
          DORKA_TAB_ID: expect.stringMatching(UUID_RE),
          DORKA_PANE_KEY: expect.any(String),
          DORKA_WORKTREE_ID: result.worktree.id
        }),
        worktreeId: result.worktree.id
      })
    )
    ...
    const setupLeafId = setupSpawnEnv.DORKA_PANE_KEY.slice(`${setupSpawnEnv.DORKA_TAB_ID}:`.length)
    ...
    expect(revealTerminalSession).toHaveBeenLastCalledWith(result.worktree.id, {
      ...
      activate: false,
      surfaceOwner: false,
      tabId: setupSpawnEnv.DORKA_TAB_ID,
      leafId: setupLeafId
    })
  })
  ...
  it('uses returned WSL setup shell metadata when runtime spawns setup', async () => {
    ...
    vi.mocked(createSetupRunnerScript).mockReturnValue({
      ...
      envVars: {
        DORKA_ROOT_PATH: 'C:\\repo',
        DORKA_WORKTREE_PATH: 'C:\\workspaces\\runtime-hook-wsl'
      }
    })
    vi.mocked(listWorktrees).mockResolvedValue([
      {
    ...
    expect(spawn).toHaveBeenNthCalledWith(
      2,
      expect.objectContaining({
        command: 'bash /mnt/c/repo/.git/dorka/setup-runner.sh',
        env: expect.objectContaining({
          DORKA_ROOT_PATH: 'C:\\repo',
          DORKA_WORKTREE_PATH: 'C:\\workspaces\\runtime-hook-wsl',
          DORKA_TAB_ID: expect.stringMatching(UUID_RE),
  ...
  it('uses the shell-aware setup runner for windowless creates without a startup command', async () => {
    ...
    vi.mocked(createSetupRunnerScript).mockReturnValue({
      ...
      envVars: {
        DORKA_ROOT_PATH: 'C:\\repo',
        DORKA_WORKTREE_PATH: 'C:\\workspaces\\runtime-hook-windowless'
      }
    })
    vi.mocked(listWorktrees).mockResolvedValue([
      {
    ...
    expect(spawn).toHaveBeenNthCalledWith(
      2,
      expect.objectContaining({
        command: 'bash /c/repo/.git/dorka/setup-runner.sh',
        worktreeId: result.worktree.id
      })
    )
  })
