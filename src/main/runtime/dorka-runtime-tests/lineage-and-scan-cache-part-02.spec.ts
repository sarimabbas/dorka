...
import { DorkaRuntimeService, join, listWorktrees, tmpdir } from '../dorka-runtime-test-mocks.spec'
import type { WorktreeLineage, WorktreeMeta } from '../dorka-runtime-test-mocks.spec'
import {
  TEST_REPO_ID,
  TEST_WORKTREE_ID,
  makeWorktreeInfo,
...
describe('DorkaRuntimeService', () => {
  it('strips Dorka provenance fields from runtime metadata updates', async () => {
    ...
    await runtime.updateManagedWorktreeMeta(`id:${TEST_WORKTREE_ID}`, {
      ...
      dorkaCreationWorkspaceLayout: { path: '/tmp', nestWorkspaces: false }
    })
