...
import { store, syncSinglePty } from '../dorka-runtime-test-fixtures.spec'
...
describe('DorkaRuntimeService', () => {
  ...
  it('strips retained terminal path line and hash locators before matching', () => {
    const colonPath = '/tmp/dorka report/result.json'
  ...
  it('keeps tmpdir artifact paths in bounded PTY path candidates', () => {
    const artifactPath = join(tmpdir(), 'dorka-runtime-retained-result.json')
    const candidates = appendRecentPtyPathCandidates(undefined, `wrote ${artifactPath}\n`)
  ...
  it('matches terminal artifact paths only when they appear in recent terminal output', () => {
    expect(
      recentTerminalOutputIncludesPath(
        'wrote /tmp/dorka report/result.json:12:3',
        '/tmp/dorka report/result.json',
        '/tmp/dorka report/result.json'
      )
    ).toBe(true)
    expect(
      recentTerminalOutputIncludesPath(
        '\x1b]8;;file:///tmp/dorka%20report/result.json\x1b\\result\x1b]8;;\x1b\\',
        '/tmp/dorka report/result.json',
        '/tmp/dorka report/result.json'
      )
    ).toBe(true)
    ...
    expect(
      recentTerminalOutputIncludesPath(
        'wrote /tmp/dorka report/other.json',
        '/tmp/dorka report/result.json',
        '/tmp/dorka report/result.json'
      )
    ).toBe(false)
