...
describe('DorkaRuntimeService', () => {
  ...
  it('splits visible pty-backed terminal sessions through the parent renderer tab', async () => {
    ...
    const sourceLeafId = sourceEnv.DORKA_PANE_KEY.slice(`${sourceEnv.DORKA_TAB_ID}:`.length)

    const split = await runtime.splitTerminal(handle, { direction: 'vertical' })
    expect(split).toMatchObject({
      handle: expect.stringMatching(/^term_/),
      tabId: sourceEnv.DORKA_TAB_ID,
      paneRuntimeId: -1
    })

    const splitEnv =
      (spawn.mock.calls[1]?.[0] as { env?: Record<string, string> } | undefined)?.env ?? {}
    const splitLeafId = splitEnv.DORKA_PANE_KEY.slice(`${sourceEnv.DORKA_TAB_ID}:`.length)
    expect(split.leafId).toBe(splitLeafId)
    expect(splitTerminal).not.toHaveBeenCalled()
    expect(splitEnv.DORKA_TAB_ID).toBe(sourceEnv.DORKA_TAB_ID)
    expect(splitEnv.DORKA_WORKTREE_ID).toBe(TEST_WORKTREE_ID)
    ...
    const siblingSurfaces = publishedTabs.filter(
      (tab): tab is Extract<typeof tab, { type: 'terminal' }> =>
        tab.type === 'terminal' && tab.parentTabId === sourceEnv.DORKA_TAB_ID
    )
  ...
  it('splits folder workspace pty-backed terminal sessions with folder cwd and env', async () => {
    const folderPath = await mkdtemp(join(tmpdir(), 'dorka-runtime-folder-split-'))
    const spawn = vi
      .fn()
    ...
    expect(splitEnv.DORKA_TAB_ID).toBe(sourceEnv.DORKA_TAB_ID)
    expect(splitEnv.DORKA_WORKSPACE_ID).toBe(TEST_FOLDER_WORKSPACE_KEY)
    expect(splitEnv.DORKA_PROJECT_GROUP_ID).toBe(TEST_FOLDER_PROJECT_GROUP_ID)
    expect(splitEnv.DORKA_WORKSPACE_ROOT).toBe(folderPath)
    expect(splitEnv.DORKA_WORKTREE_ID).toBe(TEST_FOLDER_WORKSPACE_KEY)
  ...
  it('returns an actionable discoverability warning when default adoption fails after spawn', async () => {
    ...
    try {
      ...
      expect(created.warning).toContain(`dorka terminal focus --terminal ${created.handle}`)
