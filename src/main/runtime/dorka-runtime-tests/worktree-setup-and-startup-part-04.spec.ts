...
import { TEST_REPO_ID, makeWorktreeMeta, store } from '../dorka-runtime-test-fixtures.spec'
...
describe('DorkaRuntimeService', () => {
  ...
  it('honors split setup placement for opted-in local startup-draft worktrees', async () => {
    ...
    vi.mocked(createSetupRunnerScript).mockReturnValue({
      ...
      envVars: {
        ...
        DORKA_WORKTREE_PATH: '/tmp/workspaces/runtime-startup-setup-split'
      },
    ...
    const markerPath = `/tmp/repo/.git/dorka/setup-runner.sh.${nonceMatch![1]}.done`
    ...
    expect(mainEnv.DORKA_TAB_ID).toBeDefined()
    expect(mainEnv.DORKA_PANE_KEY).toBeDefined()
  ...
  it('passes the wrapped setup command to activation when startup spawned but setup did not', async () => {
    ...
    vi.mocked(createSetupRunnerScript).mockReturnValue({
      ...
      envVars: {
        DORKA_ROOT_PATH: '/tmp/repo',
        DORKA_WORKTREE_PATH: '/tmp/workspaces/runtime-startup-setup-retry'
      },
      waitForAgentStartup: true
    })
    vi.mocked(listWorktrees).mockResolvedValue([
    ...
    expect(activateWorktree).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        runnerScriptPath: 'C:\\tmp\\repo\\.git\\dorka\\setup-runner.sh',
        command: expect.stringContaining('bash /mnt/c/tmp/repo/.git/dorka/setup-runner.sh')
      }),
      undefined,
      undefined
    )
