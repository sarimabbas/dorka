...
import { TEST_REPO_ID, store } from '../dorka-runtime-test-fixtures.spec'
  ...
  it('uses remote path joins for SSH hook checks and issue-command files', async () => {
    ...
    const fsProvider = {
      readFile: vi.fn(async (filePath: string) => ({
        content: filePath.endsWith('dorka.yaml')
          ? 'scripts:\n  setup: pnpm install\n'
    ...
    try {
      ...
      await expect(runtime.readRepoIssueCommand('id:repo-1')).resolves.toMatchObject({
        ...
        localFilePath: 'C:\\remote\\repo\\.dorka\\issue-command'
      })
    ...
    expect(fsProvider.readFile).toHaveBeenCalledWith('C:\\remote\\repo\\.dorka\\issue-command')
    expect(fsProvider.createDir).toHaveBeenCalledWith('C:\\remote\\repo\\.dorka')
    expect(fsProvider.writeFile).toHaveBeenCalledWith(
      'C:\\remote\\repo\\.dorka\\issue-command',
      'Ship it\n'
    )
    expect(fsProvider.writeFile).toHaveBeenCalledWith(
