...
import { TEST_REPO_ID, TEST_REPO_PATH, store } from '../dorka-runtime-test-fixtures.spec'
...
describe('DorkaRuntimeService', () => {
  describe('checkRepoHooks status', () => {
    ...
    it('reports ok for a missing remote dorka.yaml and error for any other read failure', async () => {
  ...
  it('resolves SSH issue commands from shared dorka.yaml and deletes empty overrides', async () => {
    const remoteStore = {
      ...store,
      getRepos: () => [
        {
    ...
    vi.mocked(parseDorkaYaml).mockReturnValue({
      scripts: {},
    ...
    const fsProvider = {
      readFile: vi.fn(async (filePath: string) => {
        ...
        if (filePath.endsWith('dorka.yaml')) {
    ...
    try {
      await expect(runtime.readRepoIssueCommand('id:repo-1')).resolves.toMatchObject({
        ...
        localFilePath: '/remote/repo/.dorka/issue-command',
        source: 'shared'
      })
    ...
    expect(fsProvider.deletePath).toHaveBeenCalledWith('/remote/repo/.dorka/issue-command', false)
  ...
  it('allows host integration slug helpers for SSH repos through provider-aware GitHub clients', async () => {
    const prRepo = { owner: 'acme', repo: 'dorka', host: 'github.acme.test' }
  ...
  it('routes runtime GitHub repo identity helpers through the selected WSL project runtime', async () => {
    ...
    getRepoSlugMock.mockResolvedValueOnce({ owner: 'acme', repo: 'dorka' })
  ...
  it('routes runtime GitHub issue and work-item actions through the selected WSL project runtime', async () => {
    ...
    createGitHubIssueMock.mockResolvedValueOnce({
      ok: true,
      number: 12,
      url: 'https://github.com/acme/dorka/issues/12'
    })
  ...
  it('pins explicit origin preference on runtime open-by-number work item lookups', async () => {
    ...
    const prRepo = { owner: 'acme', repo: 'dorka' }
