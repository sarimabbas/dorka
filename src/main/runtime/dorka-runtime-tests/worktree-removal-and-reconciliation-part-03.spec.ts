...
import { createWorktreeRemovalRuntime } from '../dorka-runtime-test-scenario-builders.spec'
...
describe('DorkaRuntimeService', () => {
  ...
  it('treats forced runtime deletion of an already-missing unregistered worktree as cleanup', async () => {
    const parentDir = await mkdtemp(join(tmpdir(), 'dorka-runtime-remove-'))
  ...
  it('routes already-missing SSH runtime history cleanup through the PTY owner', async () => {
    ...
    const metaById: Record<string, WorktreeMeta> = {
      [worktreeId]: makeWorktreeMeta({ hostId: 'ssh:ssh-1', dorkaCreationSource: 'ssh' })
    }
    ...
    const runtime = new DorkaRuntimeService(runtimeStore as never, undefined, {
  ...
  it('force-removes a legacy Dorka-created runtime orphaned worktree directory after Git tracking is gone', async () => {
    const parentDir = await mkdtemp(join(tmpdir(), 'dorka-runtime-orphan-'))
  ...
  it('prompts then force-removes an Dorka-created runtime unregistered leftover directory with no git marker', async () => {
    const parentDir = await mkdtemp(join(tmpdir(), 'dorka-runtime-leftover-'))
    const repoPath = join(parentDir, 'repo')
  ...
  it('rejects an Dorka-created runtime unregistered local directory with a git directory', async () => {
    const parentDir = await mkdtemp(join(tmpdir(), 'dorka-runtime-standalone-'))
    const repoPath = join(parentDir, 'repo')
