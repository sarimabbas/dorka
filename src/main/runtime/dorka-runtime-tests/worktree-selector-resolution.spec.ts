...
describe('DorkaRuntimeService', () => {
    ...
    vi.mocked(listWorktrees).mockResolvedValueOnce([
      {
        ...
        head: 'abc',
        branch: 'refs/heads/wolfiesch/dorka-skill-smoke',
        isBare: false,
        isMainWorktree: false
      }
    ])
  ...
  it.each([
    ...
    const folderPath = await mkdtemp(join(tmpdir(), 'dorka-runtime-folder-files-'))
  ...
  it('guides registered SSH repo ids without probing the provider', async () => {
    ...
    const runtime = new DorkaRuntimeService(remoteStore)
  ...
  it('retires a generated name before a post-create listing failure', async () => {
    const addRetiredWorktreeName = vi.fn()
    const runtime = new DorkaRuntimeService({ ...store, addRetiredWorktreeName })
