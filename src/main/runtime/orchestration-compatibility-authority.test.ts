...
describe('orchestration compatibility runtime authority', () => {
  ...
  it('uses the hydrated hook commitment for a restored exact PTY', () => {
    ...
    const uncommitted = new DorkaRuntimeService()
  ...
  it('attests current coordinators immediately without upgrading restored legacy callers', () => {
    ...
    const createIntegratedRuntime = (launchTokenHash: string | null): DorkaRuntimeService => {
      const runtime = new DorkaRuntimeService(null, undefined, {
    ...
    const evidenceFor = (runtime: DorkaRuntimeService, launchToken = TOKEN) => ({
      terminalHandle: 'term-1',
      paneKey: PANE_KEY,
      launchToken,
