...
import { createRuntimeTransportMetadata, DorkaRuntimeRpcServer } from './runtime-rpc'
...
describe('DorkaRuntimeRpcServer', () => {
  ...
  it('reclaims runtime metadata clobbered by a second instance that has since died', async () => {
    ...
    // dorka-runtime.json with its own pid, so the CLI reports stale_bootstrap
  ...
  it('stops reclaiming runtime metadata after the server is stopped', async () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'dorka-runtime-rpc-'))
    const server = new DorkaRuntimeRpcServer({ runtime: new DorkaRuntimeService(), userDataPath })
    await server.start()
  ...
  it('drops a republish from an ownership read that lands after the server stopped', async () => {
    ...
    const userDataPath = mkdtempSync(join(tmpdir(), 'dorka-runtime-rpc-'))
    const runtime = new DorkaRuntimeService()
    const server = new DorkaRuntimeRpcServer({ runtime, userDataPath, pid: 1001 })
    await server.start()
  ...
  it('flushes a lastSeen refresh scheduled while transports stop', async () => {
    const server = new DorkaRuntimeRpcServer({
      runtime: new DorkaRuntimeService(),
      userDataPath: mkdtempSync(join(tmpdir(), 'dorka-runtime-rpc-')),
