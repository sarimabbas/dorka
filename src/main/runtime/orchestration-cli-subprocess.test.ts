...
// real in-process runtime via DORKA_USER_DATA_PATH, and asserts:
...
async function runBuiltCli(
  ...
  const child = spawn(process.execPath, [CLI_PATH, ...args], {
    env: {
      ...
      DORKA_TERMINAL_HANDLE: 'term_cli',
      ...extraEnv
    },
...
describeIfBuilt('dorka orchestration check --wait subprocess (§3.4)', () => {
  it('emits newline-flushed JSON keepalives to stderr while waiting', async () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'dorka-cli-sub-'))
    ...
    try {
      // Why: use the DORKA_KEEPALIVE_INTERVAL_MS escape hatch to shrink the
      ...
      const child = spawn(
        ...
        ],
        {
          env: {
            ...process.env,
            DORKA_USER_DATA_PATH: userDataPath,
            DORKA_TERMINAL_HANDLE: 'term_nobody',
            DORKA_KEEPALIVE_INTERVAL_MS: String(keepaliveMs)
          },
...
describeIfBuilt('dorka orchestration reset subprocess', () => {
  it('validates reset scopes against an isolated runtime through the built CLI', async () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'dorka-cli-reset-'))
    const runtime = new DorkaRuntimeService()
    const db = new OrchestrationDb(':memory:')
    runtime.setOrchestrationDb(db)
...
describeIfBuilt('dorka orchestration task readiness subprocess', () => {
  it('creates a late dependent as ready through the built CLI and runtime', async () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'dorka-cli-task-readiness-'))
