...
import { DorkaRuntimeWithVerifyOrchestrationCompatibilityCaller } from './dorka-runtime-verify-orchestration-compatibility-caller'
...
export class DorkaRuntimeWithGetOrchestrationDispatchAuthority extends DorkaRuntimeWithVerifyOrchestrationCompatibilityCaller {
  ...
  getTerminalOrchestrationCliCommand(handle: string): OrchestrationCliCommand {
    ...
    return resolveTerminalOrchestrationCliCommand({
      ...
      isWsl: pty.isWsl,
      worktreeId: pty.worktreeId,
      // Dev builds run the CLI as `dorka-dev`; a packaged app must not advertise it.
      runtimeCliCommand: getAppEnvironment().isPackaged() ? undefined : 'dorka-dev',
