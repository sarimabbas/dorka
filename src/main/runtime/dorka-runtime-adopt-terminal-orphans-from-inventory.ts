...
import { DorkaRuntimeWithSubscribeToTerminalResize } from './dorka-runtime-subscribe-to-terminal-resize'
import type {
...
export class DorkaRuntimeWithAdoptTerminalOrphansFromInventory extends DorkaRuntimeWithSubscribeToTerminalResize {
  ...
  async resolveActiveTerminal(
    ...
    // structured worker is covered by the `DORKA_TERMINAL_HANDLE` its child is spawned with. That
    ...
    // pane the guess resolves. Such a child now carries `DORKA_STRUCTURED_SESSION` and the CLI
