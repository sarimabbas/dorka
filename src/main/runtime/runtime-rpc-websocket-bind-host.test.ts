...
describe('DorkaRuntimeRpcServer WebSocket bind host (STA-2370)', () => {
  ...
  it('binds all interfaces at startup when exposeNetworkByDefault is set (dorka serve)', async () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'dorka-runtime-rpc-'))
    const server = new DorkaRuntimeRpcServer({
      runtime: new DorkaRuntimeService(),
      userDataPath,
  ...
  it('upgrades a reused pending grant to network reach so its link survives a relaunch', async () => {
    ...
    const relaunched = new DorkaRuntimeRpcServer({
      runtime: new DorkaRuntimeService(),
      userDataPath,
      enableWebSocket: true,
      wsPort: 0
  ...
  it('honours a pinned bind host over exposeNetworkByDefault (dorkad --bind)', async () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'dorka-runtime-rpc-'))
    const server = new DorkaRuntimeRpcServer({
      runtime: new DorkaRuntimeService(),
      userDataPath,
  ...
  it('stays pinned to loopback even after a device has connected once', async () => {
    ...
    // network-reach device has connected. A loopback dorkad would therefore go wide one
