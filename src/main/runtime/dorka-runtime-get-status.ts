import { DorkaRuntimeWithGetRuntimeId } from './dorka-runtime-get-runtime-id'
...
import type { RuntimeWorktreeLifecycleEvent } from './dorka-runtime-core'
import { WORKTREE_CREATE_RESULT_TTL_MS } from './dorka-runtime-core'
...
export class DorkaRuntimeWithGetStatus extends DorkaRuntimeWithGetRuntimeId {
  ...
  getStatus(): RuntimeStatus {
    ...
    const capabilities: RuntimeCapability[] = RUNTIME_CAPABILITIES.filter(
      (capability) =>
        ...
        (process.env.DORKA_E2E_DISABLE_PAIRED_TERMINAL_PARKING !== '1' ||
          capability !== TERMINAL_PAIRED_PARKING_RUNTIME_CAPABILITY) &&
        (process.env.DORKA_E2E_DISABLE_AUTHORITATIVE_SESSION_TABS_INVENTORY !== '1' ||
    ...
    // Why not a static capability: dev trees and `dorka serve` installs may carry no
    ...
    // that actually applies, and a host that says "set DORKA_BROWSER_EXECUTABLE" to someone
    ...
    return {
      ...
      // Why: headless dorka serve cannot create/stream BrowserViews, so clients
