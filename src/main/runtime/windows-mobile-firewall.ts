...
const FIREWALL_RULE_NAME = 'Dorka.MobilePairing'
const FIREWALL_RULE_DISPLAY_NAME = 'Dorka Mobile Pairing'
...
New-NetFirewallRule -Name ${quotePowerShell(FIREWALL_RULE_NAME)} -DisplayName ${quotePowerShell(FIREWALL_RULE_DISPLAY_NAME)} -Description 'Allows Dorka Mobile to connect to this Dorka desktop on private networks.' -Direction Inbound -Action Allow -Enabled True -Profile Private -Protocol TCP -LocalPort ${port} -Program ${quotePowerShell(executablePath)} -EdgeTraversalPolicy Block | Out-Null`
}
