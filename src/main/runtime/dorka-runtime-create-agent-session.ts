...
import { DorkaRuntimeWithGetAgentSessionExecutionNamespace } from './dorka-runtime-get-agent-session-execution-namespace'
import type {
...
export class DorkaRuntimeWithCreateAgentSession extends DorkaRuntimeWithGetAgentSessionExecutionNamespace {
  async createAgentSession(
    ...
    const operation = (async (): Promise<RuntimeCreateAgentSessionResult> => {
      ...
      // DORKA_TERMINAL_HANDLE, so it is the only name a lost spawn can be re-found by.
      reclaim.identity = {
