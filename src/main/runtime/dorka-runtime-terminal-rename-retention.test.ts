import './dorka-runtime-test-lifecycle.spec'
import type { RuntimeStore } from './runtime-store-contract'
import { describe, expect, it, vi } from 'vitest'
import { createMobileCreateTestNotifier } from './dorka-runtime-test-scenario-builders.spec'
import { DorkaRuntimeService } from './dorka-runtime-test-mocks.spec'
import {
  HEADLESS_LEAF_ID,
  TEST_WORKTREE_ID,
...
describe('terminal rename before renderer graph hydration', () => {
  it.each(['Media Engine Orch', null])(
    ...
    async (title) => {
      ...
      const runtime = new DorkaRuntimeService(checkedStore)
      ...
      const restored = new DorkaRuntimeService(checkedStore)
  ...
  it('does not recreate a closed persisted tab from a surviving PTY record', async () => {
    ...
    const runtime = new DorkaRuntimeService(runtimeStore as RuntimeStore)
