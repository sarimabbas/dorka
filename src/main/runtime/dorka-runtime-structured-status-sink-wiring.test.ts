...
type DorkaRuntimeDeps = NonNullable<ConstructorParameters<typeof DorkaRuntimeService>[2]>
...
] as const satisfies readonly (keyof DorkaRuntimeDeps)[]

const MAIN_ROOT = join(import.meta.dirname, '..')

/** The text of the `new DorkaRuntimeService(...)` call in one entry point. */
function runtimeConstruction(relativePath: string): string {
  const source = readFileSync(join(MAIN_ROOT, relativePath), 'utf8')
  const start = source.indexOf('new DorkaRuntimeService(')
  ...
  throw new Error(`unbalanced DorkaRuntimeService construction in ${relativePath}`)
}
 ...
 *  constructs a runtime without these lists no agents at all — and `dorkad` serves `worktree.ps`
 *  and `agentSession.*` exactly like the desktop does. */
describe('every host that constructs a runtime wires the agent-status store', () => {
  it.each([['dorkad/dorkad-entry.ts'], ['startup/main-process-runtime-service.ts']])(
...
describe('structured status sink wiring', () => {
  it('hands the host the sink the runtime was constructed with', async () => {
    ...
    const runtime = new DorkaRuntimeService(null, undefined, { structuredAgentStatusSink: sink })
