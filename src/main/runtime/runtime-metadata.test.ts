...
describe('runtime metadata', () => {
  ...
  it('writes and reads runtime metadata atomically', () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'dorka-runtime-metadata-'))
  ...
  it('clears the runtime metadata file', () => {
    ...
    expect(getRuntimeMetadataPath(userDataPath)).toContain('dorka-runtime.json')
  })
  ...
  it.runIf(process.platform !== 'win32')(
    ...
    () => {
      const userDataPath = mkdtempSync(join(tmpdir(), 'dorka-runtime-secure-files-'))
      tempDirs.push(userDataPath)
      ...
      for (const path of [
        join(userDataPath, 'dorka-devices.json'),
        join(userDataPath, 'dorka-e2ee-keypair.json'),
  ...
  it.runIf(process.platform !== 'win32')(
    ...
    () => {
      const userDataPath = mkdtempSync(join(tmpdir(), 'dorka-runtime-existing-secure-files-'))
      tempDirs.push(userDataPath)
      ...
      const devicesPath = join(userDataPath, 'dorka-devices.json')
      const keypairPath = join(userDataPath, 'dorka-e2ee-keypair.json')
  ...
  it('replaces oversized E2EE keypair files instead of reading them as metadata', () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'dorka-runtime-large-keypair-'))
    tempDirs.push(userDataPath)
    const keypairPath = join(userDataPath, 'dorka-e2ee-keypair.json')
