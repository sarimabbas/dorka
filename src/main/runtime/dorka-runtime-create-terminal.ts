...
import { DorkaRuntimeWithTerminalCreateDeduplication } from './dorka-runtime-terminal-create-deduplication'
import * as dependencies from './dorka-runtime-create-terminal-dependencies'
import { createDesktopTerminal } from './dorka-runtime-create-terminal-desktop'
import { buildRuntimeAgentTeamsLaunchPlan } from './dorka-runtime-agent-teams-launch-plan'
import { createPtySpawnCommitReporter } from './dorka-runtime-report-pty-spawn-commit'
import { recordPtySurface, spawnSurfaceClaimSequence } from './pty-recorded-surface-topology'

export class DorkaRuntimeWithCreateTerminal extends DorkaRuntimeWithTerminalCreateDeduplication {
  async createTerminal(
    worktreeSelector?: string,
    opts: dependencies.TerminalCreateOptions = {}
  ): Promise<dependencies.RuntimeTerminalCreate> {
    dependencies.assertStartupAgentHasWorkspaceSelector(opts, worktreeSelector)
    ...
    const shouldCreateInBackground = dependencies.shouldCreateTerminalInBackground(
      opts,
      worktreeSelector !== undefined,
      availableAuthoritativeWindow !== null
    )
    if (shouldCreateInBackground) {
      dependencies.assertRuntimePtySpawnAvailable(this.ptyController)
      const workspace = await this.resolveTerminalWorkspaceLaunchScope(worktreeSelector)
      return await this.createTerminalInWorkspaceScope(workspace, opts)
    }
    return createDesktopTerminal(this, worktreeSelector, opts, presentation, rendererWindow)
  }

  async createTerminalInWorkspaceScope(
    workspace: dependencies.TerminalWorkspaceLaunchScope,
    opts: dependencies.TerminalCreateOptions = {}
  ): Promise<dependencies.RuntimeTerminalCreate> {
    const presentation = dependencies.resolveTerminalPresentation(opts)
    dependencies.assertRuntimePtySpawnAvailable(this.ptyController)
    ...
    const cwd = this.resolveWorkspaceTerminalStartupCwd(workspace, launchOpts.cwd) ?? workspace.path
    ...
    const canAdoptPaneIdentity = dependencies.canAdoptTerminalPaneIdentity(
      hintedTabId,
      launchOpts.leafId
    )
    ...
    try {
      ...
      const baseEnv = {
        ...launchOpts.env,
        ...(launchToken ? { DORKA_AGENT_LAUNCH_TOKEN: launchToken } : {})
      }
      ...
      const terminalColorQueryReplies =
        launchOpts.terminalColorQueryReplies ?? dependencies.getTerminalViewColorQueryReplyColors()
      if (launchOpts.signal?.aborted) {
        throw new Error('client_disconnected')
      }
