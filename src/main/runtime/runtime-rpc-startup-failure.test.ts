...
describe('runtime RPC startup failure reporting', () => {
  ...
  it.each([
    ...
    const error = Object.assign(new Error('/Users/private/dorka-runtime.json'), { code })

    expect(classifyRuntimeRpcStartFailure(error)).toBe(expected)
  })

  it('classifies a code carried on a wrapped cause', () => {
    const error = new Error('failed to publish dorka-runtime.json', {
  ...
  it('walks past an unmapped wrapper code to the mapped cause', () => {
    const error = Object.assign(new Error('failed to publish dorka-runtime.json'), {
  ...
  it('records a privacy-safe telemetry event', () => {
    ...
    const error = Object.assign(new Error('/Users/private/dorka-runtime.json'), { code: 'EACCES' })
  ...
  it('shows the CLI impact and local cause', async () => {
    ...
    expect(showMessageBoxMock).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        type: 'error',
        title: 'Dorka CLI unavailable',
        message: "Dorka couldn't start its local command transport.",
        detail: expect.stringMatching(
          /dorka status.*dorka terminal.*orchestration.*Cause: metadata write failed/s
        )
      })
    )
  })
  ...
  it.each([
    ['EACCES', "Check permissions on Dorka's data folder"],
    ['EPERM', "Check permissions on Dorka's data folder"],
    ...
    ['ENOENT', "Dorka's data folder may be missing"],
  ...
  it('falls back to a plain restart when the cause is unrecognised', async () => {
    ...
    const detail = showMessageBoxMock.mock.calls[0]?.[1]?.detail as string
    expect(detail).toContain('Restart Dorka to try again.')
    expect(detail).not.toContain("Check permissions on Dorka's data folder")
  })
