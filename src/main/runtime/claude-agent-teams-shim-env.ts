...
import { getDorkaCliCommandNameForPlatform } from '../../shared/dorka-cli-command-name'
...
export async function buildClaudeAgentTeamsLaunchPlan(args: {
  ...
  if (!shimBin) {
    // Why: without an absolute CLI path the shim would resolve a bare `dorka` against the pane cwd, so degrade instead.
    return {
      command: addClaudeTeammateModeInProcess(args.command),
      env: { CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS: '1' }
    }
...
/** Absolute path to the Dorka CLI that backs the tmux shim, or null when none can be qualified. */
export function resolveClaudeAgentTeamsShimBin(
  ...
  const override = env.DORKA_AGENT_TEAMS_SHIM_BIN
  if (override) {
  ...
  return (
    findExecutableOnPath(process.platform === 'win32' ? 'dorka-dev.cmd' : 'dorka-dev', pathValue) ??
    findExecutableOnPath(getDorkaCliCommandNameForPlatform(process.platform), pathValue)
  )
}

function defaultShimRoot(): string {
  return join(homedir(), '.dorka', 'claude-agent-teams-bin')
}
...
function bundledLauncherPath(): string | null {
  ...
  if (process.platform === 'darwin') {
    return join(process.resourcesPath, 'bin', 'dorka')
  }
  if (process.platform === 'linux') {
    return join(process.resourcesPath, 'bin', 'dorka-ide')
  }
  if (process.platform === 'win32') {
    return join(process.resourcesPath, 'bin', 'dorka.exe')
  }
  return null
}
...
// PATH entries on POSIX), so a stray `dorka` next to the agent's files would run with the team token. Demand a
// fully-qualified binary instead of guessing one.
function unixShimScript(): string {
  return [
    '#!/usr/bin/env sh',
    'set -eu',
    'dorka_bin=${DORKA_AGENT_TEAMS_SHIM_BIN:-}',
    'case $dorka_bin in',
    '  /*|[A-Za-z]:[\\\\/]*) ;;',
    '  *)',
    '    echo "dorka agent-teams tmux shim: DORKA_AGENT_TEAMS_SHIM_BIN must be an absolute path" >&2',
    '    exit 127',
    '    ;;',
    'esac',
    'exec "$dorka_bin" agent-teams-tmux "$@"',
    ''
  ].join('\n')
}
...
export function windowsClaudeAgentTeamsShimScript(): string {
  return [
    '@echo off',
    'setlocal',
    'set "DORKA_SHIM_BIN=%DORKA_AGENT_TEAMS_SHIM_BIN%"',
    'if not defined DORKA_SHIM_BIN goto :unqualified',
    'if "%DORKA_SHIM_BIN:~1,1%"==":" goto :run',
    'if "%DORKA_SHIM_BIN:~0,2%"=="\\\\" goto :run',
    'goto :unqualified',
    ':run',
    ...
    'exit /b %ERRORLEVEL%',
    ':unqualified',
    'echo dorka agent-teams tmux shim: DORKA_AGENT_TEAMS_SHIM_BIN must be an absolute path 1>&2',
    'exit /b 127',
    ''
  ].join('\r\n')
}
