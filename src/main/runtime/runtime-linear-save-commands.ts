      ...
      : await this.runLinearAgentWrite(
          ...
          (cause) =>
            linearError(
              ...
              'Linear may have applied the issue save, but Dorka could not confirm it.',
              {
                nextSteps: [
                  `Run \`dorka linear issue ${target.issue.identifier} --workspace ${target.workspaceId} --json\` before retrying.`
                ],
                ...(cause ? { cause } : {})
              }
            )
    ...
    if (!alreadySet) {
      await this.runLinearAgentWrite(
        ...
        (cause) =>
          linearError(
            'linear_write_unconfirmed',
            'Linear may have applied the task update, but Dorka could not confirm it.',
            {
              nextSteps: [
                `Run \`dorka linear issue ${target.issue.identifier} --workspace ${target.workspaceId} --json\` and check the updated field before retrying.`
              ],
              ...(cause ? { cause } : {})
            }
          )
