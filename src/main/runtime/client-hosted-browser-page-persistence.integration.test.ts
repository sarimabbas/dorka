/**
 * Drives the runtime's own wiring: a real DorkaRuntimeService, the real page registry, and the real
...
function startRuntime(profile: ReturnType<typeof createProfile>): DorkaRuntimeService {
  const runtime = new DorkaRuntimeService(profile.store, undefined, {})
  runtime.rehydrateClientHostedBrowserPages()
  return runtime
}

function placeAndPublish(runtime: DorkaRuntimeService, browserPageId: string): void {
...
function attachHost(runtime: DorkaRuntimeService) {
