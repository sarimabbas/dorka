// #11761: on a headless `dorka serve` host the HTTP agent hook is the only carrier of
...
async function projectAgentStatus(
  rows: AgentStatusIpcPayload[],
  preparePane?: (runtime: DorkaRuntimeService) => void
...
function observePaneTitle(runtime: DorkaRuntimeService, title: string): void {
  runtime.onPtyData(PTY_ID, `\x1b]0;${title}\x07`, Date.now())
}

function lastOscTitleEpochMs(runtime: DorkaRuntimeService): number {
  const pty = (
