...
function expectStablePaneKeyEnv(env: Record<string, string>): string {
  expect(env.DORKA_TAB_ID).toMatch(UUID_RE)
  const leafId = env.DORKA_PANE_KEY?.slice(`${env.DORKA_TAB_ID}:`.length)
  expect(leafId).toMatch(UUID_RE)
  expect(env.DORKA_PANE_KEY).toBe(`${env.DORKA_TAB_ID}:${leafId}`)
  return env.DORKA_PANE_KEY
}

function createRuntime(): RuntimeService {
  return new DorkaRuntimeService(store)
}
