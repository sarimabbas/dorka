...
import { DorkaRuntimeWithBuildHeadlessMobileSessionBrowserTabs } from './dorka-runtime-build-headless-mobile-session-browser-tabs'
...
export class DorkaRuntimeWithPerformMobileSessionPtyRecordsRefresh extends DorkaRuntimeWithBuildHeadlessMobileSessionBrowserTabs {
    ...
    if (tab.type === 'terminal') {
      ...
      if (shouldMaterializePendingTerminal) {
        ...
        let agentStartup: Awaited<
          ReturnType<DorkaRuntimeService['resolveMobileSessionTerminalCommand']>
        > = {}
        if (tab.launchAgent) {
          try {
