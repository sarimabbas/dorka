 ...
 * to speak for itself.
 *
 * Without `DORKA_TERMINAL_HANDLE` the worker's Bash tool has nothing to pass as `--from`, and
 ...
 *
 * `DORKA_CLI_COMMAND: 'dorka'` is honest ONLY because of the PATH prepend below. Dorka's Linux CLI
 * installs as `dorka-ide` so it never claims GNOME Dorka's /usr/bin/dorka (stablyai/orca#7904), and
 ...
 *
 * Deliberately NOT `DORKA_PANE_KEY`. Claude structured sessions run hooks, and a pane key in their
 ...
 *
 * A session that is not a dispatched worker gets ONE variable, `DORKA_STRUCTURED_SESSION`, and it
 ...
 * — which is why it is a different decision from withholding `DORKA_PANE_KEY`, not a reversal of it.
...
export function structuredWorkerChildIdentityEnv(
  ...
  if (!identity) {
    return { ...childEnv, [DORKA_STRUCTURED_SESSION_ENV]: '1' }
  }
  const env: Record<string, string> = {
    ...childEnv,
    DORKA_TERMINAL_HANDLE: identity.handle,
    DORKA_CLI_COMMAND: 'dorka'
  }
  applyDorkaCliPath(env)
  return env
}
 ...
 */
function applyDorkaCliPath(env: Record<string, string>): void {
