...
function writeLegacyLinearToken(token: string | Buffer, viewer: Record<string, unknown>): void {
  const dorkaDir = join(tempHome, '.dorka')
  mkdirSync(dorkaDir, { recursive: true })
  writeFileSync(join(dorkaDir, 'linear-token.enc'), token)
  writeFileSync(join(dorkaDir, 'linear-viewer.json'), JSON.stringify(viewer), {
    encoding: 'utf-8'
  })
}
...
function writeMultiWorkspaceFiles(
  ...
  const dorkaDir = join(tempHome, '.dorka')
  mkdirSync(join(dorkaDir, 'linear-tokens'), { recursive: true })
  writeFileSync(
    join(dorkaDir, 'linear-workspaces.json'),
    JSON.stringify({
      version: 1,
...
beforeEach(() => {
  tempHome = mkdtempLike('dorka-linear-client-')
  fixtures = new Map([
    [
      'token-alpha',
      {
...
describe('Linear client workspace storage', () => {
  ...
  it('migrates legacy token storage to a real workspace id when explicitly tested', async () => {
    ...
    expect(existsSync(join(tempHome, '.dorka', 'linear-token.enc'))).toBe(false)
    expect(readFileSync(join(tempHome, '.dorka', 'linear-workspaces.json'), 'utf-8')).toContain(
      'org-alpha'
    )
  })
  ...
  it('does not pass encrypted safeStorage bytes to the Linear SDK when encryption is unavailable', async () => {
    const tokenPath = join(tempHome, '.dorka', 'linear-token.enc')
