...
/** True when any record is real user work rather than a shared symlink Dorka put
 *  in the worktree.
 *
 *  Fails closed on purpose: anything not positively identified as an Dorka-owned
...
async function anyRecordIsUserDirt(
  ...
  // fail-closed check would block review creation over Dorka's own symlink.
