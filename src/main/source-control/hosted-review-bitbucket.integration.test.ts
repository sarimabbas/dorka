...
describe('Bitbucket hosted review integration', () => {
  beforeEach(() => {
    process.env = { ...OLD_ENV, DORKA_BITBUCKET_ACCESS_TOKEN: 'local-token' }
    delete process.env.DORKA_BITBUCKET_EMAIL
    delete process.env.DORKA_BITBUCKET_API_TOKEN
  ...
  it('resolves a Bitbucket PR through real git remote parsing and HTTP API calls', async () => {
    ...
    const repoPath = await mkdtemp(join(tmpdir(), 'dorka-bitbucket-review-'))
    try {
      ...
      process.env.DORKA_BITBUCKET_API_BASE_URL = `http://127.0.0.1:${address.port}/2.0`
  ...
  it('does not cache a transient API failure as a definitive no-review result', async () => {
    ...
    const repoPath = await mkdtemp(join(tmpdir(), 'dorka-bitbucket-review-recovery-'))
    try {
      const address = server.address()
      if (!address || typeof address === 'string') {
        throw new Error('expected TCP server address')
