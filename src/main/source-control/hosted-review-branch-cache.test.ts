...
describe('hosted review branch cache (#11532)', () => {
  ...
  it('retires a cached no-review answer when Dorka opens a review', async () => {
    const lookup = vi
      .fn<() => Promise<HostedReviewInfo | null>>()
  ...
  it('discards a lookup that was already in flight when Dorka opened a review', async () => {
    let resolveLookup: (value: HostedReviewInfo | null) => void = () => {}
    const lookup = vi
      .fn<() => Promise<HostedReviewInfo | null>>()
  ...
  describe('in-flight deadline (P1-D)', () => {
    ...
    it('does not adopt a straggler whose invalidated scope was evicted', async () => {
      ...
      // Stored, that "no review" would hide the review Dorka had just opened for
