...
// cannot commit — it is a symlink Dorka created.
describe('createHostedReview with shared symlinks', () => {
  ...
  beforeEach(() => {
    worktree = mkdtempSync(join(tmpdir(), 'dorka-hosted-shared-'))
  ...
  // Why: only an *untracked* record can be Dorka's artifact. A tracked change at a
