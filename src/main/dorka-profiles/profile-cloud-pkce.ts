...
import {
  DORKA_CLOUD_CALLBACK_RESPONSE_HEADERS,
  DORKA_CLOUD_CALLBACK_SUCCESS_PAGE
} from './profile-cloud-callback-page'

export type DorkaCloudAuthorizationCode = {
  code: string
  codeVerifier: string
  nonce: string
  redirectUri: string
...
export function beginDorkaCloudPkceFlow(
  config: DorkaCloudAuthConfig,
  localProfileId: string
): Promise<DorkaCloudAuthorizationCode> {
  ...
  return new Promise((resolve, reject) => {
    ...
    function writeInvalidCallback(response: ServerResponse): void {
      ...
      response.end('Invalid Dorka sign-in response.')
    }
    ...
    const server = createServer((request, response) => {
      try {
        ...
        if (url.searchParams.has('error')) {
          ...
          response.end(
            cancelled
              ? 'Dorka sign-in was cancelled.'
              : 'Dorka sign-in failed. Return to Dorka and try again.'
          )
          rejectFlow(
            new Error(cancelled ? 'dorka_cloud_auth_denied' : 'dorka_cloud_auth_callback_failed')
          )
          return
        }
        if (!code) {
        ...
        response.writeHead(200, DORKA_CLOUD_CALLBACK_RESPONSE_HEADERS)
        response.end(DORKA_CLOUD_CALLBACK_SUCCESS_PAGE)
        resolveFlow(code)
      } catch (error) {
        rejectFlow(error instanceof Error ? error : new Error('dorka_cloud_auth_callback_failed'))
      }
    })

    const timeout = setTimeout(() => {
      rejectFlow(new Error('dorka_cloud_auth_timeout'))
    }, AUTH_TIMEOUT_MS)
    ...
    server.listen(0, '127.0.0.1', () => {
      ...
      if (!address || typeof address === 'string') {
        rejectFlow(new Error('dorka_cloud_auth_loopback_unavailable'))
        return
      }
      ...
      void shell.openExternal(authorizeUrl.toString()).catch((error) => {
        rejectFlow(
          error instanceof Error ? error : new Error('dorka_cloud_auth_browser_open_failed')
        )
      })
    })
  })
