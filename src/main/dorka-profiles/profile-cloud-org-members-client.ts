...
const ORG_ROLES: readonly DorkaOrgRole[] = ['owner', 'admin', 'member']

function isOrgRole(value: unknown): value is DorkaOrgRole {
  return typeof value === 'string' && (ORG_ROLES as readonly string[]).includes(value)
}

function normalizeRole(value: unknown, fallback: DorkaOrgRole): DorkaOrgRole {
...
function normalizeMember(value: unknown): DorkaOrgMember | null {
...
function normalizePendingInvite(value: unknown): DorkaOrgPendingInvite | null {
  if (!value || typeof value !== 'object') {
    return null
  }
  const record = value as Record<string, unknown>
...
function normalizeRoster(value: unknown): DorkaOrgMembersRoster {
  if (!value || typeof value !== 'object') {
    throw new Error('invalid_dorka_org_members_roster')
  }
  const record = value as Record<string, unknown>
  const members = Array.isArray(record.members)
    ? record.members
        .map(normalizeMember)
        .filter((member): member is DorkaOrgMember => member !== null)
    : []
  const pendingInvites = Array.isArray(record.pendingInvites)
    ? record.pendingInvites
        .map(normalizePendingInvite)
        .filter((invite): invite is DorkaOrgPendingInvite => invite !== null)
    : []
  return {
    members,
    pendingInvites,
...
function orgMembersUrl(config: DorkaCloudAuthConfig, orgId: string, path: string): string {
...
async function requestOrgMembers<T>(
  ...
  if (!response.ok) {
    throw new DorkaCloudRequestError(response.status, await extractErrorCode(response))
  }
  return parse((await response.json()) as unknown)
}

export async function listDorkaCloudOrgMembers(
  config: DorkaCloudAuthConfig,
  session: DorkaCloudSession,
  orgId: string
): Promise<DorkaOrgMembersRoster> {
...
export async function inviteDorkaCloudOrgMember(
  config: DorkaCloudAuthConfig,
  session: DorkaCloudSession,
  args: { orgId: string; email: string; role: DorkaOrgRole }
): Promise<void> {
...
export async function revokeDorkaCloudOrgInvite(
  config: DorkaCloudAuthConfig,
  session: DorkaCloudSession,
...
export async function changeDorkaCloudOrgMemberRole(
  config: DorkaCloudAuthConfig,
  session: DorkaCloudSession,
  args: { orgId: string; userId: string; role: DorkaOrgRole }
): Promise<void> {
  await requestOrgMembers(
...
export async function removeDorkaCloudOrgMember(
  config: DorkaCloudAuthConfig,
  session: DorkaCloudSession,
