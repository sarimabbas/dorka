...
import type { DorkaCloudSessionReadResult } from './profile-cloud-session-store'
import { getDorkaProfileAuthStatusFromProfile } from './profile-cloud-auth-status'

const { readSession, configuration } = vi.hoisted(() => ({
  readSession: vi.fn<() => DorkaCloudSessionReadResult>(),
  configuration: { configured: true }
}))

vi.mock('./profile-cloud-session-store', () => ({ readDorkaCloudSession: readSession }))
vi.mock('./profile-cloud-auth-config', () => ({
  getDorkaCloudAuthConfig: () => configuration,
  isDorkaCloudDevAuthEnabled: () => false
}))

function activeProfile(linked: boolean): ActiveDorkaProfileState {
  const profile: ActiveDorkaProfileState['profile'] = {
    id: 'profile-1',
    name: 'Personal',
...
const absentSessions: DorkaCloudSessionReadResult[] = [
...
describe('unexpected sign-out auth evidence', () => {
  ...
  it.each(absentSessions)('requires a preserved cloud link for $status credentials', (session) => {
    ...
    expect(getDorkaProfileAuthStatusFromProfile(linked, '')).toMatchObject({
    ...
    const signedOut = getDorkaProfileAuthStatusFromProfile(activeProfile(false), '')
  ...
  it.each(absentSessions)(
    ...
    (session) => {
      ...
      readSession.mockReturnValue(session)
      expect(getDorkaProfileAuthStatusFromProfile(activeProfile(true), '').state).toBe(
        'unconfigured'
      )
      expect(getDorkaProfileAuthStatusFromProfile(activeProfile(false), '').state).toBe(
        'unconfigured'
      )
    }
  )
  ...
  it('treats a live memory-only session as connected, then reconnects after its loss', () => {
    ...
    expect(getDorkaProfileAuthStatusFromProfile(linked, '').state).toBe('reconnect-required')
  })
})
