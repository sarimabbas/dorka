...
import {
  ...
  DEFAULT_LOCAL_DORKA_PROFILE_NAME,
  DORKA_PROFILE_INDEX_SCHEMA_VERSION,
  type DorkaProfileIndex
...
describe('profile index store', () => {
  beforeEach(() => {
    testState.dir = mkdtempSync(join(tmpdir(), 'dorka-profile-test-'))
  ...
  it('creates the default local profile and copies legacy state without deleting it', async () => {
    ...
    writeFileSync(join(testState.dir, 'dorka-data.json'), JSON.stringify(legacyState), 'utf-8')
    writeFileSync(
      join(testState.dir, 'dorka-data.json.bak.0'),
    ...
    const { ensureActiveDorkaProfile, getDorkaProfileIndexPath } = await loadProfileIndexStore()
    const activeProfile = ensureActiveDorkaProfile()

    expect(activeProfile.profile.id).toBe(DEFAULT_LOCAL_DORKA_PROFILE_ID)
    expect(activeProfile.profile.name).toBe(DEFAULT_LOCAL_DORKA_PROFILE_NAME)
    expect(activeProfile.dataFile).toBe(
      join(testState.dir, 'profiles', DEFAULT_LOCAL_DORKA_PROFILE_ID, 'dorka-data.json')
    )
    ...
    expect(
      readJson(
        join(testState.dir, 'profiles', DEFAULT_LOCAL_DORKA_PROFILE_ID, 'browser-session-meta.json')
      )
    ).toEqual(legacyBrowserSessionMeta)
    expect(existsSync(join(testState.dir, 'dorka-data.json'))).toBe(true)

    expect(readJson(getDorkaProfileIndexPath())).toMatchObject({
      schemaVersion: DORKA_PROFILE_INDEX_SCHEMA_VERSION,
      activeProfileId: DEFAULT_LOCAL_DORKA_PROFILE_ID,
      profiles: [expect.objectContaining({ id: DEFAULT_LOCAL_DORKA_PROFILE_ID, kind: 'local' })]
    })
  })
  ...
  it('uses an existing active profile data file without overwriting it from legacy state', async () => {
    ...
    writeFileSync(join(profileDirectory, 'dorka-data.json'), JSON.stringify(profileData), 'utf-8')
    writeFileSync(
      join(testState.dir, 'dorka-data.json'),
      JSON.stringify({ schemaVersion: 1, repos: [{ id: 'legacy-repo' }] }),
      'utf-8'
    )
    const index: DorkaProfileIndex = {
      schemaVersion: DORKA_PROFILE_INDEX_SCHEMA_VERSION,
    ...
    writeFileSync(join(testState.dir, 'dorka-profile-index.json'), JSON.stringify(index), 'utf-8')

    const { ensureActiveDorkaProfile } = await loadProfileIndexStore()
    const activeProfile = ensureActiveDorkaProfile()

    expect(activeProfile.profile.id).toBe(profileId)
    expect(activeProfile.dataFile).toBe(join(profileDirectory, 'dorka-data.json'))
  ...
  it('creates an empty local profile without copying legacy state into it', async () => {
    ...
    const { createLocalDorkaProfile, getDorkaProfileDataFile, getDorkaProfileListState } =
      await loadProfileIndexStore()
    const created = createLocalDorkaProfile({ name: ' Work ' })

    expect(created.profile.name).toBe('Work')
    expect(created.profile.id).toMatch(/^local-/)
    expect(created.activeProfileId).toBe(DEFAULT_LOCAL_DORKA_PROFILE_ID)
    expect(created.profiles.map((profile) => profile.id)).toContain(created.profile.id)
    expect(existsSync(getDorkaProfileDataFile(created.profile.id))).toBe(false)
    expect(getDorkaProfileListState().profiles.map((profile) => profile.id)).toContain(
      created.profile.id
    )
  })

  it('switches the active profile and updates last-opened metadata', async () => {
    const { createLocalDorkaProfile, setActiveDorkaProfile } = await loadProfileIndexStore()
    const created = createLocalDorkaProfile({ name: 'Work' })

    const switched = setActiveDorkaProfile(created.profile.id)
  ...
  it('rejects switching to an unknown profile', async () => {
    const { setActiveDorkaProfile } = await loadProfileIndexStore()

    expect(() => setActiveDorkaProfile('missing-profile')).toThrow('unknown_dorka_profile')
  })
  ...
  posixIt('writes a fresh profile index when umask removes owner-write permission', async () => {
    ...
    const indexPath = store.getDorkaProfileIndexPath()
    const profile = createDefaultLocalDorkaProfile(1)
    const index: DorkaProfileIndex = {
      schemaVersion: DORKA_PROFILE_INDEX_SCHEMA_VERSION,
  ...
  it('recovers a corrupted profile index from the backup copy', async () => {
    const store = await loadProfileIndexStore()
    store.ensureActiveDorkaProfile()
    const created = store.createLocalDorkaProfile({ name: 'Work' })
    // Trigger one more write so the backup captures the two-profile index.
    store.setActiveDorkaProfile(created.profile.id)

    const indexPath = store.getDorkaProfileIndexPath()
    expect(existsSync(`${indexPath}.bak`)).toBe(true)
    writeFileSync(indexPath, '{ not json', 'utf-8')

    const recovered = store.getDorkaProfileListState()
  ...
  it('rejects profile ids that are not safe path segments', async () => {
    ...
    const state = store.ensureActiveDorkaProfile()
    expect(state.profile.id).toBe(DEFAULT_LOCAL_DORKA_PROFILE_ID)
  })
})
