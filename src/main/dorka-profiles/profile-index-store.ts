...
import {
  ...
  type CreateLocalDorkaProfileArgs,
  type CreateLocalDorkaProfileResult,
  type DorkaProfileIndex,
  type DorkaProfileListState,
  type DorkaProfileSummary
} from '../../shared/dorka-profiles'
import {
  getDorkaProfileBrowserSessionMetaFile,
  getDorkaProfileDataFile,
...
export {
  ...
  getDorkaProfileDirectory,
  getDorkaProfileIndexPath,
  getDorkaProfilesDirectory,
  initDorkaProfilePaths
} from './profile-storage-paths'

export type ActiveDorkaProfileState = {
  index: DorkaProfileIndex
...
function isProfileSummary(value: unknown): value is DorkaProfileSummary {
  if (!isObject(value)) {
    return false
  }
...
function normalizeProfileIndex(raw: unknown): DorkaProfileIndex | null {
...
function readProfileIndexFile(indexPath: string): DorkaProfileIndex | null {
  try {
...
export function readProfileIndex(indexPath: string): DorkaProfileIndex | null {
...
export function writeProfileIndex(indexPath: string, index: DorkaProfileIndex): void {
...
function copyLegacyStateToProfile(userDataPath: string, profileId: string): void {
  const profileDataFile = getDorkaProfileDataFile(profileId, userDataPath)
  copyIfPresent(legacyDataFilePath(userDataPath), profileDataFile)
  copyIfPresent(
    legacyBrowserSessionMetaPath(userDataPath),
    getDorkaProfileBrowserSessionMetaFile(profileId, userDataPath)
  )
...
export function seedNewDorkaProfileTelemetryConsent(
  profileId: string,
  ...
  const dataFile = getDorkaProfileDataFile(profileId, userDataPath)
...
function createInitialProfileIndex(now = Date.now()): DorkaProfileIndex {
  const profile = createDefaultLocalDorkaProfile(now)
  return {
    schemaVersion: DORKA_PROFILE_INDEX_SCHEMA_VERSION,
...
export function loadOrCreateProfileIndex(userDataPath: string): DorkaProfileIndex {
  const indexPath = getDorkaProfileIndexPath(userDataPath)
...
function getActiveProfile(index: DorkaProfileIndex): DorkaProfileSummary {
  return (
    index.profiles.find((profile) => profile.id === index.activeProfileId) ??
    index.profiles[0] ??
    createDefaultLocalDorkaProfile(Date.now())
  )
}

export function ensureActiveDorkaProfile(
  userDataPath = getProfileUserDataPath()
): ActiveDorkaProfileState {
  const indexPath = getDorkaProfileIndexPath(userDataPath)
  ...
  const profileDirectory = getDorkaProfileDirectory(activeProfile.id, userDataPath)
  mkdirSync(profileDirectory, { recursive: true })
  if (activeProfile.id === DEFAULT_LOCAL_DORKA_PROFILE_ID) {
  ...
  return {
    ...
    dataFile: getDorkaProfileDataFile(activeProfile.id, userDataPath),
    profileDirectory
  }
}

export function isDefaultLocalDorkaProfileId(profileId: string): boolean {
  return profileId === DEFAULT_LOCAL_DORKA_PROFILE_ID
}

export function getDorkaProfileListState(
  userDataPath = getProfileUserDataPath()
): DorkaProfileListState {
  const { index } = ensureActiveDorkaProfile(userDataPath)
  return {
...
export function createLocalDorkaProfile(
  args: CreateLocalDorkaProfileArgs = {},
  userDataPath = getProfileUserDataPath()
  ...
  const profile: DorkaProfileSummary = {
    ...
    avatar: {
      kind: 'initials',
      initials: (
        name.match(/[A-Za-z0-9]/)?.[0] ?? DEFAULT_LOCAL_DORKA_PROFILE_NAME[0]
      ).toUpperCase(),
      color: 'neutral'
    },
    kind: 'local',
  ...
  const nextIndex: DorkaProfileIndex = {
    ...index,
...
export function setActiveDorkaProfile(
  profileId: string,
  userDataPath = getProfileUserDataPath()
): DorkaProfileListState {
  const index = loadOrCreateProfileIndex(userDataPath)
  ...
  mkdirSync(getDorkaProfileDirectory(profileId, userDataPath), { recursive: true })
  writeProfileIndex(getDorkaProfileIndexPath(userDataPath), nextIndex)
  return {
    activeProfileId: nextIndex.activeProfileId,
    profiles: nextIndex.profiles
