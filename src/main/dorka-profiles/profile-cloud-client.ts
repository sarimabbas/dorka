...
export class DorkaCloudRequestError extends Error {
...
function assertString(value: unknown, field: string): string {
  if (typeof value !== 'string') {
    throw new Error(`invalid_dorka_cloud_${field}`)
  }
...
function normalizeCapabilities(value: unknown): DorkaCloudCapabilities {
...
function normalizeOrganizations(value: unknown): DorkaCloudOrgSummary[] | undefined {
...
function normalizeCloudSummary(value: unknown): DorkaProfileCloudSummary {
  if (!value || typeof value !== 'object') {
    throw new Error('invalid_dorka_cloud_profile')
  }
  const record = value as Record<string, unknown>
  return {
...
function normalizeSessionResponse(value: unknown): DorkaCloudSessionExchangeResponse {
  if (!value || typeof value !== 'object') {
    throw new Error('invalid_dorka_cloud_session')
  }
  const record = value as Record<string, unknown>
  return {
...
export function isAmbiguousCloudRequestFailure(error: unknown): boolean {
  return !(error instanceof DorkaCloudRequestError)
}
...
async function postJson<T>(url: string, body: unknown, options?: PostJsonOptions): Promise<T> {
  ...
  if (!response.ok) {
    ...
    throw new DorkaCloudRequestError(response.status)
  }
  return (await response.json()) as T
}

export async function exchangeDorkaCloudAuthCode(
  config: DorkaCloudAuthConfig,
  args: ExchangeCodeArgs
): Promise<DorkaCloudSessionExchangeResponse> {
...
export async function refreshDorkaCloudCapabilities(
  config: DorkaCloudAuthConfig,
...
export async function refreshDorkaCloudSession(
  config: DorkaCloudAuthConfig,
  session: DorkaCloudSession
): Promise<DorkaCloudSessionExchangeResponse> {
  return normalizeSessionResponse(
...
export async function createDorkaCloudProfile(
  config: DorkaCloudAuthConfig,
  session: DorkaCloudSession,
...
export async function selectDorkaCloudOrg(
  config: DorkaCloudAuthConfig,
  session: DorkaCloudSession,
  orgId: string
...
export async function revokeDorkaCloudSession(
  config: DorkaCloudAuthConfig,
  session: DorkaCloudSession
): Promise<void> {
  await postJson(
