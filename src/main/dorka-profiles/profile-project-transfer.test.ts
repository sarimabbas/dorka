...
function profile(id: string, name: string): DorkaProfileIndex['profiles'][number] {
  return {
    id,
    name,
...
function profileDataPath(profileId: string): string {
  return join(testState.dir, 'profiles', profileId, 'dorka-data.json')
}
...
describe('profile project transfer', () => {
  beforeEach(() => {
    testState.dir = mkdtempSync(join(tmpdir(), 'dorka-profile-transfer-'))
    writeIndex()
  })
  ...
  it('copies a project into another profile with a new repo id and re-keyed metadata', async () => {
    const sourceWorktreeId = 'repo-1::/workspace/dorka-feature'
    writeProfileState(
      'personal',
      makeState({
        repos: [makeRepo()],
        ...
        retiredWorktreeNamesByNamespace: {
          'ssh:ssh-1:/workspace/dorka-nautilus': { exhaustedTiers: 0, names: ['seahorse'] }
        },
        worktreeMeta: {
    ...
    const { transferDorkaProfileProject } = await loadTransferModule()
    const result = transferDorkaProfileProject(
      {
    ...
    const targetWorktreeId = `${targetRepoId}::/workspace/dorka-feature`
    expect(target.repos).toEqual([
      expect.objectContaining({ id: targetRepoId, path: '/workspace/dorka' })
    ])
  ...
  it('moves a project, preserving SSH identity and restorable workspace session state', async () => {
    const sourceWorktreeId = 'repo-ssh::/srv/dorka-feature'
    ...
    writeProfileState(
      ...
      makeState({
        ...
        retiredWorktreeNamesByNamespace: {
          'ssh:ssh-1:posix:/srv/dorka-dorka-retirement-probe': {
            exhaustedTiers: 0,
            names: ['seahorse']
          }
        },
            ...
            [sourceWorktreeId]: [
              {
                ...
                sessionPartition: 'persist:dorka-profile-personal-deadbeef-browser-default',
    ...
    expect(target.retiredWorktreeNamesByNamespace).toEqual({
      'ssh:builder.example.com|22|dev:posix:/srv/dorka-dorka-retirement-probe': {
        exhaustedTiers: 0,
        names: ['seahorse']
      }
    })
  ...
  it('rejects a duplicate physical project inside the target profile', async () => {
    writeProfileState(
      'personal',
      makeState({
        repos: [makeRepo({ path: 'C:\\Work\\Dorka\\' })]
      })
    )
    writeProfileState(
      'work',
      makeState({
        repos: [makeRepo({ id: 'repo-existing', path: 'c:/work/dorka' })]
      })
    )
