...
async function startedFlow(): Promise<{
  authUrl: URL
  flow: ReturnType<typeof beginDorkaCloudPkceFlow>
  nonce: string
  redirectUri: string
  state: string
}> {
  const flow = beginDorkaCloudPkceFlow(config, 'local-default')
...
describe('Dorka cloud PKCE flow', () => {
  beforeEach(() => {
  ...
  it('keeps the loopback listener alive after an invalid callback', async () => {
    ...
    expect(validResponse.body).toContain('<h1>Signed in to Dorka</h1>')
  ...
  it('rejects a provider error that matches the flow state', async () => {
    ...
    expect(response.body).toBe('Dorka sign-in was cancelled.')
    await expect(observedFlow).resolves.toMatchObject({ message: 'dorka_cloud_auth_denied' })
  })
  ...
  it.each(['server_error', 'temporarily_unavailable', 'unknown-error', ''])(
    ...
    async (error) => {
      ...
      expect(response.statusCode).toBe(400)
      expect(response.body).toBe('Dorka sign-in failed. Return to Dorka and try again.')
      await expect(observedFlow).resolves.toMatchObject({
        message: 'dorka_cloud_auth_callback_failed'
      })
    }
  )
  ...
  it('keeps the first loopback alive when a second sign-in starts', async () => {
    const first = beginDorkaCloudPkceFlow(config, 'local-default')
    await vi.waitFor(() => expect(openExternalMock).toHaveBeenCalledTimes(1))
    ...
    const second = beginDorkaCloudPkceFlow(config, 'local-default')
