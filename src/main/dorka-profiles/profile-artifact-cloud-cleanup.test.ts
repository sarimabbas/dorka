...
import { linkDorkaProfileToCloud, unlinkDorkaProfileFromCloud } from './profile-cloud-index'
import {
...
describe('profile artifact cloud cleanup', () => {
  it('preserves recovery state when the profile write fails', async () => {
    ...
    expect(() => linkDorkaProfileToCloud(profileId, cloud('org-b'), userDataPath)).toThrow(
      'profile write failed'
    )
    expect(getArtifactCreateIntent(profileId, userDataPath, '/report.md', scope)).not.toBeNull()

    linkDorkaProfileToCloud(profileId, cloud('org-b'), userDataPath)
  ...
  it('reconciles an interrupted transition before linking again', async () => {
    ...
    expect(() => linkDorkaProfileToCloud(profileId, cloud('org-c'), userDataPath)).toThrow(
      'cleanup failed'
    )
    ...
    linkDorkaProfileToCloud(profileId, cloud('org-c'), userDataPath)
  ...
  it('reconciles an interrupted transition before unlinking', async () => {
    ...
    expect(() => unlinkDorkaProfileFromCloud(profileId, userDataPath)).toThrow('cleanup failed')
    expect(writeProfileIndex).not.toHaveBeenCalled()
    expect(currentCloud(userDataPath)?.activeOrgId).toBe('org-b')
    expect(getArtifactCreateIntent(profileId, userDataPath, '/report.md', scope)).not.toBeNull()
  ...
  it('preserves an orphaned local marker for an unknown profile', async () => {
    ...
    const scope = shareScope('org-a')
    mkdirSync(getDorkaProfileDirectory(orphanProfileId, userDataPath), { recursive: true })
    createIntent(userDataPath, scope, orphanProfileId)
    prepareArtifactCloudCleanup(orphanProfileId, userDataPath, undefined)

    const transitions = [
      () => linkDorkaProfileToCloud(orphanProfileId, cloud('org-b'), userDataPath),
      () => unlinkDorkaProfileFromCloud(orphanProfileId, userDataPath)
    ]
    for (const transition of transitions) {
      expect(transition).toThrow('unknown_dorka_profile')
...
function cloud(activeOrgId: string): DorkaProfileCloudSummary {
  return {
...
async function createLinkedProfile(cloudSummary: DorkaProfileCloudSummary): Promise<string> {
  const userDataPath = await mkdtemp(join(tmpdir(), 'dorka-profile-artifact-cleanup-'))
  createdPaths.push(userDataPath)
  loadOrCreateProfileIndex(userDataPath)
  linkDorkaProfileToCloud(profileId, cloudSummary, userDataPath)
  vi.clearAllMocks()
  return userDataPath
}
...
function currentCloud(userDataPath: string): DorkaProfileCloudSummary | undefined {
  return readProfileIndex(getDorkaProfileIndexPath(userDataPath))?.profiles.find(
