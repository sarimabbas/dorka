...
const savedEnvironment = {
  ...
  DORKA_TEST_SSH_REPOSITORIES: process.env.DORKA_TEST_SSH_REPOSITORIES
}
...
describe('private Git marketplace integration', () => {
  it('uses the caller SSH environment for marketplace preview and install', async () => {
    const root = await mkdtemp(join(tmpdir(), 'dorka-private-marketplace-'))
    ...
    const pluginRepository = await createGitRepository(root, 'locale-source', {
      'dorka-plugin.json': JSON.stringify({
        manifestVersion: 1,
        id: 'private-locale',
        publisher: 'private',
        name: 'Private Locale',
    ...
    const marketplaceRepository = await createGitRepository(root, 'marketplace-source', {
      'dorka-marketplace.json': JSON.stringify({
    ...
    process.env.DORKA_TEST_SSH_REPOSITORIES = JSON.stringify({
