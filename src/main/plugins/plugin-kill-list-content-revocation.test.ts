...
const pluginKey = 'dorka-samples.recipes'
...
async function pluginRoot(): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), 'dorka-plugin-kill-content-'))
  roots.push(root)
  await Promise.all([mkdir(join(root, 'locales')), mkdir(join(root, 'recipes'))])
  await Promise.all([
    writeFile(join(root, 'dorka-plugin.json'), JSON.stringify(contentManifest())),
