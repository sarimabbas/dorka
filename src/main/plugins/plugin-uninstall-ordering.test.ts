...
it('retains a bundled plugin worker and history when ordinary IPC removal is refused', async () => {
  const { root, service, install } = await createUninstallFixture()
  const key = await install('dorka-bundled', {}, true)
  await publishBundledUninstallSuccessor(root, 'dorka-bundled', key)
...
it('checks bundled ownership after an earlier queued refresh before deactivation', async () => {
  const { root, service, install } = await createUninstallFixture()
  const key = await install('dorka-queued', {}, true)
  ...
  try {
    ...
    await publishBundledUninstallSuccessor(root, 'dorka-queued', key)
  } finally {
...
it('rechecks bundled ownership under the filesystem queue after a held worker shutdown', async () => {
  const { root, service, install } = await createUninstallFixture()
  const key = await install('dorka-deactivate', {}, true)
  ...
  try {
    ...
    await publishBundledUninstallSuccessor(root, 'dorka-deactivate', key)
  } finally {
    release.resolve()
  }
  expect(await failed).toEqual(
