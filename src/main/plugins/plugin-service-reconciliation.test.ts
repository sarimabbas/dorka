...
async function pluginRoot(pluginManifest = manifest()): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), 'dorka-plugin-reconcile-'))
  roots.push(root)
  await writeFile(join(root, 'dorka-plugin.json'), JSON.stringify(pluginManifest))
  ...
  const service = new PluginService({
    ...
    getPluginKillListEntry: (key) =>
      killed && key === pluginKey
        ? { pluginKey, reason: 'Security incident', advisoryUrl: 'https://dorka.example/advisory' }
        : null,
...
describe('PluginService worker reconciliation', () => {
  it('blocks every runtime surface until a saved override resolves a content conflict', async () => {
    ...
    const service = new PluginService({
      ...
      getDisabledPlugins: () => [],
      getPluginConsents: () => ({
        'dorka-samples.first': fingerprintPluginConsent(firstManifest, firstHash.hash),
        'dorka-samples.second': fingerprintPluginConsent(secondManifest, secondHash.hash)
      }),
    ...
    expect(service.activationError('dorka-samples.first')).toContain('conflicts')
    expect(service.getGrantedCapabilities('dorka-samples.first')).toBeNull()
    await expect(service.invokeCommand('dorka-samples.first', 'run')).rejects.toThrow('not enabled')
    await expect(service.panels.readEntry('dorka-samples.first', 'panel')).resolves.toBeNull()
    ...
    keybindings = { 'plugin:dorka-samples.first/run': ['Mod+Shift+T'] }
    await service.reconcileActivationState()

    expect(service.activationError('dorka-samples.first')).toBeNull()
    await expect(service.panels.readEntry('dorka-samples.first', 'panel')).resolves.toMatchObject({
      html: expect.stringContaining('<h1>Panel</h1>')
    })
    await expect(service.invokeCommand('dorka-samples.first', 'run')).resolves.toBeNull()
