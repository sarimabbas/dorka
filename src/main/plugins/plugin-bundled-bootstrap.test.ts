...
async function writeBundle(root: string, name = 'Skills'): Promise<{ path: string; hash: string }> {
  const path = 'stablyai.dorka-skills'
  const pluginRoot = join(root, path)
  await mkdir(pluginRoot, { recursive: true })
  await writeFile(
    join(pluginRoot, 'dorka-plugin.json'),
    JSON.stringify({
      manifestVersion: 1,
      id: 'dorka-skills',
      publisher: 'stablyai',
...
async function writeIndex(root: string, path: string, contentHash: string): Promise<void> {
  await writeFile(
    ...
    JSON.stringify({
      version: 1,
      plugins: [{ pluginKey: 'stablyai.dorka-skills', path, contentHash }]
    })
  )
}
...
describe('bundled plugin bootstrap', () => {
  it('installs release-indexed content once and keeps unchanged startup work bounded', async () => {
    const root = await tempRoot('dorka-bundled-resources-')
    const userDataPath = await tempRoot('dorka-bundled-user-data-')
    ...
    ).resolves.toEqual({ installed: ['stablyai.dorka-skills'], unchanged: [], errors: [] })
    await expect(
      bootstrapBundledPlugins({ root, userDataPath, hostVersion: '1.4.0' })
    ).resolves.toEqual({ installed: [], unchanged: ['stablyai.dorka-skills'], errors: [] })
  })
  ...
  it('publishes an updated immutable bundle only when the indexed hash matches', async () => {
    ...
    expect(updated).toEqual({ installed: ['stablyai.dorka-skills'], unchanged: [], errors: [] })
    const lock = await readPluginLockfile(join(userDataPath, 'plugins'))
    expect(lock.plugins['stablyai.dorka-skills']?.contentHash).toBe(second.hash)
  })
  ...
  it('repairs a missing or modified bundled current version', async () => {
    ...
    await writeIndex(root, bundle.path, bundle.hash)
    await bootstrapBundledPlugins({ root, userDataPath, hostVersion: '1.4.0' })
    const versionDir = join(userDataPath, 'plugins', 'stablyai.dorka-skills', bundle.hash)
    await writeFile(join(versionDir, 'dorka-plugin.json'), '{}')
