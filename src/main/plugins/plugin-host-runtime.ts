...
/** API surface handed to a plugin's `activate(dorka)` export. Everything is
 *  EXPERIMENTAL until pluginApi v1 freezes. */
export type PluginWorkerDorkaApi = {
...
export function createPluginWorkerRuntime(
  ...
  async function handleInit(input: {
    ...
    const dorka: PluginWorkerDorkaApi = {
      commands: {
