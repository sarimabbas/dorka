...
// Why: Gemini has no permission-prompt hook (approvals are inline UI), so Dorka can't show a waiting state — upstream limitation.
...
function getManagedScript(target: 'local' | 'posix' = 'local'): string {
  ...
  return [
    ...
    // Why: source refreshes endpoint coords so a PTY surviving an Dorka restart keeps reporting. See claude/hook-service.ts.
    ...
    'printf \'%s\' "$payload" | curl -sS -X POST "http://127.0.0.1:${DORKA_AGENT_HOOK_PORT}/hook/gemini" \\',
...
export class GeminiHookService {
  ...
  async installRemote(sftp: SFTPWrapper, remoteHome: string): Promise<AgentHookInstallStatus> {
    ...
    const remoteScriptPath = `${remoteHome.replace(/\/$/, '')}/.dorka/agent-hooks/gemini-hook.sh`
    try {
      ...
      // Why: SSH remotes always use POSIX `.sh` paths even when Dorka runs on Windows.
