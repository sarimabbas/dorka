...
function getAuthConfig(): AzureDevOpsCreateAuthConfig {
  return {
    pat: envValue('DORKA_AZURE_DEVOPS_TOKEN') ?? envValue('DORKA_AZURE_DEVOPS_PAT'),
    accessToken: envValue('DORKA_AZURE_DEVOPS_ACCESS_TOKEN'),
    username: envValue('DORKA_AZURE_DEVOPS_USERNAME')
  }
}
...
function classifyCreateError(error: unknown): CreateHostedReviewResult {
  ...
  if (
    ...
    return {
      ...
      code: 'auth_required',
      error:
        'Create PR failed: Azure DevOps is not authenticated. Next step: set DORKA_AZURE_DEVOPS_TOKEN in this environment.'
    }
  }
