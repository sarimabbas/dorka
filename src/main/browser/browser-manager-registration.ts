...
export abstract class BrowserManagerRegistration extends BrowserManagerGuestPolicy {
  ...
  // Why: headless dorka serve has no <webview> window; back pages with offscreen WebContents and skip the webview-only setup.
  ...
  unregisterAll(): void {
    ...
    for (const downloadId of this.downloadsById.keys()) {
      this.cancelDownloadInternal(downloadId, 'Dorka is shutting down.')
    }
