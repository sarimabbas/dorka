...
const HIDE_BROWSER_GRAB_OVERLAY_SCRIPT = `(function(){
  var g = window.__dorkaGrab;
  if (g && g.host) g.host.style.display = 'none';
  document.querySelectorAll('[data-dorka-browser-annotation-overlay]').forEach(function(el) {
    el.setAttribute('data-dorka-previous-display', el.style.display || '');
...
const RESTORE_BROWSER_GRAB_OVERLAY_SCRIPT = `(function(){
  ...
  document.querySelectorAll('[data-dorka-browser-annotation-overlay]').forEach(function(el) {
    el.style.display = el.getAttribute('data-dorka-previous-display') || '';
    el.removeAttribute('data-dorka-previous-display');
  });
})()`
