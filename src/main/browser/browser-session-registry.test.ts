...
describe('BrowserSessionRegistry', () => {
  ...
  it('has a default profile on construction', () => {
    ...
    expect(defaultProfile.partition).toBe(DORKA_BROWSER_PARTITION)
  })

  it('allows the default partition', () => {
    expect(browserSessionRegistry.isAllowedPartition(DORKA_BROWSER_PARTITION)).toBe(true)
  })
  ...
  it('creates an isolated profile with a unique partition', async () => {
    ...
    expect(profile!.partition).toMatch(/^persist:dorka-browser-session-/)
    expect(profile!.partition).not.toBe(DORKA_BROWSER_PARTITION)
  ...
  it('resolves default partition for null/undefined profileId', () => {
    expect(browserSessionRegistry.resolvePartition(null)).toBe(DORKA_BROWSER_PARTITION)
    expect(browserSessionRegistry.resolvePartition(undefined)).toBe(DORKA_BROWSER_PARTITION)
  })

  it('resolves default partition for unknown profileId', () => {
    expect(browserSessionRegistry.resolvePartition('nonexistent')).toBe(DORKA_BROWSER_PARTITION)
  })
  ...
  it('strictly resolves known profile partitions without downgrading unknown profiles', async () => {
    ...
    expect(browserSessionRegistry.resolveKnownPartition(null)).toBe(DORKA_BROWSER_PARTITION)
    expect(browserSessionRegistry.resolveKnownPartition(undefined)).toBe(DORKA_BROWSER_PARTITION)
    expect(browserSessionRegistry.resolveKnownPartition('default')).toBe(DORKA_BROWSER_PARTITION)
  ...
  it('hydrates profiles from persisted data', () => {
    const fakeProfile = {
      ...
      partition: 'persist:dorka-browser-session-00000000-0000-0000-0000-000000000001',
      label: 'Hydrated',
  ...
  it('rejects a persisted profile whose partition belongs to a different profile id', () => {
    ...
    const claimedPartition = 'persist:dorka-browser-session-00000000-0000-4000-8000-000000000022'
  ...
  it('applies and clears existing browser-profile policy on an opaque route partition', async () => {
    const partition =
      'persist:dorka-browser-v1-1111111111111111222222222222222233333333333333334444444444444444'
    setBrowserNetworkProxySettingsResolver(() => ({
  ...
  it('rejects route partitions for missing browser profiles', () => {
    const partition =
      'persist:dorka-browser-v1-aaaaaaaaaaaaaaaabbbbbbbbbbbbbbbbccccccccccccccccdddddddddddddddd'
  ...
  it('routes media permission requests through macOS TCC for isolated partitions', async () => {
    ...
    // after the user grants Camera/Microphone to Dorka.
  ...
  it('uses profile-owned partitions for non-default Dorka profiles', async () => {
    const dorkaProfileId = 'local-work'
    ...
    expect(browserSessionRegistry.getDefaultProfile().partition).toBe(
      getDorkaProfileBrowserDefaultPartition(dorkaProfileId)
    )
    expect(browserSessionRegistry.isAllowedPartition(DORKA_BROWSER_PARTITION)).toBe(false)

    const profile = await browserSessionRegistry.createProfile('isolated', 'Work Browser')
    expect(profile).not.toBeNull()
    expect(profile!.partition).toBe(
      getDorkaProfileBrowserSessionPartition(dorkaProfileId, profile!.id)
    )

    browserSessionRegistry.configureForDorkaProfile({
      dorkaProfileId: DEFAULT_LOCAL_DORKA_PROFILE_ID,
