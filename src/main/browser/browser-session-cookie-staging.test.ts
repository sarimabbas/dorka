...
const defaultPartition = 'persist:dorka-browser-default'
const routePartition = `persist:dorka-browser-v1-${'c'.repeat(64)}`
let userData = ''
...
beforeEach(() => {
  ...
  userData = realpathSync(mkdtempSync(join(tmpdir(), 'dorka-cookie-staging-')))
...
describe('supportsPendingBrowserCookieImportReplay', () => {
  it('refuses a client-hosted route partition and allows profile partitions', () => {
    ...
    expect(supportsPendingBrowserCookieImportReplay('persist:dorka-browser-profile-a')).toBe(true)
  })
})
...
describe('applyPendingBrowserCookieImports', () => {
  it('unlinks the plaintext staged database when the partition is not replayable', () => {
    ...
    applyPendingBrowserCookieImports({
      ...
      defaultPartition,
      activeDorkaProfileId: 'profile-a'
    })
