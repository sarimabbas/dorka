...
/** Session-name namespace Dorka gives one daemon per browser tab. */
export const DORKA_TAB_SESSION_PREFIX = 'dorka-tab-'
...
/**
 * Close agent-browser daemons left behind by a previous Dorka run.
 *
 ...
 *
 * Scoping — this only runs when Dorka derived the socket directory itself
 * (`ownsSocketDirectory`), because that private per-profile directory is what
 * proves the enumeration can only see this Dorka profile's daemons. An inherited
 * `AGENT_BROWSER_SOCKET_DIR` can be shared with a second Dorka profile, and
 * Windows gets none at all (named pipes make the directory moot); both cases
 * skip the sweep rather than run a `session list` that could close a daemon Dorka
 * does not own, and stay bounded by `AGENT_BROWSER_IDLE_TIMEOUT_MS` instead.
 *
 * `DORKA_DISABLE_AGENT_BROWSER_SWEEP=1` turns it off in the field. The other two
...
export async function sweepOrphanedAgentBrowserSessions(options: {
  ...
  if (!options.ownsSocketDirectory || process.env.DORKA_DISABLE_AGENT_BROWSER_SWEEP === '1') {
    return []
  }
  let listed: string[]
  try {
  ...
  for (const sessionName of listed) {
    if (!sessionName.startsWith(DORKA_TAB_SESSION_PREFIX) || options.isSessionLive?.(sessionName)) {
      continue
    }
    try {
      await runProcess({
