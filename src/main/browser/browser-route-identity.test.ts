...
describe('browser route partition identity', () => {
  it('derives a stable opaque cross-platform partition and binding fingerprint', () => {
    ...
    expect(first.partition).toMatch(/^persist:dorka-browser-v1-[a-f0-9]{64}$/)
  ...
  it('makes every identity component load-bearing', () => {
    const derived = [
      identity,
      { ...identity, dorkaProfileId: 'dorka/profile:beta' },
  ...
  it('pins the derived partition and fingerprint against silent relocation', () => {
    expect(deriveBrowserRoutePartition(pinnedIdentity)).toEqual({
      partition:
        'persist:dorka-browser-v1-955a5db671b210d053d64d1e557d8cdf1e60e1e6cb710a033f1b5cd1b61e6586',
  ...
  it('scopes partition ownership to one dorka profile and one environment', () => {
    ...
    expect(
      deriveBrowserRoutePartitionStorageScope({
        dorkaProfileId: 'dorka/profile:beta',
        environmentId: 'environment-a'
      })
    ).not.toBe(scope)
  })
