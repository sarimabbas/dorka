...
describe('agent-browser process environment', () => {
  it('bounds Unix socket paths independently of a long profile path', () => {
    ...
    expect(socketDirectory).toMatch(/^\/tmp\/dorka-ab-[0-9a-f]{16}$/)
    expect(
      `${socketDirectory}/dorka-tab-00000000-0000-4000-8000-000000000000.sock`.length
    ).toBeLessThan(104)
  })

  // Why ownsSocketDirectory is false for both: a directory Dorka did not derive can be shared with
  // another Dorka profile, so `session list` under it is no proof of ownership.
  ...
  // Why: the only daemon bound that survives a SIGKILL'd Dorka, so it must be set on every platform.
