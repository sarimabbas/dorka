...
vi.mock('electron', () => ({ app: { getPath: () => '/tmp/dorka-test-user-data' } }))
vi.mock('../../shared/runtime-environment-store', () => ({
  listEnvironments: () => mocks.environments
}))
vi.mock('./browser-route-partition-binding-runtime', () => ({
  activeBrowserRoutePartitionDorkaProfileId: () => 'local-default'
}))
vi.mock('./browser-route-partition-storage-dependencies', () => ({
  browserRoutePartitionStorageDependencies: (isPartitionLive: (p: string) => boolean) => ({
    ...
    partitionDataRoot: '/tmp/dorka-test-partitions',
    isPartitionLive,
...
const ENV_PARTITION = `persist:dorka-browser-v1-${'a'.repeat(64)}`
const LIVE_TARGET_PARTITION = `persist:dorka-browser-v1-${'b'.repeat(64)}`
const REMOVED_TARGET_PARTITION = `persist:dorka-browser-v1-${'c'.repeat(64)}`
