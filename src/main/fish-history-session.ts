 ...
 * Dorka's history tree. That is why deletion needs its own path here instead of
 ...
 */
const SESSION_PREFIX = 'dorka_'
const RELAY_SESSION_PREFIX = 'dorka_relay_'
const SAFE_SESSION_NAME = /^dorka_(?:relay_)?[0-9a-f]{1,64}$/
/** Deliberately does NOT match the relay prefix. A relay host keyed by its
 *  CLIENT's worktree ids shares the one fish data dir with any desktop Dorka on
...
const DORKA_HISTORY_FILE = /^dorka_([0-9a-f]{1,64})_history$/
...
/** Drop a `fish_history` this process inherited from an outer Dorka.
 *  fish EXPORTS the variable, so an Dorka launched from a fish pane hands the
 *  LAUNCHING worktree's session name to every pane of the nested app, which then
 *  writes into that worktree's history file (STA-4682). Only Dorka-minted names
...
export function dropInheritedDorkaFishHistory(env: Record<string, string | undefined>): void {
 ...
 *  Resolve this from the PTY's SPAWN env: Dorka can be launched with a different
 ...
 *  the two disagree whenever Dorka's environment differs from the PTY's. */
...
/**
 * Delete every Dorka-minted fish history file whose worktree is gone.
 *
 ...
 * scan of the data dir can find those. Matching on Dorka's own `dorka_<hex>_`
...
export function sweepOrphanedFishHistoryFiles(
  ...
  for (const dir of new Set(dirs)) {
    ...
    for (const entry of entries) {
      const hash = DORKA_HISTORY_FILE.exec(entry)?.[1]
