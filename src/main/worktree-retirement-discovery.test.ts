...
describe('extractBucketLeafCandidates', () => {
  it('takes everything past the encoded parent as the leaf', () => {
    expect(extractBucketLeafCandidates(`-w-dorka-${FIRST}`, ['-w-dorka'])).toEqual([FIRST])
  })
  ...
  it('offers the first segment too, so an agent run in a subdirectory still retires the leaf', () => {
    expect(extractBucketLeafCandidates(`-w-dorka-${FIRST}-packages-api`, ['-w-dorka'])).toEqual([
      `${FIRST}-packages-api`,
      FIRST
    ])
  })
  ...
  it('rejects a sibling directory that shares the parent prefix', () => {
    expect(extractBucketLeafCandidates(`-w-dorkadyne-${FIRST}`, ['-w-dorka'])).toEqual([])
    expect(extractBucketLeafCandidates(`-w-dorka-secret-${FIRST}`, ['-w-dorka-fix'])).toEqual([])
  })

  it('yields nothing for the parent bucket itself', () => {
    expect(extractBucketLeafCandidates('-w-dorka', ['-w-dorka'])).toEqual([])
  })
})
...
describe('discoverRetiredWorktreeNames', () => {
  ...
  async function withFakeHome(
    ...
    const home = await mkdtemp(join(tmpdir(), 'dorka-retirement-home-'))
    try {
  ...
  it('calls a machine with no agent state a complete answer, not a hole to rescan forever', async () => {
    ...
    const retired = await discoverRetiredWorktreeNames({
      workspaceRoots: [join(tmpdir(), 'dorka-retirement-absent-root')],
      home: join(tmpdir(), 'dorka-retirement-absent-home'),
      env: {}
    })
  ...
  it('matches a plain POSIX workspace root', async () => {
    await withFakeHome([`-Users-ada-dorka-workspaces-dorka-${FIRST}`], async (home) => {
      const retired = await discoverRetiredWorktreeNames({
  ...
  it('matches a dot-directory root, where the separator run encodes to two dashes', async () => {
    await withFakeHome([`-Users-ada--dorka-worktrees-${FIRST}`], async (home) => {
      const retired = await discoverRetiredWorktreeNames({
        workspaceRoots: ['/Users/ada/.dorka/worktrees'],
        home,
        env: {}
      })
      expect(retired.names).toEqual(new Set([FIRST]))
  ...
  it('matches a Windows drive root', async () => {
    // `getDefaultWorkspaceDir` returns `C:\Users\<user>\dorka\workspaces` on Windows, so an encoder
    // that collapsed `:\` rejected every bucket on that platform by default.
    await withFakeHome([`C--Users-ada-dorka-workspaces-${FIRST}`], async (home) => {
      const retired = await discoverRetiredWorktreeNames({
        workspaceRoots: ['C:\\Users\\ada\\dorka\\workspaces'],
        home,
        env: {}
      })
      expect(retired.names).toEqual(new Set([FIRST]))
  ...
  it('matches a WSL UNC root', async () => {
    await withFakeHome([`--wsl--Ubuntu-home-ada-dorka-workspaces-${FIRST}`], async (home) => {
      const retired = await discoverRetiredWorktreeNames({
        workspaceRoots: ['\\\\wsl$\\Ubuntu\\home\\ada\\dorka\\workspaces'],
        home,
        env: {},
  ...
  it('reads buckets from CLAUDE_CONFIG_DIR when it is set', async () => {
    const configDir = await mkdtemp(join(tmpdir(), 'dorka-retirement-config-'))
  ...
  it('still reads the Windows-side home for a WSL root when the distro home cannot be resolved', async () => {
    ...
    await withFakeHome(
      [`--wsl-localhost-Ubuntu-home-ada-dorka-workspaces-${FIRST}`],
      async (home) => {
        const retired = await discoverRetiredWorktreeNames({
          workspaceRoots: ['\\\\wsl.localhost\\Ubuntu\\home\\ada\\dorka\\workspaces'],
          home,
  ...
  it('retires live workspace directories alongside surviving buckets', async () => {
    const root = await mkdtemp(join(tmpdir(), 'dorka-retirement-roots-'))
    await withFakeHome([], async (home) => {
      try {
