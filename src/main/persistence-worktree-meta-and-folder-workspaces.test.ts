...
describe('Store', () => {
  ...
  it('discards malformed persisted task-source metadata without aborting store load', async () => {
    ...
    expect(store.getWorktreeMeta('wt-malformed')?.linkedWorkItem?.jiraIdentifier).toBe('DORKA-123')
  ...
  it('drops corrupt worktreeMeta entries while still normalizing valid siblings', async () => {
    ...
    expect(store.getWorktreeMeta('wt-sibling')?.linkedWorkItem?.jiraIdentifier).toBe('DORKA-123')
