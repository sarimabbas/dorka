...
beforeEach(() => {
  ...
  fakeHomeDir = mkdtempSync(join(tmpdir(), 'dorka-codex-resource-home-'))
  userDataDir = mkdtempSync(join(tmpdir(), 'dorka-codex-resource-user-data-'))
...
describe('syncSystemCodexResourcesIntoManagedHome', () => {
  it('uses DORKA_USER_DATA_PATH when Electron cannot be required', async () => {
    vi.resetModules()
    vi.doMock('electron', () => {
      throw new Error('electron unavailable in packaged CLI')
    })
    const previousUserDataPath = process.env.DORKA_USER_DATA_PATH
    process.env.DORKA_USER_DATA_PATH = userDataDir
    try {
      const { getDorkaManagedCodexHomePath: getCliSafeManagedPath } =
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      mkdirSync(join(managedHomePath, '.dorka-resource-copies', 'AGENTS.md.json'), {
        recursive: true
      })
  ...
  it('removes an unowned copy when recording copy ownership fails', () => {
    ...
    mkdirSync(managedHomePath, { recursive: true })
    writeFileSync(join(managedHomePath, '.dorka-resource-copies'), 'blocks marker directory\n')
