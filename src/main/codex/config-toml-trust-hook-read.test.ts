...
describe('readHookTrustEntries', () => {
  ...
  it('recognizes and removes a first trust block after a leading BOM', () => {
    ...
    expect(readHookTrustEntriesFromContent(content).get(key)?.trustedHash).toBe('sha256:DORKA')
  ...
  it('supports case-insensitive lookups for Windows hook trust keys read from config', () => {
    ...
    const rawKey = 'C:\\Users\\rod\\AppData\\Roaming\\dorka\\hooks.json:session_start:0:0'
    const lookupKey = 'C:/Users/Rod/AppData/Roaming/dorka/hooks.json:session_start:0:0'
    const original = [
      `[hooks.state.'${rawKey}']`,
      'enabled = true',
