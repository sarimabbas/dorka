...
export async function refreshCodexRuntimeUserHooksExclusively(
  ...
  // Why: same as install() — capture in-Dorka approvals before this refresh
  ...
  try {
    ...
    // Why: this path is used when Dorka status hooks are disabled. The
    // runtime CODEX_HOME should keep user hooks, but not Dorka-managed trust.
