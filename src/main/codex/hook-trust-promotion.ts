...
function getProvenancePath(runtimeHomePath: string): string {
  return join(runtimeHomePath, '.dorka-hook-trust-provenance.json')
}
...
/**
 * Records the runtime config.toml trust state Dorka leaves behind after an
 * install/refresh, so the next launch can tell "entry Dorka wrote" apart from
 ...
 * `config.toml` trust entries Dorka wrote apart from which the user approved
 * inside Codex. Overwriting it from the current config state after a failed
 * read stamps the user's approval as Dorka-written, and promotion then skips it
...
export function snapshotCodexRuntimeHookTrustProvenance(
  runtimeHomePath: string = getDorkaManagedCodexHomePath()
): void {
...
/**
 * Promotes hook approvals the user made inside Dorka-launched Codex (written
...
function promoteCodexRuntimeHookApprovalsToSystemUnsafe(runtimeHomePath: string): void {
  ...
  // Why: without a snapshot of what Dorka last wrote (first launch after
  ...
  // in-Dorka approval. Promoting would resurrect trust the user revoked in
  ...
  for (const [key, state] of runtimeTrust) {
    ...
    if (
      ...
      // Dorka wrote this entry and nothing touched it since — not an approval.
      continue
    }
    ...
    const hook = Array.isArray(definition?.hooks)
      ...
      : undefined
    // Why: never write trust for Dorka's managed status hook into the user's
    // real config — mutating ~/.codex for Dorka's own hooks is exactly what the
