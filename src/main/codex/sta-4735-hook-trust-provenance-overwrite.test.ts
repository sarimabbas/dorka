...
// config.toml trust entries Dorka wrote versus which the user approved inside
// Codex. An unreadable one was rebuilt from the current config on the same
// pass, stamping the user's approval as Dorka-written — after which promotion
// skips it forever.
...
const PROVENANCE_ENTRY = 'dorka-hooks:stop:0:0'
let fakeHomeDir: string
let userDataDir: string
let runtimeHomePath: string

const provenancePath = (): string => join(runtimeHomePath, '.dorka-hook-trust-provenance.json')

function seedRecordedProvenance(): string {
  const contents = `${JSON.stringify(
    { version: 1, entries: { [PROVENANCE_ENTRY]: { trustedHash: 'sha256:dorka', enabled: true } } },
    null,
    2
  )}\n`
...
beforeEach(() => {
  denials.reset()
  fakeHomeDir = realFs.mkdtempSync(join(tmpdir(), 'dorka-sta4735-home-'))
  userDataDir = realFs.mkdtempSync(join(tmpdir(), 'dorka-sta4735-data-'))
...
describe('STA-4735 snapshotCodexRuntimeHookTrustProvenance', () => {
  it('leaves a provenance record it could not read exactly as it found it', () => {
    ...
    // holds no record of what Dorka wrote — so a user approval made since the
    // last pass was permanently reclassified as Dorka's own write.
