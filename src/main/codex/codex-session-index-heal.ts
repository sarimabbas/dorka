...
// stamped `complete` on first app-server startup), so rollouts that Dorka's
...
// Codex's DB-driven surfaces. Dorka never writes Codex's sqlite schema itself.
...
function buildNativeHealInvocation(
  ...
  return {
    ...
    args: spawnArgs,
    cliPath: command,
    // Why: pin the real home explicitly — nested Dorka launches can inherit a
