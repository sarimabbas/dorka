...
beforeEach(() => {
  userDataDir = mkdtempSync(join(tmpdir(), 'dorka-trust-ledger-'))
...
describe('codex trust grant ledger', () => {
  ...
  it('treats Windows path-case variants as the same home', () => {
    const home = 'C:\\Users\\Alice\\AppData\\Roaming\\dorka\\codex-runtime-home\\home'
    writeCodexTrustGrantLedgerHome(home, { binary: null, entries: {} })
    expect(
      readCodexTrustGrantLedgerHome('c:/users/alice/appdata/roaming/dorka/codex-runtime-home/home')
    ).not.toBeNull()
  })
