...
// Why: Pre/PostToolUse feed the live in-flight-tool readout; PermissionRequest exits with no decision so Codex still shows its approval UI while Dorka flips the pane to waiting.
...
export function getConfigPath(runtimeHomePath: string = getDorkaManagedCodexHomePath()): string {
...
export function writeCodexHooksJson(
  ...
  // Why: Codex rejects unknown top-level hooks.json fields, so plugin bookkeeping like `_managed` must not survive Dorka's rewrite.
