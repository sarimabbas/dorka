...
export function hasCustomCodexHomeOverride(env: NodeJS.ProcessEnv = process.env): boolean {
  ...
  const normalizedDorkaCodexHome = dorkaCodexHome
    ? normalizePathForComparison(dorkaCodexHome)
    : undefined
  // Why: phase 1 owns only ~/.codex and can clean that path on downgrade. A
  // custom home needs cross-home ownership tracking before Dorka may mutate it.
  return Boolean(
    normalizedCodexHome &&
    normalizedCodexHome !== normalizedDorkaCodexHome &&
...
export function getCustomCodexHomeOverrideForLaunch(
  ...
  const effectiveEnv = launchEnv
    ? {
        ...
        DORKA_CODEX_HOME: getLaunchEnvValue(launchEnv, 'DORKA_CODEX_HOME')
      }
    : process.env
...
function getLaunchEnvValue(
  launchEnv: NodeJS.ProcessEnv,
  key: 'CODEX_HOME' | 'DORKA_CODEX_HOME' | 'HOME' | 'SHELL' | 'XDG_CONFIG_HOME'
): string | undefined {
