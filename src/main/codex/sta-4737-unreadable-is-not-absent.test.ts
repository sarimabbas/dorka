...
beforeEach(() => {
  denials.reset()
  fakeHomeDir = realFs.mkdtempSync(join(tmpdir(), 'dorka-sta4737-home-'))
  userDataDir = realFs.mkdtempSync(join(tmpdir(), 'dorka-sta4737-data-'))
...
describe('STA-4737 promotion must not rebuild a system config it could not read', () => {
  function seedPromotableRuntimeChange(): void {
    ...
    // The baseline records what Dorka last mirrored; the later edit then reads as
  ...
  it('leaves a denied system config exactly as it found it', () => {
    ...
    // by a reconstruction built from Dorka's runtime copy.
...
describe('STA-4737 the resource sync must not delete a mirror whose source it could not read', () => {
  ...
  const MIRRORED = '# instructions Dorka copied for the distro\n'
  ...
  function seedOwnedMirrorCopy(): { sourcePath: string; targetPath: string } {
    ...
    // Dorka owns this copy, which is what entitles the sync to remove it.
  ...
  it('still removes the mirrored copy when the source is genuinely gone', () => {
    ...
    // Why: removing Dorka's own copy of a resource the user deleted is the
