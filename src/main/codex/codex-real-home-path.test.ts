...
describe('hasCustomCodexHomeOverride', () => {
  ...
  it('preserves a genuinely custom CODEX_HOME', () => {
    expect(
      hasCustomCodexHomeOverride({
        ...
        DORKA_CODEX_HOME: `${process.cwd()}${sep}codex-runtime-home${sep}home`
      })
    ).toBe(true)
  })
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      const paneHome = mkdtempSync(join(tmpdir(), 'dorka-codex-pane-home-'))
  ...
  // a Dock-launched Dorka, so the launch env is the only place it appears. Reading the
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      const paneHome = mkdtempSync(join(tmpdir(), 'dorka-codex-fish-home-'))
      temporaryHomes.push(paneHome)
