  ...
  switch (request.method) {
    ...
    case CODEX_USER_INPUT_METHOD:
      connection.respondWithError(
        request.id,
        -32001,
        `Dorka could not model ${request.method} as a durable prompt`
      )
      break
    ...
    case CODEX_AUTH_TOKEN_REFRESH_METHOD:
      connection.respondWithError(request.id, -32001, 'Dorka cannot refresh app-server auth tokens')
      break
    case CODEX_ATTESTATION_METHOD:
      connection.respondWithError(request.id, -32001, 'Dorka did not negotiate attestation')
      break
    default:
      connection.respondWithError(
        request.id,
        -32000,
        `Dorka rejected unrecognized blocking request ${request.method}`
      )
  }
