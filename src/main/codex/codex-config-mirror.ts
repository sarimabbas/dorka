...
export function syncSystemConfigIntoManagedCodexHome(
  ...
  // user made inside Dorka-launched Codex (/model, /approvals) must be written
  ...
  // unpromoted runtime change as Dorka-written would strand it forever.
  snapshotCodexRuntimeSettingsBaseline(homes.runtimeHomePath, {
    conflicts: new Map(
...
function mergeSystemCodexConfigIntoRuntime(runtimeConfig: string, systemConfig: string): string {
  ...
  // trust and project trust are written under Dorka's managed CODEX_HOME and
