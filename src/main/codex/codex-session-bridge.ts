...
export function syncSystemCodexSessionsIntoManagedHome(sourceCodexHomePath?: string): void {
  ...
  const managedSessionsRoot = join(getDorkaManagedCodexHomePath(), 'sessions')
...
function replaceSymlinkSessionBridgeWithHardlink(
  ...
  try {
    ...
    replacementPath = `${targetPath}.dorka-link-${process.pid}-${Date.now()}`
...
function getLegacySessionCopyMarkerPath(relativePath: string): string {
  return join(getDorkaManagedCodexHomePath(), '.dorka-session-copies', `${relativePath}.json`)
}
