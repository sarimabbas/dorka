...
export function getManagedScript(target: 'local' | 'posix' = 'local'): string {
  ...
  return [
    ...
    // Why: sourcing refreshes PORT/TOKEN/ENV/VERSION from the current Dorka so a surviving PTY keeps reporting after a restart (see claude/hook-service.ts).
    'load_hook_endpoint() {',
    '  endpoint_path="$1"',
    '  unset DORKA_AGENT_HOOK_TRANSPORT',
    '  case "$endpoint_path" in',
    '    *.cmd)',
    // Why: Windows passes endpoint.cmd into WSL via WSLENV; parse only Dorka's known assignments since cmd.exe `set` lines aren't shell syntax.
    ...
    '          "set DORKA_AGENT_HOOK_PORT="*) DORKA_AGENT_HOOK_PORT=${endpoint_line#*=} ;;',
    '          "set DORKA_AGENT_HOOK_TOKEN="*) DORKA_AGENT_HOOK_TOKEN=${endpoint_line#*=} ;;',
    '          "set DORKA_AGENT_HOOK_ENV="*) DORKA_AGENT_HOOK_ENV=${endpoint_line#*=} ;;',
    '          "set DORKA_AGENT_HOOK_VERSION="*) DORKA_AGENT_HOOK_VERSION=${endpoint_line#*=} ;;',
    '          "set DORKA_AGENT_HOOK_TRANSPORT="*) DORKA_AGENT_HOOK_TRANSPORT=${endpoint_line#*=} ;;',
    '        esac',
    '      done < "$endpoint_path"',
    '      ;;',
    '    *)',
    ...
    '  load_hook_endpoint "$DORKA_AGENT_HOOK_ENDPOINT"',
    'fi',
