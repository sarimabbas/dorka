...
describe('upsertHookTrustEntries', () => {
  ...
  it('finds and replaces a legacy forward-slash block when Dorka upserts with native backslash key', () => {
    // Why: Codex 0.140 exposes Windows keys with either separator depending on cwd, so replace both.
    const backslashPath = 'C:\\Users\\Rod\\AppData\\Roaming\\dorka\\hooks.json'
  ...
  it('produces exactly one Windows separator pair after two consecutive upserts', () => {
    ...
    const entry: CodexTrustEntry = {
      sourcePath: 'C:\\Users\\Rod\\AppData\\Roaming\\dorka\\hooks.json',
      eventLabel: 'session_start',
      groupIndex: 0,
      handlerIndex: 0,
      command: 'echo session'
  ...
  it('falls back to TOML basic-string headers when a Windows path contains an apostrophe', () => {
    ...
    const entry: CodexTrustEntry = {
      sourcePath: "C:\\Users\\O'Connor\\AppData\\Roaming\\dorka\\hooks.json",
      eventLabel: 'session_start',
      groupIndex: 0,
      handlerIndex: 0,
      command: 'echo session'
    ...
    expect((written.match(/\[hooks\.state\."/g) ?? []).length).toBe(2)
    expect(written).toContain(
      `[hooks.state."C:\\\\Users\\\\O'Connor\\\\AppData\\\\Roaming\\\\dorka\\\\hooks.json:session_start:0:0"]`
    )
    expect(written).toContain(
      `[hooks.state."C:/Users/O'Connor/AppData/Roaming/dorka/hooks.json:session_start:0:0"]`
    )
  ...
  it('finds a Codex-written block with lowercased username when Dorka key has mixed-case username', () => {
    // Why: realpathSync.native casing can differ from what Codex wrote, so case-fold to replace not duplicate.
    const lowercasePath = 'C:\\Users\\rod\\AppData\\Roaming\\dorka\\hooks.json'
    const mixedCasePath = 'C:\\Users\\Rod\\AppData\\Roaming\\dorka\\hooks.json'
