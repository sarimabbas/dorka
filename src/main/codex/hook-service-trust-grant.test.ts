...
beforeEach(() => {
  previousDisableTrustRpc = process.env.DORKA_DISABLE_CODEX_TRUST_RPC
...
afterEach(() => {
  ...
  if (previousDisableTrustRpc === undefined) {
    ...
    process.env.DORKA_DISABLE_CODEX_TRUST_RPC = previousDisableTrustRpc
  }
...
// separated (the shape the real 0.144.x binary writes). Dorka's
...
describe('CodexHookService app-server trust grant lane', () => {
  it('treats Codex hashes as authoritative and records the verified grant', async () => {
    ...
    const selfComputed = computeTrustedHash({
      ...
      groupIndex: 0,
      handlerIndex: 0,
      command: wrapPosixHookCommand(join(tmpHome, '.dorka', 'agent-hooks', 'codex-hook.sh')),
      timeoutSec: 10
    })
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      const material = getCodexManagedHookInstallMaterial()
      const userHook = { type: 'command' as const, command: 'after-dorka.sh' }
      writeFileSync(
        targetPath,
        `${JSON.stringify(
          {
  ...
  it('upgrades self-computed trust in place without duplicate logical entries', async () => {
    prepareSystemHome()
    const service = new CodexHookService()
    process.env.DORKA_DISABLE_CODEX_TRUST_RPC = '1'
    expect((await service.install()).state).toBe('installed')
    const managedHome = join(userDataDir, 'codex-runtime-home', 'home')
    delete process.env.DORKA_DISABLE_CODEX_TRUST_RPC
    installCodexLikeGrantRunner()
