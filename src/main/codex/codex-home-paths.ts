...
export function resolveDorkaManagedCodexHomePath(): string {
  return join(getDorkaUserDataPath(), 'codex-runtime-home', 'home')
}

export function getDorkaManagedCodexHomePath(): string {
  const managedHomePath = resolveDorkaManagedCodexHomePath()
  mkdirSync(managedHomePath, { recursive: true })
  return managedHomePath
}

export function getCodexSessionBackfillStateDirPath(): string {
  return join(getDorkaUserDataPath(), 'codex-session-backfill')
}

export function getDorkaUserDataPath(): string {
  if (process.env.DORKA_USER_DATA_PATH) {
    return process.env.DORKA_USER_DATA_PATH
  }
  ...
  return join(process.env.XDG_CONFIG_HOME || join(homedir(), '.config'), 'dorka')
}
...
export function syncSystemCodexResourcesIntoManagedHome(managedHomePath?: string): void {
  const targetHome = managedHomePath ?? getDorkaManagedCodexHomePath()
...
function linkSystemCodexResource(
  ...
  // Why: both branches below DELETE Dorka's mirrored copy because the system
