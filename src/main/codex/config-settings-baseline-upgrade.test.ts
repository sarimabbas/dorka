...
beforeEach(() => {
  tmpHome = mkdtempSync(join(tmpdir(), 'dorka-codex-settings-upgrade-home-'))
  userDataDir = mkdtempSync(join(tmpdir(), 'dorka-codex-settings-upgrade-data-'))
...
function baselinePath(): string {
  return join(runtimeHomePath(), '.dorka-config-settings-baseline.json')
}
