 ...
 * `DORKA_ORIG_ZDOTDIR` — the output of Dorka's own shell-side ZDOTDIR discovery.
 * That discovery is gone: the wrapper hands ZDOTDIR back on its first lines and
 * zsh resolves the rest natively, so there is no Dorka-computed value left to
...
const REPORTED = ['ZDOTDIR', 'DORKA_TEST_MARK', 'DORKA_TEST_FROM_ZSHRC', 'PATH'] as const
...
const CASES: ConfigCase[] = [
  {
    ...
    setup: (home) => {
      writeFileSync(join(home, '.zshenv'), 'export DORKA_TEST_MARK=plain\n')
      return home
    }
  },
  {
    ...
    setup: (home) => {
      ...
      writeFileSync(join(home, '.zshenv'), `export DORKA_TEST_MARK=xdg\nexport ZDOTDIR="${dir}"\n`)
      return dir
    }
  },
  {
    ...
    setup: (home) => {
      ...
      writeFileSync(join(home, '.zshenv'), `export DORKA_TEST_MARK=sourced\nsource "${common}"\n`)
      return dir
    }
  },
  {
  ...
  },
  {
    ...
    setup: (home) => {
      ...
      writeFileSync(
        join(home, '.zshenv'),
        `export DORKA_TEST_MARK=trailing\nexport ZDOTDIR="${dir}/"\n`
      )
      return dir
    }
  },
  {
    ...
    setup: (home) => {
      writeFileSync(
        join(home, '.zshenv'),
        `export DORKA_TEST_MARK=missing\nexport ZDOTDIR="${join(home, 'nope')}"\n`
      )
      return home
    }
  },
  {
    ...
    setup: (home) => {
      writeFileSync(join(home, '.zshenv'), 'export DORKA_TEST_MARK=empty\nexport ZDOTDIR=""\n')
      return home
    }
  },
  {
    ...
    setup: (home) => {
      writeFileSync(join(home, '.zshenv'), 'export DORKA_TEST_MARK=home\nexport ZDOTDIR="$HOME"\n')
      return home
    }
  },
  {
    ...
    setup: (home) => {
      writeFileSync(join(home, '.zshenv'), 'export DORKA_TEST_MARK=broken\nif [ ; then\n')
      return home
    }
  },
  {
  ...
  },
  {
    ...
    setup: (home) => {
      writeFileSync(join(home, '.zshenv'), 'export DORKA_TEST_MARK=nohome\nunset HOME\n')
      return home
    }
  },
  {
    ...
    setup: (home) => {
      writeFileSync(join(home, '.zshenv'), 'export DORKA_TEST_MARK=slashes\nexport ZDOTDIR="///"\n')
      return home
    }
  },
  {
  ...
  },
  {
    ...
    setup: (home) => {
      writeFileSync(
        join(home, '.zshenv'),
        'export DORKA_TEST_MARK=conditional\nexport ZDOTDIR="$HOME/x"\nunset ZDOTDIR\n'
      )
      return home
    }
  },
  {
    ...
    setup: (home) => {
      ...
      writeFileSync(
        join(home, '.zshenv'),
        'typeset -U path\npath=(/usr/bin /bin /usr/bin)\nexport DORKA_TEST_MARK=uniqpath\n'
      )
      return home
    }
  },
  {
    ...
    setup: (home) => {
      ...
      writeFileSync(
        join(home, '.zshenv'),
        `fpath=(${JSON.stringify(fns)} $fpath)\ndorka_test_fn() { : }\nexport DORKA_TEST_MARK=fnscope\n`
      )
      return home
    }
  },
  {
    ...
    setup: (home) => {
      writeFileSync(join(home, '.zshenv'), 'export DORKA_TEST_MARK=exiting\nexit 0\n')
      return home
    }
  }
]
...
function wrappedEnv(home: string): Record<string, string> {
  const features = selectShellStartupFeatures({
    shellPath: ZSH_PATH,
    env: { HOME: home, DORKA_HISTFILE: join(home, 'scoped_history') },
  ...
  // Why DORKA_ORIG_ZDOTDIR is dropped rather than pinned to the sandbox home:
  // these cases are about a user who has no inherited ZDOTDIR, so the pane must
  // resolve purely from HOME — and Dorka must not invent a ZDOTDIR for it. The
  ...
  const { DORKA_ORIG_ZDOTDIR: _dropped, ...env } = launch.env
  return {
    PATH: '/usr/bin:/bin',
    HOME: home,
    DORKA_HISTFILE: join(home, 'scoped_history'),
    ...env
  }
}
    ...
    it.each(CASES.map((testCase) => [testCase.name, testCase] as const))(
      ...
      async (_name, testCase) => {
        ...
        try {
          ...
          writeFileSync(join(zshrcDir, '.zshrc'), 'export DORKA_TEST_FROM_ZSHRC=1\n')
...
describe.skipIf(process.platform === 'win32')('the fixes the old wrapper was built for', () => {
  ...
  beforeAll(() => {
    ...
    userDataPath = mkdtempSync(join(tmpdir(), 'dorka-hook-regression-'))
   ...
   * be able to show the wrapper ran. DORKA_SHELL_FEATURES coming back consumed is
  ...
  async function runFromRelocatedRoot(home: string, movedRoot: string) {
    ...
    try {
      return await runZshPty({
        ...
        report: ['DORKA_TEST_FROM_ZSHRC', 'DORKA_SHELL_FEATURES', 'HISTFILE']
      })
    } finally {
  ...
  itWithZsh(
    ...
    async () => {
      ...
      const home = makeZshHome({ '.zshrc': 'export DORKA_TEST_FROM_ZSHRC=1\n' })
      try {
        const { values } = await runFromRelocatedRoot(home, `${userDataPath}-wsl-view`)

        expect(values.DORKA_TEST_FROM_ZSHRC).toBe('1')
        // The wrapper really was read from the relocated path.
        expect(values.DORKA_SHELL_FEATURES).toBe('UNSET')
  ...
  itWithZsh('gives the user’s startup files their own ZDOTDIR while they run (#4667)', async () => {
    ...
    // $ZDOTDIR, so sourcing them with Dorka's dir in place sent those lookups into
    ...
    writeFileSync(join(xdg, '.zshrc'), 'export DORKA_TEST_IN_ZSHRC="$ZDOTDIR"\n')
    writeFileSync(join(xdg, '.zprofile'), 'export DORKA_TEST_IN_ZPROFILE="$ZDOTDIR"\n')
    try {
      const report = ['DORKA_TEST_IN_ZSHRC', 'DORKA_TEST_IN_ZPROFILE']
      ...
      expect(wrapped.values.DORKA_TEST_IN_ZSHRC).toBe(xdg)
      expect(wrapped.values.DORKA_TEST_IN_ZPROFILE).toBe(xdg)
  ...
  itWithZsh('refuses an inherited ZDOTDIR that is an Dorka wrapper dir (#15258)', async () => {
    // Why the shell checks this and not only Node: the launch config sets
    // DORKA_ORIG_ZDOTDIR when it resolved a usable dir, but a pane also inherits
    ...
    // marker file, or Dorka's own path shape for wrappers older builds wrote.
    const home = makeZshHome({ '.zshrc': 'export DORKA_TEST_FROM_ZSHRC=1\n' })
    const foreign = join(home, 'other-terminal', 'zsh')
    mkdirSync(foreign, { recursive: true })
    writeFileSync(join(foreign, '.zshrc'), 'export DORKA_TEST_FROM_WRAPPER_DIR=1\n')
    writeFileSync(join(foreign, ZSH_WRAPPER_DIR_MARKER_FILE), '')
    try {
      const { values } = await runZshPty({
        env: { ...wrappedEnv(home), DORKA_ORIG_ZDOTDIR: foreign },
        report: ['ZDOTDIR', 'DORKA_TEST_FROM_ZSHRC', 'DORKA_TEST_FROM_WRAPPER_DIR']
      })

      // Rejected, so zsh falls back to $HOME and the user's own config loads.
      expect(values.ZDOTDIR).toBe('UNSET')
      expect(values.DORKA_TEST_FROM_ZSHRC).toBe('1')
      expect(values.DORKA_TEST_FROM_WRAPPER_DIR).toBe('UNSET')
    } finally {
      rmSync(home, { recursive: true, force: true })
    }
  })

  itWithZsh('leaves a nested Dorka nothing of its own to inherit (#11044, #11146)', async () => {
    // Why this closes #11044's plain shape rather than repairing it: that bug was
    // a nested zsh inheriting Dorka's ZDOTDIR, so /etc/zshrc derived HISTFILE
    // inside the wrapper dir. A pane can no longer hand any child a ZDOTDIR that
    // is Dorka's, because it does not have one itself past the first few lines.
    const home = makeZshHome({ '.zshrc': 'export DORKA_TEST_FROM_ZSHRC=1\n' })
    try {
      const { values } = await runZshPty({
        env: wrappedEnv(home),
        commands: [
          'DORKA_CHILD_ENV="$(env | grep -cE \'^(DORKA_SHELL_FEATURES|DORKA_HISTFILE)=\' || true)"',
          'DORKA_CHILD_ZDOTDIR="$(env | sed -n \'s/^ZDOTDIR=//p\')"'
        ],
        report: ['DORKA_CHILD_ENV', 'DORKA_CHILD_ZDOTDIR']
      })

      // Neither channel survives into a child, and no ZDOTDIR of Dorka's does.
      // `UNSET` here is the probe's rendering of an empty capture, i.e. `env`
      // printed no ZDOTDIR line at all.
      expect(values.DORKA_CHILD_ENV).toBe('0')
      expect(values.DORKA_CHILD_ZDOTDIR).toBe('UNSET')
    } finally {
      rmSync(home, { recursive: true, force: true })
    }
  })
  ...
  itWithZsh('survives a user .zshenv that returns early (#1947)', async () => {
    const home = makeZshHome({
      '.zshenv': 'export DORKA_TEST_MARK=early\nreturn 0\nexport DORKA_TEST_MARK=late\n',
      '.zshrc': 'export DORKA_TEST_FROM_ZSHRC=1\n'
    })
    try {
      const report = ['DORKA_TEST_MARK', 'DORKA_TEST_FROM_ZSHRC', 'ZDOTDIR']
      const wrapped = await runZshPty({ env: wrappedEnv(home), report })
      const unwrapped = await runZshPty({ env: { PATH: '/usr/bin:/bin', HOME: home }, report })

      expect(wrapped.values).toEqual(unwrapped.values)
      expect(wrapped.values.DORKA_TEST_FROM_ZSHRC).toBe('1')
    } finally {
      rmSync(home, { recursive: true, force: true })
    }
  })
