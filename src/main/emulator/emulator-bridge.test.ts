...
import { RuntimeEmulatorCommands } from '../runtime/dorka-runtime-emulator'
...
describe('EmulatorBridge helper ownership', () => {
  ...
  it('stops the previous Dorka-managed helper when a worktree switches devices', async () => {
  ...
  it('shuts down the previous Dorka-managed device when requested', async () => {
    const bridge = new EmulatorBridge()
    bridge.registerActiveEmulator('wt-1', session('device-old'), { managed: true })
  ...
  it('only kills Dorka-managed helpers during app shutdown cleanup', async () => {
    const bridge = new EmulatorBridge()
