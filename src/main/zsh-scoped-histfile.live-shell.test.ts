 ...
 * that the rest of Dorka's startup features arrive with it.
 *
 * macOS `/etc/zshrc` assigns `HISTFILE=${ZDOTDIR:-$HOME}/.zsh_history` with no
 * check-before-set. Dorka used to fight that by keeping its own ZDOTDIR in place
 ...
 * Dorka would not wrap cannot pass here by construction.
 */
...
const systemZshrcClobbersHistfile = (() => {
  ...
  const home = mkdtempSync(join(tmpdir(), 'dorka-histfile-probe-'))
  try {
...
/** The launch config Dorka produces for a pane with exactly these features. */
function launchPane(
  home: string,
  ...
  const env: Record<string, string> = {
    HOME: home,
    ...(scopedHistfile ? { HISTFILE: scopedHistfile, DORKA_HISTFILE: scopedHistfile } : {})
  }
  ...
  return {
    features,
    launch,
    // Why DORKA_ORIG_ZDOTDIR overridden: the launch config resolves the user's
    // config dir from the real process env, and these runs must resolve against
    // the sandbox home instead.
    env: { PATH: '/usr/bin:/bin', ...env, ...launch.env, DORKA_ORIG_ZDOTDIR: home }
  }
}

const USER_FILES = {
  '.zshenv': 'export DORKA_TEST_USER_ZSHENV=1\n',
  '.zprofile': 'export DORKA_TEST_USER_ZPROFILE=1\n',
  '.zshrc': 'export DORKA_TEST_USER_ZSHRC=1\n'
}
...
describe.skipIf(process.platform === 'win32')(
  ...
  () => {
    itWithZsh(
      'wraps a plain pane once Dorka injected a worktree HISTFILE',
      withHome(USER_FILES, async (home) => {
        ...
        expect(launch.env.DORKA_SHELL_FEATURES).toBe('history')
      })
    )
    ...
    itWithClobber(
      ...
      withHome(USER_FILES, async (home) => {
        const scoped = join(home, 'dorka-history', 'zsh_history')
    ...
    itWithZsh(
      ...
      withHome(USER_FILES, async (home) => {
        const { env, launch } = launchPane(home, join(home, 'dorka-history', 'zsh_history'))

        const { values } = await runZshPty({
          env,
          report: [
            'ZDOTDIR',
            'DORKA_TEST_USER_ZSHENV',
            'DORKA_TEST_USER_ZPROFILE',
            'DORKA_TEST_USER_ZSHRC'
          ]
        })

        // Each user file is read from the user's own dir, exactly as unwrapped.
        expect(values.DORKA_TEST_USER_ZSHENV).toBe('1')
        expect(values.DORKA_TEST_USER_ZPROFILE).toBe('1')
        expect(values.DORKA_TEST_USER_ZSHRC).toBe('1')
    ...
    itWithZsh(
      'never leaves history inside Dorka’s own wrapper directory',
      withHome(USER_FILES, async (home) => {
        const { env, launch } = launchPane(home, join(home, 'dorka-history', 'zsh_history'))
    ...
    itWithZsh(
      'consumes DORKA_HISTFILE so nothing the shell spawns can inherit it',
      withHome(USER_FILES, async (home) => {
        const scoped = join(home, 'dorka-history', 'zsh_history')
        const { env } = launchPane(home, scoped)
        ...
        const { values } = await runZshPty({
          env,
          report: ['DORKA_HISTFILE', 'DORKA_SHELL_FEATURES', 'HISTFILE']
        })

        // Root-cause fix for #11146: the variables no longer exist after use.
        expect(values.DORKA_HISTFILE).toBe('UNSET')
    ...
    itWithZsh(
      'leaves HISTFILE exactly as an unwrapped zsh would when Dorka injects nothing',
      withHome(USER_FILES, async (home) => {
        ...
        // The contract is that Dorka's wrapper does not change it either way.
        const overlayEnv = { DORKA_CODEX_HOME: join(home, 'codex') }
        const features = selectShellStartupFeatures({
          shellPath: ZSH_PATH,
...
describe.skipIf(process.platform === 'win32')('the deferred hook delivers every feature', () => {
  ...
  itWithZsh(
    'restores Dorka’s overlay values after the user’s config overwrites them',
    withHome(
      {
        ...USER_FILES,
      ...
      async (home) => {
        const overlayEnv = {
          DORKA_CODEX_HOME: '/dorka/codex',
          DORKA_AGENT_TEAMS_SHIM_DIR: '/dorka/shim'
        }
        const features = selectShellStartupFeatures({
          shellPath: ZSH_PATH,
          env: { HOME: home, ...overlayEnv },
        ...
        // the spawn env did, and Dorka's values still win.
        expect(values.CODEX_HOME).toBe('/dorka/codex')
        expect(values.PATH.startsWith('/dorka/shim:')).toBe(true)
      }
    )
  )
})
...
describe.skipIf(process.platform === 'win32')(
  ...
  () => {
     ...
     * `.zshenv` or `.zprofile` cost the pane all of Dorka's features. Only one
    ...
    it.each([
      ...
      try {
        ...
        const { values } = await runZshPty({ env, report: ['HISTFILE', 'DORKA_HISTFILE'] })
    ...
    itWithZsh(
      ...
      withHome({ ...USER_FILES, '.zshrc': 'precmd_functions=()\n' }, async (home) => {
        ...
        const report = ['HISTFILE', 'DORKA_HISTFILE', 'ZDOTDIR']
        ...
        // host's call, not Dorka's. macOS /etc/zshrc overwrites HISTFILE, so it
        ...
        expect(values.HISTFILE).not.toContain(launch.env.ZDOTDIR)
        // DORKA_HISTFILE was consumed in .zshenv precisely so a dropped hook
    ...
    itWithZsh(
      ...
      withHome(
        ...
        async (home) => {
          const { env } = launchPane(home, join(home, 'dorka-history', 'zsh_history'))

          const { values } = await runZshPty({
            env,
            report: ['ZDOTDIR', 'DORKA_TEST_USER_ZSHRC']
          })
...
describe.skipIf(process.platform === 'win32')('the relay variant of the hook', () => {
  itWithZsh(
    ...
    withHome(USER_FILES, async (home) => {
      const relayRoot = mkdtempSync(join(tmpdir(), 'dorka-relay-wrapper-'))
      try {
        ...
        const { output, values } = await runZshPty({
          env: {
            ...
            DORKA_ORIG_ZDOTDIR: home,
            DORKA_SHELL_FEATURES: 'overlay,history,ready',
            DORKA_HISTFILE: scoped,
            DORKA_REMOTE_CLI_BIN_DIR: '/dorka/remote-bin'
          },
          commands: ['true'],
          report: ['HISTFILE', 'ZDOTDIR', 'PATH', 'DORKA_HISTFILE']
        })

        expect(values.HISTFILE).toBe(scoped)
        expect(values.ZDOTDIR).toBe(home)
        expect(values.DORKA_HISTFILE).toBe('UNSET')
        expect(values.PATH.startsWith('/dorka/remote-bin:')).toBe(true)
        expect(output).toContain(MARKERS.ready)
