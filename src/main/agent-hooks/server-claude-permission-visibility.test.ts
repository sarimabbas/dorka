...
describe('AgentHookServer listener replay', () => {
  it('keeps Claude permission visible when another subagent reports tool activity', async () => {
    ...
    try {
      ...
      const postClaudeHook = async (payload: Record<string, unknown>): Promise<Response> =>
        fetch(`http://127.0.0.1:${env.DORKA_AGENT_HOOK_PORT}/hook/claude`, {
          method: 'POST',
          headers: {
      ...
      await expect(
        postClaudeHook({
          ...
          tool_name: 'Bash',
          tool_input: { command: 'rm -rf /tmp/dorka-subagent-repro' }
        })
      ...
      expect(server.getStatusSnapshot()).toEqual([
        expect.objectContaining({
          ...
          agentType: 'claude',
          toolName: 'Bash',
          toolInput: 'rm -rf /tmp/dorka-subagent-repro'
        })
      ])
    } finally {
      server.stop()
  ...
  it('does not replay cleared pane state to a newly attached listener', async () => {
    ...
    try {
      const env = server.buildPtyEnv()
      await fetch(`http://127.0.0.1:${env.DORKA_AGENT_HOOK_PORT}/hook/codex`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-Dorka-Agent-Hook-Token': env.DORKA_AGENT_HOOK_TOKEN
