...
/** Opens a POST that announces a body and then never sends it, so Dorka's own slowloris cap ends it. */
...
describe('AgentHookServer transport interference', () => {
  ...
  async function startServer(): Promise<{
    ...
    return {
      server,
      port: Number(env.DORKA_AGENT_HOOK_PORT),
      token: env.DORKA_AGENT_HOOK_TOKEN,
      reports
    }
  }
  ...
  it('excludes requests the slowloris cap destroyed, so the count stays honest', async () => {
    ...
    await postTruncatedHook(port, token)
    await postTruncatedHook(port, token)
    // Why: Dorka destroys this one itself at HOOK_REQUEST_SLOWLORIS_MS; counting it would make
