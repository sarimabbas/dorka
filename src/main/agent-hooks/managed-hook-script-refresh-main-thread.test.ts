...
vi.mock('electron', () => ({ app: { getPath: () => '/tmp/dorka-user-data' } }))
...
describe('managed hook script refresh stays off the main thread', () => {
  beforeEach(async () => {
    state.home = await mkdtemp(join(tmpdir(), 'dorka-hook-refresh-main-thread-'))
    state.syncCalls = []
  ...
  it('uses no synchronous HOME filesystem calls for missing or stale scripts', async () => {
    const hooksDir = join(state.home, '.dorka', 'agent-hooks')
