...
describe.skipIf(process.platform === 'win32')(
  ...
  () => {
    ...
    let dorkaServer: AgentHookServer | null
    ...
    it('delivers a Claude hook POST from the live relay into ingestRemote and installs guest hooks', async () => {
      ...
      const server = dorkaServer

      const warns: string[] = []
      const codexHome = join(fakeHome, '.local', 'share', 'dorka', 'codex-runtime-home', 'home')
      manager = new WslHookRelayManager({
        platform: () => 'win32',
        remoteHooksEnabled: () => true,
        hookCoordsEnv: () => ({
          DORKA_AGENT_HOOK_PORT: String(preferredPort),
          DORKA_AGENT_HOOK_TOKEN: 'live-token',
      ...
      const claudeScript = readFileSync(
        join(fakeHome, '.dorka', 'agent-hooks', 'claude-hook.sh'),
        'utf8'
      )
      ...
      const port = Number(/DORKA_AGENT_HOOK_PORT=['"]?(\d+)/.exec(endpointText)?.[1])
      const token = /DORKA_AGENT_HOOK_TOKEN=['"]?([A-Za-z0-9-]+)/.exec(endpointText)?.[1]
