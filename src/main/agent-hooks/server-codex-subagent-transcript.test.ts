...
describe('AgentHookServer Codex subagent transcript polling', () => {
  ...
  // Why: a nested non-codex CLI inherits the pane's DORKA_PANE_KEY, so its hook must not tear down the codex poll.
  it('keeps polling when a nested non-codex hook lands on the same pane', async () => {
    ...
    try {
      ...
      const post = (path: string, payload: unknown): Promise<Response> =>
        fetch(`http://127.0.0.1:${env.DORKA_AGENT_HOOK_PORT}${path}`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'X-Dorka-Agent-Hook-Token': env.DORKA_AGENT_HOOK_TOKEN
