...
describe('Windows managed hook post interpreter', () => {
  ...
  beforeEach(() => {
    ...
    isolatedUserDataDir = mkdtempSync(join(tmpdir(), 'dorka-hook-interpreter-user-data-'))
    // Why: Dorka-managed Codex hooks resolve through DORKA_USER_DATA_PATH before the mocked
    ...
    home = mkdtempSync(join(tmpdir(), 'dorka-hook-interpreter-'))
  ...
  it('posts through curl.exe from every managed batch script, spawning no interpreter', async () => {
    ...
    const posts = (body: string): boolean => body.includes('127.0.0.1:%DORKA_AGENT_HOOK_PORT%')
