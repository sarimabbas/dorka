...
export type FirstWorkBranchRenameDeps = {
  ...
  /** True only for Dorka-created worktrees whose branch Dorka is allowed to rename. */
  canRenameDorkaCreatedBranch: (worktreeId: string) => boolean
...
async function runAutoRename(
  ...
  if (!deps.canRenameDorkaCreatedBranch(worktreeId)) {
    return stop(`worktree is not eligible for auto-rename`, true)
  }
  // Only rename a branch Dorka auto-named — never one the user chose.
