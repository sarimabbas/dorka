...
describe('Copilot hook normalization', () => {
  ...
  it('PermissionRequest stays working and preserves tool context', () => {
    const result = _internals.normalizeHookPayload(
      ...
      buildBody({
        ...
        tool_name: 'bash',
        tool_input: { command: 'rm -rf /tmp/dorka-test' }
      }),
      'production'
    )
    expect(result?.payload.state).toBe('working')
    expect(result?.payload.toolName).toBe('bash')
    expect(result?.payload.toolInput).toBe('rm -rf /tmp/dorka-test')
  })
  ...
  it('Stop reads the final assistant message from Copilot transcript events', () => {
    const tmpDir = mkdtempSync(join(tmpdir(), 'dorka-copilot-transcript-'))
  ...
  it('accepts authenticated HTTP posts on /hook/copilot', async () => {
    ...
    try {
      ...
      const response = await fetch(`http://127.0.0.1:${env.DORKA_AGENT_HOOK_PORT}/hook/copilot`, {
        method: 'POST',
        headers: {
  ...
  it('updates Copilot Stop with final transcript text after a non-blocking retry', async () => {
    const server = new AgentHookServer()
    const tmpDir = mkdtempSync(join(tmpdir(), 'dorka-copilot-transcript-retry-'))
    const transcriptPath = join(tmpDir, 'events.jsonl')
    ...
    try {
      ...
      await fetch(`http://127.0.0.1:${env.DORKA_AGENT_HOOK_PORT}/hook/copilot`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-Dorka-Agent-Hook-Token': env.DORKA_AGENT_HOOK_TOKEN
  ...
  it('updates Grok Stop with final chat-history text after a non-blocking retry', async () => {
    const server = new AgentHookServer()
    const tmpDir = mkdtempSync(join(tmpdir(), 'dorka-grok-chat-history-retry-'))
    ...
    try {
      ...
      await fetch(`http://127.0.0.1:${env.DORKA_AGENT_HOOK_PORT}/hook/grok`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-Dorka-Agent-Hook-Token': env.DORKA_AGENT_HOOK_TOKEN
