...
const SNAPSHOT_FILE = 'dorka-workspace-cleanup-scan.json'
...
describe('workspace cleanup scan snapshot', () => {
  beforeEach(async () => {
    ...
    userDataDirHolder.dir = await mkdtemp(join(tmpdir(), 'dorka-cleanup-snapshot-'))
  })
  ...
  it('keeps profile snapshots isolated', async () => {
    const otherProfile = await mkdtemp(join(tmpdir(), 'dorka-cleanup-snapshot-other-'))
    try {
      await persistWorkspaceCleanupScanResult(
        userDataDirHolder.dir,
        { includeAllWorkspaces: true },
