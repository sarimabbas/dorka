...
/** Basename of the file every Dorka-generated zsh wrapper dir is stamped with. */
export const ZSH_WRAPPER_DIR_MARKER_FILE = '.dorka-shell-wrapper'

export const ZSH_WRAPPER_DIR_MARKER_CONTENT = `# Dorka-generated zsh startup wrapper directory.
# Its presence is how Dorka recognises its own wrapper dir instead of treating it
 ...
 *
 * Why destroy it here: `_dorka_shell_features` is a plain (non-exported) shell
 ...
 * Dorka's feature selection.
 */
export const ZSH_FEATURE_CHANNEL_BLOCK = `builtin typeset -ga _dorka_shell_features
_dorka_shell_features=(\${(s:,:)\${${SHELL_STARTUP_FEATURE_ENV}:-}})
...
export const BASH_FEATURE_CHANNEL_BLOCK = `_dorka_shell_features=",\${${SHELL_STARTUP_FEATURE_ENV}:-},"
builtin unset ${SHELL_STARTUP_FEATURE_ENV}
__dorka_has_feature() { [[ "$_dorka_shell_features" == *",$1,"* ]]; }`

// Why one line usable by both languages: __dorka_has_feature is defined with the
// same name and semantics in the zsh and bash channel blocks above.
export const SHELL_STARTUP_IDENTITY_MARKER_BLOCK = `__dorka_has_feature identity && printf "\\033]777;dorka-shell-start:%s\\007" "$$"`
 ...
 * instead of one inside Dorka's wrapper dir, so #11044 cannot happen rather than
 * having to be repaired afterwards.
 *
 * DORKA_ORIG_ZDOTDIR is consumed: it has done its job, and leaving it exported
 ...
 * environment, so a stale DORKA_ORIG_ZDOTDIR written by an older build can arrive
 * on its own. Handing that back would point ZDOTDIR at an Dorka wrapper dir — the
 ...
 * stamped marker file, or Dorka's own path shape for wrappers older builds wrote.
 */
export const ZSH_ZDOTDIR_HANDBACK_BLOCK = `__dorka_usable_zdotdir() {
  [[ -n "\${1:-}" ]] || return 1
 ...
 * check-before-set, and it runs before any wrapper file Dorka controls — so the
 * injected value is already gone, and because ZDOTDIR still points at Dorka's
 ...
 *
 * `builtin unset DORKA_HISTFILE` is the root-cause fix for #11146: the variable
 ...
 */
export const BASH_HISTFILE_RESTORE_BLOCK = `if [[ -n "\${DORKA_HISTFILE:-}" ]]; then
  HISTFILE="$DORKA_HISTFILE"
  builtin unset DORKA_HISTFILE
elif [[ "\${HISTFILE:-}" == "$ZDOTDIR/.zsh_history" ]]; then
  # Why also when Dorka injected nothing: /etc/zshrc derived this from Dorka's
  ...
  HISTFILE="\${DORKA_ORIG_ZDOTDIR:-$HOME}/.zsh_history"
fi`
...
zle -N zle-line-init __dorka_prompt_mark`.replace(/\n\n}\n$/, '\n}\n')
}
...
export function getFishShellReadyInitCommand(
  ...
  const startupCommandBlock = supportsStartupCommand
    ...
    set -l __dorka_command "\$${POSIX_SHELL_STARTUP_COMMAND_ENV}"
    ...
    : ''
  return `function __dorka_shell_ready_marker --on-event fish_prompt
${readyMarkerBlock}  functions -e __dorka_shell_ready_marker
