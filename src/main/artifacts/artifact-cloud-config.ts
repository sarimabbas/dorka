...
const PRODUCTION_ARTIFACTS_API_URL = 'https://share.ondorka.dev'
...
export function resolveArtifactCloudApiUrl(
  ...
  const candidate = override?.trim() || env.DORKA_ARTIFACTS_API_URL?.trim()
  const url = new URL(candidate || PRODUCTION_ARTIFACTS_API_URL)
  const loopback = ['127.0.0.1', 'localhost', '[::1]'].includes(url.hostname)
  const firstParty = url.hostname === 'ondorka.dev' || url.hostname.endsWith('.ondorka.dev')
  ...
  if (!firstParty && !loopback) {
    throw new Error('Artifact API URLs must use an ondorka.dev or loopback host.')
  }
