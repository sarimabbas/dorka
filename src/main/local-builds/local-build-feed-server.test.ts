...
describe('startLocalBuildFeed', () => {
  it('serves only tokenized manifest and validated artifact routes', async () => {
    const directory = await mkdtemp(join(tmpdir(), 'dorka-local-feed-'))
    const artifactPath = join(directory, 'dorka-macos-arm64.zip')
    ...
    const candidate = {
      ...
      artifacts: new Map([['dorka-macos-arm64.zip', { file: artifactFile, size: 3 }]]),
