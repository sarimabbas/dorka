...
async function fixture(options: { sha512?: string; url?: string } = {}) {
  const directory = await mkdtemp(join(tmpdir(), 'dorka-local-build-'))
  tempDirectories.push(directory)
  const artifactName = 'dorka-macos-arm64.zip'
...
describe('loadLocalBuildCandidate', () => {
  it('returns a sanitized, architecture-specific feed after hash validation', async () => {
    ...
    expect([...candidate.artifacts.keys()]).toEqual(['dorka-macos-arm64.zip'])
    expect(candidate.manifestContent).toContain('dorka-macos-arm64.zip')
  ...
  it('rejects mismatched hashes and traversal paths', async () => {
    ...
    const traversal = await fixture({ url: '../dorka-macos-arm64.zip' })
    await expect(
  ...
  it('serves the same artifact descriptor that passed validation', async () => {
    ...
    try {
      await expect(
        fetch(`${feed.url}dorka-macos-arm64.zip`).then((response) => response.text())
  ...
  it.runIf(process.platform === 'darwin')(
    ...
    async () => {
      const directory = await mkdtemp(join(tmpdir(), 'dorka-local-build-zip-'))
      tempDirectories.push(directory)
      const zipRoot = join(directory, 'zip-root')
      const resources = join(zipRoot, 'Dorka.app', 'Contents', 'Resources')
      await mkdir(resources, { recursive: true })
      await writeFile(join(resources, 'dorka-local-build.json'), JSON.stringify(compatibility()))
      const artifactName = 'dorka-macos-arm64.zip'
      const artifactPath = join(directory, artifactName)
      await execFileAsync('/usr/bin/zip', ['-qry', artifactPath, 'Dorka.app'], { cwd: zipRoot })
  ...
  it('requires exactly one ZIP for the running architecture', async () => {
    ...
    ).rejects.toThrow('exactly one x64 Dorka ZIP')
  })
})
