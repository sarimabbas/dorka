...
import { checkDorkaStarred } from '../github/client'
...
export class StarNagService {
  ...
  registerIpcHandlers(): void {
    ...
    ipcMain.handle('star-nag:starDorka', () => this.starDorkaFromNag())
  ...
  private async maybeShow(
    ...
    try {
      // Why: checkDorkaStarred lets us skip users who already starred outside
      ...
      // button.
      const starred = await checkDorkaStarred()
  ...
  private async starDorkaFromNag(): Promise<boolean> {
    const session = this.promptSession
    if (!session) {
      return false
    }
    ...
    const attempt = this.runStarDorkaAttempt(session)
  ...
  private async runStarDorkaAttempt(session: StarNagPromptSession): Promise<boolean> {
