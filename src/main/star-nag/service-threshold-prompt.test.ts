...
const { browserWindowMock, checkDorkaStarredMock, trackMock } = mocks
...
describe('StarNagService', () => {
  ...
  it('shows the browser fallback when checkDorkaStarred cannot determine star state', async () => {
    const window = createWindow()
    browserWindowMock.getAllWindows.mockReturnValue([window])
    checkDorkaStarredMock.mockResolvedValue(null)
    const { service, emitAgentStarted } = createHarness()
  ...
  it('does not log a threshold exposure when checkDorkaStarred returns true', async () => {
    const window = createWindow()
    browserWindowMock.getAllWindows.mockReturnValue([window])
    checkDorkaStarredMock.mockResolvedValue(true)
    const { service, emitAgentStarted } = createHarness()
