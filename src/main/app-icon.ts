...
import watercolorIcon from '../../resources/app-icons/dorka-watercolor.png?asset'
import watercolorMacDockIcon from '../../resources/app-icons/dorka-watercolor.png?asset&asarUnpack'
import blueIcon from '../../resources/app-icons/dorka-blue.png?asset'
import blueMacDockIcon from '../../resources/app-icons/dorka-blue.png?asset&asarUnpack'
...
const MAC_DOCK_ICON_SCRIPT = [
  ...
  'set appPath to system attribute "DORKA_APP_BUNDLE_PATH"',
  'set iconPath to system attribute "DORKA_APP_ICON_PATH"',
  "set image to current application's NSImage's alloc()'s initWithContentsOfFile:iconPath",
  'if image is missing value then error "Dorka app icon image could not be loaded"',
  "set ok to current application's NSWorkspace's sharedWorkspace()'s setIcon:image forFile:appPath options:0",
  'if ok is false then error "Dorka app icon could not be persisted"'
]
...
const MAC_DOCK_ICON_CLEAR_SCRIPT = [
  ...
  'if ok is false then error "Dorka app icon could not be cleared"'
]
...
function runMacCustomIconCommand(
  ...
  return runBoundedMacDockIconCommand({
    ...
    options: {
      env: {
        ...process.env,
        DORKA_APP_BUNDLE_PATH: appBundlePath,
        DORKA_APP_ICON_PATH: iconPath
      }
    },
