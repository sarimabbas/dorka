/**
 * The single env variable Dorka uses to tell a launched shell which startup
 ...
 * every earlier switch was a negative, exported one (`DORKA_SHELL_READY_MARKER=0`,
 * `DORKA_SHELL_COMMAND_MARKERS=0`). Those live in the pane's PTY env, so every
 * child inherits them — a pane launched with a feature suppressed suppressed it
 * for an Dorka started from that pane too. With an allowlist, an inherited or
...
export const SHELL_STARTUP_FEATURE_ENV = 'DORKA_SHELL_FEATURES'
...
/** Spawn-env keys that mean this pane carries an Dorka overlay the wrapper must re-apply. */
const OVERLAY_ENV_KEYS = [
  'DORKA_OPENCODE_CONFIG_DIR',
  'DORKA_MIMOCODE_HOME',
  'DORKA_OMP_STATUS_EXTENSION',
  'DORKA_CODEX_HOME',
  'DORKA_AGENT_TEAMS_SHIM_DIR',
  'DORKA_REMOTE_CLI_BIN_DIR'
] as const
...
export type ShellStartupFeatureInput = {
  ...
  /** True when Dorka will deliver a startup command into this pane. */
  ...
  /** True when Dorka needs the shell to announce its PID at startup. */
 ...
 * `DORKA_SHELL_FEATURES`, so a value inherited from a parent shell cannot
 * enable or disable anything for the shell Dorka is about to launch.
 */
export function selectShellStartupFeatures(input: ShellStartupFeatureInput): ShellStartupFeature[] {
  ...
  // Exactly the panes Dorka wrapped before history widened wrapping.
  ...
  // login startup-file chain for Dorka's approximation of one.
  // Why also when Dorka injected nothing: any wrapped pane has Dorka's ZDOTDIR in
  // place while the system zshrc runs, so the clobbered value it derives lands
  // inside Dorka's wrapper dir and has to be repaired the same way.
  const history =
    shellName(input.shellPath) === 'zsh' && (Boolean(input.env.DORKA_HISTFILE) || wrappedBefore)
