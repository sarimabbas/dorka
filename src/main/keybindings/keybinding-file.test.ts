...
describe('keybinding-file', () => {
  ...
  beforeEach(() => {
    dir = mkdtempSync(join(tmpdir(), 'dorka-keybindings-'))
  ...
  it('resolves the user-facing keybindings path under ~/.dorka', () => {
    expect(getUserKeybindingsPath('/home/test')).toBe(
      join('/home/test', '.dorka', 'keybindings.json')
    )
  })
  ...
  it('preserves valid plugin overrides while rejecting malformed plugin action IDs', () => {
    writeFileSync(
      ...
      JSON.stringify({
        keybindings: {
          'plugin:dorka-samples.tasks/open': 'Mod+Shift+T',
    ...
    writeKeybindingOverride(filePath, 'linux', 'plugin:dorka-samples.tasks/open', [])
    expect(readKeybindingFile(filePath, 'linux').overrides).toEqual({
      'plugin:dorka-samples.tasks/open': []
    })
  })
