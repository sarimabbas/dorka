...
beforeEach(async () => {
  scratchDir = await mkdtemp(join(tmpdir(), 'dorka-worktree-trash-'))
})
...
describe('sweepStaleWorktreeTrash', () => {
  it('removes leftover entries from both workspace layouts and nothing else', async () => {
    ...
    // Not generated by Dorka: a stray directory and file inside the trash root must survive.
...
describe('collectWorktreeTrashSweepRoots', () => {
  const settings = { workspaceDir: '/home/dev/dorka/workspaces', nestWorkspaces: true }

  function repo(overrides: Partial<Repo>): Repo {
    return { id: 'repo-1', path: '/home/dev/code/dorka', ...overrides } as unknown as Repo
  }
  ...
  it('collects one root per local git repo', () => {
    ...
    ).toEqual(['/home/dev/dorka/workspaces'])
  })
  ...
  it('skips WSL repos that cannot create host trash', () => {
    ...
    try {
      expect(
        collectWorktreeTrashSweepRoots(
          [
            repo({ path: '\\\\wsl.localhost\\Ubuntu\\home\\dev\\dorka' }),
            repo({
              id: 'repo-2',
              path: 'C:\\code\\dorka',
