...
/**
 * PIDs of Dorka's own Chromium processes — browser, renderers, GPU, utilities.
 *
 ...
 * kill in every field Dorka records (#10680). A pid in this set is proof the
 ...
 * (main-process-preflight). The standalone daemon installs none and `dorkad`
 ...
 * the *killing* process's own pid. Dorka's Chromium processes are children of
 * Electron main, so they never classify `own` from a daemon or dorkad host, and
 * on an SSH/serve host there is no Chromium on the machine at all.
 */
export function readDorkaChromiumProcessPids(): ReadonlySet<number> {
