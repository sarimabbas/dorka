...
describe('resolveConsent', () => {
  ...
  it('returns dorka_disabled when DORKA_TELEMETRY_DISABLED=1', () => {
    process.env.DORKA_TELEMETRY_DISABLED = '1'
    expect(
      resolveConsent(
        settingsWithTelemetry({
          optedIn: true,
      ...
      effective: 'disabled',
      reason: 'dorka_disabled'
    })
  })

  it('prefers do_not_track over dorka_disabled when both are set', () => {
    process.env.DO_NOT_TRACK = '1'
    process.env.DORKA_TELEMETRY_DISABLED = '1'
    expect(
      resolveConsent(
  ...
  it('env-var override wins over a stored pending_banner state', () => {
    ...
    ).toEqual({ effective: 'disabled', reason: 'dorka_disabled' })
  })
})
