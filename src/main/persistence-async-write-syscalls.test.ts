...
function dataFile(dir: string): string {
  return join(dir, 'dorka-data.json')
}
...
function ringSnapshot(dir: string): Record<string, string> {
  ...
  for (const name of readdirSync(dir).sort()) {
    if (name === 'dorka-data.json' || name.startsWith('dorka-data.json.bak.')) {
...
describe('async persistence write path avoids synchronous fs syscalls', () => {
  ...
  function makeDir(): string {
    const dir = mkdtempSync(join(tmpdir(), 'dorka-async-write-'))
    dirs.push(dir)
    return dir
  }
  ...
  it('issues no sync fs syscall under the profile dir when rotation runs with an empty ring', async () => {
    ...
    expect(ringSnapshot(dir)['dorka-data.json.bak.0']).toBe(readFileSync(dataFile(dir), 'utf-8'))
  })
  ...
  it('issues no sync fs syscall while rotating a saturated ring', async () => {
    ...
    expect(Object.keys(ring)).toContain(`dorka-data.json.bak.${BACKUP_COUNT - 1}`)
    expect(Object.keys(ring)).not.toContain(`dorka-data.json.bak.${BACKUP_COUNT}`)
  })
  ...
  it('skips rotation when a sync flush rotated during the rotation-interval await', async () => {
    ...
    const ring = ringSnapshot(dir)
    expect(ring['dorka-data.json.bak.0']).toBe(ring['dorka-data.json'])
    expect(ring['dorka-data.json.bak.1']).toBe(staleBackup)
    expect(ring['dorka-data.json.bak.2']).toBeUndefined()
  })
  ...
  it('the throwing async barrier drains mutations made during sidecar I/O', async () => {
    ...
    const renameStarted = deferred()
    fsCalls.waitAsync = (fn, target) => {
      if (fn !== 'rename' || !target.includes('dorka-github-cache.json.')) {
        return null
      }
      renameStarted.resolve()
      return renameRelease.promise
  ...
  it('serializes a second writer behind the owned rotation', async () => {
    ...
    const ring = ringSnapshot(dir)
    expect(JSON.parse(ring['dorka-data.json']).ui.sidebarWidth).toBe(373)
    expect(JSON.parse(ring['dorka-data.json.bak.0']).ui.sidebarWidth).toBe(372)
    expect(ring['dorka-data.json.bak.1']).toBe(staleBackup)
    expect(ring['dorka-data.json.bak.2']).toBeUndefined()
  })
