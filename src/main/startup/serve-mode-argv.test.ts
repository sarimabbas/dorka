...
describe('serve-mode-argv', () => {
  it('detects --serve and bare serve subcommand', () => {
    expect(argvRequestsServeMode(['dorka', '--serve'])).toBe(true)
    ...
    expect(argvRequestsServeMode(['dorka'])).toBe(false)
  })
  ...
  it('leaves real GUI launch argv alone', () => {
    ...
    for (const argv of [
      ['/Applications/Dorka.app/Contents/MacOS/Dorka', '-psn_0_123456'],
      ['C:\\Program Files\\Dorka\\Dorka.exe', '--squirrel-firstrun'],
      ['C:\\Program Files\\Dorka\\Dorka.exe', 'dorka://worktree/serve'],
      ['/opt/dorka/dorka-ide', '/home/u/serve'],
      // `--pairing-code` takes the next token, so its value is never the subcommand.
      ['/opt/dorka/dorka-ide', '--pairing-code', 'serve'],
      ['/opt/dorka/dorka-ide', '--environment=serve'],
      ['/opt/dorka/dorka-ide', '--', 'serve'],
      ['/opt/dorka/dorka-ide', 'Serve']
    ]) {
      expect(argvRequestsServeMode(argv), argv.join(' ')).toBe(false)
      expect(normalizeServeModeArgv(argv)).toEqual(argv)
  ...
  it('leaves already-normalized argv unchanged', () => {
    // Why every value flag: the CLI's own `dorka serve` spawns the app with exactly this shape
    // (serveDorkaApp), and the rewrite now runs over it too — a bad mapping would drop the port here.
    const argv = [
      'dorka',
      '--serve',
      '--serve-json',
  ...
  it('translates serve flags in the mixed `--serve --port` form', () => {
    ...
    expect(normalizeServeModeArgv(['dorka', '--serve', '--port', '9090', '--no-pairing'])).toEqual([
      'dorka',
      '--serve',
      '--serve-port',
      '9090',
  ...
  it('leaves a non-serve launch untouched', () => {
    const argv = ['dorka', '--no-sandbox', '/home/u/project']
