...
const BASE_APP_NAME = 'Dorka'
const BASE_APP_USER_MODEL_ID = 'com.stablyai.dorka'
...
export type DevInstanceIdentity = AppIdentity & {
  ...
  // creating a new one per branch and re-prompting. Distinct from prod's 'Dorka'.
  appName: string
}
 ...
 * CFBundleName, which downstream forks ship differently ("Dorka ALab Edition").
...
export function getDevInstanceIdentity(
  ...
  const repoRoot = cleanEnvValue(env.DORKA_DEV_REPO_ROOT)
  const branch = cleanEnvValue(env.DORKA_DEV_BRANCH)
  const worktreeName =
    cleanEnvValue(env.DORKA_DEV_WORKTREE_NAME) ??
    cleanEnvValue(path.basename(repoRoot ?? process.cwd()))
  const devLabel = cleanEnvValue(env.DORKA_DEV_INSTANCE_LABEL) ?? formatLabel(branch, worktreeName)
  const dockTitle =
    cleanEnvValue(env.DORKA_DEV_DOCK_TITLE) ?? `${BASE_APP_NAME}: ${branch ?? devLabel ?? 'dev'}`

  return {
    name: dockTitle,
    // Why: one stable Keychain key ('Dorka Dev Safe Storage') for all dev
