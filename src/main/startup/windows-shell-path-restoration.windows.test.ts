...
describe.runIf(process.platform === 'win32')('Windows shell PATH restoration', () => {
  ...
  beforeEach(() => {
    _resetHydrateShellPathCache()
    fixtureRoot = mkdtempSync(join(tmpdir(), 'dorka-shell-path-'))
    ...
    writeFileSync(join(shellADir, 'dorka-path-probe.cmd'), '@echo shell-a\r\n')
    writeFileSync(join(shellBDir, 'dorka-path-probe.cmd'), '@echo shell-b\r\n')
  })
