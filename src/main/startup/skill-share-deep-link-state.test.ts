...
describe('SkillShareDeepLinkState', () => {
  it('queues a startup share until the renderer consumes it once', () => {
    ...
    expect(state.capture(['dorka', 'https://app.dorka.dev/skills/share/share_startup'])).toBe(true)
  ...
  it('publishes a later second-instance intent and keeps it for renderer recovery', () => {
    ...
    state.capture(['dorka', 'https://app.dorka.dev/skills/share/share_first'])
    expect(state.capture(['dorka', 'dorka://skills/share/share_second'], publish)).toBe(true)
  ...
  it('ignores untrusted URLs without replacing a pending intent', () => {
    const state = new SkillShareDeepLinkState()
    state.capture(['dorka', 'https://app.dorka.dev/skills/share/share_safe'])

    expect(state.capture(['dorka', 'https://attacker.test/skills/share/share_bad'])).toBe(false)
