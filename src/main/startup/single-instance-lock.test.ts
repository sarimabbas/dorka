...
describe('acquireSingleInstanceLock', () => {
  ...
  it('forwards the second launch argv so the owner can decide whether to activate', () => {
    ...
    registered?.({}, ['/opt/dorka/dorka-linux.AppImage', '--serve'], '/home/dorka')

    expect(onSecondInstance).toHaveBeenCalledTimes(1)
    expect(onSecondInstance).toHaveBeenCalledWith(['/opt/dorka/dorka-linux.AppImage', '--serve'])
  })
})
...
describe('shouldActivateDesktopForSecondInstance', () => {
  it('ignores a duplicate serve launch but still activates for a desktop launch', () => {
    // Why: a supervisor respawning `dorka serve` must not open a window on a display-less host (#11935).
    const serveArgv = ['/opt/dorka/dorka-linux.AppImage', '--serve']
    expect(shouldActivateDesktopForSecondInstance(serveArgv)).toBe(false)
    expect(shouldActivateDesktopForSecondInstance(['/Applications/Dorka.app/dorka'])).toBe(true)
  })
  ...
  it('ignores a duplicate CLI-form serve launch the CLI redirect never rewrote', () => {
    ...
    expect(
      shouldActivateDesktopForSecondInstance([
        '/opt/dorka/squashfs-root/dorka-ide',
        'serve',
        '--port',
        '6768',
        '--pairing-address',
    ...
    expect(
      shouldActivateDesktopForSecondInstance(['/opt/dorka/dorka-ide', '/home/u/serve-repo'])
    ).toBe(true)
  })
  ...
  it('lets isolated E2E exercise the production single-instance path', () => {
    expect(
      shouldSkipSingleInstanceLock({
        isDev: true,
        isServeMode: false,
        env: { DORKA_E2E_ENFORCE_SINGLE_INSTANCE_LOCK: '1' }
      })
    ).toBe(false)
  })
})
...
describe('shouldBypassSingleInstanceLock', () => {
  it('allows the hidden diagnostic bypass only for packaged macOS app launches', () => {
    expect(
      shouldBypassSingleInstanceLock({
        env: { DORKA_BYPASS_SINGLE_INSTANCE_LOCK: '1' },
        isDev: false,
        isServeMode: false,
        platform: 'darwin'
      })
