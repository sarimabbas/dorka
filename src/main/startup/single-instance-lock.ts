...
export const SINGLE_INSTANCE_LOCK_FAILURE_MESSAGE =
  '[single-instance] Another Dorka instance is already running for this userData profile; exiting this launch after requesting the existing window. If no Dorka process is running, this may be an Electron/macOS single-instance lock failure.'
export const SINGLE_INSTANCE_LOCK_BYPASS_ENV = 'DORKA_BYPASS_SINGLE_INSTANCE_LOCK'
export const SINGLE_INSTANCE_LOCK_E2E_ENFORCE_ENV = 'DORKA_E2E_ENFORCE_SINGLE_INSTANCE_LOCK'
export const SINGLE_INSTANCE_LOCK_BYPASS_MESSAGE =
  '[single-instance] DORKA_BYPASS_SINGLE_INSTANCE_LOCK=1 is set; bypassing the packaged macOS single-instance lock for diagnostics. Do not use this with another Dorka instance running for the same profile.'
...
// Why: a duplicate `dorka serve` is a supervisor artifact, not a user asking for a window; fail open when argv is unavailable.
...
/**
 * Why: Dorka writes two canonical discovery files into `<userData>/`:
 * `dorka-runtime.json` (RPC endpoint + authToken for the bundled CLI) and
 ...
 * instance quits, metadata points at a dead pid and `dorka status` reports
 ...
 * way dev (`dorka-dev` userData) and packaged (`dorka` userData) runs lock in
