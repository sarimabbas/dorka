...
} from '../runtime/dorka-runtime-browser'
...
import { sendDorkadSidecarRequest } from './dorkad-sidecar-runtime-client'
import {
...
export class ElectronServeBrowserProcess {
  ...
  async start(): Promise<void> {
    ...
    const userDataPath = await mkdtemp(join(temporaryRoot, 'dorkad-browser-'))
      ...
      if (metadata) {
        try {
          const status = RuntimeStatusResult.parse(
            await sendDorkadSidecarRequest(metadata, 'status.get', undefined, 5_000)
          )
  ...
  private async invoke(
    ...
    if (method === 'browserScreencast') {
      throw new BrowserError(
        ...
        'The dorkad Electron provider does not proxy screencast.'
      )
    }
    ...
    let result: unknown
    try {
      result = await sendDorkadSidecarRequest(metadata, rpcMethod, params)
    } catch (error) {
      if (
        targetPage &&
