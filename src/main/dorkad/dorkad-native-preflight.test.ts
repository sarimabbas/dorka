...
import {
  DORKAD_NATIVE_PRECONDITION_EXIT_CODE,
  runDorkadNativePreflight
} from './dorkad-native-preflight'
import {
...
const harness = (given: NodePtyPreconditionVerdict) => {
  ...
  const continued = runDorkadNativePreflight({
    check: () => given,
...
describe('runDorkadNativePreflight', () => {
  it('stops the boot on a proven-unloadable binary instead of reaching the require', () => {
    ...
    expect(exit).toHaveBeenCalledWith(DORKAD_NATIVE_PRECONDITION_EXIT_CODE)
  ...
  it('exits with EX_CONFIG so a supervisor does not restart an unequippable host forever', () => {
    expect(DORKAD_NATIVE_PRECONDITION_EXIT_CODE).toBe(78)
    expect(DORKAD_NATIVE_PRECONDITION_EXIT_CODE).not.toBe(1)
  })
