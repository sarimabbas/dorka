/**
 * Stands in for an installed Dorka app running `--serve`, so the dorkad Electron
 ...
 * listen on a local transport, then publish `dorka-runtime.json` there.
 *
 * Test-only channels (injected by the spawn wrapper, never by production code):
 *   DORKA_FAKE_SIDECAR_LOG      JSONL of every request received.
 *   DORKA_FAKE_SIDECAR_CONTROL  JSON re-read per request: { capabilities, errors }.
 *   DORKA_FAKE_SIDECAR_MODE     'exit-before-ready' to die without publishing.
 */
'use strict'
...
const logPath = process.env.DORKA_FAKE_SIDECAR_LOG
const controlPath = process.env.DORKA_FAKE_SIDECAR_CONTROL

if (!userDataDir) {
  process.stderr.write('fake sidecar: missing --user-data-dir\n')
  process.exit(2)
}
if (process.env.DORKA_FAKE_SIDECAR_MODE === 'exit-before-ready') {
  process.exit(3)
}
...
const endpoint =
  process.platform === 'win32'
    ? `\\\\.\\pipe\\dorkad-fake-sidecar-${process.pid}`
...
server.listen(endpoint, () => {
  const metadataPath = join(userDataDir, 'dorka-runtime.json')
