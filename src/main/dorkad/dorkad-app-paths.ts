/**
 * The path half of dorkad's `AppEnvironment`: every `AppPathName` gets a real answer for
...
/** XDG-ish data root. `$DORKA_USER_DATA` wins so a smoke test can isolate state. */
export function resolveUserDataPath(): string {
  const explicit = env('DORKA_USER_DATA')
  if (explicit) {
    return explicit
  }
  const xdg = env('XDG_DATA_HOME')
  return xdg ? join(xdg, 'Dorka') : join(homedir(), '.dorka')
}
...
/**
 * The directory dorkad was launched from — the bundle root that `dorkad.js` and the
 ...
 */
export function resolveDorkadInstallRoot(scriptPath = process.argv[1]): string {
  if (!scriptPath) {
    throw new Error(
      'dorkad_install_root_unavailable — process.argv[1] is unset, so this process has no ' +
...
export function resolveDorkadPath(name: AppPathName): string {
  switch (name) {
    case 'userData':
