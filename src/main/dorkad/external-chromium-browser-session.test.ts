...
const BASE = {
  executablePath: '/opt/dorka/chromium',
  profilePath: '/state/browser-chromium',
  sessionName: 'dorka-dorkad-0123456789abcdef'
}
...
describe('dorkad external-chromium agent-browser environment', () => {
  beforeEach(() => {
  ...
  // Why (#16367): dorkad is the backend a remote user's Chromium hangs off, and this session
  // never passes --cdp, so a daemon an earlier dorkad left behind is not bound to the dead
  ...
  // that is the killed-dorkad case the stable session name exists to recover.
