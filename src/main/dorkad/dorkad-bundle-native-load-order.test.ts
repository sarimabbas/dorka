 ...
 * lazy initializer because `dorkad-entry` reaches it through `await import('../ipc/pty')`,
 ...
 *
 * Why this builds the bundle instead of skipping without one: no CI job builds dorkad and
 * runs vitest. `smoke:dorkad-terminal` builds it in the static-analysis job, which never
 * runs vitest; the `dorkad_browser` job runs vitest but deliberately does not build dorkad.
 * A `runIf(existsSync(...))` guard therefore skips in every shard, forever — the same way
 * an unset DORKA_BROWSER_EXECUTABLE kept the browser provider uncovered.
 *
...
const BUNDLE = join(REPO_ROOT, 'out', 'dorkad', 'dorkad.js')
const BUILD_SCRIPT = join(REPO_ROOT, 'config', 'scripts', 'build-dorkad.mjs')
 ...
 */
function ensureDorkadBundle(): void {
...
describe('dorkad bundle native load order', () => {
  const dirs: string[] = []
  afterEach(() => {
  ...
  it('does not require node-pty in-process before the entry rejects its argv', () => {
    ensureDorkadBundle()
    const dir = mkdtempSync(join(tmpdir(), 'dorkad-load-order-'))
    dirs.push(dir)
    ...
    writeFileSync(
      ...
      [
        ...
        '}',
        "process.argv.push('--dorkad-load-order-check')",
    ...
    expect(output).toContain('Unknown argument: --dorkad-load-order-check')
