...
vi.mock('../daemon/daemon-init', () => ({
  ...
  // dorkad shutdown is what would make an dorkad restart destructive again.
...
describe('startDorkadDaemon', () => {
  it('reports live with the daemon pid once the provider is installed', async () => {
    await expect(startDorkadDaemon()).resolves.toEqual({ state: 'live', pid: 4242 })
  })

  it('does not arm the macOS login-session death watch', async () => {
    await startDorkadDaemon()
    // That watch retires the daemon when the spawning GUI login session dies. An dorkad
  ...
  it('reports degraded when fresh terminals would fall back to the local provider', async () => {
    ...
    const result = await startDorkadDaemon()
  ...
  it('fails open when the daemon cannot start at all', async () => {
    ...
    await expect(startDorkadDaemon()).resolves.toEqual({
      state: 'unavailable',
...
describe('stopDorkadDaemon', () => {
  it('disconnects and never shuts the daemon down', async () => {
    ...
    // The whole point of item 4: an dorkad restart is non-destructive only if the daemon
