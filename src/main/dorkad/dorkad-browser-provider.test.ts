...
beforeEach(async () => {
  root = await mkdtemp(join(tmpdir(), 'dorkad-browser-provider-'))
...
describe('ExternalChromiumBrowserProcess', () => {
  it('navigates, evaluates, and screenshots through the operator browser', async () => {
    ...
    const commands = processHandle.createCommands({
      ...
      // DorkaRuntimeService, which owns the client-hosted registries.
...
describe('cross-platform browser provider paths', () => {
  it('resolves installed Electron launchers on macOS, Linux, and Windows', () => {
    expect(installedElectronCandidates('darwin', '/Users/test', {})).toContain(
      '/Users/test/Applications/Dorka.app/Contents/MacOS/Dorka'
    )
    expect(installedElectronCandidates('linux', '/home/test', {})).toContain(
      '/home/test/.local/bin/dorka-ide'
    )
    expect(
      installedElectronCandidates('win32', 'C:\\Users\\test', {
        LOCALAPPDATA: 'C:\\Users\\test\\AppData\\Local'
      })
    ).toContain('C:\\Users\\test\\AppData\\Local\\Programs\\Dorka\\Dorka.exe')
  })

  it('uses platform-specific bundled agent-browser names', () => {
    expect(dorkadAgentBrowserNativeName('darwin', 'arm64')).toBe('agent-browser-darwin-arm64')
    expect(dorkadAgentBrowserNativeName('linux', 'x64')).toBe('agent-browser-linux-x64')
    expect(dorkadAgentBrowserNativeName('win32', 'x64')).toBe('agent-browser-win32-x64.exe')
  })
})

describe('resolveDorkadBrowserProvider', () => {
  it('uses DORKA_BROWSER_EXECUTABLE when Electron is absent', async () => {
    ...
    const provider = await resolveDorkadBrowserProvider({
      userDataPath: root,
      environment: { DORKA_BROWSER_EXECUTABLE: executable },
  ...
  it('reports an DORKA_BROWSER_EXECUTABLE path that does not exist', async () => {
    const missing = join(root, 'absent-chromium')

    await expect(
      resolveDorkadBrowserProvider({
        userDataPath: root,
        environment: { DORKA_BROWSER_EXECUTABLE: missing },
        resolveInstalledElectronExecutable: async () => null,
        resolveAgentBrowserBinary: () => '/agent-browser'
      })
    ).resolves.toBeNull()
  ...
  it.skipIf(process.platform === 'win32')(
    'separates a non-executable DORKA_BROWSER_EXECUTABLE from a missing one',
    async () => {
  ...
  it('reports a failed Electron start instead of staying invisible to the client', async () => {
    ...
    await expect(
      resolveDorkadBrowserProvider({
        userDataPath: root,
        environment: {},
        resolveInstalledElectronExecutable: async () => '/opt/Dorka/dorka-ide',
        resolveAgentBrowserBinary: () => '/agent-browser'
      })
    ).resolves.toBeNull()
    expect(runtimeBrowserUnavailableCause()).toEqual({
  ...
  it('prefers the configured Chromium fault over an earlier Electron failure', async () => {
    ...
    await expect(
      resolveDorkadBrowserProvider({
        userDataPath: root,
        environment: { DORKA_BROWSER_EXECUTABLE: join(root, 'absent-chromium') },
        resolveInstalledElectronExecutable: async () => '/opt/Dorka/dorka-ide',
        resolveAgentBrowserBinary: () => null
      })
    ).resolves.toBeNull()
