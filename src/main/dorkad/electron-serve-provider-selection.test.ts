...
import { resolveDorkadBrowserProvider, type DorkadBrowserProvider } from './dorkad-browser-provider'
...
let provider: DorkadBrowserProvider | null
...
beforeEach(async () => {
  harnessRoot = await mkdtemp(join(tmpdir(), 'dorkad-provider-selection-'))
...
describe('resolveDorkadBrowserProvider Electron preference', () => {
  it('uses the installed Electron app even when a Chromium executable is configured', async () => {
    provider = await resolveDorkadBrowserProvider({
      userDataPath: join(harnessRoot, 'state'),
      environment: { DORKA_BROWSER_EXECUTABLE: chromiumExecutable },
  ...
  it('falls back to the operator Chromium when the installed Electron app cannot start', async () => {
    ...
    expect(warn).toHaveBeenCalledWith(
      '[dorkad] Installed Electron browser provider unavailable:',
      expect.anything()
    )
  })
