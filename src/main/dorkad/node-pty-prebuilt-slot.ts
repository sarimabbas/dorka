 ...
 * by config/scripts/build-dorkad-prebuilds.mjs. An upstream tarball would not do: the patch
 ...
 * `dorkad.js` itself, so this stays correct wherever the install directory ends up.
 */
export function resolveDorkadPrebuildsDir(entryScript = process.argv[1]): string | null {
  const override = process.env.DORKA_DORKAD_PREBUILDS_DIR
  if (override) {
    return override
  }
...
export function installPrebuiltSlot(options: {
  ...
  const prebuildsDir = options.prebuildsDir ?? resolveDorkadPrebuildsDir()
