...
const executablePath = process.env.DORKA_BROWSER_EXECUTABLE
...
describe('ExternalChromiumBrowserProcess integration', () => {
  it.runIf(Boolean(executablePath))(
    ...
    async () => {
      const root = await mkdtemp(join(tmpdir(), 'dorkad-external-browser-'))
      ...
      const provider = await resolveDorkadBrowserProvider({
        userDataPath: root,
        environment: { DORKA_BROWSER_EXECUTABLE: executablePath },
