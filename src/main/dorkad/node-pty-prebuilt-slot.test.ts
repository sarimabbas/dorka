...
const dirs: string[] = []
const temp = (): string => {
  const dir = mkdtempSync(join(tmpdir(), 'dorkad-slot-'))
  dirs.push(dir)
  return dir
}
afterEach(() => {
  ...
  delete process.env.DORKA_DORKAD_PREBUILDS_DIR
})
...
describe('resolveDorkadPrebuildsDir', () => {
  ...
  it('honours an explicit override', () => {
    process.env.DORKA_DORKAD_PREBUILDS_DIR = '/custom/prebuilds'
    expect(resolveDorkadPrebuildsDir('/opt/dorkad/dorkad.js')).toBe('/custom/prebuilds')
  })
})
...
describe('installPrebuiltSlot', () => {
  ...
  it('installs a Linux slot without claiming a spawn-helper it never execs', () => {
    ...
    // macOS is what made every Linux dorkad boot degraded on spawn_helper_missing (#17844).
    const prebuilds = temp()
    const nodePtyDir = temp()
