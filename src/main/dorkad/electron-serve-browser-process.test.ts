...
import type { RuntimeBrowserCommandHost } from '../runtime/dorka-runtime-browser'
...
const FAKE_SIDECAR = join(import.meta.dirname, '__fixtures__', 'fake-dorkad-electron-sidecar.cjs')
const INSTALLED_EXECUTABLE = join('/Applications', 'Dorka.app', 'Contents', 'MacOS', 'Dorka')

/** Every key the provider must strip so the sidecar cannot inherit dorkad's own browser config. */
...
beforeEach(async () => {
  harnessRoot = await mkdtemp(join(tmpdir(), 'dorkad-electron-harness-'))
  ...
  spawnProcessMock.mockImplementation((spec: RunProcessModule.ProcessSpec) =>
    actual.spawnProcess({
      ...
      env: {
        ...spec.env,
        DORKA_FAKE_SIDECAR_LOG: logPath,
        DORKA_FAKE_SIDECAR_CONTROL: controlPath,
        ...(sidecarMode ? { DORKA_FAKE_SIDECAR_MODE: sidecarMode } : {})
      }
    })
  )
})
...
describe('ElectronServeBrowserProcess start-up', () => {
  it('launches the installed app in headless serve mode without dorkad browser env', async () => {
    for (const key of AGENT_BROWSER_ENVIRONMENT_KEYS) {
      vi.stubEnv(key, `leaked-${key}`)
    }
    vi.stubEnv('DORKA_HARNESS_UNRELATED', 'preserved')
    ...
    expect(spec.env?.DORKA_HARNESS_UNRELATED).toBe('preserved')
  ...
  it('stops the sidecar process and removes its user data directory', async () => {
    ...
    const metadata = JSON.parse(
      await readFile(join(userDataPath, 'dorka-runtime.json'), 'utf8')
    ) as { pid: number }
