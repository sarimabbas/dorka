/**
 * `dorkad` — the Dorka runtime served from plain Node, with no Electron.
 *
 * Installs the Node host adapters, constructs the same `DorkaRuntimeService` the
...
import { acquireDorkadInstanceLock, DorkadInstanceLockError } from './dorkad-instance-lock'
import { startDorkadWithLifecycle } from './dorkad-lifecycle'
import { parseArgs } from './dorkad-command-arguments'
import {
...
let runDorkadQuitHandlers = (): void => {}
...
function createNodeAppEnvironment(): AppEnvironment {
  ...
  runDorkadQuitHandlers = (): void => {
    for (const handler of quitHandlers.splice(0)) {
      try {
        handler()
      } catch (error) {
        console.error('[dorkad] shutdown handler failed:', error)
      }
    }
  }
  return {
    getPath: resolveDorkadPath,
    getAppPath: () => resolveDorkadInstallRoot(),
    getVersion: () => process.env.DORKA_VERSION ?? '0.0.0-dorkad',
...
export type DorkadOptions = {
  ...
  /** Literal IP to bind. Defaults to loopback; see dorkad-bind-address.ts. */
  bind?: string
}

export type DorkadHandle = {
 ...
 */
export async function startDorkad(options: DorkadOptions = {}): Promise<DorkadHandle> {
  installDorkadHostAdapters()
  const userDataPath = resolveUserDataPath()
  // Why before anything else touches the root: the profile index, the store and the daemon
  // runtime dir all live under it, and two dorkads sharing them corrupt state silently. This
  // is also the last point at which refusing costs nothing.
  const instanceLock = acquireDorkadInstanceLock(userDataPath)
  const browserProvider = await resolveDorkadBrowserProvider({ userDataPath })
  ...
  return startDorkadWithLifecycle(
    (registerCleanup) => startDorkadRuntime(options, registerCleanup),
    async () => {
      try {
        await browserProvider?.stop()
      } finally {
        setRuntimeBrowserCommandsFactory(null)
        runDorkadQuitHandlers()
...
async function startDorkadRuntime(
  options: DorkadOptions,
  ...
  // Why: dorkad IS the runtime authority — loading as 'desktop' would classify its
  ...
  const runtime = new DorkaRuntimeService(store, undefined, {
    ...
    // Why the daemon predicate and not a constant: dorkad now spawns the terminal daemon, so
    // its PTYs DO survive an dorkad restart — but only while a daemon that owns fresh
    ...
    // projection read from — unwired, dorkad lists no PTY agents at all.
    onTerminalAgentStatus: (event) => agentHookServer.ingestTerminalStatus(event),
    // Why here too and not only on the desktop: dorkad serves `worktree.ps` and `agentSession.*`,
  ...
  // pane's status row changes, and dorkad's whole job is serving paired clients.
  ...
  console.error(`[dorkad] ${describeDorkadBindExposure(bindHost)}`)
  ...
  const readiness: ServeReadiness = {
    ...
    // dorkad never runs it, so there is no pending repair a client could race.
...
/**
 * Exit codes a supervisor can act on. Closed set — see docs/reference/dorkad-operations.md.
 *
 * `DORKAD_EXIT_CONFIGURATION` is the load-bearing one: a data root owned by someone else, or
 * held by another dorkad, is not fixed by restarting. Restarting on it is the crash-loop the
 ...
 */
export const DORKAD_EXIT_OK = 0
export const DORKAD_EXIT_FAILED = 1
export const DORKAD_EXIT_CONFIGURATION = 78

/** Bounded so a wedged transport cannot hold a supervisor's stop past its own deadline. */
export const DORKAD_SHUTDOWN_DEADLINE_MS = 15_000

export function resolveDorkadExitCode(error: unknown): number {
  return error instanceof DorkadInstanceLockError || error instanceof DorkadBindAddressError
    ? DORKAD_EXIT_CONFIGURATION
    : DORKAD_EXIT_FAILED
}

export async function main(argv: string[] = process.argv.slice(2)): Promise<void> {
  const handle = await startDorkad(parseArgs(argv))
  let stopping = false
  const shutdown = (signal: NodeJS.Signals): void => {
    if (stopping) {
      ...
      console.error(`dorkad: second ${signal} during shutdown — exiting immediately`)
      process.exit(DORKAD_EXIT_FAILED)
    }
    stopping = true
    ...
    const deadline = setTimeout(() => {
      console.error(
        `dorkad: shutdown after ${signal} exceeded ${DORKAD_SHUTDOWN_DEADLINE_MS}ms — exiting`
      )
      process.exit(DORKAD_EXIT_FAILED)
    }, DORKAD_SHUTDOWN_DEADLINE_MS)
    deadline.unref()
    handle
      .stop()
      .then(() => process.exit(DORKAD_EXIT_OK))
      // Why not rethrow: we are already tearing down on a signal, and an exit code is
      // the only thing a supervisor can act on.
      .catch((error) => {
        console.error(`dorkad: shutdown after ${signal} failed:`, error)
        process.exit(DORKAD_EXIT_FAILED)
      })
  }
