/**
 * The native precondition dorkad runs before its runtime loads anything native.
 *
 ...
 */
export const DORKAD_NATIVE_PRECONDITION_EXIT_CODE = 78
 ...
 */
export function runDorkadNativePreflight(hooks: NativePreflightHooks = {}): boolean {
  ...
  if (verdict.status === 'blocked') {
    ...
    fail(`dorkad: ${formatNodePtyPreconditionReport(verdict, message, hints)}`)
    exit(DORKAD_NATIVE_PRECONDITION_EXIT_CODE)
    return false
  }
  ...
  warn(`dorkad: ${formatNodePtyPreconditionReport(verdict, message)}`)
  return true
}
