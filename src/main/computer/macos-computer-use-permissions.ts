...
const DEFAULT_COMPUTER_USE_BUNDLE_ID = 'com.stablyai.dorka.computer-use'
...
async function openComputerUsePermissionsAsync(
  ...
  if (!helperAppPath) {
    throw new RuntimeClientError('accessibility_error', 'Dorka Computer Use.app was not found')
  }
...
function closeExistingPermissionHelpers(): void {
  ...
  const setupHelperPatterns = [
    'dorka-computer-use-macos[[:space:]]+--permission([[:space:]]|$)',
    'dorka-computer-use-macos[[:space:]]+--permissions([[:space:]]|$)'
  ]
...
function nextPermissionStep(
  ...
  return `Grant ${missing.id === 'accessibility' ? 'Accessibility' : 'Screen Recording'} to Dorka Computer Use, then retry get-app-state.`
}
