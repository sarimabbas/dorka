...
const permissionStatusTempDir = '/tmp/dorka-computer-use-permissions-test'
const helperAppPath = '/Applications/Dorka Computer Use.app'
...
const plistBuddyStdout = vi.hoisted(() => ({ value: 'com.example.dorka.computer-use\n' }))
...
describe('openComputerUsePermissions', () => {
  ...
  it('does not launch the setup helper when all permissions are granted', async () => {
    resolveHelperAppPathMock.mockReturnValue('/Applications/Dorka Computer Use.app')
    ...
    expect(spawn).not.toHaveBeenCalledWith(
      '/usr/bin/open',
      ['-n', '/Applications/Dorka Computer Use.app', '--args', '--permissions'],
  ...
  it('launches the helper app in permissions mode', async () => {
    ...
    await expect(openComputerUsePermissions()).resolves.toEqual({
      ...
      nextStep: 'Grant Screen Recording to Dorka Computer Use, then retry get-app-state.'
    })
    expect(spawnSync).toHaveBeenCalledWith(
      '/usr/bin/pkill',
      ['-f', 'dorka-computer-use-macos[[:space:]]+--permission([[:space:]]|$)'],
      { stdio: 'ignore' }
    )
    expect(spawnSync).toHaveBeenCalledWith(
      '/usr/bin/pkill',
      ['-f', 'dorka-computer-use-macos[[:space:]]+--permissions([[:space:]]|$)'],
      { stdio: 'ignore' }
    )
  ...
  it('launches a targeted permission helper flow', async () => {
    ...
    await expect(openComputerUsePermissions('accessibility')).resolves.toEqual({
      ...
      nextStep: 'Grant Accessibility to Dorka Computer Use, then retry get-app-state.'
    })
    expect(spawn).toHaveBeenCalledWith(
      '/usr/bin/open',
      ['-n', '/Applications/Dorka Computer Use.app', '--args', '--permission', 'accessibility'],
      { detached: true, stdio: 'ignore' }
    )
  })
  ...
  it('throws when the helper app is missing on macOS', async () => {
    ...
    await expect(openComputerUsePermissions()).rejects.toThrow(
      'Dorka Computer Use.app was not found'
    )
  })
  ...
  it('throws when the helper executable is missing during setup', async () => {
    ...
    await expect(openComputerUsePermissions('accessibility')).rejects.toThrow(
      '/Applications/Dorka Computer Use.app/Contents/MacOS/dorka-computer-use-macos was not found'
    )
  })
  ...
  it('resets stale macOS TCC grants for the helper bundle id', async () => {
    ...
    await expect(resetComputerUsePermissions()).resolves.toEqual({
      ...
      bundleId: 'com.example.dorka.computer-use',
      permissions: [
        { id: 'accessibility', status: 'not-granted' },
        { id: 'screenshots', status: 'not-granted' }
      ]
    ...
    expect(spawn).toHaveBeenCalledWith(
      '/usr/bin/tccutil',
      ['reset', 'Accessibility', 'com.example.dorka.computer-use'],
      throughChokepoint
    )
    expect(spawn).toHaveBeenCalledWith(
      '/usr/bin/tccutil',
      ['reset', 'ScreenCapture', 'com.example.dorka.computer-use'],
      throughChokepoint
    )
