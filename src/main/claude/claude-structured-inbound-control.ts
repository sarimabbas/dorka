...
/**
 * The blocking control requests Dorka answers, each mapped to the SDK consumer callback that
 ...
 * blocking control Dorka must answer means adding its callback here, and the catalog test
...
function denySafeResult(toolUseId: string | undefined): PermissionResult {
  return {
    behavior: 'deny',
    message: 'Dorka could not decode this permission request.',
 ...
 * by `answerClaudePrompt`. `onUserDialog` is deny-safe; the CLI only emits dialog kinds Dorka
