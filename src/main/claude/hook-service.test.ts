...
describe('getWindowsManagedLifecycleHook', () => {
  const SAFE_SCRIPT_PATH = 'C:\\Users\\alice\\.dorka\\agent-hooks\\claude-hook.cmd'
  const UNSAFE_SCRIPT_PATH = 'C:\\Users\\%name%\\a^b&c\\.dorka\\agent-hooks\\claude-hook.cmd'
  ...
  it('registers the script itself, with no interpreter in front of it (#18875)', () => {
    ...
    // event (471ms vs 201ms measured) before the .cmd could reach its DORKA_PANE_KEY guard, and
    ...
    expect(hook.command).toBe('C:/Users/alice/.dorka/agent-hooks/claude-hook.cmd || echo {}')
  ...
  it('falls back to the encoded launcher when the profile path is not cmd-safe', () => {
    ...
    expect(decoded).toContain('.dorka\\agent-hooks\\claude-hook.cmd')
  })
...
describe('ClaudeHookService.install', () => {
  it('installs managed hooks into Claude settings and preserves user Bedrock settings', () => {
    const tmpHome = mkdtempSync(join(tmpdir(), 'dorka-claude-hooks-'))
    ...
    try {
            ...
            Stop: [
              ...
              },
              {
                hooks: [
                  {
                    type: 'command',
                    command: '/Users/old/.dorka/agent-hooks/claude-hook.sh'
                  }
                ]
              }
            ]
      ...
      expect(
        legacyHooks.some((hook: TestHook) =>
          hook.command.includes('/Users/old/.dorka/agent-hooks/claude-hook.sh')
        )
      ).toBe(false)
      ...
      const managedScript = readFileSync(
        join(tmpHome, '.dorka', 'agent-hooks', CLAUDE_SCRIPT_FILE_NAME),
        'utf-8'
      )
  ...
  it('installs the managed statusLine command and forwards rate_limits posts', () => {
    const tmpHome = mkdtempSync(join(tmpdir(), 'dorka-claude-statusline-'))
    vi.stubEnv('HOME', tmpHome)
    vi.stubEnv('USERPROFILE', tmpHome)
    try {
      ...
      expect(settings.statusLine?.command).toContain(
        '"${HOME-}/.dorka/agent-hooks/claude-statusline.cmd"'
      )
      expect(settings.statusLine?.command).toContain(
        '"${HOME-}/.dorka/agent-hooks/claude-statusline.sh"'
      )
      expect(settings.statusLine?.command).not.toContain(tmpHome.replaceAll('\\', '/'))

      const script = readFileSync(
        join(tmpHome, '.dorka', 'agent-hooks', STATUSLINE_SCRIPT_FILE_NAME),
        'utf-8'
      )
      ...
      if (process.platform === 'win32') {
        ...
        expect(script).toContain('--data-urlencode "payload@%DORKA_STATUSLINE_PAYLOAD_FILE%"')
      } else {
  ...
  it('never overwrites a user-owned statusLine command', () => {
    const tmpHome = mkdtempSync(join(tmpdir(), 'dorka-claude-user-statusline-'))
    vi.stubEnv('HOME', tmpHome)
    vi.stubEnv('USERPROFILE', tmpHome)
    try {
  ...
  it('removes the managed statusLine on remove()', () => {
    const tmpHome = mkdtempSync(join(tmpdir(), 'dorka-claude-statusline-remove-'))
    vi.stubEnv('HOME', tmpHome)
    vi.stubEnv('USERPROFILE', tmpHome)
    try {
  ...
  it('does not re-install a managed statusLine the user deleted, until remove() resets the opt-out', () => {
    const tmpHome = mkdtempSync(join(tmpdir(), 'dorka-claude-statusline-optout-'))
    vi.stubEnv('HOME', tmpHome)
    vi.stubEnv('USERPROFILE', tmpHome)
    try {
      const settingsPath = join(tmpHome, '.claude', 'settings.json')
      ...
      // An Dorka-level remove() resets the opt-out memory, so a fresh install re-adds it.
      new ClaudeHookService().remove()
      new ClaudeHookService().install()
      expect(JSON.parse(readFileSync(settingsPath, 'utf-8')).statusLine).toBeTruthy()
    } finally {
  ...
  it('keeps refreshing a still-managed statusLine across installs', () => {
    const tmpHome = mkdtempSync(join(tmpdir(), 'dorka-claude-statusline-refresh-'))
    vi.stubEnv('HOME', tmpHome)
    vi.stubEnv('USERPROFILE', tmpHome)
    try {
      const settingsPath = join(tmpHome, '.claude', 'settings.json')
  ...
  it.skipIf(process.platform !== 'win32')(
    ...
    () => {
      const tmpHome = mkdtempSync(join(tmpdir(), 'dorka claude home with spaces '))
      vi.stubEnv('HOME', tmpHome)
      vi.stubEnv('USERPROFILE', tmpHome)
      try {
        expect(new ClaudeHookService().install().state).toBe('installed')
        ...
        const scriptPath = join(tmpHome, '.dorka', 'agent-hooks', CLAUDE_SCRIPT_FILE_NAME)
        ...
        for (const eventName of ['UserPromptSubmit', 'Stop', 'StopFailure']) {
          ...
          const decoded = Buffer.from(encoded ?? '', 'base64').toString('utf16le')
          expect(decoded).toContain('$env:USERPROFILE')
          expect(decoded).toContain(`.dorka\\agent-hooks\\${CLAUDE_SCRIPT_FILE_NAME}`)
        }
      } finally {
        vi.unstubAllEnvs()
        rmSync(tmpHome, { recursive: true, force: true })
  ...
  it.skipIf(process.platform !== 'win32')(
    ...
    () => {
      const tmpHome = mkdtempSync(join(tmpdir(), 'dorka-claude-direct-'))
      vi.stubEnv('HOME', tmpHome)
      vi.stubEnv('USERPROFILE', tmpHome)
      const scriptPath = join(tmpHome, '.dorka', 'agent-hooks', CLAUDE_SCRIPT_FILE_NAME)
  ...
  it.skipIf(process.platform !== 'win32')(
    ...
    () => {
      const tmpHome = mkdtempSync(join(tmpdir(), 'dorka-claude-migrate-'))
      vi.stubEnv('HOME', tmpHome)
      vi.stubEnv('USERPROFILE', tmpHome)
      const scriptPath = join(tmpHome, '.dorka', 'agent-hooks', CLAUDE_SCRIPT_FILE_NAME)
      if (!WINDOWS_CMD_SAFE_PATH.test(scriptPath)) {
  ...
  it.skipIf(process.platform !== 'win32')(
    ...
    () => {
      ...
      const tmpHome = mkdtempSync(join(tmpdir(), 'dorka-claude-moved-'))
      vi.stubEnv('HOME', tmpHome)
      vi.stubEnv('USERPROFILE', tmpHome)
      const scriptPath = join(tmpHome, '.dorka', 'agent-hooks', CLAUDE_SCRIPT_FILE_NAME)
      if (!WINDOWS_CMD_SAFE_PATH.test(scriptPath)) {
      ...
      try {
        const settingsPath = join(tmpHome, '.claude', 'settings.json')
        mkdirSync(join(tmpHome, '.claude'), { recursive: true })
        const staleCommand = 'C:/Users/someone-else/.dorka/agent-hooks/claude-hook.cmd || echo {}'
  ...
  it.skipIf(process.platform !== 'win32')(
    ...
    () => {
      const tmpHome = mkdtempSync(join(tmpdir(), 'dorka-claude-curl-'))
      vi.stubEnv('HOME', tmpHome)
      vi.stubEnv('USERPROFILE', tmpHome)
      try {
        expect(new ClaudeHookService().install().state).toBe('installed')
...
describe('backgrounded-session pane guard (#9236)', () => {
  ...
  // env of whichever pane started that daemon, so DORKA_PANE_KEY names a pane the session
  ...
  it('declines to post from a daemon worker, before spawning curl', async () => {
    ...
    const script = fs.files.get('/home/dev/.dorka/agent-hooks/claude-hook.sh')!
  ...
  it('guards the statusline too, on both branches', () => {
    ...
    for (const target of ['darwin', 'win32'] as const) {
      const tmpHome = mkdtempSync(join(tmpdir(), `dorka-claude-sl-${target}-`))
      ...
      try {
        ...
        expect(script.indexOf('CLAUDE_JOB_DIR')).toBeLessThan(script.indexOf('curl'))
        if (target === 'win32') {
          // Why: a worker is outside an Dorka pane, where reading stdin to EOF never returns (#11549).
  ...
  it('exits rather than draining stdin on Windows, where a worker has no Dorka pane', () => {
    const platform = Object.getOwnPropertyDescriptor(process, 'platform')!
    const tmpHome = mkdtempSync(join(tmpdir(), 'dorka-claude-bg-'))
    Object.defineProperty(process, 'platform', { value: 'win32', configurable: true })
    vi.stubEnv('HOME', tmpHome)
    vi.stubEnv('USERPROFILE', tmpHome)
    try {
      expect(new ClaudeHookService().install().state).toBe('installed')
      const script = readFileSync(join(tmpHome, '.dorka', 'agent-hooks', 'claude-hook.cmd'), 'utf-8')
...
describe('ClaudeHookService.installRemote', () => {
  it('writes Claude settings + managed script under the remote $HOME', async () => {
    ...
    for (const event of [
      ...
      expect(cmd).toContain('"${HOME-}/.dorka/agent-hooks/claude-hook.sh"')
      expect(cmd).not.toContain('/home/dev/.dorka/agent-hooks/claude-hook.sh')
    }
    // Managed script body
    const script = fs.files.get('/home/dev/.dorka/agent-hooks/claude-hook.sh')
    ...
    expect(script).toContain('dorka_hook_metadata=$(printf')
    expect(script).toContain('unset DORKA_AGENT_HOOK_TRANSPORT')
    expect(script).toContain('-H "X-Dorka-Agent-Hook-Meta: ${dorka_hook_metadata}"')
    expect(script).toContain('--data-binary @-')
    expect(script).toContain('--data-urlencode "payload@-"')
    expect(fs.modes.get('/home/dev/.dorka/agent-hooks/claude-hook.sh')).toBe(0o755)
    ...
    expect(fs.files.get('/home/dev/.dorka/agent-hooks/claude-statusline.sh')).toBeUndefined()
  })
  ...
  it('preserves user-authored hook entries while sweeping old managed entries', async () => {
              ...
              hooks: [
                {
                  type: 'command',
                  command:
                    'if [ -x /home/dev/.dorka/agent-hooks/claude-hook.sh ]; then /bin/sh /home/dev/.dorka/agent-hooks/claude-hook.sh; fi'
                }
              ]
            }
          ]
    ...
    // Original user-authored entry survives, while stale Dorka entries are
...
describe('OpenClaudeHookService-compatible install', () => {
  ...
  it('installs managed hooks into OpenClaude settings without touching Claude settings', () => {
    const tmpHome = mkdtempSync(join(tmpdir(), 'dorka-openclaude-hooks-'))
    vi.stubEnv('HOME', tmpHome)
    vi.stubEnv('USERPROFILE', tmpHome)
    try {
      ...
      for (const event of ['UserPromptSubmit', 'Stop', 'StopFailure']) {
        ...
        expect(command).toContain('"${HOME-}/.dorka/agent-hooks/openclaude-hook.cmd"')
        expect(command).toContain('"${HOME-}/.dorka/agent-hooks/openclaude-hook.sh"')
        expect(command).not.toContain(tmpHome.replaceAll('\\', '/'))
      }
      expect(
        readFileSync(join(tmpHome, '.dorka', 'agent-hooks', OPENCLAUDE_SCRIPT_FILE_NAME), 'utf-8')
  ...
  it('writes remote OpenClaude settings under .openclaude', async () => {
    ...
    expect(command).toContain('"${HOME-}/.dorka/agent-hooks/openclaude-hook.sh"')
    expect(command).not.toContain('/home/dev/.dorka/agent-hooks/openclaude-hook.sh')
    expect(fs.files.get('/home/dev/.dorka/agent-hooks/openclaude-hook.sh')).toContain('/hook/claude')
  })
})
