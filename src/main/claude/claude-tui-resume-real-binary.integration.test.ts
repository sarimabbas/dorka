...
async function installCaptureHook(
  ...
  await writeFile(
    ...
    [
      ...
      '  payload.launchToken = process.env.DORKA_AGENT_LAUNCH_TOKEN',
...
function structuredIdentity(providerSessionId: string): AgentSessionJournalIdentity {
  return {
    sessionId: 'dorka-real-claude-resume',
...
describe.skipIf(!claudeAuthenticated)('real Claude TUI resume proof', () => {
  it('resumes a product-created structured session and proves its exact child', async () => {
    const root = await mkdtemp(join(tmpdir(), 'dorka-claude-tui-resume-'))
    roots.push(root)
      ...
      await expect(
        adapter.dispatch({
          ...
          body: {
            kind: 'message',
            role: 'user',
            blocks: [{ type: 'text', text: 'Reply only with DORKA_RESUME_READY.' }]
          }
        })
