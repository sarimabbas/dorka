...
describe('getManagedStatusLineScript (posix)', () => {
  it('guards on rate_limits before sourcing the endpoint or spawning curl', () => {
    ...
    const endpointIndex = script.indexOf('DORKA_AGENT_HOOK_ENDPOINT')
...
describe('getManagedStatusLineScript (win32 local)', () => {
  it('guards on rate_limits via findstr before the endpoint call and curl spawn', () => {
    ...
    const endpointIndex = script.indexOf('call "%DORKA_AGENT_HOOK_ENDPOINT%"')
    const curlIndex = script.indexOf('curl.exe')
    expect(captureIndex).toBeGreaterThan(-1)
    expect(guardIndex).toBeGreaterThan(captureIndex)
    expect(guardIndex).toBeLessThan(endpointIndex)
    expect(endpointIndex).toBeLessThan(curlIndex)
    expect(script).toContain('if errorlevel 1 goto :dorka_statusline_cleanup')
  })
  ...
  it('posts the buffered payload file and deletes it afterwards', () => {
    ...
    expect(script).toContain('set "DORKA_STATUSLINE_PANE_ID=%DORKA_PANE_KEY:~-36%"')
    expect(script).toContain('set "DORKA_STATUSLINE_PANE_ID=%DORKA_STATUSLINE_PANE_ID::=_%"')
    ...
    const curlIndex = script.indexOf('curl.exe')
    const delIndex = script.indexOf('del "%DORKA_STATUSLINE_PAYLOAD_FILE%"')
  ...
  it('never posts a literal %CLAUDE_CONFIG_DIR% token when the var is unset', () => {
    ...
    expect(script).toContain('set "DORKA_STATUSLINE_CONFIG_DIR_FIELD=configDir="')
    expect(script).toContain(
      'if defined CLAUDE_CONFIG_DIR set "DORKA_STATUSLINE_CONFIG_DIR_FIELD=configDir=%CLAUDE_CONFIG_DIR%"'
    )
    expect(script).toContain('--data-urlencode "%DORKA_STATUSLINE_CONFIG_DIR_FIELD%"')
  ...
  it('drains stdin before exiting when the pane key is missing', () => {
    ...
    const paneGuardIndex = script.indexOf(
      'if "%DORKA_PANE_KEY%"=="" goto :dorka_agent_hook_drain_stdin'
    )
    const captureIndex = script.indexOf('more.com')
    expect(paneGuardIndex).toBeGreaterThan(-1)
    expect(paneGuardIndex).toBeLessThan(captureIndex)
    expect(script).toContain(':dorka_agent_hook_drain_stdin')
  })
  ...
  it('throttles with an all-builtin seconds-of-day stamp that fails open to posting', () => {
    ...
    const throttleIndex = script.indexOf(
      `if %DORKA_STATUSLINE_ELAPSED% GEQ 0 if %DORKA_STATUSLINE_ELAPSED% LSS ${CLAUDE_STATUSLINE_MIN_POST_INTERVAL_SECONDS} goto :dorka_statusline_cleanup`
    )
    ...
    const tokenGuardIndex = script.indexOf('if "%DORKA_AGENT_HOOK_TOKEN%"=="" goto')
    const curlIndex = script.indexOf('curl.exe')
    ...
    expect(script).toContain('if not defined DORKA_STATUSLINE_ELAPSED goto :dorka_statusline_probe')
    expect(script).toContain(
      'for /f "delims=0123456789" %%d in ("%DORKA_STATUSLINE_LAST%") do set "DORKA_STATUSLINE_LAST="'
    )
    expect(script).toContain(
    ...
    expect(script).toContain('set "DORKA_STATUSLINE_TIME=%TIME: =0%"')
  })
})
...
describe('statusline curl throttle (posix)', () => {
  it('checks the per-pane stamp after the env guards and before curl', () => {
    stubPlatform('darwin')
    const script = getManagedStatusLineScript('local')
    const envGuardIndex = script.indexOf('-z "$DORKA_AGENT_HOOK_PORT"')
    const durationIndex = script.indexOf('"total_duration_ms"')
    const stampIndex = script.indexOf('dorka-claude-statusline-last-${dorka_statusline_pane_id}')
    ...
    expect(script).toContain(
      'case "$dorka_statusline_now" in 0|[1-9]|[1-9][0-9]*) ;; *) dorka_statusline_now='
    )
    expect(script).toContain(
      'case "$dorka_statusline_last" in 0|[1-9]|[1-9][0-9]*) ;; *) dorka_statusline_last='
    )
  })
})
...
describe.skipIf(process.platform === 'win32')('statusline curl throttle (posix behavioral)', () => {
  ...
  function makeHarness(): {
    ...
    const dir = mkdtempSync(join(tmpdir(), 'dorka-statusline-throttle-'))
    dirs.push(dir)
  ...
  function stampPathFor(dir: string, leafId = LEAF_ID): string {
    return join(dir, `dorka-claude-statusline-last-${leafId}`)
  }
