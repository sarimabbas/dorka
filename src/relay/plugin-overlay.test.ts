...
describe('PluginOverlayManager', () => {
  ...
  it('materializes OpenCode plugin into <overlay>/plugins/<file>', () => {
    ...
    const expected = join(dir!, 'plugins', 'dorka-opencode-status.js')
  ...
  it('keeps the OpenCode 2 plugin in a separate overlay and filename', () => {
    ...
    expect(dir).not.toBeNull()
    expect(readFileSync(join(dir!, 'plugins', 'dorka-opencode2-status.js'), 'utf8')).toBe(
      'export const V2 = 1'
    )
    expect(existsSync(join(dir!, 'plugins', 'dorka-opencode-status.js'))).toBe(false)
  })
  ...
  it('installs OpenCode plugins in the canonical XDG config roots', () => {
    ...
    expect(
      readFileSync(join(homeDir, 'xdg', 'opencode', 'plugins', 'dorka-opencode-status.js'), 'utf8')
    ).toBe('v1 plugin')
    expect(
      readFileSync(join(homeDir, 'xdg', 'opencode', 'plugins', 'dorka-opencode2-status.js'), 'utf8')
    ).toBe('v2 plugin')
  })

  it('mirrors a preexisting remote OpenCode config dir before adding Dorka plugin', () => {
    ...
    writeFileSync(join(userConfigDir, 'plugins', 'dorka-opencode-status.js'), 'user same-name')

    manager.setSources({ opencodePluginSource: 'dorka plugin' })
    ...
    expect(readFileSync(join(dir!, 'plugins', 'dorka-opencode-status.js'), 'utf8')).toBe(
      'dorka plugin'
    )
  ...
  it('installs Pi extension into the real agent extensions dir', () => {
    ...
    const file = join(result!.sourceAgentDir!, 'extensions', 'dorka-agent-status.ts')
    expect(result?.statusExtensionPath).toBe(file)
    expect(existsSync(file)).toBe(true)
    expect(readFileSync(file, 'utf8')).toContain('@dorka-managed-pi-extension')
  })

  it("does not overwrite a user's same-named remote Pi extension file", () => {
    const piAgentDir = join(homeDir, '.pi', 'agent')
    const extensionFile = join(piAgentDir, 'extensions', 'dorka-agent-status.ts')
  ...
  it('uses the kind-specific Pi-compatible extension source when available', () => {
    ...
    expect(
      readFileSync(join(piResult!.sourceAgentDir!, 'extensions', 'dorka-agent-status.ts'), 'utf8')
    ).toContain('// pi extension')
    expect(
      readFileSync(join(ompResult!.sourceAgentDir!, 'extensions', 'dorka-agent-status.ts'), 'utf8')
  ...
  it('installs Dorka status extension into the remote default Pi agent dir', () => {
    const piAgentDir = join(homeDir, '.pi', 'agent')
  ...
  it('mirrors a preexisting remote Pi agent dir instead of the default', () => {
    ...
    expect(readFileSync(join(dir!, 'extensions', 'dorka-agent-status.ts'), 'utf8')).toContain(
      '// pi extension'
    )
  })
  ...
  it('leaves lazy OMP agent.db in the real remote home on the relay', () => {
    ...
    expect(existsSync(join(homeDir, '.dorka-relay', 'omp-overlays'))).toBe(false)
  ...
  describe('per-agent default source dir (no cross-agent fallback)', () => {
    ...
    it('launching pi with both ~/.pi/agent and ~/.omp/agent present installs into ~/.pi/agent', () => {
      ...
      expect(extensions).toContain('dorka-agent-status.ts')
    ...
    it('launching omp when only ~/.pi/agent exists does NOT mirror Pi state', () => {
      ...
      const extensions = readdirSync(join(dir!, 'extensions')).sort()
      expect(extensions).toEqual(['dorka-agent-status.ts'])
    })
    ...
    it('bare-shell prep does not create missing remote agent homes (#10196)', () => {
      ...
      // Why: bare OMP keeps status via ~/.dorka-relay/… without SOURCE_AGENT_DIR or ~/.omp.
      expect(bareOmp?.sourceAgentDir).toBeUndefined()
      expect(bareOmp?.statusExtensionPath).toEqual(
        expect.stringContaining(join('.dorka-relay', 'omp-managed-status-extension'))
      )
  ...
  it('produces stable overlay dirs for a given id (idempotent re-materialization)', () => {
    ...
    expect(readFileSync(join(dirA, 'plugins', 'dorka-opencode-status.js'), 'utf8')).toBe('second')
  })
...
describe('resolvePiSourceAgentDir', () => {
  it('uses only the selected kind source shadow when resolving inherited overlays', () => {
    const env = {
      ...
      DORKA_PI_CODING_AGENT_DIR: '/tmp/parent-dorka-pi-overlay',
      DORKA_PI_SOURCE_AGENT_DIR: '/user/.pi/agent'
    }
    try {
  ...
  it('keeps explicit PI_CODING_AGENT_DIR values when they are not Dorka overlays', () => {
    const env = {
      HOME: mkdtempSync(join(tmpdir(), 'plugin-overlay-env-')),
