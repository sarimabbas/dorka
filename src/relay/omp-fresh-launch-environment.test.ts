...
it('prepares the execution host OMP config and status extension for a guarded launch', async () => {
  state.home = mkdtempSync(join(tmpdir(), 'dorka-relay-fresh-'))
  ...
  vi.spyOn(PluginOverlayManager.prototype, 'materializePi').mockImplementation(function (
    ...
    this.setSources({ ompExtensionSource: '// Dorka managed test status extension' })
  ...
  try {
    ...
    expect(readFileSync(environment.DORKA_OMP_FRESH_CONFIG, 'utf8')).toBe('autoResume: false\n')
    expect(existsSync(environment.DORKA_OMP_STATUS_EXTENSION)).toBe(true)
    expect(environment.DORKA_OMP_SOURCE_AGENT_DIR).toBe(join(state.home, '.omp', 'agent'))
    expect(environment.PI_CODING_AGENT_DIR).toBeUndefined()
    expect(environment.DORKA_PI_SOURCE_AGENT_DIR).toBeUndefined()
    if (process.platform !== 'win32' && existsSync('/bin/bash')) {
      ...
      const result = await runProcess({
        ...
        env: {
          ...
          DORKA_OMP_PREFILL: 'remote draft'
        }
      })
      expect(result.code).toBe(17)
      expect(result.stdout.trim().split('\n')).toEqual([
        '--extension',
        environment.DORKA_OMP_STATUS_EXTENSION,
        '--config',
        environment.DORKA_OMP_FRESH_CONFIG,
        'remote draft'
      ])
    }
    ...
    ).toEqual({ DORKA_OPENCODE_AGENT: 'opencode' })
  } finally {
    runtime.stop()
    dispatcher.dispose()
  }
...
it('fails the config guard safely when the relay-owned directory cannot be created', () => {
  state.home = mkdtempSync(join(tmpdir(), 'dorka-relay-fresh-fail-'))
