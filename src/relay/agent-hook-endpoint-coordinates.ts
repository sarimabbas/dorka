...
// the DORKA_AGENT_HOOK_* env vars injected into relay-spawned PTYs. IO-free.
...
const RELAY_HOOKS_DIR_NAME = '.dorka-relay'
...
export function buildRelayHookPtyEnv(coordinates: {
  ...
  const env: Record<string, string> = {
    DORKA_AGENT_HOOK_PORT: String(coordinates.port),
    DORKA_AGENT_HOOK_TOKEN: coordinates.token,
    DORKA_AGENT_HOOK_ENV: coordinates.env,
  ...
  if (coordinates.endpointFileWritten) {
    env.DORKA_AGENT_HOOK_ENDPOINT = coordinates.endpointFilePath
  }
  return env
}
