...
describe('PtyHandler', () => {
  ...
  it('reports not-found and reaps a reattach whose backing shell is dead', async () => {
    ...
    await dispatcher.callRequest('pty.spawn', { env: { DORKA_PANE_KEY: 'tab-dead:0' } })
  ...
  it('uses attach identity metadata without exporting it to the shell env', async () => {
    ...
    expect(spawnOptions.env.DORKA_PANE_KEY).toBeUndefined()
    expect(spawnOptions.env.DORKA_TAB_ID).toBeUndefined()
