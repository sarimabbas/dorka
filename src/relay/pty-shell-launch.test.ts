...
describe('getRelayShellLaunchConfig', () => {
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      const config = getRelayShellLaunchConfig('/bin/zsh', {
        HOME: homeDir,
        DORKA_OPENCODE_CONFIG_DIR: '/tmp/dorka-opencode-overlay'
      })
      const zshRoot = join(homeDir, '.dorka-relay', 'shell-ready', 'zsh')

      expect(config.args).toEqual(['-l'])
      expect(config.env.ZDOTDIR).toBe(zshRoot)
      const zshenv = readFileSync(join(zshRoot, '.zshenv'), 'utf8')
      // Why no DORKA_USER_ZDOTDIR: the relay used to republish the inherited
      ...
      expect(zshenv).not.toContain('DORKA_USER_ZDOTDIR')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      // bridge sets DORKA_REMOTE_CLI_BIN_DIR, which was already enough to wrap.
      const env = buildSshPtySpawnEnv({
        env: { HOME: homeDir, PATH: '/usr/bin:/bin' },
        remoteCliBridgeEnv: {
          binDir: '/home/remote/.dorka-relay/bin',
          relayDir: '/home/remote/.dorka-relay',
          nodePath: '/home/remote/.dorka-relay/node',
          sockPath: '/home/remote/.dorka-relay/relay.sock'
        }
      })
      env.DORKA_HISTFILE = join(homeDir, 'dorka-history', 'zsh_history')

      const config = getRelayShellLaunchConfig('/bin/zsh', env)

      expect(config.env.ZDOTDIR).toBe(join(homeDir, '.dorka-relay', 'shell-ready', 'zsh'))
      expect(config.env.DORKA_SHELL_FEATURES).toBe('overlay,history,markers')
    }
  )
  ...
  it.skipIf(process.platform === 'win32')(
    'keeps a remote zsh with nothing Dorka-owned on the plain login path',
    () => {
      const env = buildSshPtySpawnEnv({ env: { HOME: homeDir, PATH: '/usr/bin:/bin' } })
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      const config = getRelayShellLaunchConfig('/bin/zsh', {
        HOME: homeDir,
        DORKA_MIMOCODE_HOME: '/tmp/dorka-mimocode-overlay'
      })
      const zshRoot = join(homeDir, '.dorka-relay', 'shell-ready', 'zsh')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      expect(config.env).toEqual({ DORKA_SHELL_FEATURES: '' })
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      expect(config.env.DORKA_SHELL_FEATURES).toContain('ready')
      ...
      expect(zshenv).toContain('builtin unset DORKA_SHELL_FEATURES')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      expect(config.supportsReadyMarker).toBe(true)
      expect(config.env.DORKA_SHELL_FEATURES).toContain('ready')
      expect(bashRc).toContain('printf "\\033]777;dorka-shell-start:%s\\007" "$$"')
      expect(bashRc).toContain('builtin unset DORKA_SHELL_FEATURES')
      expect(bashRc).toContain('__dorka_append_prompt_command "__dorka_prompt_mark"')
      expect(bashRc).toContain('printf "\\033]777;dorka-shell-ready\\007"')
    }
  )
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      expect(config.env.ZDOTDIR).toBe(join(homeDir, '.dorka-relay', 'shell-ready', 'zsh'))
      expect(config.env.DORKA_SHELL_FEATURES).toContain('identity')
      ...
      expect(config.env.DORKA_SHELL_FEATURES).not.toContain('ready')
  ...
  itWithBash('runs the relay bash wrapper without fake C/D markers before the first prompt', () => {
    ...
    expect(bashRc).toContain('[[ -z "${__dorka_in_command:-}" ]] || return 0')
    expectBashOsc133Lifecycle(output)
  })

  itWithBash('emits lifecycle for foreground text ending like an internal hook', () => {
    const config = getRelayShellLaunchConfig('/bin/bash', { HOME: homeDir })
    const input = 'echo user:__dorka_osc133_prompt_done\nfalse\nexit 0\n'
    const output = runInteractiveBashRcfile(config.args[1] as string, homeDir, input)

    expect(output).toContain('user:__dorka_osc133_prompt_done')
    expectBashOsc133Lifecycle(output)
  })
  ...
  itWithBash('preserves relay bash prompt hooks and DEBUG traps without fake markers', () => {
    ...
    expect(output).not.toContain('USER_DEBUG_AFTER:<(( __dorka_exit_code == 0 ))>')
    expect(output).not.toContain('USER_DEBUG_AFTER:<__dorka_restore_prompt_status')
    expectBashOsc133Lifecycle(output)
  })
  ...
  // Dorka inherits ends in a ";"+whitespace separator. Prepend/append must not
