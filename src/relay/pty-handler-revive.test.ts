...
describe('PtyHandler', () => {
  ...
  it('revive restores pane identity env alongside hook-server coordinates', async () => {
    await dispatcher.callRequest('pty.spawn', {
      ...
      cwd: '/tmp',
      env: {
        DORKA_PANE_KEY: 'tab-5:1',
        DORKA_TAB_ID: 'tab-5',
        DORKA_WORKTREE_ID: 'wt-5'
      }
    })
    ...
    const oldShellFeatures = process.env.DORKA_SHELL_FEATURES
    process.env.DORKA_SHELL_FEATURES = 'ready,identity,markers,overlay'
    try {
      ...
      if (oldShellFeatures === undefined) {
        ...
        process.env.DORKA_SHELL_FEATURES = oldShellFeatures
      }
      killSpy.mockRestore()
    }
    ...
    expect(callArgs.env.DORKA_PANE_KEY).toBe('tab-5:1')
    expect(callArgs.env.DORKA_TAB_ID).toBe('tab-5')
    expect(callArgs.env.DORKA_WORKTREE_ID).toBe('wt-5')
    ...
    expect(callArgs.env.TERM_PROGRAM).toBe('Dorka')
    expect(callArgs.env.DORKA_SHELL_FEATURES).not.toContain('ready')
    expect(callArgs.env.DORKA_SHELL_FEATURES).not.toContain('identity')
  })
  ...
  it('normalizes an explicit empty TERM and preserves sanitized env deletions on revive', async () => {
    await dispatcher.callRequest('pty.spawn', {
      env: { TERM: '' },
      envToDelete: ['DORKA_STALE_TEST_ENV', '', 42]
    })
    ...
    expect(serialized.envToDelete).toEqual(['DORKA_STALE_TEST_ENV'])
    ...
    handler = createTestPtyHandler(dispatcher)
    handler.addEnvAugmenter(() => ({
      DORKA_STALE_TEST_ENV: '/tmp/revived-stale'
    }))
    const killSpy = vi.spyOn(process, 'kill').mockImplementation(() => true)
    try {
      await dispatcher.callRequest('pty.revive', { state })
    ...
    expect(revivedEnv.env.DORKA_STALE_TEST_ENV).toBeUndefined()
  })
    ...
    const state = JSON.stringify([
      {
        ...
        cwd: process.cwd(),
        explicitTerm: '',
        envToDelete: ['DORKA_STALE_TEST_ENV']
      }
    ])
    handler.addEnvAugmenter(() => ({
      DORKA_STALE_TEST_ENV: '/tmp/legacy-empty-stale'
    }))
    const killSpy = vi.spyOn(process, 'kill').mockImplementation(() => true)
    try {
      await dispatcher.callRequest('pty.revive', { state })
  ...
  it('preserves explicit TERM and env deletions through repeated revive cycles', async () => {
    ...
    try {
      ...
      handler = createTestPtyHandler(dispatcher)
      handler.addEnvAugmenter(() => ({
        DORKA_STALE_TEST_ENV: '/tmp/first-revive'
      }))
      await dispatcher.callRequest('pty.revive', { state })
      ...
      expect(firstRevivedEnv.env.DORKA_STALE_TEST_ENV).toBeUndefined()
      ...
      handler = createTestPtyHandler(dispatcher)
      handler.addEnvAugmenter(() => ({
        DORKA_STALE_TEST_ENV: '/tmp/second-revive'
      }))
      await dispatcher.callRequest('pty.revive', { state })
    } finally {
      killSpy.mockRestore()
    ...
    expect(secondRevivedEnv.env.DORKA_STALE_TEST_ENV).toBeUndefined()
  })

  it('revives legacy serialized entries with default TERM and no env deletions', async () => {
    handler.addEnvAugmenter(() => ({
      DORKA_STALE_TEST_ENV: '/tmp/legacy-stale'
    }))
    const state = JSON.stringify([
      {
        id: 'pty-7',
    ...
    expect(revivedEnv.env.TERM).toBe('xterm-256color')
    expect(revivedEnv.env.DORKA_STALE_TEST_ENV).toBe('/tmp/legacy-stale')
  })

  it('revive preserves attach identity metadata without exporting hook identity env', async () => {
    const oldPaneKey = process.env.DORKA_PANE_KEY
    const oldTabId = process.env.DORKA_TAB_ID
    delete process.env.DORKA_PANE_KEY
    delete process.env.DORKA_TAB_ID
    try {
      await dispatcher.callRequest('pty.spawn', {
        cols: 90,
        rows: 30,
      ...
      if (oldPaneKey === undefined) {
        ...
        process.env.DORKA_PANE_KEY = oldPaneKey
      }
      if (oldTabId === undefined) {
        delete process.env.DORKA_TAB_ID
      } else {
        process.env.DORKA_TAB_ID = oldTabId
      }
    }
    const state = (await dispatcher.callRequest('pty.serialize', { ids: [PTY_1] })) as string
    ...
    const callArgs = mockPtySpawn.mock.calls[0][2] as { env: Record<string, string> }
    expect(callArgs.env.DORKA_PANE_KEY).toBeUndefined()
    expect(callArgs.env.DORKA_TAB_ID).toBeUndefined()
  ...
  it('skips a pane whose serialized cwd is gone from this host, keeping the batch', async () => {
    const removedCwd = join(tmpdir(), `dorka-revive-removed-${process.pid}`)
