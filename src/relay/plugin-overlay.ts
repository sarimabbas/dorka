...
// Why: relay-side equivalent of Dorka's local agent integration installers.
// OpenCode still needs a config overlay, while Pi/OMP now get Dorka-managed
...
// independently from Dorka and the plugin source changes frequently as new
...
// directly: those modules import `electron` and ride on Dorka's userData
// path. The relay's electron-free constraint forces a thin parallel
// implementation rooted at $HOME/.dorka-relay/ for OpenCode and at the remote
...
const OPENCODE_PLUGIN_FILE = 'dorka-opencode-status.js'
const OPENCODE2_PLUGIN_FILE = 'dorka-opencode2-status.js'
const PI_EXTENSION_FILE = 'dorka-agent-status.ts'
const PI_AGENT_SUBDIR = 'agent'
const OMP_MANAGED_STATUS_EXTENSION_DIR = 'omp-managed-status-extension'
// Why: bare-shell OMP still needs DORKA_OMP_STATUS_EXTENSION without mkdir ~/.omp.
...
function withDorkaManagedPiExtensionMarker(source: string): string {
...
function safeDirName(input: string): string {
  ...
  // filesystem-unsafe characters in some Dorka builds. Hash to a fixed-width
...
export type PluginSources = {
  /** Source body of `dorka-opencode-status.js` to drop into <overlay>/plugins/. */
  opencodePluginSource?: string
  /** Source body of OpenCode 2's status plugin. */
  opencode2PluginSource?: string
  /** Source body of Pi's `dorka-agent-status.ts` to drop into <overlay>/extensions/. */
  piExtensionSource?: string
  /** Source body of OMP's `dorka-agent-status.ts` to drop into <overlay>/extensions/. */
  ompExtensionSource?: string
  /** Source body of Prime Agent's `dorka-agent-status.ts` to install in its real agent dir. */
...
export type MaterializePiResult = {
  ...
  /** Absolute path to dorka-agent-status.ts (real home or relay-managed fallback). */
...
export class PluginOverlayManager {
  ...
  /** Replace the cached source bodies. Called from relay.ts when Dorka sends
   *  `agent_hook.installPlugins`. The first install enables the augmenter
   *  output; subsequent installs (e.g. Dorka version upgrade in flight) refresh
  ...
  setSources(sources: PluginSources): void {
    ...
    if (typeof sources.piExtensionSource === 'string') {
      this.piExtensionSources.pi = withDorkaManagedPiExtensionMarker(sources.piExtensionSource)
    }
    if (typeof sources.ompExtensionSource === 'string') {
      this.piExtensionSources.omp = withDorkaManagedPiExtensionMarker(sources.ompExtensionSource)
    }
    if (typeof sources.primeAgentExtensionSource === 'string') {
      this.piExtensionSources['prime-agent'] = withDorkaManagedPiExtensionMarker(
  ...
  materializeOpenCode(
    ...
    try {
      ...
      if (existingConfigDir) {
        ...
        // remote root into the overlay before adding Dorka's plugin so status
