...
beforeAll(async () => {
  const externalEntry = process.env.DORKA_SKILL_UPLOAD_RELAY_ENTRY
  if (externalEntry) {
    relayEntry = resolve(externalEntry)
    return
  }
  bundleRoot = await mkdtemp(join(tmpdir(), 'dorka-skill-multi-relay-bundle-'))
...
async function uploadRootForOracle(home: string): Promise<string> {
  const installRoot = join(home, '.dorka', 'skill-installs')
...
describe('skill upload ownership across relay processes', () => {
  it('keeps each live relay upload isolated and cleans each exact owner', async () => {
    const root = await mkdtemp(join(tmpdir(), 'dorka-skill-multi-relay-'))
    roots.push(root)
  ...
  it
    .runIf(Boolean(process.env.DORKA_SKILL_UPLOAD_LEGACY_RELAY_ENTRY))
    .each(['legacy-first', 'current-first'] as const)(
    'keeps %s mixed-version uploads isolated',
    async (order) => {
      const root = await mkdtemp(join(tmpdir(), 'dorka-skill-mixed-relay-'))
      roots.push(root)
      const home = join(root, 'home')
      const environment = { ...process.env, HOME: home, USERPROFILE: home }
      const legacyEntry = resolve(process.env.DORKA_SKILL_UPLOAD_LEGACY_RELAY_ENTRY!)
      const entries =
        order === 'legacy-first'
          ? [
