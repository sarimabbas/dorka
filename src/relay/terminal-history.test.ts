...
const historyDir = join(homedir(), '.dorka-remote', 'terminal-history')
...
describe('relay shell history', () => {
  ...
  ])('replaces a %s HISTFILE inherited from a parent Dorka', (_kind, inherited) => {
    // HISTFILE is exported, so a relay (or the client that spawned it) started
    // from an Dorka pane would otherwise scope every remote pane to that one
