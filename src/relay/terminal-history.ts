...
const HISTORY_ROOT = join(homedir(), '.dorka-remote', 'terminal-history')
...
export function injectRelayHistoryEnv(
  ...
  // Why first: same reason as the desktop path — an inherited DORKA_HISTFILE
  ...
  // Why: HISTFILE stays exported, so a relay (or a client) launched from an Dorka
  ...
  try {
    ...
    env.DORKA_HISTFILE = env.HISTFILE
    return HISTORY_ROOT
  } catch {
    return null
  }
...
export function injectRelayFishHistoryEnv(env: Record<string, string>, worktreeId: string): void {
  ...
  // user value, and fish EXPORTS `fish_history` so an Dorka-minted name arrives from
