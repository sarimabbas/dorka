...
export async function runRelayDorkaCliChannel(
  sockPath: string,
  argv: string[],
  ...
  const stdin =
    ...
    (shouldReadRemoteCliStdin(argv) ? await readDorkaCliStdin() : undefined)
  ...
  const onDecodeError = (error: Error): void => {
    ...
    process.stderr.write(`[dorka-cli] Relay protocol error: ${error.message}\n`, () => {
      sock.destroy()
      process.exit(1)
    })
  }
  ...
  const connectTimeout = setTimeout(() => {
    process.stderr.write(`[dorka-cli] Relay connection timed out after ${CONNECT_TIMEOUT_MS}ms\n`)
    sock.destroy()
    process.exit(1)
  }, CONNECT_TIMEOUT_MS)
  ...
  sock.on('error', (error) => {
    clearTimeout(connectTimeout)
    process.stderr.write(`[dorka-cli] Relay socket error: ${error.message}\n`)
    process.exit(1)
  })
}

async function readDorkaCliStdin(): Promise<string | undefined> {
