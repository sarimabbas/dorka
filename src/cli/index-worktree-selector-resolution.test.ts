...
describe('dorka cli worktree awareness', () => {
  ...
  it('resolves the invocation cwd from DORKA_CLI_CWD when no cwd is passed', async () => {
    // Why: the SSH relay bridge runs the CLI on the Dorka host with the remote
    // shell's cwd carried in DORKA_CLI_CWD (#7716); cwd-based selectors must
    // resolve against it, not the host process cwd.
    process.env.DORKA_CLI_CWD = '/tmp/repo/feature/src'
    try {
      queueFixtures(
        callMock,
        worktreeListFixture([
      ...
      delete process.env.DORKA_CLI_CWD
    }
  })

  it.skipIf(process.platform === 'win32')(
    'prepares and starts Claude Agent Teams in the current Dorka terminal',
    async () => {
      process.env.DORKA_PANE_KEY = 'tab-1:11111111-1111-4111-8111-111111111111'
      queueFixtures(
        callMock,
        okFixture('req_agent_teams_prepare', {
          launch: {
            env: {
              ...
              TMUX: '/tmp/dorka-claude-agent-teams/team-1,0,1',
              TMUX_PANE: '%1',
              PATH: '/tmp/dorka-shim:/usr/bin'
            }
          }
        })
      )
      ...
      expect(callMock).toHaveBeenCalledWith('agentTeams.prepareLaunch', {
        ...
        env: expect.objectContaining({
          DORKA_PANE_KEY: 'tab-1:11111111-1111-4111-8111-111111111111'
        })
      })
