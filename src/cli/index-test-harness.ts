...
export async function createRuntimeClientModuleMock(mocks: RuntimeClientModuleMocks) {
    ...
    constructor(
      ...
      const effectivePairingCode =
        remotePairingCode === undefined
          ? (process.env.DORKA_PAIRING_CODE ?? process.env.DORKA_REMOTE_PAIRING)
          : remotePairingCode
      const effectiveEnvironment =
        environmentSelector === undefined ? process.env.DORKA_ENVIRONMENT : environmentSelector
  ...
  return {
    ...
    serveDorkaApp: mocks.serveDorkaAppMock,
...
export function useWorktreeAwarenessEnvironment(mocks: WorktreeAwarenessMocks): void {
  ...
  const originalDevCliInvocation = process.env.DORKA_DEV_CLI_INVOCATION
  const originalPairingCode = process.env.DORKA_PAIRING_CODE
  const originalRemotePairing = process.env.DORKA_REMOTE_PAIRING
  const originalEnvironment = process.env.DORKA_ENVIRONMENT
  const originalWorkspaceId = process.env.DORKA_WORKSPACE_ID
  const originalWorktreeId = process.env.DORKA_WORKTREE_ID

  beforeEach(() => {
    mocks.callMock.mockReset()
    delete process.env.DORKA_TERMINAL_HANDLE
    delete process.env.DORKA_USER_DATA_PATH
    delete process.env.DORKA_DEV_CLI_INVOCATION
    delete process.env.DORKA_WORKSPACE_ID
    delete process.env.DORKA_WORKTREE_ID
    ...
    mocks.serveDorkaAppMock.mockReset()
  ...
  afterEach(() => {
    ...
    if (originalDevCliInvocation === undefined) {
      ...
      process.env.DORKA_DEV_CLI_INVOCATION = originalDevCliInvocation
    }
    if (originalPairingCode === undefined) {
      delete process.env.DORKA_PAIRING_CODE
    } else {
      process.env.DORKA_PAIRING_CODE = originalPairingCode
    }
    if (originalRemotePairing === undefined) {
      delete process.env.DORKA_REMOTE_PAIRING
    } else {
      process.env.DORKA_REMOTE_PAIRING = originalRemotePairing
    }
    if (originalEnvironment === undefined) {
      delete process.env.DORKA_ENVIRONMENT
    } else {
      process.env.DORKA_ENVIRONMENT = originalEnvironment
    }
    if (originalWorkspaceId === undefined) {
      delete process.env.DORKA_WORKSPACE_ID
    } else {
      process.env.DORKA_WORKSPACE_ID = originalWorkspaceId
    }
    if (originalWorktreeId === undefined) {
      delete process.env.DORKA_WORKTREE_ID
    } else {
      process.env.DORKA_WORKTREE_ID = originalWorktreeId
    }
  })
}
