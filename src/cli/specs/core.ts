...
export const CORE_COMMAND_SPECS: CommandSpec[] = [
  {
    path: ['open'],
    summary: 'Launch Dorka and wait for the runtime to be reachable',
    usage: 'dorka open [--json]',
    allowedFlags: [...GLOBAL_FLAGS],
    examples: ['dorka open', 'dorka open --json']
  },
  ...SERVE_COMMAND_SPECS,
  {
    path: ['status'],
    summary: 'Show app/runtime/graph readiness',
    usage: 'dorka status [--json]',
    allowedFlags: [...GLOBAL_FLAGS],
    examples: ['dorka status', 'dorka status --json']
  },
  {
    path: ['claude-teams'],
    argumentMode: 'passthrough',
    summary: 'Start Claude Code Agent Teams in the current Dorka terminal',
    usage: 'dorka claude-teams [claude args...]',
    allowedFlags: [...GLOBAL_FLAGS],
    notes: [
      'Passes all following arguments through to Claude Code after enabling Agent Teams native panes.',
      'Must be run from inside an Dorka terminal. Starts Claude Code Agent Teams in the current pane and opens teammates as native Dorka splits.'
    ],
    examples: ['dorka claude-teams', 'dorka claude-teams --resume <session-id>']
  },
  {
    path: ['repo', 'list'],
    summary: 'List repos registered in Dorka',
    usage: 'dorka repo list [--json]',
    allowedFlags: [...GLOBAL_FLAGS]
  },
  {
    path: ['repo', 'add'],
    summary: 'Add a project to Dorka by filesystem path',
    usage: 'dorka repo add --path <path> [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'path']
  },
  {
    path: ['repo', 'show'],
    summary: 'Show one registered repo',
    usage: 'dorka repo show --repo <selector> [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'repo']
  },
  {
    path: ['repo', 'set-base-ref'],
    summary: "Set the repo's default base ref for future worktrees",
    usage: 'dorka repo set-base-ref --repo <selector> --ref <ref> [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'repo', 'ref']
  },
  {
    path: ['repo', 'search-refs'],
    summary: 'Search branch/tag refs within a repo',
    usage: 'dorka repo search-refs --repo <selector> --query <text> [--limit <n>] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'repo', 'query', 'limit']
  },
  {
    path: ['worktree', 'list'],
    summary: 'List Dorka-managed worktrees',
    usage: 'dorka worktree list [--repo <selector>] [--limit <n>] [--json]',
  ...
  },
  {
    ...
    usage: 'dorka worktree show --worktree <selector> [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'worktree']
  },
  {
    path: ['worktree', 'current'],
    summary: 'Show the Dorka-managed worktree for the current directory',
    usage: 'dorka worktree current [--json]',
    allowedFlags: [...GLOBAL_FLAGS],
    notes: [
      'Resolves the current shell directory to a path: selector so agents can target the enclosing Dorka worktree without spelling out $PWD.'
    ],
    examples: ['dorka worktree current', 'dorka worktree current --json']
  },
  {
    path: ['worktree', 'create'],
    summary: 'Create a new Dorka-managed worktree',
    usage:
    ...
    notes: [
      'This creates a new checkout. For a fresh agent in an existing worktree, use `dorka terminal create --worktree active --command "codex"` instead.',
      'By default, Dorka records the new worktree as a child of the caller context when it can infer one from the Dorka terminal or current directory.',
      'If --repo is omitted, Dorka infers the repo from the current Dorka-managed worktree.',
      'Use --project with --host to create on a ready project host setup without spelling the backing repo id.',
      '--host runtime:<environment-id> creates on that paired Dorka server; use the id from `dorka environment list`, not the environment name.',
      'For related work, use the inferred parent or pass --parent-worktree active, folder:<id>, or worktree:<worktreeId> to make the relationship explicit. Worktree ids are the full <repo-id>::<path> values returned by `dorka worktree list --json`.',
      'Use --no-parent when the new worktree should be independent of the current context.',
      '--no-parent only affects Dorka lineage; omit --base-branch to use the repo default base, or pass the default base ref explicitly for independent top-level work.',
      'By default this creates the worktree and its first terminal without switching the active Dorka view.',
    ...
    examples: [
      'dorka worktree create --name agent-task --agent codex --prompt "hi" --json',
      'dorka worktree create --repo id:<repoId> --name related-task --json',
      'dorka worktree create --project github:stablyai/orca --host runtime:03ef704c-b180-4b10-998d-e28fbd5de9a3 --name benchmark --json',
      'dorka worktree create --repo id:<repoId> --name linear-task --linear-issue https://linear.app/stably/issue/STA-335/test-issue --json',
      'dorka worktree create --repo id:<repoId> --name agent-task --agent codex --prompt "hi" --json',
      ...
      'dorka worktree create --repo id:<repoId> --name independent-task --no-parent --json'
    ]
  },
  {
    path: ['worktree', 'set'],
    summary: 'Update Dorka metadata for a worktree',
    usage:
    ...
    examples: [
      'dorka worktree set --worktree active --linear-issue STA-335 --json',
      'dorka worktree set --worktree active --linear-issue null --json'
    ]
  },
  {
    path: ['worktree', 'rm'],
    ...
    destructive: true,
    summary: 'Remove a worktree from Dorka and git',
    usage:
    ...
    notes: [
      'Repo-defined dorka.yaml archive hooks are skipped unless --run-hooks is passed.',
      ...
      'For Git worktrees, removal also attempts to delete the checked-out local branch, with or without --force. Dorka retains branches it knows predated the worktree and any branch whose changes it cannot prove are already merged.'
    ]
  },
  {
    path: ['worktree', 'ps'],
    summary: 'Show a compact orchestration summary across worktrees',
    usage: 'dorka worktree ps [--limit <n>] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'limit'],
    notes: [...WORKTREE_LISTING_SCOPE_NOTES]
  },
  {
    path: ['terminal', 'list'],
    summary: 'List live Dorka-managed terminals',
    usage:
  ...
  },
  {
    ...
    usage: 'dorka terminal show [--terminal <handle>] [--json]',
  ...
  },
  {
    ...
    usage:
      'dorka terminal read [--terminal <handle>] [--cursor <n>] [--limit <n>] [--screen] [--json]',
    ...
    examples: [
      ...
      'dorka terminal read --terminal term_abc123 --cursor 42 --limit 1000 --json',
      'dorka terminal read --terminal term_abc123 --screen --json'
    ]
  },
  ...
  },
  {
    ...
    usage: 'dorka terminal stop --worktree <selector> [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'worktree'],
    notes: [
  ...
  },
  {
    ...
    notes: [
      ...
      'A host that cannot apply --shell refuses the create rather than quietly spawning its default shell: macOS and Linux execution hosts spawn the login shell, terminals routed over SSH resolve their shell on the SSH host, a --shell that contradicts the project execution runtime (WSL vs Windows host) is refused, and an Dorka host older than --shell is refused by the CLI.'
    ],
    examples: [
      'dorka terminal create --json',
      'dorka terminal create --worktree active --command "codex" --json',
      'dorka terminal create --worktree path:/projects/myapp --title "RUNNER" --command "opencode"',
      'dorka terminal create --worktree path:/projects/myapp --command "opencode" --focus',
      'dorka terminal create --worktree path:C:/src/app --shell cmd.exe --json'
    ]
  },
  {
    ...
    usage: 'dorka terminal switch [--terminal <handle>] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'terminal'],
    examples: ['dorka terminal switch --terminal term_abc123']
  },
  TERMINAL_CLOSE_COMMAND_SPEC,
  {
    path: ['terminal', 'rename'],
    summary: 'Set or clear the title of a terminal tab',
    usage: 'dorka terminal rename [--terminal <handle>] [--title <text>] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'terminal', 'title'],
    notes: ['Omit --title or pass an empty string to reset to the auto-generated title.'],
    examples: [
      'dorka terminal rename --terminal term_abc123 --title "RUNNER"',
      'dorka terminal rename --terminal term_abc123 --json'
    ]
  },
  {
    path: ['terminal', 'split'],
    ...
    usage:
      'dorka terminal split [--terminal <handle>] [--direction horizontal|vertical] [--command <text>] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'terminal', 'direction', 'command'],
    examples: [
      'dorka terminal split --terminal term_abc123 --direction horizontal --json',
      'dorka terminal split --terminal term_abc123 --command "codex"'
    ]
  }
]
