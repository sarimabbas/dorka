...
export const VM_COMMAND_SPECS: CommandSpec[] = [
  {
    ...
    usage:
      'dorka vm recipe doctor <recipe-id> [--repo-path <path>] [--provision|--connect] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'recipe-id', 'repo-path', 'provision', 'connect'],
    positionalArgs: ['recipe-id'],
    notes: [
      'Reads environmentRecipes from dorka.yaml in the repo path, validates the selected recipe, and reports agent-friendly checks.',
    ...
    examples: [
      'dorka vm recipe doctor cloud-sandbox',
      'dorka vm recipe doctor cloud-sandbox --repo-path /path/to/repo --json',
      'dorka vm recipe doctor cloud-sandbox --provision --json'
    ]
  }
]
