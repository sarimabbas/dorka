...
// ~/.cursor — a green test run silently deleted every Dorka-managed hook on the machine, so agent
// status stopped reporting until the next Dorka restart (STA-5679). The byte-for-byte equivalence
...
describe('RuntimeClient module-graph deferral', () => {
  ...
  beforeEach(() => {
    testUserDataPathRef.current = mkdtempSync(join(tmpdir(), 'dorka-runtime-deferral-userdata-'))
  ...
  // is what stops the DORKA_* env fallback re-activating for local-only groups.
  //
  ...
  it.each(SUPPRESSED_GROUPS)(
    ...
    async (_name, argv, constructs) => {
      ...
      vi.stubEnv('DORKA_ENVIRONMENT', 'some-environment')
