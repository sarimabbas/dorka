...
// and serveDorkaApp binding are referenced by the module under test.
...
describe('dorka claude-teams CLI handler', () => {
  ...
  beforeEach(() => {
    ...
    previousPaneKey = process.env.DORKA_PANE_KEY
    previousExitCode = process.exitCode
    // The `dorka` launcher runs Dorka's Electron binary as Node, so the CLI process
  ...
  afterEach(() => {
    ...
    if (previousPaneKey === undefined) {
      ...
      process.env.DORKA_PANE_KEY = previousPaneKey
    }
  ...
  it.skipIf(isWindows)(
    ...
    async () => {
      const previousMarker = process.env.DORKA_TEST_MARKER
      process.env.DORKA_TEST_MARKER = 'keep-me'
      try {
        await runClaudeTeams()
      } finally {
        if (previousMarker === undefined) {
          delete process.env.DORKA_TEST_MARKER
        } else {
          process.env.DORKA_TEST_MARKER = previousMarker
        }
      }

      const spawnEnv = spawnMock.mock.calls.at(-1)?.[2].env as SpawnEnv
      expect(spawnEnv.DORKA_TEST_MARKER).toBe('keep-me')
