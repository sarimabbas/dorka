...
function getDataPath(): string {
  ...
  const indexPath = join(userDataPath, 'dorka-profile-index.json')
  for (const candidate of [indexPath, `${indexPath}.bak`]) {
    try {
      ...
      if (
        ...
        return join(userDataPath, 'profiles', profileId, 'dorka-data.json')
      }
    } catch {
      // Try the profile-index backup, then the legacy pre-profile path.
    }
  }
  return join(userDataPath, 'dorka-data.json')
}
      ...
      try {
        await client.call(
          'agentHooks.prepareCodexForWslPane',
          {
            ...
            dorkaCodexHome: process.env.DORKA_CODEX_HOME ?? '',
