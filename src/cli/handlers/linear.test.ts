...
vi.mock('../runtime-client', async () => {
  class RuntimeClient {
    ...
    constructor(
      ...
      remotePairingCode = process.env.DORKA_PAIRING_CODE ?? null,
...
describe('dorka linear CLI handlers', () => {
  ...
  beforeEach(() => {
    ...
    // Why: these tests can run inside an Dorka-managed terminal, which exports
  ...
  it('passes verified current-context hints without resolving cwd for remote runtimes', async () => {
    process.env.DORKA_TERMINAL_HANDLE = 'term_123'
    process.env.DORKA_WORKTREE_ID = 'repo::/srv/app'
    process.env.DORKA_PAIRING_CODE = 'dorka://pair?payload=bad'
    queueFixtures(callMock, okFixture('req_linear', issueResult()))
