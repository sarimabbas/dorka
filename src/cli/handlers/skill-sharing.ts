...
function rejectForwardedSkillFilesystem(ctx: HandlerContext, command: string): void {
  if (!process.env.DORKA_CLI_CWD && !ctx.client.isRemote) {
    return
  }
  throw new RuntimeClientError(
    'invalid_environment',
    `dorka skills ${command} must run on the machine whose installed skills you want to use. Run the command from an Dorka terminal on that machine.`
  )
}
...
function requireCloudOperation<T>(operation: SkillCloudOperation<T>): T {
  ...
  if (operation.status === 'reconnect-required') {
    throw new RuntimeClientError('authentication_required', 'Sign in to Dorka and try again.')
  }
...
async function callShare(
  ...
  try {
    ...
    if (error instanceof RuntimeRpcFailureError && error.code === 'method_not_found') {
      throw new RuntimeClientError(
        'update_required',
        'The connected Dorka runtime does not support agent skill sharing yet. Update Dorka on that machine and try again.'
      )
    }
    throw error
  }
...
export const SKILL_SHARING_HANDLERS: Record<string, CommandHandler> = {
  ...
  'skills share': async (ctx) => {
    ...
    if (skillSelectors.length === 0) {
      throw new RuntimeClientError(
        'invalid_argument',
        'Select at least one installed skill with --skill. Run `dorka skills installed` to list them.'
      )
    }
