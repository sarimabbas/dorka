 ...
 * GUI flow drives this via a browser Dorka can't reach on a headless host.
 */
...
async function addClaudeAccount({ client, json }: HandlerContext): Promise<void> {
  const configDir = mkdtempSync(join(tmpdir(), 'dorka-account-add-claude-'))
...
async function addCodexAccount({ client, json }: HandlerContext): Promise<void> {
  const codexHome = mkdtempSync(join(tmpdir(), 'dorka-account-add-codex-'))
  const session: InteractiveLoginSession = {
    child: null,
    registering: false,
    terminationPromise: null
...
async function assertAccountImportSupported({ client }: HandlerContext): Promise<void> {
  ...
  if (!status.result.capabilities?.includes(ACCOUNT_IMPORT_RUNTIME_CAPABILITY)) {
    throw new RuntimeClientError(
      ...
      'The running Dorka runtime is too old to add accounts from the CLI. Update or restart Dorka and try again.'
    )
  }
}

/** CLI handlers for `dorka account add [--agent claude|codex]` and `dorka account list`. */
export const ACCOUNT_HANDLERS: Record<string, CommandHandler> = {
  'account add': async (ctx) => {
    ...
    rejectAccountRemoteSelectionFlags(ctx, 'dorka account add')
  ...
  'account list': async (ctx) => {
    rejectAccountRemoteSelectionFlags(ctx, 'dorka account list')
