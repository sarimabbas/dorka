...
describe('dorka emulator CLI handlers', () => {
  ...
  it('uses the folder workspace exported by the current Dorka terminal', async () => {
    process.env.DORKA_WORKSPACE_ID = 'folder:folder-1'
    delete process.env.DORKA_WORKTREE_ID
  ...
  it('uses the current git worktree exported by the Dorka terminal', async () => {
    process.env.DORKA_WORKSPACE_ID = 'folder:stale-parent'
    process.env.DORKA_WORKTREE_ID = 'repo-1::/repo/project '
    callMock.mockResolvedValue(
      okFixture('req_attach', {
        attached: true,
        info: { deviceUdid: 'device-1', streamUrl: 'scrcpy://device-1' }
