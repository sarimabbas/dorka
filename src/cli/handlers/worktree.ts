...
function getEnvParentWorkspace(): string | undefined {
  const workspaceId = process.env.DORKA_WORKSPACE_ID
  if (typeof workspaceId === 'string' && isWorkspaceKey(workspaceId)) {
    return workspaceId
  }
  const worktreeId = process.env.DORKA_WORKTREE_ID
...
async function getCreateRepoSelector(
  ...
  throw new RuntimeClientError(
    'invalid_argument',
    'Missing repo selector. Pass --repo or run from inside an Dorka-managed worktree.'
  )
}
...
export const WORKTREE_HANDLERS: Record<string, CommandHandler> = {
  ...
  'worktree create': async ({ flags, client, cwd, json }) => {
    ...
    const callerTerminalHandle =
      typeof process.env.DORKA_TERMINAL_HANDLE === 'string' &&
      process.env.DORKA_TERMINAL_HANDLE.length > 0
    ...
    if (
      ...
      try {
        // Why: agent shells can lose DORKA_TERMINAL_HANDLE while still running
        // inside an Dorka worktree. Cwd keeps CLI-created children nestable and
  ...
  'worktree rm': async ({ flags, client, cwd, json }) => {
    ...
    if (!hostId) {
      throw new RuntimeClientError(
        ...
        'Dorka cannot tell which host owns this workspace. Refresh projects and try again.'
      )
    }
