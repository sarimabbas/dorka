...
const originalPackagedLauncher = process.env.DORKA_WINDOWS_PACKAGED_CLI_LAUNCHER
...
describe('packaged Windows legacy ask protocol', () => {
  beforeEach(() => {
    callMock.mockReset()
    process.env.DORKA_WINDOWS_PACKAGED_CLI_LAUNCHER = '1'
  ...
  afterEach(() => {
    ...
    restoreEnv('DORKA_CLI_COMMAND', originalCliCommand)
    restoreEnv('DORKA_WINDOWS_PACKAGED_CLI_LAUNCHER', originalPackagedLauncher)
    restoreEnv('DORKA_TERMINAL_HANDLE', originalTerminalHandle)
  })

  it.each(['dorka', 'dorka-ide'] as const)(
    'commits with the %s launcher and exits 75 before resume',
    async (command) => {
      process.env.DORKA_CLI_COMMAND = command
  ...
  it('resumes the committed question without another exit-75 handoff', async () => {
    process.env.DORKA_CLI_COMMAND = 'dorka'
    callMock.mockResolvedValue({
      result: {
        answer: 'yes',
        messageId: 'msg_question',
    ...
    expect(callMock).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        question: undefined,
        resume: 'msg_question',
        compatibilityWindowsCommand: 'dorka'
      }),
      expect.any(Object)
    )
