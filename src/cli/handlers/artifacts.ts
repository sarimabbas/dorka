...
function cloudOptions(ctx: HandlerContext): ArtifactCloudOptions {
  const apiUrl = stringFlag(ctx, 'api-url') ?? process.env.DORKA_ARTIFACTS_API_URL?.trim()
  const authToken = process.env.DORKA_CLOUD_AUTH_TOKEN?.trim()
  return {
    ...(apiUrl ? { apiUrl } : {}),
    ...(authToken ? { authToken } : {})
  }
  ...
  for await (const chunk of process.stdin) {
    ...
    if (bytes > maxBytes) {
      throw new RuntimeClientError(
        'invalid_argument',
        'Artifact is too large for the Dorka CLI transport. Use the browser upload page instead.'
      )
    }
    chunks.push(buffer)
  }
