...
import { launchDorkaApp } from './runtime/launch'

vi.mock('./runtime/launch', () => ({
  launchDorkaApp: vi.fn()
}))
...
afterEach(async () => {
  vi.mocked(launchDorkaApp).mockClear()
...
describe.skipIf(process.platform === 'win32')('RuntimeClient', () => {
  it('adds an opaque durable request ID only to orchestration mutations', async () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'dorka-runtime-client-'))
    ...
    const priorLaunchToken = process.env.DORKA_AGENT_LAUNCH_TOKEN
    process.env.DORKA_AGENT_LAUNCH_TOKEN = 'launch-secret'
    ...
    try {
      ...
      if (priorLaunchToken === undefined) {
        delete process.env.DORKA_AGENT_LAUNCH_TOKEN
      } else {
        process.env.DORKA_AGENT_LAUNCH_TOKEN = priorLaunchToken
      }
    }
  ...
  it('openDorka activates the app even when a desktop runtime is already reachable', async () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'dorka-runtime-client-'))
    const endpoint = join(userDataPath, 'runtime.sock')
    const server = createServer((socket) => {
      sockets.add(socket)
    ...
    const client = new RuntimeClient(userDataPath, 100)
    const status = await client.openDorka(100)

    expect(status.result.runtime.state).toBe('ready')
    expect(status.result.runtime.reachable).toBe(true)
    expect(launchDorkaApp).toHaveBeenCalledOnce()
  })

  it('openDorka waits for a reachable headless runtime to expose a desktop window', async () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'dorka-runtime-client-'))
    const endpoint = join(userDataPath, 'runtime.sock')
    ...
    const client = new RuntimeClient(userDataPath, 100)
    const status = await client.openDorka(1_000)
  ...
  it('openDorka fails explicitly when the serve owner cannot promote safely', async () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'dorka-runtime-client-'))
    const endpoint = join(userDataPath, 'runtime.sock')
    const server = createServer((socket) => {
      sockets.add(socket)
    ...
    await expect(client.openDorka(100)).rejects.toMatchObject({
  ...
  it('preserves a dropped read-only orchestration failure exactly', async () => {
    ...
    expect((failure as RuntimeClientError).message).toBe(
      'The Dorka runtime closed the connection before responding. Restart Dorka and try again.'
    )
