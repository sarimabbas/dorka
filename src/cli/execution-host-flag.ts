...
export type HostFlagRoutingSelection = {
  // Why: SSH targets live in the running Dorka host, not on disk, so enumerating them needs a
  ...
  // Why: an ambient DORKA_ENVIRONMENT counts as a selection too, so carry the label to
...
export async function resolveHostFlagEnvironmentId(
  ...
  if (!environment) {
    ...
    throw new RuntimeClientError(
      'invalid_argument',
      `Unknown Dorka server in --host ${host.id}: no paired Dorka server is named or has id ${host.environmentId}.`,
      {
        ...
        nextSteps: [
          ...
          'Run `dorka environment list` to see paired Dorka servers.',
  ...
  if (selection.pairingCode) {
    throw new RuntimeClientError(
      'invalid_argument',
      `--host ${host.id} already selects a paired Dorka server; use either --host runtime:<id> or --pairing-code, not both.`
    )
  }
  if (selection.environmentSelector) {
    ...
    if (selected.id !== environment.id) {
      throw new RuntimeClientError(
        'invalid_argument',
        `--host ${host.id} and ${selection.environmentSelector.label} ${selection.environmentSelector.value} name different Dorka servers.`
      )
    }
  }
...
function assertEnvironmentNameUnambiguous(
  ...
  throw new RuntimeClientError(
    'invalid_argument',
    `Ambiguous Dorka server in ${flag}: ${ambiguous.length} paired servers are named ${name}. Use the environment id.`,
    {
...
export async function assertEnvironmentSelectorResolvable(
  ...
  throw new RuntimeClientError(
    'invalid_argument',
    `Unknown Dorka server in --environment ${selector}: no paired Dorka server is named or has id ${selector}.`,
    {
      knownEnvironments: environments,
      knownSshTargets: sshTargets,
      nextSteps: [
        ...crossKindNextSteps(selector, { environments, sshTargets }, 'environment'),
        'Run `dorka host list` to see every machine you can target and the flag for each.'
      ]
    }
  )
}
