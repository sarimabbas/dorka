  ...
  return await new Promise((resolve, reject) => {
    ...
    if (!transport) {
      reject(
        new RuntimeClientError(
          'runtime_unavailable',
          'No compatible transport found in Dorka runtime metadata.'
        )
      )
      return
    }
    ...
    socket.once('error', (error) => {
      finish({
        ...
        error:
          ...
          new RuntimeClientError(
            'runtime_unavailable',
            'Could not connect to the running Dorka app. Restart Dorka and try again.'
          )
      })
    })
    ...
    socket.on('data', (chunk: string) => {
      ...
      while (cursor < chunk.length && !settled) {
        ...
        try {
          ...
          finish({
            ...
            error: new RuntimeClientError(
              ...
              'The Dorka runtime returned an invalid response frame.'
            )
          })
          return
        }
        ...
        // setTimeout().refresh() is stable since Node 10 (Dorka ships on
        ...
        // the decode boundary so version skew between the CLI and the Dorka
        ...
        if (response.id !== requestId) {
          finish({
            ...
            error: new RuntimeClientError(
              'invalid_runtime_response',
              'The Dorka runtime returned a mismatched response id.'
            )
          })
          return
        }
        if (response._meta?.runtimeId && response._meta.runtimeId !== metadata.runtimeId) {
          finish({
            ...
            error: new RuntimeClientError(
              'runtime_unavailable',
              'The Dorka runtime changed while the request was in flight. Retry the command.'
            )
          })
          return
        }
