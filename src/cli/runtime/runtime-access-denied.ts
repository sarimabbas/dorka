...
export function runtimeAccessDeniedError(
  ...
  // Why: a sandbox still sees ESRCH, so a dead Dorka's leftover socket gets not-running advice.
  ...
  const message = codexSandbox
    ? `The Codex sandbox blocked this command from connecting to Dorka (${systemCode}). Dorka may be running normally.`
    : `Permission denied connecting to Dorka (${systemCode}). Dorka may be running normally; this command's sandbox or OS permissions block the connection.`
  const retryStep = codexSandbox
    ? 'Re-run this command with escalated permissions, outside the Codex sandbox.'
    : 'Re-run this command outside its sandbox, or as a user allowed to reach the Dorka runtime.'
  return new RuntimeClientError('runtime_access_denied', message, {
    systemCode,
    nextSteps: [
      retryStep,
      "Do not restart Dorka or run 'dorka open'; a restart cannot grant this command access."
    ]
  })
}
