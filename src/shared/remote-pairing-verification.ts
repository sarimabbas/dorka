...
export function verifyRemotePairingRuntimeStatus(
  ...
  if (status.deviceScope === 'mobile') {
    return {
      ...
      message: 'This link grants mobile-only access. Generate a link for another Dorka client.'
    }
  }
  const versionFields = [
    ...
    return {
      ...
      message:
        compatibility.reason === 'client-too-old'
          ? 'Update this Dorka client before adding the remote host.'
          : 'Update Dorka on the remote host before adding it.'
    }
  }
