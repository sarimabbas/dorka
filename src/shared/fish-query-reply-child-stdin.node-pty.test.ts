/**
 * Real-fish regression for #13892: a terminal query reply Dorka held back is overtaken
...
const QUERY_GRAMMARS = [
  ...
  { re: /^\x1b\[>0?q/, reply: () => '\x1bP>|Dorka\x1b\\' },
...
describe('a held query reply never reaches the next child process (#13892)', () => {
  ...
  itWithFish(
    ...
    async () => {
      configHome = mkdtempSync(path.join(tmpdir(), 'dorka-fish-13892-'))
      ...
      const term = nodePty.spawn(FISH.path as string, ['-l', '-i'], {
        ...
        env: {
          ...
          DORKA_NODE_BIN: process.execPath,
          DORKA_CHILD_SCRIPT: childScript
        }
      })
      ...
      try {
        ...
        await sleep(150)
        term.write('"$DORKA_NODE_BIN" "$DORKA_CHILD_SCRIPT"\r')
        await sleep(1_500)
