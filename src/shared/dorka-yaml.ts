...
import type {
  DorkaDefaultTabTemplate,
  DorkaHooks,
  DorkaVmRecipe,
  DorkaVmRecipeDiagnostic
} from './dorka-yaml-hook-types'
import {
  isDorkaYamlFieldWithinLimit,
  isDorkaYamlTextWithinLimit,
...
function asTrimmedString(value: unknown): string | undefined {
  if (typeof value !== 'string' || !isDorkaYamlFieldWithinLimit(value)) {
    return undefined
  }
...
export const DORKA_VM_RECIPE_ID_PATTERN = /^[a-z0-9][a-z0-9._-]{0,63}$/
export const DORKA_VM_RECIPE_ID_RULE =
...
function normalizeDefaultTabs(value: unknown): DorkaDefaultTabTemplate[] {
  if (!Array.isArray(value) || value.length > MAX_DORKA_YAML_COLLECTION_ENTRIES) {
    return []
  }
  ...
  return value
    ...
    .filter((entry): entry is DorkaDefaultTabTemplate => entry !== null)
}

type VmRecipeParseResult = {
  recipes: DorkaVmRecipe[]
  diagnostics: DorkaVmRecipeDiagnostic[]
}

function normalizeVmRecipes(value: unknown): VmRecipeParseResult {
  const diagnostics: DorkaVmRecipeDiagnostic[] = []
  if (!Array.isArray(value)) {
    return { recipes: [], diagnostics }
  }
  if (value.length > MAX_DORKA_YAML_COLLECTION_ENTRIES) {
    return {
      recipes: [],
      diagnostics: [
        {
          index: MAX_DORKA_YAML_COLLECTION_ENTRIES,
          message: `At most ${MAX_DORKA_YAML_COLLECTION_ENTRIES} environment recipes are supported.`
        }
      ]
    }
  }
  ...
  const recipes = value
    .map((entry, index) => {
      ...
      if (!DORKA_VM_RECIPE_ID_PATTERN.test(id)) {
        diagnostics.push({
          index,
          field: 'id',
          message: `Invalid recipe id "${id}". ${DORKA_VM_RECIPE_ID_RULE}`
        })
        return null
      }
      if (seenIds.has(id)) {
      ...
      if (checkoutMode && checkoutMode !== 'dorka-worktree' && checkoutMode !== 'provisioned-root') {
        diagnostics.push({
          index,
          field: 'checkoutMode',
          message: `Recipe "${id}" checkoutMode must be "dorka-worktree" or "provisioned-root".`
        })
        return null
      }
    ...
    .filter((entry): entry is DorkaVmRecipe => entry !== null)
  return { recipes, diagnostics }
}

/**
 * Parse the supported project defaults from `dorka.yaml`.
 */
export function parseDorkaYaml(content: string): DorkaHooks | null {
  if (!isDorkaYamlTextWithinLimit(content)) {
    return null
  }
  ...
  try {
    ...
    root = document.toJS({ maxAliasCount: MAX_DORKA_YAML_ALIAS_COUNT })
  } catch {
    return null
  }
