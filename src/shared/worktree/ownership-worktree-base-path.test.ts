...
import { buildKnownDorkaWorkspaceLayouts, classifyWorktreeOwnership } from './ownership'
...
describe('repo-specific worktree ownership layouts', () => {
  ...
  it('resolves the same relative base path from each repo root', () => {
    ...
    expect(buildKnownDorkaWorkspaceLayouts(settings, repoA)[0]).toEqual({
      path: '/projects/a/worktrees',
      nestWorkspaces: true
    })
    expect(buildKnownDorkaWorkspaceLayouts(settings, repoB)[0]).toEqual({
    ...
    expect(
      classifyWorktreeOwnership({
        ...
        knownDorkaLayouts: buildKnownDorkaWorkspaceLayouts(settings, repoA)
      })
    ).toBe('external')
    expect(
      classifyWorktreeOwnership({
        ...
        settings,
        worktree: makeWorktree('/projects/a/worktrees/repo/feature'),
        knownDorkaLayouts: buildKnownDorkaWorkspaceLayouts(settings, repoB)
      })
    ).toBe('external')
  })
  ...
  it('resolves an absolute Linux base path of a WSL repo into its distro layout (STA-4772)', () => {
    ...
    expect(layouts[0]).toEqual({
      path: '//wsl.localhost/Ubuntu-24.04/home/jin/src/.dorka-worktrees',
      nestWorkspaces: true
    })
    expect(
      classifyWorktreeOwnership({
        ...
        worktree: makeWorktree(
          '\\\\wsl.localhost\\Ubuntu-24.04\\home\\jin\\src\\.dorka-worktrees\\repo\\feature'
        ),
  ...
  it('classifies worktrees under a dotted Linux base exactly where creation collapses it', () => {
    ...
    expect(layouts[0]).toEqual({
      path: '//wsl.localhost/Ubuntu-24.04/home/jin/.dorka-worktrees',
      nestWorkspaces: true
    })
    expect(
      classifyWorktreeOwnership({
        ...
        settings,
        worktree: makeWorktree(
          '\\\\wsl.localhost\\Ubuntu-24.04\\home\\jin\\.dorka-worktrees\\repo\\feature'
        ),
        knownDorkaLayouts: layouts
      })
    ).toBe('external')
  ...
  it('includes relative global layouts for SSH repos without applying absolute desktop paths', () => {
    ...
    expect(buildKnownDorkaWorkspaceLayouts(relativeSettings, repo)[0]).toEqual({
      path: '/remote/worktrees',
      nestWorkspaces: true
    })
    expect(
      buildKnownDorkaWorkspaceLayouts(absoluteSettings, repo).some(
