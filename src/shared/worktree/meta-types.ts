...
import type { EphemeralVmCheckoutMode } from '../dorka-yaml-hook-types'
import type {
...
import type { DorkaWorkspaceLayout } from '../global-settings-types'
...
export type WorktreeMeta = {
  ...
  /** See {@link Worktree.createdAt}. Persisted to dorka-data.json. */
  createdAt?: number
  /** See {@link Worktree.createdWithAgent}. Persisted to dorka-data.json. */
  ...
  baseRef?: string
  /** True when Dorka checked out a pre-existing local branch that delete must not prune. */
  preserveBranchOnDelete?: boolean
  /** See {@link Worktree.pushTarget}. Persisted so refreshed worktree lists keep the target. */
  pushTarget?: GitPushTarget
  /** Explicit marker stamped when Dorka creates the worktree. */
  dorkaCreatedAt?: number
  dorkaCreationSource?: 'desktop' | 'runtime' | 'cli' | 'ssh'
  /** Workspace layout active when Dorka created the worktree. */
  dorkaCreationWorkspaceLayout?: DorkaWorkspaceLayout
  ...
  /** System-owned provenance for workspaces created via `dorka worktree create`. */
