...
// Only the detail after it is Dorka's own wording, so verdict matchers anchor on the boundary
...
// making a different decision than one killing something Dorka just saw running. The marker
...
export function classifyWorktreeForceDeleteReason(
  ...
  if (isLockedWorktreeRemovalError(error)) {
    ...
    // unlocked explicitly rather than folded into Dorka's dirty-file force path.
    return null
  }
