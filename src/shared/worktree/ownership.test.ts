...
describe('worktree ownership classification', () => {
  it('treats explicit Dorka metadata as managed even outside the workspace root', () => {
    const repo = makeRepo()
    ...
    expect(
      classifyWorktreeOwnership({
        ...
        meta: makeMeta({ dorkaCreatedAt: 1 }),
        knownDorkaLayouts: buildKnownDorkaWorkspaceLayouts(settings, repo)
      })
    ).toBe('dorka-managed')
  })

  it('treats nested Dorka workspace paths without metadata as external', () => {
    const repo = makeRepo()
    const settings = makeSettings()
    const layouts = buildKnownDorkaWorkspaceLayouts(settings, repo)
    expect(
      classifyWorktreeOwnership({
        repo,
        settings,
        worktree: makeWorktree({ path: '/dorka/workspaces/app/feature' }),
    ...
    expect(
      classifyWorktreeOwnership({
        repo,
        settings,
        worktree: makeWorktree({ path: '/dorka/workspaces/other/feature' }),
        knownDorkaLayouts: layouts
      })
    ).toBe('external')
  })

  it('treats explicit Dorka creation layout metadata as managed', () => {
    const repo = makeRepo()
    const settings = makeSettings()
    expect(
      classifyWorktreeOwnership({
  ...
  it('does not treat metadata-free nested workspace paths as Dorka-managed for new repos', () => {
    ...
    const detected = toDetectedWorktree({
      ...
      worktree: makeWorktree({
        path: '/dorka/workspaces/app/manual-git-worktree',
  ...
  it('does not treat generic discovery metadata on nested workspace paths as Dorka-managed', () => {
    const repo = makeRepo({ externalWorktreeVisibility: 'hide' })
    const settings = makeSettings()
    const detected = toDetectedWorktree({
      repo,
  ...
  it('treats flat workspace-root descendants as unknown legacy without strong metadata', () => {
    ...
    expect(
      classifyWorktreeOwnership({
        repo,
        settings,
        worktree: makeWorktree({ path: '/dorka/workspaces/feature' }),
        knownDorkaLayouts: buildKnownDorkaWorkspaceLayouts(settings, repo)
      })
    ).toBe('unknown-legacy')
  })
  ...
  it('keeps flat-layout history weak after switching the same root to nested mode', () => {
    ...
    const settings = makeSettings({
      nestWorkspaces: true,
      workspaceDirHistory: [{ path: '/dorka/workspaces', nestWorkspaces: false }]
    })
    expect(
      classifyWorktreeOwnership({
        repo,
  ...
  it('handles Windows drive casing and separators', () => {
    ...
    const settings = makeSettings({ workspaceDir: 'C:\\Dorka\\Workspaces' })
    expect(
      classifyWorktreeOwnership({
        repo,
        settings,
        worktree: makeWorktree({
          id: 'repo-1::C:\\DORKA\\WORKSPACES\\App\\Feature',
          path: 'C:\\DORKA\\WORKSPACES\\App\\Feature',
          isMainWorktree: false
        }),
        knownDorkaLayouts: buildKnownDorkaWorkspaceLayouts(settings, repo)
      })
...
describe('agent scratch worktrees', () => {
  ...
  it('classifies scratch worktrees created inside another linked checkout', () => {
    const repo = makeRepo()
    const settings = makeSettings()
    const linkedCheckoutPath = '/dorka/workspaces/app/feature-x'
    expect(
      classifyWorktreeOwnership({
        repo,
        settings,
  ...
  it('keeps strong Dorka metadata authoritative over the scratch path match', () => {
    const repo = makeRepo()
    const settings = makeSettings()
    expect(
      classifyWorktreeOwnership({
