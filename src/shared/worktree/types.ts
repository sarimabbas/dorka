...
export type Worktree = {
  ...
  /** Set once when Dorka creates the worktree. Absent for worktrees discovered
  ...
  createdAt?: number
  /** Agent selected when Dorka originally created the worktree. Used only to
  ...
  baseRef?: string
  /** Remote/branch Dorka should publish review commits to when it created this worktree. */
...
/** Provenance for workspaces created through `dorka worktree create`. Absent on
...
export type CliWorkspaceProvenance = {
  kind: 'created-by-cli'
  createdAt: number
  /** Dorka terminal the CLI ran inside, when the caller had one — distinguishes
...
export type GitPushTarget = {
  ...
  branchName: string
  remoteUrl?: string
  /** True when Dorka added this remote while preparing a fork-PR worktree. */
...
export type WorktreeOwnership = 'dorka-managed' | 'external' | 'unknown-legacy' | 'agent-scratch'
