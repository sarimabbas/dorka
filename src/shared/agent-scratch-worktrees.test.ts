...
describe('isAgentScratchWorktreePath', () => {
  ...
  it('matches scratch worktrees created from a linked checkout', () => {
    const matchesAgentScratch = createAgentScratchWorktreePathMatcher(
      [repoPath, '/Users/dev/dorka/workspaces/app/feature-x'],
      []
    )

    expect(
      matchesAgentScratch(
        '/Users/dev/dorka/workspaces/app/feature-x/.claude/worktrees/agent-a04ccaaa'
      )
    ).toBe(true)
  ...
  it('does not match user worktree conventions', () => {
    ...
    expect(isAgentScratchWorktreePath('/Users/dev/app', '/dorka/workspaces/app/feature', [])).toBe(
      false
    )
  })
})
