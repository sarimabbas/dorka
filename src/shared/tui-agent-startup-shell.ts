...
/**
 * `'posix'` covers every Unix shell Dorka can type into, fish included — not
 ...
 * is deliberately no fish member: a dialect Dorka has to detect is a dialect it
...
export function tokenizeStartupCommand(
  ...
  // Why one Unix parse: the input is a string the user typed into an Dorka
...
/**
 * Quotes one argument so the SAME text is literal in every Unix shell Dorka can
 ...
 * of them, and Dorka never has to know which shell will read the line.
 *
 ...
 *
 * Why not a helper function defined by Dorka's shell wrappers: Dorka only wraps
 ...
 * shells Dorka never spawned — in all of those a helper would be `command not
 ...
 * clipboard command, which a user may run with no Dorka-injected value at all.
