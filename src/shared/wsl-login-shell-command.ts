...
export function buildWslLoginShellCommand(command: string): string {
  ...
  return [
    '_dorka_wsl_shell=$(getent passwd "$(id -un)" 2>/dev/null | cut -d: -f7)',
    'if [ -z "$_dorka_wsl_shell" ] || [ ! -x "$_dorka_wsl_shell" ]; then',
    '  _dorka_wsl_shell="${SHELL:-/bin/bash}"',
    'fi',
    'if [ -z "$_dorka_wsl_shell" ] || [ ! -x "$_dorka_wsl_shell" ]; then',
    '  _dorka_wsl_shell=/bin/sh',
    'fi',
    '_dorka_wsl_shell_name=$(basename "$_dorka_wsl_shell" | tr "[:upper:]" "[:lower:]")',
    'case "$_dorka_wsl_shell_name" in',
    `  sh|dash) exec "$_dorka_wsl_shell" -lc ${quotedCommand} ;;`,
    `  bash|zsh|ksh|mksh|ash) exec "$_dorka_wsl_shell" -ilc ${quotedCommand} ;;`,
...
export function buildWslCapturedLoginShellCommand(
  ...
  const begin = `__DORKA_WSL_CAPTURE_BEGIN_${nonce}__`
  const end = `__DORKA_WSL_CAPTURE_END_${nonce}__`
  return {
    beginMarker: begin,
    endMarker: end,
    command: buildWslLoginShellCommand(
      [
        ...
        command,
        '_dorka_capture_status=$?',
        `printf %s ${quotePosixShell(end)}`,
        'exit $_dorka_capture_status'
      ].join('\n')
    ),
    readStdout: (stdout) => {
...
export function buildWslInteractiveLoginShellCommand(): string {
  return [
    ...
    // resolved root and WSLENV /p-translates it. The DORKA_USER_DATA_PATH branch
    // stays as the fallback for an older host that exports only that.
    'if [ -n "${DORKA_SHELL_READY_ROOT:-}" ]; then',
    '  _dorka_shell_ready_root="${DORKA_SHELL_READY_ROOT%/}"',
    'elif [ -n "${DORKA_USER_DATA_PATH:-}" ]; then',
    '  _dorka_shell_ready_root="${DORKA_USER_DATA_PATH%/}/shell-ready"',
    'fi',
    '_dorka_wsl_shell_name=$(basename "$_dorka_wsl_shell" | tr "[:upper:]" "[:lower:]")',
    'case "$_dorka_wsl_shell_name" in',
    '  bash)',
    '    if [ -n "${_dorka_shell_ready_root:-}" ] && [ -f "${_dorka_shell_ready_root}/bash/rcfile" ]; then',
    '      exec "$_dorka_wsl_shell" --rcfile "${_dorka_shell_ready_root}/bash/rcfile"',
    '    fi',
    '    ;;',
    '  zsh)',
    '    if [ -n "${_dorka_shell_ready_root:-}" ] && [ -d "${_dorka_shell_ready_root}/zsh" ]; then',
    '      export ZDOTDIR="${_dorka_shell_ready_root}/zsh"',
    '    fi',
    '    ;;',
    'esac',
    'exec "$_dorka_wsl_shell" -l'
  ].join('\n')
}
