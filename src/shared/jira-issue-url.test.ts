...
function issue(overrides: Partial<JiraIssue> = {}): JiraIssue {
  return {
    ...
    project: { id: '10', key: 'DORKA', name: 'Dorka' },
...
describe('parseJiraIssueUrl', () => {
  it.each([
    [
      'https://company.atlassian.net/browse/dorka-123?focusedCommentId=1#comment',
      { issueKey: 'DORKA-123', origin: 'https://company.atlassian.net', sitePath: '' }
    ],
    [
  ...
  it.each([
    'DORKA-123',
    '/browse/DORKA-123',
    'ftp://jira.example.com/browse/DORKA-123',
    'https://user:secret@jira.example.com/browse/DORKA-123',
    'https://jira.example.com/browse/123',
    'https://jira.example.com/browse/-123',
    'https://jira.example.com/browse/DORKA_123',
    'https://jira.example.com/browse/DORKA-X',
    'https://jira.example.com/browse/DORKA-123/extra',
    'https://jira.example.com/browse/DORKA-123/'
  ])('rejects %s', (value) => {
...
describe('Jira site and issue matching', () => {
  it('matches the complete origin and base path while retaining duplicate accounts', () => {
    const parsed = parseJiraIssueUrl('https://jira.company.com:8443/jira/browse/DORKA-123')!
  ...
  it('requires the key, site id, and canonical URL site to agree', () => {
    const parsed = parseJiraIssueUrl('https://company.atlassian.net/browse/DORKA-123')!
    ...
    expect(isResolvedJiraIssueMatch(parsed, connectedSite, issue({ key: 'DORKA-124' }))).toBe(false)
    expect(isResolvedJiraIssueMatch(parsed, connectedSite, issue({ siteId: 'other' }))).toBe(false)
    expect(
      isResolvedJiraIssueMatch(
        parsed,
        connectedSite,
        issue({ url: 'https://other.atlassian.net/browse/DORKA-123' })
      )
    ).toBe(false)
  })
})
