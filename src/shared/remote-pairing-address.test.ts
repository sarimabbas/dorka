...
describe('remote pairing address', () => {
  ...
  it('extracts a sanitized display endpoint without credentials', () => {
    expect(parseHostAccessLink(accessLink('wss://dorka.example.com/runtime'))).toEqual({
      ok: true,
      value: {
        pairing: expect.objectContaining({ endpoint: 'wss://dorka.example.com/runtime' }),
        displayEndpoint: 'dorka.example.com',
        endpointKind: 'public'
      }
    })
  })
  ...
  it('rejects invalid and unsupported endpoints', () => {
    ...
    expect(parseHostAccessLink(accessLink('https://dorka.example.com'))).toMatchObject({
      ok: false,
      kind: 'unsupported-destination'
    })
    expect(parseHostAccessLink(accessLink('wss://dorka.example.com/#fragment'))).toMatchObject({
      ok: false,
      kind: 'unsupported-destination'
    })
    ...
    expect(parseHostAccessLink(accessLink('wss://dorka.example.com:0'))).toMatchObject({
      ok: false,
      kind: 'non-connectable-destination'
    })
  })
  ...
  it('rejects mobile-only access grants', () => {
    const link = encodePairingOffer({
      v: PAIRING_OFFER_VERSION,
      endpoint: 'wss://dorka.example.com',
      deviceToken: 'token',
      publicKeyB64: 'key',
      scope: 'mobile'
    })
