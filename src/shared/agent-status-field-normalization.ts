...
import {
  ...
  isDorkaDispatchStatusPrompt
} from './dorka-dispatch-status-prompt'
...
/** Normalize the agent prompt field, compacting Dorka dispatch preambles. */
export function normalizePromptField(value: unknown): string {
  if (typeof value !== 'string') {
    return ''
  }
  if (isDorkaDispatchStatusPrompt(value)) {
