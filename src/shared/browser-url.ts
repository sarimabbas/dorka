...
export function normalizeBrowserNavigationUrl(
  ...
  if (trimmed.length === 0 || trimmed === 'about:blank' || trimmed === DORKA_BROWSER_BLANK_URL) {
    return DORKA_BROWSER_BLANK_URL
  }
...
export function normalizeExternalBrowserUrl(rawUrl: string): string | null {
  ...
  if (normalized === null || normalized === DORKA_BROWSER_BLANK_URL) {
    return null
  }
