...
/**
 * The community `skills` CLI's own `--agent` key for each agent Dorka detects.
 *
 ...
 * rather than guessed. Dorka ids and skills keys agree less often than they look
 * (`claude` is `claude-code`, `rovo` is `rovodev`, `aug` is `augment`), and some
 * near-matches are different products — Dorka's `aider` CLI is not the CLI's
 * `aider-desk`, and Dorka's `openclaude` is its `openclaw` in name only, so it
 * follows Dorka's own rule that OpenClaude reads Claude-owned roots.
 */
export const SKILLS_CLI_AGENT_KEY_BY_TUI_AGENT = {
  claude: 'claude-code',
  'claude-agent-teams': 'claude-code',
  // Why: Dorka states OpenClaude reads Claude-owned roots (native-chat-agent-profiles).
  ...
  devin: 'devin',
  ante: null,
  // Why: Dorka detects trae by `traecli`, an alias only TRAE CN ships.
  trae: 'trae-cn',
  muse: null
 ...
 * so agents Dorka cannot map still receive the skill.
 */
...
/** Map detected Dorka agents onto `skills --agent` keys, plus the universal target. */
