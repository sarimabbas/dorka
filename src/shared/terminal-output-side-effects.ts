 ...
 * transport (`createPtyOutputProcessor`) and main's per-PTY tracker (`DorkaRuntimeService.onPtyData`).
...
export function createTerminalTitleTracker(
  ...
  function applyObservedTitle(rawTitle: string): void {
    // Why: cursor-agent re-emits its bare native title mid-turn; passing it through would stomp Dorka's synthesized spinner state.
