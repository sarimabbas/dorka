...
export function encodePairingOffer(offer: PairingOffer): string {
  ...
  return `dorka://pair?code=${base64url}`
}
...
export function decodePairingOffer(url: string): PairingOffer {
  ...
  if (!code) {
    throw new Error('Invalid pairing URL: must start with dorka://pair and include a pairing code')
  }
...
function extractPairingCodeFromUrl(url: string): string | null {
  ...
  // Why: prefix checks accepted routes like `dorka://pairing?...`; only the
  // pairing deep-link host may carry runtime auth material.
  if (parsed.protocol !== 'dorka:' || parsed.hostname !== 'pair') {
    return null
  }
...
// Why: accept either an `dorka://pair?...` URL or the bare base64
...
export function parsePairingCode(input: string): PairingOffer | null {
  ...
  try {
    if (trimmed.toLowerCase().startsWith('dorka://')) {
