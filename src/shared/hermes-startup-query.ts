...
const QUERY_PLACEHOLDER = '__DORKA_HERMES_STARTUP_QUERY__'
const QUERY_ARG_PLACEHOLDER = `--query=${QUERY_PLACEHOLDER}`
const POSIX_QUERY_VARIABLE = '__dorka_hermes_startup_query'
const POWERSHELL_QUERY_VARIABLE = 'dorkaHermesStartupQuery'
const POWERSHELL_NATIVE_QUERY_VARIABLE = 'dorkaHermesNativeQuery'

export const DORKA_HERMES_STARTUP_QUERY_ENV = 'DORKA_HERMES_STARTUP_QUERY'
...
function stripDorkaOwnedHermesArgs(args: readonly string[]): string[] {
...
function normalizeHermesArgv(
  ...
  return [
    ...
    ...stripDorkaOwnedHermesArgs(baseArgsWithoutChat),
    ...stripDorkaOwnedHermesArgs(configuredArgsWithoutChat),
    '--tui'
  ]
}
...
function buildQueryCommand(argv: string[], shell: AgentStartupShell): string {
  if (!isPosixStartupShell(shell)) {
    ...
    const script = `$${POWERSHELL_QUERY_VARIABLE} = $env:${DORKA_HERMES_STARTUP_QUERY_ENV}; $${POWERSHELL_NATIVE_QUERY_VARIABLE} = $${POWERSHELL_QUERY_VARIABLE} -replace '(\\\\*)"', '$1$1\\"'; Remove-Item Env:${DORKA_HERMES_STARTUP_QUERY_ENV} -ErrorAction SilentlyContinue; ${invocation}`
  ...
  const script = `${POSIX_QUERY_VARIABLE}="\${${DORKA_HERMES_STARTUP_QUERY_ENV}}"; unset ${DORKA_HERMES_STARTUP_QUERY_ENV}; eval "$(printf %b "${encodedInvocation}")"`
...
export function planHermesStartupQuery(args: {
  ...
  const env = {
    ...args.agentEnv,
    [DORKA_HERMES_STARTUP_QUERY_ENV]: args.prompt
  }
