...
// the relay sends to Dorka. Consumed by `src/relay/agent-hook-server.ts` (which
...
// between Dorka's main process and the remote relay.
//
// Invariants both ends rely on:
// - The relay normalizes; Dorka routes. The envelope's `payload` field has
//   already been through `normalizeHookPayload` (which calls
//   `parseAgentStatusPayload` → `normalizeAgentStatusObject`) on the relay
//   side. Dorka's `ingestRemote` re-runs the canonical payload normalizer at
...
// - The wire `connectionId` is **always `null`**: a `connectionId` is Dorka's
//   local handle on an `ssh2` connection, not a wire identity. Dorka stamps the
...
//   POST body so Dorka's warn-once protocol diagnostics still fire. The relay
...
// with the same value so Dorka can attribute the event back to the right CLI.
...
/** Wire envelope for a single hook event flowing relay → Dorka. */
export type AgentHookRelayEnvelope = {
  ...
  tabId?: string
  worktreeId?: string
  /** Always `null` on the wire — relay does not know Dorka's local connectionId. */
  connectionId: null
  /** Preserved from the relay-side normalized hook event so Dorka can
  ...
  /** True when the relay is replaying its cache after Dorka reconnects. */
  isReplay?: boolean
  ...
  env?: string
  /** Forwarded verbatim from the agent CLI POST body. Lets Dorka's warn-once
   ...
   *  Dorka's `ingestRemote` validates it again at the SSH trust boundary. */
...
/**
 * Re-attaches shed fields from Dorka's cached payload for this pane, so a transport-level
...
/** JSON-RPC request method Dorka issues after `--connect` reattach to ask the
 *  relay to replay its per-paneKey last-payload cache. Pull, not push: a relay
 *  that pushed on `setWrite` can emit before Dorka has wired its `agent.hook`
...
/** JSON-RPC request method Dorka issues at session-ready to ship the
...
export type AgentHookInstallManagedHooksParams = {
  /** SHA-256 fingerprint of the server key negotiated by Dorka's SSH transport. */
...
/** Feature-flag env var. Read once at process start by Dorka and the relay.
 *  Remote agent hooks ship as the default SSH behavior; set "0" to opt out. */
export const DORKA_FEATURE_REMOTE_AGENT_HOOKS_ENV = 'DORKA_FEATURE_REMOTE_AGENT_HOOKS' as const

export function isRemoteAgentHooksEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  const raw = env[DORKA_FEATURE_REMOTE_AGENT_HOOKS_ENV]
  if (raw === undefined) {
    return true
  }
