...
export const WSL_HOOK_RELAY_DIR = '.dorka-wsl/hook-relay'
 ...
 *  namespaces the guest install dir, so concurrent Dorka instances with
 *  different bundle versions (dev + prod) never reinstall over each other. */
export const WSL_HOOK_RELAY_VERSION_ENV = 'DORKA_WSL_HOOK_RELAY_VERSION'
 ...
 *  after an Dorka restart — the exact re-coordination this exists to serve. */
export const WSL_HOOK_RELAY_INSTANCE_ENV = 'DORKA_WSL_HOOK_INSTANCE'
...
export function wslHookRelayEndpointDir(guestHome: string, instanceKey: string): string {
  ...
  return `${home}/.dorka-wsl/agent-hooks/instance-${instanceKey}`
}
