...
describe('subscribeRemoteRuntimeRequest', () => {
  it('includes WebSocket close details when subscription admission is rejected', async () => {
    ...
    await expect(
      ...
      'Remote Dorka runtime closed the connection (1013: Maximum connections reached).'
    )
  })
...
describe('sendRemoteRuntimeRequest', () => {
  ...
  it('classifies a non-Dorka handshake as a host identity mismatch', async () => {
...
async function createInvalidHandshakeServer(): Promise<{ pairing: PairingOffer }> {
  ...
  servers.push(wss)
  wss.on('connection', (ws) => {
    ws.once('message', () => ws.send(JSON.stringify({ type: 'not_dorka' })))
  })
