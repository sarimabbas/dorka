...
describe('getAgentRowConversationName', () => {
  ...
  it('rejects spinner+cwd titles instead of surfacing paths as names', () => {
    expect(
      getAgentRowConversationName(makeTab({ title: '⠋ ~/dorka/workspaces' }), 'codex', false)
    ).toBeNull()
    expect(
      getAgentRowConversationName(makeTab({ title: '/Users/dev/repo' }), 'codex', false)
    ).toBeNull()
    expect(
      getAgentRowConversationName(makeTab({ title: 'C:\\repos\\dorka' }), 'codex', false)
    ).toBeNull()
    expect(
      getAgentRowConversationName(makeTab({ title: 'dorka/workspaces' }), 'codex', false)
    ).toBeNull()
    expect(
      getAgentRowConversationName(
        makeTab({ title: '\\\\wsl.localhost\\Ubuntu\\home\\dev\\dorka' }),
        'codex',
        false
      )
    ).toBeNull()
    expect(
      getAgentRowConversationName(makeTab({ title: 'repos\\dorka' }), 'codex', false)
    ).toBeNull()
  })
  ...
  it('rejects identity-echo, management, and placeholder titles', () => {
    ...
    expect(
      getAgentRowConversationName(makeTab({ title: '◇ Ready (dorka)' }), 'gemini', false)
    ).toBeNull()
    expect(
