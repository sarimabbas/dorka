// Why: full Dorka dispatch preambles are multi-KB (CLI instructions before
...
export const DORKA_DISPATCH_STATUS_PREAMBLE_PREFIX =
  'You are working inside Dorka, a multi-agent IDE.'
export const DORKA_DISPATCH_STATUS_TASK_MARKER = '=== TASK ==='
const DORKA_DISPATCH_STATUS_TASK_ID_MARKER = 'Your task ID is:'
...
const DORKA_DISPATCH_STATUS_SOURCE_SCAN_LIMIT = 24_576

export function isDorkaDispatchStatusPrompt(value: string): boolean {
  ...
  const scanEnd = Math.min(value.length, DORKA_DISPATCH_STATUS_SOURCE_SCAN_LIMIT)
  let start = 0
  while (start < scanEnd && isEcmaTrimWhitespace(value.charCodeAt(start))) {
    start++
  }
  return (
    start + DORKA_DISPATCH_STATUS_PREAMBLE_PREFIX.length <= scanEnd &&
    value.startsWith(DORKA_DISPATCH_STATUS_PREAMBLE_PREFIX, start)
  )
}
...
export function compactDispatchPromptForStatus(
  ...
  let taskId = ''
  const idMarkerIndex = scan.indexOf(DORKA_DISPATCH_STATUS_TASK_ID_MARKER)
  if (idMarkerIndex !== -1) {
    const afterId = scan.slice(idMarkerIndex + DORKA_DISPATCH_STATUS_TASK_ID_MARKER.length)
    let idStart = 0
  ...
  if (taskMarkerIndex !== -1) {
    const body = scan.slice(taskMarkerIndex + DORKA_DISPATCH_STATUS_TASK_MARKER.length)
  ...
  // Why: keep the dispatch prefix (isDorkaDispatchPrompt) + task id (label match)
  // + task body (fallback preview) so UI helpers still work on the 200-char field.
  let compact = DORKA_DISPATCH_STATUS_PREAMBLE_PREFIX
  if (taskId) {
    compact += ` ${DORKA_DISPATCH_STATUS_TASK_ID_MARKER} ${taskId}`
  }
  if (taskBody) {
    compact += ` ${DORKA_DISPATCH_STATUS_TASK_MARKER} ${taskBody}`
  }
  return normalizeSingleLine(compact, maxLength)
}

/**
 * Locate the Dorka task separator in a dispatch prompt scan window.
 * Why: base-drift commit subjects are repository-controlled and may mention
 * `=== TASK ===`. Raw multi-line preambles must use the standalone line Dorka
 ...
 */
export function findDorkaDispatchTaskMarkerIndex(value: string): number {
  let searchFrom = 0
  while (searchFrom < value.length) {
    const markerIndex = value.indexOf(DORKA_DISPATCH_STATUS_TASK_MARKER, searchFrom)
    if (markerIndex === -1) {
      break
    }
    const markerEnd = markerIndex + DORKA_DISPATCH_STATUS_TASK_MARKER.length
  ...
  return value.includes('\n') || value.includes('\r')
    ? -1
    : value.indexOf(DORKA_DISPATCH_STATUS_TASK_MARKER)
}
