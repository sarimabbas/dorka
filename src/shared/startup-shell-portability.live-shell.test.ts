 ...
 * ONE emitted string is correct in sh, bash, zsh, dash and fish alike, so Dorka
...
/** Every Unix shell on this machine a queued Dorka command line could land in. */
...
const SANDBOX_HOME = mkdtempSync(path.join(tmpdir(), 'dorka-shell-portability-'))

/** Env with no user shell config reachable, so only Dorka's own text is exercised. */
  ...
  () => {
    ...
    describe.each(SHELLS)('$name', (shell) => {
      ...
      it('clears an exported variable with no wrapper installed', () => {
        const clear = clearEnvCommand('DORKA_PI_PREFILL', 'posix')
        const probe =
          shell.name === 'fish'
            ? `set -gx DORKA_PI_PREFILL draft; ${clear}; set -q DORKA_PI_PREFILL; and echo STILL; or echo CLEARED`
            : `DORKA_PI_PREFILL=draft; export DORKA_PI_PREFILL; ${clear}; echo "\${DORKA_PI_PREFILL:+STILL}\${DORKA_PI_PREFILL:-CLEARED}"`
        expect(runInShell(shell, probe).trim()).toBe('CLEARED')
      })

      it('clears several variables in one statement', () => {
        const clear = clearEnvCommand(['DORKA_A', 'DORKA_B'], 'posix')
        const probe =
          shell.name === 'fish'
            ? `set -gx DORKA_A 1; set -gx DORKA_B 2; ${clear}; set -q DORKA_A; or set -q DORKA_B; and echo STILL; or echo CLEARED`
            : `DORKA_A=1 DORKA_B=2; export DORKA_A DORKA_B; ${clear}; echo "\${DORKA_A:+STILL}\${DORKA_B:+STILL}\${DORKA_A:-CLEARED}"`
        expect(runInShell(shell, probe).trim()).toBe('CLEARED')
      })
      ...
      it.each([
        ...
        const setUp = preset
          ? shell.name === 'fish'
            ? 'set -gx DORKA_PI_PREFILL draft; '
            : 'DORKA_PI_PREFILL=draft; export DORKA_PI_PREFILL; '
          : ''
      ...
      // interactive shell, which is the mode Dorka types into — a `-c` run cannot
      ...
      it.runIf(['bash', 'zsh'].includes(shell.name))(
        ...
        () => {
          ...
          const script = [
            ...
            'DORKA_PI_PREFILL=draft; export DORKA_PI_PREFILL',
            clear,
            'echo "RESULT=${DORKA_PI_PREFILL:+STILL}${DORKA_PI_PREFILL:-CLEARED}"'
          ].join('\n')
      ...
      // observed. Interactively — the mode Dorka types into — they survive without
      ...
      it.runIf(shell.name === 'fish')('never deletes a lone universal variable', () => {
        const clear = clearEnvCommand('DORKA_PI_PREFILL', 'posix')
        const probe = `set -Ux DORKA_PI_PREFILL persisted; ${clear}; set -q DORKA_PI_PREFILL; and echo "RESULT=$DORKA_PI_PREFILL"; or echo RESULT=DESTROYED; set -Ue DORKA_PI_PREFILL`
      ...
      it.runIf(shell.name === 'fish')('reveals a shadowed universal instead of deleting it', () => {
        const clear = clearEnvCommand('DORKA_PI_PREFILL', 'posix')
        const probe = `set -Ux DORKA_PI_PREFILL mine; set -gx DORKA_PI_PREFILL injected; ${clear}; echo "RESULT=$DORKA_PI_PREFILL"; set -Ue DORKA_PI_PREFILL`
