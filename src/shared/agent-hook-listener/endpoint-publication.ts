...
export function writeEndpointFile(
  ...
  const valuesToWrite: [string, string][] = [
    ['DORKA_AGENT_HOOK_PORT', String(fields.port)],
    ['DORKA_AGENT_HOOK_TOKEN', fields.token],
    ['DORKA_AGENT_HOOK_ENV', fields.env],
    ['DORKA_AGENT_HOOK_VERSION', fields.version]
  ]
  if (fields.transport) {
    valuesToWrite.push(['DORKA_AGENT_HOOK_TRANSPORT', fields.transport])
  }
