import { DORKA_HOOK_PROTOCOL_VERSION } from '../agent-hook-types'
...
export function warnOnHookEnvOrVersionMismatch(
  ...
  if (
    ...
    console.warn(
      `[agent-hooks] received hook v${version}; server expects v${DORKA_HOOK_PROTOCOL_VERSION}. ` +
    ...
    if (!state.warnedEnvs.has(key) && state.warnedEnvs.size < MAX_WARNED_KEYS) {
      ...
      console.warn(
        `[agent-hooks] received ${env} hook on ${expectedEnv} server. ` +
          'Likely a stale terminal from another Dorka install.'
      )
    }
  }
}
