...
export class RemoteRuntimeRequestResponseRouter<TResult> {
  ...
  private handleRpcFrame(plaintext: string): void {
    ...
    try {
      ...
      this.invalidResponse('Remote Dorka runtime returned an invalid response frame.')
      return
    }
    ...
    if (parsed.data.id !== this.awaitingRequestId) {
      this.invalidResponse('Remote Dorka runtime returned a mismatched response id.')
      return
    }
