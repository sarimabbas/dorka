...
const EphemeralVmRecipeDorkaServerConnectionSchema = z
  .object({
    type: z.literal('dorka-server'),
...
export const EphemeralVmRecipeConnectionSchema = z.discriminatedUnion('type', [
  EphemeralVmRecipeDorkaServerConnectionSchema,
...
export function parseEphemeralVmRecipeResult(stdout: string): EphemeralVmRecipeResultParseResult {
  ...
  if (connection.type === 'dorka-server' && !parsePairingCode(connection.pairingCode)) {
    return { ok: false, error: 'Recipe result pairingCode is not a valid Dorka pairing code.' }
  }
...
export function getEphemeralVmRecipeResultCheckoutMode(
  result: EphemeralVmRecipeResult
): 'dorka-worktree' | 'provisioned-root' {
  return result.schemaVersion === 2 ? 'provisioned-root' : 'dorka-worktree'
}
...
export function getEphemeralVmRecipeResultPairingCode(
  ...
  return connection.type === 'dorka-server' ? connection.pairingCode : null
}
