...
const FUNCTION_NAME = '__dorka_omp_draft'
...
export function withOmpDraftCleanup(command: string, shell: AgentStartupShell): string {
  if (shell !== 'posix') {
    ...
    return `${launch}${commandSeparator(shell)}${clearEnvCommand('DORKA_OMP_PREFILL', shell)}`
  }
  const fish = `function ${FUNCTION_NAME}; ${command}; set -l __dorka_status $status; set -e -g DORKA_OMP_PREFILL; return $__dorka_status; end`
  const posix = `${FUNCTION_NAME}() { ${command}; set -- "$?"; unset DORKA_OMP_PREFILL; return "$1"; }`
