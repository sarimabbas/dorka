...
class DorkaUnicodeProvider implements IUnicodeVersionProvider {
  public readonly version = DORKA_UNICODE_VERSION
...
export function activateDorkaTerminalUnicodeProvider(terminal: XtermTerminalWithUnicodeCore): void {
  const { unicode } = terminal
  if (unicode.activeVersion === DORKA_UNICODE_VERSION) {
    return
  }
  ...
  if (!unicode.versions.includes(DORKA_UNICODE_VERSION)) {
    unicode.register(new DorkaUnicodeProvider(baseProvider))
  }
  unicode.activeVersion = DORKA_UNICODE_VERSION
}
