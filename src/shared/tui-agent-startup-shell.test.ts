...
describe('one Unix startup dialect', () => {
  it('clears variables with a self-contained branch, not a per-shell builtin', () => {
    ...
    // Dorka only wraps zsh/bash/fish, so an `sh`/`dash`/`ksh` login shell — and
  ...
  it('clears an agent draft prefill variable with the portable teardown', () => {
    ...
    expect(plan?.launchCommand).toBe(
      `pi; command test -n "$fish_pid" && set --erase -g DORKA_PI_PREFILL; command test -z "$fish_pid" && unset DORKA_PI_PREFILL; true`
    )
    expect(plan?.env?.DORKA_PI_PREFILL).toBe('hello')
  })
})
