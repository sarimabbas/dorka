...
export class RepoRefMaintenance {
   ...
   * pack, or yielding to work Dorka asked us to yield to, does not: both end on
  ...
  private async packIfNeeded(
    ...
    // Dorka sessions a branch moving mid-pack is the normal case, and Git's
  ...
  /** Record an aborted attempt: retry soon if Dorka yielded, back off if it stalled. */
  private yieldTo(
    key: string,
    tracked: TrackedRepo,
    span: RefMaintenanceSpan,
