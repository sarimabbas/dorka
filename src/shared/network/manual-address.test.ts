...
describe('parseManualNetworkAddress', () => {
  ...
  describe('full URL', () => {
    ...
    it('accepts reverse-proxy paths and query strings', () => {
      expect(parseManualNetworkAddress('wss://example.com/dorka?route=runtime').ok).toBe(true)
    })

    it('accepts HTTP URLs that the main process normalizes to WebSocket URLs', () => {
      expect(parseManualNetworkAddress('http://example.com/dorka').ok).toBe(true)
      expect(parseManualNetworkAddress('https://example.com/dorka?route=runtime').ok).toBe(true)
    })
