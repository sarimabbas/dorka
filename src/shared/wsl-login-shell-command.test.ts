...
describe('wsl login shell command helpers', () => {
  ...
  it('runs commands through the distro user login shell', () => {
    ...
    expect(command).toContain('exec "$_dorka_wsl_shell" -ilc')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      const root = mkdtempSync(join(tmpdir(), 'dorka-wsl-login-codex-'))
      ...
      writeFileSync(
        loginShell,
        '#!/bin/sh\nexport PATH="$DORKA_TEST_CODEX_BIN:/usr/bin:/bin"\nexec /bin/sh -c "$2"\n'
      )
  ...
  it('keeps command-scoped environment variables in the quoted payload', () => {
    const command = buildWslLoginShellCommand('HISTFILE=/tmp/dorka-history printf "$HISTFILE"')

    expect(command).toContain('\'HISTFILE=/tmp/dorka-history printf "$HISTFILE"\'')
  ...
  describe('captured login shell', () => {
      ...
      expect(
        captured.readStdout(
          'To run a command as administrator (user "root"), use "sudo <command>".\n\n' +
            '__DORKA_WSL_CAPTURE_BEGIN_nonce1__directory__DORKA_WSL_CAPTURE_END_nonce1__'
        )
      ).toBe('directory')
    })
    ...
    it('emits the status plumbing the payload needs', () => {
      ...
      expect(captured.command).toContain('_dorka_capture_status=$?')
      expect(captured.command).toContain('exit $_dorka_capture_status')
    })
    ...
    it('keeps payload bytes that themselves contain a fence', () => {
      ...
      expect(
        captured.readStdout(
          `banner\n__DORKA_WSL_CAPTURE_BEGIN_nonce2__${payload}__DORKA_WSL_CAPTURE_END_nonce2__`
        )
      ).toBe(payload)
    })
    ...
    it('returns the tail when a payload exits before the closing fence', () => {
      ...
      expect(captured.readStdout('banner\n__DORKA_WSL_CAPTURE_BEGIN_nonce1__partial')).toBe(
        'partial'
      )
    })
  ...
  it('starts an interactive login shell without assuming bash', () => {
    ...
    expect(command).toContain('getent passwd')
    expect(command).toContain('if [ -z "$_dorka_wsl_shell" ] || [ ! -x "$_dorka_wsl_shell" ]; then')
    expect(command).toContain('_dorka_shell_ready_root=""')
    expect(command).toContain('if [ -n "${DORKA_USER_DATA_PATH:-}" ]; then')
    expect(command).toContain('_dorka_wsl_shell_name=$(basename "$_dorka_wsl_shell"')
    expect(command).toContain('bash)')
    expect(command).toContain('--rcfile "${_dorka_shell_ready_root}/bash/rcfile"')
    expect(command).toContain('zsh)')
    expect(command).toContain('export ZDOTDIR="${_dorka_shell_ready_root}/zsh"')
    expect(command).toContain('exec "$_dorka_wsl_shell" -l')
    expectValidShSyntax(command)
  })
})
...
describe('in-guest wrapper root resolution', () => {
  ...
  // rebuilt the root as `${DORKA_USER_DATA_PATH}/shell-ready`, which stopped
  ...
  it('prefers the host-published root over the legacy user-data guess', () => {
    ...
    expect(script).toContain('if [ -n "${DORKA_SHELL_READY_ROOT:-}" ]; then')
    expect(script).toContain('_dorka_shell_ready_root="${DORKA_SHELL_READY_ROOT%/}"')
    // The legacy branch must remain reachable only as a fallback, so an older
    // host that exports just DORKA_USER_DATA_PATH still wraps its shells.
    expect(script).toContain('elif [ -n "${DORKA_USER_DATA_PATH:-}" ]; then')
  })
  ...
  it('resolves the published root ahead of the legacy path under a real shell', () => {
    ...
    const prologue = script.split('_dorka_wsl_shell_name=')[0] as string
    const probe = [
      'DORKA_SHELL_READY_ROOT=/mnt/c/ud/shell-wrappers/deadbeefdeadbeef/shell-ready',
      'DORKA_USER_DATA_PATH=/mnt/c/ud',
      'export DORKA_SHELL_READY_ROOT DORKA_USER_DATA_PATH',
      prologue,
      'printf "%s" "$_dorka_shell_ready_root"'
    ].join('\n')
