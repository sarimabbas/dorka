    ...
    if (plaintext === null) {
      this.options.fail(
        new RemoteRuntimeClientError(
          ...
          'Remote Dorka runtime returned an undecryptable frame.'
        )
      )
      return
    }
    ...
    if (readyFrame !== 'ready') {
      this.options.fail(
        new RemoteRuntimeClientError(
          'invalid_runtime_response',
          readyFrame === 'invalid'
            ? 'Remote Dorka runtime returned an invalid E2EE handshake frame.'
            : 'Remote Dorka runtime returned an unexpected E2EE handshake frame.'
        )
      )
      return
    }
  ...
  private handleAuthenticatedFrame(plaintext: string): void {
    ...
    if (authenticated.kind === 'invalid') {
      this.options.fail(
        new RemoteRuntimeClientError(
          'invalid_runtime_response',
          'Remote Dorka runtime returned an invalid E2EE auth frame.'
        )
      )
      return
    }
    if (authenticated.kind !== 'authenticated') {
      ...
      this.options.fail(
        new RemoteRuntimeClientError(code, 'Remote Dorka runtime rejected the pairing token.')
      )
      return
    }
    this.state = 'ready'
  ...
  private handleRpcFrame(plaintext: string): void {
    ...
    try {
      ...
      this.options.fail(
        new RemoteRuntimeClientError(
          'invalid_runtime_response',
          'Remote Dorka runtime returned an invalid response frame.'
        )
      )
      return
    }
    ...
    this.options.fail(
      new RemoteRuntimeClientError(
        'invalid_runtime_response',
        'Remote Dorka runtime returned a mismatched response id.'
      )
    )
  }
  ...
  private handleBinaryFrame(frame: Uint8Array<ArrayBufferLike>): void {
    if (this.state !== 'ready') {
      this.options.fail(
        new RemoteRuntimeClientError(
          'invalid_runtime_response',
          'Remote Dorka runtime returned binary data before authentication.'
        )
      )
      return
    }
    ...
    if (plaintext === null) {
      this.options.fail(
        new RemoteRuntimeClientError(
          'invalid_runtime_response',
          'Remote Dorka runtime returned an undecryptable binary frame.'
        )
      )
      return
    }
