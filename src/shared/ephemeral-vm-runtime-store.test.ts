...
describe('ephemeral VM runtime store', () => {
  ...
  function makeUserDataPath(): string {
    const userDataPath = mkdtempSync(join(tmpdir(), 'dorka-ephemeral-vm-store-'))
  ...
  it('updates lifecycle and cleanup state', () => {
    ...
    const failed = updateEphemeralVmRuntimeStatus(userDataPath, 'dorka-instance-1', {
    ...
    const recovered = updateEphemeralVmRuntimeStatus(userDataPath, 'dorka-instance-1', {
      status: 'cleaned',
