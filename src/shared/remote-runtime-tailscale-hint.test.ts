...
const UNREACHABLE = 'Could not connect to the remote Dorka runtime.'
...
describe('isTailscaleEndpoint', () => {
  ...
  it('rejects non-Tailscale hosts and the surrounding 100.x space', () => {
    ...
    expect(isTailscaleEndpoint('wss://dorka.example.com')).toBe(false)
...
describe('withRemoteRuntimeTailscaleHint', () => {
  ...
  it('covers the close and timeout failure variants', () => {
    ...
    expect(
      withRemoteRuntimeTailscaleHint(
        'Timed out while connecting to the remote Dorka runtime.',
        'wss://host.ts.net'
      )
  ...
  it('leaves non-connectivity errors untouched', () => {
    const auth = 'Remote Dorka runtime rejected the pairing token.'
