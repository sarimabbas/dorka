...
// or a plugin's bundled copy. Dorka's global update never writes here, so the owner's
...
// Why: Dorka's updater only ever passes --global, so a copy a project owns is not
// something Dorka can act on — it has no remedy by design. Reporting it as global drift
...
// Why: a real read failure is a fact about the user's disk and stays actionable. Dorka's
