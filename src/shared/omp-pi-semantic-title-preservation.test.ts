 ...
 * `π ! <label>` — always the π glyph. On an Dorka-hosted pane OMP's native titler cedes
 * entirely to Dorka's OWN injected extension (src/main/pi/titlebar-extension-source.ts:21,44),
 * which writes `π - <session> - <cwd>` and `⠋ π - <session> - <cwd>` every 80ms.
 *
 * Both flapping strings were manufactured by Dorka:
...
const DORKA_EXTENSION_WORKING = (frame: string): string => `${frame} π - fixing the sidebar - dorka`
...
describe('normalizeTerminalTitle keeps the OMP/Pi session label', () => {
  it.each([
    ['Dorka extension (spinner leads)', DORKA_EXTENSION_WORKING],
  ...
  it('collapses frames under a multiplexer prefix', () => {
    const normalized = new Set(
      FRAMES.map((frame) => normalizeTerminalTitle(`zsh | ${DORKA_EXTENSION_WORKING(frame)}`))
    )
  ...
  it('keeps two different sessions distinguishable', () => {
    expect(normalizeTerminalTitle('π - session A - dorka')).not.toBe(
      normalizeTerminalTitle('π - session B - dorka')
    )
  })
})
...
describe('detectAgentStatusFromTitle reads the π state separator', () => {
  it.each([
    ...
    ['⠋ π - fixing the sidebar - dorka', 'working'],
    ['π: fixing the sidebar', 'idle']
...
describe('the churn is gone at the suppressor', () => {
  it('treats consecutive animation frames as decoration', () => {
    for (const build of [DORKA_EXTENSION_WORKING, OMP_NATIVE_WORKING]) {
  ...
  it('still commits a real working -> attention transition', () => {
    expect(
      isDecorativeAgentTitleFrameChange(
        normalizeTerminalTitle(DORKA_EXTENSION_WORKING('⠋')),
...
describe('Dorka stops writing over the working title it does not own', () => {
...
describe('the owner relabel keeps the label and swaps only the brand', () => {
  ...
  it.each([
    ['⠋ π - fixing the sidebar - dorka', '⠋ OMP - fixing the sidebar - dorka'],
    ['π - fixing the sidebar - dorka', 'OMP - fixing the sidebar - dorka'],
...
describe('one real OMP turn', () => {
  ...
  // This drives a full turn of the frames Dorka's injected extension actually emits and counts
  ...
  it('commits twice across 30 working frames plus the idle transition', () => {
    const frames = Array.from(
      { length: 30 },
      (_, index) => `${FRAMES[index % FRAMES.length]} π - fixing the sidebar - dorka`
    )
    frames.push('π - fixing the sidebar - dorka')
