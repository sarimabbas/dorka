...
const BOUNDED_COMPONENT_PATTERN = /^[\s\S]{0,40}~dorka-oversized~(?:[1-9]\d*)~[0-9a-f]{16}$/
...
export function boundJournalKeyComponent(value: string): string {
  ...
  return `${wellFormedBoundedHead(value, 40)}~dorka-oversized~${value.length}~${h1}${h2}`
}
...
export function agentJournalItemKey(identity: AgentJournalItemIdentity): string {
  ...
  if (identity.provider === 'dorka') {
    return ['dorka', encodePart(identity.clientMessageId)].join(KEY_DELIMITER)
  }
  return [
    'legacy',
...
export function agentJournalSubmissionKey(clientMessageId: string): string {
  return agentJournalItemKey({ provider: 'dorka', clientMessageId })
}
...
export function parseAgentJournalItemKey(key: string): AgentJournalItemIdentity | null {
  ...
  if (provider === 'dorka' && rest.length === 1) {
