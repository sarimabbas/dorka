...
export type GlobalSettings = {
  ...
  workspaceDirHistory?: DorkaWorkspaceLayout[]
  ...
  /** When enabled, Dorka renames a workspace's auto-generated creature branch to
  ...
  /** macOS: toggles the additive menu-bar entry (Dorka survives last-window close); doesn't change Dock behavior. */
  ...
  /** Whether to show the Dorka app name in the titlebar. */
   ...
   *  cannot raise its own cap via `dorka settings update`. */
  ...
  /** Only toggles the sidebar shortcut; Dorka Mobile stays reachable from Settings. */
  ...
  /** Dorka-first keeps app shortcuts from TUIs; terminal-first is opt-in to let shell/TUI bindings win. */
  ...
  /** Legacy keyboard-shortcut overrides; new writes go to ~/.dorka/keybindings.json, migrated once when present. */
  ...
  /** Why: durable main-owned pref so Dorka can prepare shared ~/.codex before the renderer hydrates. */
   ...
   *  History-only: does not change which account/config/hooks Dorka uses. */
  ...
  /** When true, Dorka requests local awake assertions while hook-reported agents are working. */
...
export type DorkaWorkspaceLayout = {
  path: string
  nestWorkspaces: boolean
}
