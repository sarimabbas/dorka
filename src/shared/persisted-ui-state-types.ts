...
import type { PersistedTrustedDorkaHooks } from './dorka-yaml-hook-types'
...
export type PersistedUIState = {
  ...
  /** Hide workspaces created through `dorka worktree create`. */
  ...
  /** Whether Dorka already tried triggering the macOS notification permission dialog; prevents re-firing every launch. */
  ...
  /** Why: Windows-only. Set once on first hide to tray so the "Dorka is still running" notice shows only once. */
  ...
  /** Once the user has starred Dorka (any entry point), permanently suppress the nag. */
