...
/** Dorka's own owner suffix: the agent that owns the pane is named after the final `- `. */
...
describe('getAgentLabel — characterization (pre-refactor)', () => {
  describe('the owner suffix loses to a foreign name in task text', () => {
    // DEFECT. In each case the pane owner is Grok, named by Dorka's own `- grok` suffix
