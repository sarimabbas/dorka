/**
 * What a packaged `dorkad` directory must contain, declared once — the same single-source
 ...
 *
 * Keep this file erasable-only TypeScript — build-dorkad.mjs imports it directly under
 * Node's type stripping, which rejects enums, namespaces and parameter properties.
 */

export const DORKAD_VERSION = '0.1.0'

export type DorkadArtifact = {
  filename: string
  /**
   * Absence is a degradation, not a torn install, so the remote probe must not require it.
   * The agent-browser binary is the only one: `resolveDorkadBrowserProvider` already answers
   * "no headless browser" when it is missing, and it is named per platform-arch anyway.
   */
  optional?: boolean
}

export const DORKAD_ARTIFACTS: readonly DorkadArtifact[] = [
  { filename: 'dorkad.js' },
...
export const DORKAD_VERSION_FILENAME = '.version'

/** Written last by the installer; its absence means a torn install. */
export const DORKAD_INSTALL_COMPLETE_FILENAME = '.install-complete'

export function dorkadArtifactFilenames(): string[] {
  return DORKAD_ARTIFACTS.filter((artifact) => !artifact.optional).map(
