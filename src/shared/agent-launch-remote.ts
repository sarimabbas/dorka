...
/**
 * Why: a repo reached over SSH runs the Dorka CLI through the relay shim, which is always deployed
 * as plain `dorka` (Unix) / `dorka.cmd` (Windows). The Linux-only `dorka-ide` rename — which exists
 * solely to avoid shadowing the GNOME Dorka screen reader on a local desktop — must not be applied
 * to those remotes, or `dorka-ide claude-teams` lands on a PATH where it does not exist.
 *
 ...
 *
 *   - a row carrying only `executionHostId: 'ssh:<target>'` reads as local and gets the `dorka-ide`
 ...
 * shim), while a runtime host with no nested target is a full Dorka install and stays false — as do
