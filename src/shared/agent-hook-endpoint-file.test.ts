...
describe('agent hook endpoint files', () => {
    ...
    expect(
      parseAgentHookEndpointFile(
        [
          ...
          'DORKA_AGENT_HOOK_TOKEN=token-123',
  ...
  it('parses Windows endpoint.cmd contents', () => {
    expect(
      parseAgentHookEndpointFile(
        [
          'set DORKA_AGENT_HOOK_PORT=54321',
          'set DORKA_AGENT_HOOK_TOKEN=token-abc',
  ...
  it('preserves equals signs in endpoint values', () => {
    expect(
      parseAgentHookEndpointFile(
        [
          'DORKA_AGENT_HOOK_PORT=12345',
          'DORKA_AGENT_HOOK_TOKEN=token=with=equals',
          'DORKA_AGENT_HOOK_ENV=production',
          'DORKA_AGENT_HOOK_VERSION=1'
        ].join('\n')
      ).token
  ...
  it('throws when required endpoint fields are missing', () => {
    expect(() => parseAgentHookEndpointFile('DORKA_AGENT_HOOK_PORT=12345')).toThrow(
