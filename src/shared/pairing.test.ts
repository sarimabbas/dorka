...
describe('pairing offer', () => {
  ...
  it('encode then decode round-trips correctly', () => {
    ...
    expect(url).toMatch(/^dorka:\/\/pair\?code=/)
  ...
  it('rejects dorka URLs outside the exact pairing route', () => {
    ...
    expect(parsePairingCode(`dorka://pairing?code=${code}`)).toBeNull()
    expect(parsePairingCode(`dorka://pair-extra?code=${code}`)).toBeNull()
    expect(() => decodePairingOffer(`dorka://pairing?code=${code}`)).toThrow('Invalid pairing URL')
  })

  it('rejects URLs without a pairing code', () => {
    expect(() => decodePairingOffer('dorka://pair')).toThrow('Invalid pairing URL')
  })

  it('decodes legacy hash URLs', () => {
    const url = encodePairingOffer(offer)
    const code = new URLSearchParams(url.slice(url.indexOf('?') + 1)).get('code')!
    expect(decodePairingOffer(`dorka://pair#${code}`)).toEqual(offer)
  })
  ...
  it('rejects payloads with missing fields', () => {
    ...
    expect(() => decodePairingOffer(`dorka://pair#${base64}`)).toThrow()
  })
...
describe('parsePairingCode', () => {
  ...
  it('parses a full dorka://pair# URL', () => {
    const url = encodePairingOffer(offer)
