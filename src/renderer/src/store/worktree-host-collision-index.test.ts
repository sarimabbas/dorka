...
const SHARED_ID = 'repo-1::/work/dorka'
...
const localRow: Worktree = { ...baseWorktree, hostId: 'local', displayName: 'local dorka' }
const sshRow: Worktree = { ...baseWorktree, hostId: 'ssh:build-box', displayName: 'ssh dorka' }
...
describe('id-keyed worktree projections keep distinct hosts distinct', () => {
  ...
  it('treats an unqualified row as its own bucket rather than inventing a host', () => {
    const unqualified: Worktree = { ...baseWorktree, displayName: 'unqualified dorka' }
