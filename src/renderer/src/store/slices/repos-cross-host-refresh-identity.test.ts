...
// One project cloned on the local Mac and on a remote Dorka server under distinct repo ids —
...
function runtimeResult(method: string): unknown {
  ...
  if (method === 'projectHostSetup.list') {
    return { setups: [clone(setup('remote-repo', '/srv/dorka'))] }
  }
  return {}
}
...
beforeEach(() => {
  ...
  listHostSetups.mockImplementation(async () => [clone(setup('local-repo', '/local/dorka'))])
