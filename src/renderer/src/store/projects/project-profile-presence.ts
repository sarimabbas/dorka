...
export async function warnIfProjectKnownInAnotherProfile(
  ...
  const findProjectProfiles = window.api.dorkaProfiles?.findProjectProfiles
  ...
  if (!findProjectProfiles || !activeDorkaProfileId) {
    return
  }
  try {
    const result = await findProjectProfiles({
      ...
      excludeProfileId: activeDorkaProfileId
    })
