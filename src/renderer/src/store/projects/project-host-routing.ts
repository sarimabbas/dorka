...
async function assertProjectHostSetupRuntimeCapability(target: RuntimeClientTarget): Promise<void> {
  ...
  await assertRuntimeEnvironmentCapability(
    ...
    PROJECT_HOST_SETUP_RUNTIME_CAPABILITY,
    'The selected Dorka server does not support project host setup yet. Update Dorka on the server and try again.',
    15_000
  )
}
...
export async function fetchProjectHostSetupCompatibility(
  ...
  try {
    ...
    return {
      ...
      // Dorka version can publish a row whose declared field types do not hold.
...
export async function assertProjectHostSetupMutationRuntimeCapabilities(
  ...
  await assertRuntimeEnvironmentCapability(
    target.environmentId,
    WORKSPACE_RUN_CONTEXT_RUNTIME_CAPABILITY,
    'The selected Dorka server does not support explicit workspace run hosts yet. Update Dorka on the server and try again.',
    15_000
  )
}
