...
describe('dorka.yaml trust prompt evicted from the modal slot', () => {
  beforeEach(() => {
  ...
  it('resolves the pending decision as skip when another modal takes the slot', async () => {
    ...
    await vi.waitFor(() => expect(store.getState().activeModal).toBe('confirm-dorka-yaml-hooks'))
    ...
    expect(store.getState().trustedDorkaHooks).toEqual({})
  })
