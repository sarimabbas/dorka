...
export type LinearAgentSkillUpdateTarget = {
  skillName: typeof DORKA_LINEAR_SKILL_NAME | typeof LINEAR_TICKETS_SKILL_NAME
  command: string
}
...
export function getLinearAgentSkillUpdateTarget(
  ...
  const canonicalSkillInstalled = hasInstalledAgentSkill(skills, DORKA_LINEAR_SKILL_NAME, {
  ...
  return !installed || canonicalSkillInstalled || !legacySkillInstalled
    ? { skillName: DORKA_LINEAR_SKILL_NAME, command: DORKA_LINEAR_SKILL_UPDATE_COMMAND }
