...
describe('getSkillFreshnessDisplayStatus', () => {
  ...
  it.each([
    ...
    // Why: these are Dorka's own bounds. A large but healthy plugin cache would
  ...
  // bound Dorka chose, so the only one a person can actually clear.
...
describe('hasSkillCopyNeedingAttention', () => {
  ...
  // copy inside a work directory. Dorka only updates global skills, so the project copy
  ...
  // presence rather than a freshness claim about a copy Dorka does not manage.
  ...
  // Why: an unreadable plugin path could hide a copy of anything, but a skill Dorka
