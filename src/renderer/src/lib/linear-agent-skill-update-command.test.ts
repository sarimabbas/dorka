...
describe('getLinearAgentSkillUpdateTarget', () => {
  it('targets the canonical skill for missing, canonical, and dual-name installs', () => {
    expect(getLinearAgentSkillUpdateTarget([], false)).toEqual({
      skillName: 'dorka-linear',
      command: DORKA_LINEAR_SKILL_UPDATE_COMMAND
    })
    expect(getLinearAgentSkillUpdateTarget([skill('dorka-linear')], true)).toEqual({
      skillName: 'dorka-linear',
      command: DORKA_LINEAR_SKILL_UPDATE_COMMAND
    })
    expect(
      getLinearAgentSkillUpdateTarget([skill('dorka-linear'), skill('linear-tickets')], true)
    ).toEqual({
      skillName: 'dorka-linear',
      command: DORKA_LINEAR_SKILL_UPDATE_COMMAND
    })
