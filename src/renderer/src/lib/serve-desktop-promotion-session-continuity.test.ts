 ...
 *
 * The incident chain was: Cmd+Q leaves a packaged `dorka serve` owning the
...
/** Argv macOS delivers for `open -n -a Dorka` / Finder / Dock relaunch. */
const DESKTOP_RELAUNCH_ARGV = ['/Applications/Dorka.app/Contents/MacOS/Dorka'] as const
...
describe('#8457 headless serve promotion preserves surviving agent sessions', () => {
  ...
  it('ignores a duplicate `dorka serve` launch instead of promoting the headless owner', () => {
    const serve = bootHeadlessServeOwner()
