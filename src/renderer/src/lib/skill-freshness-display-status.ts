...
export function getSkillFreshnessDisplayStatus(
  ...
  for (const installation of inventory?.installations ?? []) {
    ...
    // drift Dorka has no way to fix. Skipped before `hasPlacement` so a repo-only skill
    // reports presence, not a freshness claim about a copy Dorka does not manage.
  ...
  // fail-closed — but only for skills Dorka actually found somewhere. Flagging a skill
