...
import { hashDorkaHookScript } from './dorka-hook-trust'
import {
...
function createTestState(overrides?: Partial<AppState>): {
  ...
  const trust: PersistedTrustedDorkaHooks = {}
  const state = {
    trustedDorkaHooks: trust,
...
describe('ensureHooksConfirmed', () => {
  ...
  it('short-circuits to run when the persisted content hash matches the current script', async () => {
    ...
    const hash = await hashDorkaHookScript(script)
    state.trustedDorkaHooks['repo-1'] = {
  ...
  it('re-prompts when the script content differs from the persisted hash', async () => {
    const { state, pending } = createTestState()
    const staleHash = await hashDorkaHookScript('old script')
    state.trustedDorkaHooks['repo-1'] = {
    ...
    // dorka.yaml changed, not because they've never approved this hook.
  ...
  it('includes default tab commands in the setup trust prompt', async () => {
    ...
    expect(pending[0].data.contentHash).toBe(await hashDorkaHookScript(expectedContent))
  ...
  it('does not prompt for dorka.yaml when the repo uses local commands only', async () => {
    const { state, pending } = createTestState({
      repos: [
        {
          id: 'repo-1',
  ...
  it('does not prompt for dorka.yaml when local commands are the implicit default', async () => {
    const { state, pending } = createTestState({
      repos: [
        {
          id: 'repo-1',
  ...
  it('opens a modal with the computed content hash and resolves with the user decision', async () => {
    ...
    expect(pending[0].data).toMatchObject({
      ...
      contentHash: await hashDorkaHookScript('pnpm install'),
