...
describe('isDorkaCliAvailableOnPath', () => {
  it('requires the installed CLI command to be visible on PATH', () => {
    expect(isDorkaCliAvailableOnPath(cliStatus())).toBe(true)
    expect(isDorkaCliAvailableOnPath(cliStatus({ pathConfigured: false }))).toBe(false)
    expect(isDorkaCliAvailableOnPath(cliStatus({ state: 'not_installed' }))).toBe(false)
  })
})

describe('ensureDorkaCliAvailableForAgentSkillTerminal', () => {
  afterEach(() => {
    vi.useRealTimers()
    vi.unstubAllGlobals()
    vi.clearAllMocks()
  ...
  it('does not run the CLI installer when the Windows PATH read is unknown', async () => {
    ...
    await expect(
      ensureDorkaCliAvailableForAgentSkillTerminal({ registrationPromptDelayMs: 0 })
  ...
  it('lets the registration toast paint before opening the native installer', async () => {
    ...
    const pending = ensureDorkaCliAvailableForAgentSkillTerminal({ registrationPromptDelayMs: 700 })
    await vi.waitFor(() => {
      expect(toast.message).toHaveBeenCalledWith(CLI_PREREQUISITE_REGISTRATION_TOAST, {
        description: CLI_PREREQUISITE_REGISTRATION_TOAST_DESCRIPTION
      })
