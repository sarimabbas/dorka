...
import { hashDorkaHookScript, type DorkaHookScriptKind } from './dorka-hook-trust'
import {
  checkRuntimeHooks,
...
export type HookScriptKind = DorkaHookScriptKind
...
function getSetupTrustContent(yamlHooks: DorkaHooks | null): string {
...
function getVmRecipeTrustContent(yamlHooks: DorkaHooks | null): string {
...
function canUseRepoWideTrust(state: AppState, repoId: string): boolean {
  ...
  return Boolean(state.trustedDorkaHooks[repoId]?.all) && !hasDuplicateRepoId
}
...
async function confirmScriptContent(
  ...
  const contentHash = await hashDorkaHookScript(scriptContent)
  if (isCancelled()) {
    return 'skip'
  }
  const existingHash = state.trustedDorkaHooks[repoId]?.[scriptKind]?.contentHash
  ...
  return new Promise<'run' | 'skip'>((resolve) => {
    ...
    state.openModal('confirm-dorka-yaml-hooks', {
      repoId,
      repoName,
      scriptKind,
      scriptContent,
  ...
  return enqueueTrustPrompt(async () => {
    ...
    try {
      if (scriptKind === 'issueCommand') {
        // Local overrides are user-owned; only shared dorka.yaml commands need repo trust.
        ...
        const yamlHooks = (result.hooks as DorkaHooks | null) ?? null
        scriptContent =
          scriptKind === 'setup'
