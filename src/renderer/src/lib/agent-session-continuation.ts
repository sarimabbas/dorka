...
export function buildAgentSessionContinuationPrompt(
  ...
  const sourceLines = [
    ...
    source.sourceLabel ? `Dorka pane: ${source.sourceLabel}` : null,
  ...
  return [
    'Continue work from the prior Dorka session using the context below.',
    ...
    ...(statusHints.length > 0 ? ['', 'Latest Dorka status hints:', ...statusHints] : []),
    '',
