...
describe('terminal path helpers', () => {
  ...
  describe('extractTerminalFileLinks bare-filename tokens', () => {
    it('does not treat regular URL hosts as local file paths', () => {
      expect(
        extractTerminalFileLinks(
          'PR opened: https://github.com/stablyai/dorka-marketing-website/pull/82'
        )
      ).toEqual([])
    })
  ...
  describe('plain-text file:// URIs', () => {
    it('extracts a printed file:// URI as a file link resolving to its path', () => {
      const line = 'Report: file:///Users/dev/dorka/report.html'
      const link = extractTerminalFileLinks(line).find(
        (candidate) => candidate.displayText === 'file:///Users/dev/dorka/report.html'
      )
      expect(link).toMatchObject({ pathText: '/Users/dev/dorka/report.html' })
      expect(resolveTerminalFileLink(link!, '/Users/dev/dorka')).toEqual({
        absolutePath: '/Users/dev/dorka/report.html',
        line: null,
        column: null
      })
    })
    ...
    it('does not also emit a bare-path link for the URI body', () => {
      const links = extractTerminalFileLinks('file:///Users/dev/dorka/report.html')
      expect(links.map((link) => link.displayText)).toEqual(['file:///Users/dev/dorka/report.html'])
    })
