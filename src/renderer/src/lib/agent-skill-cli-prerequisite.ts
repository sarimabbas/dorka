...
type EnsureDorkaCliAvailableOptions = {
...
export const AGENT_SKILL_CLI_PREREQUISITE_NOTICE =
  'Before opening setup, Dorka may show a system prompt to register the Dorka CLI command on PATH.'

export const CLI_PREREQUISITE_REGISTRATION_TOAST = 'Dorka needs to register its CLI on PATH.'
export const CLI_PREREQUISITE_REGISTRATION_TOAST_DESCRIPTION =
  'Approve the system prompt so skill setup can use the Dorka CLI command.'

export function isDorkaCliAvailableOnPath(status: CliInstallStatus | null | undefined): boolean {
  return status?.state === 'installed' && status.pathConfigured === true
}

export async function ensureDorkaCliAvailableForAgentSkillTerminal({
  onStatusChange,
  registrationPromptDelayMs = 700
}: EnsureDorkaCliAvailableOptions = {}): Promise<CliInstallStatus | null> {
  try {
    ...
    if (status.state !== 'installed' || status.pathConfigured === false) {
      ...
      await showDorkaCliRegistrationPromptToast(registrationPromptDelayMs)
    ...
    toast.error(
      error instanceof Error
        ...
        : translate(
            ...
            'Failed to register the Dorka CLI in PATH.'
          )
    )
    return null
  }
...
export async function showDorkaCliRegistrationPromptToast(delayMs = 700): Promise<void> {
...
function showCliPrerequisiteWarning(status: CliInstallStatus): void {
  if (!status.supported) {
    toast.warning(
      translate(
        ...
        'Dorka CLI registration is unavailable'
      ),
      {
        description:
          status.detail ??
  ...
  if (status.state !== 'installed') {
    toast.warning(
      translate(
        ...
        'Dorka CLI registration needs attention'
      ),
      {
        description:
          status.detail ??
  ...
  if (status.pathConfigured === null) {
    toast.warning(
      translate(
        ...
        'Dorka could not check your Windows user PATH'
      ),
      {
        description:
          status.detail ??
  ...
  if (status.pathConfigured === false) {
    ...
    // Dorka affordances when that shell can resolve the Dorka CLI command.
    toast.warning(
      translate(
        'auto.lib.agent.skill.cli.prerequisite.79371593b0',
        'Dorka CLI is not visible on PATH yet'
      ),
      {
        description:
          status.detail ??
          translate(
            ...
            'Restart your shell or add the Dorka CLI directory to PATH before setup.'
          )
      }
    )
  }
