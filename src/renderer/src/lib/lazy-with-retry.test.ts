...
import { DORKA_APP_RESTART_ABORTED_EVENT } from '../../../shared/updater-renderer-events'
import {
  DORKA_EDITOR_PREPARE_HOT_EXIT_EVENT,
  type EditorPrepareHotExitDetail
} from '../../../shared/editor-save-events'

const RELOAD_GUARD_KEY = 'dorka:lazy-chunk-reload-attempted'
...
describe('loadLazyWithRetry recovery reload vs the dirty-editor-tab unload veto', () => {
  ...
  function installDirtyEditorTab(options: { hotExitBackupFails?: boolean } = {}): Harness {
    ...
    window.addEventListener(DORKA_EDITOR_PREPARE_HOT_EXIT_EVENT, hotExitBackup)
    ...
    cleanupHarness = () => {
      ...
      window.removeEventListener(DORKA_EDITOR_PREPARE_HOT_EXIT_EVENT, hotExitBackup)
      cleanupBypass()
    }
    return harness
  }
  ...
  it('refuses to reload when unsaved buffers cannot be backed up', async () => {
    ...
    window.addEventListener(DORKA_APP_RESTART_ABORTED_EVENT, restartAborted)
    const error = chunkParseError()
    ...
    window.removeEventListener(DORKA_APP_RESTART_ABORTED_EVENT, restartAborted)
  })
