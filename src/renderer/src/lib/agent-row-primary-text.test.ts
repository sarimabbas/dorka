...
import {
  ...
  getDorkaDispatchTaskId,
  isDorkaDispatchPrompt
...
describe('getAgentRowPrimaryText', () => {
  it('prefers orchestration display name over the raw hook prompt', () => {
    expect(
      getAgentRowPrimaryText({
        prompt: 'You are working inside Dorka, a multi-agent IDE.',
        orchestration: {
          taskId: 'task-1',
          dispatchId: 'ctx-1',
  ...
  it('ignores sticky orchestration labels that belong to a different task id', () => {
    expect(
      getAgentRowPrimaryText({
        prompt: `You are working inside Dorka, a multi-agent IDE. You are a dispatched worker.
...
=== CLI COMMANDS ===
dorka orchestration send --to term_parent
...
Fix the checkout race condition in payments`
  ...
  it('derives a task preview from a normalized 200-char dispatch prompt without labels', () => {
    const longCliNoise = Array.from(
      { length: 40 },
      (_, i) => `dorka orchestration send --to term_parent --type heartbeat --phase step-${i}`
    ).join('\n')
    const taskBody =
...
${taskBody}`
    ...
    expect(preview).not.toContain('You are working inside Dorka')
...
Implement the detailed worker instructions that should not stay as the final label`
  ...
  it('does not surface preamble boilerplate when the TASK marker is absent', () => {
    expect(
      getAgentRowPrimaryText({
        prompt:
          'You are working inside Dorka, a multi-agent IDE. You are a dispatched worker. Your task ID is: task_no_body CLI noise only'
      })
    ).toBe('')
  })
  ...
  // mentions `=== TASK ===` wins over Dorka's real separator.
  it('ignores adversarial === TASK === text in raw multi-line base-drift subjects', () => {
    expect(
      getAgentRowPrimaryText({
        prompt: [
          'You are working inside Dorka, a multi-agent IDE. You are a dispatched worker.',
  ...
  it('still reads the task body from a normalized single-line compact prompt', () => {
    expect(
      getAgentRowPrimaryText({
        prompt:
          'You are working inside Dorka, a multi-agent IDE. Your task ID is: task_inline === TASK === Compact body preview'
      })
    ).toBe('Compact body preview')
  })
})

describe('getDorkaDispatchTaskId', () => {
...
body`
    ...
    expect(getDorkaDispatchTaskId(normalized!.prompt)).toBe('task_9f3ab2')
  })
...
describe('isDorkaDispatchPrompt / getAgentRowGeneratedTitleText', () => {
  it('treats leading whitespace as still a dispatch preamble', () => {
    expect(
      isDorkaDispatchPrompt('  You are working inside Dorka, a multi-agent IDE. Worker task')
    ).toBe(true)
  })
