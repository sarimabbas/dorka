...
describe('openHttpLink', () => {
  it('routes into Dorka when openLinksInApp is on and a worktree is known', () => {
  ...
  it('routes floating workspace links into Dorka without changing the active repo worktree', () => {
    storeState.settings = { openLinksInApp: true }
  ...
  it('forceInApp opens a local link in Dorka when the setting is off', () => {
    storeState.settings = { openLinksInApp: false }
  ...
  it('does not force a remote link into the Dorka browser', () => {
    storeState.settings = { openLinksInApp: false }
  ...
  it('labels explicit local links from the local scan instead of a merged remote port', async () => {
    ...
    registerLocalhostLabelMock.mockResolvedValue({ url: 'http://wt-local.dorka.localhost:60016/' })
  ...
  it('labels localhost links from terminal output before opening the system browser', async () => {
    ...
    registerLocalhostLabelMock.mockResolvedValue({
      url: 'http://analytics.dorka.localhost:60016/episodes'
    })
    ...
    expect(openUrlMock).toHaveBeenCalledWith('http://analytics.dorka.localhost:60016/episodes')
  })
  ...
  it('resolves display URLs for labeled localhost links without opening them', async () => {
    ...
    registerLocalhostLabelMock.mockResolvedValue({
      url: 'http://snapstudio-main.dorka.localhost:60016/'
    })

    await expect(resolveLocalhostHttpLinkDisplayUrl('http://localhost:5180/')).resolves.toBe(
      'http://snapstudio-main.dorka.localhost:60016/'
    )
    expect(openUrlMock).not.toHaveBeenCalled()
    expect(createBrowserTabMock).not.toHaveBeenCalled()
  })
  ...
  it('labels a local-owned localhost link from the local scan', async () => {
    ...
    ).resolves.toBe('http://snapstudio-main.dorka.localhost:60016/')
  })
})

describe('openHttpLink modifier routing', () => {
  it('forces the system browser when inverting is off and links open in Dorka', () => {
  ...
  it('opens in Dorka when inverting is on and links open externally', () => {
  ...
  it('opens in the system browser when inverting is on and links open in Dorka', () => {
  ...
  // Why: remote-owned links must never land in an Dorka tab that cannot reach them.
  it('never routes a remote source into Dorka even when inverting', () => {
    storeState.settings = { openLinksInApp: false, openLinksInAppModifierInverts: true }
