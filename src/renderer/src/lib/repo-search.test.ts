...
describe('repo-search', () => {
  ...
  it('matches display names case-insensitively', () => {
    const repos = [
      makeRepo({ id: '1', displayName: 'stablyai/orca', path: '/repos/dorka' }),
      makeRepo({ id: '2', displayName: 'stablyai/noqa', path: '/repos/noqa' })
    ]

    expect(searchRepos(repos, 'DORKA').map((repo) => repo.id)).toEqual(['1'])
  })

  it('falls back to matching repo paths', () => {
    const repos = [
      makeRepo({ id: '1', displayName: 'frontend', path: '/src/team-a/dorka' }),
  ...
  it('keeps display-name matches ahead of path-only matches', () => {
    const repos = [
      makeRepo({ id: '1', displayName: 'misc', path: '/src/dorka-tools/misc' }),
      makeRepo({ id: '2', displayName: 'dorka', path: '/src/team-a/project' })
    ]

    expect(searchRepos(repos, 'dorka').map((repo) => repo.id)).toEqual(['2', '1'])
  })
