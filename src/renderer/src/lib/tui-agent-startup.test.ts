...
describe('buildAgentDraftLaunchPlan', () => {
  ...
  it('uses DORKA_PI_PREFILL env var for pi (no CLI flag exists)', () => {
    // Why: pi has no `--prefill` flag, and bracketed-paste-after-ready races
    // against pi's lengthy startup output. The Dorka overlay installs an
    // `dorka-prefill` extension that reads DORKA_PI_PREFILL on session_start
    ...
    expect(
      ...
      agent: 'pi',
      launchCommand: `pi; command test -n "$fish_pid" && set --erase -g DORKA_PI_PREFILL; command test -z "$fish_pid" && unset DORKA_PI_PREFILL; true`,
      expectedProcess: 'pi',
      env: { DORKA_PI_PREFILL: 'https://github.com/acme/repo/issues/42' },
