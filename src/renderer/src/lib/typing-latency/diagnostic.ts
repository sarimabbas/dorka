 ...
 *
 *   window.__dorkaTypingDiagnostic.start()   // then type normally for ~20s
 *   window.__dorkaTypingDiagnostic.report()  // logs + returns a JSON-safe object
 *   window.__dorkaTypingDiagnostic.stop()
 *
...
function startProbe(): string {
  if (active) {
    return 'Typing diagnostic already running. Type for ~20s, then run __dorkaTypingDiagnostic.report().'
  }
  cacheAppVersion()
  ...
  active = state
  lastState = state
  return `Typing diagnostic started on ${state.panes.length} pane(s). Click into the agent terminal, type normally for ~20 seconds, then run __dorkaTypingDiagnostic.report().`
}
...
function stopProbe(): string {
  ...
  state.panes = []
  return 'Typing diagnostic stopped. Run __dorkaTypingDiagnostic.report() to read the last samples.'
}
...
function reportProbe(): TypingLatencyReport {
  ...
  console.log('[dorka] typing latency diagnostic', report)
  return report
}
...
type TypingDiagnosticWindow = Window & { __dorkaTypingDiagnostic?: TypingDiagnosticBridge }
...
export function installTypingLatencyDiagnostic(): void {
  ...
  if (target.__dorkaTypingDiagnostic) {
    return
  }
  target.__dorkaTypingDiagnostic = { start: startProbe, stop: stopProbe, report: reportProbe }
}
