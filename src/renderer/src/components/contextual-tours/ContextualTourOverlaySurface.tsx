...
function disposeContextualTourGlobalKeyGuard(): void {
  ...
  const guardedWindow = window as Window & {
    __dorkaContextualTourGlobalKeyGuardInstalled?: boolean
  }
  if (!guardedWindow.__dorkaContextualTourGlobalKeyGuardInstalled) {
    return
  }
  window.removeEventListener('keydown', handleContextualTourGlobalKeyDown, true)
  delete guardedWindow.__dorkaContextualTourGlobalKeyGuardInstalled
}
...
if (typeof window !== 'undefined') {
  ...
  if (!guardedWindow.__dorkaContextualTourGlobalKeyGuardInstalled) {
    guardedWindow.__dorkaContextualTourGlobalKeyGuardInstalled = true
...
const PANEL_BASE_CLASSES =
  'dorka-contextual-tour-panel rounded-lg border border-border text-popover-foreground backdrop-blur-[2px]'
  ...
  return (
    ...
    >
      {showTargetRings ? (
        <div
          aria-hidden="true"
          className="dorka-contextual-tour-target-rings fixed z-[75]"
