...
describe('workspace cleanup scanned host confirmation removal', () => {
  ...
  it('deletes only host B after its real scanned candidate crosses the confirmation path', async () => {
    const scanRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'dorka-cleanup-scan-'))
    const hostARoot = fs.mkdtempSync(path.join(os.tmpdir(), 'dorka-cleanup-host-a-'))
    const hostBRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'dorka-cleanup-host-b-'))
