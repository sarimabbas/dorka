...
const hiddenWorktrees = [
  ...
  },
  {
    ...
    path: '/srv/repos/dorka/ssh-worktree',
...
describe('ImportedWorktreesVisibilityLine', () => {
  it('renders the compact repo-group line with expand and dismiss actions', () => {
    ...
    expect(markup).toContain('Expand hidden worktrees for dorka')
    expect(markup).toContain(
      'Keep 4 discovered worktrees hidden for dorka; recover from the project menu'
    )
    expect(markup).toContain('aria-expanded="false"')
    expect(markup).not.toContain('Imported 4 existing worktrees')
    expect(markup).not.toContain('Dorka found 4 worktrees')
  ...
  it('names the host when the project is checked out on more than one', () => {
    ...
    expect(markup).toContain('Expand hidden worktrees for dorka on openclaw')
    expect(markup).toContain(
      'Keep 4 discovered worktrees hidden for dorka on openclaw; recover from the project menu'
    )
  })
  ...
  it('folds the host into pinned fallback copy, which already names the repo', () => {
    ...
    expect(markup).toContain('Hiding 4 discovered worktrees in dorka on openclaw')
  })
  ...
  it('scopes pinned fallback copy to the repo name without a dismiss action', () => {
    ...
    expect(markup).toContain('Hiding 4 discovered worktrees in dorka')
    ...
    const groups = groupWorktreesByParentPath([
      {
        ...
        path: 'C:\\Repos\\Dorka\\FeatureX'
      }
    ])

    expect(groups).toMatchObject([{ path: 'C:/Repos/Dorka' }])
    expect(groups[0]?.path).not.toBe('C:\\Repos\\Dorka')
  })
