 ...
 * workspace from Dorka because the SSH target is gone entirely.
 */
export type SshWorkspaceForgetResolution =
  ...
  // impossible, so forget-from-Dorka is the only path.
...
export function resolveSshWorkspaceForget(args: {
  ...
  // the only escape is to forget it from Dorka.
  if (!isConfigured) {
