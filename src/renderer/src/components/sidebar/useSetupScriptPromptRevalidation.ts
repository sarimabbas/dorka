...
/**
 * Re-runs the setup-script prompt inspection when a shared `dorka.yaml` setup hook
...
export function useSetupScriptPromptRevalidation(input: {
  ...
  // Why: dorka.yaml is edited on disk or the hook runs in a terminal outside React
  // state. Re-inspect on window focus so returning to Dorka detects it (mirrors
  ...
  useEffect(() => {
    ...
    // Why: a runtime reconnect can turn an unreadable dorka.yaml into a readable one,
