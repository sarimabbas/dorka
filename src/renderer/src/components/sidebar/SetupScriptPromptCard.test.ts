...
describe('getRenderedSetupScriptPromptState', () => {
  ...
  it('does not reuse a matching repo id from a different host', () => {
    expect(
      getRenderedSetupScriptPromptState({
        promptState: prompt('repo-dorka', 'local'),
        activeRepoId: 'repo-dorka',
        activeRepoHostIdentity: repoIdentity('repo-dorka', 'runtime:windows'),
...
describe('markSetupScriptPromptSaved', () => {
  it('does not apply a completed save to the same repo id on another host', () => {
    const remote = prompt('repo-dorka', 'runtime:windows')

    expect(markSetupScriptPromptSaved(remote, repoIdentity('repo-dorka', 'local'))).toBe(remote)
  })

  it('marks the prompt for the saved host effective', () => {
    expect(
      markSetupScriptPromptSaved(
        prompt('repo-dorka', 'runtime:windows'),
        repoIdentity('repo-dorka', 'runtime:windows')
      )
