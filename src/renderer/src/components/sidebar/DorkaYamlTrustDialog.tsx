...
import type { DorkaHookScriptKind } from '@/lib/dorka-hook-trust'
import { translate } from '@/i18n/i18n'

type ScriptKind = DorkaHookScriptKind
...
const DorkaYamlTrustDialog = React.memo(function DorkaYamlTrustDialog() {
  const activeModal = useAppStore((s) => s.activeModal)
  const modalData = useAppStore((s) => s.modalData)
  const closeModal = useAppStore((s) => s.closeModal)
  const markDorkaHookScriptConfirmed = useAppStore((s) => s.markDorkaHookScriptConfirmed)
  const markDorkaHookRepoAlwaysTrusted = useAppStore((s) => s.markDorkaHookRepoAlwaysTrusted)

  const isOpen = activeModal === 'confirm-dorka-yaml-hooks'
  ...
  const resolveAndClose = useCallback(
    (decision: 'run' | 'skip') => {
      if (decision === 'run' && repoId) {
        if (alwaysTrust) {
          ...
          markDorkaHookScriptConfirmed(repoId, scriptKind, contentHash)
        }
      }
      onResolve?.(decision)
      closeModal()
    ...
    [
      ...
      closeModal,
      contentHash,
      markDorkaHookRepoAlwaysTrusted,
      markDorkaHookScriptConfirmed,
      onResolve,
      repoId,
      scriptKind
    ]
      ...
      <DialogContent className="max-w-md sm:max-w-md" showCloseButton={false}>
        <DialogHeader>
          <DialogTitle className="text-sm">
            {previouslyApproved
              ? translate(
                  'auto.components.sidebar.DorkaYamlTrustDialog.02b0ede5ad',
                  "{{value0}}'s {{value1}} changed — run the new version?",
                  { value0: repoName, value1: SCRIPT_KIND_LABEL[scriptKind] }
                )
              : translate(
                  'auto.components.sidebar.DorkaYamlTrustDialog.e4a51dc4b3',
          ...
          <DialogDescription className="text-xs">
            {previouslyApproved ? (
              ...
              <>
                {translate(
                  'auto.components.sidebar.DorkaYamlTrustDialog.aa3ffb33fb',
                  "This repository's"
                )}{' '}
                <code>
                  {translate('auto.components.sidebar.DorkaYamlTrustDialog.79afc6772b', 'dorka.yaml')}
                ...
                {SCRIPT_KIND_TRIGGER[scriptKind]}
                {translate(
                  'auto.components.sidebar.DorkaYamlTrustDialog.bf800b7e04',
        ...
        {scriptContent && (
          <div className="rounded-md border border-border/70 bg-muted/35 px-3 py-2">
            <div className="mb-1 text-[11px] font-medium uppercase tracking-wide text-muted-foreground">
              {previouslyApproved
                ? translate(
                    'auto.components.sidebar.DorkaYamlTrustDialog.9e52effffd',
                    'New {{value0}} script',
                    { value0: scriptKind }
                  )
                : translate(
                    'auto.components.sidebar.DorkaYamlTrustDialog.95bf974a1a',
                    '{{value0}} script',
                    { value0: scriptKind }
                  )}
            </div>
        ...
        >
          ...
          <span className="text-xs font-medium text-foreground">
            {translate('auto.components.sidebar.DorkaYamlTrustDialog.531689199b', 'Always trust')}{' '}
            <code>
              {translate('auto.components.sidebar.DorkaYamlTrustDialog.79afc6772b', 'dorka.yaml')}
            </code>{' '}
            {translate('auto.components.sidebar.DorkaYamlTrustDialog.c494b3ccb1', 'in')} {repoName}
          </span>
        </label>

        <DialogFooter>
          <Button variant="outline" onClick={() => resolveAndClose('skip')}>
            {translate('auto.components.sidebar.DorkaYamlTrustDialog.43b7bec4cd', "Don't run")}
          </Button>
          <Button onClick={() => resolveAndClose('run')}>
            {translate('auto.components.sidebar.DorkaYamlTrustDialog.f3e2b868fb', 'Run hooks')}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
...
export default DorkaYamlTrustDialog
