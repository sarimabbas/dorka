...
/** True only for `{home}/dorka/projects` on the usual OS home layouts. A configured
 *  directory that merely ends in `dorka/projects` (e.g. `/data/dorka/projects`) must
 *  stay verbatim — the `~` shorthand would otherwise lie. */
function isHomeProjectsFallback(pathValue: string): boolean {
  return /^(?:\/(?:Users|home)\/[^/]+|[A-Za-z]:[\\/]Users[\\/][^\\/]+)[\\/]dorka[\\/]projects$/.test(
    pathValue
  )
}
...
export function getDefaultCreateProjectParent(homeDir: string): string {
  ...
  return joinCreateProjectPath(joinCreateProjectPath(trimmedHomeDir, 'dorka'), 'projects')
}
