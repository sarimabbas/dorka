...
const HOST_DISMISS_STORAGE_KEY = 'dorka.linearTicketsSkill.setupDismissed.host'
...
vi.mock('@/lib/agent-skill-cli-prerequisite', () => ({
  ...
  ensureDorkaCliAvailableForAgentSkillTerminal: mocks.ensureCli,
  isDorkaCliAvailableOnPath: (status: CliInstallStatus | null | undefined) =>
...
describe('LinearAgentSkillSetupPrompt reminder toast', () => {
  ...
  it('shows a warning toast on a later modal-only activation after a casual close', async () => {
    ...
    expect(toast.warning).toHaveBeenCalledWith(
      'Dorka CLI and Linear skill are missing',
      expect.objectContaining({
        id: 'linear-agent-skill-setup-dorka.linearTicketsSkill.setupDismissed.host',
        description:
          'Install the Dorka CLI and the Linear skill to enable your agents to read and edit Linear tasks.',
        action: {
          label: 'Set up',
  ...
  it('does not repeat the Dorka CLI in CLI-only reminder toast copy', async () => {
    mocks.skillState.installed = true
    await snoozeInitialModal({ linked: true, remote: false, surface: 'modal' })
    await renderPrompt({ linked: true, remote: false, surface: 'modal' })

    expect(toast.warning).toHaveBeenCalledWith(
      'Dorka CLI is missing',
      expect.objectContaining({
        description: 'Install the Dorka CLI to enable your agents to read and edit Linear tasks.'
      })
    )
  })
  ...
  it('keeps remote setup nuance in reminder toast copy', async () => {
    ...
    expect(toast.warning).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        description:
          'Install the Dorka CLI and the Linear skill to enable your agents to read and edit Linear tasks. Remote agent environments may need their own setup.'
      })
    )
  })
  ...
  it('keeps WSL target nuance in reminder toast copy', async () => {
    ...
    expect(toast.warning).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        description:
          'Install the Dorka CLI and the Linear skill to enable your agents to read and edit Linear tasks. This setup runs in the selected WSL agent runtime.'
      })
    )
  })
  ...
  it('opens the setup dialog from the reminder toast action', async () => {
    ...
    expect(toast.dismiss).toHaveBeenCalledWith(
      'linear-agent-skill-setup-dorka.linearTicketsSkill.setupDismissed.host'
    )
  })
