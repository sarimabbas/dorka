...
describe('shouldDeferRemoteFileBrowserPasteResolve', () => {
  it('keeps small path and filter pastes on the immediate resolver path', () => {
    expect(shouldDeferRemoteFileBrowserPasteResolve('/workspaces/dorka/src')).toBe(false)
...
describe('parsePathInput', () => {
  ...
  it('`Documents/dorka` commits `Documents` and filters by `dorka`', () => {
    expect(parsePathInput('Documents/dorka')).toEqual({
      mode: 'path',
      base: 'cwd',
      committedSegments: ['Documents'],
      trailingFilter: 'dorka'
    })
  })
...
describe('resolveSegmentStep', () => {
  const listing: DirEntry[] = [
    ...
    { name: 'dorka-internal', isDirectory: true },
  ...
  it('unique prefix descends', () => {
    expect(resolveSegmentStep('dorka', '/home/neil', listing)).toEqual({
      type: 'descend',
      name: 'dorka-internal'
    })
  })
