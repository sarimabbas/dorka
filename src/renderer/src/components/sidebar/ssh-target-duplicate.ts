...
/** True when an existing Dorka host already owns this config alias / label. */
export function isDuplicateSshTargetAlias({
  ...
  // Why: the config picker's `alreadyInDorka` flag compares lowercased aliases; match it or the
 ...
 *  otherwise an alias it greys out as "In Dorka" is still savable as a second target. */
