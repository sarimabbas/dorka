  ...
  return (
    <WorktreeCardDetailSection>
      <DetailHeader
        ...
        label={translate('auto.components.sidebar.WorktreeCardMeta.cliHeader', 'Dorka CLI')}
      />
        ...
        <div className="text-[13px] font-semibold leading-snug text-foreground break-words">
          {provenance.callerTerminalHandle
            ? translate(
                ...
                'Created by an agent via `dorka worktree create`'
              )
            : translate(
                'auto.components.sidebar.WorktreeCardMeta.cliCreatedFromShell',
                'Created via `dorka worktree create`'
              )}
        </div>
        {agentLabel ? (
