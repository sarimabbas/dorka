  ...
  return {
    ...
    descriptionSuffix: args.isBatchDelete
      ? allFolderWorkspaceDeletes
        ? 'from Dorka. Project folders on disk will not be deleted.'
        : mixedFolderWorkspaceDeletes
          ? 'from Dorka. Git worktrees will also be removed from git and disk; folder workspaces will only remove the Dorka workspace entry.'
          : 'from git and delete their workspace folders.'
      : args.isFolderWorkspaceDelete
        ? 'from Dorka. The project folder on disk will not be deleted.'
