...
describe('WorktreeCard quick actions', () => {
  ...
  it('renders repo identity in the detailed metadata row', () => {
    ...
    expect(markup).not.toContain('aria-label="Project dorka"')
    expect(markup).toContain('>dorka</span>')
  ...
  it('renders detached HEAD identity in detailed card metadata', () => {
    ...
    const markup = renderToStaticMarkup(
      <WorktreeCard
        worktree={makeWorktree({ displayName: 'dorka', branch: '' })}
        repo={makeRepo()}
        isActive={false}
        hideRepoBadge
      />
