...
vi.mock('@/lib/agent-skill-cli-prerequisite', () => ({
  ...
  ensureDorkaCliAvailableForAgentSkillTerminal: vi.fn(async () => null),
...
function discoveredSkill(overrides: Partial<DiscoveredSkill>): DiscoveredSkill {
  return {
    ...
    directoryPath: '/Users/test/.agents/skills/dorka-linear',
    skillFilePath: '/Users/test/.agents/skills/dorka-linear/SKILL.md',
    installed: true,
    updatedAt: null,
    ...overrides
  }
...
describe('LinearAgentSkillSetupPrompt update command', () => {
  ...
  it('uses the canonical update command when the canonical Linear skill is installed', async () => {
    mocks.skillState.skills = [discoveredSkill({ name: 'dorka-linear' })]

    await renderPrompt()

    expect(mocks.panelProps.at(-1)).toEqual(
      expect.objectContaining({ installedCommand: 'npx skills update dorka-linear --global' })
    )
  })
  ...
  it('prefers the canonical update command when both Linear skill names are installed', async () => {
    mocks.skillState.skills = [discoveredSkill({ name: 'dorka-linear' }), legacyLinearSkillPath()]
