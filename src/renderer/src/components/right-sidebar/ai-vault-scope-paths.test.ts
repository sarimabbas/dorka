...
describe('deriveAiVaultWorkspaceScopePaths', () => {
  it('returns the active workspace path', () => {
    ...
    expect(deriveAiVaultWorkspaceScopePaths(active, [active])).toEqual(['/repo/dorka'])
  })
  ...
  it('drops a prior path another live workspace now owns', () => {
    ...
    const claimant = makeWorktree({ id: 'repo-1::/repo/dorka', path: '/repo/dorka' })
    const active = makeWorktree({
      id: 'repo-1::/repo/dorka-renamed',
      path: '/repo/dorka-renamed',
      priorWorktreeIds: ['repo-1::/repo/dorka']
  ...
  it('keeps a prior path the active workspace itself still owns', () => {
    const active = makeWorktree({ priorWorktreeIds: ['repo-1::/repo/dorka'] })
    expect(deriveAiVaultWorkspaceScopePaths(active, [active])).toEqual(['/repo/dorka'])
  })
  ...
  it('drops a claimed prior path regardless of where the claimant sits in the list', () => {
    ...
    const activeAtPriorPath = makeWorktree({ id: active.id, path: '/repo/dorka' })
  ...
  it('derives workspace scope paths at scale', () => {
    ...
    const prefix = '/Users/dev/dorka/workspaces/dorka-monorepo/feature-'
...
describe('deriveAiVaultScopeSessionPaths', () => {
  ...
  it('deduplicates paths that differ only by separators or trailing slash', () => {
    ...
    const active = makeWorktree()
    const trailing = makeWorktree({ id: 'repo-1::a', path: '/repo/dorka/' })
    const doubled = makeWorktree({ id: 'repo-1::b', path: '/repo//dorka' })
