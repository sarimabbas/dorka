...
export function useGitStatusFileWatchRefresh({
  ...
  useEffect(() => {
    ...
    window.addEventListener(DORKA_WORKTREE_FILE_CHANGE_EVENT, handleFsChanged as EventListener)

    return () => {
      if (refreshTimer) {
        clearTimeout(refreshTimer)
      }
      window.removeEventListener(DORKA_WORKTREE_FILE_CHANGE_EVENT, handleFsChanged as EventListener)
    }
