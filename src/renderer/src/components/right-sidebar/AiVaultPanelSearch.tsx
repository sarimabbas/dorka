...
export function AiVaultPanelSearch({
  ...
  if (needsLocalConsent) {
    message = translate(
      ...
      'Enable full-text search? Dorka builds an index on this computer from local agent transcripts, including full conversations and up to 3,072 characters per tool output. Content is not redacted. Authenticated paired clients can search it.'
    )
  } else if (noAgents) {
    message = translate(
    ...
    message = translate(
      ...
      'Search is unavailable on this computer. It may need an Dorka update or a runtime with search support.'
    )
