...
const baseSession: AiVaultSession = {
  ...
  resumeCommand: "cd '/Users/ada/dorka' && claude --resume 'session-1'",
  subagent: null
}
...
describe('buildAiVaultProjectContext', () => {
  it('uses durable worktree project ids before repo fallback', () => {
    const repo = makeRepo({ id: 'repo-1', displayName: 'Legacy Repo', path: '/Users/ada/dorka' })
    const project = makeProject({ id: 'project-1', displayName: 'Canonical Dorka' })
    const worktree = makeWorktree({
      id: 'wt-1',
      repoId: repo.id,
      projectId: project.id,
      path: '/Users/ada/dorka'
    })
    ...
    expect(context.sessionProjectById.get(baseSession.id)).toMatchObject({
      ...
      label: 'Canonical Dorka'
    })
  })

  it('normalizes compatibility project ids to repo keys', () => {
    const repo = makeRepo({ id: 'repo-1', displayName: 'Dorka', path: '/Users/ada/dorka' })
    const worktree = makeWorktree({
      id: 'wt-1',
      repoId: repo.id,
    ...
    const context = buildAiVaultProjectContext({
      ...
      worktrees: [worktree],
      projectHostSetupProjection: makeProjection({
        projects: [makeProject({ id: 'repo:repo-1', displayName: 'Compatibility Dorka' })],
    ...
    expect(context.projectLabelByKey.get('repo:repo-1')).toBe('Dorka')
  })
  ...
  it('inherits setup project ids for legacy worktrees without project metadata', () => {
    const repo = makeRepo({ id: 'repo-1', displayName: 'Dorka Repo', path: '/repo/dorka' })
    const worktree = makeWorktree({
      id: 'wt-legacy',
      repoId: repo.id,
      path: '/repo/dorka'
    })
    const session = makeSession({ id: 'claude:legacy-worktree', cwd: '/repo/dorka/src' })

    const context = buildAiVaultProjectContext({
      repos: [repo],
      worktrees: [worktree],
      projectHostSetupProjection: makeProjection({
        projects: [makeProject({ id: 'github:stablyai/orca', displayName: 'Canonical Dorka' })],
        setups: [
          makeSetup({
            repoId: repo.id,
  ...
  it('uses active worktree setup project ids when active repo is unavailable', () => {
    ...
    const session = makeSession({ id: 'claude:restored', cwd: '/repo/dorka/src' })
  ...
  it('inherits setup host ids for legacy worktrees without host metadata', () => {
    const repo = makeRepo({ id: 'repo-1', displayName: 'Runtime Repo', path: '/runtime/dorka' })
    const worktree = makeWorktree({
      id: 'wt-runtime',
      repoId: repo.id,
      path: '/runtime/dorka'
  ...
  it('uses the session host when matching overlapping local and SSH project paths', () => {
    const localRepo = makeRepo({ id: 'local', displayName: 'Local', path: '/srv/dorka' })
  ...
  it('falls back to folder when a legacy hostless session matches multiple host buckets', () => {
    const localRepo = makeRepo({ id: 'local', displayName: 'Local', path: '/srv/dorka' })
    const runtimeRepo = makeRepo({ id: 'runtime', displayName: 'Runtime', path: '/srv/dorka' })
    const session = makeSession({
  ...
  it('maps null cwd sessions to unknown', () => {
    const repo = makeRepo({ id: 'repo-1', displayName: 'Dorka', path: '/repo' })
