...
describe('openAiVaultSessionLogInDorka', () => {
  it('authorizes the exact path and opens a permanent read-only local tab', async () => {
    const state = makeState()
    getStateMock.mockReturnValue(state)

    await openAiVaultSessionLogInDorka({ filePath: LOG_PATH, executionHostId: 'local' })
  ...
  it('withholds blank, remote, and synthetic paths without authorizing', async () => {
    ...
    await openAiVaultSessionLogInDorka({ filePath: '   ', executionHostId: 'local' })
    await openAiVaultSessionLogInDorka({ filePath: LOG_PATH, executionHostId: 'ssh:dev-box' })
    await openAiVaultSessionLogInDorka({
  ...
  it('shares one in-flight open for concurrent clicks of the same path', async () => {
    ...
    const first = openAiVaultSessionLogInDorka({ filePath: LOG_PATH, executionHostId: 'local' })
    const second = openAiVaultSessionLogInDorka({ filePath: LOG_PATH, executionHostId: 'local' })
    resolveAuth?.()
