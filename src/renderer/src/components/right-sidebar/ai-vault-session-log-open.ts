...
/**
 * Open a local AI Vault session log inside Dorka as a permanent, read-only editor
 * tab (or activate an existing tab without reducing its authority). Reuses
 * Dorka's external-file authorize + `openFile` pipeline; it never grants write
 * capability by itself and never redirects the open to a remote host.
 */
export async function openAiVaultSessionLogInDorka(session: AiVaultLogSession): Promise<void> {
  ...
  if (!filePath || !canOpenAiVaultSessionLogInDorka(session)) {
    return
  }
  ...
  try {
    ...
    try {
      ...
      // trust gesture. Reuses Dorka's existing external R/W open grant.
