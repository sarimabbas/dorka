...
export function resolveHostedReviewAuthInstruction(provider: HostedReviewProvider): string {
  ...
  if (provider === 'azure-devops') {
    return 'Set DORKA_AZURE_DEVOPS_TOKEN'
  }
  if (provider === 'gitea') {
    return 'Set DORKA_GITEA_TOKEN'
  }
...
export function resolveUnavailableCreateReviewLookupNoticeMessage(
  ...
  return `Create ${copy.shortLabel} failed: Dorka could not confirm whether this branch already has a ${copy.reviewLabel}. Retry once the ${copy.providerName} lookup succeeds.`
}
