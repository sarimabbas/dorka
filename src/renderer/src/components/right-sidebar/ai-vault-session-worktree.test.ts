...
describe('resolveAiVaultSessionWorktreeInfo', () => {
  ...
  it('uses prior worktree paths to identify renamed active worktrees', () => {
    const worktree = makeWorktree({
      id: 'repo-1::/repo/dorka-renamed',
      path: '/repo/dorka-renamed',
      priorWorktreeIds: ['repo-1::/repo/dorka']
    })
  ...
  it('matches WSL UNC worktree paths to Linux transcript cwd values', () => {
    const worktree = makeWorktree({
      path: '\\\\wsl.localhost\\Ubuntu\\home\\ada\\dorka'
    })

    expect(
      resolveAiVaultSessionWorktreeInfo({
        session: { ...baseSession, cwd: '/home/ada/dorka/src' },
        worktrees: [worktree],
        activeWorktreeId: null
      })
    ).toMatchObject({
  ...
  it('uses the session host when multiple worktrees share the same path', () => {
    ...
    expect(
      resolveAiVaultSessionWorktreeInfo({
        session: { ...baseSession, cwd: '/srv/dorka/src', executionHostId: 'ssh:target-1' },
...
describe('extractWorktreePathFromSessionTitle', () => {
  it('reads worktree paths embedded in session titles', () => {
    expect(
      extractWorktreePathFromSessionTitle(
        'Inspect PR #6229 - Worktree: /Users/ada/projects/dorka/fix-tabs'
      )
    ).toBe('/Users/ada/projects/dorka/fix-tabs')
    expect(extractWorktreePathFromSessionTitle('Worktree: /tmp/dorka-worker')).toBe(
      '/tmp/dorka-worker'
    )
  })
})
    ...
    expect(
      resolveAiVaultSessionWorktreeDisplay({
        session: {
          ...
          cwd: null,
          branch: null,
          title: 'Fix tabs - Worktree: /Users/ada/projects/dorka/fix-tabs'
        },
...
describe('aiVaultWorktreeCompactPath', () => {
  it('keeps the last two path segments for dense detail rows', () => {
    expect(aiVaultWorktreeCompactPath('/Users/ada/projects/dorka/improve-agent-session')).toBe(
      'dorka/improve-agent-session'
    )
  })
})
...
function makeWorktreeInfo(
  ...
  return {
    ...
    label: 'dorka',
    path: '/repo/dorka',
    ...(status === 'unavailable' ? {} : { worktreeId: 'repo-1::/repo/dorka' })
  }
}
