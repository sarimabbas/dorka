...
import { DORKA_TERMINAL_COMMAND_FINISHED_EVENT } from '@/hooks/terminal-command-finished-event'
...
async function renderHookOnce(params: HookParams): Promise<{
  ...
  return {
    ...
    emitCommandFinished: (worktreeId: string) => {
      const listener = windowListeners.get(DORKA_TERMINAL_COMMAND_FINISHED_EVENT)
