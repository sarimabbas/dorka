...
// reads the clipboard with navigator.clipboard.readText(). Dorka runs the renderer with
...
// (execCommand('copy') from a user gesture) both work. We route the read through Dorka's
// trusted clipboard IPC bridge instead, matching how the terminal already reads it.
export const DORKA_CONTEXT_MENU_PASTE_PRIORITY = 10001
export const DORKA_CONTEXT_MENU_PASTE_NAME = 'dorka-ipc-paste'
...
export type DorkaContextMenuPasteDeps = {
...
export type DorkaContextMenuPasteOutcome =
 ...
 * bridge); otherwise performs the paste through Dorka's IPC clipboard read and
 * returns a Promise (truthy) so Monaco's blocked default never runs.
 */
export function runDorkaContextMenuPaste(
  deps: DorkaContextMenuPasteDeps
): false | Promise<DorkaContextMenuPasteOutcome> {
  ...
  return performDorkaContextMenuPaste(editorInstance, deps)
}

async function performDorkaContextMenuPaste(
  editorInstance: PasteCapableEditor,
  deps: DorkaContextMenuPasteDeps
): Promise<DorkaContextMenuPasteOutcome> {
  let text: string
  try {
