...
describe('MarkdownPreview source link routing', () => {
  ...
  it('derives a source root from floating file relative path', () => {
    expect(deriveMarkdownPreviewSourceRoot('/tmp/dorka/docs/note.md', 'docs/note.md')).toBe(
      '/tmp/dorka'
    )
  })

  it('falls back to the source file directory when no relative path is available', () => {
    expect(deriveMarkdownPreviewSourceRoot('/tmp/dorka/docs/note.md', null)).toBe('/tmp/dorka/docs')
  })

  it('derives Windows source roots without dropping the drive separator', () => {
    expect(deriveMarkdownPreviewSourceRoot('C:\\dorka\\docs\\note.md', 'docs\\note.md')).toBe(
      'C:/dorka'
    )
  })
  ...
  it('falls back to the matching preview tab for preview-only source metadata', () => {
    const otherOwnerEdit = {
      id: '/tmp/dorka/docs/note.md',
      filePath: '/tmp/dorka/docs/note.md',
      relativePath: 'docs/note.md',
      worktreeId: 'wt-1',
      mode: 'edit'
    }
    const preview = {
      id: 'markdown-preview::/tmp/dorka/docs/note.md',
      filePath: '/tmp/dorka/docs/note.md',
      relativePath: 'docs/note.md',
      worktreeId: FLOATING_TERMINAL_WORKTREE_ID,
      runtimeEnvironmentId: null,
      mode: 'markdown-preview',
      markdownPreviewSourceFileId: '/tmp/dorka/docs/note.md'
    }

    expect(
      findMarkdownPreviewSourceOpenFile([otherOwnerEdit, preview], {
        sourceFileId: '/tmp/dorka/docs/note.md',
        filePath: '/tmp/dorka/docs/note.md',
