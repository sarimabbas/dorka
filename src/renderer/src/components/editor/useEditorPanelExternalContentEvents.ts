...
export function useEditorPanelExternalContentEvents({
  ...
  useEffect(() => {
    ...
    window.addEventListener(DORKA_EDITOR_FILE_SAVED_EVENT, handler as EventListener)
    return () => window.removeEventListener(DORKA_EDITOR_FILE_SAVED_EVENT, handler as EventListener)
