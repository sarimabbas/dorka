...
export const DORKA_EDITOR_QUIESCE_FILE_SAVES_EVENT = 'dorka:editor-quiesce-file-saves'
export const DORKA_EDITOR_EXTERNAL_FILE_CHANGE_EVENT = 'dorka:editor-external-file-change'
export const DORKA_EDITOR_SAVE_FILE_EVENT = 'dorka:editor-save-file'
export const DORKA_EDITOR_SAVE_AND_CLOSE_EVENT = 'dorka:save-and-close'
export const DORKA_EDITOR_FILE_SAVED_EVENT = 'dorka:editor-file-saved'
export const DORKA_EDITOR_REQUEST_CMD_SAVE_EVENT = 'dorka:editor-request-cmd-save'
export const DORKA_EDITOR_REQUEST_FILE_CLOSE_EVENT = 'dorka:editor-request-file-close'
...
export async function requestEditorSaveQuiesce(target: EditorSaveQuiesceTarget): Promise<void> {
  await new Promise<void>((resolve) => {
    ...
    window.dispatchEvent(
      new CustomEvent<EditorSaveQuiesceDetail>(DORKA_EDITOR_QUIESCE_FILE_SAVES_EVENT, {
        detail: {
          ...target,
          claim: () => {
            claimed = true
...
export async function requestEditorFileSave(target: EditorSaveFileTarget): Promise<void> {
  await new Promise<void>((resolve, reject) => {
    let claimed = false
    window.dispatchEvent(
      new CustomEvent<EditorSaveFileDetail>(DORKA_EDITOR_SAVE_FILE_EVENT, {
        detail: {
          ...target,
          claim: () => {
            claimed = true
...
export function requestEditorFileClose(fileId: string): void {
  window.dispatchEvent(
    new CustomEvent<EditorRequestFileCloseDetail>(DORKA_EDITOR_REQUEST_FILE_CLOSE_EVENT, {
      detail: { fileId }
    })
  )
}
...
export function notifyEditorExternalFileChange(target: EditorPathMutationTarget): void {
  window.dispatchEvent(
    new CustomEvent<EditorPathMutationTarget>(DORKA_EDITOR_EXTERNAL_FILE_CHANGE_EVENT, {
      detail: target
    })
  )
}
