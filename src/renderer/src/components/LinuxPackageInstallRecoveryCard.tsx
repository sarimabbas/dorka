...
function copiedNote(packageFileName: string): string {
  return translate(
    ...
    'Command copied. Quit Dorka, run it in a system terminal to install {{value0}}, then reopen Dorka.',
    {
...
export function LinuxPackageInstallRecoveryCard({
  ...
  const EXPLAINER = translate(
    ...
    'Copy the command, quit Dorka, and run it in a system terminal on the computer where Dorka is installed. Reopen Dorka after it finishes.'
  )
  ...
  const TRUST_NOTE = translate(
    ...
    'Dorka checks the downloaded file against the release metadata at the moment it builds this command. The system package itself is not signature-checked, and Dorka cannot vouch for the file after that point.'
  )
