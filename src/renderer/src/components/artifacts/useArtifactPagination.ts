...
export function artifactAccountIdentity(authStatus: DorkaProfileAuthStatus | null): string | null {
...
function artifactRequestIsCurrent(
  ...
  return (
    ...
    artifactAccountIdentity(useAppStore.getState().dorkaProfileAuthStatus) === identity
  )
}
      ...
      if (result.status === 'ok') {
        ...
        setError(
          translate(
            ...
            'Sign in to Dorka again to load artifacts.'
          )
        )
      }
    } catch (loadError) {
