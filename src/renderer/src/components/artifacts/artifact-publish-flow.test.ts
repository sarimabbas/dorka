...
const mocks = vi.hoisted(() => ({
  ...
  toastSuccess: vi.fn(),
  state: {
    dorkaProfileAuthStatus: { state: 'connected' } as { state: string } | null,
    connectCurrentDorkaProfile: vi.fn()
  }
}))
...
const published = {
  ...
  item: {
    ...
    shareUrl: 'https://share.ondorka.dev/a/artifact-a'
  }
}

beforeEach(() => {
  vi.clearAllMocks()
  mocks.state.dorkaProfileAuthStatus = { state: 'connected' }
  mocks.state.connectCurrentDorkaProfile = mocks.connect
})

describe('artifact publish flow', () => {
  it('signs in before preparing and publishing the request', async () => {
    mocks.state.dorkaProfileAuthStatus = { state: 'local' }
