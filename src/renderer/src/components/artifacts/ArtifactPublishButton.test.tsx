...
const mocks = vi.hoisted(() => ({
  ...
  state: {
    dorkaProfileAuthStatus: { configured: true, state: 'connected' } as Record<string, unknown>,
...
describe('ArtifactPublishButton', () => {
  ...
  it('shows and manages an existing public link', async () => {
    const user = userEvent.setup()
    const createRequest = vi.fn()
    mocks.getPublishedLink.mockResolvedValue('https://share.ondorka.dev/a/artifact-a')
    mocks.publish.mockResolvedValue({
      change: 'updated',
      item: { shareUrl: 'https://share.ondorka.dev/a/artifact-a' }
    })

    render(<ArtifactPublishButton sourceKey="/repo/report.md" createRequest={createRequest} />)

    await user.click(screen.getByRole('button', { name: 'Share as artifact' }))
    expect(await screen.findByText('https://share.ondorka.dev/a/artifact-a')).toBeInTheDocument()
    await user.click(screen.getByRole('button', { name: 'Copy link' }))
    expect(mocks.copyLink).toHaveBeenCalledWith('https://share.ondorka.dev/a/artifact-a', {
