...
import type { DorkaHooks, SetupRunPolicy } from '../../../../shared/dorka-yaml-hook-types'
...
function getAutomationSetupSource(
  ...
  yamlHooks: DorkaHooks | null | undefined
...
export function getVisibleAutomationSetupDecision(args: {
  ...
  if (args.createTarget !== 'dorka' || args.workspaceMode !== 'new_per_run') {
    return undefined
  }
...
export function resolveAutomationSetupDecisionForSave(args: {
  ...
  if (args.yamlHooks === undefined) {
    // Why: automations cannot pause later for an dorka.yaml trust prompt; when
