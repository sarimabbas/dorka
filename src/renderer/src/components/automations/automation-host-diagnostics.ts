...
type AutomationHostDiagnosticsWindow = Window & {
  __dorkaAutomationHostDiagnostic?: AutomationHostDiagnosticsBridge
}
...
export function installAutomationHostDiagnostic(): void {
  ...
  if (target.__dorkaAutomationHostDiagnostic) {
    return
  }
  target.__dorkaAutomationHostDiagnostic = {
    report: () => {
      const snapshot = automationHostDiagnostics.snapshot()
      console.log('[dorka] automation host cache diagnostics', snapshot)
      return snapshot
    },
