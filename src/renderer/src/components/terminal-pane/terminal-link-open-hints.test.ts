...
describe('getTerminalUrlOpenHint', () => {
  ...
  // Why: with links already opening in Dorka, inverting still lands on the system
  // browser, so the hint must not promise Dorka.
  it('keeps the system-browser wording when inverting but links open in Dorka', () => {
    stubPlatform(true)
  ...
  it('names Dorka when inverting and links open externally', () => {
    stubPlatform(true)
    expect(getTerminalUrlOpenHint({ openLinksInApp: false, modifierInverts: true })).toBe(
      'Click for actions, ⌘+click to open, or ⇧⌘+click to open in Dorka'
    )
  })
  ...
  it('uses the Ctrl chord off macOS', () => {
    stubPlatform(false)
    expect(getTerminalUrlOpenHint({ openLinksInApp: false, modifierInverts: true })).toBe(
      'Click for actions, Ctrl+click to open, or Shift+Ctrl+click to open in Dorka'
    )
  })
  ...
  it('omits the action-menu gesture when terminal link actions are disabled', () => {
    ...
    ).toBe('Ctrl+click to open, or Shift+Ctrl+click to open in Dorka')
  })
})
...
describe('terminalUrlOpenHintOptionsFor', () => {
  ...
  // Why: openHttpLink refuses to route a remote-owned URL into Dorka, so promising
  // "open in Dorka" there would advertise a click that lands somewhere else.
  ...
  // can still reach Dorka while some other pane's runtime is active.
  it('keeps inversion for a local pane while a remote runtime is active', () => {
    ...
    expect(getTerminalUrlOpenHint(options)).toContain('to open in Dorka')
  })

  it('keeps inversion for a runtime pane when its host can open an Dorka browser', () => {
    const options = terminalUrlOpenHintOptionsFor(
      {
        openLinksInApp: false,
        openLinksInAppModifierInverts: true
