...
export function installAgentIdleWorkingHandlers(session: ConnectPanePtySession): void {
  ...
  // Why: inject DORKA_PANE_KEY so global Claude/Codex hooks can attribute their
  // callbacks to the correct Dorka pane without resolving worktrees from cwd.
  // The key matches the `${tabId}:${leafId}` composite used for cacheTimerByKey
  // and agentStatusByPaneKey. Treat it as opaque outside Dorka.
  ...
  session.folderWorkspace =
    session.parsedWorkspaceKey?.type === 'folder'
      ...
      : null
  session.workspaceEnv = { DORKA_WORKSPACE_ID: session.deps.worktreeId }
  if (session.folderWorkspace) {
    session.workspaceEnv.DORKA_PROJECT_GROUP_ID = session.folderWorkspace.projectGroupId
    session.workspaceEnv.DORKA_WORKSPACE_ROOT = session.folderWorkspace.folderPath
  }
  session.paneIdentityEnv = {
    ...session.workspaceEnv,
    DORKA_PANE_KEY: session.cacheKey,
    DORKA_TAB_ID: session.deps.tabId,
    DORKA_WORKTREE_ID: session.deps.worktreeId,
    ...(session.launchToken ? { DORKA_AGENT_LAUNCH_TOKEN: session.launchToken } : {})
  }
  session.paneEnv = {
