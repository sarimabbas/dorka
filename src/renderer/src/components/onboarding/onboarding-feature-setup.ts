...
import { showDorkaCliRegistrationPromptToast } from '@/lib/agent-skill-cli-prerequisite'
...
export function createOnboardingFeatureSetupDeps(
  ...
  // Register `dorka` on the same PATH used by the skill install (#12103).
  ...
  return {
    ...
    showCliRegistrationPrompt: showDorkaCliRegistrationPromptToast,
    installCli: () =>
...
export async function runOnboardingFeatureSetup(
  ...
  try {
    ...
    if (!status.supported) {
      warnings.push({
        featureId: 'cli',
        message: status.detail ?? 'Dorka CLI registration is not available on this platform.'
      })
    } else if (status.pathConfigured === null) {
      // Why: an unknown registry read cannot safely drive a PATH read-modify-write.
      warnings.push({
        featureId: 'cli',
        message: status.detail ?? 'Dorka could not check your Windows user PATH.'
      })
      ...
      if (next.state !== 'installed') {
        warnings.push({
          featureId: 'cli',
          message: next.detail ?? 'Dorka CLI registration needs attention.'
        })
