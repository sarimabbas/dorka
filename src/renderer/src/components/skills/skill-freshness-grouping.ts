 ...
 * wrong in a way the update cannot fix — an edited copy, one Dorka could not read.
 *
...
export function groupSkillFreshness(
  ...
  for (const [name, entries] of byName) {
    ...
    // row would assert Dorka considered an update it never could. Locations below are still
