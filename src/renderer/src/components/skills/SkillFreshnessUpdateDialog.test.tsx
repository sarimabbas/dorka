...
function eligibleInventory(): SkillFreshnessInventory {
  return {
    schemaVersion: 1,
    installations: [placement('dorka-cli')],
    eligibleUpdateNames: ['dorka-cli'],
    scanIssues: [],
    scannedAt: 1
  }
}
...
describe('SkillFreshnessUpdateDialog', () => {
  ...
  it('starts a background run with the eligible names', async () => {
    ...
    expect(skillsApi.startUpdateRun).toHaveBeenCalledWith(['dorka-cli'])
  })

  it('shows indeterminate progress and says the run survives closing', async () => {
    await renderDialog()
    await openViaRequest()
    await emitRun({ state: 'running', names: ['dorka-cli'], startedAt: 1, output: '' })
    ...
    expect(
      container?.querySelector('[data-skill-row="dorka-cli"]')?.getAttribute('data-state-label')
    ).toBe('pending')
  })
  ...
  it('attributes failures to the names the re-scan says are still outdated', async () => {
    mocks.inventory = {
      schemaVersion: 1,
      installations: [placement('dorka-cli'), placement('orchestration')],
      eligibleUpdateNames: ['dorka-cli', 'orchestration'],
      scanIssues: [],
      scannedAt: 1
    }
    await renderDialog()
  ...
  it('keeps the same row elements across the whole run instead of swapping layouts', async () => {
    await renderDialog()
    await openViaRequest()
    const before = container?.querySelector('[data-skill-row="dorka-cli"]')
    expect(before?.getAttribute('data-state-label')).toBe('available')

    await emitRun({ state: 'running', names: ['dorka-cli'], startedAt: 1, output: '' })
    const during = container?.querySelector('[data-skill-row="dorka-cli"]')
    ...
    mocks.inventory = {
      schemaVersion: 1,
      installations: [placement('dorka-cli', { status: 'current', installedReleaseRevision: 2 })],
      eligibleUpdateNames: [],
      scanIssues: [],
      scannedAt: 5
    }
    await emitRun({ state: 'success', names: ['dorka-cli'], finishedAt: 2, output: 'done' })
    await rerender()
    const after = container?.querySelector('[data-skill-row="dorka-cli"]')
  ...
  it('collapses each skill’s locations behind its own disclosure', async () => {
    ...
    const row = container?.querySelector('[data-skill-row="dorka-cli"]')
  ...
  it('shows the up-to-date state once every installation is current', async () => {
    ...
    expect(container?.textContent).toContain('All installed Dorka skills are up to date.')
  ...
  it('raises no row for a skill whose only finding is a project-owned copy', async () => {
    ...
    // directory. Dorka only ever updates global skills, so a "Skipped" row here asserts
  ...
  it('keeps the stale-record remedy when a project copy is listed beside it', async () => {
    ...
    // runnable command with advice about a copy the user never asked Dorka to update.
    mocks.inventory = {
      schemaVersion: 1,
      installations: [
  ...
  it('stays coherent while an idle re-scan is in flight', async () => {
    ...
    expect(container?.textContent).toContain('Checking installed Dorka skills…')
  ...
  it('surfaces a scan error instead of a stale summary', async () => {
    ...
    expect(container?.querySelector('[data-skill-row="dorka-cli"]')).not.toBeNull()
    ...
    expect(container?.querySelector('[data-skill-row="dorka-cli"]')).toBeNull()
    expect(findButton('Update 1 skill')).toBeUndefined()
  })
  ...
  it('shows incomplete plugin coverage without presenting a fabricated skill copy', async () => {
    mocks.inventory = {
      schemaVersion: 1,
      installations: [
        placement('dorka-cli', { status: 'current', observedPackageDigest: 'current' })
      ],
      eligibleUpdateNames: [],
      scanIssues: [
        {
    ...
    expect(container?.textContent).toContain(
      'Dorka could not finish checking plugin-managed skills.'
    )
    ...
    expect(container?.textContent).not.toContain('All installed Dorka skills are up to date.')
    ...
    expect(container?.textContent).not.toContain(
      '/home/.codex/plugins/cache/vendor/locked/dorka-cli'
    )
  })
  ...
  // Why: Dorka's own traversal bounds are not the user's to act on. Headlining them
