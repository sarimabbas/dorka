  ...
  switch (chip) {
    case 'newer':
      return translate(
        ...
        'This copy is a later version than the one this build of Dorka ships, so Dorka left it alone rather than roll it back. Updating Dorka will bring the two back in line.'
      )
    case 'unrecognized':
      return translate(
        'auto.components.skills.SkillFreshnessRow.skippedReasonUnrecognized',
        'The copy here doesn’t match the official version — it may be modified, or a different skill with the same name. Dorka left it out of the update so it won’t overwrite it. Remove it if you want Dorka to update this skill.'
      )
    case 'read-only':
      return translate(
        'auto.components.skills.SkillFreshnessRow.skippedReasonReadOnly',
        'This copy is in a read-only location, so Dorka left it out of the update. Change its permissions to let Dorka update it.'
      )
    case 'inaccessible':
      return translate(
        'auto.components.skills.SkillFreshnessRow.skippedReasonInaccessible',
        'Dorka couldn’t read this copy, so it left the skill out of the update.'
      )
    case 'in-a-repo':
      return translate(
        'auto.components.skills.SkillFreshnessRow.skippedReasonInRepo',
        'This is a project skill, not a global one — Dorka only updates your global skills, so it left this out of the update.'
      )
    case 'plugin-cache':
      return translate(
        'auto.components.skills.SkillFreshnessRow.skippedReasonPluginCache',
        'A plugin manages this skill, so Dorka left it out of the update — update the plugin instead.'
      )
    case 'external-link':
      return translate(
        'auto.components.skills.SkillFreshnessRow.skippedReasonExternalLink',
        'This copy is a shortcut pointing outside Dorka’s skill folders, so Dorka left it out of the update.'
      )
    case 'broken-link':
      return translate(
        'auto.components.skills.SkillFreshnessRow.skippedReasonBrokenLink',
        'This copy is a shortcut to something that no longer exists, so Dorka left it out — you can safely delete it.'
      )
    case 'duplicate':
      return translate(
    ...
    case undefined:
      ...
      return skillName
        ...
        : translate(
            ...
            'Dorka left this skill out of the update command.'
          )
  }
}
