  ...
  const handlePrimaryAction = async (): Promise<void> => {
    ...
    switch (currentTip.action) {
      case 'learn-cmd-j-palette': {
        ...
        trackCmdJPaletteFeatureTipAcknowledged(
          getDorkaCliFeatureTipTelemetrySource(modalData.source)
        )
        closeModal()
        break
      }
      ...
      case 'setup-cli': {
        ...
        const canApplySetupResult = (): boolean =>
          ...
          setupRequestIdRef.current === setupRequestId
        const telemetrySource = getDorkaCliFeatureTipTelemetrySource(modalData.source)
        trackDorkaCliFeatureTipSetupClicked(telemetrySource)
        setPrimaryBusy(true)
        try {
          const result = await installCliFromFeatureTip(() => window.api.cli.install())
          if (result.kind === 'installed') {
            trackDorkaCliFeatureTipSetupResult(telemetrySource, 'installed')
          ...
          trackDorkaCliFeatureTipSetupResult(telemetrySource, 'needs_attention')
          if (!canApplySetupResult()) {
            return
          }
          toast.warning(
          ...
          const message = error instanceof Error ? error.message : 'Failed to install Dorka CLI.'
          if (
            import.meta.env.DEV &&
            message.includes('Development mode uses a generated launcher for validation only')
          ) {
            trackDorkaCliFeatureTipSetupResult(telemetrySource, 'dev_preview')
            if (!canApplySetupResult()) {
              return
            }
            enableOrchestrationSkillSetup()
          ...
          trackDorkaCliFeatureTipSetupResult(telemetrySource, 'failed')
          if (canApplySetupResult()) {
            toast.error(message)
          }
        } finally {
