...
describe('PrivacyPane — isEnvBlocked', () => {
  ...
  it('returns true for DORKA_TELEMETRY_DISABLED', () => {
    expect(isEnvBlocked({ effective: 'disabled', reason: 'dorka_disabled' })).toBe(true)
  })
...
describe('PrivacyPane — envVarNameForReason', () => {
  ...
  it('maps dorka_disabled to DORKA_TELEMETRY_DISABLED', () => {
    expect(envVarNameForReason('dorka_disabled')).toBe('DORKA_TELEMETRY_DISABLED')
  })
...
describe('PrivacyPane — computeBlockedReason', () => {
  ...
  it('names DORKA_TELEMETRY_DISABLED as the env reason when set', () => {
    const result = computeBlockedReason({ effective: 'disabled', reason: 'dorka_disabled' })
    expect(result).toEqual({ kind: 'env', reason: 'dorka_disabled' })
  })
