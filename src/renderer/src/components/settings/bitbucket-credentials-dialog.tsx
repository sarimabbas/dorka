        ...
        <DialogHeader className="gap-3">
          ...
          <DialogDescription>
            {translate(
              ...
              'Use a Bitbucket Cloud credential to browse pull requests and build statuses. Dorka verifies it before saving.'
            )}
          </DialogDescription>
        </DialogHeader>
        {locked ? (
          <p className="text-xs text-muted-foreground">
            {translate(
              ...
              'Bitbucket is already configured through DORKA_BITBUCKET_* environment variables, which take precedence. Unset them to save a credential in Dorka.'
            )}
          </p>
        ) : (
            ...
            <p className="flex items-start gap-1.5 text-[11px] text-muted-foreground/70">
              ...
              {remoteRuntime
                ? translate(
                    ...
                    'Stored on this machine, not on the active remote runtime — set DORKA_BITBUCKET_* there instead. Environment variables always take precedence over what you save here.'
                  )
                : translate(
                    'auto.components.settings.bitbucket.credentials.dialog.storageNote',
                    'Stored on this machine with encrypted storage when the OS keychain is available. DORKA_BITBUCKET_* environment variables always take precedence over what you save here.'
                  )}
            </p>
          </div>
        )}
