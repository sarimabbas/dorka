...
describe('AgentsPane', () => {
  ...
  it('puts durable Agent presets before client launch preferences', () => {
    ...
    expect(markup.indexOf('Create reusable agent configurations')).toBeLessThan(
      markup.indexOf('Default Agent')
    )
  })
  ...
  it('renders the keep-awake modes from settings', () => {
    ...
    expect(markup).toContain(
      'Choose On, Agent, or Off. Agent mode stays awake while agents are working. Dorka also asks this device to stay awake when the lid is closed, subject to its power policy.'
    )
