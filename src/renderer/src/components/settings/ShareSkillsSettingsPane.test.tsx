...
const mocks = vi.hoisted(() => ({
  ...
  state: {
    dorkaProfileAuthStatus: { configured: true, state: 'connected' } as Record<
      string,
      unknown
    > | null,
    isWebClient: false,
...
describe('ShareSkillsSettingsPane', () => {
  ...
  it('requires an explicit desktop grant for agent publishing', async () => {
    ...
    await user.click(
      screen.getByRole('switch', {
        name: 'Allow agents and the Dorka CLI to publish skill links'
      })
    )
  ...
  it('does not offer desktop publishing from the web client', () => {
    ...
    expect(screen.getByText(/available in the Dorka desktop app/)).toBeInTheDocument()
