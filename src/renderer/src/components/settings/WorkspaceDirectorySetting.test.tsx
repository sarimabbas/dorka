...
describe('WorkspaceDirectorySetting', () => {
  it('keeps typed workspace paths local until blur', () => {
    ...
    expect(updateSettings).toHaveBeenCalledWith({ workspaceDir: 'dorka-workspaces' })
  })
  ...
  it('does not commit Enter while IME composition is active', () => {
    ...
    expect(getInput().value).toBe('dorka-workspaces')
    expect(updateSettings).not.toHaveBeenCalled()
  })
  ...
  it('allows retrying the same draft when the persisted value has not changed', () => {
    ...
    expect(updateSettings).toHaveBeenNthCalledWith(1, { workspaceDir: 'dorka-workspaces' })
    expect(updateSettings).toHaveBeenNthCalledWith(2, { workspaceDir: 'dorka-workspaces' })
  })
})
