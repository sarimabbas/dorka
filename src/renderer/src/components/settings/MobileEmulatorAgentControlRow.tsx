...
export function MobileEmulatorAgentControlRow(): React.JSX.Element {
  ...
  const cliSkillInstallCommand = buildSkillCommandForRuntime(DORKA_CLI_SKILL_INSTALL_COMMAND)
  const cliSkillUpdateCommand = buildSkillCommandForRuntime(DORKA_CLI_SKILL_UPDATE_COMMAND)
        ...
        <div className="space-y-0.5">
          ...
          <p className="text-xs text-muted-foreground">
            {translate(
              ...
              'Let coding agents control the active mobile emulator with Dorka CLI commands.'
            )}
          </p>
        </div>
        <span
          ...
          <div className="min-w-0 flex-1 space-y-1">
            ...
            <p className="text-xs text-muted-foreground">
              {translate(
                ...
                'Registers the Dorka CLI command so agents can control the active emulator from their shell.'
              )}
            </p>
        ...
        <div className={cn('py-3', setup.step2Blocked && 'opacity-60')}>
          <AgentSkillSetupPanel
            ...
            description={translate(
              ...
              'Enables agents to use Dorka CLI commands, including mobile emulator control.'
            )}
            command={cliSkillInstallCommand}
            installedCommand={cliSkillUpdateCommand}
            terminalTitle="Dorka CLI skill setup"
            terminalAriaLabel="Dorka CLI skill install terminal"
            terminalWorktreeId="settings-mobile-emulator-dorka-cli-skill-terminal"
            ...
            openingHint={translate(
              ...
              'Checking Dorka CLI before opening skill setup.'
            )}
            onBeforeOpenTerminal={async () => {
              await ensureDorkaCliAvailableForAgentSkillTerminal()
            }}
