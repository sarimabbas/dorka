...
export function MobileRelayMintFailureNotice({
  ...
  const reconnectRequired = useAppStore(
    (state) => state.dorkaProfileAuthStatus?.state === 'reconnect-required'
  )
    ...
    : providerMissing
      ...
      : reconnectRequired
        ? translate(
            ...
            'Your Dorka account session expired.'
          )
        : translate(
    ...
    : providerMissing
      ...
      : reconnectRequired
        ? translate(
            ...
            'Sign in again to use Dorka Relay, or use LAN to pair over Tailscale or the same Wi‑Fi.'
          )
        : translate(
