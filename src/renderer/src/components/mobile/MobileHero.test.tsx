...
describe('HeroFlow height', () => {
  ...
  it('shows Relay mint failure with no QR and the beta note', () => {
    ...
    expect(screen.getByText('Dorka Relay is in beta.')).toBeInTheDocument()
  })
  ...
  it('does not offer a futile retry when Relay is unavailable on the desktop', () => {
    renderFlow(1, {
      relayMintFailure: {
        ...
        message: 'Dorka Relay is not available on this desktop'
      }
    })
    expect(screen.getByRole('alert')).toHaveTextContent(
      'Dorka Relay isn’t available on this desktop'
    )
  ...
  it('demotes the network address picker to a disclosure on Dorka Relay', async () => {
    const props: React.ComponentProps<typeof MobileHeroPairingStep> = {
      pairQrDataUrl: null,
      pairingUrl: null,
      pairingQrError: false,
  ...
  it('keeps a custom address visible on Dorka Relay', () => {
