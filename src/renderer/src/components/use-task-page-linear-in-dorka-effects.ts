...
import {
  ...
  filterLinearIssuesForInDorkaWorkspace
...
export function useTaskPageLinearInDorkaEffects(model: TaskPageLinearListEffectsModel) {
  const {
    fetchLinearIssue,
    refreshLinearIssue,
    linearConnected,
    ...
    inDorkaLinkedLinearRefsSignature,
    inDorkaLinkedLinearRefsRef
  } = model
  ...
  useEffect(() => {
    ...
    if (taskSource !== 'linear' || linearMode !== 'in-dorka' || !linearConnected) {
      return
    }
    let cancelled = false
    const linkedRefs = inDorkaLinkedLinearRefsRef.current
    const requestSignature = `in-dorka::${selectedLinearWorkspaceId ?? 'default'}::${inDorkaLinkedLinearRefsSignature}`
        ...
        if (loaded.length === 0) {
          setLinearError(
            translate(
              ...
              'Unable to load Linear issues linked to an Dorka workspace.'
            )
          )
          setLinearIssues([])
          setLinearLoading(false)
        ...
        if (loaded.length !== results.length) {
          setLinearError(
            translate(
              ...
              'Some Linear issues linked to an Dorka workspace could not be loaded. Refresh to try again.'
            )
          )
        }
        setLinearIssues(filterLinearIssuesForInDorkaWorkspace(loaded, selectedLinearWorkspaceId))
        setLinearLoading(false)
      })
      .catch((err) => {
        if (
...
export type TaskPageLinearInDorkaEffectsModel = ReturnType<typeof useTaskPageLinearInDorkaEffects>
