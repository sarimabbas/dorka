        ...
        <div role="listbox" aria-label={addHostLabel} onMouseLeave={() => setHoveredKey(null)}>
          ...
          {onAddRemoteServer ? (
            <RunTargetRow
              icon={<Cloud className="size-3.5 shrink-0 text-muted-foreground" />}
              label={translate(
                'auto.components.NewWorkspaceComposerCard.addRemoteDorkaServer',
                'Add Remote Dorka Server'
              )}
              detail={translate(
                'auto.components.NewWorkspaceComposerCard.addRemoteDorkaServerHint',
                'Pair another Dorka runtime'
              )}
