...
describe('useJiraUrlSource', () => {
  ...
  it('requires an explicit account choice and binds the exact account context', async () => {
    ...
    mocks.lookupJiraIssueSummary.mockResolvedValue(issue('DORKA-123', 'site-b'))
    ...
    expect(result.current.issue?.key).toBe('DORKA-123')
  ...
  it('reuses an already-loaded connection instead of re-reading status', async () => {
    ...
    mocks.lookupJiraIssueSummary.mockResolvedValue(issue('DORKA-1'))
    ...
    const { result } = renderHook(() =>
      useJiraUrlSource({
        value: 'https://company.atlassian.net/browse/DORKA-1',
        enabled: true,
        sourceContext: context,
        connection: loaded
      })
    ...
    expect(result.current.issue?.key).toBe('DORKA-1')
  ...
  it('discards a late issue response after the URL changes', async () => {
    ...
    mocks.lookupJiraIssueSummary.mockImplementation(
      (
        ...
        return Promise.resolve(issue('DORKA-2'))
      }
    )
    const { result, rerender } = renderHook(
      ...
      { initialProps: { value: 'https://company.atlassian.net/browse/DORKA-1' } }
    )
    await advanceLookup()

    rerender({ value: 'https://company.atlassian.net/browse/DORKA-2' })
    await advanceLookup()
    expect(oldSignal?.aborted).toBe(true)
    expect(result.current.issue?.key).toBe('DORKA-2')

    await act(async () => {
      oldIssue.resolve(issue('DORKA-1'))
  ...
  it('forces Retry past an invalid fulfilled summary result', async () => {
    const context = sourceContext()
    mocks.readJiraStatus.mockResolvedValue(status([site('site-a')], 'site-a'))
    mocks.lookupJiraIssueSummary.mockResolvedValueOnce(null).mockResolvedValueOnce(issue('DORKA-1'))
    const { result } = renderHook(() =>
      useJiraUrlSource({
        value: 'https://company.atlassian.net/browse/DORKA-1',
        enabled: true,
    ...
    expect(result.current.issue?.key).toBe('DORKA-1')
    expect(mocks.lookupJiraIssueSummary).toHaveBeenLastCalledWith(context, 'DORKA-1', 'site-a', {
      force: true,
