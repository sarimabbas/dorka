...
const dorka = project('dorka', 'dorka', 'stablyai/orca')
const relay = project('relay', 'dorka-relay', 'stablyai/dorka-relay')
...
describe('rankProjectOptions', () => {
  it('ranks a name-prefix match above a mid-name match', () => {
    const ranked = rankProjectOptions([relay, dorka], 'dorka', [])
    expect(ranked[0]?.option.id).toBe('dorka')
  })
  ...
  it('orders an unfiltered list by recency', () => {
    const ranked = rankProjectOptions([dorka, relay, gateway], '', ['gateway', 'relay'])
    expect(ranked.map((r) => r.option.id)).toEqual(['gateway', 'relay', 'dorka'])
  })

  it('returns nothing for an oversized query rather than scanning it', () => {
    expect(rankProjectOptions([dorka], 'x'.repeat(4096), [])).toEqual([])
  })
})
...
describe('sectionProjectOptions', () => {
  ...
  it('keeps a short list unsectioned', () => {
    const matches = rankProjectOptions([dorka, relay], '', ['relay'])
...
describe('getAmbiguousProjectOptionIds', () => {
  it('flags only ids whose display name repeats', () => {
    ...
    const ids = getAmbiguousProjectOptionIds([a, b, dorka])
