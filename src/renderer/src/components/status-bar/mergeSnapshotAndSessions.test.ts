...
describe('mergeSnapshotAndSessions', () => {
  ...
  it('includes browser-only workspaces in their repo', () => {
    const worktree = {
      id: 'dorka::/Users/me/browser-only',
      repoId: 'dorka',
    ...
    const out = mergeSnapshotAndSessions(
      ...
      [],
      baseCtx({
        repoDisplayNameById: new Map([['dorka', 'DORKA']]),
  ...
  it('passes through snapshot worktrees with numeric metrics and hasLocalSamples', () => {
    const wt: WorktreeMemory = {
      worktreeId: 'dorka::/Users/me/Triton',
      worktreeName: 'Triton',
      repoId: 'dorka',
      repoName: 'DORKA',
      cpu: 1.5,
  ...
  it('binds a deferred SSH row so its single-row kill cannot skip confirmation', () => {
    ...
    const sessionId = 'dorka::/remote/Stingray@@deferred1'
    const out = mergeSnapshotAndSessions(
      null,
      [{ id: sessionId, cwd: '', title: 'dorka/Stingray', agentOwnership: 'absent' as const }],
      baseCtx({
        tabsByWorktree: { 'dorka::/remote/Stingray': [makeTab('tab-ssh')] },
        deferredSshSessionIdsByTabId: { 'tab-ssh': sessionId },
        repoConnectionIdById: new Map([['dorka', 'ssh-conn-1']])
      })
    )
  ...
  it('@@ parse: an SSH-style session id resolves to its worktree group', () => {
    const ds: DaemonSession[] = [
      {
        id: 'dorka::/remote/Stingray@@abcd1234',
        cwd: '',
        title: 'dorka/Stingray',
    ...
    expect(out[0].worktrees[0]).toMatchObject({
      worktreeId: 'dorka::/remote/Stingray',
    ...
    expect(out[0].worktrees[0].sessions[0]).toMatchObject({
      sessionId: 'dorka::/remote/Stingray@@abcd1234',
      hasLocalSamples: false,
      cpu: null,
      memory: null,
      bound: false
  ...
  it('warm-reattach local PTY: chip stays off when repo has no connectionId', () => {
    ...
    const ds: DaemonSession[] = [
      {
        id: 'dorka::/local/Triton@@deadbeef',
        cwd: '/local/Triton',
        title: 'dorka/Triton',
        agentOwnership: 'absent' as const
      }
    ...
    const ctx = baseCtx({
      repoConnectionIdById: new Map([['dorka', null]])
    })
    const out = mergeSnapshotAndSessions(null, ds, ctx)
    expect(out[0]).toMatchObject({
      repoId: 'dorka',
  ...
  it('tab walk wins over @@ parse when they disagree', () => {
    ...
    const ctx = baseCtx({
      tabsByWorktree: {
        'dorka::/correct/path': [makeTab(tabId, 'My Tab')]
      },
      ptyIdsByTabId: { [tabId]: ['dorka::/wrong/path@@feedface'] }
    })
    const out = mergeSnapshotAndSessions(null, ds, ctx)
    expect(out[0].worktrees[0].worktreeId).toBe('dorka::/correct/path')
  ...
  it('treats startup deferred reattach tab ptyId wake hints as bound sessions', () => {
    ...
    const sessionId = 'dorka::/Users/me/Triton@@deferred'
    const ds: DaemonSession[] = [
      {
        id: sessionId,
        cwd: '/Users/me/Triton',
    ...
    const ctx = baseCtx({
      tabsByWorktree: {
        'dorka::/Users/me/Triton': [restoredTab]
      },
      ptyIdsByTabId: { [tabId]: [] }
    })
  ...
  it('local-bound interaction state: numeric metrics + bound=true + tabId set', () => {
    ...
    const ctx = baseCtx({
      tabsByWorktree: { 'dorka::/Users/me/Triton': [makeTab(tabId)] },
  ...
  it('uses repoDisplayNameById to humanize new project groups when available', () => {
    ...
    const out = mergeSnapshotAndSessions(null, ds, ctx)
    expect(out[0].repoName).toBe('DORKA')
  })
...
it('resolves a duplicated tab id to the first tab, as the replaced scan did', () => {
  ...
  const out = mergeSnapshotAndSessions(
    makeSnapshot([wt]),
    [],
    baseCtx({ tabsByWorktree: { 'dorka::/Users/me/Triton': [first, second] } })
  )
...
it('keeps tab labels scoped to their own worktree', () => {
  ...
  const out = mergeSnapshotAndSessions(
    makeSnapshot([wt]),
    [],
    baseCtx({ tabsByWorktree: { 'dorka::/Users/me/Other': [makeTab('shared', 'Elsewhere')] } })
  )
...
it('attaches later daemon sessions to the first row of a duplicated worktree', () => {
  ...
  const out = mergeSnapshotAndSessions(
    ...
    [{ id: 'dorka::/Users/me/Triton@@late', cwd: '', title: '', agentOwnership: 'unknown' }],
    baseCtx()
  )
