...
export const EMULATOR_LOCAL_SHUTDOWN_EVENT = 'dorka:emulator-shutdown'
...
export function useEmulatorPaneSessionEvents({
  ...
  useEffect(() => {
    ...
    window.addEventListener('dorka:emulator-auto-attach', onAuto)
    return () => window.removeEventListener('dorka:emulator-auto-attach', onAuto)
