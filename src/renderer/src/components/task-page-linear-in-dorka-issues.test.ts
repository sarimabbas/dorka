...
} from './task-page-linear-in-dorka-issues'
...
describe('filterLinearIssuesForInDorkaWorkspace', () => {
  it('keeps issues without workspace metadata when a workspace is selected', () => {
    ...
    expect(
      filterLinearIssuesForInDorkaWorkspace(issues, 'ws-a').map((item) => item.identifier)
    ).toEqual(['ENG-1', 'ENG-3'])
  })
})
