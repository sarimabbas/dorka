...
const SHARED_WORKTREE_ID = 'repo-shared::/work/dorka-feature'
...
const localWorktree: Worktree = {
  ...
  repoId: 'repo-shared',
  path: '/work/dorka-feature',
  hostId: 'local',
  head: 'abc123',
  branch: 'feature',
  isBare: false,
  isMainWorktree: false,
  displayName: 'dorka-feature',
  comment: '',
  linkedIssue: null,
  linkedPR: null,
...
describe('projectWorkspaceSurfaces', () => {
  it('emits one surface per workspace id when two hosts publish the same worktree', () => {
    ...
    expect(surfaces).toEqual([{ id: SHARED_WORKTREE_ID, path: '/work/dorka-feature' }])
  })
  ...
  it('mounts the active folder workspace at its resolved host path, not the first row', () => {
    ...
    expect(surfaces).toEqual([{ id: 'folder:folder-shared', path: '/remote/dorka' }])
  })
  ...
  it('keeps first-wins for a colliding folder workspace that is not the active one', () => {
    ...
    expect(surfaces).toEqual([{ id: 'folder:folder-shared', path: '/work/dorka-local' }])
    expect(warn).toHaveBeenCalledWith(
      '[workspace-surface] dropping colliding folder path',
      expect.objectContaining({
        kept: '/work/dorka-local',
        dropped: '/remote/dorka'
      })
    )
    warn.mockRestore()
  })
  ...
  it('switches the active folder path from first-wins to the host that hydrates', () => {
    ...
    expect(hydrating).toEqual([{ id: 'folder:folder-shared', path: '/work/dorka-local' }])
    expect(hydrated).toEqual([{ id: 'folder:folder-shared', path: '/remote/dorka' }])
  ...
  it('keeps distinct workspaces on one host', () => {
    const otherWorktree: Worktree = {
      ...localWorktree,
      id: 'repo-shared::/work/dorka-other',
      path: '/work/dorka-other'
    }

    expect(project({ worktrees: [localWorktree, otherWorktree] })).toEqual([
      { id: SHARED_WORKTREE_ID, path: '/work/dorka-feature' },
      { id: 'repo-shared::/work/dorka-other', path: '/work/dorka-other' }
    ])
  })
})
...
describe('projectWorkspaceSurfaces never under-selects', () => {
  ...
  it('emits every distinct id from a mixed local, SSH, runtime and folder catalog', () => {
    const distinctWorktree: Worktree = {
      ...sshWorktree,
      id: 'repo-shared::/work/dorka-ssh-only',
      path: '/work/dorka-ssh-only'
    }
  ...
  it('mounts a local-only catalog whose rows never name a host', () => {
    ...
    expect(
      ...
      { id: SHARED_WORKTREE_ID, path: '/work/dorka-feature' },
      { id: 'repo-shared::/work/dorka-other', path: '/work/dorka-other' },
      { id: 'folder:folder-local-only', path: '/work/dorka-local' }
    ])
  })
  ...
  it('collapses folder rows across three hosts to one surface without losing the id', () => {
    ...
    expect(surfaces).toEqual([{ id: 'folder:folder-shared', path: '/ssh/dorka' }])
  })
  ...
  it('does not let a folder row that names no host win the local tie-break', () => {
    ...
    ).toEqual([{ id: 'folder:folder-shared', path: '/work/dorka-local' }])
  })
