...
function deps(
  ...
  return {
    ...
    deps: {
      ...
      destinations: overrides.destinations ?? { primary: 'system', alternate: 'dorka' },
...
describe('handleNativeChatWebLink', () => {
  ...
  it('opens the destination popover on a plain click', () => {
    ...
    expect(requests[0]?.alternate?.label).toBe('Dorka Browser')
  })
  ...
  it('opens the alternate destination on a shift+modifier click', () => {
    ...
    expect(mocks.openRoutedHttpLink).toHaveBeenCalledWith(
      'https://a/',
      expect.objectContaining({ forceDestination: 'dorka' })
    )
  })
