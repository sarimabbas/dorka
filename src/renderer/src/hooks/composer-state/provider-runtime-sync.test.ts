...
describe('useComposerProviderRuntimeSync', () => {
  it('drops an older provider slug result after the selected repo/runtime context changes', async () => {
    ...
    first.resolve({ owner: 'dorka', repo: 'stale' })
    await act(async () => first.promise)
    expect(setSelectedRepoSlug).not.toHaveBeenCalledWith({ owner: 'dorka', repo: 'stale' })

    second.resolve({ owner: 'dorka', repo: 'current' })
    await act(async () => second.promise)
    expect(setSelectedRepoSlug).toHaveBeenLastCalledWith({ owner: 'dorka', repo: 'current' })
  })
})
