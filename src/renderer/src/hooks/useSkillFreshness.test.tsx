...
describe('useSkillFreshness', () => {
  ...
  it('does not scan, subscribe, or refresh while local freshness is disabled', async () => {
    ...
    await act(async () => window.dispatchEvent(new Event('dorka:installed-agent-skills-changed')))
    ...
    expect(
      addEventListener.mock.calls.filter(
        ([name]) => name === 'focus' || name === 'dorka:installed-agent-skills-changed'
      )
    ).toHaveLength(0)
  })
  ...
  it('forces one fresh scan before restoring authority after re-enable', async () => {
    ...
    const freshnessInventory = vi
      .fn()
      .mockResolvedValueOnce(inventory(1, ['dorka-cli']))
      .mockReturnValueOnce(second.promise)
    window.api = { skills: { freshnessInventory } } as never
    ...
    expect(freshnessInventory).toHaveBeenCalledTimes(1)
    expect(state?.inventory?.eligibleUpdateNames).toEqual(['dorka-cli'])
