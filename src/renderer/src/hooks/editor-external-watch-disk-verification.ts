...
export function scheduleEditorChangedOnDiskMark(
  ...
  // Why: the fs event may be the echo of Dorka's own save — verify disk really differs from our last write before showing a "changed on disk" banner.
...
export function scheduleSelfWriteAwareEditorExternalReload(
  ...
  // Why: a self-write stamp only proves recent change; compare disk content so it suppresses only Dorka's echo, not a newer agent write in the same TTL.
  void readFileForEchoVerification({
