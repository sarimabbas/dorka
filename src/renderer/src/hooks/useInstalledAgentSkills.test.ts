...
describe('hasInstalledAgentSkill', () => {
  it('matches installed skills by summarized name', () => {
    expect(hasInstalledAgentSkill([skill({ name: 'dorka-cli' })], 'dorka-cli')).toBe(true)
  })
    ...
    expect(
      hasInstalledAgentSkill(
        [
          skill({
            name: 'Dorka CLI',
            directoryPath: 'C:\\Users\\test\\.agents\\skills\\dorka-cli'
          })
        ],
        'dorka-cli'
      )
  ...
  it('ignores non-installed discovery entries', () => {
    expect(
      hasInstalledAgentSkill([skill({ name: 'dorka-cli', installed: false })], 'dorka-cli')
    ).toBe(false)
  })
      ...
      hasInstalledAgentSkill(
        [
          skill({
            ...
            directoryPath: '/repo/.agents/skills/dorka-cli',
            skillFilePath: '/repo/.agents/skills/dorka-cli/SKILL.md'
          }),
          skill({
            id: 'skill-2',
            name: 'dorka-cli',
            ...
            directoryPath: '/Users/test/.codex/plugins/cache/vendor/dorka-cli',
            skillFilePath: '/Users/test/.codex/plugins/cache/vendor/dorka-cli/SKILL.md'
          })
        ],
        'dorka-cli',
  ...
  it('counts home skills when matching global installs', () => {
    expect(
      hasInstalledAgentSkill([skill({ name: 'dorka-cli' })], 'dorka-cli', {
        sourceKinds: GLOBAL_AGENT_SKILL_SOURCE_KINDS
      })
    ).toBe(true)
  })
    ...
    expect(
      hasInstalledAgentSkillNamed(
        [
          skill({
            name: 'Linear Tickets',
            directoryPath: 'C:\\Users\\test\\.agents\\skills\\dorka-linear'
          })
        ],
        ['dorka-linear', 'linear-tickets']
      )
  ...
  it('keeps aliases opt-in for unrelated single-name checks', () => {
    expect(hasInstalledAgentSkill([skill({ name: 'linear-tickets' })], 'dorka-linear')).toBe(false)
  })
})
...
describe('discoverInstalledAgentSkills', () => {
  ...
  it('starts a fresh scan when a forced refresh arrives during a background scan', async () => {
    ...
    const freshResult = discoveryResult([skill({ name: 'dorka-cli' })])
  ...
  it('lets completed-scan broadcasts reuse the cached result', async () => {
    ...
    for (const pending of fromSubscribers) {
      await expect(pending).resolves.toMatchObject({
        skills: [expect.objectContaining({ name: 'dorka-linear' })]
      })
    }
    expect(discover).toHaveBeenCalledTimes(1)
  })
