...
1. Set `DORKA_RELAY_LOAD_ACCESS_TOKEN` and pass `--auth-origin`. Every control and refresh then uses the real auth-plane relay-token exchange.
...
node dev/scripts/load-relay-controls.mjs \
  --director-origin https://relay-staging.ondorka.dev \
  --auth-origin https://auth-staging.ondorka.dev \
  --signing-key-file <(gcloud secrets versions access latest \
    --secret=dorka-cloud-auth-signing-key \
...
```sh
DORKA_RELAY_LOAD_ACCESS_TOKEN="$ACCESS_TOKEN" pnpm load:relay:controls -- \
  --director-origin https://relay-staging.ondorka.dev \
  --auth-origin https://auth-staging.ondorka.dev \
  --controls 800 \
  --ramp-seconds 210 \
...
DORKA_RELAY_ADMIN_ID_TOKEN="$(gcloud auth print-identity-token \
  --impersonate-service-account="${CAPACITY_SA}" \
  --project=ondorka-cloud-staging \
  --include-email \
  --audiences=https://relay-staging.ondorka.dev/v1/admin/drain)" \
node dev/scripts/load-relay-controls.mjs \
  --director-origin https://relay-staging.ondorka.dev \
  --auth-origin https://auth-staging.ondorka.dev \
  --signing-key-file <(gcloud secrets versions access latest \
