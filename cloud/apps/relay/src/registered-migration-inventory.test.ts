...
describe('registered migration inventory', () => {
  it('does not restart abandonment when an old source migration lease is refreshed', async () => {
    ...
    expect(formatRegisteredMigrationInventory(fresh)).toEqual([
      '[dorka-relay] migration inventory open=1 expiredRegisteredInactive=1 abandonedRegistered=1',
      '[dorka-relay] migration pair sourceCellId=cell-a targetCellId=cell-b open=1 expiredRegisteredInactive=1 abandoned=1'
    ])
    expect(
      (
