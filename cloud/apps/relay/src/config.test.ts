...
function cellEnvironment(capacity: number): NodeJS.ProcessEnv {
  return {
    DORKA_RELAY_PUBLIC_URL: 'https://c1.relay.example.com',
    DORKA_RELAY_CELL_URL: 'https://c1.relay.example.com',
    DORKA_RELAY_AUTH_ISSUER: 'https://auth.example.com',
    DORKA_RELAY_JWKS_URL: 'https://auth.example.com/.well-known/jwks.json',
    DORKA_RELAY_ASSIGNMENT_SIGNING_KEY: 'assignment-key-with-at-least-thirty-two-bytes',
    DORKA_RELAY_ROLE: 'cell',
    DORKA_RELAY_CELL_ID: 'gce-c1',
    DORKA_RELAY_CELL_CAPACITY: String(capacity),
    DORKA_RELAY_ADMIN_AUDIENCE: 'https://relay.example.com/v1/admin/drain',
    DORKA_RELAY_DEPLOY_SERVICE_ACCOUNT: 'deploy@example.iam.gserviceaccount.com'
  }
}
...
describe('GCE relay capacity configuration', () => {
  it('defaults optional region correction off and bounds the cohort', () => {
    ...
    env.DORKA_RELAY_REGION_CORRECTION_COHORT_PERCENT = '5'
    expect(loadRelayConfig(env).regionCorrectionCohortPercent).toBe(5)
    for (const invalid of ['-1', '101', '1.5', 'not-a-number']) {
      env.DORKA_RELAY_REGION_CORRECTION_COHORT_PERCENT = invalid
  ...
  it('defaults readiness grace to fifteen minutes for JWKS and three for SQL', () => {
    ...
    env.DORKA_RELAY_READINESS_JWKS_GRACE_MS = '0'
    env.DORKA_RELAY_READINESS_SQL_GRACE_MS = String(RELAY_MAX_READINESS_GRACE_MS)
    expect(loadRelayConfig(env)).toMatchObject({
    ...
    for (const invalid of ['-1', String(RELAY_MAX_READINESS_GRACE_MS + 1), '1.5', 'soon']) {
      env.DORKA_RELAY_READINESS_JWKS_GRACE_MS = invalid
      expect(() => loadRelayConfig(env)).toThrow()
    }
  })
  ...
  it('reads an unset readiness grace variable as the default, never as zero', () => {
    const env = cellEnvironment(4_000)
    env.DORKA_RELAY_READINESS_JWKS_GRACE_MS = ''
    env.DORKA_RELAY_READINESS_SQL_GRACE_MS = ''
    expect(loadRelayConfig(env)).toMatchObject({
      readinessJwksGraceMs: RELAY_READINESS_JWKS_GRACE_MS,
      readinessSqlGraceMs: RELAY_READINESS_SQL_GRACE_MS
    })
  ...
  it('requires distinct dedicated admin identities and accepts omitted values', () => {
    ...
    env.DORKA_RELAY_MONITOR_SERVICE_ACCOUNT = ''
    env.DORKA_RELAY_CAPACITY_SERVICE_ACCOUNT = 'capacity@example.iam.gserviceaccount.com'
    env.DORKA_RELAY_ASIA_PROOF_SERVICE_ACCOUNT = 'proof@example.iam.gserviceaccount.com'
    env.DORKA_RELAY_FENCE_SERVICE_ACCOUNT = 'fence@example.iam.gserviceaccount.com'
    expect(loadRelayConfig(env)).toMatchObject({
    ...
    env.DORKA_RELAY_MONITOR_SERVICE_ACCOUNT = env.DORKA_RELAY_DEPLOY_SERVICE_ACCOUNT
    expect(() => loadRelayConfig(env)).toThrow('relay admin service accounts must be distinct')
    env.DORKA_RELAY_MONITOR_SERVICE_ACCOUNT = undefined
    env.DORKA_RELAY_ASIA_PROOF_SERVICE_ACCOUNT = env.DORKA_RELAY_CAPACITY_SERVICE_ACCOUNT
    expect(() => loadRelayConfig(env)).toThrow('relay admin service accounts must be distinct')
  })

  it('requires a distinct paired identity and exact audience for regional host drain', () => {
    const env = cellEnvironment(4_000)
    env.DORKA_RELAY_REHOME_DIRECTOR_SERVICE_ACCOUNT =
      'relay-director@example.iam.gserviceaccount.com'
    expect(() => loadRelayConfig(env)).toThrow(
      'relay rehome identity and audience must be configured together'
    )
    env.DORKA_RELAY_REHOME_AUDIENCE = 'https://relay.example.com/v1/admin/host-drain'
    expect(loadRelayConfig(env)).toMatchObject({
    ...
    env.DORKA_RELAY_REHOME_AUDIENCE = 'https://relay.example.com/v1/admin/drain'
    expect(() => loadRelayConfig(env)).toThrow(
      'relay rehome audience must target the host drain route'
    )
    env.DORKA_RELAY_REHOME_AUDIENCE = 'https://relay.example.com/v1/admin/host-drain'
    env.DORKA_RELAY_RUNTIME_SERVICE_ACCOUNT =
      env.DORKA_RELAY_REHOME_DIRECTOR_SERVICE_ACCOUNT
    expect(() => loadRelayConfig(env)).toThrow(
  ...
  it('keeps legacy cells uncapped and requires a supported hard-cap pair', () => {
    ...
    env.DORKA_RELAY_CELL_CONNECTION_HARD_CAP = String(RELAY_CELL_CONNECTION_HARD_CAP)
    expect(() => loadRelayConfig(env)).toThrow(
      'connection hard cap and unobserved bound must be configured together'
    )
    env.DORKA_RELAY_CELL_CONNECTION_UNOBSERVED_BOUND = '60'
    expect(loadRelayConfig(env)).toMatchObject({
      connectionHardCap: 600,
      connectionUnobservedBound: 60
    })
    env.DORKA_RELAY_CELL_CONNECTION_HARD_CAP = '599'
    expect(() => loadRelayConfig(env)).toThrow()
    env.DORKA_RELAY_CELL_CONNECTION_HARD_CAP = '600'
    env.DORKA_RELAY_CELL_CONNECTION_UNOBSERVED_BOUND = '500'
    expect(() => loadRelayConfig(env)).toThrow()
    env.DORKA_RELAY_CELL_CONNECTION_UNOBSERVED_BOUND = '600'
    expect(() => loadRelayConfig(env)).toThrow()
    env.DORKA_RELAY_CELL_CONNECTION_HARD_CAP = '1000'
    env.DORKA_RELAY_CELL_CONNECTION_UNOBSERVED_BOUND = '60'
    expect(loadRelayConfig(env)).toMatchObject({
      connectionHardCap: 1_000,
      connectionUnobservedBound: 60
    })
    env.DORKA_RELAY_CELL_CONNECTION_UNOBSERVED_BOUND = '900'
    expect(() => loadRelayConfig(env)).toThrow()
  })

  it('accepts hard-cap metadata in director cell inventory', () => {
    const env = cellEnvironment(4_000)
    env.DORKA_RELAY_ROLE = 'director'
    env.DORKA_RELAY_PUBLIC_URL = 'https://relay.example.com'
    env.DORKA_RELAY_CELL_URL = 'https://relay.example.com'
    env.DORKA_RELAY_CELLS_JSON = JSON.stringify([
      {
        id: 'gce-c7',
  ...
  it('accepts only a canonical served image digest', () => {
    const env = cellEnvironment(4_000)
    env.DORKA_RELAY_IMAGE_DIGEST = `sha256:${'a'.repeat(64)}`
    expect(loadRelayConfig(env).imageDigest).toBe(env.DORKA_RELAY_IMAGE_DIGEST)
    env.DORKA_RELAY_IMAGE_DIGEST = 'relay:latest'
    expect(() => loadRelayConfig(env)).toThrow()
  })
  ...
  it('reserves a smaller PostgreSQL connection budget for directors', () => {
    ...
    const directorEnv: NodeJS.ProcessEnv = { ...cellEnv, DORKA_RELAY_ROLE: 'director' }
    directorEnv.DORKA_RELAY_CELLS_JSON = JSON.stringify([
      {
        id: 'gce-c1',
        url: 'https://c1.relay.example.com',
        capacityRequests: 4_000
    ...
    directorEnv.DORKA_RELAY_DATABASE_POOL_MAX = '7'
  ...
  it('defaults to bounded public assignment admission and supports an emergency stop', () => {
    ...
    env.DORKA_RELAY_PUBLIC_ASSIGNMENTS_ENABLED = 'false'
    env.DORKA_RELAY_REGIONAL_PLACEMENT_ENABLED = 'false'
    env.DORKA_RELAY_PUBLIC_ASSIGNMENT_CONCURRENCY = '4'
    env.DORKA_RELAY_PUBLIC_ASSIGNMENT_QUEUE_MAX = '256'
    env.DORKA_RELAY_PUBLIC_ASSIGNMENT_WAIT_MS = '8000'
    env.DORKA_RELAY_PUBLIC_ASSIGNMENT_RETRY_AFTER_SECONDS = '30'
    expect(loadRelayConfig(env)).toMatchObject({
  ...
  it('fails closed when public request lanes exceed the director database pool', () => {
    ...
    env.DORKA_RELAY_DATABASE_POOL_MAX = '2'
