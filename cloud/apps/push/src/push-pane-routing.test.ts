...
import { buildPushDelivery, dorkaDataStrings } from './push-delivery-message.js'
...
it('preserves pane identity for both APNs and FCM, and accepts older workspace-only messages', () => {
  ...
  for (const extra of [{}, { paneKey }]) {
    ...
    expect(delivery.dorka.paneKey).toBe('paneKey' in extra ? paneKey : undefined)
    expect(dorkaDataStrings(delivery.dorka).paneKey).toBe('paneKey' in extra ? paneKey : undefined)
  }
})
