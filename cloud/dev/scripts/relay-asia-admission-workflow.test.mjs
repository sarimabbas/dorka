...
test('binds selector operations and director configuration to reviewed implementations', () => {
  ...
  assert.match(workflow, /dorka-cloud-relay-regional-placement-enabled/)
...
test('requires immutable staged evidence and a timed production canary before expansion', () => {
  ...
  assert.match(workflow, /pnpm --filter @dorka-cloud\/relay-contract build/)
  assert.ok(
  ...
  assert.match(workflow, /dorka_relay_runtime_metrics/)
...
test('creates staging evidence only after the bounded launch-path load and rollback', () => {
  ...
  assert.match(stagingProof, /pnpm --filter @dorka-cloud\/relay-contract build/)
  ...
  assert.match(stagingProof, /--capacity-cell-origin https:\/\/c4\.relay-staging\.ondorka\.dev/)
...
test('keeps the production US-first switch in durable Secret Manager state', () => {
  ...
  assert.match(directorWorkflow, /dorka-cloud-relay-regional-placement-enabled/)
