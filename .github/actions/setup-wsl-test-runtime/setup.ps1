...
$distroDir = Join-Path $env:RUNNER_TEMP 'dorka-wsl-ubuntu'
...
wsl.exe --distribution Ubuntu --user root --exec /usr/bin/curl --fail --silent --show-error --location https://nodejs.org/dist/v22.14.0/node-v22.14.0-linux-x64.tar.xz --output /tmp/dorka-node.tar.xz
if ($LASTEXITCODE -ne 0) { throw 'Node download failed' }
$nodeHash = wsl.exe --distribution Ubuntu --user root --exec /usr/bin/sha256sum /tmp/dorka-node.tar.xz
...
wsl.exe --distribution Ubuntu --user root --exec /usr/bin/tar -xJf /tmp/dorka-node.tar.xz -C /usr/local --strip-components=1
