# Contributing to Dorka

Thanks for contributing to Dorka.

## Before You Start

- Keep changes scoped to a clear user-facing improvement, bug fix, or refactor.
- Dorka targets macOS, Linux, and Windows. Every change must stay compatible with all three platforms unless the code is explicitly guarded by a runtime platform check.
...
- Dorka must work against local repositories, remote servers, and SSH worktrees. Do not assume a process, file, credential, shell, or network path exists only on the local machine.
- Dorka supports many CLI agents, integrations, and git providers. Keep generic behavior provider-neutral; guard integration-specific logic behind explicit checks.
...
- **Include your X (Twitter) handle** in the PR template Author section — we shout out contributors when we merge features on [@dorka_build](https://x.com/dorka_build).
...
```bash
brew install --cask stablyai/orca/dorka@rc
```

The two casks conflict because both install `Dorka.app`. Switch channels with a
normal `brew uninstall --cask` followed by the install for the other channel.
Do not use `--zap` unless you intentionally want to remove local Dorka state.
