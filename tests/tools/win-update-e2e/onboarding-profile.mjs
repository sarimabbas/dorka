// Persistence seed for a fresh packaged Dorka profile: dismisses onboarding and
...
// dorka-data.json` BEFORE launch dismisses it. The app derives the Projects list
...
//
// IMPORTANT: seed only BEFORE the first launch. The app rewrites dorka-data.json
 ...
 * for it. A real checkout (init + one commit) is required — Dorka treats a
...
export function createSeededRepo(dir) {
  ...
  git('init', '-b', 'main')
  git('config', 'user.email', 'win-update-e2e@dorka.test')
...
export function seedFreshProfile(userDataDir, profile) {
  ...
  writeFileSync(path.join(userDataDir, 'dorka-data.json'), `${JSON.stringify(profile, null, 2)}\n`)
}
