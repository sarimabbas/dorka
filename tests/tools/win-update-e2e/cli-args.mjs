...
Installer source (version N, then N+1) — path or release tag on each side:
  --from <path>            Local dorka-windows-setup.exe for the base version (N)
  --to <path>              Local dorka-windows-setup.exe for the update (N+1)
...
Options:
  --install-dir <path>     Isolated-install mode: install the test build into
                           ...
                           leaving a developer's REAL Dorka install untouched.
  ...
  --allow-existing-install Proceed even if an Dorka install already exists. The
                           ...
                           developer's real Dorka. Clean machines (CI/VM) never
...
/** Default per-user oneClick install location: %LOCALAPPDATA%\Programs\Dorka. */
function defaultInstallDir() {
  const localAppData =
    process.env.LOCALAPPDATA ?? path.join(process.env.USERPROFILE ?? '', 'AppData', 'Local')
  return path.join(localAppData, 'Programs', 'Dorka')
}
  ...
  if (/\s/.test(installDir)) {
    errors.push(
      `--install-dir must not contain spaces (got "${installDir}"). The NSIS installer's ` +
        ...
        `be passed. Choose a spaces-free location (e.g. C:\\DorkaE2E).`
    )
  }
    ...
    if (entries.length > 0 && !looksLikeHarnessInstall(installDir)) {
      errors.push(
        `--install-dir "${installDir}" is a non-empty directory that does not look like a ` +
          `prior harness install (no Dorka.exe + "Uninstall Dorka.exe"). Refusing to overwrite ` +
