...
**Windows only.** Given two Dorka Windows installers (version N and N+1), this
...
- **The daemon is identified by command-line marker, never by exe name.** With
  `ELECTRON_RUN_AS_NODE` the daemon image is `Dorka.exe`; a relocated Phase 1
...
| Flag                                          | Meaning                                                     |
| --------------------------------------------- | ----------------------------------------------------------- |
| `--from <path>` / `--to <path>`               | Local `dorka-windows-setup.exe` for base (N) / update (N+1)  |
...
- **Pre-existing app process → hard refusal.** If an Dorka _app_ process (not a
...
- **Pre-existing install → refusal unless `--allow-existing-install`.** If an
  Dorka install already exists under `%LOCALAPPDATA%\Programs`, the run refuses,
...
(`%LOCALAPPDATA%\Programs\Dorka`). A developer's box already has a real Dorka there,
...
1. **Snapshots** the shared state (`registry-shortcut-backup.mjs`): `reg export`s
   each existing key to `.reg` files, copies the Dorka `*.lnk` shortcuts, and records
...
and the Dorka shortcuts — the state that steers a future update and the user-visible
...
pnpm win-update-e2e \
  ...
  --expect cold-restore --install-dir C:\DorkaE2E
```
...
1. **Preflight** — assert win32; warn if elevated; **refuse** to run if a
   pre-existing Dorka _app_ process (not a daemon) is running that the harness
...
2. **Install N** silently (`<setup.exe> /S`) and locate `Dorka.exe`.
...
| `registry-shortcut-backup.mjs` | Isolated mode: snapshot/restore the shared HKCU keys + Dorka shortcuts         |
...
- The harness assumes the packaged main honors `DORKA_E2E_USER_DATA_DIR` to
