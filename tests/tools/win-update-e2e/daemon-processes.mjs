// Identify and inspect the Dorka terminal daemon on Windows.
//
// The daemon is forked with ELECTRON_RUN_AS_NODE=1, so on Windows its process
// image is Dorka.exe (the Electron binary running as plain Node) — it CANNOT be
...
/** Default packaged userData root on Windows: %APPDATA%\Dorka. */
export function defaultUserDataDir() {
  const appData =
    process.env.APPDATA ?? path.join(process.env.USERPROFILE ?? '', 'AppData', 'Roaming')
  return path.join(appData, 'Dorka')
}
...
export function findDaemonProcesses(scope = '') {
  ...
  // ELECTRON_RUN_AS_NODE the daemon's image is Dorka.exe today but a relocated
