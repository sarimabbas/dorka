...
/**
 * Read-only discovery of the machine's current Dorka install registry state.
...
export function discoverInstallRegistryState() {
  const ps = [
    ...
    `  if ($dn -eq 'Dorka' -or ($il -and $il -match '\\\\Programs\\\\dorka')) {`,
    ...
    `  if ($il -and ($sn -eq 'Dorka' -or $il -match 'dorka')) {`,
...
/** The two directories NSIS writes Dorka shortcuts into: Start Menu + Desktop. */
...
/** Read-only: list Dorka *.lnk shortcuts under the given dirs (recursive). */
export function discoverDorkaShortcuts(dirs = shortcutDirs()) {
  const found = []
  for (const dir of dirs) {
    collectDorkaLnks(dir, found)
  }
  return found
}

function collectDorkaLnks(dir, out) {
  if (!existsSync(dir)) {
    return
  }
  let entries = []
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      collectDorkaLnks(full, out)
    } else if (entry.isFile() && /\.lnk$/i.test(entry.name) && /dorka/i.test(entry.name)) {
      out.push(full)
    }
  }
}
...
/**
 * Snapshot the shared install registry keys + Dorka shortcuts before an isolated
...
export function backupInstallState(runDir) {
  ...
  let i = 0
  for (const lnk of discoverDorkaShortcuts(dirs)) {
 ...
 * Pre-existing shortcuts are copied back; test-created Dorka shortcuts are removed.
...
function restoreShortcuts(manifest, result) {
  ...
  for (const lnk of discoverDorkaShortcuts(manifest.shortcutDirs)) {
...
function printMismatchWarning(manifest, expected, actual) {
  ...
  console.error('!! Your REAL Dorka install pointer may be hijacked to the test directory.')
  console.error('!! The next real Dorka update could install into the test location.')
...
if (process.argv[1] && path.resolve(process.argv[1]) === import.meta.filename) {
  ...
  console.log('[registry-shortcut-backup] Dorka shortcuts that would be backed up:')
  console.log(JSON.stringify(discoverDorkaShortcuts(), null, 2))
}
