...
//
// Dorka ships a per-user oneClick NSIS installer (electron-builder defaults:
// oneClick=true, perMachine=false) named dorka-windows-setup.exe. One-click
// silent mode is `<setup.exe> /S`; the app installs under
// %LOCALAPPDATA%\Programs\<dir> and the exe is Dorka.exe. The install dir casing
// is not guaranteed (observed lowercase "dorka" on a dev box), so the exe is
...
const PRODUCT_NAME = 'Dorka'
const EXE_NAME = 'Dorka.exe'
...
export function resolveInstaller({ localPath, releaseTag, assetPattern }) {
  ...
  const outDir = mkdtempSync(path.join(tmpdir(), 'dorka-e2e-installer-'))
...
export function silentInstall(setupExe, { timeoutMs = 180_000, installDir = null } = {}) {
  ...
  // On update runs the old Dorka.exe already exists, so wait for the exe whose
...
/**
 * Locate the installed Dorka.exe. In isolated mode (`installDir` set), the exe is
 * at a known fixed path (<installDir>\Dorka.exe). Otherwise it is discovered
 ...
 * developer's REAL Dorka. It additionally refuses to run against the default
  ...
  if (!allowDefaultLocation && pathsEqual(resolved, path.join(programsRoot(), PRODUCT_NAME))) {
    throw new Error(
      `Refusing to uninstall the default install location "${resolved}" — this is where a ` +
        `developer's REAL Dorka lives. Isolated mode must target a separate --install-dir.`
    )
  }
