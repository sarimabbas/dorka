...
//
// Given two Dorka Windows installers (version N and N+1), proves what happens to
...
async function main() {
  ...
  const canary = `DORKA-E2E-CANARY-${runId}`
  const runDir = mkdtempSync(path.join(tmpdir(), `dorka-win-update-e2e-${runId}-`))
...
function resolveScopedDaemon(userDataDir) {
  ...
  return {
    ...
    // install-dir Dorka.exe — the key survival signal.
...
async function teardown({
  ...
  if (hadPreexistingInstall) {
    console.log(
      '\n*** NOTE: an Dorka install existed before this run. It was OVERWRITTEN and the\n' +
