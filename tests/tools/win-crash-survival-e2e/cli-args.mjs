...
// installed, packaged Dorka.exe. So its only source is `--exe-path` (defaulting
...
Proves that force-killing ONLY Dorka's main process (a real crash, no tree-kill)
...
Usage:
  node tests/tools/win-crash-survival-e2e/run.mjs --expect <profile> [--exe-path <Dorka.exe>] [options]
...
Options:
  --exe-path <path>        Installed Dorka.exe to drive (default: the per-user
                           install under %LOCALAPPDATA%\\Programs\\Dorka). The
...
export function parseArgs(argv) {
  ...
  const opts = {
    ...
    // "no Dorka.exe found" default-resolution error.
    exePath:
...
function validate(opts, exePathFlagPresent, argv) {
  ...
  if (exePathFlagPresent && takeValue(argv, '--exe-path') === undefined) {
    ...
    errors.push(
      'No installed Dorka.exe found under %LOCALAPPDATA%\\Programs — pass --exe-path <Dorka.exe>'
    )
