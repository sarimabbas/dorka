...
//
// GitHub #7742: on Windows, when Dorka's main/renderer process crashed, open
...
const SENTINEL_ENV = 'DORKA_CRASH_SENTINEL'
...
async function main() {
  ...
  // clear platform message, not a confusing "no Dorka.exe found" default-resolution
  // failure.
  ...
  const canary = `DORKA-CRASH-SENTINEL-${runId}`
  const runDir = mkdtempSync(path.join(tmpdir(), `dorka-win-crash-e2e-${runId}-`))
  ...
  const diagDir = process.env.DORKA_E2E_DIAG_DIR || path.join(runDir, 'diag')
  let passed = false
  try {
 ...
 * daemons on the machine (including the developer's live Dorka) are ignored.
 ...
 * touches any other Dorka on the box (a live user instance uses a different
