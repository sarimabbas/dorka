// Launches the copied Dorka.exe as the daemon host (ELECTRON_RUN_AS_NODE=1) and
// waits for its {type:'ready'} IPC signal. Mirrors the real fork() options in
// src/main/daemon/daemon-init.ts (detached, ipc channel, DORKA_USER_DATA_PATH).
...
export function launchDaemonHost(options) {
  ...
  // if run against a build that carries the DORKA_NODE_PTY_NATIVE_DIR patch.
  if (nodePtyNativeDir) {
    env.DORKA_NODE_PTY_NATIVE_DIR = nodePtyNativeDir
  }
