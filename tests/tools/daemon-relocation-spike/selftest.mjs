...
function makeSyntheticInventory() {
  ...
  return {
    appDir: 'C:\\App',
    unpackedRoot,
    hostExe: { name: 'Dorka.exe', ...f('C:\\App\\Dorka.exe') },
    runtimeData: [
...
function run() {
  ...
  for (const tier of TIERS) {
    ...
    check(`${tier}: has exe`, dests.includes('Dorka.exe'))
  ...
  // ── Module-path filter ──────────────────────────────────────────────
  const appDir = 'C:\\Users\\me\\AppData\\Local\\Programs\\dorka'
  const modules = [
    'C:\\Users\\me\\AppData\\Local\\Programs\\dorka\\Dorka.exe',
    'C:\\Windows\\System32\\kernel32.dll',
    'C:\\Users\\me\\AppData\\Local\\dorka-daemon-host\\Dorka.exe'
  ]
  ...
  check(
    ...
    findAppDirResidentModules(['C:\\a\\dorka-daemon-host\\x.dll'], 'C:\\a\\dorka').length === 0
  )
  // Forward/back-slash + case normalization.
  check(
    'slash + case normalized',
    findAppDirResidentModules(['c:/a/DORKA/x.dll'], 'C:\\A\\dorka').length === 1
  )
  check(
    'relocated host has zero app-dir modules',
    findAppDirResidentModules(
      ['C:\\work\\daemon-host\\Dorka.exe', 'C:\\Windows\\System32\\ntdll.dll'],
      appDir
    ).length === 0
  )
