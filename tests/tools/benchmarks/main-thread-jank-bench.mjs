#!/usr/bin/env node
/**
 * Dorka main-thread jank / subprocess-churn benchmark (issue #7576).
 *
 ...
 * launched with DORKA_MAIN_THREAD_DIAGNOSTICS=1 so the main process emits one
 ...
 *     [--exe <path-to-packaged-Dorka>] [--headless] [--no-log-stream]
 *
 ...
 * the shape Dorka worktrees commonly have, which forces the effective-upstream
...
function ensureFixture(fixtureDir) {
  ...
  writeFileSync(join(fixtureDir, 'dorka-data.json'), JSON.stringify(state, null, 2))
  return repoPath
}
...
async function main() {
  ...
  const env = {
    ...
    DORKA_MAIN_THREAD_DIAGNOSTICS: '1',
