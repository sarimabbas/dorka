#!/usr/bin/env node
/**
 * A/B benchmark for worktree deletion against real Dorka dev instances.
 *
 * Usage:
...
function createFixture(instanceLabel) {
  const root = mkdtempSync(path.join(os.tmpdir(), `dorka-delete-bench-${instanceLabel}-`))
  ...
  run('git', ['config', 'user.email', 'worktree-delete-bench@dorka.invalid'], repoPath)
  run('git', ['config', 'user.name', 'Dorka Worktree Delete Bench'], repoPath)
  writeFileSync(path.join(repoPath, 'README.md'), '# Dorka worktree deletion benchmark\n')
...
function launchDevInstance({ label, repoRoot }, fixture, port) {
  ...
  const env = {
    ...process.env,
    DORKA_DEV_USER_DATA_PATH: fixture.userDataPath,
...
async function connectToDorka(instance) {
  ...
  while (Date.now() < deadline) {
    ...
    try {
      ...
      try {
        ...
        // findDorkaPage timeout drops a live browser handle and leaks a connection per retry.
        const page = await findDorkaPage(browser)
        return { browser, page }
      } catch (error) {
...
async function findDorkaPage(browser) {
  ...
  throw new Error('Dorka renderer with window.__store was not found')
}
...
async function verifyRestart(instanceConfig, fixture, port, repoId) {
  ...
  let browser = null
  try {
    const connection = await connectToDorka(relaunched)
...
async function benchmarkInstance(instanceConfig, index, options) {
  ...
  try {
    ...
    const { browser: connectedBrowser, page } = await connectToDorka(instance)
