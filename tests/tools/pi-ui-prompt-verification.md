...
1. Launch an isolated Dorka development instance with CDP using the Electron skill.
2. Open one terminal in a git worktree or folder workspace. Start Pi with Dorka's
   generated status extension and this additional extension:

   ```sh
   pi --offline --no-session -e /absolute/path/to/dorka/tests/tools/pi-ui-prompt-extension.mjs
   ```

   If launching Pi directly through `node` or disabling extension discovery, explicitly
   load Dorka's generated `dorka-agent-status.ts` with another `-e` argument.

3. Leave Pi at its input editor, then run from the Dorka repository:
...
For manual verification, run `/dorka-modal select`, `/dorka-modal confirm`,
`/dorka-modal input`, `/dorka-modal editor`, or `/dorka-modal custom` inside Pi.
