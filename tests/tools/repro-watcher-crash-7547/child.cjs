...
// watches our exit code). Each scenario mimics a real Dorka usage pattern:
//
      ...
      sub = await watcher.subscribe(
        ...
        (err, events) => {
          if (err) {
            recordError(err)
            errored = true
            // Dorka's createWatcher error path: unsubscribe from inside the
            // error callback.
            if (sub) {
              stats.unsubscribes++
...
async function main() {
  ...
  // Outside the repo worktree: a live Dorka instance may be watching the repo,
  // and this churn must not feed the production watcher.
  const baseDir = path.join(os.tmpdir(), 'dorka-7547-harness', `run-${process.pid}`)
