...
Run `DORKA_BACKGROUND_LAUNCH=1 node tests/tools/omp-child-history-rendered/run.mjs`.
...
Run `DORKA_BACKGROUND_LAUNCH=1 bun tests/tools/omp-child-session-resume-smoke.mjs /path/to/oh-my-pi`
for a zero-model-call check against real OMP session storage and CLI parsing. The smoke
builds Dorka's path-based resume command, creates parent, child and grandchild transcripts,
