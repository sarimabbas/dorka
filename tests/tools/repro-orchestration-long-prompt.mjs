...
async function callDorka(cli, args, options = {}) {
...
function buildLongSpec(sizeKb, marker) {
  ...
  const header = [
    `DORKA_LONG_PROMPT_REPRO_START ${marker}`,
  ...
  let index = 0
  while (
    Buffer.byteLength(`${lines.join('\n')}\nDORKA_LONG_PROMPT_REPRO_END ${marker}`, 'utf8') <
    targetBytes
  ) {
    lines.push(
  ...
  lines.push(`DORKA_LONG_PROMPT_REPRO_END ${marker}`)
  return lines.join('\n')
}
...
async function parentMain() {
  ...
  await callDorka(cli, ['status'], { cwd })

  const coordinator = await callDorka(
    cli,
    [
      'terminal',
      'create',
  ...
  try {
    ...
    const dispatch = await callDorka(
      cli,
      [
        'orchestration',
        'dispatch',
...
async function fakeWorkerMain() {
  ...
  const finish = async (reason) => {
    ...
    process.stdout.write(
      `\nDORKA_REPRO_REPORT ${report.contractOk ? 'ok' : 'failed'} ${reportPath}\n`
    )
...
The parent mode requires a running Dorka runtime and creates temporary Dorka
terminals. The fake worker records whether orchestration dispatch delivered a
long prompt in a safe agent-input contract.

Options:
  --cli <path>         Dorka CLI command (default: DORKA_REPRO_CLI or dorka)
