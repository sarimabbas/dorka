...
const DEFAULT_EXECUTABLE = '/Applications/Dorka.app/Contents/MacOS/Dorka'
const DEFAULT_PROFILE = path.join(os.homedir(), 'Library', 'Application Support', 'dorka')
const URL = 'https://example.com/dorka-terminal-garble-repro'
...
const runRoot = mkdtempSync(path.join(os.tmpdir(), 'dorka-terminal-garble-'))
const userDataDir = path.join(runRoot, 'user-data')
const evidenceDir = path.join(runRoot, 'evidence')
mkdirSync(userDataDir, { recursive: true })
mkdirSync(evidenceDir, { recursive: true })
function sanitizeProfile() {
  const sourceDataPath = path.join(sourceProfile, 'dorka-data.json')
  ...
  writeFileSync(path.join(userDataDir, 'dorka-data.json'), serialized)
...
try {
  ...
  app = await electron.launch({
    ...
    env: {
      ...
      ...(mainPath ? { NODE_ENV: 'development', DORKA_E2E_HEADFUL: '1' } : {})
    }
  })
