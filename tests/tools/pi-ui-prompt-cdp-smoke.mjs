// Run against an isolated Dorka dev instance with Pi and pi-ui-prompt-extension.mjs loaded.
...
try {
  ...
  assert.ok(page, 'Dorka renderer must be open')
  ...
  for (const kind of ['select', 'confirm', 'input', 'editor', 'custom']) {
    for (const ending of kind === 'select' ? ['answer', 'cancel'] : ['cancel']) {
      await input.pressSequentially(`/dorka-modal ${kind}`, { delay: 10 })
