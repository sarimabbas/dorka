...
1. **Desktop Dorka running** (`dorka status --json`).
...
dorka worktree list --environment <name> --json | head
dorka terminal list --environment <name> --json | head
```
...
DORKA_FREEZE_ENV=paired-remote \
DORKA_FREEZE_SCENARIO=idle-backlog-open \
DORKA_FREEZE_CREATE=8 \
DORKA_FREEZE_IDLE_MS=45000 \
DORKA_FREEZE_OPEN_COUNT=24 \
pnpm run repro:live-remote-realistic-freeze

# Wake-like: idle + reconnect metadata storm + open  ← hard freeze in lab
DORKA_FREEZE_ENV=paired-remote \
DORKA_FREEZE_SCENARIO=idle-backlog-reconnect-open \
DORKA_FREEZE_CREATE=10 \
DORKA_FREEZE_IDLE_MS=60000 \
DORKA_FREEZE_OPEN_COUNT=40 \
pnpm run repro:live-remote-realistic-freeze

# Restart-proxy: idle + dorka open + refresh storm + open (does not kill desktop)
DORKA_FREEZE_ENV=paired-remote \
DORKA_FREEZE_SCENARIO=restart-proxy \
DORKA_FREEZE_CREATE=0 \
DORKA_FREEZE_IDLE_MS=20000 \
DORKA_FREEZE_OPEN_COUNT=30 \
pnpm run repro:live-remote-realistic-freeze
```
...
| `restart-proxy`               | `dorka open` + refresh storm + open (post-restart discovery; no process kill)                  |
| `lockup-storm`                | Idle + flood + reconnect + **concurrent** open fan-out + **mid-storm `dorka status` watchdog** |

### Realistic knobs

| Variable                          | Default             | Meaning                                                           |
| --------------------------------- | ------------------- | ----------------------------------------------------------------- |
| `DORKA_FREEZE_ENV`                 | `paired-remote`     | Paired remote environment name                                    |
| `DORKA_FREEZE_SCENARIO`            | `idle-backlog-open` | See table above                                                   |
| `DORKA_FREEZE_CREATE`              | `0`                 | New flood terminals; mutation requires an explicit positive value |
| `DORKA_FREEZE_IDLE_MS`             | `45000`             | Time “away” while floods run                                      |
| `DORKA_FREEZE_OPEN_COUNT`          | `20`                | Sessions to open after return                                     |
| `DORKA_FREEZE_PACE_MS`             | `250`               | Base delay between opens (human pace)                             |
| `DORKA_FREEZE_PACE_JITTER_MS`      | `150`               | Random extra delay                                                |
| `DORKA_FREEZE_SOFT_MS` / `HARD_MS` | 2000 / 5000         | Thresholds                                                        |
...
| **lockup-storm** (parallel open + overlap refresh) | 12–16  | 45–60s | 64–80 @ p20–32 | **27–35s** batches; some `Terminal reveal timed out` | **HARD stalls + reveal timeouts; app still answers `dorka status` ~150ms** |
...
| Post-storm `dorka status`    | **~113ms**                          |
| Force Quit required         | **no**                              |

Bar for full-app freeze in the harness: continuous **≥30s** window where `dorka status` hangs/fails or stays ≥15s slow (`evaluateFullAppFreeze` / `foreverUiLockupObserved`).
...
DORKA_FREEZE_ENV=paired-remote DORKA_FREEZE_SCENARIO=lockup-storm \
  DORKA_FREEZE_CREATE=12 DORKA_FREEZE_IDLE_MS=30000 DORKA_FREEZE_OPEN_COUNT=80 \
  DORKA_FREEZE_STORM_PARALLEL=28 DORKA_FREEZE_FOREVER_WINDOW_MS=30000 \
  pnpm run repro:live-remote-realistic-freeze
...
DORKA_FREEZE_ENV=paired-remote \
DORKA_FREEZE_CREATE=0 \
DORKA_FREEZE_SWITCH_PASSES=3 \
DORKA_FREEZE_PARALLEL=16 \
...
| Layer    | Module                                                                             |
| -------- | ---------------------------------------------------------------------------------- |
| Runtime  | `TerminalFocusNavigationCoalescer` via `DorkaRuntimeService.focusTerminal`          |
...
- Both harnesses default to `DORKA_FREEZE_CREATE=0`. A positive value creates persistent, high-output remote terminals; use it only on an isolated target you can clean up.
- `restart-proxy` does **not** kill Dorka; it runs `dorka open` + refresh RPCs only.
- Manual capture if UI fully freezes: `sample Dorka 5 -file ~/Desktop/dorka-freeze-sample.txt`

The scripts honor `DORKA_CLI_COMMAND`, then use `dorka-dev` in a dev runtime, `dorka-ide` on Linux, and `dorka` elsewhere.

PowerShell equivalent for the first example:

```powershell
$env:DORKA_FREEZE_ENV = 'paired-remote'
$env:DORKA_FREEZE_SCENARIO = 'idle-backlog-open'
$env:DORKA_FREEZE_CREATE = '8'
$env:DORKA_FREEZE_IDLE_MS = '45000'
$env:DORKA_FREEZE_OPEN_COUNT = '24'
pnpm run repro:live-remote-realistic-freeze
```
