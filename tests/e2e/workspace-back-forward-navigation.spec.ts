...
test.describe('Workspace Back/Forward Navigation', () => {
  ...
  test('buttons are hidden outside the terminal view', async ({ dorkaPage }) => {
    await expect(await getBackButton(dorkaPage)).toBeVisible()
    await expect(await getForwardButton(dorkaPage)).toBeVisible()
    ...
    await expect(await getBackButton(dorkaPage)).toHaveCount(0)
    await expect(await getForwardButton(dorkaPage)).toHaveCount(0)
  ...
  test('both buttons disabled at cold start with a single history entry', async ({ dorkaPage }) => {
    ...
    const activeId = await getActiveWorktreeId(dorkaPage)
    expect(activeId).not.toBeNull()

    await resetNavHistory(dorkaPage)
    await seedVisit(dorkaPage, activeId!)

    const back = await getBackButton(dorkaPage)
    const forward = await getForwardButton(dorkaPage)
    await expect(back).toBeDisabled()
    await expect(forward).toBeDisabled()
  })

  test('clicking Back and Forward walks the history stack', async ({ dorkaPage }) => {
    ...
    await resetNavHistory(dorkaPage)
    await seedVisit(dorkaPage, primaryId)
    await seedVisit(dorkaPage, secondaryId)
    ...
    const primaryRow = worktreeRow(dorkaPage, primaryId)
    const secondaryRow = worktreeRow(dorkaPage, secondaryId)

    await back.click()
    await expect
      .poll(async () => getActiveWorktreeId(dorkaPage), {
  ...
  test('re-activating the current worktree is a no-op (dedupe)', async ({ dorkaPage }) => {
    const activeId = await getActiveWorktreeId(dorkaPage)
    expect(activeId).not.toBeNull()
    ...
    const snapshot = await getNavHistorySnapshot(dorkaPage)
    expect(snapshot.history).toEqual([activeId])
    expect(snapshot.index).toBe(0)
    await expect(await getBackButton(dorkaPage)).toBeDisabled()
  })

  test('new navigation after going back truncates the forward stack', async ({ dorkaPage }) => {
    const worktreeIds = await getAllWorktreeIds(dorkaPage)
    ...
    await seedVisit(dorkaPage, primaryId)
    await seedVisit(dorkaPage, secondaryId)
    await (await getBackButton(dorkaPage)).click()
    await expect.poll(() => getActiveWorktreeId(dorkaPage)).toBe(primaryId)

    // Forward button is live — a forward entry exists.
    await expect(await getForwardButton(dorkaPage)).toBeEnabled()
    ...
    await expect(await getForwardButton(dorkaPage)).toBeDisabled()
  })

  test(`${isMac ? 'Cmd' : 'Ctrl'}+Alt+Left/Right shortcuts walk history`, async ({ dorkaPage }) => {
    const worktreeIds = await getAllWorktreeIds(dorkaPage)
    ...
    await dorkaPage.keyboard.press(`${mod}+Alt+ArrowLeft`)
    await expect
      .poll(async () => getActiveWorktreeId(dorkaPage), {
    ...
    await dorkaPage.keyboard.press(`${mod}+Alt+ArrowRight`)
    await expect
      .poll(async () => getActiveWorktreeId(dorkaPage), {
  ...
  test('shortcut is a no-op in settings view', async ({ dorkaPage }) => {
    const worktreeIds = await getAllWorktreeIds(dorkaPage)
    ...
    await expect
      .poll(async () => dorkaPage.evaluate(() => window.__store!.getState().activeView))
      .toBe('settings')

    const idBefore = await getActiveWorktreeId(dorkaPage)
    await dorkaPage.evaluate(() => document.body.focus())
    await dorkaPage.keyboard.press(`${mod}+Alt+ArrowLeft`)

    // Give any erroneous nav a beat to land, then assert the active worktree
    // and the slice index both stayed put.
    await dorkaPage.waitForTimeout(150)
    expect(await getActiveWorktreeId(dorkaPage)).toBe(idBefore)
    const snapshot = await getNavHistorySnapshot(dorkaPage)
    expect(snapshot.index).toBe(1)
  })
})
