...
test.describe('SSH config host picker', () => {
  ...
  test.afterEach(async ({ dorkaPage }) => {
    await closeOpenDialogs(dorkaPage).catch(() => undefined)
    await removeSshTargetsByPrefix(dorkaPage, HOST_PREFIX).catch(() => undefined)
  })

  // ── P1 ─────────────────────────────────────────────────────────────
  test('P1: empty config shows empty state; Back returns to blank form', async ({ dorkaPage }) => {
    ...
    await expect(picker.getByRole('button', { name: 'Add all to Dorka' })).toBeDisabled()

    await picker.getByRole('button', { name: 'Back' }).click()
    const form = dorkaPage.getByRole('dialog', { name: 'Add SSH host' })
  ...
  test('P3: select host prefills form; Save persists; N3 identity hint', async ({
    ...
    await expect(
      dorkaPage.getByText(new RegExp(`Filled from ${escapeRegExp(prod.alias)}`, 'i'))
  ...
  // ── P4 ─────────────────────────────────────────────────────────────
  test('P4: filter narrows host list', async ({ electronApp, dorkaPage }) => {
    const hosts = await seedPairConfig(electronApp, HOST_PREFIX)
    const picker = await openSshConfigHostPicker(dorkaPage)
  ...
  // ── P8 ─────────────────────────────────────────────────────────────
  test('P8: Back without select leaves form empty', async ({ electronApp, dorkaPage }) => {
    const hosts = await seedPairConfig(electronApp, HOST_PREFIX)
    const picker = await openSshConfigHostPicker(dorkaPage)
    await expect(configHostRow(picker, hosts.alpha)).toBeVisible()
    ...
    await expect(fields.username).toHaveValue('')
    await expect(fields.label).toHaveValue('')
    await expect(dorkaPage.getByText(/Filled from /i)).toHaveCount(0)
  })
})
