...
const scratch = mkdtempSync(path.join(os.tmpdir(), 'dorka-paired-truncated-tail-'))
...
test.use({
  dorkaAppExtraEnv: {
    DORKA_E2E_TERMINAL_PARKING_DELAY_MS: String(RETENTION_PARK_DELAY_MS),
...
test('legacy paired hosts retain the lossy hidden-manager budget fallback @headful', async ({
  ...
  test.setTimeout(120_000)
  const worktreeIds = await dorkaPage.evaluate(() =>
    window.__store
      ?.getState()
      .allWorktrees()
      .slice(0, 2)
