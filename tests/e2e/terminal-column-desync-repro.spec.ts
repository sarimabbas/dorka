...
test.describe('Terminal column desync repro', () => {
  test('PTY columns stay in sync with xterm across a visible resize', async ({ dorkaPage }) => {
    test.setTimeout(120_000)
    await waitForSessionReady(dorkaPage)
    await waitForActiveWorktree(dorkaPage)
    await closeRightSidebarAndFeatureTips(dorkaPage)
    await ensureTerminalVisible(dorkaPage)
    const ptyId = await settleTerminal(dorkaPage)
    ...
    const expectColumnsInSync = async (label: string): Promise<void> => {
      await expect
        .poll(
          async () => {
            const snap = await readColumnSnapshot(dorkaPage, ptyId)
    ...
    await dorkaPage.setViewportSize({ width: 760, height: 800 })
    await expectColumnsInSync('after shrink')

    await dorkaPage.setViewportSize({ width: 1280, height: 800 })
  ...
  test('pty:getSize reports the size the PTY actually applied', async ({ dorkaPage }) => {
    test.setTimeout(120_000)
    await waitForSessionReady(dorkaPage)
    await waitForActiveWorktree(dorkaPage)
    await closeRightSidebarAndFeatureTips(dorkaPage)
    ...
    await dorkaPage.setViewportSize({ width: 900, height: 800 })
    ...
    await expect
      .poll(
        async () => {
          const ptyCols = await readPtyCols(dorkaPage, ptyId)
          const reportedCols = await readReportedPtyCols(dorkaPage, ptyId)
  ...
  test('PTY columns re-sync after the terminal is resized while hidden', async ({ dorkaPage }) => {
    test.setTimeout(120_000)
    await waitForSessionReady(dorkaPage)
    const homeWorktreeId = await waitForActiveWorktree(dorkaPage)
    const otherWorktreeId = (await getAllWorktreeIds(dorkaPage)).find((id) => id !== homeWorktreeId)
    ...
    const ptyId = await settleTerminal(dorkaPage)
    await dorkaPage.setViewportSize({ width: 1280, height: 800 })
    await dorkaPage.waitForTimeout(400)

    const baseline = await readColumnSnapshot(dorkaPage, ptyId)
    ...
    await dorkaPage.setViewportSize({ width: 720, height: 800 })
    ...
    const afterReturn = await readColumnSnapshot(dorkaPage, ptyId)
    expect(
      afterReturn.ptyCols,
  ...
  test('PTY columns re-sync after repeated background resizes', async ({ dorkaPage }) => {
    ...
    for (const [index, width] of widths.entries()) {
      await switchToWorktree(dorkaPage, otherWorktreeId)
      await waitForActiveTerminalManager(dorkaPage, 30_000)
      await dorkaPage.setViewportSize({ width, height: 800 })
      await dorkaPage.waitForTimeout(350)
      await switchToWorktree(dorkaPage, homeWorktreeId)
      await ensureTerminalVisible(dorkaPage)
      await waitForActiveTerminalManager(dorkaPage, 30_000)
      await dorkaPage.waitForTimeout(500)

      const snapshot = await readColumnSnapshot(dorkaPage, ptyId)
      expect(
        snapshot.ptyCols,
  ...
  test('both panes keep PTY columns synced after a vertical split reparent', async ({
    ...
    const firstPtyId = await settleTerminal(dorkaPage)

    const baseline = await readColumnSnapshot(dorkaPage, firstPtyId)
    expect(baseline.ptyCols).toBe(baseline.xtermCols)
    ...
    for (const pane of snapshot.panes) {
      ...
      const ptyCols = await readPtyCols(dorkaPage, ptyId)
      const xtermCols = await readRenderedColsForPty(dorkaPage, ptyId)
      expect(
        ptyCols,
  ...
  test('both panes stay PTY-synced when a tab MOUNTS with a split layout present', async ({
    ...
    await closeRightSidebarAndFeatureTips(dorkaPage)
    await ensureTerminalVisible(dorkaPage)
    await dorkaPage.setViewportSize({ width: 1440, height: 900 })
    await dorkaPage.waitForTimeout(300)
    await settleTerminal(dorkaPage)
    ...
    for (let attempt = 0; attempt < MOUNT_ATTEMPTS; attempt += 1) {
      ...
      await dorkaPage.setViewportSize({ width: 1180, height: 800 })
      await dorkaPage.waitForTimeout(300)
