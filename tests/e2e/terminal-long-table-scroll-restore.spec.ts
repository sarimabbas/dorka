...
test.describe('Terminal long table scroll restore repro', () => {
  test('reproduces long markdown table artifacts after workspace switch and scroll', async ({
    ...
    await dorkaPage.evaluate(() => {
      window.__store
        ?.getState()
        .markFeatureTipsSeen(['dorka-cli', 'cmd-j-palette', 'voice-dictation'])
    })
    ...
    const scriptPath = path.join(testRepoPath, `.dorka-long-table-${runId}.mjs`)
    writeFileSync(scriptPath, longMarkdownTableScript(runId))

    try {
      await sendToTerminal(dorkaPage, ptyId, `${nodeTerminalCommand([scriptPath])}\r`)
      ...
      await scrollActiveTerminalLikeUser(dorkaPage)
      await closeFeatureTips(dorkaPage)
      const diagnostics = await readTerminalRenderDiagnostics(dorkaPage)
  ...
  test('keeps narrow wrapped signer markdown table coherent after restore and scroll', async ({
    ...
    await setRenderedTableViewport(dorkaPage)
    await forceDarkTerminalRendererPath(dorkaPage)
    await ensureTerminalVisible(dorkaPage)
    await waitForActiveTerminalManager(dorkaPage, 30_000)
    const ptyId = await waitForActivePanePtyId(dorkaPage)
    await waitForPtyShellEcho(dorkaPage, ptyId, 15_000)
    ...
    const scriptPath = path.join(testRepoPath, `.dorka-narrow-signer-table-${runId}.mjs`)
    ...
    try {
      ...
      const content = await getTerminalContent(dorkaPage, 30_000)
  ...
  test('keeps real emoji markdown table right edge clean after restore and scroll', async ({
    ...
    await ensureTerminalVisible(dorkaPage)
    await waitForActiveTerminalManager(dorkaPage, 30_000)
    await setNarrowTerminalViewport(dorkaPage)
    const renderedTableTerminalCols = await waitForRenderedTerminalColumnsAtMost(
      dorkaPage,
      NARROW_TERMINAL_MAX_COLS
    )
    const ptyId = await waitForActivePanePtyId(dorkaPage)
    await waitForPtyColumnsAtMost(dorkaPage, ptyId, renderedTableTerminalCols)
    const runId = randomUUID()
    const marker = `EMOJI_FIXTURE_TABLE_RESTORE_${runId}`
    const scriptPath = path.join(testRepoPath, `.dorka-emoji-fixture-table-${runId}.mjs`)
    ...
    try {
      ...
      await waitForActiveTerminalManager(dorkaPage, 30_000)
      await setNarrowTerminalViewport(dorkaPage)
      await waitForRenderedTerminalColumnsAtMost(dorkaPage, NARROW_TERMINAL_MAX_COLS)
      await expect
        .poll(() => getTerminalContent(dorkaPage, 30_000), {
          timeout: 10_000,
        ...
        .toContain(marker)
      const generatedWidthContent = await getTerminalContent(dorkaPage, 30_000)
      ...
      await scrollActiveTerminalToText(dorkaPage, retainedEmojiCell)
      await closeFeatureTips(dorkaPage)
      await expect
        .poll(() => readActiveTerminalVisibleText(dorkaPage), {
          timeout: 5_000,
          message: `${retainedEmojiCell} row fragment should be visible before screenshot`
        })
        .toContain(retainedEmojiCell)
      const diagnostics = await readTerminalRenderDiagnostics(dorkaPage)
      const overpaint = await readTerminalRightEdgeOverpaint(dorkaPage)
      const wrapDiagnostics = await readTerminalBoxTableWrapDiagnostics(dorkaPage)
