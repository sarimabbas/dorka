...
test.describe('Terminal output scheduler', () => {
  test('background tab output bursts use the shared drain while the active tab renders', async ({
    ...
    const ptyIdsByTabId: Record<string, string> = {
      [firstTabId]: await waitForTabPtyId(dorkaPage, firstTabId)
    }

    while (tabIds.length < TAB_COUNT) {
      const tabId = await createTerminalTab(dorkaPage)
      await waitForActiveTerminalManager(dorkaPage, 30_000)
      tabIds.push(tabId)
      ptyIdsByTabId[tabId] = await waitForTabPtyId(dorkaPage, tabId)
    }

    await tabLocator(dorkaPage, firstTabId).click()
    await expect
    ...
    await resetSchedulerDebug(dorkaPage)
    ...
    await sendPtyCommands(dorkaPage, [
      {
    ...
    await expect
      .poll(async () => (await getTerminalContent(dorkaPage)).includes(foregroundMarker), {
        timeout: 5_000,
    ...
    await tabLocator(dorkaPage, firstBackgroundTabId).click()
    await expect
      .poll(() => getDomActiveTabId(dorkaPage), {
        timeout: 5_000,
    ...
    await expect
      .poll(async () => (await getTerminalContent(dorkaPage)).includes(firstBackground.marker), {
        timeout: 5_000,
  ...
  test('visible bulk output uses the high-priority drain instead of synchronous xterm writes', async ({
    ...
    const activeTabId = await createTerminalTab(dorkaPage)
    if (!activeTabId) {
      throw new Error('Expected a fresh terminal tab')
    }
    const ptyId = await waitForTabPtyId(dorkaPage, activeTabId)
    await resetSchedulerDebug(dorkaPage)
    ...
    await sendPtyCommands(dorkaPage, [{ ptyId, command: floodCommand }])

    await expect
      .poll(async () => (await getTerminalContent(dorkaPage, 12_000)).includes(marker), {
        timeout: 30_000,
  ...
  test('hidden overflow restores from main-owned terminal state when the tab becomes visible', async ({
    ...
    const foregroundTabId = await getActiveTabId(dorkaPage)
    if (!foregroundTabId) {
      throw new Error('Expected an initial terminal tab')
    }
    const hiddenTabId = await createTerminalTab(dorkaPage)
    await waitForActiveTerminalManager(dorkaPage, 30_000)
    const hiddenPtyId = await waitForTabPtyId(dorkaPage, hiddenTabId)

    await tabLocator(dorkaPage, foregroundTabId).click()
    await expect
      .poll(() => getDomActiveTabId(dorkaPage), {
        timeout: 5_000,
    ...
    await sendPtyCommands(dorkaPage, [{ ptyId: hiddenPtyId, command: floodCommand }])

    await expect
      .poll(async () => mainSnapshotContains(dorkaPage, hiddenPtyId, marker), {
        timeout: 30_000,
        message: 'Main-owned terminal snapshot did not capture the hidden flood marker'
      })
      .toBe(true)

    await tabLocator(dorkaPage, hiddenTabId).click()
    await expect
      .poll(() => getDomActiveTabId(dorkaPage), {
        timeout: 5_000,
