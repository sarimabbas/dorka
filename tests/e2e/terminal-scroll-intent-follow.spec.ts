...
test.describe('terminal scroll intent keeps following output', () => {
  test('a sub-row wheel-up that never moves the viewport must not stop follow-output', async ({
    ...
    const ptyId = await startStreamingFixturePhase1(dorkaPage)
    ...
    await dispatchSubRowWheelUp(dorkaPage)
    await dorkaPage.waitForTimeout(INTENT_SETTLE_WAIT_MS)

    // Any byte releases the fixture's phase-2 stream.
    await sendToTerminal(dorkaPage, ptyId, 'g')
    await waitForMarkerAtBottom(dorkaPage, 'STREAM_PHASE2_DONE')
  })

  test('a plain Home keypress delivered to the app must not stop follow-output', async ({
    dorkaPage
  }) => {
    await startStreamingFixturePhase1(dorkaPage)
    ...
    await dispatchPlainHomeKeydown(dorkaPage)
    await waitForMarkerAtBottom(dorkaPage, 'STREAM_PHASE2_DONE')
  })

  test('a real wheel pin stays fixed while visible output streams', async ({ dorkaPage }) => {
    const ptyId = await startStreamingFixturePhase1(dorkaPage)

    await dispatchRealWheel(dorkaPage, -240)
    await expect
      .poll(async () => {
        const probe = await probeActiveViewport(dorkaPage, 'STREAM_PHASE1_DONE')
        return probe ? probe.baseY - probe.viewportY : 0
      })
      .toBeGreaterThan(1)
    const pinned = await probeActiveViewport(dorkaPage, 'STREAM_PHASE1_DONE')
    if (!pinned) {
    ...
    await expect
      .poll(
        async () => {
          const probe = await probeActiveViewport(dorkaPage, 'STREAM_PHASE2_DONE')
  ...
  test('typing after a pinned write is queued resumes follow-output', async ({ dorkaPage }) => {
    await startStreamingFixturePhase1(dorkaPage)
    const { paneKey } = await waitForActivePaneHookDescriptor(dorkaPage)
    await waitForTerminalPtyDataInjector(dorkaPage, paneKey)

    await dispatchRealWheel(dorkaPage, -320)
    await expect
      .poll(async () => {
        const probe = await probeActiveViewport(dorkaPage, 'STREAM_PHASE1_DONE')
        return probe ? probe.baseY - probe.viewportY : 0
    ...
    await injectQueuedWriteThenType(dorkaPage, paneKey)
    await expect
      .poll(
        async () => {
          const probe = await probeActiveViewport(dorkaPage, 'STREAM_PHASE1_DONE')
