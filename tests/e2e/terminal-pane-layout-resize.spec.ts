...
test.describe('Terminal Panes', () => {
  ...
  test('Always-on pane header split button hover stays transparent', async ({ dorkaPage }) => {
    const splitButton = dorkaPage.getByRole('button', { name: 'Split Terminal Right' })
   ...
   * - resizing terminal panes works
   */
  test('shows a pane divider after splitting', async ({ dorkaPage }) => {
    ...
    await expect(dorkaPage.locator('.pane-divider.is-vertical').first()).toBeVisible({
      timeout: 3_000
    })
  })
   ...
   *   DORKA_E2E_HEADFUL=1 pnpm run test:e2e
   */
  test('@headful can resize terminal panes by real mouse drag', async ({ dorkaPage }) => {
    ...
    const paneWidthsBefore = await dorkaPage.evaluate(() => {
    ...
    await dorkaPage.mouse.move(startX + 150, startY, { steps: 20 })
    ...
    await expect
      .poll(
        async () => {
          const widthsAfter = await dorkaPage.evaluate(() => {
            const xterms = document.querySelectorAll('.xterm')
            return Array.from(xterms)
              .filter((x) => (x as HTMLElement).offsetParent !== null)
              .map((x) => (x as HTMLElement).getBoundingClientRect().width)
  ...
  test('@headful resizing split panes forwards only the settled PTY size', async ({
    ...
    await expect
      .poll(
        async () =>
          (await readVisiblePaneContents(dorkaPage)).every((content) =>
    ...
    await dorkaPage.mouse.move(startX, startY)
    await dorkaPage.mouse.down()
    await dorkaPage.mouse.move(startX - 350, startY, { steps: 40 })
    await dorkaPage.mouse.move(startX + 250, startY, { steps: 40 })
    await dorkaPage.mouse.up()
    await dorkaPage.waitForTimeout(500)

    const paneContents = await readVisiblePaneContents(dorkaPage)
   ...
   * - closing panes works
   */
  test('closing a split pane removes it and remaining pane fills space', async ({ dorkaPage }) => {
    const panesBefore = await countVisibleTerminalPanes(dorkaPage)
    ...
    const panesAfterSplit = await countVisibleTerminalPanes(dorkaPage)
    expect(panesAfterSplit).toBeGreaterThanOrEqual(2)

    await closeActiveTerminalPane(dorkaPage)
    await waitForPaneCount(dorkaPage, panesAfterSplit - 1)

    // The remaining pane should fill the available space
    const paneWidth = await dorkaPage.evaluate(() => {
      const xterms = document.querySelectorAll('.xterm')
