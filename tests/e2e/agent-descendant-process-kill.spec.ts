...
test('killing an agent PTY terminates its detached-pgid descendants', async ({ dorkaPage }) => {
  test.skip(process.platform === 'win32', 'descendant tree-kill is POSIX-only for now')

  const stage = mkdtempSync(join(tmpdir(), 'dorka-agent-descendant-'))
  ...
  try {
    ...
    const ptyId = await dorkaPage.evaluate(
    ...
    await dorkaPage.evaluate((id) => window.api.pty.kill(id), ptyId)
