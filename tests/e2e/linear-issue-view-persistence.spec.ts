...
const LINEAR_ISSUE_VIEW_STORAGE_KEY = 'dorka.linear.issue-view.v1'
...
test.describe('Linear issue view persistence', () => {
  test('preserves view mode, grouping, ordering, and filters across a tasks remount', async ({
    ...
    await openLinearTasks(dorkaPage)
    await waitForLinearIssuesChrome(dorkaPage, ISSUE_A.title)

    await setLinearViewPreferences(dorkaPage, {
      viewMode: 'Board',
      groupBy: 'Status',
      orderBy: 'Updated'
    })
    await applyStatusFilter(dorkaPage, STATE_A.name)

    await waitForLinearIssueViewPersisted(dorkaPage, (view) => {
      if (
        !view ||
        view.viewMode !== 'board' ||
        view.groupBy !== 'status' ||
    ...
    await expectRestoredLinearView(dorkaPage)
    const statusChip = dorkaPage.getByRole('button', { name: 'Remove Status filter' }).locator('..')
    await expect(statusChip).toContainText(STATE_A.name)

    await closeTasksPage(dorkaPage)
    await openLinearTasks(dorkaPage)
    await waitForLinearIssuesChrome(dorkaPage, ISSUE_A.title)

    await expectRestoredLinearView(dorkaPage)
    await expect(
      dorkaPage.getByRole('button', { name: 'Remove Status filter' }).locator('..')
  ...
  test('keeps attribute filters scoped per Linear workspace', async ({ electronApp, dorkaPage }) => {
    ...
    await applyPriorityFilter(dorkaPage, 'High')
    await expect(
      dorkaPage.getByRole('button', { name: 'Remove Priority filter' }).locator('..')
    ).toContainText('High')
    ...
    await switchLinearWorkspace(dorkaPage, WORKSPACE_B.organizationName)
    await waitForLinearIssuesChrome(dorkaPage, ISSUE_B.title)
    // Workspace B starts unfiltered — Alpha's High must not leak.
    await expect(dorkaPage.getByRole('button', { name: 'Remove Priority filter' })).toHaveCount(0)

    await applyPriorityFilter(dorkaPage, 'Low')
    await expect(
      dorkaPage.getByRole('button', { name: 'Remove Priority filter' }).locator('..')
    ).toContainText('Low')
    ...
    await switchLinearWorkspace(dorkaPage, WORKSPACE_A.organizationName)
    await waitForLinearIssuesChrome(dorkaPage, ISSUE_A.title)
    await expect(
      dorkaPage.getByRole('button', { name: 'Remove Priority filter' }).locator('..')
    ).toContainText('High')
