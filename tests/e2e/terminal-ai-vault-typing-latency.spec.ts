...
const BENCH_ENABLED = process.env.DORKA_AI_VAULT_TYPING_BENCH === '1'
...
const ITERATIONS = readPositiveInt('DORKA_AI_VAULT_BENCH_ITERATIONS', 3)
const SESSION_COUNT = readPositiveInt('DORKA_AI_VAULT_BENCH_SESSIONS', 300)
const PAYLOAD_KIB = readPositiveInt('DORKA_AI_VAULT_BENCH_PAYLOAD_KIB', 128)
const KEY_COUNT = readPositiveInt('DORKA_AI_VAULT_BENCH_KEYS', 100)
const KEY_CADENCE_MS = readPositiveInt('DORKA_AI_VAULT_BENCH_CADENCE_MS', 30)
const BENCH_LABEL = process.env.DORKA_AI_VAULT_BENCH_LABEL ?? 'dev'
...
test.describe('Terminal typing during AI Vault refresh bench', () => {
  ...
  test('alternates control typing and forced Vault refresh typing', async ({
    ...
    const scriptPath = path.join(testRepoPath, `.dorka-vault-typing-${randomUUID()}.mjs`)
