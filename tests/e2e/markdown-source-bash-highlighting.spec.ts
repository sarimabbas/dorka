...
test('highlights bash and sh fences in Markdown Source mode', async ({ dorkaPage }, testInfo) => {
  ...
  try {
    filePath = await createMarkdownFixture(
      context,
      '.dorka-e2e-markdown-source-highlighting',
      'bash-and-sh',
    ...
    await switchToSourceMode(dorkaPage)
