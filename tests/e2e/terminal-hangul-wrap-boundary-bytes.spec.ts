 ...
 * Dorka's to guarantee, and the half that had no coverage.
 */
  ...
  for (const scenario of SCENARIOS) {
    for (const policy of scenario.policies) {
      test(`sends ${scenario.name} unaltered to the pty (${policy})`, async ({
        ...
        await expectImePlatformPolicy(dorkaPage, policy)
        await narrowPaneBySplitting(dorkaPage, 2)
        ...
        try {
          ...
          const observedCols = await waitForSettledCols(dorkaPage, arena.ptyId)
          ...
          const startRow = (await readPaneGrid(dorkaPage, arena.ptyId)).cursorRow
          if (scenario.offsetByOneCell) {
            await dispatchImeRewrittenPrintableKey(arena.session, OFFSET_KEY)
          }
          await scenario.drive(arena.session, dorkaPage, run)
          ...
          await expect
            .poll(async () => (await readPaneGrid(dorkaPage, arena.ptyId)).cursorRow, {
              timeout: 10_000,
          ...
          const received = await waitForTerminalImeBytes(dorkaPage, reader, 20_000)
