...
const fakeCliDir = mkdtempSync(path.join(os.tmpdir(), 'dorka-e2e-legacy-worker-'))
...
async function emitAuthorityHook(hookEventName) {
  const port = process.env.DORKA_AGENT_HOOK_PORT
  const token = process.env.DORKA_AGENT_HOOK_TOKEN
  const launchToken = process.env.DORKA_AGENT_LAUNCH_TOKEN
  if (!port || !token || !launchToken || !process.env.DORKA_PANE_KEY) return
  try {
    ...
    appendLedger('DORKA_E2E_AUTHORITY_LEDGER', {
...
appendLedger('DORKA_E2E_SPAWN_LEDGER', { event: 'spawn', argv: process.argv.slice(2) })
...
process.stdin.on('data', (chunk) => {
  ...
  const legacyCompletion = input.match(/DORKA_E2E_RUN_LEGACY_DONE:([A-Za-z0-9+/=]+)/)
  if (!lifecycleSent && legacyCompletion) {
    lifecycleSent = true
    const identity = JSON.parse(Buffer.from(legacyCompletion[1], 'base64').toString('utf8'))
    const cliEntry = process.env.DORKA_E2E_CLI_ENTRY
    const args = [
      'orchestration',
      'send',
      '--to',
    ...
    const result = cliEntry
      ...
      : { status: 127, stdout: '', stderr: 'DORKA_E2E_CLI_ENTRY missing' }
    appendLedger('DORKA_E2E_LIFECYCLE_LEDGER', {
...
for (const contractVersion of [LEGACY_CONTRACT_VERSION, CURRENT_CONTRACT_VERSION]) {
  ...
  {}, testInfo) => {
    ...
    const session = createRestartSession(testInfo, {
      ...
      DORKA_E2E_AUTHORITY_LEDGER: authorityLedgerPath,
      DORKA_E2E_LIFECYCLE_LEDGER: lifecycleLedgerPath,
      DORKA_E2E_CLI_ENTRY: path.join(process.cwd(), 'out', 'cli', 'index.js')
    })
    ...
    try {
      ...
      if (contractVersion === LEGACY_CONTRACT_VERSION) {
        ...
        await secondClient.call('terminal.send', {
          terminal: recovered!.handle,
          text: `DORKA_E2E_RUN_LEGACY_DONE:${legacyCompletion}`,
          enter: true
        })
        await expect
