...
// Run: DORKA_ROUTING_DEMO=1 DORKA_E2E_SSH_DOCKER=1 pnpm exec playwright test \
...
test.skip(process.env.DORKA_ROUTING_DEMO !== '1', 'Demo only; run with DORKA_ROUTING_DEMO=1')
...
test('stages the per-host routing opt-out loop against a real forwarding-blocked sshd', async ({
  ...
  try {
    ...
    await expect(dorkaPage.getByText('The SSH server blocks browser traffic')).toBeVisible({
      timeout: 180_000
    })
