...
async function installClipboardReadTerminalBlurRepro(app: ElectronApplication): Promise<void> {
  await app.evaluate(({ BrowserWindow, ipcMain }) => {
    ...
    const global = globalThis as unknown as {
      __dorkaOriginalClipboardReadTextHandler?: ClipboardReadHandler
    }
    ...
    if (!invokeHandlers || !handler || global.__dorkaOriginalClipboardReadTextHandler) {
      return
    }
    global.__dorkaOriginalClipboardReadTextHandler = handler
    invokeHandlers.set('clipboard:readText', async (event, ...args) => {
      ...
      return global.__dorkaOriginalClipboardReadTextHandler!(event, ...args)
    })
  })
}
...
async function restoreClipboardReadTerminalBlurRepro(app: ElectronApplication): Promise<void> {
  await app.evaluate(({ ipcMain }) => {
    ...
    if (invokeHandlers && global.__dorkaOriginalClipboardReadTextHandler) {
      invokeHandlers.set('clipboard:readText', global.__dorkaOriginalClipboardReadTextHandler)
      delete global.__dorkaOriginalClipboardReadTextHandler
    }
  })
}
...
test.describe('terminal paste ownership', () => {
  test('keyboard paste shortcuts send clipboard text to the focused terminal exactly once', async ({
    ...
    const scriptPath = path.join(testRepoPath, `.dorka-paste-ownership-${runId}.mjs`)
    ...
    try {
      ...
      for (const [index, chord] of keyboardPasteChords().entries()) {
        const payload = `DORKA_E2E_PASTE_${runId}_${index}`
        ...
        await dorkaPage.keyboard.press(chord)
        await waitForTerminalOutput(dorkaPage, encodedPayload, 10_000, 12_000)
  ...
  test('keyboard paste survives transient terminal blur during clipboard read', async ({
    ...
    const ptyId = await waitForActivePanePtyId(dorkaPage)
    const runId = randomUUID()
    const scriptPath = path.join(testRepoPath, `.dorka-paste-blur-${runId}.mjs`)
    writeFileSync(scriptPath, pasteEchoScript(runId))
    let scriptStarted = false
    ...
    try {
      ...
      const payload = `DORKA_E2E_TRANSIENT_BLUR_PASTE_${runId}`
      const encodedPayload = Buffer.from(payload, 'utf8').toString('base64')
      await clearTerminalPtyWriteLog(electronApp)
      await dorkaPage.evaluate((text) => window.api.ui.writeClipboardText(text), payload)
      await focusActiveTerminalInput(dorkaPage)
      ...
      await dorkaPage.keyboard.press(keyboardPasteChords()[0])
      await waitForTerminalOutput(dorkaPage, encodedPayload, 10_000, 12_000)
  ...
  test('terminal context-menu Paste sends clipboard text exactly once', async ({
    ...
    const ptyId = await waitForActivePanePtyId(dorkaPage)
    const runId = randomUUID()
    const scriptPath = path.join(testRepoPath, `.dorka-paste-context-menu-${runId}.mjs`)
    writeFileSync(scriptPath, pasteEchoScript(runId))
    let scriptStarted = false
    ...
    try {
      ...
      const payload = `DORKA_E2E_CONTEXT_MENU_PASTE_${runId}`
      const encodedPayload = Buffer.from(payload, 'utf8').toString('base64')
      await clearTerminalPtyWriteLog(electronApp)
      await dorkaPage.evaluate((text) => window.api.ui.writeClipboardText(text), payload)
      await focusActiveTerminalInput(dorkaPage)
      ...
      await dorkaPage.getByRole('menuitem', { name: /Paste/ }).click()
      await waitForTerminalOutput(dorkaPage, encodedPayload, 10_000, 12_000)
  ...
  test('Windows multiline keyboard paste normalizes terminal newlines with one PTY owner', async ({
    ...
    const ptyId = await waitForActivePanePtyId(dorkaPage)
    const runId = randomUUID()
    const sentinel = `DORKA_E2E_MULTILINE_DONE_${runId}`
    const payload = [
      `DORKA_E2E_MULTILINE_${runId}`,
    ...
    // Dorka's direct Windows bracketed-paste path must produce the same bytes.
    const terminalText = payload.replace(/\r?\n/g, '\r')
    const scriptPath = path.join(testRepoPath, `.dorka-paste-multiline-${runId}.mjs`)
  ...
  test('right-click paste sends clipboard text to the focused terminal exactly once', async ({
    ...
    const ptyId = await waitForActivePanePtyId(dorkaPage)
    const runId = randomUUID()
    const scriptPath = path.join(testRepoPath, `.dorka-paste-right-click-${runId}.mjs`)
    writeFileSync(scriptPath, pasteEchoScript(runId))
    let scriptStarted = false
    ...
    try {
      ...
      const payload = `DORKA_E2E_RIGHT_CLICK_PASTE_${runId}`
      const encodedPayload = Buffer.from(payload, 'utf8').toString('base64')
      await clearTerminalPtyWriteLog(electronApp)
      await dorkaPage.evaluate((text) => window.api.ui.writeClipboardText(text), payload)
      await focusActiveTerminalInput(dorkaPage)

      await rightClickActiveTerminalSurface(dorkaPage)
      await waitForTerminalOutput(dorkaPage, encodedPayload, 10_000, 12_000)
