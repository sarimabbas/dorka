...
test('recreates a fresh profile index on disk with a restrictive umask @posix-profile-index-golden', async ({
  ...
  const indexPath = path.join(userDataDir, 'dorka-profile-index.json')
  ...
  try {
    ...
    // Why: dorkaProfiles:list reads the index from disk on every call and rebuilds
    // it when missing, so this drives the real create + fsync + rename path.
    const listed = await dorkaPage.evaluate(async () => {
      const result = await window.api.dorkaProfiles.list()
