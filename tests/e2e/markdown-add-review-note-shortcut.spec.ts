...
test.describe('Markdown add-review-note shortcut', () => {
  ...
  test('opens the review-note composer for the current selection in the rich editor', async ({
    ...
    try {
      ...
      await dorkaPage.keyboard.press('ControlOrMeta+Shift+A')

      await expect(dorkaPage.getByPlaceholder('Add note for the AI')).toBeVisible({
        timeout: 5_000
      })
    } finally {
  ...
  test('opens the inline composer for the selected block in the markdown preview', async ({
    ...
    try {
      ...
      await expect(dorkaPage.locator('[data-annotation-block-key]').first()).toBeVisible({
        timeout: 25_000
      })
  ...
  test('does not open the composer without a text selection', async ({ dorkaPage }, testInfo) => {
    const context = await getActiveWorktreeContext(dorkaPage)
    let filePath: string | null = null
    ...
    try {
      ...
      await expect(dorkaPage.getByPlaceholder('Add note for the AI')).toHaveCount(0)
    } finally {
      await cleanupMarkdownFixture(filePath)
    }
  })
