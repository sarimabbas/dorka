...
function buildSigwinchResetProbeCommand(): string {
  ...
  return stageNodeScriptForTerminal(script, { prefix: 'dorka-sigwinch-probe' }).command
}
...
test.describe('Terminal tab switch SIGWINCH restore', () => {
  test('keeps an alternate-screen Codex viewport after hidden snapshot replay', async ({
    ...
    await sendToTerminal(dorkaPage, paneIdentity.ptyId, 'ARM_SIGWINCH_PROBE\n')
    await expect
      .poll(() => getTerminalContent(dorkaPage, 8_000), {
        timeout: 10_000,
    ...
    const armedContentAfterSettle = await getTerminalContent(dorkaPage, 8_000)
    ...
    await expect
      .poll(
        async () => {
          const content = await getTerminalContent(dorkaPage, 8_000)
      ...
      .toBe('snapshot')
    await dorkaPage.waitForTimeout(1_200)
    const contentAfterSettle = await getTerminalContent(dorkaPage, 8_000)
