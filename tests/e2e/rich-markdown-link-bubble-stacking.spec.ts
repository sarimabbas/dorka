...
test.describe('Rich markdown link bubble stacking', () => {
  test('link actions stay above the right Explorer', async ({ dorkaPage }, testInfo) => {
    await dorkaPage.setViewportSize({ width: 1920, height: 1080 })
    ...
    try {
      ...
      const link = dorkaPage.locator(`.rich-markdown-editor a[href="${LINK_HREF}"]`)
      await expect(explorer).toBeVisible()
      await expect(link).toBeVisible()

      await link.click()

      const bubble = dorkaPage.locator('.rich-markdown-link-bubble')
      await expect(bubble).toBeVisible()
      await expect(bubble.locator('.rich-markdown-link-url')).toContainText('https://example.com')

      const overlap = await dorkaPage.evaluate((): OverlapHitTest => {
        const bubble = document.querySelector<HTMLElement>('.rich-markdown-link-bubble')
        const explorer = document.querySelector<HTMLElement>('[data-dorka-explorer-shell]')
        if (!bubble || !explorer) {
      ...
      await dorkaPage.getByRole('heading', { name: 'Rich markdown link overlay repro' }).click()
      await link.click()
      await expect(bubble).toBeVisible()
      const originalEditorZoom = await dorkaPage.evaluate(() => {
        const store = window.__store
        if (!store) {
          throw new Error('window.__store is not available — is the app in dev mode?')
        }
