...
test.describe('Droid notifications', () => {
  test('Codex hook completion dispatches while its worktree is inactive', async ({
    ...
    await sendToTerminal(dorkaPage, ptyId, `printf '${readyMarker}\\n'\r`)
    await waitForTerminalOutput(dorkaPage, readyMarker)
    ...
    await expect
      .poll(
        async () =>
          (await getRendererOrCachedAgentStatuses(dorkaPage)).some(
            (status) =>
    ...
    await switchToOtherExistingWorktree(dorkaPage)
    ...
    await expect
      .poll(
        async () =>
          (await getAgentStatuses(dorkaPage)).some(
            (status) =>
    ...
    await expect
      .poll(async () => isWorktreeUnread(dorkaPage, worktreeId), {
        timeout: 10_000,
  ...
  test('recognized agent title completion dispatches one task-complete notification', async ({
    ...
    await emitOscTitle(dorkaPage, ptyId, 'Codex working')
    await expect
      .poll(async () => (await getRendererTitleLog(dorkaPage)).includes('Codex working'), {
        timeout: 10_000,
        message: 'Codex working title did not reach the renderer before completion'
      })
      .toBe(true)

    await emitOscTitle(dorkaPage, ptyId, 'Codex done')
    await expect
      .poll(async () => (await getRendererTitleLog(dorkaPage)).includes('Codex done'), {
        timeout: 10_000,
  ...
  test('Factory Droid needs-input native title does not dispatch a task-complete notification', async ({
    ...
    await emitOscTitle(dorkaPage, ptyId, '⠋ Droid')
    await emitOscTitle(dorkaPage, ptyId, 'Factory Droid needs input')

    await expect
      .poll(
        async () => (await getRendererTitleLog(dorkaPage)).includes('Factory Droid needs input'),
        {
          timeout: 10_000,
