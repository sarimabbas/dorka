...
test.describe('New workspace composer linked item across project switches', () => {
  ...
  test.beforeEach(async ({ dorkaPage }) => {
    ...
    tempRoot = mkdtempSync(path.join(os.tmpdir(), 'dorka-e2e-linked-item-'))
    secondRepoPath = path.join(tempRoot, SECOND_PROJECT_NAME)
    createGitRepo(secondRepoPath)
    await addSecondProject(dorkaPage, secondRepoPath)
  })
  ...
  test('keeps a Jira issue linked when the project changes', async ({ dorkaPage }) => {
    const jiraSourceContext = await getJiraSourceContext(dorkaPage)
    ...
    const composer = dorkaPage.getByRole('dialog')
    ...
    await switchComposerProject(dorkaPage, SECOND_PROJECT_NAME)

    await expect(sourcePill).toContainText('RDG-344 Migrate homepage from NuxtJS to NextJS')
  })

  test('clears a repo-scoped GitHub issue when the project changes', async ({ dorkaPage }) => {
    await openComposerWithLinkedWorkItem(
      dorkaPage,
      {
        type: 'issue',
