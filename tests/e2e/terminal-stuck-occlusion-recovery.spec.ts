...
test.describe('terminal stuck-occlusion recovery', () => {
  ...
  test('a keystroke unlatches the hidden-delivery gate wedged by stale visibilityState', async ({
    ...
    await execInTerminal(dorkaPage, ptyId, 'echo live-before-$((41+1))')
    await expect
      .poll(async () => getTerminalContent(dorkaPage), { timeout: 15_000 })
    ...
    await expect
      .poll(async () => (await getDeliverySnapshot(dorkaPage)).hiddenDeliveryGatedPtyCount, {
        timeout: 15_000
      })
      .toBeGreaterThan(0)
    expect(
      (await getDeliverySnapshot(dorkaPage)).hiddenDeliveryGatedVisiblePtyCount
    ).toBeGreaterThan(0)

    // The freeze repro: output produced now is dropped by main, not painted.
    const droppedBefore = (await getDeliverySnapshot(dorkaPage)).hiddenDeliveryDroppedChars
    await execInTerminal(dorkaPage, ptyId, 'echo occluded-$((70+8))')
    await expect
      .poll(async () => (await getDeliverySnapshot(dorkaPage)).hiddenDeliveryDroppedChars, {
        timeout: 15_000
      })
      .toBeGreaterThan(droppedBefore)
    expect(await getTerminalContent(dorkaPage)).not.toContain('occluded-78')
    ...
    await dorkaPage.keyboard.press('Shift')

    // Gate unlatches and the missed output repaints from the main-owned
    // snapshot — no reload, visibilityState still reads 'hidden'.
    await expect
      .poll(async () => getTerminalContent(dorkaPage), { timeout: 30_000 })
      .toContain('occluded-78')
    await expect
      .poll(async () => (await getDeliverySnapshot(dorkaPage)).hiddenDeliveryGatedVisiblePtyCount, {
        timeout: 15_000
      })
      .toBe(0)

    // Live delivery continues under the override.
    await execInTerminal(dorkaPage, ptyId, 'echo live-after-$((200+56))')
    await expect
      .poll(async () => getTerminalContent(dorkaPage), { timeout: 15_000 })
    ...
    const report = await dorkaPage.evaluate(() =>
      (
        window as Window & {
          __dorkaTerminalFreezeReport?: () => Promise<{
