...
import { test, expect, forwardElectronProcessLogs } from './helpers/dorka-app'
import { TEST_REPO_PATH_FILE } from './global-setup'
import { getE2ECompletedOnboardingProfile } from './helpers/e2e-completed-onboarding-profile'
import { getDorkaElectronLaunchArgs } from './helpers/electron-launch-args'
...
{}, testInfo) => {
  ...
  const userDataDir = mkdtempSync(path.join(os.tmpdir(), 'dorka-e2e-serve-promotion-'))
  ...
  try {
    serveApp = await electron.launch({
      args: [...getDorkaElectronLaunchArgs(mainPath, false), '--serve', '--serve-no-pairing'],
      env
    })
    ...
    const forwardAppLogs = process.env.DORKA_E2E_FORWARD_APP_LOGS === '1'
    activatingProcess = spawn(electronPath, getDorkaElectronLaunchArgs(mainPath, false), {
      env,
