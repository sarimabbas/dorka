...
const scratch = mkdtempSync(path.join(os.tmpdir(), 'dorka-paired-probe-gap-'))
...
test('replaces a stale paired stream when the PTY snapshot advanced @headful', async ({
  ...
  try {
    ...
    const hostTerminals = await callRuntime<RuntimeTerminalListResult>(dorkaPage, 'terminal.list', {
      worktree: `id:${worktreeId}`,
    ...
    await client.page
      ...
      .catch(() => undefined)
    if (terminal) {
      await callRuntime(dorkaPage, 'terminal.closeTab', { terminal }).catch(() => undefined)
    }
