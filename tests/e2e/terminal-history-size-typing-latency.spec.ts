...
test.describe('Terminal typing latency vs scrollback history size', () => {
  test('typing stays responsive as terminal history grows', async ({
    ...
    const scriptPath = path.join(testRepoPath, `.dorka-history-benchmark-${runId}.mjs`)
    ...
    try {
      ...
      commandSent = true
      await waitForRecentTerminalMarker(dorkaPage, `HIST_READY_${runId}`, 10_000)
      await focusActiveTerminalInput(dorkaPage)

      const phases: PhaseLatency[] = []
      let seq = 0

      const baseline = await measureTypingLatency(dorkaPage, runId, 'empty history', seq)
