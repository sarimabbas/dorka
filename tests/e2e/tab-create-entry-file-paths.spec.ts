...
const relativeFilePath =
  'packages/dorka/src/renderer/src/components/navigation/worktree/secondary-nav/SecondaryNav.tsx'
...
test('new-tab file results prioritize the filename and reveal the full path on hover', async ({
  ...
  const newTab = dorkaPage.getByRole('button', { name: 'New tab' })
  ...
  // translated copy.
  const input = dorkaPage.locator('input[role="combobox"][aria-autocomplete="list"]')
  const row = dorkaPage.locator('[role="option"]').filter({ hasText: 'Open file' }).first()
  ...
  await expect(
    dorkaPage.locator('[data-slot="tooltip-content"]').filter({ hasText: relativeFilePath })
  ).toBeVisible()

  const proofPath = process.env.DORKA_STA3424_PROOF_PATH
  if (proofPath) {
