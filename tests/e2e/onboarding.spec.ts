...
test.describe('Onboarding flow', () => {
  ...
  test.beforeEach(async ({ dorkaPage }) => {
    // Per-test userData is freshly minted by the dorkaPage fixture, so persisted
  ...
  test('renders on first launch with the agent step active', async ({ dorkaPage }) => {
    await expect(dorkaPage.getByRole('heading', { name: /Pick your default agent/i })).toBeVisible({
      timeout: 15_000
    })
    await expectOnboardingProgress(dorkaPage, /^1 of [345]$/)
    await expect(onboardingFooterButton(dorkaPage, /^Continue\b/)).toBeVisible()
    await expect(onboardingFooterButton(dorkaPage, SKIP_TO_PROJECT_SETUP_BUTTON)).toBeVisible()
    ...
    await expect(dorkaPage.getByRole('button', { name: 'Back', exact: true })).toHaveCount(0)
    ...
    await expect(
      onboardingFooterButton(dorkaPage, /^Continue\b/)
        .locator('span')
        .filter({ hasText: /⌘|Ctrl/ })
        .first()
    ).toBeVisible()
  ...
  test('Continue advances steps, persists progress, and applies user-visible settings', async ({
    ...
    const codexButton = dorkaPage.getByRole('button', { name: /^Codex\s/ })
    ...
    const codexVisible = await codexButton
      ...
      .catch(() => false)
    if (!codexVisible) {
      await dorkaPage.getByText(/Show \d+ more agents/).click()
    }
    ...
    await expectOnboardingProgress(dorkaPage, /^2 of [345]$/)
    await expect
      .poll(async () => (await getOnboardingState(dorkaPage)).lastCompletedStep, {
        timeout: 5_000,
    ...
    await expect
      .poll(async () => (await getSettings(dorkaPage)).defaultTuiAgent, { timeout: 5_000 })
      .toBe(targetAgent)
    ...
    await dorkaPage.waitForFunction(
      () =>
        document.documentElement.classList.contains('dark') ||
        document.documentElement.classList.contains('light')
    )
    const startingTheme = await getDocumentThemeClass(dorkaPage)
    ...
    await dorkaPage.getByRole('button', { name: oppositeTileName }).click()
    await expect
      .poll(async () => getDocumentThemeClass(dorkaPage), { timeout: 5_000 })
      .toBe(oppositeTheme)
    ...
    await expect
      .poll(async () => (await getSettings(dorkaPage)).theme, { timeout: 5_000 })
      .toBe(oppositeTheme)
    await continueThroughOptionalTaskSourcesAndWindowsTerminal(dorkaPage)
    await expectOnboardingProgress(dorkaPage, /^[345] of [345]$/)
    await expect
      .poll(async () => [3, 4].includes((await getOnboardingState(dorkaPage)).lastCompletedStep), {
        timeout: 5_000,
    ...
    await expectOnboardingNotificationSound(dorkaPage, /System Default/i)
    await expect(dorkaPage.getByRole('button', { name: /Send Test Notification/i })).toBeVisible()
    await expectOnboardingCustomSoundOption(dorkaPage)

    await continueFromPostNotificationsToRepo(dorkaPage)
    ...
    await expect
      .poll(
        async () => {
          const s = await getSettings(dorkaPage)
          return {
    ...
    await expect
      .poll(
        async () => {
          const state = await getOnboardingState(dorkaPage)
          return {
  ...
  test('Cmd/Ctrl+Enter advances steps like Continue', async ({ dorkaPage }) => {
    await expect(dorkaPage.getByRole('heading', { name: /Pick your default agent/i })).toBeVisible({
      timeout: 15_000
    })
    ...
    await dorkaPage.locator('footer').click({ position: { x: 1, y: 1 } })
    await dorkaPage.keyboard.press(accelerator)
    await expect(dorkaPage.getByRole('heading', { name: /Make it feel like home/i })).toBeVisible()
    await expect
      .poll(async () => (await getOnboardingState(dorkaPage)).lastCompletedStep, {
        timeout: 5_000
  ...
  test('Skip opens Add Project, saves the selected agent, and completes onboarding', async ({
    ...
    await onboardingFooterButton(dorkaPage, SKIP_TO_PROJECT_SETUP_BUTTON).click()
  ...
  test('Skip from theme restores the entry theme choice', async ({ dorkaPage }) => {
    await expect(dorkaPage.getByRole('heading', { name: /Pick your default agent/i })).toBeVisible({
      timeout: 15_000
    })
    await continueOnboarding(dorkaPage)
    ...
    const entryTheme = (await getSettings(dorkaPage)).theme
    const startingTheme = await getDocumentThemeClass(dorkaPage)
    const oppositeTheme: 'dark' | 'light' = startingTheme === 'dark' ? 'light' : 'dark'
    const oppositeTileName = oppositeTheme === 'light' ? /Bright & crisp/ : /Easy on the eyes/
    await dorkaPage.getByRole('button', { name: oppositeTileName }).click()
  ...
  test('Skip preserves runtime server project setup UI', async ({ dorkaPage }) => {
    await expect(dorkaPage.getByRole('heading', { name: /Pick your default agent/i })).toBeVisible({
      timeout: 15_000
    })
    ...
    const environmentId = await dorkaPage.evaluate(async (code) => {
    ...
    await expect
      .poll(async () => (await getSettings(dorkaPage)).activeRuntimeEnvironmentId, {
        timeout: 5_000
      })
      .toBe(environmentId)
    ...
    await expect(onboardingFooterButton(dorkaPage, SKIP_TO_PROJECT_SETUP_BUTTON)).toHaveCount(0)
    expect((await getOnboardingState(dorkaPage)).closedAt).not.toBeNull()
  })

  test('Skip from notifications does not request permission', async ({ dorkaPage }) => {
    await expect(dorkaPage.getByRole('heading', { name: /Pick your default agent/i })).toBeVisible({
      timeout: 15_000
    })
    await continueOnboarding(dorkaPage)
    await expect(dorkaPage.getByRole('heading', { name: /Make it feel like home/i })).toBeVisible()
    await continueFromThemeToNotifications(dorkaPage)

    await dorkaPage.evaluate(() => {
      localStorage.removeItem('dorka.e2e.notificationPermissionRequested')
      window.api.notifications.requestPermission = async () => {
        localStorage.setItem('dorka.e2e.notificationPermissionRequested', '1')
  ...
  test('selected agent button reports aria-pressed=true', async ({ dorkaPage }) => {
    await expect(dorkaPage.getByRole('heading', { name: /Pick your default agent/i })).toBeVisible({
      timeout: 15_000
    })
  ...
  test('notification sound choice persists on Continue', async ({ dorkaPage }) => {
    await expect(dorkaPage.getByRole('heading', { name: /Pick your default agent/i })).toBeVisible({
      timeout: 15_000
    })
    await continueOnboarding(dorkaPage)
    ...
    await chooseOnboardingNotificationSound(dorkaPage, /^Ding$/i)
  ...
  test('typing in the clone-url input does not hijack Enter as a global shortcut', async ({
    ...
    await continueOnboarding(dorkaPage)
    await continueFromPostNotificationsToRepo(dorkaPage)
    await dorkaPage.getByRole('button', { name: /Clone from URL/i }).click()
    ...
    const isMac = await dorkaPage.evaluate(() => navigator.userAgent.includes('Mac'))
    const accelerator = isMac ? 'Meta+Enter' : 'Control+Enter'
    const input = dorkaPage.getByPlaceholder('https://github.com/user/repo.git')
    await input.click()
    ...
    await expect(dorkaPage.getByRole('heading', { name: /Clone from URL/i })).toBeVisible()
    await expect(input).toBeVisible()
    expect((await getOnboardingState(dorkaPage)).closedAt).not.toBeNull()
  })

  test('Back returns to the previous step without losing progress', async ({ dorkaPage }) => {
    await expect(dorkaPage.getByRole('heading', { name: /Pick your default agent/i })).toBeVisible({
      timeout: 15_000
    })
    ...
    await dorkaPage.getByRole('button', { name: 'Back', exact: true }).click()
    await expect(dorkaPage.getByRole('heading', { name: /Pick your default agent/i })).toBeVisible()
    await expectOnboardingProgress(dorkaPage, /^1 of [345]$/)
  ...
  test('final notification step can be dismissed via Escape or click-off', async ({ dorkaPage }) => {
    ...
    await expect(dorkaPage.getByRole('heading', { name: /Set up notifications/i })).toBeVisible()
    await expect(onboardingFooterButton(dorkaPage, SKIP_TO_PROJECT_SETUP_BUTTON)).toHaveCount(0)
    ...
    await expectOnboardingSkipConfirmationOpen(dorkaPage)
    await dorkaPage.getByRole('button', { name: /No, keep going/i }).click()
    await expectOnboardingSkipConfirmationClosed(dorkaPage)
    await expect(dorkaPage.getByRole('heading', { name: /Set up notifications/i })).toBeVisible()
    expect((await getOnboardingState(dorkaPage)).closedAt).toBeNull()
    ...
    await dorkaPage.locator('[data-onboarding-overlay]').click({ position: { x: 8, y: 40 } })
    await expectOnboardingSkipConfirmationOpen(dorkaPage)
    await dorkaPage.getByRole('button', { name: /^Skip$/ }).click()
