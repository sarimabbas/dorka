...
test.describe('SSH host add/edit modal', () => {
  ...
  test.afterEach(async ({ dorkaPage }) => {
    const ids = await listTargetIdsByLabelPrefix(dorkaPage, HOST_PREFIX)
    if (ids.length > 0) {
      await removeSshTargets(dorkaPage, ids)
    }
  })
  ...
  test('opens add/edit form in a viewport-stable dialog over a long host list', async ({
    ...
    const seeded = await seedSshTargets(dorkaPage, 10)
    await openSshHostSettings(dorkaPage)

    const sshSection = dorkaPage.locator('[data-settings-section="ssh"]')
    ...
    const addDialog = dorkaPage.getByRole('dialog', { name: 'Add SSH host' })
    ...
    const editDialog = dorkaPage.getByRole('dialog', { name: 'Edit SSH host' })
    ...
    await dorkaPage.locator('[data-slot="dialog-overlay"]').click({ position: { x: 8, y: 8 } })
    await expect(editDialog).toBeVisible()
    ...
    await createdCard.getByRole('button', { name: 'Edit target' }).click()
    const reopened = dorkaPage.getByRole('dialog', { name: 'Edit SSH host' })
  ...
  test('add-ssh-host settings intent opens the same modal dialog', async ({ dorkaPage }) => {
    ...
    await expect(dorkaPage.getByPlaceholder('Search settings')).toBeVisible({ timeout: 10_000 })
    await dismissTransientAnnouncement(dorkaPage)

    const dialog = dorkaPage.getByRole('dialog', { name: 'Add SSH host' })
