...
test.describe('Setup script import prompt', () => {
  ...
  test('imports Superset local overlays through the prompt UI', async ({ dorkaPage }, testInfo) => {
    const repoPath = createSupersetSetupRepo(testInfo.outputPath('superset-setup-repo'))
    const repoId = await addAndActivateRepo(dorkaPage, repoPath)
    ...
    await dorkaPage.getByRole('button', { name: 'Save local setup' }).click()

    await expect(
      dorkaPage.getByText('2 unsupported fields skipped. Saved the setup command.')
    ).toBeVisible()

    const localCommands = await openImportedSetupSettingsFromToast(dorkaPage, repoId)
  ...
  test('imports cmux setup commands through the prompt UI', async ({ dorkaPage }, testInfo) => {
    ...
    await expect(
      dorkaPage.getByRole('button', { name: "project's settings", exact: true })
    ).toBeVisible()

    const repoSettings = await openRepoSettings(dorkaPage, repoId)
