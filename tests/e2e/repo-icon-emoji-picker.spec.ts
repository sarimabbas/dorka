...
async function openRepoSettings(page: Page, repoId: string): Promise<void> {
  // Why: the host OS locale (e.g. ko-KR) drives Dorka's default UI language.
...
test.describe('Repository icon emoji picker', () => {
  test('search selects a native emoji and persists it as the repo icon', async ({
    ...
    const repos = await getStoreState<Repo[]>(dorkaPage, 'repos')
    expect(repos.length).toBeGreaterThan(0)
    const repo = repos[0]

    await openRepoSettings(dorkaPage, repo.id)

    const repoSection = dorkaPage.locator(`[data-settings-section="repo-${repo.id}"]`)
    ...
    await attachScreenshot(dorkaPage, testInfo, 'repo-icon-emoji-picker-search')
    ...
    await expect
      .poll(
        async () => {
          const current = await getStoreState<Repo[]>(dorkaPage, 'repos')
    ...
    await attachScreenshot(dorkaPage, testInfo, 'repo-icon-emoji-picker-selected')
  })
})
