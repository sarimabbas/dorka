...
// scoped to this spec's app launches via dorkaAppExtraEnv (same pattern as
...
test.use({
  ...
  dorkaAppExtraArgs: ['--enable-precise-memory-info']
})

// Why: 8 hidden tabs is below the 12-tab hot-retain limit, but that limit
// never retains anything here — the DORKA_E2E_TERMINAL_PARKING_DELAY_MS
...
test.describe('Terminal parked memory', () => {
  test('releases renderer terminal memory when hidden tabs park', async ({
    ...
    const scriptPath = path.join(testRepoPath, `.dorka-parked-memory-${runId}.mjs`)
    writeScrollbackFillScript(scriptPath, runId)
    try {
      const { worktreeId, scrollbackTabs } = await setUpScrollbackTabs(dorkaPage, scriptPath, runId)
      ...
      // the other 7 park.
      const visibleTab = await createActiveTerminalTab(dorkaPage, worktreeId)
      ...
      const metrics = await sampleParkedMemoryMetrics(dorkaPage)
      ...
      for (const tab of parkableTabs) {
        expect((await readTerminalTabViewState(dorkaPage, tab.tabId)).hasManager).toBe(false)
      }
      expect((await readTerminalTabViewState(dorkaPage, lastActiveTab.tabId)).hasManager).toBe(true)
      const visibleState = await readTerminalTabViewState(dorkaPage, visibleTab.tabId)
  ...
  test('retains terminal views when parking is disabled', async ({
    ...
    await expect
      .poll(
        () =>
          dorkaPage.evaluate(() => window.__store?.getState().settings?.terminalHiddenViewParking),
    ...
    try {
      ...
      await dorkaPage.waitForTimeout(PARKING_DELAY_MS * 3)
      expect(await countMountedPaneManagers(dorkaPage, scrollbackTabIds)).toBe(SCROLLBACK_TAB_COUNT)
      ...
      for (const tab of scrollbackTabs) {
        const state = await readTerminalTabViewState(dorkaPage, tab.tabId)
        expect(state.hasManager).toBe(true)
        expect(state.paneCount).toBeGreaterThan(0)
      }
      expect((await readTerminalTabViewState(dorkaPage, visibleTab.tabId)).hasManager).toBe(true)
...
test.describe('Terminal hidden worktree retention budget', () => {
  ...
  test('releases un-parkable hidden worktree buffers only once the retention budget engages', async ({
    ...
    await waitForSessionReady(dorkaPage)
    const victimWorktreeId = await waitForActiveWorktree(dorkaPage)
    await skipUnlessParkingWired(dorkaPage)

    const decoyWorktreeId = (await getAllWorktreeIds(dorkaPage)).find(
    ...
    await updateTerminalSettings(dorkaPage, {
      terminalHiddenWorktreeRetentionBudget: false,
      terminalScrollbackRows: RETENTION_SCROLLBACK_ROWS
    })
    await waitForRetentionBudgetSetting(dorkaPage, false)

    const runId = randomUUID()
    const scriptPath = path.join(testRepoPath, `.dorka-retention-memory-${runId}.mjs`)
    ...
    try {
      ...
      const baselineSnapshot = await waitForPaneIdentitySnapshot(dorkaPage, 1)
      const baselinePtyId = baselineSnapshot.panes[0]?.ptyId
      if (!baselinePtyId) {
      ...
      for (let tabIndex = 0; tabIndex < RETENTION_TAB_COUNT; tabIndex += 1) {
        if (tabIndex > 0) {
          victimTabs.push(await createActiveTerminalTab(dorkaPage, victimWorktreeId))
        }
        ...
        await stageUnparkableWorktreeTabs(dorkaPage, victimWorktreeId)
      }
      ...
      await switchToWorktree(dorkaPage, decoyWorktreeId)
      await expect
      ...
      const decoySnapshot = await waitForPaneIdentitySnapshot(dorkaPage, 1)
      const decoyTabIds = [decoySnapshot.tabId]
      expect(await countMountedPaneManagers(dorkaPage, decoyTabIds)).toBe(1)
      ...
      await stageUnparkableWorktreeTabs(dorkaPage, victimWorktreeId)
      await stageUnparkableWorktreeTabs(dorkaPage, decoyWorktreeId)
      await waitForUnparkableWorktreeTabs(dorkaPage, victimWorktreeId)
      await waitForUnparkableWorktreeTabs(dorkaPage, decoyWorktreeId)
      ...
      await expect
        .poll(
          async () => {
            await stageUnparkableWorktreeTabs(dorkaPage, victimWorktreeId)
            await stageUnparkableWorktreeTabs(dorkaPage, decoyWorktreeId)
            const victimMounted = await countMountedPaneManagers(dorkaPage, victimTabIds)
            const decoyMounted = await countMountedPaneManagers(dorkaPage, decoyTabIds)
      ...
      const before = await readRetentionMemorySample(dorkaPage)
      expect(before.bufferMb).toBeGreaterThan(MIN_STAGED_BUFFER_MB)

      const flipStartedAt = Date.now()
      await updateTerminalSettings(dorkaPage, { terminalHiddenWorktreeRetentionBudget: true })
      await waitForRetentionBudgetSetting(dorkaPage, true)
      await expect
        .poll(() => countMountedPaneManagers(dorkaPage, victimTabIds), {
          timeout: 30_000,
      ...
      const after = await readRetentionMemorySample(dorkaPage)
      testInfo.annotations.push({
      ...
      expect(await countMountedPaneManagers(dorkaPage, decoyTabIds)).toBe(decoyTabIds.length)
