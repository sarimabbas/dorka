...
async function dispatchCtrlCToActiveTerminalTextarea(
  ...
  return page.evaluate((dispatchOptions) => {
    ...
    // synthetic DOM events still exercise Dorka's installed xterm boundary.
...
test.describe('Terminal Shortcuts', () => {
  ...
  test('Shift+Enter follows the pane Kitty keyboard state', async ({ dorkaPage, electronApp }) => {
    await installMainProcessPtyWriteSpy(electronApp)
    const ptyId = await waitForActivePanePtyId(dorkaPage)

    await pressAndExpectWrite(dorkaPage, electronApp, 'Shift+Enter', '\x1b\r')
    ...
    await execInTerminal(dorkaPage, ptyId, "printf '\\033[>1u'")
    await expect.poll(() => getKittyKeyboardFlags(dorkaPage)).toBe(1)
    await pressAndExpectWrite(dorkaPage, electronApp, 'Shift+Enter', '\x1b[13;2u')
    ...
    await sendToTerminal(dorkaPage, ptyId, '\x15\x03')
    await execInTerminal(dorkaPage, ptyId, "printf '\\033[=0u'")
    await expect.poll(() => getKittyKeyboardFlags(dorkaPage)).toBe(0)
    await pressAndExpectWrite(dorkaPage, electronApp, 'Shift+Enter', '\x1b\r')
  })
  ...
  test('Droid gets CSI-u Shift+Enter on Windows without changing Antigravity', async ({
    ...
    const paneKey = await setActivePaneForegroundAgent(dorkaPage, 'droid')
    try {
      await pressAndExpectWrite(dorkaPage, electronApp, 'Shift+Enter', '\x1b[13;2u', 2)
      await setActivePaneForegroundAgent(dorkaPage, 'antigravity')
      await pressAndExpectWrite(dorkaPage, electronApp, 'Shift+Enter', '\x1b\r')
    } finally {
  ...
  test('Windows forwards genuine Ctrl+Alt text chords to the PTY', async ({
    ...
    await pressAndExpectWrite(dorkaPage, electronApp, 'Control+Alt+u', '\x1b\x15')
    await pressAndExpectWrite(dorkaPage, electronApp, 'Control+Alt+2', '\x1b2')
    await pressAndExpectWrite(dorkaPage, electronApp, 'Control+Alt+;', '\x1b;')
  })
  ...
  test('Ctrl+Enter protects local ConPTY shells without breaking trusted TUI chords', async ({
    ...
    if (process.platform === 'win32') {
      await pressAndExpectWrite(dorkaPage, electronApp, 'Control+Enter', '\r')
      const paneKey = await setActivePaneForegroundAgent(dorkaPage, 'droid')
      try {
        // Droid queries CSI-u without activating live flags; trusted process evidence preserves cue/queue.
        await pressAndExpectWrite(dorkaPage, electronApp, 'Control+Enter', '\x1b[13;5u')
      } finally {
        await dorkaPage.evaluate(
          (key) => window.__store?.getState().clearPaneForegroundAgent(key),
          paneKey
  ...
  test('plain Ctrl+C sends ETX under kitty keyboard reporting', async ({
    ...
    await installMainProcessPtyWriteSpy(electronApp)
    await waitForActivePanePtyId(dorkaPage)
    await enableKittyKeyboardReporting(dorkaPage, 31)
    await clearPtyWriteLog(electronApp)
    await focusActiveTerminalInput(dorkaPage)
    await dorkaPage.keyboard.down('Control')
    await dorkaPage.keyboard.up('Control')
    ...
    await expect
      .poll(async () => await getKittyKeyboardFlags(dorkaPage), {
        timeout: 5_000,
  ...
  test('@headful Codex-like background output stays visible without disabling WebGL in auto mode', async ({
    ...
    const hasPane = await dorkaPage.evaluate(() => {
      const state = window.__store?.getState()
      const worktreeId = state?.activeWorktreeId
      const tabId =
        state?.activeTabType === 'terminal'
    ...
    await execInTerminal(dorkaPage, ptyId, `printf '\\033[48;2;52;52;52m  ${marker}  \\033[0m\\n'`)
    await waitForTerminalOutput(dorkaPage, marker)

    await expect
      .poll(
        () =>
          dorkaPage.evaluate((expectedMarker) => {
            const state = window.__store?.getState()
            const worktreeId = state?.activeWorktreeId
            const tabId =
              state?.activeTabType === 'terminal'
  ...
  test('floating terminal owns tab switch shortcuts while focused', async ({ dorkaPage }) => {
    const scenario = await seedFloatingTerminalTabSwitchScenario(dorkaPage)
    await dorkaPage.evaluate(async () => {
      ...
      if (!document.querySelector('[data-floating-terminal-panel][aria-hidden="false"]')) {
        window.dispatchEvent(new CustomEvent('dorka-toggle-floating-terminal'))
      }
    })
    await expect(
      dorkaPage.locator('[data-floating-terminal-panel][aria-hidden="false"]')
    ).toBeVisible()
    await focusFloatingTerminal(dorkaPage)

    await dorkaPage.keyboard.press(`${mod}+Shift+BracketRight`)
    await expect
      .poll(() => getActiveFloatingTerminalTabId(dorkaPage), {
        timeout: 5_000,
        message: 'floating terminal did not switch to the next tab'
      })
      .toBe(scenario.floatingSecondTabId)
    await expect
      .poll(() => getActiveBackgroundTerminalTabId(dorkaPage), {
        timeout: 1_000,
    ...
    await focusFloatingTerminal(dorkaPage)
    await dorkaPage.keyboard.press(`${mod}+Shift+BracketLeft`)
    await expect
      .poll(() => getActiveFloatingTerminalTabId(dorkaPage), {
        timeout: 5_000,
    ...
    await expect(getActiveBackgroundTerminalTabId(dorkaPage)).resolves.toBe(
  ...
  test('all terminal chords reach the PTY or fire their action', async ({
    ...
    await pressAndExpectWrite(dorkaPage, electronApp, 'Alt+ArrowLeft', '\x1bb')
    await pressAndExpectWrite(dorkaPage, electronApp, 'Alt+ArrowRight', '\x1bf')
    ...
    if (!isMac && process.platform !== 'win32') {
      await pressAndExpectWrite(dorkaPage, electronApp, 'Control+ArrowLeft', '\x1bb')
      await pressAndExpectWrite(dorkaPage, electronApp, 'Control+ArrowRight', '\x1bf')
    }

    // Alt+Backspace → Esc+DEL (readline backward-kill-word).
    await pressAndExpectWrite(dorkaPage, electronApp, 'Alt+Backspace', '\x1b\x7f')

    // Ctrl+Backspace → \x17 (unix-word-rubout).
    await pressAndExpectWrite(dorkaPage, electronApp, 'Control+Backspace', '\x17')
    ...
    if (isMac) {
      ...
      await pressAndExpectWrite(dorkaPage, electronApp, 'Meta+ArrowLeft', '\x01')
      await pressAndExpectWrite(dorkaPage, electronApp, 'Meta+ArrowRight', '\x05')

      // Cmd+Backspace → Ctrl+U (kill line). Cmd+Delete → Ctrl+K (kill to EOL).
      await pressAndExpectWrite(dorkaPage, electronApp, 'Meta+Backspace', '\x15')
      await pressAndExpectWrite(dorkaPage, electronApp, 'Meta+Delete', '\x0b')
    }
    ...
    await focusActiveTerminalInput(dorkaPage)
    await dorkaPage.keyboard.press(`${mod}+k`)
    await expect
      .poll(async () => (await getTerminalContent(dorkaPage)).includes(marker), {
        timeout: 5_000,
    ...
    const panesBeforeSplit = await countVisibleTerminalPanes(dorkaPage)
    await focusActiveTerminalInput(dorkaPage)
    await dorkaPage.keyboard.press(splitVerticalChord)
    await waitForPaneCount(dorkaPage, panesBeforeSplit + 1)
    ...
    await focusActiveTerminalInput(dorkaPage)
    await dorkaPage.keyboard.press(`${mod}+BracketRight`)
    await focusActiveTerminalInput(dorkaPage)
    await dorkaPage.keyboard.press(`${mod}+BracketLeft`)
    expect(await countVisibleTerminalPanes(dorkaPage)).toBe(panesBeforeSplit + 1)
    ...
    await focusActiveTerminalInput(dorkaPage)
    await dorkaPage.keyboard.press(`${mod}+Shift+Enter`)
    await expect
    ...
    await closeActivePaneAndSettle(dorkaPage, panesBeforeSplit)

    // Split horizontally (chord varies by platform — see splitHorizontalChord).
    const panesBeforeHSplit = await countVisibleTerminalPanes(dorkaPage)
    await focusActiveTerminalInput(dorkaPage)
    await dorkaPage.keyboard.press(splitHorizontalChord)
    await waitForPaneCount(dorkaPage, panesBeforeHSplit + 1)
    await waitForActivePanePtyId(dorkaPage)
    await closeActivePaneAndSettle(dorkaPage, panesBeforeHSplit)

    // Cmd/Ctrl+F toggles the search overlay.
    await focusActiveTerminalInput(dorkaPage)
    await dorkaPage.keyboard.press(`${mod}+f`)
    const searchInput = dorkaPage.locator('[data-terminal-search-root] input').first()
    ...
    await expect(dorkaPage.locator('[data-terminal-search-root]').first()).toBeHidden({
      timeout: 3_000
    })
  })
  ...
  test('Cmd+Up/Down scrolls terminal viewport without writing to the PTY on macOS', async ({
    ...
    await execInTerminal(dorkaPage, ptyId, `for i in {1..120}; do echo ${marker}_$i; done`)
    await waitForTerminalOutput(dorkaPage, `${marker}_120`)

    await expect
      .poll(
        async () => {
          const viewport = await getActiveTerminalViewport(dorkaPage)
    ...
    await clearPtyWriteLog(electronApp)
    await focusActiveTerminalInput(dorkaPage)
    await dorkaPage.keyboard.press('Meta+ArrowUp')
    await expect
      .poll(async () => getActiveTerminalViewport(dorkaPage), {
        timeout: 5_000,
    ...
    await focusActiveTerminalInput(dorkaPage)
    await dorkaPage.keyboard.press('Meta+ArrowDown')
    await expect
      .poll(
        async () => {
          const viewport = await getActiveTerminalViewport(dorkaPage)
  ...
  test('Shift with Russian layout text reaches the PTY as Cyrillic under kitty keyboard reporting', async ({
    ...
    const dispatch = await pressShiftedRussianLayoutKey(dorkaPage)
