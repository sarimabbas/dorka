...
test.skip(
  process.env.DORKA_E2E_SSH_CLIENT_HOSTED_BROWSER !== '1',
  'Run with DORKA_E2E_SSH_CLIENT_HOSTED_BROWSER=1 (requires Docker)'
)
...
test('recovers client-hosted SSH-routed browser pages across a real SSH drop', async ({
  ...
  try {
    ...
    const beforeDrop = await readHubSshState(dorkaPage, remote.targetId)
    ...
    const afterDrop = await reconnectHubSshTarget(dorkaPage, remote.targetId)
    expect(
