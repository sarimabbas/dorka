...
const fakeGhDir = mkdtempSync(path.join(os.tmpdir(), 'dorka-e2e-fake-gh-'))
...
test('GitHub Tasks drawer recovers when gh stalls on issue details', async ({
  ...
  const { repoId } = await dorkaPage.evaluate((repoPath) => {
  ...
  const detailHeading = dorkaPage.getByRole('heading', {
  ...
  await expect(dorkaPage.getByText('Unable to load details for this GitHub item.')).toBeVisible({
    timeout: 5_000
  })
