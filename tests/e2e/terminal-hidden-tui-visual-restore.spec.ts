...
async function writeHiddenSideEffectBurst(
  ...
  await runNodeScriptInTerminal(page, ptyId, script, { prefix: 'dorka-hidden-side-effect' })
}
...
test.describe('Hidden terminal TUI visual restore', () => {
  test('restores hidden full-screen TUI output without visible corruption', async ({
    ...
    const hiddenSnapshot = await waitForPaneIdentitySnapshot(dorkaPage, 1)
    ...
    const scriptPath = path.join(testRepoPath, `.dorka-hidden-tui-visual-${runId}.mjs`)
    writeHiddenFrameScript(scriptPath, runId)
    await resetHiddenDebug(dorkaPage)
    try {
      await writeHiddenFrames(dorkaPage, hiddenPane.ptyId, scriptPath)
      await resetHiddenDebug(dorkaPage)
      ...
      await expect
        .poll(() => readMainHiddenDeliveryDroppedChars(dorkaPage), {
          timeout: 10_000,
          message: 'visually rich hidden TUI output was not withheld from the renderer'
        })
        .toBeGreaterThan(1024)
      await expect
        .poll(() => readMainSnapshotSource(dorkaPage, hiddenPane.ptyId!), {
          timeout: 10_000,
      ...
      await expect
        .poll(() => readTuiCursorState(dorkaPage), {
          timeout: 5_000,
      ...
      await sendToTerminal(dorkaPage, hiddenPane.ptyId, '\x03').catch(() => undefined)
  ...
  test('keeps newer live output correct after plain hidden output restores', async ({
    ...
    const scriptPath = path.join(testRepoPath, `.dorka-low-risk-hidden-${runId}.mjs`)
    writeLowRiskFrameScript(scriptPath, hiddenFrame)
    await resetHiddenDebug(dorkaPage)
    await sendToTerminal(dorkaPage, hiddenPane.ptyId, `node ${JSON.stringify(scriptPath)}\r`)
    await resetHiddenDebug(dorkaPage)
    ...
    await ensureTerminalVisible(dorkaPage)
    await waitForActiveTerminalManager(dorkaPage, 30_000)
    await injectPaneData(dorkaPage, paneKey, liveFrame, {
  ...
  test('restores rich synchronized TUI output from the headless model', async ({
    ...
    const runId = randomUUID()
    const finalMarker = `VISUAL_RESTORE_FINAL_${runId}_24`
    const scriptPath = path.join(testRepoPath, `.dorka-hidden-rich-model-${runId}.mjs`)
    writeHiddenFrameScript(scriptPath, runId)
    await resetHiddenDebug(dorkaPage)
    try {
      await writeHiddenFrames(dorkaPage, hiddenPane.ptyId, scriptPath)
  ...
  test('keeps hidden terminal side effects live while hidden output may restore', async ({
    ...
    await resetHiddenDebug(dorkaPage)
    await writeHiddenSideEffectBurst(dorkaPage, hiddenPane.ptyId, hiddenTitle, marker)

    await expect
      .poll(() => getRuntimePaneTitle(dorkaPage, hiddenSnapshot.tabId, hiddenPane.numericPaneId), {
        timeout: 10_000,
        message: 'hidden OSC title did not update renderer-visible model state'
      })
      .toBe(hiddenTitle)
    await expect
      .poll(async () => (await getUnreadTerminalTabIds(dorkaPage)).includes(hiddenSnapshot.tabId), {
        timeout: 10_000,
