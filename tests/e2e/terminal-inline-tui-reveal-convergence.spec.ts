...
test.describe('Inline TUI reveal convergence', () => {
  test('hidden-but-mounted tab reveal converges while the inline TUI streams', async ({
    ...
    const setup = await startStreamingInlineTui(dorkaPage, testInfo)
    try {
      ...
      const tabBId = await createActiveTerminalTab(dorkaPage, setup.worktreeId)
      expect(tabBId).not.toBe(setup.tabId)
      await dorkaPage.waitForTimeout(Math.max(50, Math.min(PARKING_DELAY_MS / 2, 200)))
      expect(
        await isTerminalPaneMounted(dorkaPage, setup.tabId),
        'hidden-mounted scenario cold-parked before reveal'
      ).toBe(true)

      await activateTerminalTab(dorkaPage, setup.tabId)
      await waitForActiveTerminalManager(dorkaPage, 30_000)
      await assertRevealConvergence(dorkaPage, testInfo, setup, 'hidden-mounted-reveal')
    } finally {
      await setup.stop()
    }
  })
  ...
  test('worktree switch reveal converges after a hidden-time window resize', async ({
    ...
    try {
      ...
      const otherWorktreeId = await switchToOtherWorktree(dorkaPage, setup.worktreeId)
      ...
      await switchToWorktree(dorkaPage, setup.worktreeId)
      await activateTerminalTab(dorkaPage, setup.tabId)
      await waitForActiveTerminalManager(dorkaPage, 30_000)
      await assertRevealConvergence(dorkaPage, testInfo, setup, 'worktree-resize-reveal')
    } finally {
  ...
  test('parked tab reveal converges across repeated cycles while the inline TUI streams heavily', async ({
    ...
    try {
      ...
      const tabBId = await createActiveTerminalTab(dorkaPage, setup.worktreeId)
      const decoyTabId = await createActiveTerminalTab(dorkaPage, setup.worktreeId)
      ...
      for (let cycle = 0; cycle < CYCLES; cycle += 1) {
        ...
        await waitForTabParked(dorkaPage, setup.tabId, { parkDelayMs: PARKING_DELAY_MS })
        ...
        await withCpuThrottle(dorkaPage, 6, async () => {
          await activateTerminalTab(dorkaPage, setup.tabId)
          await waitForActiveTerminalManager(dorkaPage, 30_000)
        ...
        await assertRevealConvergence(dorkaPage, testInfo, setup, `parked-heavy-reveal-c${cycle}`)
      }
    } finally {
      await setup.stop()
    }
  ...
  test('rapid tab hide/reveal flapping never wedges delivery for the streaming inline TUI', async ({
    ...
    try {
      ...
      for (let flap = 0; flap < 12; flap += 1) {
        await activateTerminalTab(dorkaPage, tabBId)
        await dorkaPage.waitForTimeout(50 + (flap % 3) * 120)
        await activateTerminalTab(dorkaPage, setup.tabId)
        await dorkaPage.waitForTimeout(50 + ((flap * 7) % 5) * 90)
      }
      await waitForActiveTerminalManager(dorkaPage, 30_000)
      await assertRevealConvergence(dorkaPage, testInfo, setup, 'tab-flapping-reveal')
    } finally {
      await setup.stop()
    }
  })
    ...
    try {
      ...
      await withCpuThrottle(dorkaPage, 6, async () => {
        for (let flap = 0; flap < 10; flap += 1) {
          await switchToWorktree(dorkaPage, otherWorktreeId!)
          await dorkaPage.waitForTimeout(60 + (flap % 4) * 110)
          await switchToWorktree(dorkaPage, setup.worktreeId)
          await dorkaPage.waitForTimeout(60 + ((flap * 5) % 4) * 130)
        }
      })
      await activateTerminalTab(dorkaPage, setup.tabId)
      await waitForActiveTerminalManager(dorkaPage, 30_000)
      await assertRevealConvergence(dorkaPage, testInfo, setup, 'worktree-flapping-reveal')
    } finally {
      await setup.stop()
    }
  })
  ...
  test('parked tab reveal converges after a parked-time window resize', async ({
    ...
    try {
      await createActiveTerminalTab(dorkaPage, setup.worktreeId)
      await createActiveTerminalTab(dorkaPage, setup.worktreeId)
      await waitForTabParked(dorkaPage, setup.tabId, { parkDelayMs: PARKING_DELAY_MS })
      ...
      const revealed = await waitForPaneIdentitySnapshot(dorkaPage, 1)
      expect(revealed.panes[0]?.ptyId).toBe(setup.ptyId)
      await assertRevealConvergence(dorkaPage, testInfo, setup, 'parked-resize-reveal')
    } finally {
      await resizeAppWindow(electronApp, 180, 120).catch(() => {})
      await setup.stop()
    }
