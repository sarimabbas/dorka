...
const SETUP_SCRIPT_COMMAND = 'echo dorka-e2e-setup'
...
/** Repo whose shared dorka.yaml carries a real setup script, plus a second worktree. */
function createRepoWithSharedSetupScript(repoPath: string, featureWorktreePath: string): void {
  ...
  writeFileSync(path.join(repoPath, 'README.md'), '# Unreadable dorka.yaml E2E\n')
...
/**
 * Makes the main process report the failure the fix now surfaces: dorka.yaml could
 ...
 */
async function installUnreadableDorkaYamlFault(electronApp: ElectronApplication): Promise<void> {
  await electronApp.evaluate(({ ipcMain }) => {
    type InvokeHandler = (event: unknown, ...args: unknown[]) => unknown
    const faultState = globalThis as typeof globalThis & { __dorkaE2eDorkaYamlUnreadable?: boolean }
    ...
    faultState.__dorkaE2eDorkaYamlUnreadable = true
    ipcMain.removeHandler('hooks:check')
    ipcMain.handle('hooks:check', async (event, ...args) => {
      if (faultState.__dorkaE2eDorkaYamlUnreadable) {
...
/** dorka.yaml becomes readable again — every later check runs the production handler. */
async function healDorkaYamlRead(electronApp: ElectronApplication): Promise<void> {
  await electronApp.evaluate(() => {
    ;(
      globalThis as typeof globalThis & { __dorkaE2eDorkaYamlUnreadable?: boolean }
    ).__dorkaE2eDorkaYamlUnreadable = false
  })
}
...
test.describe('Setup script prompt', () => {
  ...
  test('recovers from an unreadable dorka.yaml instead of pinning the failed verdict', async ({
    electronApp,
    dorkaPage
  }, testInfo) => {
    const repoPath = testInfo.outputPath('unreadable-dorka-yaml-repo')
    const featureWorktreePath = testInfo.outputPath('unreadable-dorka-yaml-feature')
    createRepoWithSharedSetupScript(repoPath, featureWorktreePath)

    await installUnreadableDorkaYamlFault(electronApp)
    ...
    const promptCard = dorkaPage.locator('[data-setup-script-prompt-layer]')
    const inspectionError = promptCard.getByText(INSPECTION_ERROR_TEXT)
    await expect(inspectionError).toBeVisible({ timeout: 20_000 })

    // dorka.yaml is readable again; the card is still pinned to the failed verdict.
    await healDorkaYamlRead(electronApp)
    const healthyCheck = await dorkaPage.evaluate(
    ...
    await dorkaPage.waitForTimeout(RECORDING_DWELL_MS)

    // Activating another worktree in the same repo must re-inspect.
    const featureRow = worktreeRow(dorkaPage, featureWorktreeId)
    await expect(featureRow).toBeVisible()
    await worktreeRowSurface(dorkaPage, featureWorktreeId).click()
    await expect(featureRow).toHaveAttribute('aria-current', 'page')

    // The repo has a valid dorka.yaml scripts.setup, so no prompt may remain.
