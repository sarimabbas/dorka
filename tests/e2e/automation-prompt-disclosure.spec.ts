...
test('automation detail keeps short prompts readable and reveals a very long prompt at narrow width', async ({
  ...
  await dorkaPage.setViewportSize({ width: 820, height: 700 })
  ...
  await dorkaPage.getByRole('button', { name: new RegExp(`^${SHORT_NAME}`) }).click()
  await expect(dorkaPage.getByText('Synthetic short prompt.')).toBeVisible()
  await expect(dorkaPage.getByRole('button', { name: 'Show more' })).toHaveCount(0)

  await dorkaPage.getByRole('button', { name: 'All automations' }).click()
  await dorkaPage.getByRole('button', { name: new RegExp(`^${RESIZE_NAME}`) }).click()
  const resizePrompt = dorkaPage.getByText(RESIZE_PROMPT)
  const resizeToggle = dorkaPage.getByRole('button', { name: 'Show more' })
  await expect(resizeToggle).toBeVisible()
  await resizeToggle.focus()
  await dorkaPage.setViewportSize({ width: 1400, height: 700 })
  await expect(resizeToggle).toHaveCount(0)
  await expect(resizePrompt).toBeFocused()

  await dorkaPage.setViewportSize({ width: 820, height: 700 })
  await dorkaPage.getByRole('button', { name: 'All automations' }).click()
  await dorkaPage.getByRole('button', { name: new RegExp(`^${LONG_NAME}`) }).click()

  const prompt = dorkaPage.getByText(new RegExp(END_MARKER))
  const showMore = dorkaPage.getByRole('button', { name: 'Show more' })
  ...
  await expect(dorkaPage.getByRole('button', { name: 'Show less' })).toBeFocused()
