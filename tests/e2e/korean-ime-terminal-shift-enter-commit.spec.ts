  ...
  for (const chord of COMMITTING_ENTER_CHORDS) {
    for (const redispatchAfterKeyup of [false, true]) {
      ...
      test(`${chord.name} sends once with ${order}`, async ({
        ...
        const scriptPath = path.join(testRepoPath, `.dorka-korean-ime-harness-${runId}.cjs`)
        ...
        try {
          ...
          await installImeKeyEventLog(dorkaPage)
          ...
          await composeHangulSyllable(session, dorkaPage)
          await commitSyllableAndSpace(session, dorkaPage)
          await composeHangulSyllable(session, dorkaPage)
          await commitSyllableAndSpace(session, dorkaPage)
          ...
          await chord.assertOutcome(dorkaPage)
          await dispatchPlainEnter(session)
          await expect
            .poll(() => readReceived(dorkaPage), {
              timeout: 10_000,
              message: 'the next physical Enter must not be consumed by stale IME state'
            })
            .toBe(chord.expectedAfterPlainEnter.received)
          expect(await readSubmitted(dorkaPage)).toEqual(chord.expectedAfterPlainEnter.submitted)
          await attachEvidence(dorkaPage, testInfo, `korean-${chord.slug}-${order}-commit`)
        } finally {
          await attachEvidence(dorkaPage, testInfo, `korean-${chord.slug}-${order}-final`).catch(
            () => undefined
          )
