...
test('@golden stages and commits a file through Source Control', async ({
  ...
  await openGoldenSourceControl(dorkaPage, testRepoPath, fixture)

  const unstagedRow = dorkaPage
  ...
  await dorkaPage.getByRole('textbox', { name: 'Commit message' }).fill('test: golden daily loop')
  await dorkaPage.getByRole('button', { name: 'Commit', exact: true }).click()
  ...
  await expect(
    dorkaPage.locator('[data-sonner-toast]').filter({ hasText: /node|command not found|cmd\.exe/i })
  ).toHaveCount(0)
})
