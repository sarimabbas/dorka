...
test('mobile subscribe mounts overlay; collapse → chip; Take back dismisses', async ({
  ...
  const overlay = dorkaPage.locator('.mobile-driver-banner')
  await expect(overlay).toHaveCount(0)

  // Fire the IPC events main emits when a mobile client subscribes in 'auto'
  // mode (handleMobileSubscribe in src/main/runtime/dorka-runtime.ts). The
  ...
  await expectExpandedOverlayLeavesPaneReadable(dorkaPage, ptyId)
  ...
  await captureAttachment(dorkaPage, testInfo, 'overlay-loud.png')
  ...
  await expectChipIsCompactInPane(dorkaPage, ptyId)

  await captureAttachment(dorkaPage, testInfo, 'overlay-collapsed.png')
...
test('held phone-fit state mounts restore overlay without collapse', async ({
  ...
  await captureAttachment(dorkaPage, testInfo, 'overlay-held-fit.png')
...
test('restore this terminal refits the active restored pane', async ({ dorkaPage, electronApp }) => {
  await waitForSessionReady(dorkaPage)
  await waitForActiveWorktree(dorkaPage)
  await ensureTerminalVisible(dorkaPage)
  await waitForActiveTerminalManager(dorkaPage)
  ...
  await expect(dorkaPage.locator('.mobile-driver-banner')).toHaveCount(1, { timeout: 15_000 })
  await expect
    .poll(() => getPaneTerminalCols(dorkaPage, ptyId), {
...
test('restore all refits non-focused restored terminal panes', async ({
  ...
  const ptyIds = await waitForVisiblePanePtyIds(dorkaPage, 2)
  const focusPtyId = await waitForActivePanePtyId(dorkaPage)
  ...
  await expect(dorkaPage.locator('.mobile-driver-banner')).toHaveCount(2, { timeout: 15_000 })

  await forcePaneToOneColumn(dorkaPage, inactivePtyId)
  await expect
    .poll(() => getPaneTerminalCols(dorkaPage, inactivePtyId), {
...
test('restore all recovers a hidden workspace held at narrow terminal geometry', async ({
  ...
  const secondWorktreeId = (await getAllWorktreeIds(dorkaPage)).find(
  ...
  await ensureTerminalVisible(dorkaPage)
  await waitForActiveTerminalManager(dorkaPage)
  const hiddenWorkspacePtyId = await waitForActivePanePtyId(dorkaPage)
  await sendHeldPhoneFitIpc(electronApp, { ptyId: hiddenWorkspacePtyId, cols: 45, rows: 20 })
  await expect(dorkaPage.locator('.mobile-driver-banner')).toHaveCount(1, { timeout: 15_000 })

  await forcePaneToOneColumnAndSwitchWorktree(dorkaPage, hiddenWorkspacePtyId, secondWorktreeId)
  await expect
  ...
  await expect
    .poll(() => getPaneTerminalCols(dorkaPage, hiddenWorkspacePtyId), {
      message: 'test harness should hold workspace 1 in the bad narrow state'
    })
    .toBeLessThanOrEqual(2)
  await ensureTerminalVisible(dorkaPage)
  await waitForActiveTerminalManager(dorkaPage)
  const activeWorkspacePtyId = await waitForActivePanePtyId(dorkaPage)
  await installRestoreTerminalFitAutoRestoreRecorder(electronApp)
  await sendHeldPhoneFitIpc(electronApp, { ptyId: activeWorkspacePtyId, cols: 45, rows: 20 })
  await expect(
    dorkaPage.locator(`[data-pty-id="${activeWorkspacePtyId}"] .mobile-driver-banner`)
