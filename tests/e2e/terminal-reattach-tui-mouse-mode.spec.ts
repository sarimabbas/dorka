/**
 * Regression proof for #8291: a real alt-screen TUI survives an Dorka quit/relaunch, and after the
...
function createIsolatedProofRepo(): string {
  // Why realpathSync: macOS tmpdir symlinks through /private and Dorka canonicalizes repo.path.
  const repoDir = realpathSync(mkdtempSync(path.join(os.tmpdir(), 'dorka-mouse-reattach-repo-')))
  ...
  writeFileSync(path.join(repoDir, 'README.md'), '# Dorka mouse-mode reattach proof repo\n')
  git('add', '-A')
...
// Why: this suite quits and relaunches Dorka against one userDataDir, and the
...
test.describe('terminal reattach mouse mode', () => {
  ...
  {}, testInfo) => {
    ...
    try {
      ...
      // Why a screenshot and not the video fixture: this spec quits and relaunches Dorka,
      ...
      const proofShot = process.env.DORKA_E2E_PROOF_SCREENSHOT
      if (proofShot) {
