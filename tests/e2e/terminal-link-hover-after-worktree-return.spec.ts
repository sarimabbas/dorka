 ...
 * Dorka's file-path provider resolves link candidates asynchronously.
 */
...
test.describe('Terminal link hover after worktree return', () => {
  ...
  test('re-establishes a URL link on hover after the pointer leaves the terminal', async ({
    ...
    const url = `https://example.com/dorka-link-${randomUUID()}`
    await sendToTerminal(dorkaPage, ptyId, `echo ${url}\r`)
    await expect
      .poll(() => getTerminalContent(dorkaPage, 4000), {
        timeout: 10_000,
    ...
    const probe = await locateHoverProbe(dorkaPage, url)
    await expect
      .poll(() => hoverAndReadActiveLinkText(dorkaPage, probe), {
        timeout: 5_000,
        message: 'baseline hover never established the URL link'
      })
      .toContain(url)

    await dispatchScreenMouseLeave(dorkaPage, probe.tabId)
    await expect.poll(() => readActiveLinkText(dorkaPage, probe.tabId)).toBeNull()
    await expect.poll(() => readTerminalCursor(dorkaPage, probe.tabId)).not.toBe('pointer')
    ...
    await expect
      ...
      .toContain(url)
    await expect.poll(() => readTerminalCursor(dorkaPage, probe.tabId)).toBe('pointer')
  })
  ...
  test('re-establishes a file-path link on hover after switching worktrees and back', async ({
    ...
    const worktreePath = await activeWorktreePath(dorkaPage)
    const fileName = `dorka-linkfile-${randomUUID().slice(0, 8)}.txt`
    const filePath = path.join(worktreePath, fileName)
    writeFileSync(filePath, 'dorka file link target\n')
    const needle = `./${fileName}`

    try {
      await sendToTerminal(dorkaPage, ptyId, `echo ${needle}\r`)
      await expect
        .poll(() => getTerminalContent(dorkaPage, 4000), {
          timeout: 10_000,
      ...
      const probe = await assertLinkRecoversAfterReturn(dorkaPage, {
        firstWorktreeId,
        secondWorktreeId,
        needle,
        expectContains: fileName
      })
      await activateHoveredLink(dorkaPage, probe)
      ...
      await expect(dorkaPage.locator('.editor-header-path').first()).toContainText(fileName, {
        timeout: 20_000
      })
    } finally {
      await dorkaPage.evaluate((filePath) => {
        const state = window.__store?.getState()
  ...
  test('re-establishes a URL link on hover after switching worktrees and back', async ({
    ...
    await assertLinkRecoversAfterReturn(dorkaPage, {
      firstWorktreeId,
      secondWorktreeId,
      needle: url,
      expectContains: url
