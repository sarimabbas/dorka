...
test('@golden opens an unstaged file diff from Source Control', async ({
  ...
  await expect(dorkaPage.locator('.monaco-diff-editor')).toBeVisible({ timeout: 20_000 })
  await expect(
    dorkaPage
  ...
  await expect(dorkaPage.locator('.editor-header-path').first()).toHaveAttribute(
    'title',
    `${realpathSync(path.join(fixture.worktreePath, GOLDEN_CHANGED_PATH)).replaceAll('\\', '/')} (diff)`
  )

  const probe = dorkaPage.getByRole('button', { name: /Source Control/ })
  await probe.focus()
