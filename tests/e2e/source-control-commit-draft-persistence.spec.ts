...
test.describe('Source Control commit draft persistence', () => {
  test('preserves a typed draft when the sidebar tab remounts', async ({
    ...
    try {
      ...
      await openSourceControlForWorktree(dorkaPage, testRepoPath, firstWorktree.worktreePath)
      ...
      await expect
        .poll(
          async () => dorkaPage.evaluate(() => window.__store?.getState().rightSidebarTab ?? null),
          { timeout: 5_000 }
        )
        .toBe('explorer')
      ...
      await openSourceControlForWorktree(dorkaPage, testRepoPath, secondWorktree.worktreePath)
      await expect(textarea).toBeVisible({ timeout: 10_000 })
