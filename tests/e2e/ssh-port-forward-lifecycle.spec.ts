...
const FORCE_SYSTEM_SSH = process.env.DORKA_SSH_FORCE_SYSTEM_TRANSPORT === '1'
...
test.describe('Docker SSH port-forward lifecycle', () => {
  ...
  test('keeps a user-forwarded listener live across scan refresh @headful', async ({
    ...
    const marker = `DORKA_FORWARD_${Date.now()}`
    ...
    try {
      ...
      await openPortsPanel(dorkaPage)
      ...
      await expect
        ...
        .toBe(remotePid)
      await expect(dorkaPage.getByText(`:${REMOTE_PORT}`, { exact: true })).toBeVisible()
      await expect
        .poll(
          () =>
            dorkaPage.evaluate(
        ...
        .toBe(true)
      await expect(dorkaPage.getByText(`:${REFRESH_BARRIER_PORT}`, { exact: true })).toBeVisible()
      ...
      await waitForSessionReady(dorkaPage, 60_000)
      await expect
        .poll(() => waitForActiveWorktree(dorkaPage), { timeout: 60_000 })
      ...
      await installRendererForwardCapture(dorkaPage)
      await openPortsPanel(dorkaPage)
      await localPortReservation.release()
      await forwardPortFromPanel(dorkaPage, localPort, REMOTE_PORT)
      await expect(dorkaPage.getByText('Forwarded', { exact: true })).toBeVisible()
      await expect(
        dorkaPage.getByText(`:${localPort} → :${REMOTE_PORT}`, { exact: true })
      ).toBeVisible()
      await expect.poll(() => requestForward(localPort)).toContain(marker)
      await expectForwardEvidence(dorkaPage, remote.targetId, [
      ...
      const postHydrationRoundTripForwards = await dorkaPage.evaluate(
      ...
      await expect(dorkaPage.getByText(`:${REFRESH_BARRIER_PORT}`, { exact: true })).toBeVisible()
      const staleSnapshotEvidence = await readPortForwardEvidence(dorkaPage, remote.targetId)
      ...
      await expect
        ...
        .toBe(true)
      await expect(dorkaPage.getByText(`:${SCAN_REFRESH_PORT}`, { exact: true })).toBeVisible()

      await unrelatedLocalPortReservation.release()
      const unrelatedForward = await addPortForward(dorkaPage, {
      ...
      await forceDockerSshRelayChannelReconnect(dorkaPage, target, remote.targetId)
      await expectForwardEvidence(dorkaPage, remote.targetId, [
        { localPort, remotePort: REMOTE_PORT },
        { localPort: unrelatedLocalPort, remotePort: REFRESH_BARRIER_PORT }
      ])
      ...
      const authorityBeforeTransportReconnect = await dorkaPage.evaluate(
      ...
      await expect
        .poll(
          async () => {
            const state = await dorkaPage.evaluate(
              (targetId) => window.__store?.getState().sshConnectionStates.get(targetId),
              remote.targetId
            )
            return (
      ...
      await expect(
        dorkaPage.getByText(`:${unrelatedLocalPort} → :${REFRESH_BARRIER_PORT}`, { exact: true })
      ).toBeVisible()
      ...
      try {
        const collisionResult = await dorkaPage.evaluate(
      ...
      const evidence = await readPortForwardEvidence(dorkaPage, remote.targetId)
      ...
      const relayReconnectStates = await readSshStateCapture(dorkaPage)
