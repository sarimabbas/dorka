 ...
 *
 * The E2E app runs unpackaged, so its bundle identifier is Electron's own rather than Dorka's. That
...
test.describe('macOS press-and-hold default', () => {
  ...
  test('records a decision for the bundle it is actually running from', async ({ electronApp }) => {
    ...
    const ownsDomain =
      record.domain === 'com.stablyai.dorka' || record.domain!.startsWith('com.stablyai.dorka.')
    if (ownsDomain) {
      ...
      // The unpackaged harness. Anything but a refusal means Dorka wrote into a foreign domain.
