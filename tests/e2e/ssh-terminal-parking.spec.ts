...
test.describe('SSH terminal hidden view parking', () => {
  ...
  test('parks a hidden SSH tab and restores its scrollback on reveal', async ({
    ...
    try {
      ...
      const sshPtyId = await waitForActivePanePtyId(dorkaPage, 60_000)
      const sshTabId = await getActiveTabId(dorkaPage)
      if (!sshTabId) {
      ...
      await expect
        .poll(() => terminalTailContains(dorkaPage, `${marker}_200:`), {
          timeout: 30_000,
      ...
      await expect
        .poll(() => terminalTailContains(dorkaPage, `${marker}_PAD_DONE:`), {
          timeout: 60_000,
      ...
      await parkHiddenTabBehindDecoy(dorkaPage, remote.worktreeId, sshTabId, {
      ...
      await expect
        .poll(() => getTerminalContent(dorkaPage, 2_000_000), {
          timeout: 15_000,
