...
test.describe('Workspace board lane virtualization', () => {
  ...
  test('mounts a window of cards for a large lane and keeps lane indexes', async ({ dorkaPage }) => {
    ...
    await dorkaPage.getByRole('button', { name: 'Workspace board' }).click()

    const cards = dorkaPage.locator('[data-workspace-board-card-id]')
  ...
  test('renders later lane indexes after the lane scrolls', async ({ dorkaPage }) => {
    await dorkaPage.evaluate((count) => {
      const store = window.__store
      if (!store) {
        throw new Error('window.__store is not available')
  ...
  test('bounds mounted lanes and cards while preserving a 21-status workflow', async ({
    ...
    const board = dorkaPage.locator('[data-workspace-board-selection-surface]')
    ...
    await expect
      .poll(() => dorkaPage.evaluate(() => window.__store?.getState().workspaceBoardColumnWidth))
      .toBe(328)
  ...
  test('selects the full lane across a single large marquee scroll jump', async ({ dorkaPage }) => {
    ...
    const lane = dorkaPage.locator(`[data-workspace-status="${statusId}"]`)
    ...
    const emptyLaneScroll = dorkaPage.locator(
    ...
    await dorkaPage.mouse.move(startPoint.x, startPoint.y)
    await dorkaPage.mouse.down()
    await dorkaPage.mouse.move(box.x + box.width - 18, box.y + 80, { steps: 4 })
    ...
    await expect(dorkaPage.locator('[data-workspace-board-selection-rect]')).toBeVisible()
    await expect
    ...
    await dorkaPage.mouse.move(box.x + box.width - 18, box.y + box.height - 12)
    await dorkaPage.mouse.up()
    ...
    await expect(dorkaPage.getByText(/^\d+ selected$/)).toHaveText(
