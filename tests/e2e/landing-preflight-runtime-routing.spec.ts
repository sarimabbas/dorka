...
const missingGitPath = mkdtempSync(path.join(os.tmpdir(), 'dorka-preflight-path-'))
...
test('routes landing preflight across runtime switch and reconnect', async ({
  electronApp,
  dorkaPage
}, testInfo) => runRuntimePreflightJourney(electronApp, dorkaPage, testInfo, false))

test('routes landing preflight across runtime switch and reconnect @headful', async ({
  electronApp,
  dorkaPage
}, testInfo) => runRuntimePreflightJourney(electronApp, dorkaPage, testInfo, true))
