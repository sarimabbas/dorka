...
const FIXTURE = {
  ...
  webUrl: 'https://gitlab.example.test/acme/dorka/-/jobs/987654',
  mrUrl: 'https://gitlab.example.test/acme/dorka/-/merge_requests/4242',
...
const SCREENSHOT_DIR =
  process.env.DORKA_GITLAB_CHECKS_JOB_DETAILS_SCREENSHOT_DIR ??
...
test.describe('#7732 GitLab Checks panel job details', () => {
  test('expanding a failed pipeline job shows its log, not "No inline details"', async ({
    ...
    await dorkaPage.waitForTimeout(8_000)

    const jobRow = dorkaPage.getByText(`${FIXTURE.stage}: ${FIXTURE.jobName}`, { exact: true })
    for (let attempt = 0; attempt < 40 && (await jobRow.count()) === 0; attempt++) {
      await linkGitLabMRToWorktree(dorkaPage, worktreeId)
    ...
    await jobRow.click()
    const noDetails = dorkaPage.getByText('No inline details are available for this check.')
    const viewFullLogs = dorkaPage.getByRole('button', { name: 'View full logs' })
