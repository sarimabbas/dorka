...
test.describe('Diff note delete', () => {
  ...
  test('clicking the trash button removes the saved note', async ({ dorkaPage }) => {
    ...
    const deleteButton = dorkaPage.getByTitle('Delete note').first()
    ...
    await expect(dorkaPage.getByTitle('Delete note')).toHaveCount(0, {
      timeout: 5_000
    })
  })
})
