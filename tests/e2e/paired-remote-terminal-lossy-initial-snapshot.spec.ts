...
const scratch = mkdtempSync(path.join(os.tmpdir(), 'dorka-paired-lossy-snapshot-'))
...
test.use({
  dorkaAppExtraEnv: { DORKA_E2E_FORCE_REMOTE_TERMINAL_INITIAL_SNAPSHOT_TRUNCATED: '1' }
})
...
test('paints a nonempty lossy initial snapshot on a paired Electron client @headful', async ({
  ...
  try {
    ...
    }>(dorkaPage, 'session.tabs.createTerminal', {
