...
test.describe('Markdown ordered-list exit regression', () => {
  ...
  for (const row of rows) {
    test(row.name, async ({ dorkaPage }, testInfo) => {
      ...
      try {
        ...
        const activeFile = await openMarkdownFixture(dorkaPage, context, filePath)
        const draftKey = activeFile.filePath

        await row.run(dorkaPage, row.sentinel)

        await expectSentinelParagraphOutsideOrderedList(dorkaPage, row.sentinel)
        await expectSerializedDraftOutsideOrderedList(dorkaPage, draftKey, row.sentinel)
      } finally {
