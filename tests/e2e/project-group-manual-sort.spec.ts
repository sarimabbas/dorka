...
async function createProjectHeaderSortFixture(): Promise<string[]> {
  ...
  const root = realpathSync(await mkdtemp(path.join(os.tmpdir(), 'dorka-e2e-project-sort-')))
  tempRoots.push(root)
...
test.describe('Project Group manual sorting', () => {
  test('dragging a project header body reorders the visible project headers', async ({
    ...
    const projects = await seedProjectHeaderSortScenario(dorkaPage, repoPaths)

    await expect
      .poll(() => getProjectHeaderOrder(dorkaPage, projects), {
        timeout: 12_000,
  ...
  test('dragging a duplicate-ranked Project Group header reorders the visible headers', async ({
    ...
    await waitForSessionReady(dorkaPage)
    const groups = await seedDuplicateTabOrderProjectGroups(dorkaPage)

    await expect
      .poll(() => getProjectGroupHeaderOrder(dorkaPage, groups), {
        timeout: 12_000,
