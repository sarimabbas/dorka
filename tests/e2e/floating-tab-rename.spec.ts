...
test('concurrent floating Markdown renames do not clobber the destination', async ({
  ...
  const directory = await dorkaPage.evaluate(() => window.api.app.getFloatingMarkdownDirectory())
  const suffix = Date.now().toString(36)
  ...
  const result = await dorkaPage.evaluate(
...
test('Enter commits a floating Markdown rename only once', async ({ dorkaPage }) => {
  const seeded = await seedFloatingMarkdownFile(dorkaPage)
  await openFloatingPanel(dorkaPage)

  const panel = dorkaPage.locator(OPEN_PANEL_SELECTOR)
  const tab = panel.locator(`[data-tab-id="${seeded.tabId}"]`)
  await tab.click({ button: 'right' })
  await dorkaPage.getByRole('menuitem').filter({ hasText: 'Rename' }).first().click()
  ...
  await expect(dorkaPage.getByText(/Failed to rename/)).toHaveCount(0)
})

test('Electron serializes native Unicode rename aliases', async ({ dorkaPage }) => {
