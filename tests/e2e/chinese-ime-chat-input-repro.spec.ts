...
function emitState(reason) {
  ...
  process.stdout.write('\\x1b]1337;DorkaImeState=' + Buffer.from(JSON.stringify({
    reason,
    model,
    cursor,
    submitted
...
async function installImeEventProbe(page: Page): Promise<void> {
  await page.evaluate(() => {
    const targetWindow = window as unknown as { __dorkaImeEventLog?: ImeEventLogEntry[] }
    targetWindow.__dorkaImeEventLog = []
    ...
    const record = (event: Event): void => {
      ...
      targetWindow.__dorkaImeEventLog!.push({
        type: event.type,
        at: performance.now(),
...
async function readImeEventLog(page: Page): Promise<ImeEventLogEntry[]> {
  return page.evaluate(() => {
    const targetWindow = window as unknown as { __dorkaImeEventLog?: ImeEventLogEntry[] }
    return targetWindow.__dorkaImeEventLog ?? []
  })
}
...
async function dispatchSogouEmptyCompositionUpdate(page: Page): Promise<void> {
  ...
  // popup is still open (#6765); Dorka's tracker must not flip inactive on it.
  await page.evaluate(() => {
    const active = document.activeElement
    if (!(active instanceof HTMLTextAreaElement)) {
      throw new Error('xterm helper textarea is not focused')
...
async function dispatchSogouPostCompositionEnd(page: Page, data: string): Promise<void> {
  ...
  // compositionend; target the terminal element so Dorka's tracker sees the end
...
test.describe('Chinese IME terminal chat input repro', () => {
  test('keeps composed Chinese text, cursor movement, and Backspace stable in the agent input surface', async ({
    ...
    const scriptPath = path.join(testRepoPath, `.dorka-chinese-ime-harness-${runId}.cjs`)
    writeFileSync(scriptPath, terminalImeHarnessScript(runId))
    const session = await dorkaPage.context().newCDPSession(dorkaPage)

    try {
      await sendToTerminal(dorkaPage, ptyId, `node ${JSON.stringify(scriptPath)}\r`)
      await waitForTerminalOutput(dorkaPage, `IME_HARNESS_READY_${runId}`, 10_000, 20_000)
      await focusActiveTerminalInput(dorkaPage)
      await installImeEventProbe(dorkaPage)

      await dispatchImeProcessKey(session, 'KeyN')
      await composeAndCommitChineseText(session, dorkaPage, ['n', 'ni', '你', '你好'], '你好')
      await waitForLivePrompt(dorkaPage, '你好')
      await attachImeEvidence(dorkaPage, testInfo, 'after-compose-hello')
      await dorkaPage.keyboard.press('Enter')
      await expect
        .poll(async () => (await readPromptState(dorkaPage))?.submitted.at(-1) ?? null, {
          timeout: 5_000,
      ...
      await waitForLivePrompt(dorkaPage, '一二三四五六七八九十')
      for (let index = 0; index < 5; index += 1) {
        await dorkaPage.keyboard.press('ArrowLeft')
      }
      await dispatchImeProcessKey(session, 'KeyZ')
      await composeAndCommitChineseText(session, dorkaPage, ['z', 'zh', '中'], '中')
      await waitForLivePrompt(dorkaPage, '一二三四五中六七八九十')
      await attachImeEvidence(dorkaPage, testInfo, 'after-middle-insert')
      ...
      await dorkaPage.keyboard.press('Backspace')
      await waitForLivePrompt(dorkaPage, '一二三四五六七八九十')
      await attachImeEvidence(dorkaPage, testInfo, 'after-single-backspace')
      ...
      const log = await readImeEventLog(dorkaPage)
      expect(
      ...
      await attachImeEvidence(dorkaPage, testInfo, 'final-ime-evidence').catch(() => undefined)
  ...
  test('commits the active Pinyin preedit when Shift toggles the Windows IME', async ({
    ...
    await reloadWithWindowsImePolicy(dorkaPage)
    await waitForSessionReady(dorkaPage)
    await waitForActiveWorktree(dorkaPage)
    await ensureTerminalVisible(dorkaPage)
    await waitForActiveTerminalManager(dorkaPage, 30_000)
    ...
    const ptyId = await waitForActivePanePtyId(dorkaPage)
    const runId = randomUUID()
    const scriptPath = path.join(testRepoPath, `.dorka-windows-shift-ime-${runId}.cjs`)
    const inputLogPath = path.join(testRepoPath, `.dorka-windows-shift-ime-${runId}.jsonl`)
    ...
    try {
      ...
      await waitForLivePrompt(dorkaPage, 's')
      await expect.poll(() => readActiveCompositionText(dorkaPage)).toBe('')
      await expect.poll(async () => (await readPromptState(dorkaPage))?.submitted).toEqual([])
      await expect
        .poll(() => readPtyInputs(inputLogPath).slice(inputCountBeforeShift))
        .toEqual(['s'])
    } finally {
      await attachImeEvidence(dorkaPage, testInfo, 'windows-shift-ime-evidence').catch(
        () => undefined
      )
      await session.detach().catch(() => undefined)
      await sendToTerminal(dorkaPage, ptyId, '\x03').catch(() => undefined)
  ...
  test('keeps Sogou-style candidate selection keys out of the PTY while committing Chinese text', async ({
    ...
    await reloadWithLinuxImePolicy(dorkaPage)
    await waitForSessionReady(dorkaPage)
    await waitForActiveWorktree(dorkaPage)
    await ensureTerminalVisible(dorkaPage)
    await waitForActiveTerminalManager(dorkaPage, 30_000)
    ...
    const ptyId = await waitForActivePanePtyId(dorkaPage)
    const runId = randomUUID()
    const scriptPath = path.join(testRepoPath, `.dorka-sogou-ime-harness-${runId}.cjs`)
    ...
    try {
      ...
      writeFileSync(scriptPath, terminalImeHarnessScript(runId))
      session = await dorkaPage.context().newCDPSession(dorkaPage)
      await sendToTerminal(dorkaPage, ptyId, `node ${JSON.stringify(scriptPath)}\r`)
      harnessStarted = true
      await waitForTerminalOutput(dorkaPage, `IME_HARNESS_READY_${runId}`, 10_000, 20_000)
      await focusActiveTerminalInput(dorkaPage)
      ...
      await dorkaPage.waitForTimeout(80)
      await setImeComposition(session, 'ni')
      await dorkaPage.waitForTimeout(80)
      await dispatchSogouEmptyCompositionUpdate(dorkaPage)
      await dispatchCandidateSelectionKey(session, { key: ' ', code: 'Space', keyCode: 32 }, () =>
        commitImeText(session, '你')
      )
      await waitForLivePrompt(dorkaPage, '你')
      await attachImeEvidence(dorkaPage, testInfo, 'sogou-after-space-commit')
      await dorkaPage.keyboard.press('Enter')
      await expect
        .poll(async () => (await readPromptState(dorkaPage))?.submitted.at(-1) ?? null, {
          timeout: 5_000,
      ...
      await waitForLivePrompt(dorkaPage, '你好')
      await attachImeEvidence(dorkaPage, testInfo, 'sogou-after-digit-commit')
      await dorkaPage.keyboard.press('Enter')
      await expect
        .poll(async () => (await readPromptState(dorkaPage))?.submitted.at(-1) ?? null, {
          timeout: 5_000,
      ...
      const postCompositionLogStart = (await readImeEventLog(dorkaPage)).length
      await setImeComposition(session, 'zaijian')
      await dorkaPage.waitForTimeout(80)
      await dispatchSogouEmptyCompositionUpdate(dorkaPage)
      await dispatchSogouPostCompositionEnd(dorkaPage, '再见')
      await dispatchCandidateSelectionKey(session, { key: '3', code: 'Digit3', keyCode: 51 }, () =>
        commitImeText(session, '再见')
      )
      await waitForLivePrompt(dorkaPage, '再见')
      const postCompositionLog = await readImeEventLog(dorkaPage)
      ...
      await attachImeEvidence(dorkaPage, testInfo, 'sogou-after-post-composition-digit-commit')
      await dorkaPage.keyboard.press('Enter')
      await expect
        .poll(async () => (await readPromptState(dorkaPage))?.submitted.at(-1) ?? null, {
          timeout: 5_000,
      ...
      await expect
        ...
        .toBe('')
      await attachImeEvidence(dorkaPage, testInfo, 'sogou-after-orphan-keyup-digit-suppression')

      const promptState = await readPromptState(dorkaPage)
      expect(
        promptState?.submitted,
        'candidate Space/digit selectors and pinyin preedit must not leak into the PTY'
      ).toEqual(['你', '你好', '再见', ''])
    } finally {
      await attachImeEvidence(dorkaPage, testInfo, 'sogou-final-ime-evidence').catch(() => undefined)
      await session?.detach().catch(() => undefined)
      if (harnessStarted) {
        await sendToTerminal(dorkaPage, ptyId, '\x03').catch(() => undefined)
      }
  ...
  test('keeps composed Chinese text stable in the real Codex TUI input @real-codex-ime', async ({
    ...
    test.skip(
      process.env.DORKA_E2E_REAL_CODEX_IME !== '1',
      'Set DORKA_E2E_REAL_CODEX_IME=1 to exercise the locally installed Codex TUI'
    )
    ...
    try {
      await launchCodexTui(dorkaPage, ptyId)
      await installImeEventProbe(dorkaPage)

      await dispatchImeProcessKey(session, 'KeyN')
      await composeAndCommitChineseText(session, dorkaPage, ['n', 'ni', '你', '你好'], '你好')
      await waitForCleanTerminalText(dorkaPage, /你好/, 'Codex input did not show composed Chinese')
      await attachImeEvidence(dorkaPage, testInfo, 'codex-after-compose-hello', {
        cleanTerminal: stripTerminalControls(await getTerminalContent(dorkaPage, 20_000))
      })
      ...
      await attachImeEvidence(dorkaPage, testInfo, 'codex-after-composition-backspace', {
        cleanTerminal: stripTerminalControls(await getTerminalContent(dorkaPage, 20_000))
      })

      const cleanTerminal = stripTerminalControls(await getTerminalContent(dorkaPage, 20_000))
      expect(
        cleanTerminal,
      ...
      await attachImeEvidence(dorkaPage, testInfo, 'codex-final-ime-evidence', {
        cleanTerminal: stripTerminalControls(await getTerminalContent(dorkaPage, 20_000))
      }).catch(() => undefined)
      await session.detach().catch(() => undefined)
      await sendToTerminal(dorkaPage, ptyId, '\x03/quit\r').catch(() => undefined)
    }
  })
})
