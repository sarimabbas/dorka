...
test('sidebar keeps a Cursor pane visible and an OpenCode pane out of Claude Code hands', async ({
  ...
  await useFullAgentActivityRows(dorkaPage)

  const openCode = await openAgentTab(dorkaPage, worktreeId, 'opencode')
  ...
  await waitForTerminalOutput(dorkaPage, PANE_HOLD_MARKER, 15_000)
  ...
  await expect
    .poll(() => paneTitles(dorkaPage, openCode.tabId), {
      timeout: 15_000,
      message: 'the OpenCode task title never reached the renderer'
    })
    .toContain(OPENCODE_TASK_OSC_TITLE)

  const cursor = await openAgentTab(dorkaPage, worktreeId, 'cursor')
  ...
  const agentList = worktreeRow(dorkaPage, worktreeId).locator('[aria-label="Agents"]')
  ...
  expect(await settledSidebarAgentRowIdentities(dorkaPage, agentListSelector)).toEqual([
    'Cursor',
    'OpenCode'
  ])
