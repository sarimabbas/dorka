...
test('opens a terminal file link and observes an external edit @golden', async ({ dorkaPage }) => {
  ...
  const worktreePath = await dorkaPage.evaluate((id) => {
    return (
  ...
  try {
    ...
    await sendToTerminal(dorkaPage, ptyId, `${command}\r`)
    await expect
      .poll(() => getTerminalContent(dorkaPage, LINK_SCAN_CHAR_LIMIT), { timeout: 15_000 })
    ...
    await expect
      .poll(
        async () => {
          probe = await locateLink(dorkaPage, printedPath)
          return probe ? hoverLink(dorkaPage, probe) : null
        },
    ...
    await clickLink(dorkaPage, probe)

    const actionPopover = dorkaPage.locator('[data-terminal-link-action-popover]')
    ...
    const editorHeader = dorkaPage.locator('.editor-header-path').first()
      ...
      .poll(
        async () =>
          canonicalFileIdentity(
            (await dorkaPage.evaluate(() => window.__monacoEditorE2E?.filePath)) ?? ''
          ),
    ...
    await expect
      .poll(
        async () => {
          const snapshot = await dorkaPage.evaluate(() => window.__monacoEditorE2E?.snapshot())
          const reloadVisible = await dorkaPage
...
test('reuses a terminal file link already open in a sibling workspace @golden', async ({
  ...
  test.setTimeout(180_000)
  await waitForSessionReady(dorkaPage)
  const sourceWorktreeId = await waitForActiveWorktree(dorkaPage)
  const worktrees = await dorkaPage.evaluate((sourceId) => {
    const state = window.__store?.getState()
  ...
  await ensureTerminalVisible(dorkaPage)
  await waitForActiveTerminalManager(dorkaPage, 30_000)
  await dorkaPage.evaluate((sourceWorktreeId) => {
    const state = window.__store?.getState()
  ...
  await expect
    .poll(
      async () => {
        const rendered = await dorkaPage.evaluate(() => ({
  ...
  await expect(dorkaPage.getByText('Loading...', { exact: true })).toHaveCount(0)
})
