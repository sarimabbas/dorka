...
async function createBrowserFixture(
  ...
  const fixtureDir = mkdtempSync(path.join(os.tmpdir(), 'dorka-browser-recovery-'))
...
test('browser chrome recovers a live registered file guest after renderer loss', async ({
  ...
  await expect
    .poll(() => readBrowserGuestState(dorkaPage, browserTab.id), { timeout: 10_000 })
    .toMatchObject({
      chromePresent: true,
      marker: 'painted-file-guest',
      url: fixtureUrl
    })
  const before = await readBrowserGuestState(dorkaPage, browserTab.id)
  expect(before.webContentsId).not.toBeNull()
  const beforeProcessId = await readGuestProcessId(electronApp, before.webContentsId!)
  await expect
    .poll(
      async () =>
        (await listRegisteredBrowserPages(dorkaPage, worktreeId)).result?.tabs?.find(
  ...
  const recovered = await readBrowserGuestState(dorkaPage, browserTab.id)
  ...
  await expect
    .poll(() => listRegisteredBrowserPages(dorkaPage, worktreeId), { timeout: 10_000 })
    .toMatchObject({
      ok: true,
  ...
  await dorkaPage.evaluate((browserPageId) => {
    return window.api.browser.unregisterGuest({ browserPageId })
  }, browserTab.activePageId)
  await expect
    .poll(
      async () =>
        (await listRegisteredBrowserPages(dorkaPage, worktreeId)).result?.tabs?.some(
          (tab) => tab.browserPageId === browserTab.activePageId
        ) ?? false
    )
    .toBe(false)
  ...
  const resumeRecovered = await readBrowserGuestState(dorkaPage, browserTab.id)
  ...
  const backgroundTab = await dorkaPage.evaluate(
    ({ targetWorktreeId }) =>
  ...
  await expect
    .poll(() => readBrowserGuestState(dorkaPage, backgroundTab!.id), { timeout: 10_000 })
  ...
  await expect
    .poll(() => readBrowserGuestState(dorkaPage, browserTab.id), { timeout: 10_000 })
    .toMatchObject({ chromePresent: true, marker: 'painted-file-guest', url: fixtureUrl })
  const rendererReloaded = await readBrowserGuestState(dorkaPage, browserTab.id)
  expect(rendererReloaded.webContentsId).not.toBe(beforeRendererReloadId)

  await dorkaPage.evaluate((targetBrowserTabId) => {
    window.__store?.getState().setActiveBrowserTab(targetBrowserTabId)
  }, backgroundTab!.id)
  const hiddenBefore = await readBrowserGuestState(dorkaPage, browserTab.id)
...
test('dom-ready ID loss waits for validation without reloading the guest', async ({
  ...
  await expect
    .poll(() => readBrowserGuestState(dorkaPage, browserTab.id))
  ...
  await expect
    .poll(() => listRegisteredBrowserPages(dorkaPage, worktreeId))
    .toMatchObject({
      ok: true,
      result: { tabs: [{ browserPageId: browserTab.activePageId, url: fixtureUrl }] }
    })
...
test('explicit navigation repairs a recovery error without dom-ready churn', async ({
  ...
  await expect
    .poll(() => readBrowserPageRecoveryState(dorkaPage, browserTab.id, browserTab.activePageId))
  ...
  const addressBar = dorkaPage.locator(
    `[data-browser-overlay-tab-id="${browserTab.id}"] [data-dorka-browser-address-bar="true"]`
  )
  ...
  await expect(dorkaPage.getByText('Recovery fixture error', { exact: true })).toBeVisible()
  expect(
    await electronApp.evaluate(() => {
      const testState = globalThis as typeof globalThis & {
        browserRecoveryValidationCalls?: number
...
test('attachment keeps recovery error until document readiness', async ({
  ...
  await dorkaPage
    ...
    .nth(2)
    .click()
  const recoveryErrorCode = await dorkaPage.evaluate(
    ({ workspaceId, browserPageId }) =>
...
test('minimized browser guest stays painted and registered after restore @headful', async ({
  ...
  await occludeHeadedWindow(electronApp, dorkaPage)
  await revealHeadedWindow(electronApp, dorkaPage)

  await expect
    .poll(() => readBrowserGuestState(dorkaPage, browserTab.id))
    .toMatchObject({ chromePresent: true, marker: 'painted-file-guest', url: fixtureUrl })
  const restored = await readBrowserGuestState(dorkaPage, browserTab.id)
