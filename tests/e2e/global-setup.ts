...
export const E2E_TEST_REPO_PATH_FILE_ENV = 'DORKA_E2E_TEST_REPO_PATH_FILE'
...
export const TEST_REPO_PATH_FILE =
  ...
  path.join(os.tmpdir(), `dorka-e2e-test-repo-path-${randomUUID()}.txt`)
...
export default function globalSetup(): void {
  ...
  if (process.env.DORKA_E2E_WEB_CLIENT === '1') {
  ...
  if (
    ...
    (process.env.DORKA_E2E_SKILL_STAGING === '1' && Boolean(process.env.DORKA_E2E_SKILL_SSH_HOST))
  ) {
  ...
  const testRepoDir = realpathSync(mkdtempSync(path.join(os.tmpdir(), 'dorka-e2e-repo-')))
  ...
  writeFileSync(
    ...
    `${JSON.stringify({ name: 'dorka-e2e-test', version: '0.0.0', private: true }, null, 2)}\n`
  )
  ...
  const worktreeDir = path.join(testRepoDir, '..', `dorka-e2e-worktree-${randomUUID()}`)
