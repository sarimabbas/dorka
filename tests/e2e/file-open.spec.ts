...
test.describe('File Open & Markdown Preview', () => {
   ...
   * - you can open files (from the right sidebar)
   */
  test('opening the right sidebar shows file explorer', async ({ dorkaPage }) => {
    await openFileExplorer(dorkaPage)

    // Why: the load-bearing check is that `FileExplorer` actually mounted.
    // `data-dorka-explorer-shell` is the stable marker the component renders
    ...
    // painted empty.
    await expect(dorkaPage.locator('[data-dorka-explorer-shell]')).toBeVisible({ timeout: 5_000 })
  })
   ...
   * - you can open files (from the right sidebar)
   */
  test('clicking a file in the file explorer opens it in an editor tab', async ({ dorkaPage }) => {
    const worktreeId = (await getActiveWorktreeId(dorkaPage))!
    await openFileExplorer(dorkaPage)

    const filesBefore = await getOpenFiles(dorkaPage, worktreeId)

    // Click a known non-directory file
    const clickedFile = await clickFileInExplorer(dorkaPage, [
      'package.json',
      'tsconfig.json',
      '.gitignore',
      'README.md'
    ...
    await expect.poll(async () => getActiveTabType(dorkaPage), { timeout: 5_000 }).toBe('editor')

    // There should be a new open file
    await expect
      .poll(async () => (await getOpenFiles(dorkaPage, worktreeId)).length, { timeout: 5_000 })
    ...
    await expect(dorkaPage.locator('.editor-header-path').first()).toContainText(clickedFile!, {
      timeout: 20_000
    })
  })
   ...
   * - you can open .md files and they show up as preview (from the right sidebar)
   */
  test('opening a .md file shows markdown content', async ({ dorkaPage }) => {
    await openFileExplorer(dorkaPage)
    const clickedFile = await clickFileInExplorer(dorkaPage, ['README.md', 'CLAUDE.md'])
    expect(clickedFile).not.toBeNull()

    // Wait for the editor tab to become active
    await expect.poll(async () => getActiveTabType(dorkaPage), { timeout: 5_000 }).toBe('editor')

    // The seeded README.md starts with `# Dorka E2E Test Repo`, so the rich
    ...
    await expect(dorkaPage.getByRole('heading', { name: expectedHeading, level: 1 })).toBeVisible({
      timeout: 25_000
    })
  })
   ...
   */
  test('editor tab retains state when switching to terminal and back', async ({ dorkaPage }) => {
    const worktreeId = (await getActiveWorktreeId(dorkaPage))!
    await openFileExplorer(dorkaPage)
    ...
    const openFilesBefore = await getOpenFiles(dorkaPage, worktreeId)
