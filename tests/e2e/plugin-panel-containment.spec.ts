...
async function materializeHostilePlugin(origin: string): Promise<string> {
  const tempRoot = await mkdtemp(join(tmpdir(), 'dorka-hostile-panel-e2e-'))
...
test('contains hostile panel network and navigation probes', async ({
  ...
  dorkaPage.on('pageerror', (error) => {
    browserEvents.push(`pageerror:${error.message}`)
  })
  dorkaPage.on('framenavigated', (frame) => {
    browserEvents.push(`framenavigated:${frame.url()}`)
  })
  try {
    const panel = await installApprovedPanel(dorkaPage, pluginRoot)
    await openPanel(dorkaPage, panel)

    const iframe = dorkaPage.locator(`iframe[title="${panel.title}"]`)
    await expect(iframe).toHaveAttribute('sandbox', 'allow-scripts')
    const frame = dorkaPage.frameLocator(`iframe[title="${panel.title}"]`)
        ...
        new Promise<string>((resolve, reject) => {
          ...
          const onMessage = (event: MessageEvent): void => {
            ...
            if (
              ...
              !data ||
              data.type !== 'dorka-panel-action-result' ||
    ...
    expect(dorkaPage.url()).toBe(appUrl)
...
test('detects and suspends a busy-looping panel in an isolated renderer', async ({
  ...
  try {
    ...
    await iframe.evaluate((element) => {
      const panelWindow = (element as HTMLIFrameElement).contentWindow
      panelWindow?.postMessage({ type: 'dorka-hostile-busy-probe' }, '*')
    })

    await expect(
      dorkaPage.getByText('This plugin panel stopped responding and was suspended.')
    ).toBeVisible({ timeout: 20_000 })
    await expect(
      dorkaPage.getByRole('button', { name: new RegExp(`${panel.title}.*Error`) })
    ).toBeVisible()
    expect(dorkaPage.url()).toBe(appUrl)
    expect(server.requests).toEqual([])
  } finally {
