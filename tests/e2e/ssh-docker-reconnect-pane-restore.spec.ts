...
test.describe('SSH reconnect pane restore', () => {
  ...
  test('restores shell scrollback, a full-screen frame, and a usable new tab across a reconnect', async ({
    ...
    try {
      ...
      await execInTerminal(dorkaPage, ptyId, 'top -b -n 1 > /dev/null; top')
      await waitForTerminalOutput(dorkaPage, 'load average', 30_000, 8000)

      await reconnectDockerSshRelayTarget(dorkaPage, remote.targetId)
      await waitForActiveTerminalManager(dorkaPage, 60_000)
      await waitForActivePanePtyId(dorkaPage, 60_000)

      await waitForTerminalOutput(dorkaPage, 'load average', 60_000, 8000)
      const tuiContent = await getTerminalContent(dorkaPage, 8000)
      ...
      await openTerminalTabInActiveGroup(dorkaPage)
      await waitForActiveTerminalManager(dorkaPage, 60_000)
      const freshPtyId = await waitForActivePanePtyId(dorkaPage, 60_000)
      ...
      await execInTerminal(dorkaPage, freshPtyId, `echo ${freshMarker}`)
      await waitForTerminalOutput(dorkaPage, freshMarker, 60_000)

      // And it must be a FRESH shell, not a repaint of the old pane's history.
      const freshContent = await getTerminalContent(dorkaPage, 8000)
      ...
      // never printed a prompt for Dorka to read one from.
      await expect
        .poll(
          async () =>
            dorkaPage.evaluate(() => {
