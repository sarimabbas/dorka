...
test.describe('Terminal hidden view parking', () => {
  test('parks a hidden terminal tab and restores rich TUI output on reveal', async ({
    ...
    const setup = await setUpParkableTabA(dorkaPage)
    const { worktreeId, tabAId, tabAPtyId } = setup

    const runId = randomUUID()
    const finalMarker = `PARKED_RESTORE_FINAL_${runId}_${PARKED_FRAME_COUNT - 1}`
    const scriptPath = path.join(testRepoPath, `.dorka-parked-rich-tui-${runId}.mjs`)
    writeParkedFrameScript(scriptPath, runId)
    try {
      await sendToTerminal(dorkaPage, tabAPtyId, `node ${JSON.stringify(scriptPath)}\r`)
      await expect
        .poll(() => getTerminalContent(dorkaPage, 12_000), {
          timeout: 15_000,
          message: 'rich TUI final frame did not render while tab A was visible'
        })
        .toContain(finalMarker)

      const tabBId = await createActiveTerminalTab(dorkaPage, worktreeId)
      const parkDetectedAfterMs = await parkHiddenTabBehindDecoy(dorkaPage, worktreeId, tabAId, {
        parkDelayMs: PARKING_DELAY_MS
      })
      const wiring = await readParkingWiring(dorkaPage)
      ...
      const tabBState = await readTerminalTabViewState(dorkaPage, tabBId)
      expect(tabBState.hasManager).toBe(true)
      expect(tabBState.paneCount).toBeGreaterThan(0)

      await activateTerminalTab(dorkaPage, tabAId)
      await waitForActiveTerminalManager(dorkaPage, 30_000)
      const revealedSnapshot = await waitForPaneIdentitySnapshot(dorkaPage, 1)
      ...
      const content = await getTerminalContent(dorkaPage, 12_000)
      expect(content).toContain(`Frame ${String(PARKED_FRAME_COUNT - 1).padStart(3, '0')}`)
      expect(content).toContain('╭')
      expect(content).toContain('├')
      expect(content).toContain('█')
      expect(content).not.toContain('Dorka skipped hidden terminal output')
      ...
      await sendToTerminal(dorkaPage, tabAPtyId, '\x03')
      ...
      await waitForPtyShellEcho(dorkaPage, tabAPtyId, 15_000).catch((error: unknown) => {
        throw new Error(
      ...
      await runNodeScriptInTerminal(dorkaPage, tabAPtyId, typedProbeScript, {
        prefix: 'dorka-parked-typed-probe'
      })
      await expect
        .poll(() => getTerminalContent(dorkaPage, 12_000), {
          timeout: 10_000,
  ...
  test('keeps bell and title side effects live while parked', async ({ dorkaPage }) => {
    await waitForSessionReady(dorkaPage)
    const setup = await setUpParkableTabA(dorkaPage)
    const { worktreeId, tabAId, tabAPtyId } = setup

    await createActiveTerminalTab(dorkaPage, worktreeId)
    await parkHiddenTabBehindDecoy(dorkaPage, worktreeId, tabAId, {
      parkDelayMs: PARKING_DELAY_MS
    })
    ...
    // Why: delivered via a temp file — `node -e` quoting is not PowerShell-safe (#8521).
    await runNodeScriptInTerminal(dorkaPage, tabAPtyId, sideEffectScript, {
      prefix: 'dorka-parked-side-effect'
    })

    await expect
      .poll(() => getTerminalTabTitle(dorkaPage, worktreeId, tabAId), {
        timeout: 10_000,
        message: 'parked OSC 0 title did not update the tab title in the store'
      })
      .toBe(parkedTitle)
    await expect
      .poll(async () => (await getUnreadTerminalTabIds(dorkaPage)).includes(tabAId), {
        timeout: 10_000,
        message: 'parked BEL did not mark the terminal tab unread'
      })
      .toBe(true)
    await expect
      .poll(() => isWorktreeUnread(dorkaPage, worktreeId), {
        timeout: 10_000,
    ...
    expect((await readTerminalTabViewState(dorkaPage, tabAId)).hasManager).toBe(false)
  ...
  test('does not park excluded tabs', async ({ dorkaPage }) => {
    await waitForSessionReady(dorkaPage)
    const setup = await setUpParkableTabA(dorkaPage)
    ...
    const tabCId = await createActiveTerminalTab(dorkaPage, worktreeId)
    ...
    expect(await hasPendingStartupCommand(dorkaPage, tabCId)).toBe(true)

    // Tab B on top hides both A and C.
    const tabBId = await createActiveTerminalTab(dorkaPage, worktreeId)
    await expect
      .poll(() => getActiveTabId(dorkaPage), {
        timeout: 5_000,
    ...
    expect(await hasPendingStartupCommand(dorkaPage, tabCId)).toBe(true)
    const tabCState = await readTerminalTabViewState(dorkaPage, tabCId)
  ...
  test('reproduces a static frame byte-for-byte across 25 park/reveal cycles', async ({
    ...
    const scriptPath = path.join(testRepoPath, `.dorka-cycle-reference-${runId}.mjs`)
    ...
    try {
      ...
      const tabBId = await createActiveTerminalTab(dorkaPage, worktreeId)
      const decoyTabId = await createActiveTerminalTab(dorkaPage, worktreeId)
      ...
      const runOneParkRevealCycle = async (cycle: number): Promise<string[]> => {
        await activateTerminalTab(dorkaPage, tabBId)
        // Hide tab B behind the decoy so B (not A) holds the #8262 exemption.
        await activateTerminalTab(dorkaPage, decoyTabId)
        await waitForTabParked(dorkaPage, tabAId, { parkDelayMs: PARKING_DELAY_MS })
        await activateTerminalTab(dorkaPage, tabAId)
        await waitForActiveTerminalManager(dorkaPage, 30_000)
        const revealed = await waitForPaneIdentitySnapshot(dorkaPage, 1)
        ...
        await expect
          ...
          .toContain(marker)
        const rows = terminalContentRows(await getTerminalContent(dorkaPage, 12_000))
        // Garble sentinel: the hidden-skip banner must never appear.
        expect(rows.join('\n')).not.toContain('Dorka skipped hidden terminal output')
        return rows
      }
      ...
      for (let cycle = 1; cycle < CYCLES; cycle++) {
        ...
        await dorkaPage.waitForTimeout(PARK_VERDICT_BURST_SETTLE_MS)
