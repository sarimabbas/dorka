...
test('floating Mobile Emulator tab renders content and closes from the tab strip', async ({
  ...
  const tab = await seedFloatingSimulatorTab(dorkaPage)
  await openFloatingPanelIfNeeded(dorkaPage)

  const openPanel = dorkaPage.locator(OPEN_PANEL_SELECTOR).first()
  ...
  await attachPanelScreenshot(dorkaPage, testInfo)
