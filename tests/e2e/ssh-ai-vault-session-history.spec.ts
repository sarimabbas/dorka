...
test.describe('SSH Agent Session History', () => {
  ...
  test('shows remote session history only for the SSH host and resumes Codex on that worktree', async ({
    ...
    try {
      ...
      const scan = await dorkaPage.evaluate(
      ...
      expect(scan.remoteCommands.join('\n')).toContain(
        "CODEX_HOME='/root/.local/share/dorka/codex-runtime-home/home'"
      )

      const defaultSessionTitle = dorkaPage.getByText(defaultTitle, { exact: true })
      const runtimeSessionTitle = dorkaPage.getByText(runtimeTitle, { exact: true })

      await openAiVaultSidebar(dorkaPage)
      await expect(defaultSessionTitle.first()).toBeVisible({ timeout: 30_000 })

      const hostButton = dorkaPage.getByRole('button', { name: /Session History host:/ })
      await hostButton.click()
      await dorkaPage.getByRole('menuitemradio', { name: /Local/ }).click()
      await expect(defaultSessionTitle).toHaveCount(0, { timeout: 30_000 })

      await hostButton.click()
      await dorkaPage.getByRole('menuitemradio', { name: 'All hosts' }).click()
      ...
      await installStartupQueueProbe(dorkaPage)
      await defaultSessionTitle.first().click()
      await dorkaPage.getByText('Resume in Worktree', { exact: true }).click()

      await expect
        .poll(() => readLastQueuedStartupCommand(dorkaPage), { timeout: 30_000 })
        .toContain(`CODEX_HOME='/root/.codex' codex resume '${defaultSessionId}'`)
      const queuedWorktreeId = await readLastQueuedStartupWorktreeId(dorkaPage)
...
function seedRemoteAiVaultHistory(
  ...
  dockerExec(
    ...
    [
      ...
      'mkdir -p /root/.local/share/dorka/codex-runtime-home/home/sessions/2026/07/04',
      'mkdir -p /root/.claude/projects/dorka'
    ].join(' && ')
  )
  dockerWriteFile(
    target,
  ...
  dockerWriteFile(
    target,
    `/root/.local/share/dorka/codex-runtime-home/home/sessions/2026/07/04/${args.runtimeSessionId}.jsonl`,
    codexTranscript({
  ...
  dockerWriteFile(
    target,
    `/root/.claude/projects/dorka/${args.claudeSessionId}.jsonl`,
    claudeTranscript({
