...
test.describe('Browser Tab', () => {
   ...
   * - Browser works and also retains state when switching tabs etc.
   */
  test('creating a browser tab adds it and activates browser view', async ({ dorkaPage }) => {
    const worktreeId = (await getActiveWorktreeId(dorkaPage))!
    const browserTabsBefore = await getBrowserTabs(dorkaPage, worktreeId)

    await createBrowserTab(dorkaPage, worktreeId)

    // Wait for the browser tab to appear in the store
    await expect
      .poll(async () => (await getBrowserTabs(dorkaPage, worktreeId)).length, { timeout: 5_000 })
      .toBe(browserTabsBefore.length + 1)

    // The active tab type should switch to 'browser'
    await expect.poll(async () => getActiveTabType(dorkaPage), { timeout: 3_000 }).toBe('browser')
  })
   ...
   * - Browser works and also retains state when switching tabs etc.
   */
  test('browser tab is created and active in the store', async ({ dorkaPage }) => {
    const worktreeId = (await getActiveWorktreeId(dorkaPage))!

    await createBrowserTab(dorkaPage, worktreeId)
    await expect.poll(async () => getActiveTabType(dorkaPage), { timeout: 5_000 }).toBe('browser')

    // Verify the browser tab exists in the store
    const browserTabs = await getBrowserTabs(dorkaPage, worktreeId)
    ...
    const activeBrowserTabId = await dorkaPage.evaluate(() => {
      const store = window.__store
   ...
   * - Browser works and also retains state when switching tabs etc.
   */
  test('browser tab retains state when switching to terminal and back', async ({ dorkaPage }) => {
    const worktreeId = (await getActiveWorktreeId(dorkaPage))!
    ...
    await switchToTerminalTab(dorkaPage, worktreeId)
    await expect.poll(async () => getActiveTabType(dorkaPage), { timeout: 3_000 }).toBe('terminal')

    // Switch back to browser tab
    await switchToBrowserTab(dorkaPage, worktreeId, browserTabId!)
    await expect.poll(async () => getActiveTabType(dorkaPage), { timeout: 3_000 }).toBe('browser')

    // The browser tab should still exist with the same ID
    const browserTabsAfter = await getBrowserTabs(dorkaPage, worktreeId)
  ...
  test('browser webview form state survives switching between browser tabs', async ({
    ...
    try {
      ...
      await writeBrowserInputValue(dorkaPage, firstTab!.id, 'first typed value')
      ...
      await writeBrowserInputValue(dorkaPage, secondTab!.id, 'second typed value')
      ...
      await expect
        .poll(async () => readBrowserInputValue(dorkaPage, firstTab!.id), { timeout: 5_000 })
        .toBe('first typed value')

      await switchToBrowserTab(dorkaPage, worktreeId, secondTab!.id)
      await expect
        .poll(async () => readBrowserInputValue(dorkaPage, secondTab!.id), { timeout: 5_000 })
  ...
  test('browser page reload restores the configured 100% zoom', async ({ dorkaPage }) => {
    const formServer = await startBrowserFormServer()
    try {
      const worktreeId = (await getActiveWorktreeId(dorkaPage))!
      ...
      await expect
        .poll(async () => readBrowserInputValue(dorkaPage, browserTab!.id), { timeout: 5_000 })
        .not.toBeNull()

      const zoomLevels = await dorkaPage.evaluate(async (browserTabId) => {
  ...
  test('Cmd/Ctrl+0 resets a zoomed browser page to 100%', async ({ dorkaPage }) => {
    const formServer = await startBrowserFormServer()
    try {
      const worktreeId = (await getActiveWorktreeId(dorkaPage))!
      const browserTab = await createBrowserTab(
      ...
      await dorkaPage.evaluate(
        async ({ browserTabId, browserPageId, modifier }) => {
          ...
          window.dispatchEvent(
            new CustomEvent('dorka:browser-page-zoom', {
      ...
      await expect
        .poll(() =>
          dorkaPage.evaluate((browserTabId) => {
            const slot = document.querySelector(`[data-browser-overlay-tab-id="${browserTabId}"]`)
  ...
  test('reloading one browser tab does not adopt another tab zoom', async ({ dorkaPage }) => {
    ...
    try {
      ...
      for (const tab of [tabA, tabB]) {
        await expect
          .poll(async () => readBrowserInputValue(dorkaPage, tab!.id), { timeout: 5_000 })
          .not.toBeNull()
      }

      const levels = await dorkaPage.evaluate(
  ...
  test('new-tab link gestures follow Chrome foreground and background behavior', async ({
    ...
    try {
      ...
      const sourceTabLocator = dorkaPage.locator(`[data-tab-id="${sourceTab!.id}"]`)
      await clickBrowserLink(dorkaPage, sourceTab!.id, '#blank-link')
      await expectBrowserTabActive(dorkaPage, 'Blank target destination')
      await expect(sourceTabLocator).toContainText('Source page')
      await switchToBrowserTab(dorkaPage, worktreeId, sourceTab!.id)

      // Context-menu links keep the source visible until the new tab is selected.
      await clickBrowserLink(dorkaPage, sourceTab!.id, '#external-link', { button: 'right' })
      await dorkaPage
        .getByRole('menuitem', { name: 'Open Link In Dorka Browser', exact: true })
        .click()
      await expectBrowserTabOpenedInBackground(dorkaPage, sourceTab!.id, 'Linked destination')
      await clickBrowserLink(dorkaPage, sourceTab!.id, '#frame-link', {
        frameSelector: '#link-frame'
      })
      await expectBrowserTabActive(dorkaPage, 'Frame destination')
      await switchToBrowserTab(dorkaPage, worktreeId, sourceTab!.id)

      await clickBrowserLink(dorkaPage, sourceTab!.id, '#frame-modifier-link', {
        frameSelector: '#link-frame',
      ...
      await clickBrowserLink(dorkaPage, sourceTab!.id, '#frame-middle-link', {
        button: 'middle',
        frameSelector: '#link-frame'
      })
      await expectBrowserTabOpenedInBackground(dorkaPage, sourceTab!.id, 'Frame middle destination')

      await clickBrowserLink(dorkaPage, sourceTab!.id, '#modifier-link', {
        modifiers: process.platform === 'darwin' ? ['meta'] : ['control']
      })
      await expectBrowserTabOpenedInBackground(dorkaPage, sourceTab!.id, 'Modifier destination')

      await clickBrowserLink(dorkaPage, sourceTab!.id, '#frame-shift-middle-link', {
        button: 'middle',
        modifiers: ['shift'],
        frameSelector: '#link-frame'
      })
      await expectBrowserTabActive(dorkaPage, 'Frame shift middle destination')
      await switchToBrowserTab(dorkaPage, worktreeId, sourceTab!.id)

      const tabCountBeforeCancelledClick = await dorkaPage.locator('[data-tab-id]').count()
      await clickBrowserLink(dorkaPage, sourceTab!.id, '#cancelled-link')
      await expect(
        dorkaPage.locator('[data-tab-id]').filter({ hasText: 'Click handled in page' })
      ).toBeVisible({ timeout: 10_000 })
      await expect(dorkaPage.locator('[data-tab-id]')).toHaveCount(tabCountBeforeCancelledClick)

      await clickBrowserLink(dorkaPage, sourceTab!.id, '#middle-link', { button: 'middle' })
      await expectBrowserTabOpenedInBackground(dorkaPage, sourceTab!.id, 'Middle-click destination')
      await expect
  ...
  test('blocked window.close in a link-created tab does not break tab switching', async ({
    ...
    try {
      ...
      await clickBrowserLink(dorkaPage, sourceTab!.id, '#window-close-link')
      let closeTabId: string | null = null
      await expect
        .poll(async () => {
          const tabs = await getBrowserTabs(dorkaPage, worktreeId)
          closeTabId = tabs.find((tab) => tab.url === closeServer.url)?.id ?? null
          return closeTabId
        })
        .not.toBeNull()

      await dorkaPage.locator(`[data-tab-id="${neighboringTab!.id}"]`).click()
      await expect.poll(async () => getActiveTabType(dorkaPage), { timeout: 5_000 }).toBe('browser')
      await expect
        .poll(() => readBrowserWindowCloseStatus(dorkaPage, closeTabId!), { timeout: 5_000 })
  ...
  test('directly created browser tabs block window.close and remain usable', async ({
    ...
    try {
      ...
      await expect
        .poll(() => readBrowserWindowCloseStatus(dorkaPage, directTab!.id), { timeout: 5_000 })
        .toContain('window.close() was blocked')

      await expect
        .poll(
          () =>
            dorkaPage.evaluate(async (targetBrowserTabId) => {
   ...
   * - Browser works and also retains state when switching tabs etc.
   */
  test('browser tab retains state when switching worktrees and back', async ({ dorkaPage }) => {
