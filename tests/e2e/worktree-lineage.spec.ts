...
async function captureEvidence(page: Page, name: string, locator?: Locator): Promise<void> {
  if (process.env.DORKA_CAPTURE_EVIDENCE !== '1') {
    return
  }
...
test.describe('Worktree Lineage', () => {
  ...
  test('renders existing child lineage in the sidebar', async ({ dorkaPage }) => {
    const { parentId, childId } = await seedLineageScenario(dorkaPage)
    const parentRow = worktreeOption(dorkaPage, parentId)
    const childRow = worktreeOption(dorkaPage, childId)
    ...
    const positions = await dorkaPage.evaluate(
      ({ parentId, childId }) => {
    ...
    await dorkaPage.evaluate(async (childId) => {
  ...
  test('renders legacy-only inline lineage when side-map hydration is absent', async ({
    ...
    const { parentId, childId } = await seedLineageScenario(dorkaPage, { inlineOnly: true })
    const parentRow = worktreeOption(dorkaPage, parentId)
    const childRow = worktreeOption(dorkaPage, childId)
    ...
    await expect
      ...
      .toBe(true)
    await captureSidebarEvidence(dorkaPage, 'legacy-inline-lineage-nested.png')
  })
  ...
  test('updates nested child preview status when the child terminal sleeps', async ({
    ...
    const childTabId = await seedWorkspaceLiveTerminal(dorkaPage, childId)
    await expect(childRow).toContainText('Active')

    await markWorkspaceTerminalSlept(dorkaPage, { worktreeId: childId, tabId: childTabId })
  ...
  test('sleeps a workspace and every descendant from the parent context menu', async ({
    ...
    const { parentId, childId } = await seedLineageScenario(dorkaPage)
    await dorkaPage.evaluate((parentId) => {
      const store = window.__store
      if (!store) {
        throw new Error('window.__store is not available')
      }
    ...
    const parentTabId = await seedWorkspaceLiveTerminal(dorkaPage, parentId)
    const childTabId = await seedWorkspaceLiveTerminal(dorkaPage, childId)
    ...
    await worktreeOption(dorkaPage, parentId).click({ button: 'right' })
    const sleepSubtree = dorkaPage.getByRole('menuitem', {
      name: 'Sleep with Descendants (1)'
    })
    await expect(sleepSubtree).toBeVisible()
    await expect(sleepSubtree).toBeEnabled()
    await expect(dorkaPage.getByRole('menuitem', { name: 'Delete with Descendants…' })).toBeVisible()
    await captureEvidence(dorkaPage, 'workspace-descendant-actions.png')
  ...
  test('shows parent and child agent rows while the parent workspace is active', async ({
    ...
    const parentAgentPrompt = await seedWorkspaceAgentStatus(dorkaPage, parentId, 'PARENT')
    const childAgentPrompt = await seedWorkspaceAgentStatus(dorkaPage, childId, 'CHILD')
