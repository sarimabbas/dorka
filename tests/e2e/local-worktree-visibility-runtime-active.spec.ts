/**
 * Regression: a worktree created via the CLI (`dorka worktree
...
test.describe('worktree visibility with a remote runtime active', () => {
  test('a CLI-created worktree appears in the sidebar while a remote runtime is active', async ({
    ...
    const repoId = await dorkaPage.evaluate(() => {
    ...
    // dir — exactly what `dorka worktree create` does from a terminal.
    ...
    const worktreeRow = (worktreeId: string) =>
      dorkaPage.locator(`[data-worktree-id=${JSON.stringify(worktreeId)}]`).first()
