...
test('OMP model picker dispatches its advertised command and adopts the host report', async ({
  ...
  const picker = dorkaPage.getByRole('button', { name: 'Model Sonnet 4.5', exact: true })
  await expect(picker).toBeVisible()
  await picker.click()
  await expect(
    dorkaPage.getByRole('menuitemradio', { name: 'Sonnet 4.6', exact: true })
  ).toBeVisible()
  await dorkaPage.screenshot({
    animations: 'disabled',
    path: testInfo.outputPath('omp-model-choices.png')
  })
  await dorkaPage.getByRole('menuitemradio', { name: 'Sonnet 4.6', exact: true }).click()
  await expect
    .poll(async () => readFile(output, 'utf8').catch(() => ''))
    .toContain('/dorka-model anthropic/claude-sonnet-4-6')
  await dorkaPage.evaluate(({ paneKey, worktreeId }) => {
  ...
  await expect(
    dorkaPage.getByRole('button', { name: 'Model Sonnet 4.6', exact: true })
  ).toBeVisible()
  await dorkaPage.screenshot({
    animations: 'disabled',
