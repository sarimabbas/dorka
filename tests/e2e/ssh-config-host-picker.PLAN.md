...
secondary **Add all N to Dorka** action (no re-adopt). Settings → SSH → **Import**
...
| Search, result limit, suppressed aliases, alreadyInDorka  | `ssh-config-host-picker.test.ts`                     |
...
1. **Isolated HOME** — E2E already sets `HOME` to `{userDataDir}/home`. Seed config with:
   ...
   ```
2. **Unique aliases** — prefix Host entries and Dorka labels with
...
| **Expect** | Dialog title **Choose from ~/.ssh/config**; body **No hosts in ~/.ssh/config**; **Add all to Dorka** disabled; **Back** returns to form. |
...
| **Steps**  | Open picker.                                                                                                          |
| **Expect** | Host list `SSH config hosts` shows both aliases; subtitle `user@hostname:port`; button **Add all 2 to Dorka** enabled. |
...
### P5 — Already-in-Dorka badge + disabled row

|            |                                                                                                              |
| ---------- | ------------------------------------------------------------------------------------------------------------ |
| **Setup**  | Config hosts alpha + bravo. Seed Dorka target with `configHost`/`label` matching alpha (via `ssh.addTarget`). |
| **Steps**  | Open picker.                                                                                                 |
| **Expect** | Alpha shows **In Dorka** badge and is not clickable; bravo still selectable; **Add all 1 to Dorka** (not 2).   |

### P6 — Add all N to Dorka imports new hosts only

|            |                                                                                                                                     |
| ---------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| **Setup**  | Config with 2 new hosts; no Dorka targets for them.                                                                                  |
| **Steps**  | **Add all 2 to Dorka** → wait for success toast / return to form or list refresh.                                                    |
| **Expect** | Both targets exist (DOM in Settings SSH and/or listTargets); re-open picker shows **All hosts already in Dorka** / both **In Dorka**. |
...
| **Expect** | Deleted config host reappears as an Dorka target; toast sync count ≥ 1. |
...
- **N2** Case-insensitive alias: config `Prod`, existing label `prod` → **In Dorka**.
...
| P5 In Dorka badge / count              | `ssh-config-host-import.spec.ts` |
