...
test('shows Stop and a spinner during a held reload, then returns to Reload', async ({
  ...
  try {
    ...
    const tab = await dorkaPage.evaluate(
      ({ worktreeId, url }) => {
    ...
    const pane = dorkaPage.locator(`[data-browser-overlay-tab-id="${tab.id}"]`)
    const reload = pane.getByRole('button', { name: 'Reload' })
    await expect(reload).toBeVisible({ timeout: 20_000 })
    await reload.click()
    await secondRequest
    await expect
      .poll(async () => (await getBrowserTabs(dorkaPage, worktreeId)).at(-1)?.url, {
        timeout: 20_000
      })
      .toBe(url)
    ...
    await dorkaPage.screenshot({ path: testInfo.outputPath('reload-held.png') })
    reloadReleased = true
