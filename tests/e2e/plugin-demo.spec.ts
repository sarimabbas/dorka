/**
 * Invariant: the documented hello-dorka plugin stays inert before visible
...
async function openDemoPanel(page: Page): Promise<void> {
  ...
  const panelButton = page.getByRole('button', { name: 'Hello Dorka', exact: true })
  await expect(panelButton).toBeVisible({ timeout: 15_000 })
  await panelButton.click()
  const frame = page.frameLocator('iframe[title="Hello Dorka"]')
  await expect(frame.getByRole('heading', { name: 'Hello Dorka 👋' })).toBeVisible()
...
test('runs hello-dorka panel, command, and event behind visible consent', async ({ dorkaPage }) => {
  const tempRoot = await mkdtemp(join(tmpdir(), 'dorka-hello-plugin-e2e-'))
  const pluginRoot = join(tempRoot, 'hello-dorka')
  let createdWorktreeId: string | null = null
  await cp(join(process.cwd(), 'examples', 'plugins', 'hello-dorka'), pluginRoot, {
    recursive: true
  })

  try {
    const installed = await dorkaPage.evaluate(async (sourcePath) => {
    ...
    await openPluginSettings(dorkaPage)
    await dorkaPage.getByRole('tab', { name: /^Installed/ }).click()
    const row = dorkaPage.locator(`[data-plugin-key="${installed.pluginKey}"]`)
    await expect(row).toContainText('Needs review')
    await row.getByRole('button', { name: 'Review & enable' }).click()
    const consent = dorkaPage.getByRole('dialog', { name: 'Review permissions' })
    ...
    const commandResults = await dorkaPage.evaluate(async (pluginKey) => {
    ...
    await dorkaPage.evaluate(async (sourcePath) => {
      const settings = await window.api.settings.set({ devPluginPaths: [sourcePath] })
      window.__store?.setState({ settings })
      await window.api.plugins.refresh()
    }, pluginRoot)

    await openDemoPanel(dorkaPage)

    const panelPath = join(pluginRoot, 'panel.html')
    const panelHtml = await readFile(panelPath, 'utf8')
    await writeFile(panelPath, panelHtml.replace('Hello Dorka 👋', 'Hello Dorka reloaded'))
    await expect(
      dorkaPage.frameLocator('iframe[title="Hello Dorka"]').getByRole('heading', {
        name: 'Hello Dorka reloaded'
      })
    ).toBeVisible({ timeout: 15_000 })

    createdWorktreeId = await createWorktree(dorkaPage, `plugin-event-${Date.now()}`)
    await expect
      .poll(
        () =>
          dorkaPage.evaluate(
