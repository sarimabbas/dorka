...
test.describe('Source Control Create PR intent worktree switching', () => {
  ...
  test('keeps Create PR intent running after switching worktrees', async ({
        ...
        store.setState((current) => ({
          ...
          createHostedReview: async (repoPath, input) => {
            ...
            return {
              ok: true as const,
              number: 74,
              url: 'https://github.com/acme/dorka/pull/74'
            }
          },
          gitStatusByWorktree: {
    ...
    const completedWhileSwitchedEvidence = await dorkaPage.evaluate(() => {
  ...
  test('carries unavailable dirty intent through push to the final create preflight', async ({
    ...
    await waitForSessionReady(dorkaPage)
    await waitForActiveWorktree(dorkaPage)
    const { prWorktreeId, prWorktreePath } = await seedCreatePrComposer(dorkaPage)
    const remoteRoot = mkdtempSync(path.join(os.tmpdir(), 'dorka-e2e-create-pr-remote-'))
          ...
          repos: current.repos.map((repo) =>
            repo.id === worktree.repoId
              ? {
                  ...repo,
                  gitRemoteIdentity: {
                    canonicalKey: 'github.com/acme/dorka',
                    remoteName: 'origin',
                    remoteUrl: 'https://github.com/acme/dorka.git'
                  }
                }
              : repo
          ),
    ...
    await openSourceControl(dorkaPage, prWorktreeId)
    await expect(dorkaPage.getByText('e2e-commit-message-generation.txt')).toBeVisible({
      timeout: 10_000
    })
    await dorkaPage
    ...
    await expect
      ...
      .toBe(true)
    await expect(dorkaPage.getByText(finalCreateError)).toBeVisible({ timeout: 10_000 })
  })
})
