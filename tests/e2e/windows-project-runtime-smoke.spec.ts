...
test.describe('Windows project runtime smoke', () => {
  test('keeps a Windows-host project and WSL project available side by side', async ({
    ...
    const wsl = await dorkaPage.evaluate(async () => ({
    ...
    const wslRepoPath = await createGitRepo('dorka-e2e-project-runtime-', 'wsl-runtime-project')

    const smoke = await dorkaPage.evaluate(
    ...
    await openRepoSettings(dorkaPage, smoke.hostRepoId)
    const hostSection = dorkaPage.locator(`[data-settings-section="repo-${smoke.hostRepoId}"]`)
    ...
    await chooseProjectRuntime(dorkaPage, smoke.hostRepoId, 'WSL')
    await expect(
      hostSection.getByText(`This project runs in ${smoke.wslDistro} via WSL.`)
    ).toBeVisible()
    const hostAfterWslUiSwitch = await dorkaPage.evaluate((hostRepoId) => {
      const state = window.__store!.getState()
      const hostProject = state.projects.find((project) =>
    ...
    await chooseProjectRuntime(dorkaPage, smoke.hostRepoId, 'Windows')
    await expect(hostSection.getByText('This project runs on Windows.')).toBeVisible()
    const hostAfterWindowsUiSwitch = await dorkaPage.evaluate((hostRepoId) => {
      const state = window.__store!.getState()
      const hostProject = state.projects.find((project) =>
        project.sourceRepoIds.includes(hostRepoId)
      )
    ...
    await openRepoSettings(dorkaPage, smoke.wslRepoId)
    const wslSection = dorkaPage.locator(`[data-settings-section="repo-${smoke.wslRepoId}"]`)
    ...
    await dorkaPage.evaluate(async (repoId) => {
