...
test.describe('Settings sidebar search on the Shortcuts pane', () => {
  test('pane-title-only query keeps rows visible and local search usable', async ({ dorkaPage }) => {
    ...
    const searchInput = dorkaPage.getByPlaceholder('Search settings')
    ...
    await expect(dorkaPage.getByRole('heading', { name: 'Shortcuts', exact: true })).toBeVisible()

    // Regression: a pane-title-only query used to blank the whole list (0/112).
    await expect(dorkaPage.getByText('Go to File', { exact: true })).toBeVisible()
    await expect(dorkaPage.getByText('No shortcuts match those filters.')).not.toBeVisible()
    ...
    const localSearch = dorkaPage.getByPlaceholder('Search command or keys')
    await localSearch.fill('go to')
    await expect(dorkaPage.getByText('Go to File', { exact: true })).toBeVisible()
    await expect(dorkaPage.getByText('Force Reload', { exact: true })).not.toBeVisible()

    // A row-matching global query still narrows the list as before.
    await localSearch.clear()
    await searchInput.fill('worktree')
    await expect(dorkaPage.getByText('Create worktree', { exact: true })).toBeVisible()
    await expect(dorkaPage.getByText('Go to File', { exact: true })).not.toBeVisible()
  })
})
