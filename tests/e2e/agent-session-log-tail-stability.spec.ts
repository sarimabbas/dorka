...
test.describe('Agent Session History live log', () => {
  test.use({
    dorkaAppExtraArgs: ['--js-flags=--expose-gc', '--enable-precise-memory-info']
  })
  ...
  test('keeps a long View Log viewport stable while the transcript grows', async ({
    ...
    await dorkaPage.setViewportSize({ width: 520, height: 720 })
    await openSessionHistory(dorkaPage)
    const title = dorkaPage.getByText(fixture.title, { exact: true }).first()
    await expect(title).toBeVisible({ timeout: 30_000 })
    await title.click()
    await dorkaPage.getByText('View Log', { exact: true }).click()
    await expect
      .poll(() => readProbeOrNull(dorkaPage), { timeout: 120_000 })
      .toMatchObject({ filePath: fixture.filePath, valueLength: fixture.initialLength })
    await dorkaPage.setViewportSize({ width: 900, height: 720 })
    ...
    let baseline = await restoreAnchorState(dorkaPage, true)
    ...
    for (let batch = 0; batch < 3; batch++) {
      ...
      replacementMemory.push(await measureLegacyControl(electronApp, dorkaPage, baseline, batch))
      baseline = await restoreAnchorState(dorkaPage, baseline.find.open, equivalentState.scrollTop)
      ...
      await forceGcAndSettle(dorkaPage)
      const before = await readMemory(electronApp, dorkaPage)
      appendFileSync(fixture.filePath, suffix)
      fixture.initialLength += suffix.length
      await dorkaPage.waitForTimeout(APPEND_CADENCE_MS)
      await expect
        .poll(() => readProbe(dorkaPage), { timeout: 30_000 })
        .toMatchObject({ valueLength: fixture.initialLength })
      const after = await readMemory(electronApp, dorkaPage)
      await forceGcAndSettle(dorkaPage)
      const settled = await readMemory(electronApp, dorkaPage)
      suffixMemory.push({ before, after, settled })
      const current = await readProbe(dorkaPage)
      ...
      expect(await dorkaPage.evaluate(() => 2 + 2)).toBe(4)
      expect((await readMainProcessCrashProbe(electronApp)).processGone).toBeNull()
      baseline = current
      if (batch === 0) {
        await dorkaPage.keyboard.press('Escape')
        await expect(dorkaPage.locator('.monaco-editor .find-widget')).toHaveAttribute(
          'aria-hidden',
          'true'
        )
        baseline = await readProbe(dorkaPage)
