...
function codexLikeStartupCommand(marker: string): string {
  ...
  return stageNodeScriptForTerminal(script, { prefix: 'dorka-codex-startup-bg' }).command
}
...
test.describe('Codex hidden startup composer background', () => {
  test('restores the input background when a Codex worktree first becomes visible', async ({
    ...
    const hiddenTabId = await dorkaPage.evaluate(
    ...
    const hiddenPtyId = await waitForHiddenTabPtyId(dorkaPage, hiddenTabId)
    await expect
      .poll(() => mainSnapshotContains(dorkaPage, hiddenPtyId, marker), {
        timeout: 20_000,
    ...
    await expect
      .poll(
        async () => {
          try {
            const nextTarget = await readCodexStartupBackgroundTarget(dorkaPage, marker)
            target = nextTarget
    ...
    const visibleBackgroundPixels = await countVisibleBackgroundPixels(dorkaPage, target)
