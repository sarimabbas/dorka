...
test.describe('Pet status segment layout', () => {
  test('does not reserve empty space after its label', async ({ dorkaPage }, testInfo) => {
    await waitForSessionReady(dorkaPage)
    await enableExperimentalPet(dorkaPage)

    for (const { name, size } of VIEWPORTS) {
      await dorkaPage.setViewportSize(size)
      await assertPetTriggerFitsLabel(dorkaPage)
      await attachTriggerScreenshot(dorkaPage, testInfo, name)
    }
  })
})
