...
test.describe('terminal window-wake stale grid repro', () => {
  test('window focus heals a local PTY whose applied grid drifted from xterm', async ({
    ...
    const baseline = await readGridSnapshot(dorkaPage, ptyId)
    ...
    await expect
      .poll(async () => (await readGridSnapshot(dorkaPage, ptyId)).applied, { timeout: 10_000 })
      .toEqual(staleGrid)
    expect((await readGridSnapshot(dorkaPage, ptyId)).xterm).toEqual(baseline.xterm)
    ...
    await expect
      .poll(
        async () => {
          const snapshot = await readGridSnapshot(dorkaPage, ptyId)
