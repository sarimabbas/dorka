...
test.describe('PTY input write queue over SSH', () => {
  ...
  test('returns an xterm OSC query reply through the live SSH PTY', async ({
    ...
    try {
      ...
      await execInTerminal(dorkaPage, ptyId, `node -e ${shellQuote(remoteOscQueryScript(runId))}`)
      await waitForTerminalOutput(dorkaPage, `REMOTE_OSC_READY_${runId}`, 30_000, 80_000)
      await waitForTerminalOutput(dorkaPage, `REMOTE_OSC_REPLY_${runId}`, 30_000, 80_000)
    } finally {
  ...
  test('keeps fish query replies out of the next child stdin on an upstream relay pty', async ({
    ...
    try {
      ...
      const runId = String(Date.now())
      const home = `/tmp/dorka-fish-${runId}`
      const prompt = `DORKA_SSH_FISH_${runId}> `
      ...
      const relayExports = execDockerSshRelayTargetCommand(
        target,
        'module=$(find /root/.dorka-remote -type d -path \'*/node_modules/node-pty\' | head -n 1); node -e "const p=require(process.argv[1]); console.log(Object.keys(p.native || {}).join(\',\'))" "$module"'
      )
      ...
      await waitForTerminalOutput(dorkaPage, prompt, 30_000, 80_000)

      const blocker = `node -e ${shellQuote(`console.log('BLOCKER_STARTED_${runId}'); setTimeout(() => process.exit(0), 5000)`)}`
      await execInTerminal(dorkaPage, ptyId, blocker)
      await waitForTerminalOutput(dorkaPage, `BLOCKER_STARTED_${runId}`, 30_000, 80_000)
      await execInTerminal(dorkaPage, ptyId, `node ${shellQuote(childScript)}`)
      await waitForTerminalOutput(dorkaPage, childReady, 30_000, 80_000)
      await sendToTerminal(dorkaPage, ptyId, 'hello\r')
      await waitForTerminalOutput(dorkaPage, `${childRead}:"hello\\n"`, 30_000, 80_000)
