...
test.describe('Terminal inactive cursor rendering', () => {
  ...
  test('keeps an unfocused prompt cursor rendered as one block outline', async ({ dorkaPage }) => {
    await splitActiveTerminalPane(dorkaPage, 'vertical')
    await waitForPaneCount(dorkaPage, 2)
    await placeInactiveCursorAtPrompt(dorkaPage)

    const fixedBehavior = await renderInactiveCursor(dorkaPage)
    ...
    const oldBehavior = await renderInactiveCursor(dorkaPage, 'outline')
