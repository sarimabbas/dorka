...
test.describe('Codex terminal cursor jitter repro', () => {
  test('keeps queued-message cursor out of the Working status row in native Windows Codex @headful', async ({
    ...
    await installPtyWriteDiagnostics(dorkaPage)
    rmSync(ARTIFACT_DIR, { recursive: true, force: true })
    mkdirSync(ARTIFACT_DIR, { recursive: true })

    const shellCase = SHELL_CASES[0]!
    const { tabId, ptyId } = await prepareCodexTerminal(dorkaPage, shellCase)
    await installPtyOutputDiagnostics(dorkaPage)
    await dorkaPage.keyboard.type(CODEX_REPO_PROMPT)
    await dorkaPage.waitForTimeout(250)
    await dorkaPage.keyboard.press('Enter')
    await dorkaPage.waitForTimeout(1_000)
    if (!CODEX_WORKING_STATUS_RE.test(await getTerminalContentForTab(dorkaPage, tabId, 8_000))) {
      await dorkaPage.keyboard.press('Enter')
    }
    await dorkaPage.waitForTimeout(3_000)
    const submittedContent = await getTerminalContentForTab(dorkaPage, tabId, 8_000)
    writeFileSync(path.join(ARTIFACT_DIR, 'queued-message-after-submit.txt'), submittedContent)
    await expect
      .poll(
        async () =>
          CODEX_WORKING_STATUS_RE.test(await getTerminalContentForTab(dorkaPage, tabId, 8_000)),
        {
          timeout: 30_000,
      ...
      .toBe(true)
    await applyCursorProbeTheme(dorkaPage, tabId)
    ...
    await dorkaPage.keyboard.insertText('s')
    await expect
      .poll(
        async () =>
          (await readScreenLines(dorkaPage, tabId)).some((line) => isQueuedInputLine(line.text)),
        {
          timeout: 5_000,
    ...
    const snapshot = await readScreenSnapshot(dorkaPage, shellCase.label, tabId, ptyId)
    const rawChunks = await readPtyOutputDiagnostics(dorkaPage)
