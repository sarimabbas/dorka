...
test.describe('Docker SSH relay perf', () => {
  test.skip(!RUN_DOCKER_SSH, 'Set DORKA_E2E_SSH_DOCKER=1 to run Docker-backed SSH relay perf.')
  ...
  test('keeps remote typing responsive while the Linux relay streams TUI output', async ({
    ...
    try {
      ...
      await execInTerminal(dorkaPage, ptyId, `node -e ${shellQuote(remoteTypingLoadScript(runId))}`)
      await waitForTerminalOutput(dorkaPage, `REMOTE_TUI_READY_${runId}`, 30_000, 80_000)
      const measurement = await measureRemoteTyping(dorkaPage, ptyId, runId)
      ...
      await stopRemoteLoad(dorkaPage, ptyId)
    } finally {
  ...
  test('keeps active remote typing responsive while a background SSH PTY stream is ACK-stalled', async ({
    ...
    try {
      ...
      await ensureTerminalVisible(dorkaPage, 45_000)
      await waitForActiveTerminalManager(dorkaPage, 60_000)
      backgroundPtyId = await waitForActivePanePtyId(dorkaPage, 60_000)
      ...
      await waitForTerminalOutput(dorkaPage, `REMOTE_ACK_FLOOD_READY_${runId}`, 30_000, 80_000)
      await holdSshPtyAckGate(dorkaPage, [backgroundPtyId])
      await dorkaPage.evaluate((ptyId) => window.api.pty.write(ptyId, 'g'), backgroundPtyId)

      await splitActiveTerminalPane(dorkaPage, 'vertical')
      await focusLastTerminalPane(dorkaPage)
      activePtyId = await waitForActivePanePtyId(dorkaPage, 60_000)
      ...
      await waitForTerminalOutput(dorkaPage, `REMOTE_TUI_READY_${activeRunId}`, 30_000, 80_000)
      const heldAckPressure = expect.poll(
        async () => (await readSshPtyAckGate(dorkaPage))?.heldAckChars ?? 0,
        {
          timeout: 30_000,
      ...
      const measurement = await measureRemoteTyping(dorkaPage, activePtyId, activeRunId)
      const ackGate = await readSshPtyAckGate(dorkaPage)
      const summary = `median=${measurement.medianLatencyMs.toFixed(
        1
      ...
      await releaseSshPtyAckGate(dorkaPage)
      const releasedAckGate = await readSshPtyAckGate(dorkaPage)
      expect(releasedAckGate?.heldAckChars ?? 0).toBe(0)
    } finally {
      await releaseSshPtyAckGate(dorkaPage).catch(() => undefined)
      if (activePtyId) {
        await stopRemoteLoad(dorkaPage, activePtyId).catch(() => undefined)
      }
      if (backgroundPtyId) {
        await stopRemoteLoad(dorkaPage, backgroundPtyId).catch(() => undefined)
      }
      cleanupDockerSshRelayTarget(target)
    }
  })
  ...
  test('keeps remote typing responsive while relay file streams and git churn are active', async ({
    ...
    try {
      ...
      const loadFile = `/tmp/dorka-relay-load-${runId}.png`
      ...
      await waitForTerminalOutput(dorkaPage, `LOAD_FILES_READY_${runId}`, 60_000, 80_000)
      ...
      const measurement = await measureRemoteTyping(dorkaPage, ptyId, runId)
      const load = await dorkaPage.evaluate(() => {
        const state = (
  ...
  test('keeps an SSH workspace terminal usable after disconnect and reconnect', async ({
    ...
    try {
      ...
      await ensureTerminalVisible(dorkaPage, 45_000)
      await waitForActiveTerminalManager(dorkaPage, 60_000)
      const beforePtyId = await waitForActivePanePtyId(dorkaPage, 60_000)
      ...
      await execInTerminal(dorkaPage, beforePtyId, beforeCommand)
      await waitForTerminalOutput(dorkaPage, beforeMarker, 20_000, 60_000)
      ...
      await execInTerminal(dorkaPage, beforePtyId, recoveryCommand)
      await waitForTerminalOutput(dorkaPage, recoveryStartedMarker, 30_000, 80_000)

      await reconnectDockerSshRelayTarget(dorkaPage, remote.targetId)
      await ensureTerminalVisible(dorkaPage, 45_000)
      await waitForActiveTerminalManager(dorkaPage, 60_000)
      const afterPtyId = await waitForActivePanePtyId(dorkaPage, 60_000)
      await waitForTerminalOutput(dorkaPage, recoveryMarker, 30_000, 80_000)
      ...
      await execInTerminal(dorkaPage, afterPtyId, afterCommand)
      await waitForTerminalOutput(dorkaPage, afterMarker, 20_000, 60_000)
