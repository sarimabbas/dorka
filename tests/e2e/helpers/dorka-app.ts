/**
 * Shared Electron fixture for Dorka E2E tests.
 *
...
type DorkaTestFixtures = {
  ...
  dorkaAppExtraEnv: Record<string, string>
  ...
  // developer's shell or already-running Dorka instance.
  launchEnv: NodeJS.ProcessEnv
}

type DorkaWorkerFixtures = {
  /** Absolute path to the test git repo created by globalSetup. */
  testRepoPath: string
}

// Why: parse + warn at module scope so a bad DORKA_E2E_SLOWMO_MS value logs once
// per worker instead of once per test (otherwise hundreds of lines per CI run).
const DORKA_E2E_SLOWMO_MS_RAW = process.env.DORKA_E2E_SLOWMO_MS
const DORKA_E2E_SLOWMO_MS = ((): number => {
  if (DORKA_E2E_SLOWMO_MS_RAW === undefined) {
    return 0
  }
  const parsed = Number(DORKA_E2E_SLOWMO_MS_RAW)
  if (!Number.isFinite(parsed)) {
    console.warn(
      `[dorka-e2e] DORKA_E2E_SLOWMO_MS="${DORKA_E2E_SLOWMO_MS_RAW}" is not a number; ignoring (using 0).`
    )
    return 0
  }
...
function shouldLaunchHeadful(testInfo: TestInfo): boolean {
  // Why: DORKA_E2E_FORCE_HEADFUL lets a developer watch any spec in a real
  // window without retagging it `@headful` or switching projects.
  if (process.env.DORKA_E2E_FORCE_HEADFUL === '1') {
    return true
  }
  return testInfo.project.metadata.dorkaHeadful === true
}
...
// same DORKA_E2E_FORWARD_APP_LOGS-gated stdout/stderr capture.
...
/**
 * Extended Playwright test with Dorka-specific fixtures.
 *
 * `dorkaPage` — the main Dorka renderer window.
 *
 ...
 */
export const test = base.extend<DorkaTestFixtures, DorkaWorkerFixtures>({
  ...
  electronApp: async (
    ...
    const userDataDir = mkdtempSync(path.join(os.tmpdir(), 'dorka-e2e-userdata-'))
    ...
    // Dorka's own agent runtime) set it so Electron behaves as a plain Node
    ...
    const homeIsolation = createElectronHomeIsolation({
      ...
      launchEnv,
      extraEnv: dorkaAppExtraEnv,
      userDataDir
    })
    // Why: DORKA_E2E_SLOWMO_MS adds a pause between every Playwright action so a
    // developer running with DORKA_E2E_FORCE_HEADFUL=1 can actually watch what
    // the test does. Defaults to 0 (no slowdown) for normal runs.
    const slowMo = DORKA_E2E_SLOWMO_MS
    // Why: DORKA_E2E_RECORD_VIDEO=1 captures a webm of the renderer so a
    ...
    const recordVideoDir = process.env.DORKA_E2E_RECORD_VIDEO === '1' ? testInfo.outputDir : null
    if (recordVideoDir) {
      mkdirSync(recordVideoDir, { recursive: true })
    }
    const app = await electron.launch({
      args: [...dorkaAppExtraArgs, ...getDorkaElectronLaunchArgs(mainPath, headful)],
      ...(slowMo > 0 ? { slowMo } : {}),
      ...(recordVideoDir ? { recordVideo: { dir: recordVideoDir } } : {}),
      // Why: keep NODE_ENV=development so window.__store is exposed and
      // dev-only helpers activate. DORKA_E2E_USER_DATA_DIR overrides the usual
      // shared dev profile so every spec gets a clean persistence root.
      // Why: DORKA_E2E_HEADLESS suppresses mainWindow.show() so the app
      // window stays hidden during test runs, avoiding focus stealing and
      // screen clutter. Playwright interacts via CDP regardless.
      // Why: DORKA_E2E_HEADLESS suppresses mainWindow.show() for CI/headless
      // runs. DORKA_E2E_HEADFUL overrides this for tests that need a visible
      ...
      env: {
        ...
        ...((process.env.DORKA_E2E_SSH_LOCALHOST === '1' ||
          process.env.DORKA_E2E_SSH_DOCKER === '1' ||
          process.env.DORKA_E2E_NESTED_RUNTIME_SSH === '1') &&
        !cleanEnv.DORKA_RELAY_PATH
          ? { DORKA_RELAY_PATH: path.join(process.cwd(), 'out', 'relay') }
          : {}),
        ...(headful ? { DORKA_E2E_HEADFUL: '1' } : { DORKA_E2E_HEADLESS: '1' })
      }
    })
  ...
  dorkaAppExtraEnv: [{}, { option: true }],
  dorkaAppExtraArgs: [[], { option: true }],
  ...
  dorkaPage: async ({ sharedPage }, provideFixture) => {
