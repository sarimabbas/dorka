...
export type PairedQuickOpenLargeTreeFixture = {
  ...
  dorkaIgnoredTargetPath: string
  root: string
}

export function createPairedQuickOpenLargeTreeFixture(): PairedQuickOpenLargeTreeFixture {
  const root = mkdtempSync(path.join(os.tmpdir(), 'dorka-paired-quick-open-large-tree-'))
  try {
    execFileSync('git', ['init'], { cwd: root, stdio: 'pipe' })
    writeFileSync(path.join(root, '.gitignore'), 'data/\n')
    writeFileSync(path.join(root, '.dorkaignore'), 'dorka-ignored/\n')
    ...
    const dorkaIgnoredTargetPath = 'dorka-ignored/sta-4354-dorkaignore-target.ts'
    mkdirSync(path.join(root, 'dorka-ignored'))
    closeSync(openSync(path.join(root, ...dorkaIgnoredTargetPath.split('/')), 'w'))
    truncateSync(
      path.join(root, 'data', 'chunk-000000', `${LONG_STEM}000000.bin`),
      LARGE_TREE_NOMINAL_IGNORED_BYTES
    )

    execFileSync('git', ['add', '.gitignore', '.dorkaignore'], { cwd: root, stdio: 'pipe' })
    execFileSync(
      'git',
      [
        '-c',
        'user.name=Dorka E2E',
        '-c',
        'user.email=dorka-e2e@example.invalid',
        'commit',
        '-m',
