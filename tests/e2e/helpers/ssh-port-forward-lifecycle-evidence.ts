...
export function startRemoteHttpListener(
  ...
  execDockerSshRelayTargetCommand(
    target,
    [
      `nohup node -e ${shellQuote(script)} >/tmp/dorka-http-${port}.log 2>&1 < /dev/null &`,
      `echo $! >/tmp/dorka-http-${port}.pid`
    ].join(' ')
  )
  return Number(execDockerSshRelayTargetCommand(target, `cat /tmp/dorka-http-${port}.pid`))
}
...
export function readRemoteListenerIdentity(
  ...
  const pid = Number(execDockerSshRelayTargetCommand(target, `cat /tmp/dorka-http-${port}.pid`))
  return {
    pid,
