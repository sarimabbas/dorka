...
const TEMP_PREFIX = 'dorka-9749-dg-'
...
function knownDorkaUserDataDirs(): string[] {
  if (process.platform === 'darwin') {
    const appSupport = path.join(homedir(), 'Library', 'Application Support')
    return [path.join(appSupport, 'dorka'), path.join(appSupport, 'dorka-dev')]
  }
  if (process.platform === 'win32') {
    const roaming = process.env.APPDATA ?? path.join(homedir(), 'AppData', 'Roaming')
    return [path.join(roaming, 'dorka'), path.join(roaming, 'dorka-dev')]
  }
  const config = process.env.XDG_CONFIG_HOME ?? path.join(homedir(), '.config')
  return [path.join(config, 'dorka'), path.join(config, 'dorka-dev')]
}
...
function assertDisposableRoot(rootDir: string): void {
  ...
  for (const userDataDir of knownDorkaUserDataDirs()) {
    if (isEqualToOrInside(rootDir, userDataDir)) {
      throw new Error('Refusing daemon-generation fixture inside real Dorka user data')
    }
  }
}
