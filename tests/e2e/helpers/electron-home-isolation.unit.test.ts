...
function createUserDataDir(): string {
  const tempDir = mkdtempSync(path.join(os.tmpdir(), 'dorka-home-isolation-test-'))
  ...
  it('strips ambient home and Codex state before forcing a disposable home', () => {
    ...
    const isolation = createElectronHomeIsolation({
      inheritedEnv: {
        ...
        DORKA_CODEX_HOME: '/real/dorka-codex',
    ...
    expect(isolation.env.DORKA_CODEX_HOME).toBeUndefined()
  ...
  it('rejects generic fixture overlays that could escape the boundary', () => {
    ...
    expect(() =>
      createElectronHomeIsolation({
        inheritedEnv: {},
        launchEnv: {},
        extraEnv: { DORKA_E2E_USER_DATA_DIR: '/unsafe' },
        userDataDir: createUserDataDir(),
        realHome: '/real/home'
      })
    ).toThrow(/dorkaAppExtraEnv\.DORKA_E2E_USER_DATA_DIR/)
  })
