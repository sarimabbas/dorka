...
export async function ensureTextEditLaunched(): Promise<void> {
  await killTextEdit()
  textEditTempDir = await mkdtemp(join(tmpdir(), 'dorka-computer-e2e-'))
...
export async function ensureSafariDraftFixtureLaunched(): Promise<SafariDraftFixture> {
  ...
  safariDraftTempDir = await mkdtemp(join(tmpdir(), 'dorka-computer-safari-e2e-'))
  safariDraftTitle = `Dorka Computer Use Draft Fixture ${Date.now()}`
  ...
  if (title) {
    try {
      const envelope = parseJsonOutput<{
        ...
        (await runDorkaCli(['computer', 'list-windows', '--app', 'com.apple.Safari', '--json']))
          .stdout
      )
...
async function waitForComputerWindowTitle(
  ...
  while (Date.now() < deadline) {
    try {
      ...
      }>((await runDorkaCli(['computer', 'list-windows', '--app', app, '--json'])).stdout)
...
export async function ensureGeditLaunched(): Promise<void> {
  await killGedit()
  linuxTempDir = await mkdtemp(join(tmpdir(), 'dorka-computer-linux-e2e-'))
...
export async function ensureNotepadLaunched(): Promise<void> {
  await killNotepad()
  windowsTempDir = await mkdtemp(join(tmpdir(), 'dorka-computer-windows-e2e-'))
  const filePath = join(windowsTempDir, `dorka-notepad-${Date.now()}.txt`)
  await writeFile(filePath, 'seed', 'utf8')
