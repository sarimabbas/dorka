...
export const DOCKER_SSH_RELAY_REMOTE_REPO_PATH = '/tmp/dorka-docker-relay-perf-repo'
export const DOCKER_SSH_PROXY_JUMP_REMOTE_REPO_PATH = '/tmp/dorka-docker-proxy-jump-repo'
export const DOCKER_SSH_SECOND_HUB_REMOTE_REPO_PATH = '/tmp/dorka-docker-second-hub-repo'
...
function seedRemoteRepo(target: DockerSshRelayTarget, repoPath: string): void {
  ...
  execDockerSshRelayTargetCommand(
    ...
    [
      ...
      `printf '%s\\n' ${shellQuote(sentinel)} > .dorka-e2e-destination-id`,
      `printf '%s\\n' ${shellQuote(`remote relay ${sentinel}`)} > README.md`,
      'git add README.md .dorka-e2e-destination-id',
 ...
 * an all-comments `/root/.bashrc`, so its shell never emits OSC 0. Dorka derives a tab title from
...
export function startDockerSshRelayTarget(testInfo: TestInfo): DockerSshRelayTarget {
  const host = process.env.DORKA_E2E_SSH_TARGET_HOST?.trim() || '127.0.0.1'
  if (host === 'localhost' || host === '::1' || host.startsWith('127.')) {
    if (process.env.DORKA_E2E_SSH_TARGET_HOST) {
      throw new Error(`DORKA_E2E_SSH_TARGET_HOST must be non-loopback: ${host}`)
    }
  }
  const bindHost = host === '127.0.0.1' ? host : '0.0.0.0'
  const tempDir = mkdtempSync(path.join(os.tmpdir(), 'dorka-ssh-docker-'))
  ...
  const containerName = `dorka-ssh-e2e-${testInfo.workerIndex}-${Date.now()}-${randomUUID().slice(0, 8)}`
    ...
    run(
      ...
      [
        ...
        [
          ...
          'git config --global user.name "Dorka Docker SSH E2E"',
