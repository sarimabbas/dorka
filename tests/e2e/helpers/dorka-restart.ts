 ...
 *
 * Why: the default `dorkaPage` fixture creates a fresh `userDataDir` per test
...
type RestartSession = {
  ...
  launch: (options?: LaunchOptions) => Promise<LaunchedDorka>
...
export function createRestartSession(
  ...
  const userDataDir = mkdtempSync(path.join(os.tmpdir(), 'dorka-e2e-restart-'))
  ...
  const launch = async (options?: LaunchOptions): Promise<LaunchedDorka> => {
    runtimeWsPort ??= await reserveRestartRuntimeWsPort()
    const app = await electron.launch({
      args: getDorkaElectronLaunchArgs(mainPath, headful),
      env: {
        ...homeIsolation.env,
        ...options?.extraEnv,
        DORKA_E2E_RUNTIME_WS_PORT: String(runtimeWsPort)
      }
    })
  ...
  const dispose = async (): Promise<void> => {
    ...
    if (process.env.DORKA_E2E_PRESERVE_RESTART_PROFILE === '1') {
      ...
      () =>
        readRestartRendererState(() =>
          page.evaluate(async (repoId) => {
            ...
            // worktree is external to Dorka's workspace root after the visibility rollout.
