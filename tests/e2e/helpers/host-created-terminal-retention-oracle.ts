 ...
 *
 * Every retention spec (paired desktop host, windowless `dorka serve`, local
...
export function createRetentionFixtureDirectory(): string {
  return mkdtempSync(path.join(os.tmpdir(), 'dorka-host-terminal-retention-'))
}
...
/**
 * `dorka terminal create`: the host-initiated background create. Sibling paths
