...
export async function waitForRestoredTerminalInputReady(
  ...
  while (Date.now() < deadline) {
    const marker = `DORKA_RESTORED_INPUT_READY_${randomUUID().replaceAll('-', '')}`
        ...
        ({ expectedPtyId, input, pendingAttempts }) => {
          for (const manager of window.__paneManagers?.values() ?? []) {
            ...
            const container = pane.container as HTMLElement & {
              __dorkaE2eTerminalInputReadinessInstanceId?: string
            }
            container.__dorkaE2eTerminalInputReadinessInstanceId ??= crypto.randomUUID()
            const paneInstanceId = container.__dorkaE2eTerminalInputReadinessInstanceId
