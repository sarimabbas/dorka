  ...
  const child = fork(
    ...
    ],
    {
      ...
      env: {
        ...
        DORKA_USER_DATA_PATH: runtime.userDataDir
      },
...
export async function spawnGenerationCanary(options: {
  ...
  const sessionId = `${worktreeId}@@dorka-9749-${label}-${randomUUID().slice(0, 8)}`
  ...
  await waitForCondition(`${label} canary readiness`, () =>
    output.includes(`DORKA_GENERATION_CANARY_READY ${label} ${nonce}`)
  )
  const match = new RegExp(`DORKA_GENERATION_CANARY_READY ${label} ${nonce} (\\d+)`).exec(output)
...
export async function pingGenerationCanary(canary: GenerationCanary, nonce: string): Promise<void> {
  ...
  const expected = `DORKA_GENERATION_CANARY_ACK ${label} ${nonce}`
