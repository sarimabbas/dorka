 ...
 * - PR #677 (`return to Dorka landing screen after closing last terminal`)
...
test.describe('Tab Close Navigation', () => {
   ...
   */
  test('closing the active editor tab activates its visual neighbor', async ({ dorkaPage }) => {
    const worktreeId = await waitForActiveWorktree(dorkaPage)

    const fileIds = await openSeededEditorTabs(dorkaPage, worktreeId, [
      'package.json',
      'README.md',
      'tsconfig.json'
    ])
    ...
    await setActiveFile(dorkaPage, fileIds[1])
    await expect.poll(async () => getActiveFileId(dorkaPage), { timeout: 3_000 }).toBe(fileIds[1])

    await closeFile(dorkaPage, fileIds[1])

    const openFilesAfter = await getOpenFiles(dorkaPage, worktreeId)
    ...
    await expect
      .poll(async () => getActiveFileId(dorkaPage), {
        timeout: 5_000,
    ...
    await expect.poll(async () => getActiveTabType(dorkaPage), { timeout: 3_000 }).toBe('editor')
  })
   ...
   */
  test('closing the active diff tab activates a still-open neighbor', async ({ dorkaPage }) => {
    const worktreeId = await waitForActiveWorktree(dorkaPage)

    // Seed two editor tabs + one diff tab in the same worktree.
    const editorIds = await openSeededEditorTabs(dorkaPage, worktreeId, [
      'package.json',
      'README.md'
    ])
    expect(editorIds.length).toBe(2)

    const diffId = await dorkaPage.evaluate((wId) => {
      const store = window.__store
      if (!store) {
        return null
      }
    ...
    await expect.poll(async () => getActiveFileId(dorkaPage), { timeout: 3_000 }).toBe(diffId)

    await closeFile(dorkaPage, diffId!)
   ...
   */
  test('closing the last visible surface returns the app to Landing', async ({ dorkaPage }) => {
    const worktreeId = await waitForActiveWorktree(dorkaPage)
    ...
    const editorIds = await openSeededEditorTabs(dorkaPage, worktreeId, ['package.json'])
    expect(editorIds.length).toBe(1)

    await setActiveFile(dorkaPage, editorIds[0])
    await expect.poll(async () => getActiveFileId(dorkaPage), { timeout: 3_000 }).toBe(editorIds[0])
    ...
    await closeFile(dorkaPage, editorIds[0])
