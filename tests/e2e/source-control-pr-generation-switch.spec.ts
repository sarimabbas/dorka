...
test.describe('Source Control AI PR generation worktree switching', () => {
  ...
  test('keeps checks-panel PR generation running after switching worktrees', async ({
    ...
    const { primaryWorktreeId, prWorktreeId, prWorktreePath, primaryBranch } =
      await seedCreatePrComposer(dorkaPage)
    ...
    await installDelayedPrGenerator(dorkaPage, generatorScriptPath, callLogPath, primaryBranch)

    await openChecks(dorkaPage, prWorktreeId)
    ...
    await expect(
      dorkaPage.getByRole('button', { name: 'Stop generating pull request details' })
    ).toBeVisible()
    ...
    await openChecks(dorkaPage, primaryWorktreeId)
    await expect(dorkaPage.getByText('Generated PR title after switch')).toHaveCount(0)
    await dorkaPage.screenshot({
    ...
    await openChecks(dorkaPage, prWorktreeId)
    await expect(dorkaPage.getByRole('textbox', { name: 'Pull request title' })).toHaveValue(
      'Generated PR title after switch',
      { timeout: 10_000 }
    )
    await expect(dorkaPage.getByRole('textbox', { name: 'Pull request description' })).toHaveValue(
  ...
  test('keeps pending PR generation attached to its original worktree', async ({
    ...
    await expect
      ...
      .toContain('start')
    const pendingEvidence = await dorkaPage.evaluate(() => {
    ...
    await openSourceControl(dorkaPage, primaryWorktreeId)
    await expect(dorkaPage.getByText('Generated PR title after switch')).toHaveCount(0)
    const switchedEvidence = await dorkaPage.evaluate(() => {
      const state = window.__store?.getState()
      return {
        activeWorktreeId: state?.activeWorktreeId,
    ...
    await expect
      ...
      .toContain('finish')
    await waitForPrGenerationStored(dorkaPage, prWorktreeId)
    await openSourceControl(dorkaPage, prWorktreeId)
    await waitForPrGenerationHydrated(dorkaPage, prWorktreeId)
    await expect(dorkaPage.getByRole('textbox', { name: 'Pull request title' })).toHaveValue(
      'Generated PR title after switch',
      { timeout: 10_000 }
    )
    ...
    const finalEvidence = await dorkaPage.evaluate(() => {
      const state = window.__store?.getState()
      return {
        activeWorktreeId: state?.activeWorktreeId,
  ...
  test('keeps pending commit message generation attached to its original worktree', async ({
    ...
    const { primaryWorktreeId, commitWorktreeId, commitWorktreePath } =
      await seedCommitMessageComposer(dorkaPage)
    ...
    await installDelayedCommitMessageGenerator(dorkaPage, generatorScriptPath, callLogPath)

    await openSourceControl(dorkaPage, commitWorktreeId)
    await expect(
      dorkaPage
    ...
    await generate.click()
    await expect(
      dorkaPage.getByRole('button', { name: 'Stop generating commit message' })
    ).toBeVisible()
    await expect
      .poll(() => {
        return readLog(callLogPath)
    ...
    await openSourceControl(dorkaPage, primaryWorktreeId)
    await expect(dorkaPage.getByText('Generated commit message after switch')).toHaveCount(0)
    await expect(
      dorkaPage.getByRole('button', { name: 'Stop generating commit message' })
    ).toHaveCount(0)
    const switchedEvidence = await dorkaPage.evaluate(() => {
    ...
    await expect
      .poll(() => readFileSync(callLogPath, 'utf8'), { timeout: 10_000 })
      .toContain('finish')
    await waitForCommitGenerationStored(dorkaPage, commitWorktreeId)
    await openSourceControl(dorkaPage, commitWorktreeId)
    await waitForCommitGenerationHydrated(dorkaPage, commitWorktreeId)
    await expect(dorkaPage.getByRole('textbox', { name: 'Commit message' })).toHaveValue(
      'Generated commit message after switch\n\nGenerated from staged e2e-commit-message-generation.txt after switching worktrees',
      { timeout: 10_000 }
    )
    const finalEvidence = await dorkaPage.evaluate(() => {
  ...
  test('hydrates pending commit message generation after Source Control remounts', async ({
    ...
    await waitForSessionReady(dorkaPage)
    await waitForActiveWorktree(dorkaPage)
    const { commitWorktreeId, commitWorktreePath } = await seedCommitMessageComposer(dorkaPage)
    createStagedCommitMessageChange(commitWorktreePath)
  ...
  test('hides the commit AI composer on a clean branch empty state', async ({
    ...
    await waitForSessionReady(dorkaPage)
    await waitForActiveWorktree(dorkaPage)
    const primaryWorktreeId = await seedCleanBranchEmptyState(dorkaPage)
    ...
    await expect
      .poll(
        async () => {
          ...
          await seedCleanBranchEmptyState(dorkaPage, primaryWorktreeId)
