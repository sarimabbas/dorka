...
test.describe('Terminal foreground redraw freeze repro', () => {
  test('@headful Codex-style line rewrites request a visible row refresh', async ({
    ...
    const webglAttached = await forceActivePaneWebglRenderer(dorkaPage)
    ...
    await installActivePaneRefreshProbe(dorkaPage)
    try {
      const refreshBaseline = await readRefreshProbe(dorkaPage)
      await resetSchedulerDebug(dorkaPage)
      const measurement = await measureRendererDuringRewriteBurst(dorkaPage, paneKey)
      const scheduler = await readSchedulerDebug(dorkaPage)
      ...
      await expect
        .poll(
          async () => {
            const refresh = await readRefreshProbe(dorkaPage)
      ...
      await disposeActivePaneRefreshProbe(dorkaPage)
    }
  })
  ...
  test('active OpenTUI-style redraw bursts do not monopolize the renderer', async ({
    ...
    await waitForTerminalPtyDataInjector(dorkaPage, paneKey)
    await resetSchedulerDebug(dorkaPage)
    const measurement = await measureRendererDuringBurst(dorkaPage, paneKey)
    const scheduler = await readSchedulerDebug(dorkaPage)
  ...
  test('captured OpenCode/OpenTUI redraw bytes do not monopolize foreground writes', async ({
    ...
    await waitForTerminalPtyDataInjector(dorkaPage, paneKey)
    await resetSchedulerDebug(dorkaPage)
    const measurement = await measureRendererDuringFrames(dorkaPage, paneKey, frames)
    const scheduler = await readSchedulerDebug(dorkaPage)
    annotateMeasurement(testInfo, measurement, scheduler)
