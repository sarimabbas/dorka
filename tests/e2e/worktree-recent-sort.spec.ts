...
test.describe('Worktree Recent Sort', () => {
  ...
  function createFolderFixture(): string {
    const dir = mkdtempSync(path.join(os.tmpdir(), 'dorka-e2e-folder-'))
  ...
  test.afterEach(() => {
    ...
    // matches the mkdtempSync/rmSync pairing used in helpers/dorka-app.ts
    // and helpers/dorka-restart.ts.
  ...
  test('stamps lastActivityAt on a newly-added folder repo so it sorts to the top of Recent', async ({
    ...
    const repoId = await addFolderRepo(dorkaPage, folderPath)
    const lastActivityAt = await readFolderWorktreeLastActivity(dorkaPage, repoId)
  ...
  test('leaves lastActivityAt stable across repeated list refreshes', async ({ dorkaPage }) => {
    ...
    const first = await readFolderWorktreeLastActivity(dorkaPage, repoId)

    await dorkaPage.evaluate(async (id) => {
      await window.__store?.getState().fetchWorktrees(id)
      await window.__store?.getState().fetchWorktrees(id)
    }, repoId)

    const second = await readFolderWorktreeLastActivity(dorkaPage, repoId)
