...
>({
  ...
  await deps.waitForSessionReady(dorkaPage)
  await deps.waitForActiveWorktree(dorkaPage)
  const panes = await deps.ensureActiveWorktreePaneLoad(dorkaPage, backgroundPaneCount + 1)
  const [typingPane, ...loadPanes] = panes
  await deps.focusPane(dorkaPage, typingPane.paneKey)

  const runId = randomUUID()
  const scrollRunId = randomUUID()
  const typingScriptPath = path.join(testRepoPath, `.dorka-opencode-pressure-typing-${runId}.mjs`)
  const pressureScriptPath = path.join(testRepoPath, `.dorka-opencode-pressure-load-${runId}.mjs`)
  await seedActiveTerminalScrollback(dorkaPage, typingPane.ptyId, scrollRunId)
  ...
  try {
    ...
    const pressureBeforeTyping = await deps.waitForMainPtyPressureBacklog(dorkaPage)
    ...
    const mainPressure = await deps.readMainPtyPressureDebug(dorkaPage)
    ...
    deps.annotateTypingMeasurement(
      ...
      panes.length,
      measurement,
      await deps.readTerminalPtyOutputDebug(dorkaPage),
      scheduler,
      mainPressure,
      ackGate
    )
...
>({
  ...
  const scrollMeasurement = await measureActiveTerminalWheelScroll(dorkaPage)
  const mainPressureAfterScroll = await deps.readMainPtyPressureDebug(dorkaPage)
  const ackGateAfterScroll = await deps.readTerminalAckGateDebug(dorkaPage)
  annotateScrollMeasurement(
    testInfo,
  ...
  await scrollActiveTerminalToBottom(dorkaPage)
}
