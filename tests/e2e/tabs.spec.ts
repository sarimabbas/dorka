...
test.describe('Tabs', () => {
  test.beforeEach(async ({ dorkaPage }) => {
    ...
    await waitForStartupWorktreeRefresh(dorkaPage)
    await waitForActiveWorktree(dorkaPage)
    await ensureTerminalVisible(dorkaPage)
    const initialTabId = (await getActiveTabId(dorkaPage))!
    await expect(tabLocator(dorkaPage, initialTabId)).toBeVisible()
  })
   ...
   */
  test('clicking "+" then "New Terminal" creates a new terminal tab', async ({ dorkaPage }) => {
    ...
    const newTerminalMenuItem = dorkaPage.getByRole('menuitem', { name: /New Terminal/i }).first()
    ...
    await expect
      .poll(() => countRenderedTabs(dorkaPage), {
        timeout: 5_000,
    ...
    const storeActiveId = await getActiveTabId(dorkaPage)
    expect(storeActiveId).not.toBeNull()
    await expect(tabLocator(dorkaPage, storeActiveId!)).toBeVisible()
    await expect.poll(() => getDomActiveTabId(dorkaPage), { timeout: 3_000 }).toBe(storeActiveId)
    await expect
      .poll(() => getFocusedTerminalTabId(dorkaPage), {
        timeout: 5_000,
  ...
  test('clicking "+" then "New Markdown" focuses the editor', async ({
    ...
    const preExistingFileIds = await dorkaPage.evaluate(
      () => window.__store?.getState().openFiles.map((file) => file.id) ?? []
    )

    await dorkaPage.getByRole('button', { name: 'New tab' }).click({ force: true })
    const newMarkdownMenuItem = dorkaPage.getByRole('menuitem', { name: /New Markdown/i }).first()
    ...
    const createdFileHandle = await dorkaPage.waitForFunction(
      (knownFileIds) => {
    ...
    await dorkaPage.evaluate((fileId) => {
   ...
   * - New tab works
   */
  test('Cmd/Ctrl+T creates a new terminal tab', async ({ dorkaPage }) => {
    ...
    await dorkaPage.evaluate(() => document.body.focus())
    await dorkaPage.keyboard.press(`${mod}+t`)
   ...
   * right tab element.
   */
  test('Cmd/Ctrl+Shift+] and Cmd/Ctrl+Shift+[ switch between tabs', async ({ dorkaPage }) => {
    ...
    if ((await countRenderedTabs(dorkaPage)) < 2) {
      await dorkaPage.getByRole('button', { name: 'New tab' }).click()
      await dorkaPage
        .getByRole('menuitem', { name: /New Terminal/i })
        .first()
        .click()
      await expect
        .poll(() => countRenderedTabs(dorkaPage), { timeout: 5_000 })
        .toBeGreaterThanOrEqual(2)
    }

    const firstTabId = await getActiveTabId(dorkaPage)
    const orderedTabs = await getWorktreeTabs(dorkaPage, worktreeId)
    ...
    await expect.poll(() => getDomActiveTabId(dorkaPage), { timeout: 3_000 }).toBe(secondTabId)

    // Switch back.
    await dorkaPage.evaluate((tabId) => {
      window.__store?.getState().setActiveTab(tabId)
    }, firstTabId)
    await expect.poll(() => getDomActiveTabId(dorkaPage), { timeout: 3_000 }).toBe(firstTabId)
  })
   ...
   */
  test('dragging a tab to a new position reorders it', async ({ dorkaPage }) => {
    const worktreeId = (await getActiveWorktreeId(dorkaPage))!
    ...
    const domOrderBefore = await dorkaPage.$$eval(SORTABLE_TAB, (nodes) =>
      nodes.map((n) => (n as HTMLElement).dataset.tabId ?? '')
    )
    expect(domOrderBefore.length).toBeGreaterThanOrEqual(2)

    await dorkaPage.evaluate((targetWorktreeId) => {
    ...
    await expect
      .poll(
        async () =>
          dorkaPage.$$eval(SORTABLE_TAB, (nodes) =>
            nodes.map((n) => (n as HTMLElement).dataset.tabId ?? '')
          ),
  ...
  test('clicking tabs still switches after dragging a terminal tab to reorder', async ({
    ...
    await tabLocator(dorkaPage, firstTabId).click({ force: true })
    await expect.poll(() => getDomActiveTabId(dorkaPage), { timeout: 3_000 }).toBe(firstTabId)

    const firstTabBox = await tabLocator(dorkaPage, firstTabId).boundingBox()
    const secondTabBox = await tabLocator(dorkaPage, secondTabId).boundingBox()
    ...
    await dorkaPage.mouse.move(endX, endY, { steps: 8 })
    ...
    await tabLocator(dorkaPage, firstTabId).click({ force: true })
    await expect.poll(() => getDomActiveTabId(dorkaPage), { timeout: 3_000 }).toBe(firstTabId)
    await tabLocator(dorkaPage, secondTabId).click({ force: true })
    await expect
      .poll(() => getDomActiveTabId(dorkaPage), {
        timeout: 5_000,
   ...
   */
  test('Cmd/Ctrl+Shift+[ walks tabs in drag-reordered order', async ({ dorkaPage }) => {
    const isMac = process.platform === 'darwin'
    const mod = isMac ? 'Meta' : 'Control'
    const worktreeId = (await getActiveWorktreeId(dorkaPage))!
    ...
    await expect
      .poll(async () => (await getWorktreeTabs(dorkaPage, worktreeId)).length, {
        timeout: 5_000
      })
      .toBeGreaterThanOrEqual(3)

    const initialOrder = await getTabBarOrder(dorkaPage, worktreeId)
    ...
    await expect
      .poll(async () => getTabBarOrder(dorkaPage, worktreeId), {
        timeout: 3_000
      })
      .toEqual([b, c, a])
    ...
    await expect.poll(() => getDomActiveTabId(dorkaPage), { timeout: 3_000 }).toBe(a)

    await dorkaPage.keyboard.press(`${mod}+Shift+BracketLeft`)
    await expect.poll(() => getDomActiveTabId(dorkaPage), { timeout: 3_000 }).toBe(c)
    await expect(tabLocator(dorkaPage, c)).toHaveAttribute('data-active', 'true')

    await dorkaPage.keyboard.press(`${mod}+Shift+BracketLeft`)
    await expect.poll(() => getDomActiveTabId(dorkaPage), { timeout: 3_000 }).toBe(b)
    await expect(tabLocator(dorkaPage, b)).toHaveAttribute('data-active', 'true')
  })
   ...
   * re-render.
   */
  test('closing a tab removes it from the tab bar', async ({ dorkaPage }) => {
    const worktreeId = (await getActiveWorktreeId(dorkaPage))!
    ...
    const tabsBefore = await countRenderedTabs(dorkaPage)
    const activeId = await getActiveTabId(dorkaPage)
    expect(activeId).not.toBeNull()
    await closeTabFromTabBar(dorkaPage, activeId!)
   ...
   */
  test('closing the active tab activates a neighbor tab', async ({ dorkaPage }) => {
    const worktreeId = (await getActiveWorktreeId(dorkaPage))!
    ...
    const activeTabBefore = await getActiveTabId(dorkaPage)
    expect(activeTabBefore).not.toBeNull()

    await closeTabFromTabBar(dorkaPage, activeTabBefore!)
    ...
    await expect.poll(() => getDomActiveTabId(dorkaPage), { timeout: 5_000 }).not.toBeNull()
  })
})
