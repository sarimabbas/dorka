...
test.describe('Default branch visibility', () => {
  ...
  test('keeps the default branch visible when sleeping workspaces are hidden', async ({
    ...
    const { defaultBranchId, featureId } = await seedSidebarVisibilityScenario(dorkaPage)
    const defaultBranchRow = worktreeRow(dorkaPage, defaultBranchId)
    const featureRow = worktreeRow(dorkaPage, featureId)
