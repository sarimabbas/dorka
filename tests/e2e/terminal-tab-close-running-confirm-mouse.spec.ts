...
test('middle-clicking a busy tab prompts, and cancelling keeps the tab', async ({ dorkaPage }) => {
  test.setTimeout(120_000)
  const busyTabId = await startBusyTerminal(dorkaPage)
  const busyTab = dorkaPage.locator(`${SORTABLE_TAB}[data-tab-id="${busyTabId}"]`).first()
  const tabsBefore = await dorkaPage.locator(SORTABLE_TAB).count()
  ...
  expect(await dorkaPage.locator(SORTABLE_TAB).count()).toBe(tabsBefore)
})

test('confirming the X-button prompt closes the busy tab', async ({ dorkaPage }) => {
  test.setTimeout(120_000)
  const busyTabId = await startBusyTerminal(dorkaPage)
  const busyTab = dorkaPage.locator(`${SORTABLE_TAB}[data-tab-id="${busyTabId}"]`).first()
  ...
  await expect(closeDialogTitle(dorkaPage)).toBeVisible({ timeout: 15_000 })
  await dorkaPage.getByRole('button', { name: /^Stop and Close$/ }).click()

  await expect(busyTab).toHaveCount(0, { timeout: 15_000 })
  await expect(closeDialogTitle(dorkaPage)).toBeHidden()
})

test('Cmd+W on a busy single-pane tab raises exactly one dialog', async ({ dorkaPage }) => {
  test.setTimeout(120_000)
  const busyTabId = await startBusyTerminal(dorkaPage)
  const busyTab = dorkaPage.locator(`${SORTABLE_TAB}[data-tab-id="${busyTabId}"]`).first()
