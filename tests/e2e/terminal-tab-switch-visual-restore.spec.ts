...
async function startHiddenPtyOutputBurst(page: Page, ptyId: string, runId: string): Promise<void> {
  ...
  await runNodeScriptInTerminal(page, ptyId, script, { prefix: 'dorka-tab-switch-burst' })
}
...
test.describe('Terminal tab switch visual restore', () => {
  ...
  test('keeps full-width geometry after switching away and back', async ({
    ...
    const { firstTabId, secondTabId } = await ensureTwoTerminalTabs(dorkaPage)
    await forceWebglOnActiveTab(dorkaPage)

    const runId = `${Date.now()}`
    const marker = `${TAB_SWITCH_MARKER_PREFIX}_${runId}`
    const firstPtyId = await waitForPanePtyIdOnTab(dorkaPage, firstTabId)
    await writeStaticTabContent(dorkaPage, firstTabId, marker, TAB_A_GLYPH_ROW)

    const baseline = await readTabTerminalGeometry(dorkaPage, firstTabId, runId)
    ...
    await resetTerminalOutputSchedulerDebug(dorkaPage)
    await startHiddenPtyOutputBurst(dorkaPage, firstPtyId, runId)

    for (let cycle = 0; cycle < 12; cycle += 1) {
      await activateTerminalTab(dorkaPage, secondTabId)
      await injectHiddenStreamingBurst(dorkaPage, firstTabId, runId)
      // Why: rapid back-to-back switches mirror the user's leave/return pattern
      // and race the overlay's rAF/50ms refit retries.
      await activateTerminalTab(dorkaPage, firstTabId)
      if (cycle % 3 === 0) {
        await activateTerminalTab(dorkaPage, secondTabId)
        await activateTerminalTab(dorkaPage, firstTabId)
      }
      ...
      const immediate = await readTabTerminalGeometry(dorkaPage, firstTabId, runId)
      ...
      await dorkaPage.waitForTimeout(60)
      const settled = await readTabTerminalGeometry(dorkaPage, firstTabId, runId)
    ...
    const schedulerActivity = await waitForHiddenOutputSchedulerActivity(dorkaPage)
  ...
  test('keeps geometry after hidden alt-screen TUI redraws during tab switches', async ({
    ...
    const { firstTabId, secondTabId } = await ensureTwoTerminalTabs(dorkaPage)
    await forceWebglOnActiveTab(dorkaPage)
    await waitForPanePtyIdOnTab(dorkaPage, firstTabId)
    ...
    for (let cycle = 0; cycle < 6; cycle += 1) {
      ...
      const { ptyId, cols, rows } = await readPaneIdentityOnTab(dorkaPage, firstTabId)
      await setHiddenSnapshotOverride(dorkaPage, ptyId, {
        data: buildAltScreenFrame(finalMarker, restoreFrame),
        cols,
        rows
      })
      await activateTerminalTab(dorkaPage, secondTabId)
      await writeToPaneTerminal(dorkaPage, firstTabId, redraw)
      await activateTerminalTab(dorkaPage, firstTabId)

      const geometry = await readTabTerminalGeometry(dorkaPage, firstTabId, `${runId}_ALT`)
      const renderedFrame = await readRenderedAltScreenFrame(dorkaPage, firstTabId, finalMarker)
      renderPaths.push(
  ...
  test('restores skipped hidden agent output on light tab resume', async ({ dorkaPage }) => {
    await waitForSessionReady(dorkaPage)
    await waitForActiveWorktree(dorkaPage)
    await ensureTerminalVisible(dorkaPage)
    await waitForActiveTerminalManager(dorkaPage, 30_000)
    ...
    const agentTabId = await createCodexMarkedTerminalTab(dorkaPage)
    await waitForActiveTerminalManager(dorkaPage, 30_000)
    await waitForPanePtyIdOnTab(dorkaPage, agentTabId)
    const paneIdentity = await readPaneIdentityOnTab(dorkaPage, agentTabId)
    ...
    await resetHiddenOutputDebug(dorkaPage)
    await injectPaneData(dorkaPage, paneKey, hiddenFrame, {
      seq: hiddenFrame.length,
      rawLength: hiddenFrame.length
    })

    await expect
      .poll(async () => (await readHiddenOutputDebug(dorkaPage))?.hiddenRendererSkipCount ?? 0, {
        timeout: 5_000,
        message: 'Codex-marked hidden output did not take the skipped renderer path'
      })
      .toBeGreaterThan(0)
    await setHiddenSnapshotOverride(dorkaPage, paneIdentity.ptyId, {
    ...
    await activateTerminalTab(dorkaPage, agentTabId)

    await expect
      .poll(() => getTerminalContent(dorkaPage, 8_000), {
        timeout: 10_000,
  ...
  test('restores skipped hidden Grok output on light tab resume', async ({ dorkaPage }) => {
    await waitForSessionReady(dorkaPage)
    await waitForActiveWorktree(dorkaPage)
    await ensureTerminalVisible(dorkaPage)
    await waitForActiveTerminalManager(dorkaPage, 30_000)
    ...
    const shellTabId = (await getActiveTabId(dorkaPage))!
    const grokTabId = await createGrokMarkedTerminalTab(dorkaPage)
    await waitForActiveTerminalManager(dorkaPage, 30_000)
    await waitForPanePtyIdOnTab(dorkaPage, grokTabId)
    const paneIdentity = await readPaneIdentityOnTab(dorkaPage, grokTabId)
    ...
    await activateTerminalTab(dorkaPage, grokTabId)
  ...
  test('@headful keeps returned tab glyphs intact across tab switches', async ({
    ...
    await forceWebglOnActiveTab(dorkaPage)
    await activateTerminalTab(dorkaPage, firstTabId)
    const firstWebgl = await waitForWebglOnTab(dorkaPage, firstTabId)
    await activateTerminalTab(dorkaPage, secondTabId)
    await dorkaPage.evaluate((id) => {
      window.__paneManagers?.get(id)?.setTerminalGpuAcceleration?.('on')
    }, secondTabId)
    const secondWebgl = await waitForWebglOnTab(dorkaPage, secondTabId)
    ...
    const firstPtyId = await waitForPanePtyIdOnTab(dorkaPage, firstTabId)
    const secondPtyId = await waitForPanePtyIdOnTab(dorkaPage, secondTabId)
    await sendToTerminal(dorkaPage, firstPtyId, SILENT_FOREGROUND_COMMAND)
    await sendToTerminal(dorkaPage, secondPtyId, SILENT_FOREGROUND_COMMAND)
    ...
    await writeStaticTabContent(dorkaPage, firstTabId, markerA, TAB_A_GLYPH_ROW)
    await activateTerminalTab(dorkaPage, secondTabId)
    await writeStaticTabContent(dorkaPage, secondTabId, markerB, TAB_B_GLYPH_ROW)

    await activateTerminalTab(dorkaPage, firstTabId)
    await resetAtlasOnTab(dorkaPage, firstTabId)
    await dorkaPage.waitForTimeout(800)
    const baseline = await captureStableTabScreenshot(dorkaPage, firstTabId)
    ...
    for (let cycle = 0; cycle < 8; cycle += 1) {
      ...
      const afterReturn = await captureStableTabScreenshot(dorkaPage, firstTabId)
