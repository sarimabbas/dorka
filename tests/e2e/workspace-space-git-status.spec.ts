...
test.describe('Workspace Space git status checks', () => {
  test('checks every scanned deletable row, including rows after the first 50', async ({
    ...
    // Why: on symlinked tmpdirs (/var→/private/var on macOS, /tmp→… on CI) Dorka
    ...
    const worktreeParent = realpathSync(
      mkdtempSync(path.join(os.tmpdir(), 'dorka-space-git-status-'))
    )
    ...
    try {
      ...
      const repoId = await dorkaPage.evaluate((testRepoPath) => {
      ...
      await loadWorktreesUntilPathsPresent(dorkaPage, repoId, registeredWorktreePaths)

      const rowDisplayNames = await dorkaPage.evaluate(
      ...
      // anything.
      const rowCheckboxes = dorkaPage.getByRole('checkbox', {
        name: new RegExp(
      ...
      await expect(dorkaPage.getByText('Keep: git not checked')).toHaveCount(0, { timeout: 30_000 })
    } finally {
      for (const worktreePath of worktreePaths) {
        try {
