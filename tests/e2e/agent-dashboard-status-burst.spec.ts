...
test('keeps the visible Agent Dashboard interactive during a 100-pane status replay', async ({
  ...
  const panes = await dorkaPage.evaluate(
  ...
  const dashboardButton = dorkaPage.getByRole('button', { name: /Agent Dashboard/ })
  ...
  await dorkaPage.locator('[data-agent-dashboard-sheet]').waitFor({ state: 'visible' })
  const interactionElapsedMs = performance.now() - interactionStartedAt
  const statusPublicationsAtVisible = await dorkaPage.evaluate(() => {
    const probe = (
  ...
  const evidence = await dorkaPage.evaluate(
