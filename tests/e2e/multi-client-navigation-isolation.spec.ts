...
const isPairedBrowserRun = process.env.DORKA_E2E_WEB_CLIENT === '1'
...
test('keeps two paired browser clients and the host on independent worktrees', async ({
  ...
  await expect
    .poll(() => loadTestWorktreeIds(dorkaPage, branchA, branchB), {
      timeout: 30_000,
  ...
  const ids = await loadTestWorktreeIds(dorkaPage, branchA, branchB)
  if (!ids) {
    throw new Error('Test worktrees disappeared after discovery')
  }

  await selectWorktree(dorkaPage, ids.host)

  let clientA: Page | null = null
  let clientB: Page | null = null
  try {
    const offerA = await createPairingOffer(dorkaPage)
    ...
    const offerB = await createPairingOffer(dorkaPage)
    ...
    await expectActiveWorktree(dorkaPage, ids.host)
...
test('shows only provider-backed creation actions in paired web', async ({
  ...
  const visibleWorktreeId = await dorkaPage.evaluate(
  ...
  const offer = await createPairingOffer(dorkaPage)
...
test('routes Add Project folder browsing through the paired host', async ({
  ...
  const hostFolder = mkdtempSync(path.join(os.tmpdir(), 'dorka-paired-web-folder-'))
