...
test.describe('terminal document visibility WebGL recovery', () => {
  test('@headful preserves the WebGL atlas and keeps terminal text painted after document visibility resumes', async ({
    ...
    const webglActive = await forceWebgl(dorkaPage)
    test.skip(!webglActive, 'WebGL was not active in this Electron environment')

    await writeStableTerminalContent(dorkaPage)
    expect(await patchAtlasCounter(dorkaPage)).toBe(true)
    expect(await countPatchedWebglAddons(dorkaPage)).toBeGreaterThanOrEqual(2)
    const baseline = await terminalScreenshots(dorkaPage)
    ...
    try {
      ...
      await resetAtlasResetCount(dorkaPage)
      ...
      if (!browserWindowVisibilityWorked) {
        await resetAtlasResetCount(dorkaPage)
        await dispatchDocumentVisibilityCycle(dorkaPage)
      }

      await waitForTerminalPaint(dorkaPage)
      expect(
        await readAtlasResetCount(dorkaPage),
        'ordinary document visibility resume cleared the shared WebGL atlas'
      ).toBe(0)

      const afterResume = await terminalScreenshots(dorkaPage)
