...
test.describe('Terminal 2-Set Korean preedit visibility', () => {
  test('shows every assembling jamo at non-zero size and commits the syllable ahead of the newline', async ({
    ...
    try {
      ...
      await expectPreeditHidden(dorkaPage, 'before composing')

      for (const frame of HAN_FRAMES) {
        await composeHangulSyllable(arena.session, dorkaPage, [frame])
        await expectPreeditRendered(dorkaPage, frame.preedit, `composing ${frame.preedit}`)
      }

      await commitImeText(arena.session, '한')
      await expectPreeditHidden(dorkaPage, 'after committing 한')

      await dispatchPlainEnter(arena.session)

      const received = await waitForTerminalImeBytes(dorkaPage, reader)
  ...
  test('keeps a preedit the IME resumes without a compositionstart visible', async ({
    ...
    try {
      ...
      await dispatchResumedCompositionUpdate(dorkaPage, '한')

      const sample = await samplePreeditOverlay(dorkaPage)
  ...
  test('renders the preedit at every update of a recorded Windows/WSL Hangul session', async ({
    ...
    await applyImePlatformPolicy(dorkaPage, 'windows')
    const arena = await openTerminalImePaneArena(dorkaPage)
    let completed = false
    try {
      const replay = await replayRecordedImeDomTrace(dorkaPage, RECORDED_TRACE)
  ...
  test('loses and duplicates nothing when back-to-back syllables commit at full speed', async ({
    ...
    try {
      ...
      for (let repetition = 0; repetition < 4; repetition += 1) {
        await composeHangulSyllable(arena.session, dorkaPage, HAN_FRAMES, 0)
        await commitImeText(arena.session, '한')
        await composeHangulSyllable(arena.session, dorkaPage, GEUL_FRAMES, 0)
