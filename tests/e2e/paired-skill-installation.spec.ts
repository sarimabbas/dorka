...
test('installs on a headed desktop runtime without a local fallback', async ({
  ...
  const folderRoot = mkdtempSync(join(tmpdir(), 'dorka-paired-skill-folder-'))
  let client: PairedElectronClient | null = null
  try {
    const hostHome = await electronApp.evaluate(({ app }) => app.getPath('home'))
    const worktreeId = await activeWorktreeId(dorkaPage)
    const folderWorkspaceId = await createHostFolderWorkspace(dorkaPage, folderRoot)
...
function cloudClientEnvironment(): Record<string, string> {
  ...
  return {
    DORKA_ARTIFACTS_API_URL: origin,
    DORKA_CLOUD_API_URL: origin,
    DORKA_CLOUD_CLIENT_ID: 'skills-e2e-client',
    DORKA_CLOUD_DEV_AUTH: '1',
    DORKA_CLOUD_ALLOW_PLAINTEXT_SESSION: '1',
    DORKA_SKILL_PACKAGE_DOWNLOAD_ORIGINS: origin
  }
}

async function connectCloud(page: Page): Promise<void> {
  const auth = await page.evaluate(() => window.api.dorkaProfiles.connectCurrent())
