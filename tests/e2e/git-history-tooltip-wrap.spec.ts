...
test('keeps conventional commit-message lines intact in the history tooltip', async ({
  ...
  await openSourceControlForWorktree(dorkaPage, testRepoPath, fixture.worktreePath)

  const commitsToggle = dorkaPage.getByRole('button', { name: /Commits/ })
  await expect(commitsToggle).toBeVisible()
  await commitsToggle.click()

  const row = dorkaPage.getByTestId('git-history-row').filter({ hasText: subject })
