...
for (const theme of ['dark', 'light'] as const) {
  test(`Resource Manager names folder workspaces and their groups (${theme})`, async ({
    ...
    const root = mkdtempSync(join(tmpdir(), 'dorka-resource-folders-'))
    ...
    await dorkaPage.getByRole('button', { name: /^Resource Manager,/ }).click()
    const popover = dorkaPage.getByRole('dialog')
...
{}, testInfo) => {
  const root = mkdtempSync(join(tmpdir(), 'dorka-recovered-folder-'))
