...
test.describe('Wedged terminal write pipeline recovery', () => {
  test('pane recovers rendering and input after its write pipeline wedges', async ({
    ...
    await dorkaPage.keyboard.type(`echo ${beforeMarker}`, { delay: 20 })
    await dorkaPage.keyboard.press('Enter')
    await expect
      .poll(async () => (await getTerminalContent(dorkaPage)).includes(beforeMarker), {
        timeout: 15_000,
        message: 'Baseline echo did not render — pane unhealthy before wedge'
      })
      .toBe(true)

    await wedgeActivePaneWritePipeline(dorkaPage)
    ...
    await focusActiveTerminalInput(dorkaPage)
    await dorkaPage.keyboard.type(`echo ${afterMarker}`, { delay: 20 })
    await dorkaPage.keyboard.press('Enter')

    await expect
      .poll(async () => (await getTerminalContent(dorkaPage)).includes(afterMarker), {
  ...
  test('pane recovers after its xterm is disposed under live bindings (zombie pane)', async ({
    ...
    await sendToTerminal(dorkaPage, ptyId, `echo ${outputMarker}\r`)

    await expect
      .poll(async () => (await getTerminalContent(dorkaPage)).includes(outputMarker), {
        timeout: 45_000,
        message:
    ...
    await dorkaPage.keyboard.type(`echo ${typedMarker}`, { delay: 20 })
    await dorkaPage.keyboard.press('Enter')
    await expect
      .poll(async () => (await getTerminalContent(dorkaPage)).includes(typedMarker), {
        timeout: 15_000,
