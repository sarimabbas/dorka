...
import { expect, test as base } from './helpers/dorka-app'
import {
...
const isDockerNestedRuntimeRun =
  process.env.DORKA_E2E_NESTED_RUNTIME_SSH === '1' && process.env.DORKA_E2E_WEB_CLIENT === '1'
...
const test = base.extend<{ proxyJumpFixture: NestedRuntimeProxyJumpFixture | null }>({
  ...
  dorkaAppExtraEnv: async ({ proxyJumpFixture }, provideFixture) => {
    await provideFixture(
      proxyJumpFixture ? { DORKA_SYSTEM_SSH_PATH: proxyJumpFixture.wrapperPath } : {}
    )
  }
})
...
test.skip(
  ...
  'Run with DORKA_E2E_NESTED_RUNTIME_SSH=1 and DORKA_E2E_WEB_CLIENT=1'
)
...
test('routes HUB desktop, web, and two paired desktops through HUB-owned SSH', async ({
  ...
  try {
    ...
    const remote = await connectDockerSshRelayTarget(dorkaPage, sshTarget)
    await installProxyJumpFixture(proxyJumpFixture, proxyJumpDestination, proxyJumpHost)
    const proxyJumpRemote = await connectDockerSshRelayTarget(dorkaPage, proxyJumpDestination, {
      viaProxyJump: true
    })
    const localRepoId = await dorkaPage.evaluate(() => {
    ...
    const webOffer = await createRuntimeDesktopPairingOffer(dorkaPage)
    ...
    const offerA = await createRuntimeDesktopPairingOffer(dorkaPage)
    ...
    const offerB = await createRuntimeDesktopPairingOffer(dorkaPage)
    ...
    await disconnectDockerSshRelayTarget(dorkaPage, remote.targetId)
    await assertRuntimeSshStatus(clientA, remote.targetId, 'disconnected')
    await reconnectDisconnectedDockerSshRelayTarget(dorkaPage, remote.targetId)
    ...
    const rePairOffer = await createRuntimeDesktopPairingOffer(dorkaPage)
