...
test.describe('Tab Rename (Inline)', () => {
  ...
  test('double-clicking a tab opens an inline rename input and Enter commits', async ({
    ...
    const originalTitle = await getActiveTabTitle(dorkaPage, worktreeId)
    expect(originalTitle.length).toBeGreaterThan(0)

    const tabLocator = tabLocatorByTitle(dorkaPage, originalTitle)
    await tabLocator.dblclick()

    const renameInput = dorkaPage.getByRole('textbox', {
    ...
    await expect
      .poll(async () => getActiveCustomTitle(dorkaPage, worktreeId), { timeout: 3_000 })
      .toBe('My Custom Title')
    await expect(renameInput).toBeHidden()
    await expect(tabLocatorByTitle(dorkaPage, 'My Custom Title')).toBeVisible()
  })
  ...
  test('context-menu Change Title opens a focused select-all rename input', async ({
    ...
    await tabLocatorByTitle(dorkaPage, originalTitle).click({ button: 'right' })
    await dorkaPage.getByRole('menuitem', { name: /^Change Title(?:\s|$)/ }).click()
    ...
    await expect(tabLocatorByTitle(dorkaPage, 'Context Menu Title')).toBeVisible()
  })

  test('Escape during inline rename discards the edit', async ({ dorkaPage }) => {
    const worktreeId = (await getActiveWorktreeId(dorkaPage))!
    const originalTitle = await getActiveTabTitle(dorkaPage, worktreeId)
    ...
    await expect(tabLocatorByTitle(dorkaPage, originalTitle)).toBeVisible()
    await expect
      .poll(async () => getActiveCustomTitle(dorkaPage, worktreeId), { timeout: 3_000 })
      .toBe(null)
  })

  test('renaming to an empty string resets the tab to its default title', async ({ dorkaPage }) => {
    const worktreeId = (await getActiveWorktreeId(dorkaPage))!
    ...
    const defaultTitle = await getActiveTabTitle(dorkaPage, worktreeId)
    ...
    const tabLocator = tabLocatorByTitle(dorkaPage, 'Seeded Custom')
    await tabLocator.dblclick()
    ...
    await expect(tabLocatorByTitle(dorkaPage, defaultTitle)).toBeVisible()
    await expect
      .poll(async () => getActiveCustomTitle(dorkaPage, worktreeId), { timeout: 3_000 })
      .toBe(null)
  })

  test('clicking away (blur) commits the rename', async ({ dorkaPage }) => {
    const worktreeId = (await getActiveWorktreeId(dorkaPage))!
    ...
    await expect
      .poll(async () => (await getWorktreeTabs(dorkaPage, worktreeId)).length, { timeout: 3_000 })
      .toBeGreaterThanOrEqual(2)

    const tabs = await getWorktreeTabs(dorkaPage, worktreeId)
    const activeId = await getActiveTabId(dorkaPage)
    const activeTab = tabs.find((t) => t.id === activeId)!
    const otherTab = tabs.find((t) => t.id !== activeId)!

    const tabLocator = tabLocatorByTitle(dorkaPage, activeTab.title!)
    await tabLocator.dblclick()
    ...
    await tabLocatorByTitle(dorkaPage, otherTab.title!).click()

    await expect(renameInput).toBeHidden()
    await expect(tabLocatorByTitle(dorkaPage, 'Committed By Blur')).toBeVisible()
    expect(
      await dorkaPage.evaluate(
  ...
  test('terminal title updates do not resize neighboring tabs', async ({ dorkaPage }) => {
    const worktreeId = (await getActiveWorktreeId(dorkaPage))!
    const tabIds = await dorkaPage.evaluate((targetWorktreeId) => {
    ...
    const tabs = tabIds.map((id) =>
      dorkaPage.locator(`[data-testid="sortable-tab"][data-tab-id="${id}"]`)
    )
  ...
  test('rename input stays at a usable width when many tabs are open', async ({ dorkaPage }) => {
    const worktreeId = (await getActiveWorktreeId(dorkaPage))!
    const targetTabId = await getActiveTabId(dorkaPage)
    ...
    const tabLocator = tabLocatorByTitle(dorkaPage, targetTitle)
  ...
  test('middle-clicking inside the rename input does not close the tab', async ({ dorkaPage }) => {
    const worktreeId = (await getActiveWorktreeId(dorkaPage))!
    const tabsBefore = (await getWorktreeTabs(dorkaPage, worktreeId)).length
    const originalTitle = await getActiveTabTitle(dorkaPage, worktreeId)
    ...
    await expect(renameInput).toBeVisible()
    await expect(tabLocatorByTitle(dorkaPage, originalTitle)).toBeVisible()
    expect((await getWorktreeTabs(dorkaPage, worktreeId)).length).toBe(tabsBefore)
  })
})
