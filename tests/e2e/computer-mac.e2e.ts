...
describe.skipIf(!isMac || !e2eOptIn)('computer-use macOS e2e (TextEdit)', () => {
  ...
  test('list-apps includes TextEdit', async () => {
    const result = await runDorkaCli(['computer', 'list-apps', '--json'])
  ...
  test('get-app-state returns TextEdit state', async () => {
    const result = await runDorkaCli(['computer', 'get-app-state', '--app', 'TextEdit', '--json'])
  ...
  test('click, type-text, and re-observe show inserted text', async () => {
    const before = parseJsonOutput<{ result: ComputerSnapshotResult }>(
      (await runDorkaCli(['computer', 'get-app-state', '--app', 'TextEdit', '--json'])).stdout
    )
    ...
    const marker = `dorka computer e2e ${Date.now()}`
    await runDorkaCli(['computer', 'type-text', '--app', 'TextEdit', '--text', marker])
  ...
  test('paste-text and hotkey verify TextEdit text replacement', async () => {
    ...
    const marker = `dorka paste final ${Date.now()}`
    ...
    expect(after.result.snapshot.treeText).toContain(marker)
    expect(after.result.snapshot.treeText).not.toContain('dorka paste first')
  })
  ...
  test('accessibility text actions work when TextEdit is not frontmost', async () => {
    ...
    const marker = `dorka unfocused final ${Date.now()}`
    ...
    expect(after.result.snapshot.treeText).toContain(marker)
    expect(after.result.snapshot.treeText).not.toContain('dorka unfocused first')
  })
