...
test.describe('Terminal Panes', () => {
  ...
  test('Set Title strip activates its pane and accepts file-path drops', async ({ dorkaPage }) => {
    ...
    const initialSnapshot = await waitForPaneIdentitySnapshot(dorkaPage, 1)
    ...
    const splitSnapshot = await waitForPaneIdentitySnapshot(dorkaPage, 2)
    ...
    await expect
      .poll(async () => (await readPaneIdentitySnapshot(dorkaPage))?.activeLeafId ?? null)
      .toBe(otherPane.leafId)

    const titleBar = dorkaPage.locator('.pane-title-bar', { hasText: title }).first()
    ...
    await titleBar.evaluate((element, path) => {
      ...
      dataTransfer.setData('text/x-dorka-file-path', path)
    ...
    await expect
      ...
      .toBe(titledLeafId)
    await expect
      .poll(async () => (await getTerminalContent(dorkaPage)).includes(droppedPath), {
        timeout: 5_000,
  ...
  test('Set Title overlay follows its pane after same-count pane move', async ({ dorkaPage }) => {
    ...
    await splitActiveTerminalPane(dorkaPage, 'vertical')
    await waitForPaneCount(dorkaPage, 2)
    const beforeMove = await waitForPaneIdentitySnapshot(dorkaPage, 2)
    ...
    const beforeOrder = await readTerminalPaneDomLeafOrder(dorkaPage)

    await expectPaneTitleAttachedToLeaf(dorkaPage, title, titledLeafId)
    await moveTerminalPaneByLeafId(dorkaPage, titledLeafId, target.leafId, 'right')

    await expect
      .poll(async () => readTerminalPaneDomLeafOrder(dorkaPage), {
        timeout: 10_000,
  ...
  test('Set Title keeps the pane drag handle available over the title strip', async ({
    ...
    const titleTopHit = await dorkaPage.evaluate(
      ({ title, titledLeafId }) => {
    ...
    await dorkaPage.locator('.pane-title-bar', { hasText: title }).click({
      position: { x: 20, y: 18 }
    })
    await expect(dorkaPage.locator('.pane-title-input')).toBeVisible()
  })

  test('@headful Set Title pane can be dragged from the title strip', async ({ dorkaPage }) => {
    ...
    await splitActiveTerminalPane(dorkaPage, 'vertical')
    await waitForPaneCount(dorkaPage, 2)
    const beforeDrag = await waitForPaneIdentitySnapshot(dorkaPage, 2)
    ...
    const titleDragHandle = dorkaPage
    ...
    await dorkaPage.mouse.move(sourceBox!.x + sourceBox!.width / 2, sourceBox!.y + 4)
    await dorkaPage.mouse.down()
    await dorkaPage.mouse.move(targetDropX, targetBox!.y + targetBox!.height / 2, {
      steps: 20
    })
    ...
    await expect
      ...
      .not.toEqual(beforeOrder)
    const afterDrag = await waitForPaneIdentitySnapshot(dorkaPage, 2)
