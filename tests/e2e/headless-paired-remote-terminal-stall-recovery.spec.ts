...
const scratch = mkdtempSync(path.join(os.tmpdir(), 'dorka-headless-stalled-stream-'))
...
test('recovers an ACK-starved stream from an isolated headless Dorka host @headful', async ({
  testRepoPath
}) => {
