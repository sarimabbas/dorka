...
function createCombinedDiffScrollRepo(): CombinedDiffScrollRepo {
  const repoPath = realpathSync(mkdtempSync(path.join(os.tmpdir(), 'dorka-combined-diff-scroll-')))
...
test.describe('Combined diff scroll restore', () => {
  ...
  test('keeps the visible section anchored after switching tabs', async ({ dorkaPage }) => {
    await waitForSessionReady(dorkaPage)
    const fixture = createCombinedDiffScrollRepo()

    try {
      const worktreeId = await addAndActivateRepo(dorkaPage, fixture.repoPath)
      const diffTabId = await openCombinedDiff(dorkaPage, worktreeId, fixture.repoPath)
      await expect(dorkaPage.locator('.combined-diff-scroll-container')).toBeVisible()
      await expect(dorkaPage.getByText(`${FILE_COUNT} changed files`)).toBeVisible()

      await scrollCombinedDiffDeep(dorkaPage)
      await waitForStableViewportAnchor(dorkaPage)
      const activeScrollSamples = await wheelCombinedDiffDown(dorkaPage)
      ...
      const beforeSwitch = await waitForStableViewportAnchor(dorkaPage)
      ...
      await expect(dorkaPage.locator('.combined-diff-scroll-container')).toHaveCount(0)

      await dorkaPage.locator(`[data-tab-id="${diffTabId}"]`).click({ force: true })
      await expect(dorkaPage.locator('.combined-diff-scroll-container')).toBeVisible()
      const afterSwitch = await waitForRestoredViewportAnchor(dorkaPage, beforeSwitch)

      expect(afterSwitch.key).toBe(beforeSwitch.key)
      expect(Math.abs(afterSwitch.top - beforeSwitch.top)).toBeLessThan(80)

      await clickVisibleDiffLine(dorkaPage)
      const afterLineClick = await waitForStableViewportAnchor(dorkaPage)
