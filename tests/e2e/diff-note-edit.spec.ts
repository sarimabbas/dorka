...
test.describe('Diff note edit', () => {
  ...
  test('editing a saved inline note updates the open diff card', async ({ dorkaPage }) => {
    ...
    const addResult = await dorkaPage.evaluate(
      async ({ wId, rel, body }) => {
        const store = window.__store
        if (!store) {
    ...
    await expect(card.locator('.dorka-diff-comment-body')).toHaveText(seededBody)

    await card.getByTitle('Edit note').click()

    const textarea = card.locator('.dorka-diff-comment-popover-textarea')
    await expect(textarea).toBeVisible()
    await expect(textarea).toHaveValue(seededBody)

    const saveButton = card
      .locator('.dorka-diff-comment-popover-footer button')
      .filter({ hasText: 'Save' })
    ...
    const updatedCard = dorkaPage
      .locator('.dorka-diff-comment-card')
      .filter({ has: dorkaPage.locator('.dorka-diff-comment-body', { hasText: editedBody }) })
      .first()
    await expect(updatedCard, 'inline card did not update in the open diff').toBeVisible()
    await expect(updatedCard.locator('.dorka-diff-comment-body')).toHaveText(editedBody)
    await expect(updatedCard.locator('.dorka-diff-comment-body')).not.toHaveText(seededBody)
