 ...
 *
 * Topology: headed Dorka desktop host (remote server) + a separate paired Dorka
...
const scratch = mkdtempSync(path.join(os.tmpdir(), 'dorka-host-park-viewer-'))
...
test.use({ dorkaAppExtraEnv: { DORKA_E2E_TERMINAL_PARKING_DELAY_MS: String(PARK_DELAY_MS) } })
...
test('a cold-parked host pane keeps serving its paired remote viewer', async ({
  ...
  const previousParkDelay = process.env.DORKA_E2E_TERMINAL_PARKING_DELAY_MS
  process.env.DORKA_E2E_TERMINAL_PARKING_DELAY_MS = String(PARK_DELAY_MS)
  ...
  try {
    ...
    await expect
      .poll(() => dorkaPage.evaluate((id) => window.__paneManagers?.has(id) ?? false, hostTabId), {
        timeout: 60_000,
    ...
    await waitForTabParked(dorkaPage, hostTabId, { parkDelayMs: PARK_DELAY_MS })
    ...
    while (Date.now() < timelineDeadline) {
      const hostMounted = await dorkaPage.evaluate(
    ...
    if (previousParkDelay === undefined) {
      delete process.env.DORKA_E2E_TERMINAL_PARKING_DELAY_MS
    } else {
      process.env.DORKA_E2E_TERMINAL_PARKING_DELAY_MS = previousParkDelay
    }
