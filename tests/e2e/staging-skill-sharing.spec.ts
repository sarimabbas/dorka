...
const RUN_STAGING = process.env.DORKA_E2E_SKILL_STAGING === '1'
const AUTH_TOKEN = process.env.DORKA_CLOUD_AUTH_TOKEN?.trim()
const PHYSICAL_HOST_PAIRING_URL = process.env.DORKA_E2E_SKILL_PHYSICAL_PAIRING_URL?.trim()
const RUN_HEADLESS_PAIRED = process.env.DORKA_E2E_SKILL_PAIRED_HEADLESS === '1'
const PHYSICAL_WSL_DISTRO = 'Ubuntu-24.04'
const SKILL_NAME = `dorka-staging-${randomUUID().slice(0, 8)}`
const SSH_TARGET = RUN_STAGING ? stagingSkillSshTargetFromEnvironment() : null

if (RUN_STAGING && !AUTH_TOKEN) {
  throw new Error('DORKA_CLOUD_AUTH_TOKEN is required for the noninteractive staging journey.')
}
...
test.use({
  dorkaAppExtraEnv: {
    DORKA_ARTIFACTS_API_URL: 'https://cloud-api-staging.ondorka.dev',
    DORKA_CLOUD_API_URL: 'https://auth-staging.ondorka.dev',
    DORKA_CLOUD_AUTH_URL: 'https://auth-staging.ondorka.dev',
    DORKA_CLOUD_CLIENT_ID: 'dorka-desktop',
    ...(AUTH_TOKEN ? { DORKA_CLOUD_AUTH_TOKEN: AUTH_TOKEN } : {})
  }
})

test.skip(!RUN_STAGING, 'Set DORKA_E2E_SKILL_STAGING=1 to run the live staging journey.')
...
test('publishes, updates, revokes, and deletes without losing local state', async ({
  ...
  const sourceRoot = mkdtempSync(join(tmpdir(), 'dorka-staging-skill-source-'))
  ...
  try {
    if (RUN_HEADLESS_PAIRED) {
      ...
      physicalEnvironmentId = await addPhysicalHost(dorkaPage, pairedHost.offer.pairingUrl)
    } else if (PHYSICAL_HOST_PAIRING_URL) {
      physicalEnvironmentId = await addPhysicalHost(dorkaPage, PHYSICAL_HOST_PAIRING_URL)
    }
    if (SSH_TARGET) {
      sshTargetId = await connectStagingSkillSshTarget(dorkaPage, SSH_TARGET)
    }
    ...
    const firstInstall = await installVersion(dorkaPage, first.published, { scope: 'global' })
    ...
    for (const target of externalTargets(physicalEnvironmentId, sshTargetId)) {
      ...
      await expectManagedRemoteVersion(dorkaPage, target, second.published.version.versionId)
    }
    const conflict = await installVersion(dorkaPage, second.published, { scope: 'global' })
    ...
    for (const target of externalTargets(physicalEnvironmentId, sshTargetId)) {
      ...
      await expectManagedRemoteVersion(dorkaPage, target, first.published.version.versionId)
    }
    ...
    const revokedInstall = await dorkaPage.evaluate(
    ...
    const removed = await dorkaPage.evaluate(
    ...
    expect(existsSync(globalSkill)).toBe(false)
    expect(
      await dorkaPage.evaluate((id) => window.api.skills.getPackage(id), packageId)
    ).toMatchObject({ status: 'ok' })
    expect(
      await dorkaPage.evaluate((id) => window.api.skills.deletePackage(id), packageId)
    ).toMatchObject({ status: 'ok' })
    packageId = null
  } finally {
    for (const target of externalTargets(physicalEnvironmentId, sshTargetId)) {
    ...
    if (sshTargetId) {
      await removeStagingSkillSshTarget(dorkaPage, sshTargetId).catch(() => undefined)
    }
...
function writeSkill(directory: string, version: string): void {
  writeFileSync(
    ...
    `---\nname: ${SKILL_NAME}\ndescription: Dorka staging installation journey\n---\n\n# Staging journey\n\nversion: ${version}\n`
  )
}
