...
test.describe('Terminal raw emoji table scroll restore repro', () => {
  ...
  test('uses WebGL by default for ordinary terminal output when available @terminal-rendering-golden', async ({
    ...
    const marker = `DORKA_AUTO_WEBGL_SMOKE_${randomUUID()}`

    await sendToTerminal(dorkaPage, ptyId, `printf ${JSON.stringify(`${marker}\\n`)}\r`)
    await waitForTerminalOutput(dorkaPage, marker, 10_000)

    const expectedWebgl = await expectAutoWebgl(dorkaPage)
    ...
    let diagnostics = await readTerminalRenderDiagnostics(dorkaPage)
    await expect
      .poll(
        async () => {
          diagnostics = await readTerminalRenderDiagnostics(dorkaPage)
  ...
  test('keeps raw emoji box table aligned after restore and scroll @terminal-rendering-golden', async ({
    ...
    await ensureTerminalVisible(dorkaPage)
    await waitForActiveTerminalManager(dorkaPage, 30_000)
    await setWideRenderedTableViewport(dorkaPage)
    await waitForActiveTerminalColumns(dorkaPage, RAW_EMOJI_BOX_TABLE_WIDTH)
    const ptyId = await waitForActivePanePtyId(dorkaPage)
    const runId = randomUUID()
    const scriptPath = path.join(testRepoPath, `.dorka-raw-emoji-fixture-table-${runId}.mjs`)
    ...
    try {
      ...
      await waitForTerminalOutput(dorkaPage, completionMarker, 20_000, 30_000)
      await expect
      ...
      await scrollActiveTerminalToText(dorkaPage, 'Singer')
      await closeFeatureTips(dorkaPage)
      const expectedWebgl = await expectAutoWebgl(dorkaPage)
      ...
      const singerGeometry = await readVisibleSingerRowGeometry(dorkaPage)
