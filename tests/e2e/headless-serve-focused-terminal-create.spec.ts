...
// Why (#10333): a windowless `dorka serve` host answered every focus-requested
...
test('creates a focus-requested terminal against a headless serve host', async ({
  ...
  try {
    ...
    // Exactly what `dorka --environment <remote> terminal create --worktree <wt>
    ...
    const created = await host.client.call<{
      ...
      command: 'echo dorka-10333-focus',
      focus: true,
