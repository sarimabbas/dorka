...
for (const { id, menuItemName } of GOLDEN_STUB_AGENTS) {
  test(`tab-bar + menu launches ${id} into a live TUI @tab-bar-agent-launch-golden`, async ({
    ...
    await openWorkspaceTerminal(dorkaPage)
    await configureGoldenStubAgent(dorkaPage, { agent: id })
    await launchGoldenStubAgentFromNewTab(dorkaPage, menuItemName)
    ...
    expect(await getTerminalContent(dorkaPage)).toContain(GOLDEN_STUB_READY_MARKER)
  })
}
...
test.describe('Windows runtimes', () => {
  ...
  for (const shell of WINDOWS_SHELLS) {
    test(`tab-bar + menu launches an agent under ${shell} @tab-bar-agent-launch-golden`, async ({
      ...
      await configureGoldenStubAgent(dorkaPage, { agent: 'codex', windowsShell: shell })
  ...
  test('tab-bar + menu launches an agent inside WSL @tab-bar-agent-launch-golden', async ({
    ...
    try {
      ...
      await configureGoldenStubAgent(dorkaPage, { agent: 'codex' })
      await launchGoldenStubAgentFromNewTab(dorkaPage)
