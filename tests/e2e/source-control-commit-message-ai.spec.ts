...
test.describe('Source Control AI commit messages', () => {
  ...
  for (const { label, linkedIssue, expected } of [
    ...
    test(`${label} the commit-message recipe`, async ({ dorkaPage, testRepoPath }) => {
      ...
      try {
        ...
        await openSourceControlForWorktree(dorkaPage, testRepoPath, worktreePath)
        ...
        const textarea = dorkaPage.getByRole('textbox', { name: 'Commit message' })
        await expect(textarea).toBeVisible({ timeout: 10_000 })

        const generate = dorkaPage.getByRole('button', { name: 'Generate commit message with AI' })
