...
test.describe('Worktree Lifecycle', () => {
  ...
  test('removing a worktree clears its tabs, open files, and browser tabs', async ({
    ...
    await switchToWorktree(dorkaPage, newWorktreeId)
    await expect
    ...
    await dorkaPage.evaluate((worktreeId) => {
      const store = window.__store
      if (!store) {
        return
      }
    ...
    await clickFileInExplorer(dorkaPage, ['README.md', 'package.json'])

    // Baseline: the new worktree now has tabs/browser tabs/open files.
    expect((await getWorktreeTabs(dorkaPage, newWorktreeId)).length).toBeGreaterThan(0)
    expect((await getBrowserTabs(dorkaPage, newWorktreeId)).length).toBeGreaterThan(0)
    expect((await getOpenFiles(dorkaPage, newWorktreeId)).length).toBeGreaterThan(0)
    ...
    // is protecting.
    await switchToWorktree(dorkaPage, originalWorktreeId)
    await expect
      .poll(async () => getActiveWorktreeId(dorkaPage), { timeout: 10_000 })
      .toBe(originalWorktreeId)

    const result = await removeWorktreeViaStore(dorkaPage, newWorktreeId)
    ...
    await expect
      .poll(async () => (await getWorktreeTabs(dorkaPage, newWorktreeId)).length, {
        timeout: 10_000,
        message: 'tabsByWorktree still holds entries for the removed worktree'
      })
      .toBe(0)
    await expect
      .poll(async () => (await getBrowserTabs(dorkaPage, newWorktreeId)).length, { timeout: 5_000 })
      .toBe(0)
    await expect
      .poll(async () => (await getOpenFiles(dorkaPage, newWorktreeId)).length, { timeout: 5_000 })
      .toBe(0)

    const allIds = await getAllWorktreeIds(dorkaPage)
  ...
  test('switching worktrees preserves per-worktree state across a round-trip', async ({
    ...
    const originalState = await dorkaPage.evaluate((wId) => {
      const store = window.__store
      if (!store) {
    ...
    const afterRoundTrip = await dorkaPage.evaluate((wId) => {
      const store = window.__store
      if (!store) {
   ...
   */
  test('terminal tabs stay scoped to the worktree that created them', async ({ dorkaPage }) => {
    const allIds = await getAllWorktreeIds(dorkaPage)
    expect(
      allIds.length,
      'fixture should provide primary + e2e-secondary worktrees'
    ...
    const worktreeA = await waitForActiveWorktree(dorkaPage)
    ...
    await expect
      .poll(async () => (await getWorktreeTabs(dorkaPage, worktreeA)).length, { timeout: 5_000 })
      .toBeGreaterThanOrEqual(2)

    // Switch to B and create a tab there too.
    await switchToWorktree(dorkaPage, worktreeB)
    await expect
      .poll(async () => getActiveWorktreeId(dorkaPage), { timeout: 10_000 })
      .toBe(worktreeB)
    await ensureTerminalVisible(dorkaPage)
    ...
    await expect
      .poll(async () => (await getWorktreeTabs(dorkaPage, worktreeB)).length, { timeout: 5_000 })
      .toBeGreaterThanOrEqual(2)

    const tabsA = await getWorktreeTabs(dorkaPage, worktreeA)
    const tabsB = await getWorktreeTabs(dorkaPage, worktreeB)
