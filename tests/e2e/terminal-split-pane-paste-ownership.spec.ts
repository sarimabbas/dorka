...
test.describe('split terminal pane paste ownership', () => {
  test('keyboard paste writes only to the active split pane PTY', async ({
    ...
    const scriptPath = path.join(testRepoPath, `.dorka-split-paste-${runId}.mjs`)
    writeFileSync(scriptPath, pasteEchoScript(runId))
    let scriptStarted = false

    try {
      await sendToTerminal(dorkaPage, activePane.ptyId, `node ${JSON.stringify(scriptPath)}\r`)
      scriptStarted = true
      await waitForTerminalOutput(dorkaPage, `SPLIT_PASTE_READY_${runId}`, 10_000)

      const payload = `DORKA_E2E_SPLIT_PASTE_${runId}`
      ...
      await dorkaPage.keyboard.press(keyboardPasteChord())
      ...
      if (scriptStarted) {
        await sendToTerminal(dorkaPage, activePane.ptyId, '\x03').catch(() => undefined)
      }
  ...
  test('internal file drop writes only to the pane under the drop target', async ({
    ...
    await dorkaPage.evaluate(
      ({ leafId, pathValue }) => {
        ...
        dataTransfer.setData('text/x-dorka-file-path', pathValue)
