...
async function createBrowserFixture(
  ...
  const fixtureDir = mkdtempSync(path.join(os.tmpdir(), 'dorka-browser-attachment-'))
...
test('resume validation waits for an attaching guest without replacing it', async ({
  ...
  await expect
    .poll(() => readGuestState(dorkaPage, browserTab.id))
    .toMatchObject({ marker: 'painted-attachment-guest', url: fixtureUrl })
  const before = await readGuestState(dorkaPage, browserTab.id)
  ...
  await expect
    .poll(() =>
      dorkaPage.evaluate((targetBrowserTabId) => {
        const webview = document.querySelector(
          `[data-browser-overlay-tab-id="${targetBrowserTabId}"] webview`
        ) as Electron.WebviewTag | null
        return {
