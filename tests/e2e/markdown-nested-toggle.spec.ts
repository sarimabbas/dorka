...
test.describe('Markdown nested toggle regression', () => {
  ...
  test('a plain details block opens as an editable toggle', async ({ dorkaPage }, testInfo) => {
  ...
  test('a nested toggle on disk reopens as editable toggles', async ({ dorkaPage }, testInfo) => {
    const context = await getActiveWorktreeContext(dorkaPage)
    let filePath: string | null = null
    ...
    try {
      ...
      await expectEditableNestedToggles(dorkaPage)
    } finally {
      await cleanupMarkdownFixture(filePath)
    }
  })

  test('editing a nested toggle survives save and reopen', async ({ dorkaPage }, testInfo) => {
    const context = await getActiveWorktreeContext(dorkaPage)
    ...
    try {
      ...
      await placeCaretInNestedToggleBody(dorkaPage)
      await dorkaPage.keyboard.type(` ${sentinel}`)
      await expectSentinelInsideNestedToggle(dorkaPage, sentinel)
      ...
      await closeActiveEditorTab(dorkaPage, savedPath)
      await openMarkdownFixture(dorkaPage, context, savedPath)
      await waitForRichMarkdownEditor(dorkaPage)
      await expectEditableNestedToggles(dorkaPage)
      await expectSentinelInsideNestedToggle(dorkaPage, sentinel)
    } finally {
  ...
  test('a nested toggle that cannot be represented stays raw passthrough', async ({
    ...
    try {
      ...
      await expectPassthroughFallback(dorkaPage)
    } finally {
      await cleanupMarkdownFixture(filePath)
    }
  })
