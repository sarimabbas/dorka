 ...
 *   DORKA_LARGE_FILE_COUNT=9500 npx playwright test \
...
async function addAndActivateRepo(dorkaPage: Page, repoPath: string): Promise<string> {
  ...
  const sourceControlButton = dorkaPage.getByRole('button', { name: /^Source Control/ })
  await expect(sourceControlButton).toBeVisible()
  await expect
    .poll(() => dorkaPage.evaluate(() => window.__store?.getState().rightSidebarTab))
    .toBe('source-control')
  await expect(dorkaPage.getByRole('button', { name: 'Filter files by name' })).toBeVisible()
...
async function unregisterLargeFileCountRepos(
  ...
  for (const repoPath of repoPaths) {
    await dorkaPage.evaluate(async (pathToRepo) => {
      const store = window.__store
...
async function measureSourceControlLoad(
  ...
  return await dorkaPage.evaluate(async ({ worktreeId, repoPath, expectedRows, pollCycles }) => {
    const store = window.__store
    if (!store) {
      throw new Error('window.__store is not available')
    }
...
test.describe('Source Control large file count (#8013)', () => {
  ...
  test('a large untracked set under the status cap stays responsive', async ({
    ...
    const untrackedFiles = Number(process.env.DORKA_LARGE_FILE_COUNT ?? '950')
    // Why: DORKA_LARGE_FILE_BYTES gives untracked files realistic sizes so the
    ...
    const untrackedFileBytes = Number(process.env.DORKA_LARGE_FILE_BYTES ?? '0')
    ...
    try {
      ...
      const measurement = await measureSourceControlLoad(dorkaPage, {
        worktreeId,
      ...
      await unregisterLargeFileCountRepos(dorkaPage, [fixture.repoPath])
    }
  })
  ...
  test('a large modified set under the status cap stays responsive', async ({
    ...
    test.setTimeout(600_000)
    const modifiedFiles = Number(process.env.DORKA_LARGE_FILE_COUNT ?? '750')
  ...
  test('a change set over the status cap degrades to the too-many-changes state', async ({
    ...
    try {
      ...
      const activationMaxLagMs = await dorkaPage.evaluate(() => {
      ...
      const tooManyChangesBanner = dorkaPage.getByTestId('too-many-changes-banner')
      await expect(tooManyChangesBanner).toBeVisible()
      if (process.env.DORKA_LARGE_FILE_SCREENSHOT_PATH) {
        ...
        await tooManyChangesBanner.screenshot({
          path: process.env.DORKA_LARGE_FILE_SCREENSHOT_PATH
        })
      }
      ...
      const hugeState = await dorkaPage.evaluate(
  ...
  test('untracked line-stat cache stays effective up to the status cap', async ({
    ...
    try {
      ...
      const warmRescanPerFileMs = async (repoPath: string, files: number): Promise<number> => {
        const worktreeId = await addAndActivateRepo(dorkaPage, repoPath)
        const measurement = await measureSourceControlLoad(dorkaPage, {
          worktreeId,
          repoPath,
          expectedRows: files,
      ...
      await unregisterLargeFileCountRepos(dorkaPage, [
        smallRepo.repoPath,
  ...
  test('a large clean repo (tracked files only) loads instantly', async ({
    ...
    test.setTimeout(600_000)
    const trackedFiles = Number(process.env.DORKA_LARGE_FILE_COUNT ?? '15000')
