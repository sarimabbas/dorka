...
const PARKING_DELAY_MS = Number(process.env.DORKA_E2E_TERMINAL_PARKING_DELAY_MS) || 2_000
...
test.describe('terminal pane close vs hidden/park lifecycle keeps layout consistent', () => {
  test('control: close while visible', async ({ dorkaPage }) => {
    const { tabId } = await setUpSplitTab(dorkaPage)
    await closeLastPaneOnTab(dorkaPage, tabId)
    await expectLayoutConsistent(dorkaPage, tabId, 1, 'close-visible')
  })

  test('close and hide the tab in the same tick', async ({ dorkaPage }) => {
    const { worktreeId, tabId } = await setUpSplitTab(dorkaPage)
    ...
    await waitForTabRemounted(dorkaPage, tabId)
    await expectLayoutConsistent(dorkaPage, tabId, 1, 'close-then-hide-same-tick')
  })

  test('close while hidden but still mounted (hot-retain window)', async ({ dorkaPage }) => {
    const { worktreeId, tabId } = await setUpSplitTab(dorkaPage)
    ...
    await activateTerminalTab(dorkaPage, tabId)
    await waitForTabRemounted(dorkaPage, tabId)
    await expectLayoutConsistent(dorkaPage, tabId, 1, 'close-while-hidden-mounted')
  })

  test('close immediately after reveal remount, before panes settle', async ({ dorkaPage }) => {
    const { worktreeId, tabId } = await setUpSplitTab(dorkaPage)
    await createActiveTerminalTab(dorkaPage, worktreeId)
    await parkHiddenTabBehindDecoy(dorkaPage, worktreeId, tabId, {
      parkDelayMs: PARKING_DELAY_MS
    ...
    await expectLayoutConsistent(dorkaPage, tabId, 1, 'close-mid-reveal')
  })

  test('clean visible close survives a later park/reveal cycle', async ({ dorkaPage }) => {
    const { worktreeId, tabId } = await setUpSplitTab(dorkaPage)
    await closeLastPaneOnTab(dorkaPage, tabId)
    await expectLayoutConsistent(dorkaPage, tabId, 1, 'pre-park close')
    await createActiveTerminalTab(dorkaPage, worktreeId)
    await parkHiddenTabBehindDecoy(dorkaPage, worktreeId, tabId, {
      parkDelayMs: PARKING_DELAY_MS
    })
    await activateTerminalTab(dorkaPage, tabId)
    await waitForTabRemounted(dorkaPage, tabId)
    await expectLayoutConsistent(dorkaPage, tabId, 1, 'post-park-reveal')
  })

  test('split pane shell exits while hidden but still mounted', async ({ dorkaPage }) => {
    const { worktreeId, tabId, splitPtyId } = await setUpSplitTab(dorkaPage)
    await createActiveTerminalTab(dorkaPage, worktreeId)
    // The setup-script analog: the split's shell ends on its own while the
    // tab is hidden-but-mounted.
    await sendToTerminal(dorkaPage, splitPtyId, 'exit\r')
    await dorkaPage.waitForTimeout(PARKING_DELAY_MS / 2)
    await activateTerminalTab(dorkaPage, tabId)
    await waitForTabRemounted(dorkaPage, tabId)
    await expectLayoutConsistent(dorkaPage, tabId, 1, 'shell-exit-while-hidden-mounted', splitPtyId)
  })

  test('split pane shell exits while the tab is parked', async ({ dorkaPage }) => {
    const { worktreeId, tabId, splitPtyId } = await setUpSplitTab(dorkaPage)
    await createActiveTerminalTab(dorkaPage, worktreeId)
    await parkHiddenTabBehindDecoy(dorkaPage, worktreeId, tabId, {
      parkDelayMs: PARKING_DELAY_MS
    ...
    await sendToTerminal(dorkaPage, splitPtyId, 'exit\r')
    await dorkaPage.waitForTimeout(PARKING_DELAY_MS)
    await activateTerminalTab(dorkaPage, tabId)
    await waitForTabRemounted(dorkaPage, tabId)
    ...
    await expectLayoutConsistent(dorkaPage, tabId, 1, 'shell-exit-while-parked', splitPtyId)
  })
})
