...
test.describe('Notification settings', () => {
  ...
  test('can be toggled from settings and disables child controls', async ({ dorkaPage }) => {
    await openNotificationSettings(dorkaPage)

    const notificationsSection = dorkaPage.locator('[data-settings-section="notifications"]')
    ...
    await expect
      .poll(async () => (await getSettings(dorkaPage)).notifications.agentTaskComplete, {
        timeout: 5_000,
    ...
    await expect
      .poll(async () => (await getSettings(dorkaPage)).notifications.enabled, {
        timeout: 5_000,
    ...
    await expect
      .poll(async () => {
        const settings = await getSettings(dorkaPage)
        return {
