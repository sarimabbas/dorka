...
test('Linear filter chips keep readable names after the dropdown closes', async ({
  ...
  await expect
    .poll(() => getStoreState<string>(dorkaPage, 'activeView'), { timeout: 5_000 })
    .toBe('tasks')
  const filtersButton = dorkaPage.getByRole('button', { name: 'Filters', exact: true })
  await expect(filtersButton).toBeVisible()
  await expect(dorkaPage.getByText(FIXTURE.issue.title, { exact: true })).toBeVisible()

  await filtersButton.click()
  const popover = dorkaPage.locator('[data-slot="popover-content"]')
