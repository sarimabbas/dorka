...
whenever `DORKA_E2E_HEADLESS=1`, `DORKA_E2E_HEADFUL=1`, or `DORKA_BACKGROUND_LAUNCH=1`:
...
- Launch through `tests/e2e/helpers/dorka-app.ts` (or `dorka-restart.ts`) — they already set the env.
- A raw `electron.launch()` outside those helpers must pass `DORKA_BACKGROUND_LAUNCH: '1'`.
...
- Native-focus tests belong on an isolated display or CI. Do not set `DORKA_E2E_FOREGROUND=1`
...
- `DORKA_E2E_TERMINAL_PERF_XVFB=1`, set inside `xvfb-run` by that workflow;
...
Keep `DORKA_BACKGROUND_LAUNCH=1`: the application must still suppress automatic reveals and focus.
