...
// (DORKA_BUILD_IDENTITY='stable' | 'rc', DORKA_POSTHOG_WRITE_KEY=phc_...);
...
const dorkaBuildIdentity = process.env.DORKA_BUILD_IDENTITY
const DORKA_BUILD_IDENTITY_LITERAL =
  dorkaBuildIdentity === 'stable' || dorkaBuildIdentity === 'rc'
    ? JSON.stringify(dorkaBuildIdentity)
    : 'null'
const dorkaPostHogWriteKey = process.env.DORKA_POSTHOG_WRITE_KEY
const DORKA_POSTHOG_WRITE_KEY_LITERAL =
  typeof dorkaPostHogWriteKey === 'string' && dorkaPostHogWriteKey.length > 0
    ? JSON.stringify(dorkaPostHogWriteKey)
    : 'null'
const dorkaDiagnosticsTokenUrl = process.env.DORKA_DIAGNOSTICS_TOKEN_URL
const DORKA_DIAGNOSTICS_TOKEN_URL_LITERAL =
  typeof dorkaDiagnosticsTokenUrl === 'string' && dorkaDiagnosticsTokenUrl.length > 0
    ? JSON.stringify(dorkaDiagnosticsTokenUrl)
    : 'null'
...
;(() => {
  ...
  const mode = env?.DORKA_STARTUP_DIAGNOSTICS
  ...
  const diagnosticFile = env?.DORKA_STARTUP_DIAGNOSTICS_FILE
  ...
  if (!globalThis.__DORKA_BOOTSTRAP_EXIT_LOG_INSTALLED__) {
    globalThis.__DORKA_BOOTSTRAP_EXIT_LOG_INSTALLED__ = true
  ...
  if (mode === 'trace' && !globalThis.__DORKA_BOOTSTRAP_REQUIRE_TRACE_INSTALLED__) {
    globalThis.__DORKA_BOOTSTRAP_REQUIRE_TRACE_INSTALLED__ = true
    try {
      const Module = require('node:module')
      const originalLoad = Module._load
      const parsedTraceLimit = Number(env?.DORKA_STARTUP_DIAGNOSTICS_TRACE_LIMIT ?? 20000)
...
export const electronViteConfig: UserConfig = {
  main: {
    build: {
      ...
      rollupOptions: {
        ...
        input: {
          ...
          // this path for `dorka agent hooks ...`, so it must survive rebuilds.
    ...
    define: {
      DORKA_BUILD_IDENTITY: DORKA_BUILD_IDENTITY_LITERAL,
      DORKA_POSTHOG_WRITE_KEY: DORKA_POSTHOG_WRITE_KEY_LITERAL,
      DORKA_DIAGNOSTICS_TOKEN_URL: DORKA_DIAGNOSTICS_TOKEN_URL_LITERAL
    },
