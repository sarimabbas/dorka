...
This is the Dorka-server connection mode: the recipe emits a pairing URL. If the user chose SSH in
...
verdict="$(vercel sandbox exec "$auth" "${vercel_args[@]}" --timeout 30s \
  -- bash -lc 'if codex login status >/dev/null 2>&1; then echo DORKA_AGENT_LOGGED_IN; else echo DORKA_AGENT_LOGGED_OUT; fi')"
case "$verdict" in
  *DORKA_AGENT_LOGGED_IN*) ;;
...
recipe_id="${DORKA_RECIPE_ID:-vercel-sandbox}"
recipe_id="${recipe_id//./-}"  # Vercel names forbid dots.
instance_id="${DORKA_VM_INSTANCE_ID:-$(date +%s)}"
max_recipe_id_length=$((128 - ${#instance_id} - 6))  # Preserve the unique instance suffix.
[ "$max_recipe_id_length" -gt 0 ] || { echo "DORKA_VM_INSTANCE_ID is too long for a Vercel sandbox name" >&2; exit 1; }
...
vercel sandbox exec "$name" "${vercel_args[@]}" --timeout 20m \
  --env "GH_TOKEN=$gh_token" --env "DORKA_PROJECT_ROOT=$project_root" \
  --env "DORKA_REPO_URL=$repo_url" --env "DORKA_REPO_REF=$repo_ref" \
  -- bash -lc 'set -euo pipefail; cd "$DORKA_PROJECT_ROOT"; \
    ...
    git fetch origin "$DORKA_REPO_REF"; \
    git checkout -B "$DORKA_REPO_REF" FETCH_HEAD; \
    rm -f /tmp/askpass.sh; \
    c="$(git rev-parse HEAD)"; [ -f .dorka-built ] && [ "$(cat .dorka-built)" = "$c" ] || { \
      ...
      printf "%s" "$c" > .dorka-built; }' >&2

# 3. (remote) start dorka serve in the background, writing recipe JSON to a file; poll until it parses
recipe_json="$(vercel sandbox exec "$name" "${vercel_args[@]}" --timeout 60s \
  --env "DORKA_PORT=$port" --env "DORKA_PROJECT_ROOT=$project_root" --env "DORKA_PAIRING_ADDRESS=$pairing_ws" \
  -- bash -lc 'set -euo pipefail; cd "$DORKA_PROJECT_ROOT"; rm -f /tmp/dorka-recipe.json /tmp/dorka-serve.log; \
    nohup pnpm exec dorka-dev serve --port "$DORKA_PORT" --project-root "$DORKA_PROJECT_ROOT" \
      --pairing-address "$DORKA_PAIRING_ADDRESS" --recipe-json >/tmp/dorka-recipe.json 2>/tmp/dorka-serve.log </dev/null & \
    pid=$!; for _ in $(seq 1 80); do \
      node -e "JSON.parse(require(\"node:fs\").readFileSync(\"/tmp/dorka-recipe.json\",\"utf8\"))" >/dev/null 2>&1 && { cat /tmp/dorka-recipe.json; exit 0; }; \
      kill -0 "$pid" 2>/dev/null || { cat /tmp/dorka-serve.log >&2; exit 1; }; sleep 0.25; \
    done; cat /tmp/dorka-serve.log >&2; echo "serve recipe JSON timed out" >&2; exit 1')"
