...
Load this when the recipe connects over SSH instead of starting `dorka serve`, and when the user has
explicitly asked for `checkoutMode: provisioned-root`.

SSH mode is a different shape, not the Dorka-server templates relabeled. `create` runs no
`dorka serve` and emits no `pairingCode`. Dorka connects over its SSH relay, brings up the git and
filesystem providers, and imports the repo. The script only readies the host and prints the SSH
details Dorka dials.

## The result shape

Dorka rejects anything else. Required fields only; add optionals from the next section as the
network needs them.
...
These describe how the user's desktop reaches the box; there is no `dorka serve` URL in SSH mode.
...
- `relayGracePeriodSeconds` bounds how long Dorka keeps the SSH relay alive after the workspace
...
# 1. ensure the repo is present and at the right commit on the host (NO dorka serve here).
...
# 2. print the SSH connection block (NO pairingCode, NO dorka serve). host/port/username tell Dorka's
...
and resume. Dorka still disconnects and reconnects its own SSH relay on sleep, wake, and delete, which
...
`DORKA_RECIPE_RESULT_SCHEMA_VERSION`, `DORKA_REPO_URL`, `DORKA_REPO_REF`, `DORKA_REPO_REF_HEAD`, and
`DORKA_REPO_BRANCH`. Use `DORKA_REPO_REF` to fetch the selected source, but create `DORKA_REPO_BRANCH`
at the exact `DORKA_REPO_REF_HEAD` commit, because resolving the symbolic ref again can race with an
upstream update. `DORKA_REPO_URL` and `DORKA_REPO_REF` are a matched fetch pair, and the URL is the
remote Dorka resolved the base ref against, which is not necessarily named `origin` on the desktop.
Fetch from the URL the pair supplies:

```bash
[ -n "${DORKA_REPO_REF_HEAD:-}" ] || { echo "missing pinned source commit" >&2; exit 1; }
git fetch "$DORKA_REPO_URL" "$DORKA_REPO_REF"
git cat-file -e "${DORKA_REPO_REF_HEAD}^{commit}"
git checkout -B "$DORKA_REPO_BRANCH" "$DORKA_REPO_REF_HEAD"
```
