...
`.sh` will not execute there. Either require WSL or Git Bash and point `dorka.yaml` at a launcher such
as `bash ./scripts/dorka-vm/<name>.sh` through a `.cmd` file, or scaffold PowerShell equivalents.
...
# Dorka-server mode: @{ schemaVersion=1; pairingCode=$pairingCode; projectRoot=$projectRoot; userData=@{...} }
