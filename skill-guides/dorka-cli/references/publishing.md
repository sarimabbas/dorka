...
```text
DORKA artifacts share <file> --json
DORKA artifacts update <file> --json
DORKA artifacts unshare <file> --json
DORKA artifacts list [--cursor <cursor>] --json
DORKA artifacts delete <id> --json
```

- `share`, `update`, and `unshare` accept `.html`, `.htm`, `.md`, and `.markdown` files.
- `share` saves the returned edit token in the active Dorka profile and never includes it
  in CLI output. `update` and `unshare` look up that record by the resolved local file
  path, so use the same path and Dorka profile that originally shared the file.
...
- For local or staging development, `--api-url <url>` overrides the artifact service;
  `DORKA_ARTIFACTS_API_URL` provides the same override for the session.
- `DORKA_CLOUD_AUTH_TOKEN` is a development-only authentication override. Prefer the active
  Dorka profile's normal PropelAuth session and never expose the token in logs or agent output.

## Skill sharing

Agents can publish one or more installed skills behind one unlisted link through the
signed-in Dorka account. The user must first grant the separate, default-off permission in
Settings → Share Skills ("Allow agents and the Dorka CLI to publish skill links"). There is
...
DORKA skills share --skill <selector> [--skill <selector> ...] --bundle-name <name> --json
```
...
- Dorka stages one agent-published bundle at a time per host. If another publish is active,
  wait for it to finish before retrying `agent_skill_sharing_busy`.
- Run the command in an Dorka terminal on the machine that stores the skills. Forwarded WSL,
  SSH, and paired-runtime invocations fail before discovery so Dorka cannot read from the
  wrong filesystem.
