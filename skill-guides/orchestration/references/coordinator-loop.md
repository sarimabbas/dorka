...
```text
DORKA orchestration task-create --spec "<dependent work>" --deps <json_array> --json
DORKA orchestration task-list --ready --brief --json
```
...
```text
DORKA orchestration worker-start --task <task_id> --worktree current --agent claude --model opus --effort high --json
DORKA orchestration worker-start --task <task_id> --worktree current --agent muse --model muse-spark-1.3 --json
```
...
connected worker server must advertise launch-preference support before Dorka
...
DORKA orchestration worker-start --task <next_task_id> --terminal <agent_terminal_handle> --json
```
