...
`DORKA emulator exec --command "<serve-sim command>"`, which forwards the string to serve-sim
...
- An active session for the worktree before any input verb: run `DORKA emulator attach` or
...
Dorka reports a clear error when the host is missing macOS or the Xcode tools.
...
| ------------------------ | ----------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| List available / running | `DORKA emulator list --json`                                 | Dorka-managed sessions plus raw serve-sim streams. Use its ids for `--device` / `--emulator`.                                                                          |
| List devices everywhere  | `DORKA emulator devices --json`                              | Every backend's devices with a platform column, booted and shutdown.                                                                                                  |
| Attach / make active     | `DORKA emulator attach "iPhone 16 Pro" --json`               | Starts the helper if needed and makes the device active for the worktree. `--focus` switches the UI; it does not by default.                                          |
| Single tap               | `DORKA emulator tap <x> <y> --json`                          | Normalized 0..1 coordinates.                                                                                                                                          |
| Multi-step gesture       | `DORKA emulator gesture '<json>' --json`                     | Begin/move/end points. Use `tap` for a single tap.                                                                                                                    |
| Type text                | `DORKA emulator type "text" --json`                          | US-ASCII only.                                                                                                                                                        |
| Hardware button          | `DORKA emulator button home --json`                          | `home` and `side_button` are documented by the CLI spec; other names such as `swipe_home`, `app_switcher`, `lock`, and `siri` are forwarded to serve-sim unvalidated. |
| Rotate device            | `DORKA emulator rotate landscape_left --json`                | The orientation persists for subsequent gestures.                                                                                                                     |
| Accessibility tree       | `DORKA emulator ax --json`                                   | serve-sim node tree, capped at 500 nodes, frames normalized 0..1 with a top-left origin. Needs an active session.                                                     |
| Raw passthrough          | `DORKA emulator exec --command "ca-debug blended on" --json` | serve-sim subcommand string, without a `serve-sim` prefix.                                                                                                            |
| Stop the helper          | `DORKA emulator kill --json`                                 | Leaves the device booted.                                                                                                                                             |
| Stop and power off       | `DORKA emulator shutdown --json`                             | Stops the helper and shuts the simulator device down.                                                                                                                 |
...
DORKA emulator attach "iPhone 16 Pro" --json
DORKA emulator tap 0.5 0.8 --json
DORKA emulator type "user@example.com" --json
DORKA emulator button home --json
DORKA emulator ax --json
DORKA emulator exec --command "ca-debug blended on" --json
DORKA emulator kill --device "iPhone 16 Pro" --json
```

See also: `dorka-emulator-android` for Android devices, `dorka-cli` for terminals, worktrees,
