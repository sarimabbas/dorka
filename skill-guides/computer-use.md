...
Use this skill to drive a visible app window through `dorka computer`. Prefer a programmatic path (shell, filesystem, git, HTTP, existing CLIs) whenever it can complete the task; use this skill only when a visible window needs GUI control those cannot reach. Do not use it for Dorka's embedded browser (`dorka-cli`).
...
```text
DORKA computer capabilities --json
```

## Core Loop

```text
DORKA computer list-apps --json
DORKA computer get-app-state --app com.spotify.client --json
DORKA computer click --app com.spotify.client --element-index 42 --json
```
...
```text
DORKA computer get-app-state --app com.microsoft.edgemac --json
DORKA computer get-app-state --app Spotify --json
DORKA computer get-app-state --app pid:12345 --json
```
...
```text
DORKA computer permissions --json
DORKA computer capabilities --json
DORKA computer list-apps --json
DORKA computer list-windows --app <app> --json
DORKA computer get-app-state --app <app> --json
DORKA computer get-app-state --app <app> --restore-window --json
DORKA computer click --app <app> --element-index <index> --json
DORKA computer click --app <app> --x 100 --y 100 --json
DORKA computer click --app <app> --x 100 --y 100 --modifiers CmdOrCtrl+Shift --json
DORKA computer click --app <app> --element-index <index> --mouse-button right --json
DORKA computer click --app <app> --element-index <index> --mouse-button middle --json
DORKA computer perform-secondary-action --app <app> --element-index <index> --action <name> --json
DORKA computer set-value --app <app> --element-index <index> --value "text" --json
DORKA computer type-text --app <app> --text "text" --json
DORKA computer press-key --app <app> --key Return --json
DORKA computer hotkey --app <app> --key CmdOrCtrl+A --json
DORKA computer paste-text --app <app> --text "text" --json
DORKA computer scroll --app <app> (--element-index <index> | --x <x> --y <y>) --direction down --json
DORKA computer drag --app <app> --from-element-index <index> --to-element-index <index> --json
DORKA computer drag --app <app> --from-x 100 --from-y 100 --to-x 300 --to-y 300 --json
```
...
```bash
printf '%s' "$TEXT" | DORKA computer set-value --app <app> --element-index <index> --value-stdin --json
```
...
```text
DORKA computer get-app-state --app com.microsoft.edgemac --restore-window --json
DORKA computer set-value --app com.microsoft.edgemac --element-index <addressBarIndex> --value "test123" --json
DORKA computer press-key --app com.microsoft.edgemac --key Return --json
```
...
- `app_not_found`: run `list-apps` and retry with the bundle ID. If the target is a web app such as Gmail, choose the desktop browser app/window that contains it; do not retry `DORKA computer ... --app Gmail` unchanged because `dorka computer` app selectors refer to desktop apps, not website names.
...
- `screenshot_failed`: use `--no-screenshot` if tree state is enough; if the message names Screen Recording or screenshots permission, run `DORKA computer permissions --id screenshots --json`.
- `accessibility_error`: run `DORKA computer capabilities --json`; if the message names Accessibility permission, run `DORKA computer permissions --id accessibility --json`.
- Empty tree or no screenshot: app may have no visible window, be minimized, or need permissions.
- Permission errors: run `DORKA computer permissions --json`, or `DORKA computer permissions --id accessibility --json` / `--id screenshots --json` when the message names one permission, use the setup UI, then retry.
