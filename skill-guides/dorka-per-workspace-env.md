...
Inside the lifecycle scripts the placeholder does not apply: `dorka serve` written there runs on
the remote machine's own binary.

## Autonomy envelope

Without asking again you may read the repo and its `dorka.yaml`, detect provider CLIs and their
login state, scaffold and edit files under `scripts/dorka-vm/`, and run `DORKA vm recipe doctor`
...
done. Never create an Dorka workspace except for the step-10 test the user asked for. Do not create
...
In **Dorka-server** mode `create` runs `dorka serve` in the environment and emits a `pairingCode`. In
**SSH** mode `create` runs no server and emits a `connection.type:"ssh"` block Dorka dials into.
Settle this first; it changes the `create` output and half the templates.

Keep Dorka's checkout behavior unchanged by default: omit `checkoutMode`, emit schema version 1, and
let Dorka create a linked worktree. Use `checkoutMode: provisioned-root` only when the user
...
1. **Inspect the repo** for an existing `environmentRecipes` entry, `scripts/dorka-vm/`, a state
...
2. **Interview the user up front.** Gather these choices and confirm them back before scaffolding
   ...
   - **Connection mode:** an Dorka server or SSH, as above. Settle it first.
   ...
   - **Provider:** Vercel Sandbox, Fly, Modal, an existing SSH host, and so on. For a non-obvious
     ...
     Dorka's SSH mode needs the former.
...
7. **Wire the recipe** so `dorka.yaml` points create, suspend, resume, and destroy at the scripts.
   ...
   any branch; the picker needs `dorka.yaml` on the primary branch.
...
- Trap errors and remove the half-built environment, so a crash does not leave a paid resource
  running.
- **Never snapshot a machine on which the Dorka runtime has already run.** The first `dorka serve`
  ...
  (`dorka-devices.json`, `dorka-e2ee-keypair.json`), `agent-session-authority.key`, and the build
  ...
  `dorka_user_data_path="${DORKA_USER_DATA_PATH:-${XDG_CONFIG_HOME:-$HOME/.config}/dorka}"`.
  Resolve symlinks and inspect that path before deleting it: it must be an absolute directory
  dedicated to Dorka runtime data, never `/`, the home directory, or an ancestor of home. Refuse
  empty or relative paths. Remove only that verified directory, not an unchecked environment value.
  That matches Dorka's Linux precedence for custom and default paths; deleting a named file list
  drifts as Dorka adds state.
...
Section 3's rule still applies: if you ran `dorka serve` on this machine to smoke-test it, delete
...
A repo-local JSON file such as `scripts/dorka-vm/<provider>-state.json` threads non-secret values
...
Scaffold under `scripts/dorka-vm/`. These are shapes; fill in the provider's real commands. **Every
...
You run this by hand, not via `dorka.yaml`, after exporting the first-run inputs state does not have
...
# name = dorka-${DORKA_RECIPE_ID}-${DORKA_VM_INSTANCE_ID} (sanitized, length-capped)
...
# 3. Dorka-server mode only: remote exec starting dorka serve and reading the recipe JSON it writes
...
#!/usr/bin/env bash
set -euo pipefail
payload="$(cat)"                       # Dorka passes lifecycle JSON on stdin
...
# destroy: provider remove "$resource_id"   (or set destroy: none in dorka.yaml)
```
...
Define recipes in `dorka.yaml`:

```yaml
environmentRecipes:
  - id: cloud-sandbox
    name: Cloud Sandbox
    create: ./scripts/dorka-vm/cloud-sandbox-create.sh
    suspend: ./scripts/dorka-vm/cloud-sandbox-suspend.sh
    resume: ./scripts/dorka-vm/cloud-sandbox-resume.sh
    destroy: ./scripts/dorka-vm/cloud-sandbox-destroy.sh
```
...
The base result, which is what Dorka-server mode prints:

```json
{
  "schemaVersion": 1,
  "pairingCode": "dorka-pairing-code-or-url",
...
- **`dorka serve --recipe-json` output** is this same object without `userData`. Merge your own
...
- **SSH mode** replaces `pairingCode` and `projectRoot` with a `connection` block whose `type` is
  `"ssh"`, and does not run `dorka serve`. The exact target shape is in `references/ssh-host.md`.
- **Provisioned root** applies only to direct SSH and only when the user explicitly asked for it. Add
  `checkoutMode: provisioned-root` to the recipe, require `DORKA_RECIPE_RESULT_SCHEMA_VERSION=2`, and
...
### The `dorka serve` invocation

Inside the environment, in Dorka-server mode, run exactly this. These flags are verified; do not
improvise them.
...
In an environment built from source, run it as `pnpm exec dorka-dev serve …` from the repo root;
`dorka-dev` is the in-repo entrypoint. Plain `dorka serve …` is the same command when the built CLI is
...
`DORKA vm recipe doctor <recipe-id> --repo-path <repo> --json` validates static wiring only; it boots
...
run `DORKA skills get dorka-per-workspace-env --reference references/<file>.md` and read only that
document; `--references` lists the names. Read the reference at the gate, not before. If the CLI
rejects `--reference`, run `DORKA skills get dorka-per-workspace-env --full` once instead: it returns
...
| The recipe connects over SSH instead of starting `dorka serve`, including provisioned root | `references/ssh-host.md`        |
