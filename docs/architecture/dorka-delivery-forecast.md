...
`feat/dorka-computers` through `09bd41365` and the evidence listed below. A checked item means the
...
| P1       | Now     | Feature     | Expose existing safe Computer creation, ordinary environment, resource, and operator-allowlisted premount configuration through the completed redacted revisioned get/plan/replace RPC and a pending confirmation UI. Keep secrets out of environment and engine arguments. | Redacted UI and mixed-version capability handling                | 2–4 days               |
...
| ✅     | Revisioned Computer configuration backend redacts environment values and atomically plans/replaces resources, ordinary environment, and allowlisted premounts                                                           | configuration/service/store/manager/RPC contracts and 50 focused Computer tests; commits `65e2fb4f7`, `8905ebdb6`, `09bd41365`                                                                                                         |
