...
`feat/dorka-computers` through `91e35c152` and the evidence listed below. A checked item means the
...
exit publication while the Server is offline remains the only P0 acceptance blocker: the fixture exits,
but the tracked PTY stays alive and no pending certificate is published.
...
| P0       | Now     | Validation  | Prove host-certified PTY exit replay across a Server outage on native Linux. First isolate why the terminating fixture leaves the tracked PTY alive, then verify journal creation, Server replacement, exact `running → waiting`, projection-before-ack, acknowledgement replay, and all negative unverifiable cases with disposable Computer volumes. | Fast PTY/certificate reproducer; production Computer entrypoint and relay | 1–2 engineering days   |
...
| ⬜     | Server-offline exit replay and projection-before-ack native proof                                                                                                                                                       | Blocked at `timed out waiting for pending certificate`: the fixture exits but the tracked PTY remains alive. Build a focused reproducer before rerunning the full release gate.         |
...
- **Exit-fixture ownership:** killing the fake Agent process has not closed the relay-owned PTY, so no
  exact exit certificate is published. Diagnose PTY/process-group ownership in a fast isolated
  reproducer; do not weaken the exact-generation or exact-incarnation checks.
