...
`feat/dorka-computers` through `09c4e4384` and the evidence listed below. A checked item means the
...
| P1       | Now     | Feature     | Finish versioned Agent skill/MCP references. The v2 roster migration, portable strict reference schema, Agent revision, and Run revision snapshot are complete; next add CAS update RPC, Computer-local launch resolution, capability negotiation, and compact UI.                        | Existing skill discovery and SSH filesystem providers          | 3–5 days               |
...
| 🟡     | Versioned portable Agent skill/MCP reference foundation                                                                                                                                                                 | Strict names-only reference schema, v1→v2 roster migration, Agent revision, and Run revision snapshot; focused migration/schema/execution tests; commit `09c4e4384`. Launch resolution, update RPC, and UI remain open.                |
