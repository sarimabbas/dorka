...
ship alongside Dorka's source code.

Open-source product documentation for [Dorka](https://www.ondorka.dev), served at `/docs` (same URL shape as `https://www.ondorka.dev/docs`).
...
`www.ondorka.dev`; configure its Next/Vercel proxy with these `beforeFiles`
...
5. Prepare the three rewrites above in the `www.ondorka.dev` marketing project,
