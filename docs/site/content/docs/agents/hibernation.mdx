...
description: Let Dorka pause idle background agent terminals and auto-resume them when you reopen the worktree.
---

import { Callout } from '@/components/docs/prose'

When you keep dozens of worktrees open, idle agents add up — each one is a live PTY holding a model session in memory. Agent hibernation lets Dorka quietly stop those terminals once they've been done and untouched long enough, then resume the same session the next time you open the worktree.
...
Dorka only hibernates an agent terminal when **all** of the following are true:
...
Under the **Agent hibernation** toggle, **Hibernate after** sets how many idle minutes a finished background agent has to wait before Dorka pauses it.
...
- [Agent session history](/docs/agents/session-history) — manually resume any past session, including ones Dorka didn't hibernate.
