...
description: Pair the Dorka mobile app to your desktop to monitor agents and unstick worktrees from your phone.
---

import { Callout } from '@/components/docs/prose'

The Dorka mobile companion is an iOS/Android app that pairs with your desktop Dorka and gives you a read-mostly view of running agents — agent status, recent terminal scrollback, and the controls you actually want from a phone (replying to a prompt, sleeping a worktree, reviewing source control, switching agent accounts). Pairing is one-time and the desktop is always the source of truth.

<Callout title="Beta">
  The mobile companion is in beta. Install iOS from the [App
  Store](https://apps.apple.com/us/app/dorka-ide/id6766130217), join the [TestFlight preview
...
- See every worktree, its agent, and its current status (working / done / waiting on input). Mobile lists worktrees from every host you're connected to (local desktops and remote Dorka servers) in a single view, so switching between them doesn't need a device change.
...
1. On desktop, open Dorka's pairing flow from the account / status menu. Dorka shows a one-time pairing code.
1. On mobile, open the Dorka companion app and choose **Pair**, then paste the code. (You can also follow a deep link from the desktop directly into the mobile pairing screen.)
1. Prefer **Dorka Relay** for pairing when it is available — sign-in is required for Relay only. The local network address picker is optional on Relay (expand it when you want a LAN fast path); LAN pairing still needs an address.
...
For chat-capable agents (Claude, Codex, and others Dorka recognizes), mobile can open the session in **Chat UI** instead of the raw terminal — the same idea as desktop's [Chat UI](/docs/agents/native-chat): a readable transcript, composer, permissions/questions, and image attachments.
...
- **Pairing fails** — make sure your desktop and phone are signed into the same Dorka account. Pairing codes expire after a few minutes; generate a fresh one if it's been sitting on the screen.
...
<Callout title="Next steps">
  ...
  alerts reach the right device. For headless machines, use [Remote Dorka
