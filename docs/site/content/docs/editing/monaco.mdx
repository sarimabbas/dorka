...
Dorka's code editor is Monaco — the same editor VS Code uses — with a few Dorka-specific tweaks.
...
Syntax highlighting ships for the languages Monaco supports out of the box. Dorka is intentionally editor-first, not IDE-first — run type-checkers and linters in a terminal pane.
