...
- For [SSH worktrees](/docs/ssh), drag-drop also works — Dorka uploads the file to the remote host before completing the drop, so the agent sees the file as a real on-disk path.
...
Right-click a single file and choose **Copy** to place the file itself on the OS clipboard. For SSH worktrees, Dorka first stages the remote file locally, then writes that staged file reference to the clipboard; remote folders are excluded.

## Download (SSH / remote)

On the desktop app, right-click a remote file → **Download**, or a remote folder → **Download Folder** when the connection supports recursive transfer. Dorka opens a native save/folder dialog. Not available in the web client. Local worktrees do not show this action.
