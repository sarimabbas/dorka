...
Design Mode turns the Dorka browser into a pointer-to-code tool. Toggle it on, click any UI element on the rendered page, and the element drops into the agent chat as rich context — with its DOM, computed styles, and a screenshot.
...
Click an element. Dorka captures:
...
The agent edits the source, Dorka hot-reloads, you click again to verify. The tightest version of this loop is the hero recipe [Fix a UI bug with Design Mode](/docs/recipes/design-mode-fix).
