...
The entry is on by default. It shows an unread badge when new events have arrived since you last visited; clicking an entry jumps you to the matching worktree and pane — the same handoff Dorka does from a system notification.
...
The Agents feed doesn't replace the [header bell](/docs/notifications) or system notifications — those still fire on agent-finished pings. The feed is the catch-up surface when you've been away from Dorka: instead of scrolling each worktree to find what changed, scan it.

## Next steps

- [Notifications & Inbox](/docs/notifications) — system pings, the header bell, and per-category tuning.
- [Agents & sessions](/docs/model/agents-sessions) — how Dorka tracks the working/idle/blocked transitions this feed surfaces.
