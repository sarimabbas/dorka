...
Dorka's pane system is designed for watching multiple agents work without losing context. Tabs group into panes; panes split into layouts.
...
Remap under [Settings → Shortcuts](/docs/settings). Existing installs keep customized overrides in `~/.dorka/keybindings.json`.
