...
description: Quit Dorka, reopen it, and pick up exactly where you left off — worktrees, splits, scrollback, focused tab.
---

import { Callout } from '@/components/docs/prose'

When you close Dorka, the next launch rehydrates the whole workspace: every open worktree, every terminal split, the scrollback in each pane, and the tab you had focused. You shouldn't have to remember which agents you had running on which branches — that's Dorka's job.
...
- **Running agent processes** — agents you had running keep running across an Dorka quit. A background daemon owns the PTYs, so closing the app window doesn't kill Claude Code, Codex, or any other agent CLI mid-task. On next launch, Dorka warm-reattaches to the same processes.
- **Terminal scrollback** — the buffer in each terminal, including output produced while Dorka was closed.
- **Focused worktree and tab** — Dorka opens to the same view you closed.
...
A daemon crash while Dorka is closed has the same effect for any sessions it was holding, but the layout and scrollback still restore on next launch.
...
- **Auto-updater relaunch** — Dorka restarts to install an update; agents are unaffected.
- **App crash** — if Dorka itself crashes, the daemon keeps your sessions alive for warm reattach on next launch.
...
<Callout title="Starting fresh">
  ...
  session" mode — Dorka always restores. Closed worktrees stay closed.
</Callout>
