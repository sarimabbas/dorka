...
description: Diagnose rate limits, auth failures, and other GitHub CLI errors in Dorka PR checks, Tasks, and Source Control.
---

Dorka talks to GitHub through the **GitHub CLI (`gh`)** on your machine (or remote Dorka host). When PR status, checks, issues, or Tasks fail to refresh, the cause is almost always GitHub auth, permissions, or API rate limits — not a broken PR panel by itself.
...
| ----------------------------------------------------------------- | ---------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- |
| “GitHub is rate-limiting requests” / “rate limit exceeded (core)” | GitHub REST (core) quota exhausted for your user           | Wait for reset; stop extra `gh` / agent / Dorka usage; check [Settings → Git → GitHub API Budget](/docs/settings) |
...
| “GitHub CLI is unavailable”                                       | `gh` missing from PATH Dorka uses                           | Install `gh` and restart Dorka                                                                                    |

## Rate limits (most common)

GitHub gives each **authenticated user** a shared hourly budget. **Every tool on that account shares it**: Dorka, `gh` in terminals, Claude/Codex/Grok agents that call `gh`, CI scripts, browser extensions, and other apps.

### Buckets Dorka cares about
...
When a primary bucket is exhausted, GitHub returns HTTP **403** with a message like `API rate limit exceeded`. Dorka classifies that as rate-limited, keeps the last known PR status when it can, and **stops spawning more `gh` calls** for a short window so a single limit doesn’t turn into a storm of failures.
...
- Many Dorka windows or electron-dev builds open at once (each may refresh PRs / Tasks)
...
2. **Reduce concurrent GitHub clients** — quit extra Dorka instances and pause bulk `gh` automations.
3. Avoid hammering refresh on the PR panel while limited; Dorka is already backing off.
...
Dorka inherits whatever `gh` uses on that host.
...
`gh auth status` may show the token as invalid. Run `gh auth login` (or `gh auth refresh`) and restart Dorka so it picks up the new credentials.
...
GitHub auth is **per host**. Logging in on your laptop does not log in `gh` on a remote machine. SSH into the host and run `gh auth login` there, or use Dorka’s remote-server GitHub budget view for that environment.
...
If Dorka reports the GitHub CLI is unavailable:

1. Install [`gh`](https://cli.github.com/)
2. Confirm `which gh` works in a normal terminal
3. Fully quit and reopen Dorka (so PATH matches)
4. On Windows, install for the same environment Dorka launches (WSL vs native)

## How Dorka behaves when GitHub fails
...
- **Circuit breaker**: After a primary rate-limit 403, Dorka briefly refuses new `gh` spawns for that bucket (`core`, `search`, or `graphql`) so the app stays responsive and doesn’t dig a deeper hole.

## Check GitHub API Budget in Dorka
...
- On remote Dorka servers, use the remote advanced budget view for the **server-owned** `gh` identity (local Settings only shows the desktop client)

## Still stuck?

1. Reproduce once in a terminal with `gh pr view` or `gh api user` from the same machine/user Dorka uses.
...
- [Troubleshooting & FAQ](/docs/troubleshooting) — general Dorka issues
