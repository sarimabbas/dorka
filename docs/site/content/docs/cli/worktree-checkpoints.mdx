...
Every Dorka worktree carries a lightweight, free-text **comment** field visible in the UI — a status snapshot of what the worktree is doing right now. Agents can update it from the CLI, and it's the pattern we recommend for keeping human collaborators in the loop without forcing chat.

## The pattern

```
dorka worktree set --worktree active --comment "reproduced auth failure; testing credential-chain fix" --json
```
...
```bash
dorka worktree set --worktree active \
...
```
dorka worktree set --worktree active --comment "added debounce to SearchBar onChange (src/components/SearchBar.tsx); ready for review
