...
Orchestration is Dorka's structured multi-agent layer: a **Run** (namespace + coordinator inbox), **Tasks**, **Dispatches**, supervised **workers**, messages, and decision gates.

Use it when you need ownership, completion tracking, or a DAG. For one-off prompts, use `dorka terminal send`. For full ownership handoffs without supervision, use worktree/terminal commands from the `dorka-cli` skill.

<Callout title="Experimental">
  Enable orchestration under Settings → Experimental before using these commands. The CLI talks to
  the running Dorka runtime, so `dorka status --json` should succeed first.
</Callout>

<Callout title="Legacy commands retired">
  `dorka orchestration run` and `run-stop` (and `coordinator-start` / `coordinator-stop`) perform
  **no effects**. They return recovery text pointing at `dorka skills get orchestration --full`. Use
...
Task IDs printed in terminals, such as `task_...`, are clickable links. Clicking one asks the Dorka runtime for the task's current dispatch and focuses the assigned terminal, including when the task lives in a remote or SSH runtime.

## Preferred supervised loop

```bash
dorka orchestration run-create --objective "Split checkout QA and summarize blockers" --json
dorka orchestration task-create --spec "Audit billing settings for mobile layout" --task-title "Billing audit" --json
dorka orchestration worker-start --task <taskId> --worktree current --agent codex --json
# or new worktree:
dorka orchestration worker-start --task <taskId> --worktree new-child --name billing-audit --agent codex --setup run --json
# optional per-worker model / effort (Claude, Codex, Cursor, Antigravity, Muse; not with --terminal):
dorka orchestration worker-start --task <taskId> --worktree current --agent claude --model <opaque-model-id> --effort high --json
```

`--agent` takes any Dorka agent ID enabled on the worker server, for example `claude`, `codex`, `cursor`, `antigravity`, `muse`, `opencode`, or `opencode2`. `--model` accepts opaque provider model IDs for Claude, Codex, Cursor, Antigravity, and Muse (for example `--agent muse --model muse-spark-1.3`); other agents, including opencode, run the model from their own config. `--effort` requires `--model` and only applies when that agent/model supports the level. Neither flag can combine with `--terminal` (reuse an existing pane). Overrides apply to that launch only and show under `launch.requested` / `launch.effective` in the start receipt. Federated starts need a worker host that advertises launch-preference support.
...
dorka orchestration check --ack <deliveryId> --wait --types worker_done,escalation,question --timeout-ms 900000 --json
```

Worker completion (from the worker pane; include injected IDs):

```bash
dorka orchestration send \
  --type worker_done \
...
```bash
dorka orchestration worker-show --dispatch <dispatchId> --json
dorka orchestration worker-read --dispatch <dispatchId> --limit 50 --json
dorka orchestration worker-stop --dispatch <dispatchId> --json
...
dorka orchestration worker-release --dispatch <dispatchId> --json
# Keep a settled worker live for debugging when the user asked to retain it:
dorka orchestration worker-retain --dispatch <dispatchId> --json
# retry placement is explicit — --retry-of does not inherit --on/worktree:
dorka orchestration worker-start --task <taskId> --retry-of <dispatchId> --worktree current --agent codex --json
```
...
```bash
dorka orchestration worker-start \
  --task <taskId> \
  --on windows \
  ...
  --setup run \
  --json
dorka orchestration send --to dispatch:<dispatchId> --subject "Follow-up" --body "…" --json
```
...
```bash
dorka worktree create --name billing-audit --agent codex --json
dorka terminal wait --terminal <workerHandle> --for tui-idle --timeout-ms 60000 --json
dorka orchestration dispatch --task <taskId> --to <workerHandle> --inject --json
```
...
```bash
dorka orchestration send --to @all --subject "Heads up" --body "Pausing dispatches for a review." --json
dorka orchestration send --to @idle --subject "Anyone free?" --json
dorka orchestration send --to @codex --subject "Codex workers in this Run only" --json
```
...
- Use `dorka orchestration ask` for blocking questions instead of local TUI prompts.

```bash
dorka orchestration ask \
...
```bash
dorka orchestration gate-create \
  --task <taskId> \
  --question "Merge the shared button change into the task branch?" \
  --options '["yes","no"]' \
  --json

dorka orchestration gate-resolve --id <gateId> --resolution "yes" --json
```

## Recovery

```bash
dorka orchestration dispatch-show --task <taskId> --json
dorka orchestration dispatch-show --task <taskId> --preamble --json
dorka orchestration task-list --json
dorka orchestration task-update --id <taskId> --status blocked --result '{"reason":"waiting on credentials"}' --json
```

Reset only when you are intentionally abandoning orchestration state:

```bash
dorka orchestration reset --tasks --json
dorka orchestration reset --messages --json
dorka orchestration reset --all --json
```
...
Use `dorka terminal send` for a lightweight prompt to an agent you are watching.

Use `dorka orchestration worker-start` (or `dispatch --inject`) when a worker must report `worker_done`, ask questions through the coordinator, and be tracked by task ID.

Use `dorka orchestration run-create` + tasks + workers when you want a durable Run namespace and supervised multi-agent loop — not the retired `orchestration run` command.
