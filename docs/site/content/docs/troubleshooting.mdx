...
- Open the terminal and run the agent's CLI manually. If it fails there, it's an auth or install problem in the CLI itself — not Dorka.
- Make sure the CLI is on the `PATH` Dorka sees (check [Settings → Agents](/docs/settings)).
- Try the **Restart** chip on the tab.

## Diff view looks wrong / stuck

- Click the refresh icon on the diff toolbar — Dorka re-reads the worktree.
...
## Dorka CLI says "command not found"

Register the CLI under [Settings → General → Dorka CLI](/docs/settings). On macOS it installs a shim into `~/.local/bin`; make sure that's on your shell's `PATH`.
...
- Reconnect after installing tools so Dorka can reinstall native modules.
...
- Use an SSH worktree (not a Remote Dorka Server active runtime).
...
No tab is open in the current worktree. Open one with `dorka tab create --url ...` or open the browser pane manually and navigate.
...
**Help → Open Logs** opens the directory with Dorka's logs. Attach these when filing a bug.
