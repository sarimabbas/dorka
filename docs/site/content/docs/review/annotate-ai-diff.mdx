...
Annotate AI Diff is Dorka's inline review loop for agent-generated code. You leave comments on any line of any AI-generated hunk, then send them back to the agent as a single batch for revision — no copying line numbers, no context-switching.
...
Comments pin to the exact line; Dorka tracks them across edits so they follow the line if the diff shifts.

## Send the batch

When your review is done, click **Send to agent** at the top of the diff. Dorka composes a single prompt with all your comments, line-anchored, then opens a **Send notes to** menu for the worktree's available agents. Pick the agent that should revise the change, or start a new agent from the same menu.
