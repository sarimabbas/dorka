---
title: Commit & push from Dorka
---

You can commit, push, and open the review without leaving Dorka. The commit panel lives next to the diff viewer and is designed for the common case — review, stage, commit, push, move on.

## Commit

1. Stage changed files from the Source Control panel.
1. Write a commit message in the bottom panel, or use **Generate with AI** when you want Dorka to draft one from the staged changes.
...
Pre-commit hooks from the repo run as usual. If a hook fails Dorka surfaces the output inline.
...
**Push** pushes the worktree's branch to `origin`, setting upstream the first time. If the branch is behind, Dorka will not silently force-push.
...
Amending is explicit — **Commit → Amend**. Dorka won't amend commits that have already been pushed unless you confirm.
