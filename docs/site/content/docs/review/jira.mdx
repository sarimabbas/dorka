...
Jira sits next to GitHub and Linear in the task drawer. Browse Jira issues, update them, and create a worktree from any issue without leaving Dorka.
...
<Callout title="Use HTTPS">
  Use an `https://` URL for Cloud and self-hosted Jira whenever possible. Dorka sends credentials in
...
4. Click **Connect**. Dorka verifies the credentials and loads your sites.
...
- From the **Create workspace** dialog you can also paste a Jira issue URL (`https://…/browse/ABC-123`) into the name field, or switch the field to **Jira** search and pick an issue by text. Dorka fills the workspace name, links the issue, and shows the key + summary on the worktree card with **View on Jira**. Paste is multi-site aware: when more than one connected site matches the URL origin, Dorka asks which site to use; when none match, it says the site is not connected.
- Dorka remembers your last-used task source per repo, so a Jira-driven repo defaults to Jira on next open.
...
<Callout title="Where credentials live">
  ...
  account settings if you stop using Dorka.
</Callout>
