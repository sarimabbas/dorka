...
On native Windows, Dorka writes the `dorka.yaml` setup script (and the issue command) to a generated
...
- Two people on the same repo got different interpreters for the same `dorka.yaml`, so no project
...
A `#!` line only takes effect when Dorka can actually launch bash from the configured terminal — the
