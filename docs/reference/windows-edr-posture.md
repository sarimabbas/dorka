...
Dorka's Windows process tree is shaped like the thing behavioural EDR is built to
find. An enterprise Windows 11 / Intune tenant opened **six Microsoft Defender
for Endpoint incidents against Dorka 1.4.192 in eight days**. All six fired as
...
> **Defender is not malfunctioning. It is describing the code accurately.** Dorka
...
that Dorka is malware. It is a map of which of our behaviours are legible to an
...
| ----------------- | --------- | ------------------------------------------------------------------------------------------------------------------------------------- |
| **Update**        | A, B, C   | `dorka-windows-setup.exe` → `old-uninstaller.exe`, `Uninstall Dorka.exe` (electron-builder generates these; they are in no repo file)   |
| **Spawn**         | all six   | `Dorka.exe` → `dorka-terminal-daemon.exe` → `powershell.exe` / `pwsh.exe` / `cmd.exe` / `reg.exe` → `claude.exe`, `gh.exe`, `codex.cmd` |
...
In the update cluster the uninstaller is genuinely `NotSigned`, while `Dorka.exe`
and `dorka-terminal-daemon.exe` report `Valid CN=SignPath Foundation`.
...
`%LOCALAPPDATA%\Dorka\daemon-host\<version>\` and forks the terminal daemon from
there.
...
`Dorka.exe` rather than `node.exe` so there is no console flash and asar still
resolves; `config/nsis/dorka-installer-hooks.nsh` reaps it on a real uninstall
...
`dorka-terminal-daemon.exe`, the image name every incident here reports, and
`DAEMON_HOST_EXE_NAME`'s comment stated the reason without varnish: _"so the NSIS
updater's `taskkill /IM Dorka.exe` can't match it."_ The rename has since been
...
every pid, including `lsass.exe`. Restricting the command-line pass to Dorka's own
...
`Dorka.exe` → the relocated daemon host (`dorka-terminal-daemon.exe` in the builds
these incidents cover, `Dorka.exe` since) → a shell → an agent CLI is what a
...
breadth: every interpreter hop between Dorka and the thing the user asked for adds
...
decoded payload was a `Test-Path` and a call to `~/.dorka/agent-hooks/claude-hook.cmd`.
...
70% from Claude sessions that were not running under Dorka at all and whose hook
exits at its first `DORKA_PANE_KEY` guard.
...
flagged, while Dorka's is signed and was flagged six times.** Their conclusion:
...
on install count alone. Dorka's signature is a free OV certificate from SignPath
...
- **Shorten the interpreter chain.** Each hop between Dorka and the user's actual
...
## Guidance for administrators deploying Dorka
...
`%LOCALAPPDATA%\Programs\dorka\` to the AV exclusion list and expecting the
...
- **Alert titles** — `A suspicious file was observed` and
  ...
  this list.
- **File paths** — `Dorka.exe` and `dorka-terminal-daemon.exe` under
  `%LOCALAPPDATA%\Programs\dorka\` and `%LOCALAPPDATA%\Dorka\daemon-host\`.

Scope it as narrowly as your tenant will tolerate, and review it when Dorka
...
`%LOCALAPPDATA%\Dorka\daemon-host\` is the normal location rather than a
guaranteed one. Prefer a prefix match on `…\Dorka\daemon-host\` over a rule
...
criterion"** enabled. If it is, that alone explains a freshly signed Dorka build
...
allowlist the Dorka install paths for that rule or expect a hit on each update.
...
Every part of that is an accurate description of what the feature does. Dorka's
...
- **Allowlist it**, with a suppression rule covering the computer-use tree
  (`powershell.exe` with `-File …\runtime.ps1`) as well as the base Dorka paths,
