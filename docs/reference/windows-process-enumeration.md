...
Dorka needs three things from the Windows process table: who a PID's parent is
...
relay's bare addon, which has no queue of its own, it is also what keeps Dorka
...
`DORKA_REQUIRE_RELAY_NATIVE_ADDONS` is a per-arch list so a future arch can be
...
1. **Spectre mitigation.** The upstream `binding.gyp` requires Spectre-mitigated
   libraries, which Dorka's Windows build agents do not install. `node-pty` is
...
Windows 10, so every OS Dorka supports has it. The entry point is resolved with
...
pass to Dorka's own subtree is the complementary change, and it belongs with the
...
`@dorka/windows-registry` (asserted by
...
addon can produce is unusable for the sizes Dorka now sees: `process.cc` stores
...
Start time is a proxy for identity, not identity. For the process trees Dorka
itself spawns the durable answer is still an inherited handle: a job object
names the tree Dorka created, so no start-time comparison is needed. The
`creationTimeMs` this snapshot now carries is for the trees Dorka did **not**
