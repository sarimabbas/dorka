...
`dorkad` is the Dorka runtime served from plain Node. This is the contract between it and
...
A deployment is **dorkad** plus **the terminal daemon**.

|            | dorkad                            | terminal daemon                       |
| ---------- | -------------------------------- | ------------------------------------- |
| Started by | the supervisor                   | dorkad, detached                       |
| Owns       | RPC, git, worktrees, persistence | every local PTY                       |
| Lifetime   | one supervised run               | detached from dorkad, not its service  |
| Endpoint   | `ws://<bind>:<port>`             | `<data-root>/daemon/daemon-v<N>.sock` |

dorkad detaches the daemon and calls `disconnectDaemon()`, never `shutdownDaemon()`. The
built-in remote deployment path stops only the recorded dorkad PID, so the daemon and its PTYs
...
Process detachment is not service isolation. A daemon that dorkad launches directly, and every
...
Service-restart survival therefore requires a separately supervised cgroup, and dorkad now asks
for one: on Linux it launches the daemon through `systemd-run --user --scope`, which places the
daemon and its PTYs in their own transient `dorka-daemon-<launch-nonce>.scope` unit under the
...
`dorka serve`'s wide default, and a startup where some device has connected before — and an
...
Under the shipping design a client reaches a remote dorkad over an SSH local port-forward, so
loopback is the correct default and the pairing credential travels over SSH.

## Data root and the instance lock

The data root is `$DORKA_USER_DATA`, else `$XDG_DATA_HOME/Dorka`, else `~/.dorka`.

Before the profile index or the store is touched, dorkad takes `<data-root>/dorkad.lock`.
It refuses to start when:

| Code                                   | Meaning                                                       |
| -------------------------------------- | ------------------------------------------------------------- |
| `dorkad_data_root_wrong_owner`          | the root is owned by another uid (POSIX)                      |
| `dorkad_data_root_shared`               | the root is group/world accessible and could not be tightened |
| `dorkad_instance_lock_held`             | another live dorkad owns this root                             |
| `dorkad_instance_lock_foreign_identity` | the lock belongs to a different identity                      |
| `dorkad_data_root_unusable`             | the root cannot be created, stat'd or written                 |

A root that is merely too permissive and that we own is tightened to `0700` rather than
refused — dorkad stores credentials there unsealed (no OS keyring on this host), so the goal
...
`health.terminalDaemon.cgroupUnit` names an `dorka-daemon-*.scope` on that host.

Before a cgroup-wide stop, obtain a fresh `dorka-ide terminal list --json` result using the same OS
...
`sudo -Hu dorka /home/dorka/.local/bin/dorka-ide terminal list --json`). Replace both `dorka` and
`/home/dorka` with the service account and home used by the unit; an extracted deployment may use
its absolute `resources/bin/dorka-ide` launcher instead. A safe empty census is untruncated, has an explicit `hostScope`, covers every
...
not admit new work after the census. Dorka does not yet provide an atomic census-and-stop fence.

### Who supervises dorkad

An external supervisor (systemd, launchd, a process manager). dorkad conforms to it:

- **Readiness.** One JSON line on stdout (`--json`), `type: "dorka_server_ready"`, published
...
- **Shutdown.** `SIGTERM` or `SIGINT` starts a graceful stop. A **second** signal exits
  ...
  the one teardown that skips the daemon handoff. dorkad also imposes its own 15s deadline
...
- **Logs.** dorkad writes human-readable diagnostics to **stderr** and its readiness contract
  ...
  `DORKA_DIAGNOSTICS_DISABLED=1`). Rotation of that file is not implemented — see
  [What is not covered](#what-is-not-covered).

### dorkad supervising the daemon
...
- **Launch.** On Linux, through `systemd-run --user --scope` so the daemon gets its own
  ...
  `dorkad.js` with its own PID record, token and socket under `<data-root>/daemon`.
...
- **Crash-loop containment.** At most **5 launches per 60s rolling window** per dorkad run;
  ...
  recovers without restarting dorkad. An operator-initiated daemon restart clears it — that
  is the deliberate "try again".
- **No macOS login-session watch.** That watch retires the daemon when the spawning GUI login
  session dies. An dorkad daemon must survive its SSH session ending.
- **Shutdown.** dorkad never stops the daemon. A daemon that was never adopted retires itself
  after its adoption window; an adopted one stays resident (see Decommissioning).

### Decommissioning

After a PID-scoped stop, an adopted daemon stays resident so the next dorkad can reattach. A
...
above, stop dorkad, then stop the daemon named by `health.terminalDaemon.pid`.
...
```
buildHash    sha256 (16 hex) of the running dorkad bundle — build identity that a version
             string cannot give, so a rollback that did not replace the file is visible
buildVersion DORKA_VERSION
...
terminalDaemon:
  ...
  buildVersion       the build the LIVE daemon was forked from (may legitimately predate
                     this dorkad after an update — reporting dorkad's version for both would
                     hide exactly that)
...
processes: dorkad drives it, the daemon performs it, the verdict crosses the socket.
...
because those terminals die with dorkad. A daemon that answered and then failed its spawn
...
- **A continuous health endpoint.** `health` is published once, in the readiness payload. A
  ...
  `collectDorkadHealth()`; that surface does not exist yet.
- **Supervision of an unscoped fallback daemon.** When the durable `systemd-run --user --scope`
  launch is unavailable (see [above](#two-long-lived-processes-not-one)) dorkad and its daemon
