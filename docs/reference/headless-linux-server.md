...
Use this guide when you want to run `dorka serve` on a Linux machine without a
desktop session, such as an Ubuntu VPS or a remote build box.

`dorka serve` starts the Dorka runtime without opening the desktop window. On
Linux, the packaged AppImage still needs the libraries that Electron expects at
startup. Current Dorka builds start Xvfb automatically for `dorka serve` when no
`DISPLAY` is set, but Xvfb must be installed first. A separate D-Bus session is
not required. When `DISPLAY` is set, Dorka uses that display instead of starting
...
whose lock names a dead process is refused rather than replaced, and `dorka serve`
exits — unset `DISPLAY` to let Dorka start its own Xvfb. A socket published with
...
`dorka serve` then fails before Electron starts:
...
```bash
sudo mkdir -p /opt/dorka
sudo curl -L https://github.com/stablyai/orca/releases/latest/download/dorka-linux.AppImage \
  -o /opt/dorka/dorka-linux.AppImage
sudo chmod +x /opt/dorka/dorka-linux.AppImage
```
...
```bash
cd /opt/dorka
sudo ./dorka-linux.AppImage --appimage-extract
sudo chmod -R a+rX /opt/dorka/squashfs-root
/opt/dorka/squashfs-root/AppRun serve --port 6768
```
...
extract-and-run wrapper can print extracted paths before Dorka starts, so
...
```bash
LIBGL_ALWAYS_SOFTWARE=1 /opt/dorka/dorka-linux.AppImage serve --port 6768
```
...
```bash
LIBGL_ALWAYS_SOFTWARE=1 /opt/dorka/dorka-linux.AppImage serve \
  --port 6768 \
  --pairing-address 100.64.1.20
```

`--pairing-address` is only the address advertised to clients. It does not
change the listener bind address. Dorka binds its WebSocket listener, then
...
proxy URL such as `https://dorka.example.com/runtime` (`http(s)` is normalized
...
Pairing URL: dorka://pair?code=...
```

For supervisors, request the versioned single-line JSON contract:

```bash
/opt/dorka/dorka-linux.AppImage serve --port 6768 \
...
```bash
sudo useradd --system --create-home --shell /usr/sbin/nologin dorka
sudo chown root:root /opt/dorka /opt/dorka/dorka-linux.AppImage
sudo chmod 755 /opt/dorka /opt/dorka/dorka-linux.AppImage
...
`dorka` service user cannot read or traverse the extracted tree and the unit fails
at startup. `chmod 755 /opt/dorka` alone does not reach into it.

For most hosts, one `dorka serve` service is enough because Dorka starts Xvfb on
display `:99` when no display exists:

```ini
# /etc/systemd/system/dorka-serve.service
[Unit]
Description=Dorka runtime server
...
Type=simple
User=dorka
WorkingDirectory=/home/dorka
Environment=LIBGL_ALWAYS_SOFTWARE=1
ExecStart=/opt/dorka/dorka-linux.AppImage serve --port 6768 --pairing-address 100.64.1.20
...
`KillMode=mixed` sends the graceful stop signal only to Dorka's main process,
...
process itself, never a grace window for the cgroup's remains. This lets Dorka
...
own transient `dorka-daemon-<launch-nonce>.scope` unit rather than in
`dorka-serve.service`'s cgroup. A `systemctl stop` or `restart` of this unit
...
With `User=dorka` and no interactive login there is none by default, so enable
lingering once:

```bash
sudo loginctl enable-linger dorka
```
...
`dorka-serve.service`'s cgroup, and is then killed when the stop completes:
...
payload: a `dorka-daemon-*.scope` value means isolated, `null` means the
unscoped fallback.
...
subprocesses; without it, Dorka is PID 1 and those children accumulate as
...
`sudo systemctl reset-failed dorka-serve.service` first to clear it — the
...
20.04, Dorka's oldest supported base, ships systemd 245.

Enable the service:

```bash
sudo systemctl daemon-reload
sudo systemctl enable --now dorka-serve.service
sudo journalctl -u dorka-serve.service -f
```
...
```bash
sudo journalctl -u dorka-serve.service -o cat \
  | jq -Rrc 'fromjson? | select(.type == "dorka_server_ready" and .schemaVersion == 1)'
```
...
separate service and set `DISPLAY=:99` for Dorka.

```ini
# /etc/systemd/system/dorka-xvfb.service
[Unit]
Description=Virtual X display for Dorka
After=network-online.target
Wants=network-online.target
...
Then add the display dependency to the Dorka service:

```ini
# /etc/systemd/system/dorka-serve.service
[Unit]
Description=Dorka runtime server
After=network-online.target dorka-xvfb.service
Wants=network-online.target dorka-xvfb.service
StartLimitIntervalSec=300
StartLimitBurst=5
...
```bash
sudo systemctl daemon-reload
sudo systemctl enable --now dorka-xvfb.service dorka-serve.service
```

## CLI Install Note

The registered Linux CLI command is `dorka-ide`, not `dorka`, to avoid shadowing
the GNOME Dorka screen reader. Desktop-managed terminals receive a
terminal-scoped bare-`dorka` shim. A packaged headless `dorka serve` also makes a
best-effort dispatcher at `$HOME/.local/bin/dorka` for the service user's own
shell, so the Claude Teams launcher can resolve its bare command; it does not
replace another user's `dorka`. From an ordinary shell outside that service
user's managed environment, substitute `dorka-ide` for `dorka` in commands below.

On a headless host, you do not need to open the desktop UI just to run the
server. Invoke the AppImage directly:

```bash
/opt/dorka/dorka-linux.AppImage serve --help
```
...
```bash
/opt/dorka/dorka-linux.AppImage --no-sandbox serve --port 6768
```
...
The Linux CLI is named `dorka-ide`, not `dorka`, so it never shadows the GNOME
Dorka screen reader at `/usr/bin/dorka`. The `.deb` and `.rpm` packages put
`dorka-ide` on `PATH` themselves at install time; with the AppImage it arrives
as `~/.local/bin/dorka-ide` when the CLI is registered.

A packaged `dorka serve` start also writes a bare `dorka` into `~/.local/bin`
that execs the same launcher, which is why the skills commands below can be
typed as `dorka`. It writes it while starting, so it is never the command that
starts the server — the first launch is `dorka-ide serve`, or the AppImage
...
a file Dorka does not own already holds that name (ownership is a marker on the
...
If you later install the desktop CLI from Dorka settings, use that CLI for normal
shell workflows. Keep the AppImage path in systemd so service restarts do not
depend on an interactive shell profile.

## Upgrade

`dorka serve` never updates itself. In headless mode Dorka wires up no auto-updater
...
- **State lives in the service user's home, not next to the binary.** Persisted
  data is under `/home/dorka/.config/` (Dorka uses both an `dorka` and an `Dorka`
  directory there), fully independent of `/opt/dorka/dorka-linux.AppImage`.
...
- **New builds migrate old state on load.** Dorka loads older `dorka-data.json`
...
`dorka-daemon-*.scope`, as reported by `health.terminalDaemon.cgroupUnit`. The
...
`dorka-daemon-*.scope` on that host.
...
Replace both `dorka` and `/home/dorka` with the service account and home used by
your unit; for an extracted deployment, use its absolute `resources/bin/dorka-ide`
...
and the stop; Dorka does not yet provide an atomic census-and-stop fence.

Rolling back is the case that needs care — see [Roll back](#roll-back).

### Record the version you deploy

The bundled CLI launcher prints the Dorka build with `dorka-ide --version`. For an
extracted deployment, that launcher is
`squashfs-root/resources/bin/dorka-ide`; deb/rpm installs and CLI registration put
it on `PATH`. Do not use `dorka-linux.AppImage --version` for this audit because
...
next to the binary. The steps below keep that record in `/opt/dorka/VERSION`.

### Upgrade steps

Never download straight onto `/opt/dorka/dorka-linux.AppImage`. The AppImage is
...
```bash
sudo chown root:root /opt/dorka
sudo chmod 755 /opt/dorka
sudo test ! -L /opt/dorka/dorka-linux.AppImage
sudo chown root:root /opt/dorka/dorka-linux.AppImage
sudo chmod 755 /opt/dorka/dorka-linux.AppImage
# Clear predictable staging names left by an older attempt after locking the directory
sudo rm -f /opt/dorka/dorka-linux.AppImage.new /opt/dorka/VERSION.new \
  /opt/dorka/dorka-linux.AppImage.recovering /opt/dorka/VERSION.recovering
sudo du -sh /home/dorka/.config
df -h /opt/dorka /home/dorka
```

`/opt/dorka` needs room for the compressed Dorka profile archive, the staged
build, and the rollback binary. A rollback extracts the old profile and preserves
the post-upgrade Dorka profile directories, so `/home` needs room for both copies.
...
DORKA_VERSION=v1.4.147

# Select the release asset on the server where Dorka runs
case "$(uname -m)" in
  x86_64)
    DORKA_ASSET=dorka-linux.AppImage
    DORKA_FILE_MACHINE=x86-64
    ;;
  aarch64 | arm64)
    DORKA_ASSET=dorka-linux-arm64.AppImage
    DORKA_FILE_MACHINE='ARM aarch64'
    ;;
  *)
...
DORKA_ROLLBACK_NEW=
DORKA_ROLLBACK=
DORKA_SERVICE_STOPPED=0
DORKA_BINARY_PROMOTED=0
recover_failed_upgrade() {
  ...
  if ((exit_status != 0)) && [[ -n "$DORKA_ROLLBACK_NEW" ]] && \
    sudo test -d "$DORKA_ROLLBACK_NEW"; then
    sudo rm -rf -- "$DORKA_ROLLBACK_NEW"
  fi
  if ((exit_status != 0 && DORKA_SERVICE_STOPPED)); then
    recovery_ok=1
    if ((DORKA_BINARY_PROMOTED)); then
      if ! sudo cp -a "$DORKA_ROLLBACK/dorka-linux.AppImage" \
        /opt/dorka/dorka-linux.AppImage.recovering || \
        ! sudo mv -f /opt/dorka/dorka-linux.AppImage.recovering \
          /opt/dorka/dorka-linux.AppImage; then
        recovery_ok=0
      fi
      if sudo test -f "$DORKA_ROLLBACK/VERSION"; then
        if ! sudo cp -a "$DORKA_ROLLBACK/VERSION" /opt/dorka/VERSION.recovering || \
          ! sudo mv -f /opt/dorka/VERSION.recovering /opt/dorka/VERSION; then
          recovery_ok=0
        fi
      elif ! sudo rm -f /opt/dorka/VERSION; then
        recovery_ok=0
      fi
    fi
    sudo rm -f /opt/dorka/dorka-linux.AppImage.recovering \
      /opt/dorka/VERSION.recovering
    if ((recovery_ok)); then
      # A tripped StartLimitBurst refuses a plain start
      sudo systemctl reset-failed dorka-serve.service || true
      sudo systemctl start dorka-serve.service || true
    else
...
DORKA_VERSION=${DORKA_VERSION:-${ORCA_VERSION:-}}
DORKA_ASSET=${DORKA_ASSET:-${ORCA_ASSET:-}}
sudo curl -fL --retry 3 "https://github.com/stablyai/orca/releases/download/${DORKA_VERSION}/${DORKA_ASSET}" \
  -o /opt/dorka/dorka-linux.AppImage.new
sudo chown root:root /opt/dorka/dorka-linux.AppImage.new
sudo chmod 755 /opt/dorka/dorka-linux.AppImage.new

# Both checks must match; either grep stops this fail-fast block otherwise
DORKA_FILE_INFO=$(LC_ALL=C file /opt/dorka/dorka-linux.AppImage.new)
grep 'ELF .* executable' <<<"$DORKA_FILE_INFO"
grep -F "$DORKA_FILE_MACHINE" <<<"$DORKA_FILE_INFO"

# 2. Assemble the prior binary and version in a root-only rollback bundle
DORKA_ROLLBACK_BASE=/opt/dorka/dorka-rollback-$(date +%F-%H%M%S-%N)
DORKA_ROLLBACK_NEW=${DORKA_ROLLBACK_BASE}.new
DORKA_ROLLBACK=${DORKA_ROLLBACK_BASE}.ready
sudo install -d -m 700 "$DORKA_ROLLBACK_NEW"
sudo cp -a /opt/dorka/dorka-linux.AppImage "$DORKA_ROLLBACK_NEW/dorka-linux.AppImage"
if sudo test -f /opt/dorka/VERSION; then
  sudo cp -a /opt/dorka/VERSION "$DORKA_ROLLBACK_NEW/VERSION"
fi

# Stage the new version record before the stop window
printf '%s\n' "$DORKA_VERSION" | sudo tee /opt/dorka/VERSION.new >/dev/null
sudo chown root:root /opt/dorka/VERSION.new
sudo chmod 644 /opt/dorka/VERSION.new

# 3. Stop the server so the profile backup is consistent
DORKA_SERVICE_STOPPED=1
sudo systemctl stop dorka-serve.service

# Add only Dorka-owned profile directories, then publish the complete bundle
DORKA_PROFILE_DIRS=()
for profile_dir in dorka Dorka; do
  if sudo test -L "/home/dorka/.config/$profile_dir"; then
    echo "Refusing symlinked Dorka profile: /home/dorka/.config/$profile_dir" >&2
    exit 1
  fi
  if sudo test -d "/home/dorka/.config/$profile_dir"; then
    if [[ "$profile_dir" == Dorka ]] && \
      sudo test /home/dorka/.config/dorka -ef /home/dorka/.config/Dorka; then
      continue
    fi
    DORKA_PROFILE_DIRS+=("$profile_dir")
  fi
done
if ((${#DORKA_PROFILE_DIRS[@]} == 0)); then
  echo 'No Dorka profile directory found under /home/dorka/.config' >&2
  exit 1
fi
sudo tar czf "$DORKA_ROLLBACK_NEW/profile.tgz" \
  -C /home/dorka/.config "${DORKA_PROFILE_DIRS[@]}"
sudo chmod 600 "$DORKA_ROLLBACK_NEW/profile.tgz"
sudo mv "$DORKA_ROLLBACK_NEW" "$DORKA_ROLLBACK"

# 4. Atomically replace the binary and version record, then start
DORKA_BINARY_PROMOTED=1
sudo mv -f /opt/dorka/dorka-linux.AppImage.new /opt/dorka/dorka-linux.AppImage
sudo mv -f /opt/dorka/VERSION.new /opt/dorka/VERSION
# Clears a start-limit hit left by the version being replaced
sudo systemctl reset-failed dorka-serve.service
sudo systemctl start dorka-serve.service
DORKA_SERVICE_STOPPED=0
trap - EXIT
```

The profile archive created in step 3 captures both Dorka profile directory names
when present without rewinding unrelated tools under `/home/dorka/.config`. The
...
`dorka-serve.service` needs restarting — leave `dorka-xvfb.service` running.
...
A healthy start prints one `Dorka server ready` block with the actual bound and
...
```bash
shopt -s nullglob
DORKA_ROLLBACK_SETS=(/opt/dorka/dorka-rollback-*.ready)
((${#DORKA_ROLLBACK_SETS[@]} > 0))
DORKA_ROLLBACK=${DORKA_ROLLBACK_SETS[${#DORKA_ROLLBACK_SETS[@]} - 1]}
printf 'Removing rollback bundle: %s\n' "$DORKA_ROLLBACK"
sudo test -d "$DORKA_ROLLBACK"
sudo rm -rf -- "$DORKA_ROLLBACK"
```
...
rewrite `dorka-data.json` in the current schema. If an older build then writes
that file, it can discard fields it does not recognize. The rolling
`dorka-data.json.bak.*` files are corruption-recovery snapshots, not a dedicated
...
((${#DORKA_ROLLBACK_SETS[@]} > 0))
DORKA_ROLLBACK=${DORKA_ROLLBACK_SETS[${#DORKA_ROLLBACK_SETS[@]} - 1]}
sudo test -f "$DORKA_ROLLBACK/dorka-linux.AppImage"
sudo tar tzf "$DORKA_ROLLBACK/profile.tgz" >/dev/null

# Extract and validate the old profile while the current server stays online
sudo test ! -L /home
DORKA_HOME_OWNER=$(sudo stat -c %u /home)
DORKA_HOME_MODE=$(sudo stat -c %a /home)
if [[ "$DORKA_HOME_OWNER" != 0 ]] || ((8#$DORKA_HOME_MODE & 0022)) || \
  sudo -u dorka test -w /home; then
  echo 'Refusing rollback because /home is not root-controlled' >&2
  exit 1
fi
DORKA_RESTORE=$(sudo mktemp -d /home/.dorka-restore.XXXXXX)
DORKA_SERVICE_STOPPED=0
DORKA_MOVED_CURRENT_DIRS=()
DORKA_INSTALLED_RESTORE_DIRS=()
DORKA_CURRENT_BINARY_MOVED=0
DORKA_CURRENT_VERSION_MOVED=0
DORKA_VERSION_REPLACEMENT_STARTED=0
DORKA_POST_UPGRADE=
DORKA_ROLLBACK_BINARY_STAGED=
DORKA_ROLLBACK_VERSION_STAGED=
DORKA_ROLLBACK_HAS_VERSION=0
restart_after_rollback_error() {
  ...
  if ((exit_status != 0 && DORKA_SERVICE_STOPPED)); then
    recovery_ok=1
    if ((${#DORKA_INSTALLED_RESTORE_DIRS[@]})); then
      for profile_dir in "${DORKA_INSTALLED_RESTORE_DIRS[@]}"; do
        if sudo test -d "/home/dorka/.config/$profile_dir"; then
          if ! sudo mv "/home/dorka/.config/$profile_dir" \
            "$DORKA_RESTORE/$profile_dir.failed"; then
            recovery_ok=0
          fi
        fi
      done
    fi
    if ((${#DORKA_MOVED_CURRENT_DIRS[@]})); then
      for profile_dir in "${DORKA_MOVED_CURRENT_DIRS[@]}"; do
        if sudo test -d "$DORKA_POST_UPGRADE/$profile_dir"; then
          if ! sudo mv "$DORKA_POST_UPGRADE/$profile_dir" /home/dorka/.config/; then
            recovery_ok=0
          fi
        elif ! sudo test -d "/home/dorka/.config/$profile_dir"; then
          recovery_ok=0
        fi
      done
    fi
    if [[ -n "$DORKA_POST_UPGRADE" ]]; then
      sudo rmdir "$DORKA_POST_UPGRADE" 2>/dev/null || true
    fi
    if ((DORKA_CURRENT_BINARY_MOVED)); then
      if sudo test -f "$DORKA_CURRENT_BINARY"; then
        if ! sudo mv -f "$DORKA_CURRENT_BINARY" /opt/dorka/dorka-linux.AppImage; then
          recovery_ok=0
        fi
      elif ! sudo test -f /opt/dorka/dorka-linux.AppImage; then
        recovery_ok=0
      fi
    fi
    if ((DORKA_CURRENT_VERSION_MOVED)); then
      if sudo test -f "$DORKA_CURRENT_VERSION"; then
        if ! sudo mv -f "$DORKA_CURRENT_VERSION" /opt/dorka/VERSION; then
          recovery_ok=0
        fi
      elif ! sudo test -f /opt/dorka/VERSION; then
        recovery_ok=0
      fi
    elif ((DORKA_VERSION_REPLACEMENT_STARTED)); then
      if ! sudo rm -f /opt/dorka/VERSION; then
        recovery_ok=0
      fi
    fi
    if ((recovery_ok)); then
  ...
  if [[ -n "$DORKA_ROLLBACK_BINARY_STAGED" ]]; then
    sudo rm -f -- "$DORKA_ROLLBACK_BINARY_STAGED"
  fi
  if [[ -n "$DORKA_ROLLBACK_VERSION_STAGED" ]]; then
    sudo rm -f -- "$DORKA_ROLLBACK_VERSION_STAGED"
  fi
  sudo rm -rf -- "$DORKA_RESTORE"
  exit "$exit_status"
}
trap restart_after_rollback_error EXIT

if [[ "$(sudo stat -c %d "$DORKA_RESTORE")" != \
  "$(sudo stat -c %d /home/dorka/.config)" ]]; then
  echo 'Refusing rollback because staging and the Dorka profile are on different filesystems' >&2
  exit 1
fi
sudo tar xzf "$DORKA_ROLLBACK/profile.tgz" -C "$DORKA_RESTORE"
DORKA_RESTORE_DIRS=()
for profile_dir in dorka Dorka; do
  if sudo test -L "$DORKA_RESTORE/$profile_dir"; then
    echo "Rollback bundle contains a symlinked profile: $profile_dir" >&2
    exit 1
  fi
  if sudo test -d "$DORKA_RESTORE/$profile_dir"; then
    if [[ "$profile_dir" == Dorka ]] && \
      sudo test "$DORKA_RESTORE/dorka" -ef "$DORKA_RESTORE/Dorka"; then
      continue
    fi
    DORKA_RESTORE_DIRS+=("$profile_dir")
  fi
done
if ((${#DORKA_RESTORE_DIRS[@]} == 0)); then
  echo "Rollback bundle has no Dorka profile directories: $DORKA_ROLLBACK" >&2
  exit 1
fi
for profile_dir in "${DORKA_RESTORE_DIRS[@]}"; do
  sudo chown -R dorka:dorka "$DORKA_RESTORE/$profile_dir"
done

DORKA_ROLLBACK_STAMP=$(date +%F-%H%M%S-%N)
DORKA_ROLLBACK_BINARY_STAGED=/opt/dorka/dorka-linux.AppImage.rollback-staged-$DORKA_ROLLBACK_STAMP
sudo cp -a "$DORKA_ROLLBACK/dorka-linux.AppImage" "$DORKA_ROLLBACK_BINARY_STAGED"
if sudo test -f "$DORKA_ROLLBACK/VERSION"; then
  DORKA_ROLLBACK_HAS_VERSION=1
  DORKA_ROLLBACK_VERSION_STAGED=/opt/dorka/VERSION.rollback-staged-$DORKA_ROLLBACK_STAMP
  sudo cp -a "$DORKA_ROLLBACK/VERSION" "$DORKA_ROLLBACK_VERSION_STAGED"
fi

DORKA_SERVICE_STOPPED=1
sudo systemctl stop dorka-serve.service

# Preserve and replace only Dorka-owned profile directories
DORKA_CURRENT_DIRS=()
for profile_dir in dorka Dorka; do
  if sudo test -L "/home/dorka/.config/$profile_dir"; then
    echo "Refusing symlinked Dorka profile: /home/dorka/.config/$profile_dir" >&2
  ...
  if sudo test -d "/home/dorka/.config/$profile_dir"; then
    ...
    DORKA_CURRENT_DIRS+=("$profile_dir")
  fi
done
DORKA_POST_UPGRADE=/home/dorka/.config/dorka-rollback-$DORKA_ROLLBACK_STAMP
sudo install -d -o dorka -g dorka -m 700 "$DORKA_POST_UPGRADE"
if ((${#DORKA_CURRENT_DIRS[@]})); then
  for profile_dir in "${DORKA_CURRENT_DIRS[@]}"; do
    DORKA_MOVED_CURRENT_DIRS+=("$profile_dir")
    sudo mv "/home/dorka/.config/$profile_dir" "$DORKA_POST_UPGRADE/"
  done
fi
for profile_dir in "${DORKA_RESTORE_DIRS[@]}"; do
  DORKA_INSTALLED_RESTORE_DIRS+=("$profile_dir")
  sudo mv "$DORKA_RESTORE/$profile_dir" /home/dorka/.config/
done

DORKA_CURRENT_BINARY=/opt/dorka/dorka-linux.AppImage.rollback-current-$DORKA_ROLLBACK_STAMP
DORKA_CURRENT_BINARY_MOVED=1
sudo mv /opt/dorka/dorka-linux.AppImage "$DORKA_CURRENT_BINARY"
sudo mv -f "$DORKA_ROLLBACK_BINARY_STAGED" /opt/dorka/dorka-linux.AppImage

DORKA_CURRENT_VERSION=/opt/dorka/VERSION.rollback-current-$DORKA_ROLLBACK_STAMP
if sudo test -f /opt/dorka/VERSION; then
  DORKA_CURRENT_VERSION_MOVED=1
  sudo mv /opt/dorka/VERSION "$DORKA_CURRENT_VERSION"
fi
DORKA_VERSION_REPLACEMENT_STARTED=1
if ((DORKA_ROLLBACK_HAS_VERSION)); then
  sudo mv -f "$DORKA_ROLLBACK_VERSION_STAGED" /opt/dorka/VERSION
else
  sudo rm -f /opt/dorka/VERSION
fi
...
the newer `dorka-data.json` in place, where an older build can discard state it
does not understand. Keep the pre-upgrade backup until the new version is proven
on your host. The `dorka-rollback-*` directory inside `.config` is also retained
deliberately. The post-upgrade binary and version record are retained in
`/opt/dorka` with the same `rollback-current-<timestamp>` suffix. Inspect these
artifacts and remove them according to your retention policy after the rollback
is resolved.

## Installing Agent Skills Without A Desktop

Dorka's agent skills (CLI usage, orchestration, computer use, etc.) are normally
installed from Dorka Settings, which pre-fills an `npx skills add ... --global`
command in a terminal for you to run. A headless host has no Settings UI, so
use `dorka skills install` instead:

```bash
dorka skills install                                      # list installable skills
dorka skills install --skill dorka-cli --skill orchestration # install globally (default)
dorka skills install --skill dorka-cli --local              # install into the current project only
dorka skills install --all                                 # install every bundled skill
dorka skills install --all --dry-run                       # print the npx command without running it
```
...
host; it does not need a running Dorka runtime.
...
the choice Dorka makes it explicitly: it passes an `--agent` list built from the
...
dorka skills install --skill dorka-cli --agent universal
```

If Dorka detects no agent at all, `dorka skills install` stops and asks for
`--agent` rather than guessing.

To refresh already-installed skills, `dorka skills update` mirrors the same
...
```bash
dorka skills update --all                                  # update every bundled skill globally
dorka skills update --skill dorka-cli --dry-run             # print the npx command without running it
```

`dorka skills update` only refreshes skills that are already installed — it exits
...
Both commands install onto the machine that runs them. In an Dorka SSH workspace
or the WSL bridge the `dorka` shim forwards commands to the Dorka host, so they
...
- Chromium sandbox errors: confirm the service is running as the non-root
  `dorka` user and that `/opt/dorka` is readable by that user, including
  `/opt/dorka/squashfs-root` if you extracted the AppImage.
...
- Journal shows `Another Dorka instance is already running for this userData
profile` and the unit exits `3`: another process already owns the profile, so
  ...
  owner with `systemctl status dorka-serve` and `pgrep -af dorka`. Stop it (or
  keep it and leave the unit down), then run
  `sudo systemctl reset-failed dorka-serve && sudo systemctl start dorka-serve` —
  ...
  AppImage mounts, list them with `findmnt -rn -t fuse.dorka-linux.AppImage` and
...
- Service crash-loops right after an upgrade: use [Roll back](#roll-back) with
  ...
  `sudo systemctl reset-failed dorka-serve.service` first.
- Diagnosing other missing libraries: extract the AppImage without launching it
  with `./dorka-linux.AppImage --appimage-extract`, then run
  `ldd squashfs-root/dorka-ide` to list any shared libraries the host is missing.
  The Electron binary is `dorka-ide`, not `dorka`; `ldd` on a path that does not
