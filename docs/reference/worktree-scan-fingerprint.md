...
(`DorkaRuntimeService.listRepoWorktreesForResolution`). It keeps the existing
...
- `computeResolvedWorktrees` fans out over **every** registered repo
  (`src/main/runtime/dorka-runtime.ts`), calling
...
The in-Dorka mutation surface is already event-driven: create, remove, rename,
...
worktree changes made **outside** Dorka (`git worktree add/remove/move/prune`,
...
| Change                                                                | Before            | After             |
| --------------------------------------------------------------------- | ----------------- | ----------------- |
| Dorka-initiated create/remove/rename/sparse/repo edit                  | immediate (event) | immediate (event) |
...
changes Dorka does not make itself.
...
Git runtime is still probed, because it is not WSL-routed from Dorka's point of
