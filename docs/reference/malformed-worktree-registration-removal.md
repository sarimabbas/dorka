...
Dorka's desktop and runtime removal entry points use registration-only recovery
...
- Git's prune also clears other stale, unlocked registrations in the repository;
  ...
  responsibility, and Dorka verifies that the requested registration disappeared.
...
Git that cannot positively attest this file-shaped registration as prunable, Dorka
