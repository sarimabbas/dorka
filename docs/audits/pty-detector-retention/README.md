...
```sh
DORKA_BACKGROUND_LAUNCH=1 node --expose-gc docs/audits/pty-detector-retention/reproduce.mjs
```
...
16 Ki characters. The actual main feed is `dorka-runtime-on-pty-data.ts` and the
