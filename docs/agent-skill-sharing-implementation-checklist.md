...
- Dorka implementation: `skills-share` through `607e31dd04`; no PR.
- Dorka Cloud: bundle smoke PR `#329` merged as `eddb144afe`; generation-aware recovery PR `#330`
...
- Final bearer-link changes pass 143 Dorka Cloud API tests with one opt-in skip; 58 focused Dorka
...
- The opt-in desktop staging harness now publishes a bearer share without retired audience fields;
  Node typecheck and Playwright test discovery pass without starting Dorka or a browser login flow.
- Local Node and web typechecks, changed-code quality gates, 94 skill-domain files with 770 tests
  passed and 3 skipped, 134 Dorka Cloud API tests with one opt-in integration skip, the full Cloud
  monorepo test/typecheck/lint/build gates, and isolated Terraform formatting and validation.
- Final Dorka validation passed repository-wide lint, all Node/CLI/web typechecks, the production
  ...
  after removing Dorka-injected Git config variables and rerunning the slow macOS PTY case.
...
- Standalone and bundled package names reject Windows reserved device names in both Dorka and Cloud,
...
- The final combined physical `windows 2` run at `44d1266641` enabled native-Windows, real-process,
  ...
  aliases with `ln -sT`. The checkout remained clean and no `dorka-skill-*` directory remained in
  WSL `/tmp`.
...
- The exact x64 Electron directory package at `f1dccb4f42` passed its after-pack ABI scan for all
  ...
  `dorka-ide` executable and `resources/node_modules` layout. A native ARM64 package built from
...
- Real paired-runtime installation passed from an isolated Electron client to both a headed
  desktop host and headless `dorka serve` host. The host owned global and Git-worktree installs;
  the headed host also owned a plain-folder install. The client's home remained untouched.
- A fresh post-resilience E2E run passed all three headed paired-runtime, headless `dorka serve`,
...
- Isolated staging bucket, IAM, `dorka_skills` database and principal, enabled secret version, 11
  log metrics, four alerts, and one dashboard in `ondorka-cloud-staging`. The complete targeted
...
- The route-disabled baseline, API revision `dorka-cloud-api-staging-00039-rek`, first proved the
...
- Auth deploy run `31535179937` promoted revision `dorka-cloud-auth-staging-00017-dug` at 100%
...
- The historical ACL-era API deploy run `31535438327` promoted revision
  `dorka-cloud-api-staging-00042-hef` at 100%
...
- Cloud PR `#317` passed both required checks and merged as `be00db10`. Its historical ACL-era
  ...
  promoted revision `dorka-cloud-api-staging-00048-xom` at 100% traffic with
...
- Guarded wake run `31564370807` prepared the reviewed staging topology. Deploy run `31564803943`
  promoted `dorka-cloud-api-staging-00051-yoq` at 100% traffic with immutable image digest
...
- Cloud PR `#330` passed required run `31578720429`, merged as `8045c85dad`, and passed merged-main
  ...
  deploy `31579844413` promoted `dorka-cloud-api-staging-00057-kat` at 100% traffic with immutable
...
- Auth deploy `31584318896` promoted `dorka-cloud-auth-staging-00021-tuq` with digest
...
- Guarded wake `31589384191` restored SQL and the two configured Relay cells without waking C3.
  Auth deploy `31589963244` promoted `dorka-cloud-auth-staging-00025-zuz` at 100% traffic with
...
- Guarded wake `31603249983` restored only the two configured staging cells. A disposable
  ...
  package deletion. The remote install, Dorka SSH target, encrypted credential artifact, one-time
...
- Guarded wake `31605729090` restored the same configured topology. An isolated headless Dorka host
...
- Cloud PR `#355` removed internal publisher and organization identifiers from anonymous skill
  ...
  `31640677572` promoted `dorka-cloud-api-staging-00060-qay` at 100% traffic with immutable digest
...
- [x] A user can select one or many private local skills, publish one immutable Skill Bundle, and
      receive one durable Dorka share URL.
- [x] Anyone with an active unlisted link can inspect a bundle without signing in, choose all or a
      ...
      workspace.
- [x] Installation works on local macOS, Linux, native Windows, WSL, paired Dorka runtimes, and
      supported SSH targets.
- [x] The portable archive root conforms to Agent Plugins 1.0.0 for skills-only packages and keeps
      Dorka integrity metadata in `dev.dorka.skill-sharing/manifest.json`.
...
- [x] Decide Dorka will not depend on the upstream CLI or an unsupported programmatic API.
- [x] Decide V1 accepts Dorka package sources only; Git, npm, and community registries remain with
      existing tools.
...
- [x] Treat the archive as a Skill Bundle, not an Dorka plugin product or executable plugin runtime.
...
- [x] Confirm private means unlisted and protected by an unpredictable revocable bearer link, not
      end-to-end encryption from Dorka Cloud operators.
...
- [x] Replace the unpublished single-skill envelope with root `plugin.json`, `skills/<name>/`, and
      `dev.dorka.skill-sharing/manifest.json`; do not add an outer `bundle/` directory.
...
- [x] Create fresh staging roots and reject unknown, malformed, or conflicting imported
      `dev.dorka.skill-sharing` namespaces without overwriting them. The extractor now exclusively
...
- [x] Generate deterministic `plugin.json`, `skills/`, and Dorka extension metadata.
...
- [x] Validate the portable root, Dorka extension namespace, bundle manifest, and every selected
...
- [x] Allow only configured Dorka skill-bucket origins and reject credential-bearing cross-host
      redirects.
...
- [x] Permit clean updates only when installed bytes still match Dorka provenance.
- [x] Return a modified conflict when installed bytes drift from their receipt.
- [x] Return an unowned conflict when a destination exists without Dorka provenance.
...
- [x] Add data-only records for Dorka agent ID, canonical-root support, global/project resolvers,
...
- [x] Repair broken Dorka-owned aliases.
...
- [x] Remove the canonical copy only when Dorka owns it, it is unmodified, and no retained receipt
      depends on it.
...
- [x] Inspect the active `ondorka-cloud` project read-only and record the existing Cloud Run, GCS,
      Cloud SQL, IAM, and enabled-service baseline.
- [x] Locate the authoritative Dorka Cloud Terraform worktree, state, modules, and deployment
      pipeline.
...
- [x] Confirm production names and quotas, including availability of
      `ondorka-cloud-skill-packages`. A read-only 2026-08-12 check returned `404` for the global
      bucket name, found no production `dorka_skills` database or skill-database secret collision,
...
- [x] Declare private bucket `ondorka-cloud-skill-packages`, subject to Terraform validation.
...
- [x] Add exact approved production and development Dorka origins to CORS; never use `*`.
...
- [x] Declare database `dorka_skills` on existing regional PostgreSQL 17 instance
      `dorka-cloud-auth-db`.
- [x] Declare dedicated principal `dorka_skills_app` with access only to `dorka_skills`.
- [x] Store its connection URL in Secret Manager as `dorka-cloud-skills-database-url`.
- [x] Attach the existing Cloud SQL instance to `dorka-cloud-api` without replacing the service.
- [x] Inject only the skill database secret into the API service.
- [x] Verify backups and point-in-time recovery cover the new database. `dorka_skills` shares the
...
- [x] Grant `dorka-cloud-api@ondorka-cloud.iam.gserviceaccount.com` bucket-scoped
...
- [x] Extend `dorka-cloud-api` in `us-central1`; do not create a separate V1 worker service.
...
- [x] Make immutable API candidates atomically replace the complete Terraform-owned literal
      environment and the exact `DORKA_SKILLS_DATABASE_URL` Secret Manager reference.
...
- [x] Test tenant-scoped deduplication without cross-tenant timing or response disclosure. Dorka
...
- [x] Return a copyable durable Dorka URL after publication.
...
- [x] Show whether a package came from Dorka Cloud and its accessible version history.
...
- [x] Prefer destination-runtime direct download when it can reach approved Dorka Cloud storage.
...
- [x] Run the same contract against a paired headless `dorka serve` host for global and
...
The connected Dorka environment `windows 2` was reachable on 2026-08-11 and ran Dorka `1.4.180` with
runtime ID `68b5e70d-baaf-40a5-b384-be09cc088880`. Validation used the isolated checkout
`C:\Users\neil\dorka\skills-share-validation` and WSL distro `Ubuntu-24.04`; no production skill
...
`C:\Users\neil\dorka\workspaces\dorka\skills-share-staging-validation` on branch
`DorkaWin/skills-share-staging-validation`. At `44d1266641`, native-Windows, real-process, and real
...
distro filesystem. No standalone Dorka CLI is installed inside that distro; WSL skill operations
are owned by the connected Windows Dorka runtime. A reproducible second-distro setup is
`wsl --install -d Debian --no-launch`, followed by first launch and an isolated Dorka test user/home
...
matching Dorka's supported Windows placement topology.
...
- [x] Confirm `windows 2` is saved and reachable through `dorka environment list` and
      `dorka status --environment "windows 2" --json`.
...
- [x] Discover installed WSL version and distros, each distro's running state, default user, home,
      filesystem, and Dorka host support.
...
- [x] Record exact Dorka client/server versions and capabilities with each test run.
...
- [x] Test an unowned junction, external link, broken junction, and provider parent that is already
      linked. All four real NTFS junction cases passed on `windows 2` at `c7c8a2e32d`; Dorka repairs
...
- [x] Temporary local test installations, workspaces, and packages are removed through verified,
      recoverable cleanup. The final checkout was clean and WSL `/tmp` had no `dorka-skill-*` debris.

## 11. Implement and validate SSH targets

The 2026-08-11 live Dorka inventory contained only the connected `windows 2` environment and no
non-local worktree or repository. Later staging journeys registered disposable Ubuntu 20.04 and
native macOS ARM64 hosts directly through Dorka's SSH provider and removed them afterward. A
...
- [x] Add signed-policy/URL generation and IAM Credentials failure alerts. Grant-signing failures
      ...
      Data Access audit logging; evidence is in Dorka Cloud `69b388e`.
- [ ] Add budget alerts for GCS storage/egress, Cloud Run growth, and Cloud SQL storage.

Dashboard evidence in Dorka Cloud `41ef335` and `f67ee2c`: upload-grant, finalize, share, and
...
- [x] Test malicious redirects, DNS/host confusion, expired grants, mismatched generations, and
      ...
      GCS generation pinning are covered in Dorka `534abb8908` and Dorka Cloud `81a581d`.
...
- [ ] Test canonical/provider paths as regular files, directories, aliases, junctions, external
      ...
      Dorka-owned broken-link repair, aliases, and verified copy fallback. Physical Windows
...
- [x] Run Linux against Ubuntu 20.04/glibc 2.31 and verify bundled native binaries respect the
      ...
      `dorka-node-pty-floor-ok` from an ARM64 Ubuntu 20.04 container on native `aarch64` hardware.
...
- [x] Modify installed bytes, publish an update, and prove Dorka refuses silent replacement. The
...
- [x] Add deterministic fixtures authored for Dorka without copying upstream fixtures.
...
- [x] Deploy `dorka-cloud-api` skill routes disabled by server-side flags.
...
- [x] Deploy the API with all skill flags disabled. Bootstrap revision `dorka-cloud-api-00016-yem`
...
- [x] After every first-release gate passes, enable the feature for all accounts in one launch.
      ...
      artifact plus skill smoke and promoted `dorka-cloud-api-00025-qup` at 100% traffic with digest
...
- [ ] Review error budgets, cost, authorization denials, saturation, orphan counts, and support
      ...
      alerts route to the enabled dedicated `Dorka skill sharing alerts` channel, and aggregate
      monitor execution `dorka-cloud-skill-storage-monitor-lfd26` passed on the serving API digest
...
## 16. Agent and Dorka CLI publishing
...
- [x] Add `dorka skills installed --json` with discovery IDs and names but no local filesystem paths.
- [x] Add `dorka skills share` with repeated explicit `--skill` selectors, a required bundle name,
