...
const ROUNDS = Number(process.env.DORKA_CLI_DEFER_BENCH_ROUNDS ?? '30')
const WARMUP = Number(process.env.DORKA_CLI_DEFER_BENCH_WARMUP ?? '3')

for (const [name, value] of [
  ['DORKA_CLI_DEFER_BENCH_ROUNDS', ROUNDS],
  ['DORKA_CLI_DEFER_BENCH_WARMUP', WARMUP]
]) {
...
if (ROUNDS % 2 !== 0) {
  ...
  throw new Error(`DORKA_CLI_DEFER_BENCH_ROUNDS must be even, received ${ROUNDS}`)
}
...
try {
  ...
  const isolated = { DORKA_USER_DATA_PATH: userDataPath }
  /** @type {Array<[string, string[], Record<string, string>]>} */
  const cases = [
    ['dorka --help', ['--help'], {}],
    ['dorka help worktree', ['help', 'worktree'], {}],
    ['dorka (no args)', [], {}],
    ['unknown command', ['no-such-command'], {}],
    ['unknown flag', ['worktree', 'list', '--nope'], {}],
    ['dorka agent-context --json', ['agent-context', '--json'], {}],
    ['dorka status --json', ['status', '--json'], isolated],
    ['dorka worktree list --json', ['worktree', 'list', '--json'], isolated]
  ]
