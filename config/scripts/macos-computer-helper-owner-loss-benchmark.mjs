...
const INTERNAL_ENV = 'DORKA_COMPUTER_HELPER_OWNER_BENCH_INTERNAL'
const EXPECTATION_ENV = 'DORKA_COMPUTER_HELPER_OWNER_BENCH_EXPECTATION'
const HELPER_RECORD_PATH_ENV = 'DORKA_COMPUTER_HELPER_OWNER_BENCH_HELPER_RECORD_PATH'
const RESULT_PATH_ENV = 'DORKA_COMPUTER_HELPER_OWNER_BENCH_RESULT_PATH'
...
const helperPath = path.join(helperAppPath, 'Contents', 'MacOS', 'dorka-computer-use-macos')
...
function startSidecar() {
  ...
  const child = fork(sidecarPath, [], {
    ...
    env: {
      ...
      DORKA_COMPUTER_MACOS_HELPER_APP_PATH: helperAppPath
    }
  })
...
function runTrial(executable, expectation) {
  ...
  let trialOutput = ''
  try {
    launcherDir = mkdtempSync(path.join(tmpdir(), 'dorka-helper-owner-bench-launcher-'))
    trialTempDir = mkdtempSync(path.join(path.sep, 'tmp', 'dorka-owner-bench-'))
    ...
    writeFileSync(
      ...
      JSON.stringify({ name: 'dorka-helper-owner-benchmark', main: 'main.cjs' })
    )
    writeFileSync(
