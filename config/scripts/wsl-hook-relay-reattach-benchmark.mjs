...
function parseEndpoint(contents) {
  const port = Number(/DORKA_AGENT_HOOK_PORT=['"]?(\d+)/.exec(contents)?.[1])
  const token = /DORKA_AGENT_HOOK_TOKEN=['"]?([^'"\r\n]+)/.exec(contents)?.[1]
...
async function writeEndpoint(distro, path, endpoint) {
  const contents = [
    `DORKA_AGENT_HOOK_PORT=${endpoint.port}`,
    `DORKA_AGENT_HOOK_TOKEN=${endpoint.token}`,
    'DORKA_AGENT_HOOK_ENV=benchmark',
...
async function invokeHook(distro, scriptPath, endpointPath) {
  ...
  const result = await run(
    ...
    wslArgs(distro, [
      ...
      'DORKA_TAB_ID=wsl-relay-bench',
      'DORKA_WORKTREE_ID=wsl-relay-bench',
      '/bin/sh',
      scriptPath
    ]),
...
async function main() {
  ...
  if (nodeVersion.status !== 0 || Number(nodeVersion.stdout.trim().split('.')[0]) < 18) {
    throw new Error(`WSL distro '${distro}' needs Node.js 18 or newer to run Dorka's relay`)
  }
  ...
  let staller = null
  try {
    userDataDir = mkdtempSync(join(tmpdir(), 'dorka-wsl-relay-bench-'))
    ...
    const benchmarkRoot = `${guestHome}/.dorka-wsl/benchmarks/${instanceKey}`
    const scriptPath = `${benchmarkRoot}/.dorka/agent-hooks/codex-hook.sh`
    const endpointPath = `${guestHome}/.dorka-wsl/agent-hooks/instance-${instanceKey}/endpoint.env`
    cleanupPaths = [benchmarkRoot, `${guestHome}/.dorka-wsl/agent-hooks/instance-${instanceKey}`]
    if (cleanupPaths.some((cleanupPath) => !cleanupPath.startsWith(`${guestHome}/.dorka-wsl/`))) {
    ...
    const survivingSessionId = `dorka-wsl-relay-bench-${instanceKey}`
    ...
    const createManager = async (token) => {
      ...
      return new WslHookRelayManager({
        ...
        hookCoordsEnv: () => ({
          DORKA_AGENT_HOOK_PORT: String(preferredPort),
