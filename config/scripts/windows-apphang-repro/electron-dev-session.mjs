...
export function createGpuUserDataDirectory(gpuMode) {
  const userDataDir = mkdtempSync(path.join(os.tmpdir(), `dorka-apphang-${gpuMode}-userdata-`))
...
export async function installRendererProbe(page) {
  await page.evaluate(() => {
    if (globalThis.__dorkaApphangProbe) {
      return
    }
    const probe = {
      intervalMs: 50,
    ...
    globalThis.__dorkaApphangProbe = { probe, timer }
  })
}
      ...
      () =>
        page.evaluate(async () => {
          ...
          return {
            ...
            rendererProbe: globalThis.__dorkaApphangProbe?.probe ?? null,
