...
export function shouldPreserveEnglishValue(enValue, key = '') {
  ...
  if (/^https?:\/\//.test(enValue) || enValue.startsWith('dorka://')) {
    return true
  }
...
function applyCjkLatinTermSpacing(localeValue, locale) {
  ...
  if (locale === 'ko') {
    ...
    // boundary — re-glue "Dorka 에"/"PR 을"/"에서는" but keep "Jira 이슈"/"Dorka 로고"/"agent 에뮬레이터".
...
export function repairTranslatedValue({ key, enValue, localeValue, locale }) {
  ...
  if (enValue.includes('dorka://')) {
    result = result.replace(/虎鲸:\/\//g, 'dorka://')
  }

  if (enValue === 'Dorka' || enValue.startsWith('Dorka ')) {
    result = result
      .replaceAll('虎鲸', 'Dorka')
      .replaceAll('逆戟鲸', 'Dorka')
      .replaceAll('シャチ', 'Dorka')
  }

  if (enValue.includes('dorka://')) {
    result = result.replace(/シャチ:\/\//g, 'dorka://')
  }
...
export function repairCatalog(enCatalog, localeCatalog, locale) {
  ...
  if (localeCatalog.menu) {
    if (locale === 'zh') {
      if (localeCatalog.menu.exploreDorka !== '探索 Dorka') {
        localeCatalog.menu.exploreDorka = '探索 Dorka'
        repaired += 1
      }
      if (localeCatalog.menu.gettingStarted !== 'Dorka 入门') {
        localeCatalog.menu.gettingStarted = 'Dorka 入门'
        repaired += 1
      }
    }
    if (locale === 'ko') {
      if (localeCatalog.menu.exploreDorka !== 'Dorka 둘러보기') {
        localeCatalog.menu.exploreDorka = 'Dorka 둘러보기'
        repaired += 1
      }
      if (localeCatalog.menu.gettingStarted !== 'Dorka 시작하기') {
        localeCatalog.menu.gettingStarted = 'Dorka 시작하기'
        repaired += 1
      }
    }
  }
