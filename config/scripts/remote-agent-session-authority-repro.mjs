...
const agentSessionToken = '--dorka-repro-agent-session'
...
try {
  ...
  execFileSync(
    ...
    [
      ...
      projectPath,
      '-c',
      'user.name=Dorka Repro',
      '-c',
      'user.email=dorka-repro@example.invalid',
      'commit',
      '--allow-empty',
      '-m',
  ...
  writeFileSync(
    path.join(profilePath, 'dorka-data.json'),
    JSON.stringify({
  ...
  return await new Promise((resolve, reject) => {
    ...
    lines.on('line', (line) => {
      try {
        ...
        if (parsed.type === 'dorka_server_ready' && parsed.pairing?.url) {
