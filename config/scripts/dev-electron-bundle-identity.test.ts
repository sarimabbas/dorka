...
const BRANCH_ENV_KEYS = [
  'DORKA_DEV_DOCK_TITLE',
  'DORKA_DEV_BRANCH',
  'DORKA_DEV_INSTANCE_LABEL',
  'DORKA_DEV_WORKTREE_NAME'
] as const
...
function patchesUnder(dockTitle: string, branch: string) {
  ...
  Object.assign(process.env, {
    DORKA_DEV_DOCK_TITLE: dockTitle,
    DORKA_DEV_BRANCH: branch,
    DORKA_DEV_INSTANCE_LABEL: branch,
    DORKA_DEV_WORKTREE_NAME: branch
  })
  try {
...
describe('dev-electron-bundle-identity', () => {
  it('leaves process.env untouched, including its object identity', () => {
    ...
    patchesUnder('Dorka: some-branch', 'some-branch')
  ...
  it('keeps the bundle display name in step with the name safeStorage keys off', () => {
    // Two independently hardcoded 'Dorka Dev' strings: this one names the bundle (notifications,
  ...
  it('produces byte-identical patches on two different branches', () => {
    ...
    expect(patchesUnder('Dorka: fix-login-crash', 'fix-login-crash')).toEqual(
      patchesUnder('Dorka: perf-2', 'perf-2')
    )
    expect(patchesUnder('Dorka: dev', 'main')).toEqual(
      patchesUnder('Dorka: some-worktree @ feature/x', 'feature/x')
    )
  })

  it('leaks no branch, worktree, or title text into any patched value', () => {
    const branch = 'fix-login-crash'
    const worktree = 'Dorka-safe-storage-lock'
    for (const patch of patchesUnder(`Dorka: ${branch}`, branch)) {
