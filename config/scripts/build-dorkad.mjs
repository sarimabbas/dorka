#!/usr/bin/env node
/**
 * Bundle `dorkad` — the Dorka runtime served from plain Node, no Electron.
 *
...
import {
  DORKAD_VERSION,
  DORKAD_VERSION_FILENAME,
  dorkadArtifactFilenames
} from '../../src/shared/dorkad-artifacts.ts'

const ROOT = join(import.meta.dirname, '..', '..')
const OUT_DIR = join(ROOT, 'out', 'dorkad')
const ENTRY = join(ROOT, 'src/main/dorkad/main.ts')
// Why beside dorkad.js: the watcher runs in a forked child so a native @parcel/watcher
...
// Why beside dorkad.js: dorkad forks the terminal daemon so PTYs outlive the runtime process,
...
// Why only these: measured, not guessed. `node-pty` is a hard `require.resolve` — dorkad
 ...
 *  directory under `outdir`, and both children must land flat beside dorkad.js — that is where
...
const output = Object.values(result.metafile.outputs).find(
  (o) => o.entryPoint === 'src/main/dorkad/main.ts'
)
...
if (graphErrors.length > 0) {
  console.error(`[build-dorkad] ${graphErrors.join('\n')}`)
  // Why this can exceed the ratchet baseline: the ratchet measures the graph reachable
  // from dorka-runtime + runtime-rpc, but this entry also imports ipc/pty directly to
  // install the PTY controller. Once dorkad ships, it should become a ratchet entry
  ...
  // smoke-loads its entries for exactly this reason, and dorkad cannot join that guard
  ...
  const smoke = spawnSync(process.execPath, [OUT_FILE, '--dorkad-smoke-load-check'], {
    encoding: 'utf8',
    timeout: 60_000
  })
  ...
  if (smoke.error || smoke.signal || smoke.status !== 0) {
    console.error(
      `[build-dorkad] the bundle did not load under plain Node.\n` +
  ...
  // token and a PTY — `smoke:dorkad-terminal` does that end to end, through dorkad.
  ...
  const daemonSmoke = spawnSync(
    ...
    ],
    {
      encoding: 'utf8',
      timeout: 60_000,
      env: { ...process.env, DORKA_DAEMON_ENTRY_LOAD_CHECK: '1' }
    }
  )
  ...
  if (daemonSmoke.error || daemonSmoke.signal || daemonSmoke.status !== 0) {
    console.error(
      `[build-dorkad] the daemon child did not load under plain Node.\n` +
  ...
  if (watcherFailure) {
    console.error(
      `[build-dorkad] the watcher child did not run under plain Node.\n${watcherFailure}`
    )
    process.exitCode = 1
  }
}

// Why a content hash and not DORKAD_VERSION alone: the remote install directory is keyed on
...
if (process.exitCode !== 1) {
  ...
  for (const filename of dorkadArtifactFilenames()) {
    const artifactPath = join(OUT_DIR, filename)
    if (!existsSync(artifactPath)) {
      throw new Error(
        `dorkad declares ${filename} in DORKAD_ARTIFACTS but never emitted it. Add the build ` +
          'step, or drop it from src/shared/dorkad-artifacts.ts.'
      )
    }
    hash.update(readFileSync(artifactPath))
  }
  const fullVersion = `${DORKAD_VERSION}+${hash.digest('hex').slice(0, 12)}`
  writeFileSync(join(OUT_DIR, DORKAD_VERSION_FILENAME), fullVersion)
  console.log(
    `[build-dorkad] ok — ${fullVersion}, ${(output.bytes / 1024 / 1024).toFixed(2)} MB, ${Object.keys(output.inputs).length} modules, zero electron and node:sqlite imports.`
  )
}
...
async function smokeLoadWatcherChild() {
  const probeDir = mkdtempSync(join(tmpdir(), 'dorkad-watcher-smoke-'))
