...
const CANDIDATE_COUNT = Number(process.env.DORKA_PENDING_MARKER_BENCH_CANDIDATES ?? '64')
const TOTAL_TICKS = Number(process.env.DORKA_PENDING_MARKER_BENCH_TICKS ?? '900')
const STEADY_TICKS = Number(process.env.DORKA_PENDING_MARKER_BENCH_STEADY_TICKS ?? '300')

for (const [name, value] of [
  ['DORKA_PENDING_MARKER_BENCH_CANDIDATES', CANDIDATE_COUNT],
  ['DORKA_PENDING_MARKER_BENCH_TICKS', TOTAL_TICKS],
  ['DORKA_PENDING_MARKER_BENCH_STEADY_TICKS', STEADY_TICKS]
]) {
...
if (STEADY_TICKS >= TOTAL_TICKS) {
  throw new Error('DORKA_PENDING_MARKER_BENCH_STEADY_TICKS must be smaller than total ticks')
}

const root = await mkdtemp(join(os.tmpdir(), 'dorka-pending-marker-bench-'))
