...
function withoutDorkaEnvironment(extra = {}) {
  return {
    ...Object.fromEntries(Object.entries(process.env).filter(([key]) => !key.startsWith('DORKA_'))),
    ...extra
  }
}
...
async function verifyNoOpWrites(scripts, home, payload) {
  const commandCodeBin = mkdtempSync(join(tmpdir(), 'dorka-hook-command-code-bin-'))
  ...
  try {
    ...
    const result = await runShell(
      ...
      withoutDorkaEnvironment({
        ...
        DORKA_AGENT_HOOK_TOKEN: 'electron-verification-token',
        DORKA_PANE_KEY: 'electron-verification-pane'
      })
    )
    ...
    for (const script of scripts) {
      ...
      const result = await runShell(
        ...
        withoutDorkaEnvironment({
          ...
          DORKA_AGENT_HOOK_TOKEN: 'electron-verification-token',
          DORKA_PANE_KEY: 'electron-verification-pane',
          DORKA_TAB_ID: 'electron-verification-tab',
          DORKA_WORKTREE_ID: 'electron-verification-worktree',
          DORKA_AGENT_HOOK_ENV: 'test',
          DORKA_AGENT_HOOK_VERSION: '1',
          DORKA_ANTIGRAVITY_EVENT: 'PostInvocation',
          DORKA_COPILOT_HOOK_EVENT: 'PostToolUse'
        })
      )
      ...
      if (request.headers['x-dorka-agent-hook-token'] !== 'electron-verification-token') {
...
async function verifyInstalledLauncher(home, payload) {
  ...
  if (
    !command ||
    !command.includes('"${HOME-}/.dorka/agent-hooks/claude-hook.sh"') ||
    !command.includes('] && [ -r ') ||
    !command.includes('else { command -p cat')
  ) {
    throw new Error('Electron did not install the guarded Claude launcher')
  }
  const scratch = mkdtempSync(join(tmpdir(), 'dorka-hook-launcher-'))
  try {
    const missingResult = await runShell(
      command,
      payload,
      withoutDorkaEnvironment({ HOME: scratch })
    )
    assertSuccessfulWrite(missingResult, 'installed missing-script launcher')

    const failingPath = join(scratch, '.dorka', 'agent-hooks', 'claude-hook.sh')
    mkdirSync(join(scratch, '.dorka', 'agent-hooks'), { recursive: true })
