...
function makeSocketPath(userDataDir) {
  ...
  if (process.platform === 'win32') {
    return `\\\\.\\pipe\\dorka-daemon-smoke-${process.pid}-${randomUUID()}`
  }
...
async function main() {
  const userDataDir = mkdtempSync(join(tmpdir(), 'dorka-daemon-boot-smoke-'))
  ...
  const child = fork(
    ...
    ],
    {
      ...
      env: { ...process.env, DORKA_USER_DATA_PATH: userDataDir }
    }
  )
