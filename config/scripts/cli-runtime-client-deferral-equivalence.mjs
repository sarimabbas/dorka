...
function run(entry, argv, env) {
  ...
  if (result.error) {
    if (result.error.code === 'ETIMEDOUT') {
      throw new Error(
        `dorka ${argv.join(' ')} did not exit within ${RUN_TIMEOUT_MS} ms — it reached a blocking command`
      )
    }
    throw result.error
  }
...
function buildCases(isolatedUserData) {
  const isolated = { DORKA_USER_DATA_PATH: isolatedUserData }
  const cases = [
    ...
    [['no-such-command'], {}],
    [['no-such-command'], { DORKA_PAIRING_CODE: 'garbage' }],
    [['wrktree', 'list'], {}],
    [['agent-context'], {}],
    [['agent-context', '--json'], {}],
    ...
    [['status', '--pairing-code', 'dorka://pair?code=zzzz'], isolated],
    ...
    [['status', '--json'], { ...isolated, DORKA_PAIRING_CODE: 'not-a-pairing-code' }],
    [['status', '--json'], { ...isolated, DORKA_REMOTE_PAIRING: 'not-a-pairing-code' }],
    [['status', '--json'], { ...isolated, DORKA_ENVIRONMENT: 'no-such-environment' }],
    ...
    // DORKA_USER_DATA_PATH. They are deliberately NOT invoked here. The
    ...
    [['environment', 'list', '--json'], { ...isolated, DORKA_ENVIRONMENT: 'no-such-environment' }],
    [['environment', 'list', '--json'], { ...isolated, DORKA_PAIRING_CODE: 'not-a-pairing-code' }],
    [['agent-context', '--json'], { ...isolated, DORKA_PAIRING_CODE: 'not-a-pairing-code' }],
...
//
//   1. FOREGROUND — `dorka serve` runs Dorka until Ctrl+C and `dorka open` /
...
//   2. MUTATING — writes outside DORKA_USER_DATA_PATH (`agent hooks off` parks
...
const UNSAFE_TOKENS = new Map([
  ...
  ...MUTATING_TOKENS.map((token) => [token, 'can write outside DORKA_USER_DATA_PATH'])
])
