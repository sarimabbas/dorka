...
// Why: dorka-cli now ships a hybrid discovery stub, so its version-sensitive command
...
const guidePath = join(projectDir, 'skill-guides', 'dorka-cli.md')
const stubPath = join(projectDir, 'skills', 'dorka-cli', 'SKILL.md')
// Why: orchestration and dorka-emulator also ship hybrid stubs now, so their version-sensitive
...
const emulatorSkillPath = join(projectDir, 'skill-guides', 'dorka-emulator.md')

function readSkill(path = guidePath) {
  return readFileSync(path, 'utf8')
}

describe('dorka CLI skill guidance', () => {
  it('keeps external browser routing at the OS/page boundary', () => {
    ...
    expect(skill).toContain(
      'For external Chrome/Safari/webviews or Dorka app chrome/settings, use the Computer Use skill/tool only when the task requires OS/window-level control'
    )
    expect(skill).toContain(
      "Use `dorka-cli` for Dorka's embedded pages and a page-automation tool such as Playwright or CDP for external pages"
    )
  })
  ...
  it('keeps independent worktree lineage separate from Git base selection', () => {
    ...
    expect(skill).toContain('`--no-parent` only controls Dorka lineage')
    expect(skill).toContain('omit `--base-branch` so Dorka uses the repo default base')
  ...
  it('documents non-lifecycle full handoffs and custom Codex model fallback', () => {
    ...
    expect(skill).toContain(
      'DORKA worktree create --name <task-name> --no-parent --agent codex --prompt'
    )
  ...
  // behind `skills get dorka-cli --reference` so they are not charged to every turn, with
  ...
  it('gates the reconstructible command catalogs behind bundled references', () => {
    ...
    expect(skill).toContain('DORKA skills get dorka-cli --reference references/<file>.md')
    expect(skill).toContain(
      'If the CLI rejects `--reference`, run `DORKA skills get dorka-cli --full`'
    )
    for (const reference of [
      ...
      expect(readSkill(join(projectDir, 'skill-guides', 'dorka-cli', reference)).trim()).not.toBe(
        ''
      )
    }
    expect(skill).not.toContain('DORKA automations create')
    expect(skill).not.toContain('DORKA artifacts share <file>')
    expect(skill).not.toContain('DORKA goto --url')
  })
  ...
  it('keeps browser injection guidance narrow and avoids literal secret examples', () => {
    ...
    expect(skill).toContain('`dorka eval` expressions, or `dorka exec` commands')
...
describe('dorka CLI install stub', () => {
  ...
  it('does not fall through to another executable on a resolution failure', () => {
    ...
    // Falling through can silently pair a version-matched guide with the wrong Dorka build.
  ...
  it('drops the changing command reference from the installable file', () => {
    ...
    expect(stub).not.toContain('DORKA automations create')
