...
export function getRequiredReleaseAssetNames(tag) {
  ...
  return [
    ...
    `dorka-ide_${version}_amd64.deb`,
    `dorka-ide_${version}_arm64.deb`,
    `dorka-ide-${version}.x86_64.rpm`,
    `dorka-ide-${version}.aarch64.rpm`,
    'dorka-windows-setup.exe',
    'dorka-windows-setup.exe.blockmap',
    `Dorka-${version}-mac.zip`,
    `Dorka-${version}-mac.zip.blockmap`,
    `Dorka-${version}-arm64-mac.zip`,
    `Dorka-${version}-arm64-mac.zip.blockmap`,
    'dorka-macos-x64.dmg',
    'dorka-macos-x64.dmg.blockmap',
    'dorka-macos-arm64.dmg',
    'dorka-macos-arm64.dmg.blockmap'
  ]
}
