...
const ITERATIONS = Number(process.env.DORKA_HYDRATE_BENCH_ITERATIONS ?? '60')
const WARMUP = Number(process.env.DORKA_HYDRATE_BENCH_WARMUP ?? '10')
const ROUNDS = 6

for (const [name, value] of [
  ['DORKA_HYDRATE_BENCH_ITERATIONS', ITERATIONS],
  ['DORKA_HYDRATE_BENCH_WARMUP', WARMUP]
]) {
...
// Row shapes are synthetic, sized against a real dorka-data.json on a heavy machine
