...
const ITERATIONS = Number(process.env.DORKA_FRAME_CHUNK_BENCH_ITERATIONS ?? '40')
const GATE_ITERATIONS = Number(process.env.DORKA_FRAME_GATE_BENCH_ITERATIONS ?? '2000')
const WARMUP = Number(process.env.DORKA_FRAME_CHUNK_BENCH_WARMUP ?? '8')
const ROUNDS = Number(process.env.DORKA_FRAME_CHUNK_BENCH_ROUNDS ?? '6')

for (const [name, value] of [
  ['DORKA_FRAME_CHUNK_BENCH_ITERATIONS', ITERATIONS],
  ['DORKA_FRAME_GATE_BENCH_ITERATIONS', GATE_ITERATIONS],
  ['DORKA_FRAME_CHUNK_BENCH_WARMUP', WARMUP],
  ['DORKA_FRAME_CHUNK_BENCH_ROUNDS', ROUNDS]
]) {
...
if (ROUNDS % 2 !== 0) {
  throw new Error(`DORKA_FRAME_CHUNK_BENCH_ROUNDS must be even so each arm leads equally`)
}
