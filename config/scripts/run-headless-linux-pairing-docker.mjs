...
if (!appImageArg) {
  fail('Usage: run-headless-linux-pairing-docker.mjs --appimage /path/to/dorka.AppImage')
}
...
function extractAppImage(image) {
  ...
  docker([
    ...
    image,
    '-lc',
    'cp /input/dorka.AppImage /artifacts/dorka.AppImage && chmod +x /artifacts/dorka.AppImage && cd /artifacts && ./dorka.AppImage --appimage-extract >/dev/null && chmod -R a+rX /artifacts/squashfs-root'
  ])
}
...
async function validateStartupMatrix() {
  ...
  const publicEntry = await startAndWait({
    ...
    mode: 'json',
    address: '127.0.0.1',
    appPath: '/artifacts/dorka.AppImage',
...
async function validateAuthenticatedPairing() {
  const server = await startAndWait({
    ...
    launch: 'direct',
    mode: 'json',
    address: 'ws://dorka-pairing-server:6768/runtime?route=runtime',
    port: '6768',
    networkAlias: 'dorka-pairing-server'
  })
  ...
  assert(
    ...
    'paired server did not report its Dorka app version'
  )
  assert(
...
async function validateUnreachableOffer() {
  const server = await startAndWait({
    ...
    launch: 'direct',
    mode: 'json',
    address: 'dorka-pairing-server:6769',
    port: '6768',
    networkAlias: 'dorka-pairing-server'
  })
  const payload = readyJson(server.stdout)
...
async function startAndWait({
  ...
  const name = `dorka-pairing-${suffix}-${containers.size}`
  const args = [
    'run',
    '-d',
    '--init',
    ...
    '-e',
    'DORKA_KEEP_RUNNING=1',
    '-e',
    'LIBGL_ALWAYS_SOFTWARE=1',
    '-e',
    `DORKA_READY_JSON=${mode === 'json' ? '1' : '0'}`,
    '-e',
    `DORKA_PAIRING_ADDRESS=${address}`,
    '-e',
    `DORKA_SERVE_PORT=${port}`,
    '-e',
    `DORKA_TEST_APPIMAGE=${appPath}`,
    ...(noPairing ? ['-e', 'DORKA_NO_PAIRING=1'] : []),
    '-v',
...
function hasCompleteReadyContract(stdout) {
  if (
    stdout.includes('Dorka server ready\n') &&
...
function validateReady(logs, mode, expectedHost, options = {}) {
  if (mode === 'human') {
    assert(
      (logs.match(/^Dorka server ready$/gm) ?? []).length === 1,
    ...
    assert(logs.includes('Pairing URL: dorka://pair?code='), 'human pairing URL is missing')
    return
  }
...
function readyJsonObjects(logs) {
  const marker = '{"type":"dorka_server_ready"'
  return logs
    .split(/\r?\n/)
    .map((line) => {
    ...
    .filter((value) => value?.type === 'dorka_server_ready')
}
