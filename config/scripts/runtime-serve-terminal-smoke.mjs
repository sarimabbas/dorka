 ...
 * terminal creation into DorkaRuntimeService, and without an installed headless PTY
 ...
 * `dorkad` unchanged, because it drives nothing but the public pairing + RPC surface.
 *
...
const DORKAD_ENTRY = join(projectDir, 'out', 'dorkad', 'dorkad.js')
const READY_TIMEOUT_MS = 120_000
const OUTPUT_TIMEOUT_MS = 30_000
const SHUTDOWN_TIMEOUT_MS = 15_000
// Why a random high port: a fixed one collides with a developer's own `dorka serve`.
const PORT = 6800 + Math.floor(Number(process.env.DORKA_SMOKE_PORT_OFFSET ?? '0'))
...
/**
 * Prefer the CLI built from this checkout over whatever `dorka` is on PATH: it is the
 * version under test, and a CI runner has no installed Dorka app to fall back on.
 */
function resolveCli() {
  const built = join(projectDir, 'out', 'cli', 'index.js')
  return existsSync(built)
    ? { command: process.execPath, prefix: [built] }
    : { command: 'dorka', prefix: [] }
}

/** The `dorka` CLI, driven with an explicit pairing code so it targets this server only. */
function dorka(pairingCode, args) {
  ...
  if (result.error) {
    throw new Error(`dorka ${args[0]} failed to spawn: ${result.error.message}`)
  }
  ...
  if (parsed.ok === false) {
    throw new Error(
      `dorka ${args.join(' ')} returned ${parsed.error?.code}: ${parsed.error?.message}`
    )
  }
...
async function waitForNonce(pairingCode, terminalHandle, nonce) {
  ...
  while (Date.now() < deadline) {
    const read = dorka(pairingCode, ['terminal', 'read', '--terminal', terminalHandle])
...
function resolveLaunch(userDataDir) {
  ...
  const target =
    flagIndex !== -1 ? process.argv[flagIndex + 1] : (process.env.DORKA_SMOKE_TARGET ?? 'electron')
  if (target === 'dorkad') {
    return {
      label: `dorkad (${DORKAD_ENTRY})`,
      command: process.execPath,
      args: [DORKAD_ENTRY, '--port', String(PORT), '--json'],
      env: { DORKA_USER_DATA: userDataDir }
    }
  }
  if (target !== 'electron') {
    throw new Error(
      `--target (or DORKA_SMOKE_TARGET) must be 'electron' or 'dorkad', got '${target}'`
    )
  }
  const serveArgs = [
    serveEntry,
  ...
  const override = process.env.DORKA_SMOKE_ELECTRON
  return {
...
function seedGitRepo() {
  const dir = mkdtempSync(join(tmpdir(), 'dorka-smoke-repo-'))
  writeFileSync(join(dir, 'README.md'), '# dorka smoke\n')
  const git = (...args) => {
  ...
  git('init', '-b', 'main')
  git('config', 'user.email', 'smoke@dorka.test')
  git('config', 'user.name', 'Dorka Smoke')
  git('add', '-A')
  git('commit', '-m', 'seed')
  return dir
}
...
async function main() {
  const userDataDir = mkdtempSync(join(tmpdir(), 'dorka-serve-smoke-'))
  ...
  try {
    ...
    const shown = dorka(pairingCode, ['worktree', 'show', '--worktree', created.id])?.worktree
    ...
    if (process.argv.includes('--browser')) {
      const status = dorka(pairingCode, ['status'])
      ...
      writeFileSync(
        fixturePath,
        '<!doctype html><title>Dorkad Browser Smoke</title><main>browser-ready</main>'
      )
      const browserPageId = dorka(pairingCode, [
        'tab',
        'create',
        '--worktree',
        created.id,
      ...
      const navigation = dorka(pairingCode, ['goto', ...targetFlags, '--url', targetUrl])
      if (navigation?.url !== targetUrl || navigation?.title !== 'Dorkad Browser Smoke') {
        throw new Error(`browser.goto returned the wrong page: ${JSON.stringify(navigation)}`)
      }
      const evaluated = dorka(pairingCode, [
        'eval',
        ...targetFlags,
        '--expression',
      ...
      const screenshot = dorka(pairingCode, ['screenshot', ...targetFlags])
    ...
    const terminal = dorka(pairingCode, ['terminal', 'create', '--worktree', created.id])?.terminal
    if (!terminal?.handle) {
    ...
    const nonce = `DORKA_SMOKE_${randomBytes(8).toString('hex')}`
    dorka(pairingCode, [
      'terminal',
      'send',
      '--terminal',
