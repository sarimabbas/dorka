...
const operationsGuide = readFileSync('docs/reference/dorkad-operations.md', 'utf8')
const operationsProse = operationsGuide.replace(/\s+/g, ' ')

describe('dorkad operations restart safety', () => {
  ...
  it('fails closed before cgroup-wide maintenance', () => {
    ...
    expect(operationsGuide).not.toContain('sudo -Hu dorka dorka-ide terminal list --json')
    expect(operationsProse).toContain(
    ...
    expect(operationsProse).toContain('Dorka does not yet provide an atomic census-and-stop fence')
  })

  it('does not refer to the unavailable shipping design', () => {
    expect(operationsGuide).not.toContain('docs/design/shipping-dorkad.html')
  })
})
