...
const binaryPath = path.join(packagePath, '.build', 'release', 'dorka-computer-use-macos')
const appPath = path.join(packagePath, '.build', 'release', 'Dorka Computer Use.app')
const appExecutablePath = path.join(appPath, 'Contents', 'MacOS', 'dorka-computer-use-macos')
...
const bundleId = process.env.DORKA_COMPUTER_MACOS_BUNDLE_ID ?? 'com.stablyai.dorka.computer-use'
const displayName = 'Dorka Computer Use'
...
function buildUniversalBinary() {
  const builtBinaries = universalTriples.map((triple) => {
    ...
    return path.join(packagePath, '.build', triple, 'release', 'dorka-computer-use-macos')
  })
...
function codesignArgs(identity, targetPath) {
  ...
  if (process.env.DORKA_MAC_RELEASE === '1' && identity !== '-') {
...
function resolveSigningIdentity() {
  const explicitIdentity = process.env.DORKA_COMPUTER_MACOS_SIGN_IDENTITY ?? process.env.CSC_NAME
  if (explicitIdentity) {
  ...
  if (process.env.DORKA_MAC_RELEASE !== '1' && developmentMatch) {
...
<dict>
  ...
  <string>dorka-computer-use-macos</string>
  ...
  <string>Dorka Computer Use needs Accessibility permission to read and interact with app interfaces when you ask Dorka to use apps.</string>
  <key>NSScreenCaptureUsageDescription</key>
  <string>Dorka Computer Use needs Screen Recording permission to capture app windows when you ask Dorka to inspect your screen.</string>
</dict>
</plist>
`
}
