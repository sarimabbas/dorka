 ...
 *   restart-proxy                — idle, then dorka open + status/list storm + open
 *                                  (does NOT kill the desktop; proxies restore work)
 *
 * Usage:
 *   DORKA_FREEZE_ENV=paired-remote DORKA_FREEZE_SCENARIO=idle-backlog-open \
...
const scenario = process.env.DORKA_FREEZE_SCENARIO || 'idle-backlog-open'
const createCount = Math.max(0, readFreezeNumberEnv('DORKA_FREEZE_CREATE', 0))
const openCount = Math.max(2, readFreezeNumberEnv('DORKA_FREEZE_OPEN_COUNT', 20))
const idleMs = Math.max(0, readFreezeNumberEnv('DORKA_FREEZE_IDLE_MS', 45_000))
const paceMs = Math.max(0, readFreezeNumberEnv('DORKA_FREEZE_PACE_MS', 250))
const paceJitterMs = Math.max(0, readFreezeNumberEnv('DORKA_FREEZE_PACE_JITTER_MS', 150))
const createWorktreeSpan = Math.max(1, readFreezeNumberEnv('DORKA_FREEZE_CREATE_WT_SPAN', 12))
...
const stormParallel = Math.max(1, readFreezeNumberEnv('DORKA_FREEZE_STORM_PARALLEL', 16))
/** Kill a switch if it exceeds this — counts toward permanent lockup. */
const opTimeoutMs = Math.max(10_000, readFreezeNumberEnv('DORKA_FREEZE_OP_TIMEOUT_MS', 60_000))
const permanentTimeoutMs = Math.max(
  15_000,
  readFreezeNumberEnv('DORKA_FREEZE_PERMANENT_MS', 60_000)
)
const foreverWindowMs = Math.max(
  10_000,
  readFreezeNumberEnv('DORKA_FREEZE_FOREVER_WINDOW_MS', DEFAULT_FOREVER_WINDOW_MS)
)
const statusSlowMs = Math.max(
  5_000,
  readFreezeNumberEnv('DORKA_FREEZE_STATUS_SLOW_MS', DEFAULT_STATUS_SLOW_MS)
)
const watchdogIntervalMs = Math.max(
  500,
  readFreezeNumberEnv('DORKA_FREEZE_WATCHDOG_INTERVAL_MS', 1500)
)
...
const rpc = createDorkaRpc({ envName })
const { dorkaJsonSync, dorkaJsonAsync, runReconnectRefreshStorm, runRestartProxy } = rpc
...
function sampleDorkaIfPossible() {
  ...
  try {
    ...
    const out = path.join(reportDir, `dorka-sample-realistic-${Date.now()}.txt`)
...
async function main() {
  if (!REALISTIC_SCENARIOS.includes(scenario)) {
    throw new Error(
      `Unknown DORKA_FREEZE_SCENARIO=${scenario}. Expected one of: ${REALISTIC_SCENARIOS.join(', ')}`
    )
  }
  ...
  const remote = dorkaJsonSync(['status'])
  notes.push(
  ...
  try {
    ...
    const parked = await dorkaJsonAsync(['terminal', 'switch', '--terminal', parkHandle], {
      timeoutMs: 60_000
    })
  ...
  if (scenario === 'idle-backlog-reconnect-open' || scenario === 'lockup-storm') {
    ...
    console.log('[realistic-freeze] restart proxy: dorka open + refresh storm (no process kill)')
  ...
  try {
    const statusProbe = await dorkaJsonAsync(['status'], {
      local: true,
  ...
  try {
    const mem = await dorkaJsonAsync(['diagnostics', 'memory'], {
      local: true,
      timeoutMs: permanentTimeoutMs
    })
