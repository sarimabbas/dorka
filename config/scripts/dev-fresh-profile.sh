...
# no saved sessions). Your real `dorka-dev` profile is left untouched.
#
# Usage:
...
#   DORKA_FRESH_PROFILE_DIR=/some/path ./config/scripts/dev-fresh-profile.sh   # use a fixed dir
set -euo pipefail
...
PROFILE_DIR="${DORKA_FRESH_PROFILE_DIR:-$(mktemp -d "${TMPDIR:-/tmp}/dorka-fresh-profile.XXXXXXXX")}"
mkdir -p "$PROFILE_DIR"

cleanup() {
  if [[ "$KEEP" -eq 0 && -z "${DORKA_FRESH_PROFILE_DIR:-}" ]]; then
    # Guard rm -rf against accidental empty/unrelated PROFILE_DIR.
    [[ -n "${PROFILE_DIR:-}" && -d "$PROFILE_DIR" && "$PROFILE_DIR" == */dorka-fresh-profile* ]] || return 0
    rm -rf "$PROFILE_DIR"
...
DORKA_DEV_USER_DATA_PATH="$PROFILE_DIR" DORKA_DEV_SHOW_FIRST_RUN_EDUCATION=1 pnpm dev
