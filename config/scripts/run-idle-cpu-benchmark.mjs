...
function createIdleRepo(worktreeCount) {
  const repoDir = mkdtempSync(path.join(os.tmpdir(), 'dorka-idle-cpu-repo-'))
  ...
  writeFileSync(path.join(repoDir, 'README.md'), '# Dorka idle CPU benchmark\n')
  writeFileSync(
  ...
  for (let i = 2; i <= worktreeCount; i += 1) {
    const worktreeDir = path.join(
      ...
      `dorka-idle-cpu-worktree-${i}-${Date.now()}`
    )
...
async function main() {
  ...
  const userDataDir = mkdtempSync(path.join(os.tmpdir(), 'dorka-idle-cpu-userdata-'))
  ...
  const app = await electron.launch({
    ...
    env: {
      ...
      ...(options.headful ? { DORKA_E2E_HEADFUL: '1' } : { DORKA_E2E_HEADLESS: '1' })
    }
  })
