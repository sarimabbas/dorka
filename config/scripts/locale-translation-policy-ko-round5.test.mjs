  ...
  it('fixes Korean round 5 review, integration, and search keyword regressions', () => {
    ...
    expect(
      repairTranslatedValue({
        ...
        enValue: 'Install the Dorka skill so agents know to use the Dorka CLI.',
        localeValue: '에이전트가 Dorka CLI 사용 방법을 알 수 있도록 Dorka 기술을 설치합니다.',
        locale: 'ko'
      })
    ).toBe('에이전트가 Dorka CLI를 사용하도록 Dorka 스킬을 설치하세요.')
    expect(
      repairTranslatedValue({
