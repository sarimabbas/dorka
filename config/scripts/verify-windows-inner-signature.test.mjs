...
function withTempFile(callback) {
  const dir = mkdtempSync(join(tmpdir(), 'dorka-inner-signature-'))
...
describe('verify-windows-inner-signature', () => {
  const originalExpectedSigners = process.env.DORKA_WINDOWS_EXPECTED_SIGNERS
  const originalExpectedThumbprints = process.env.DORKA_WINDOWS_EXPECTED_THUMBPRINTS

  beforeEach(() => {
    delete process.env.DORKA_WINDOWS_EXPECTED_SIGNERS
    delete process.env.DORKA_WINDOWS_EXPECTED_THUMBPRINTS
  })
  ...
  afterEach(() => {
    if (originalExpectedSigners === undefined) {
      ...
      process.env.DORKA_WINDOWS_EXPECTED_SIGNERS = originalExpectedSigners
    }

    if (originalExpectedThumbprints === undefined) {
      delete process.env.DORKA_WINDOWS_EXPECTED_THUMBPRINTS
    } else {
      process.env.DORKA_WINDOWS_EXPECTED_THUMBPRINTS = originalExpectedThumbprints
    }
  })
  ...
  it('rejects missing, nonexistent, and directory executable paths before PowerShell', () => {
    ...
    expect(() => validateExecutablePath(join(tmpdir(), 'missing-Dorka.exe'))).toThrow(
      /does not exist/
    )
  ...
  it('runs PowerShell with an argument array and fails on stderr or nonzero exit', () => {
    ...
    expect(getPowerShellSignatureJson('C:\\Path With Spaces\\Dorka.exe', spawnSyncImpl)).toBe(
      JSON.stringify(validSignature)
    )
    expect(calls[0].command).toBe('pwsh')
    expect(calls[0].args).toContain('-Command')
    expect(calls[0].args.at(-1)).not.toBe('C:\\Path With Spaces\\Dorka.exe')
    expect(calls[0].options).toEqual(
      expect.objectContaining({
        encoding: 'utf8',
        env: expect.objectContaining({
          DORKA_WINDOWS_INNER_EXECUTABLE: 'C:\\Path With Spaces\\Dorka.exe'
        })
      })
    )

    expect(() =>
      getPowerShellSignatureJson('Dorka.exe', () => ({
        status: 0,
        stdout: '{}',
        stderr: 'warning'
      }))
    ...
    expect(() =>
      getPowerShellSignatureJson('Dorka.exe', () => ({ status: 7, stdout: '', stderr: '' }))
    ).toThrow(/exit code 7/)
  })
