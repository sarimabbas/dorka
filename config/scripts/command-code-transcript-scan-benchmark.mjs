...
const ITERATIONS = Number.parseInt(process.env.DORKA_CC_SCAN_BENCH_ITERATIONS ?? '150', 10)
const WARMUP = Number.parseInt(process.env.DORKA_CC_SCAN_BENCH_WARMUP ?? '20', 10)

for (const [name, value] of [
  ['DORKA_CC_SCAN_BENCH_ITERATIONS', ITERATIONS],
  ['DORKA_CC_SCAN_BENCH_WARMUP', WARMUP]
]) {
  if (!Number.isInteger(value) || value <= 0) {
...
const dir = mkdtempSync(join(tmpdir(), 'dorka-cc-transcript-bench-'))
try {
  const rows = []
