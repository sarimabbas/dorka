...
const useStableElectronName =
  process.env.DORKA_DEV_STABLE_NAME === '1' || rawForwardedArgs.includes(STABLE_NAME_FLAG)
const forwardedRaw = rawForwardedArgs.filter((arg) => arg !== STABLE_NAME_FLAG)
if (useStableElectronName) {
  process.env.DORKA_DEV_STABLE_NAME = '1'
}
...
function createDockTitle(branch, label) {
  return `Dorka: ${branch || label || 'dev'}`
}

function seedDevInstanceIdentityEnv() {
  const branch =
    process.env.DORKA_DEV_BRANCH ||
    readGitValue(['symbolic-ref', '--quiet', '--short', 'HEAD']) ||
    readGitValue(['rev-parse', '--short', 'HEAD'])
  const worktreeName = process.env.DORKA_DEV_WORKTREE_NAME || path.basename(repoRoot)
  const label = process.env.DORKA_DEV_INSTANCE_LABEL || formatDevInstanceLabel(branch, worktreeName)
  const identitySeed = process.env.DORKA_DEV_INSTANCE_KEY || repoRoot
  const dockTitle = process.env.DORKA_DEV_DOCK_TITLE || createDockTitle(branch, label)

  process.env.DORKA_DEV_REPO_ROOT ||= repoRoot
  process.env.DORKA_DEV_INSTANCE_KEY ||= identitySeed
  if (branch) {
    process.env.DORKA_DEV_BRANCH ||= branch
  }
  if (worktreeName) {
    process.env.DORKA_DEV_WORKTREE_NAME ||= worktreeName
  }
  if (label) {
    ...
    process.env.DORKA_DEV_INSTANCE_LABEL ||= label
  }
  process.env.DORKA_DEV_DOCK_TITLE ||= dockTitle
}
...
function prepareMacDevElectronApp() {
  ...
  const title = process.env.DORKA_DEV_DOCK_TITLE || 'Dorka: dev'
  const identityKey = process.env.DORKA_DEV_INSTANCE_KEY || repoRoot
  ...
  // piling up "Dorka: <branch>" rows forever and breaking the notification
  ...
  process.env.DORKA_DEV_MACOS_BUNDLE_ID = bundleId
  ...
  const requiredResourcePaths = [
    ...
    path.join(appPath, 'Contents', 'MacOS', 'dorka-keyboard-layout')
  ]
  ...
  if (
    ...
    console.warn(
      `[dorka-dev] Another dev instance is running from this bundle; reusing it instead of rebuilding. Quit the other instance (or delete ${distDir}) to force a rebuild.`
    )
    process.env.ELECTRON_EXEC_PATH = executablePath
    return
  }
  ...
  try {
    execFileSync(
      ...
      [
        ...
        '--single-arch',
        '--output',
        path.join(appPath, 'Contents', 'MacOS', 'dorka-notification-status')
      ],
      { stdio: 'inherit' }
    )
  } catch (error) {
    console.warn(
      `[dorka-dev] notification-status helper build failed (permission card falls back to probes): ${error?.message ?? error}`
    )
  }
  ...
  try {
    ...
    console.warn(
      `[dorka-dev] keyboard-layout helper build failed (shifted Option composition stays conservative): ${error?.message ?? error}`
    )
  }
  ...
  try {
    ...
    signed = false
    console.warn(
      `[dorka-dev] ad-hoc codesign failed (dev notifications will not deliver): ${error?.message ?? error}`
    )
  }
...
function getDevUserDataPath() {
  if (process.env.DORKA_DEV_USER_DATA_PATH) {
    return process.env.DORKA_DEV_USER_DATA_PATH
  }
  if (process.platform === 'darwin') {
    return path.join(process.env.HOME ?? '', 'Library', 'Application Support', 'dorka-dev')
  }
...
function prepareDevCliWrapper() {
  ...
  console.log(`[dorka-dev] Prepared wrapper in ${binDir}`)
}
...
if (process.env.DORKA_SKIP_DEV_CLI_PREPARE !== '1') {
  prepareDevCliWrapper()
}

seedDevInstanceIdentityEnv()
if (!useStableElectronName && process.env.DORKA_SKIP_DEV_ELECTRON_APP_PREPARE !== '1') {
...
function prepareDevWebClient() {
  if (process.env.DORKA_SKIP_DEV_WEB_PREPARE === '1' || isHelpOrVersion) {
    return
  }
  ...
  if (!existsSync(getDevWebClientIndexPath()) && process.env.DORKA_DEV_WEB_PREPARE !== '1') {
    console.error(
      '[dorka-dev] Web client bundle missing; skipping pairing web build. Run `pnpm run build:web` or set DORKA_DEV_WEB_PREPARE=1 when you need browser pairing.'
    )
    return
  }
  ...
  console.error('[dorka-dev] Building web client for pairing...')
  execFileSync(
    process.execPath,
...
if (!isHelpOrVersion && process.env.DORKA_DEV_INSTANCE_LABEL) {
  console.error(`[dorka-dev] Instance: ${process.env.DORKA_DEV_INSTANCE_LABEL}`)
}
...
if (!isHelpOrVersion && process.env.DORKA_BACKGROUND_LAUNCH === '1') {
  console.error('[dorka-dev] Background launch: window stays off screen; automate through CDP')
}
let forwardedExtras = []
if (!userPassedPort && !isHelpOrVersion) {
  ...
  if (envPortRaw) {
    ...
    if (port === null) {
      console.error(
        `[dorka-dev] Ignoring invalid REMOTE_DEBUGGING_PORT=${JSON.stringify(envPortRaw)}; falling back to probe.`
      )
    }
  }
  if (port === null) {
  ...
  if (port !== null) {
    ...
    console.error(`[dorka-dev] Remote debugging on http://127.0.0.1:${port}`)
  } else {
    console.error(
      '[dorka-dev] No free debug port found in sweep; starting without --remote-debugging-port.'
    )
  }
}
prepareDevWebClient()
