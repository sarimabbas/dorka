...
if (!expectedPid || !rendererId) {
  throw new Error('Usage: node capture-running-dorka-lag.mjs MAIN_PID WEB_CONTENTS_ID')
}
const connection = await connectDorkaMainInspector(expectedPid, rendererId)
...
try {
  ...
  const before = await evaluateRenderer('window.__dorkaTypingDiagnostic.report()')
  ...
  await evaluateRenderer('window.__dorkaTypingDiagnostic.start()')
  echoStarted = true
  ...
  mainProfiling = false
  await evaluateRenderer('window.__dorkaTypingDiagnostic.stop()')
  echoStarted = false
  const echo = await evaluateRenderer('window.__dorkaTypingDiagnostic.report()')
  ...
  if (echoStarted) {
    await evaluateRenderer('window.__dorkaTypingDiagnostic.stop()').catch(() => {})
  }
  if (mainProfiling) {
