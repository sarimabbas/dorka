...
describe('static AppImage package contract', () => {
  it.each([
    ['dorka-linux.AppImage', 0x3e, 1],
    ['dorka-linux-arm64.AppImage', 0xb7, 'arm64']
  ...
  it.each([
    ['generic filename for an arm64 runtime and target', 'dorka-linux.AppImage', 0xb7, 3],
    ['arm64 filename for an x64 runtime and target', 'dorka-linux-arm64.AppImage', 0x3e, 1],
    ['generic x64 runtime for an arm64 target', 'dorka-linux.AppImage', 0x3e, 3],
    ['generic arm64 runtime for an x64 target', 'dorka-linux.AppImage', 0xb7, 1],
    ['arm64 artifact filename for an x64 target', 'dorka-linux-arm64.AppImage', 0xb7, 1],
    ['x64 runtime under an arm64 artifact filename', 'dorka-linux-arm64.AppImage', 0x3e, 3]
  ...
  it.each([undefined, 0, 'ia32'])(
    ...
    async (targetArch) => {
      await withFixture('dorka-linux.AppImage', createRuntime(), (path) => {
  ...
  it('accepts PT_DYNAMIC relocation metadata without dependencies', async () => {
    ...
    await withFixture('dorka-linux.AppImage', runtime, (path) => {
  ...
  it('does not scan the appended AppImage payload as outer ELF data', async () => {
    ...
    await withFixture('dorka-linux.AppImage', Buffer.concat([createRuntime(), payload]), (path) => {
      expect(() => verifyStaticAppImagePackage(path, 1)).not.toThrow()
    })
  ...
  it('rejects artifact names outside the release contract before reading them', () => {
    expect(() => verifyStaticAppImagePackage('/missing/dorka-preview.AppImage')).toThrow(
...
async function withFixture(filename, contents, check, { mode = 0o755 } = {}) {
  const root = await mkdtemp(join(tmpdir(), 'dorka-static-appimage-contract-'))
  try {
