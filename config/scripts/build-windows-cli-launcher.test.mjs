...
describe('Windows CLI launcher', () => {
  it('reuses a compiled launcher that is at least as new as the C# source', () => {
    const root = mkdtempSync(join(tmpdir(), 'dorka-cli-launcher-reuse-'))
    try {
      const sourcePath = join(root, 'DorkaCliLauncher.cs')
      const outputPath = join(root, '.build', 'dorka.exe')
  ...
  itCrossHost('fails closed when the Windows launcher cannot be compiled on this host', () => {
    const outputRoot = mkdtempSync(join(tmpdir(), 'dorka cross-host launcher '))
    try {
  ...
  itCrossHost('never materializes the child environment block from ProcessStartInfo', () => {
    ...
    const source = readFileSync(
      join(projectRoot, 'native', 'windows-cli-launcher', 'DorkaCliLauncher.cs'),
      'utf8'
    )
  ...
  itWindows('preserves a multiline argument from PowerShell through the native launcher', () => {
    const appRoot = mkdtempSync(join(tmpdir(), 'dorka cli launcher '))
    try {
      ...
      copyFileSync(process.execPath, join(appRoot, 'Dorka.exe'))
      writeFileSync(
        cliPath,
  ...
  dorkaNodeOptions: process.env.DORKA_NODE_OPTIONS ?? null
      ...
      const powershell = spawnSync(
        ...
        [
          ...
          '-NonInteractive',
          '-Command',
          '& $env:DORKA_TEST_LAUNCHER orchestration send --body $env:DORKA_TEST_BODY --json'
        ],
        {
          encoding: 'utf8',
          env: {
            ...
            DORKA_TEST_BODY: body,
            DORKA_TEST_LAUNCHER: launcherPath
          }
        }
      )
  ...
  itWindows('survives an inherited environment block containing PATH and Path', () => {
    const appRoot = mkdtempSync(join(tmpdir(), 'dorka duplicate path launcher '))
    try {
      const resourcesPath = join(appRoot, 'resources')
      const launcherPath = join(resourcesPath, 'bin', 'dorka.exe')
      const cliPath = join(resourcesPath, 'app.asar.unpacked', 'out', 'cli', 'index.js')
      ...
      writeFileSync(
        cliPath,
        `require('node:fs').writeFileSync(process.env.DORKA_TEST_OUTPUT, JSON.stringify({
  electronRunAsNode: process.env.ELECTRON_RUN_AS_NODE,
