#!/usr/bin/env node
// Builds the dorka-notification-status helper binary.
//
...
const bundleId = readArg('--bundle-id') ?? 'com.stablyai.dorka'
...
const workDir = path.join(tmpdir(), `dorka-notification-status-${process.pid}`)
...
try {
  ...
  const builtBinaries = triples.map((triple) => {
    const output = path.join(workDir, `dorka-notification-status-${triple}`)
    execFileSync(
      'swiftc',
      [
        '-O',
...
<dict>
  ...
  <string>dorka-notification-status</string>
</dict>
</plist>
`
}
