...
const home = await mkdtemp(join(tmpdir(), 'dorka-managed-hook-node18-'))
...
try {
  ...
  assert.match(codexHooks, /\.dorka\/agent-hooks\/codex-hook\.sh/)
  assert.match(claudeSettings, /\.dorka\/agent-hooks\/claude-hook\.sh/)
  await access(join(home, '.dorka', 'agent-hooks', 'codex-hook.sh'), constants.X_OK)
  await access(join(home, '.dorka', 'agent-hooks', 'claude-hook.sh'), constants.X_OK)
} finally {
