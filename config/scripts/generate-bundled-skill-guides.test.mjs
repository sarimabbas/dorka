...
const GUIDE_REFERENCES = {
  ...
  'dorka-cli': ['automations.md', 'browser.md', 'publishing.md'],
...
async function readPerWorkspaceEnvCorpus() {
  ...
  const files = [
    path.join(guideRoot, 'dorka-per-workspace-env.md'),
    ...GUIDE_REFERENCES['dorka-per-workspace-env'].map((reference) =>
      path.join(guideRoot, 'dorka-per-workspace-env', 'references', reference)
    )
  ]
...
async function createFixture() {
  const root = await mkdtemp(path.join(tmpdir(), 'dorka-bundled-skill-guides-'))
...
describe('bundled skill guide generator', () => {
  ...
  it('uses the exported recipe id variable in per-workspace environment examples', async () => {
    ...
    expect(corpus).toContain('DORKA_RECIPE_ID')
    expect(corpus).not.toContain('DORKA_VM_RECIPE_ID')
    ...
    expect(vercelReference).toContain(
      'name="dorka-${recipe_id:0:max_recipe_id_length}-${instance_id}"'
    )
  })

  it.skipIf(process.platform === 'win32')(
    'resolves snapshot cleanup through Dorka user-data precedence',
    async () => {
      const source = await readFile(
        path.join(projectDir, 'skill-guides', 'dorka-per-workspace-env.md'),
        'utf8'
      )
      const assignment =
      ...
      const renderPath = async (env) =>
        (
          await execFileAsync(
            'bash',
            ['-u', '-c', `${assignment}; printf '%s' "$dorka_user_data_path"`],
            {
              env
            }
          )
      ...
      await expect(renderPath({ HOME: '/home/dorka' })).resolves.toBe('/home/dorka/.config/dorka')
      await expect(
        renderPath({ HOME: '/home/dorka', XDG_CONFIG_HOME: '/srv/config' })
      ).resolves.toBe('/srv/config/dorka')
      await expect(
        renderPath({
          HOME: '/home/dorka',
          XDG_CONFIG_HOME: '/srv/config',
          DORKA_USER_DATA_PATH: '/var/lib/dorka-custom'
        })
      ).resolves.toBe('/var/lib/dorka-custom')
    }
  )
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      const startMarker = 'recipe_id="${DORKA_RECIPE_ID:-vercel-sandbox}"'
      const endMarker = 'name="dorka-${recipe_id:0:max_recipe_id_length}-${instance_id}"'
      ...
      const renderName = async (recipeId, instanceId) =>
        (
          await execFileAsync('bash', ['-u', '-c', script], {
            env: { ...process.env, DORKA_RECIPE_ID: recipeId, DORKA_VM_INSTANCE_ID: instanceId }
          })
        ).stdout

      const instanceId = 'dorka-123e4567-e89b-12d3-a456-426614174000'
      ...
      expect(dotted).toBe(`dorka-provider-cloud_sandbox-${instanceId}`)
  ...
  it('keeps CLI guide examples safe across shells and Linux command names', async () => {
    for (const name of ['dorka-cli', 'computer-use', 'dorka-emulator', 'dorka-emulator-android']) {
      const source = await readFile(path.join(projectDir, 'skill-guides', `${name}.md`), 'utf8')

      expect(source).toMatch(/^DORKA .+--json$/mu)
      // Why: bare command lines can launch GNOME Dorka, while shell variables make
      // the same guide unusable from PowerShell and cmd.exe.
      expect(source).not.toMatch(/^dorka /mu)
      expect(source).not.toMatch(/\$DORKA(?:_|\b)/u)
    }
  })
  ...
  // drift again exactly as the guide copies already did (#7904 lost `/usr/bin/dorka`).
  it('projects one shared resolver fragment byte-for-byte into every stub', async () => {
    ...
    // where bare `dorka` started the screen reader talking on a user's Ubuntu box.
    expect(blocks.get('resolver').text).toContain('(`/usr/bin/dorka`)')
    ...
    for (const name of STUB_TOPICS) {
      ...
      // The `DORKA` placeholder rule is stated once, in the fragment, never restated.
  ...
  // the `DORKA` placeholder rule. Red until the guide bodies drop their ladders; retiring
  // those also retires the DORKA_CLI_COMMAND/dorka-dev/dorka-ide assertions in
  ...
  it('keeps the CLI resolver ladder out of every guide body', async () => {
    for (const name of CANONICAL_GUIDE_NAMES) {
      const source = await readFile(path.join(projectDir, 'skill-guides', `${name}.md`), 'utf8')
      expect(source, name).not.toContain('DORKA_CLI_COMMAND')
    }
  })
  ...
  it('rejects non-Markdown and empty bundled references', async () => {
    const root = await createFixture()
    const referenceRoot = path.join(root, 'skill-guides', 'dorka-cli', 'references')
...
describe('guide reference routing', () => {
  ...
  it('routes every shipped reference from its own guide, in both directions', async () => {
    ...
    // A vacuous loop would pass forever; dorka-cli is a guide that owns references today.
    expect(owners.map((owner) => owner.name)).toContain('dorka-cli')
