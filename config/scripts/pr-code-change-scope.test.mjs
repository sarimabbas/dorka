...
describe('docs-only path classification', () => {
  ...
  it('still runs PR Checks for product markdown and CI', () => {
    ...
    expect(isDocsOnlyPath('skill-guides/dorka-cli.md')).toBe(false)
...
describe('per-job path classification', () => {
  ...
  it('runs native package jobs only for the platform that ships the changed native', () => {
    expectClassification(['native/windows-cli-launcher/DorkaCliLauncher.cs'], {
      package_windows: true
    })
  ...
  it('runs dorkad browser when Chrome launch, session, or tab modules change', () => {
    for (const file of [
      'src/main/dorkad/external-chromium-browser-session.ts',
      'src/main/dorkad/external-chromium-command-arguments.ts',
      'src/main/dorkad/external-chromium-tab-registry.ts',
      'src/main/dorkad/external-chromium-tab-projection.ts'
    ]) {
      expectClassification([file], {
        dorkad_browser: true,
        package: true,
    ...
    expectClassification(['src/main/dorkad/dorkad-native-preflight.ts'], {
      package: true,
      package_windows: true
    })
  })
  ...
  it('runs the mobile web app job for the builder, the page source and the shell policy', () => {
    for (const file of [
      ...
      'mobile/modules/dorka-mobile-web-shell/ios/MobileWebShellCsp.swift',
  ...
  it('leaves it off for changes that cannot reach the page', () => {
    for (const file of ['docs/reference/x.md', 'src/main/dorkad/dorkad-native-preflight.ts']) {
...
describe('PR Checks skip wiring', () => {
  it('runs the candidate daemon shutdown Docker oracle in the existing Linux package job', () => {
    ...
    expect(steps[oracle].env.DORKA_BACKGROUND_LAUNCH).toBe('1')
  })
