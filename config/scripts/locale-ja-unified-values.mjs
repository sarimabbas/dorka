...
export const JA_UNIFIED_VALUES = {
  ...
  'Show Dorka Mobile Button': 'Dorka Mobile ボタンを表示',
  Paste: '貼り付け',
  ...
  'Show the Dorka Mobile button at the top of the left sidebar.':
    '左サイドバー上部に Dorka Mobile ボタンを表示します。',
  'Enable Dorka CLI': 'Dorka CLI を有効化',
  'Registered the Dorka CLI in PATH.': 'Dorka CLI を PATH に登録しました。',
  ...
  'Gitea credentials are configured but could not authenticate. Check the token, API base URL, and repository permissions, then restart Dorka if environment variables changed.':
    'Gitea の認証情報は設定されていますが、認証できませんでした。トークン、API ベース URL、リポジトリの権限を確認し、環境変数を変更した場合は Dorka を再起動してください。',
  'Azure DevOps credentials are configured but could not authenticate. Check the token, API base URL, and repository permissions, then restart Dorka if environment variables changed.':
    'Azure DevOps の認証情報は設定されていますが、認証できませんでした。トークン、API ベース URL、リポジトリの権限を確認し、環境変数を変更した場合は Dorka を再起動してください。',
  'Bitbucket credentials are configured but could not authenticate. Check the token and repository permissions, then restart Dorka if environment variables changed.':
    'Bitbucket の認証情報は設定されていますが、認証できませんでした。トークンとリポジトリの権限を確認し、環境変数を変更した場合は Dorka を再起動してください。',
  ...
  'Dorka will continue to work, but commands such as dorka status, dorka terminal, and orchestration are unavailable for this session.\n\n{{guidance}}\n\nCause: {{cause}}':
    'Dorka は引き続き使用できますが、このセッションでは dorka status や dorka terminal などのコマンドとオーケストレーションを利用できません。\n\n{{guidance}}\n\n原因: {{cause}}'
}
