...
export function createValidationEnv(inheritedEnv, layout) {
  ...
  return {
    ...
    DORKA_E2E_HOME_DIR: layout.homeDir,
    DORKA_E2E_USER_DATA_DIR: layout.userDataDir,
    DORKA_USER_DATA_PATH: layout.userDataDir
  }
}

export async function createValidationLayout(options = {}) {
  const primaryHome = path.resolve(options.primaryHome ?? os.homedir())
  const envTempParent = process.env.DORKA_CODEX_VALIDATION_TEMP_PARENT?.trim()
  ...
  if (samePath(tempParentReal, primaryHomeReal) || isWithin(tempParentReal, primaryHomeReal)) {
    throw new Error(
      `Refusing to place the disposable validation root inside the primary home (${primaryHome}). ` +
        'Pass --temp-parent <dir> or set DORKA_CODEX_VALIDATION_TEMP_PARENT to a directory outside it.'
    )
  }
  const tempRoot = await mkdtemp(path.join(tempParent, 'dorka-codex-real-'))
...
async function seedCompletedProfile(layout) {
  ...
  await writeFile(
    path.join(layout.userDataDir, 'dorka-data.json'),
...
function validationCliCommand() {
  if (process.env.DORKA_VALIDATION_CLI) {
    return process.env.DORKA_VALIDATION_CLI
  }
  if (process.env.DORKA_CLI_COMMAND) {
    return process.env.DORKA_CLI_COMMAND
  }
  return process.platform === 'linux' ? 'dorka-ide' : 'dorka'
}

async function probeTerminalEnvironment(terminalHandle, launchEnv) {
  const marker = `__DORKA_CODEX_VALIDATION_${randomUUID()}__`
  const command = [
    'node -e',
    `"console.log('${marker}:' + JSON.stringify({home: require('node:os').homedir(), codexHome: process.env.CODEX_HOME || null, dorkaCodexHome: process.env.DORKA_CODEX_HOME || null}))"`
  ].join(' ')
...
async function main() {
  ...
  const reportPath =
    options.reportPath ??
    path.join(os.tmpdir(), `dorka-codex-real-account-${options.scenario}-${Date.now()}.json`)
  ...
  try {
    ...
    if (!options.dryRun) {
      ...
      app = await electron.launch({
        ...
        env: { ...launchEnv, DORKA_BACKGROUND_LAUNCH: '1' }
      })
