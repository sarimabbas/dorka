...
const ITERATIONS = Number(process.env.DORKA_ADVERTISED_URL_BENCH_ITERATIONS ?? '10000')
const ROUNDS = Number(process.env.DORKA_ADVERTISED_URL_BENCH_ROUNDS ?? '12')
const WARMUP = Number(process.env.DORKA_ADVERTISED_URL_BENCH_WARMUP ?? '1000')

for (const [name, value] of [
  ['DORKA_ADVERTISED_URL_BENCH_ITERATIONS', ITERATIONS],
  ['DORKA_ADVERTISED_URL_BENCH_ROUNDS', ROUNDS],
  ['DORKA_ADVERTISED_URL_BENCH_WARMUP', WARMUP]
]) {
...
if (ROUNDS % 2 !== 0) {
  throw new Error('DORKA_ADVERTISED_URL_BENCH_ROUNDS must be even')
}
