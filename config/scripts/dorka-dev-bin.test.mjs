...
const wrapperPath = path.join(projectDir, 'config', 'scripts', 'dorka-dev.mjs')

describe('dorka-dev package bin', () => {
  it('uses a Node entrypoint for cross-platform package installs', () => {
    expect(packageJson.bin['dorka-dev']).toBe('./config/scripts/dorka-dev.mjs')
    expect(readFileSync(wrapperPath, 'utf8')).toMatch(/^#!\/usr\/bin\/env node\n/)
  })

  it('runs the dev CLI through Node without requiring Bash', () => {
    const root = mkdtempSync(path.join(tmpdir(), 'dorka-dev-bin-'))
    ...
    writeFileSync(
      ...
      [
        ...
        '  devCliInvocation: process.env.DORKA_DEV_CLI_INVOCATION,',
        '  appExecutable: process.env.DORKA_APP_EXECUTABLE',
        '}));'
      ].join('\n'),
      'utf8'
    )
    ...
    execFileSync(process.execPath, [wrapperPath, '--help'], {
      env: {
        ...process.env,
        DORKA_DEV_CLI_ENTRY_PATH: cliEntry,
        DORKA_DEV_USER_DATA_PATH: path.join(root, 'user-data'),
        DORKA_APP_EXECUTABLE: path.join(root, 'Electron')
      },
      stdio: 'ignore'
    })
