...
function readAppDirArg(argv) {
  ...
  if (process.platform === 'darwin') {
    return 'dist/mac-arm64/Dorka.app'
  }
...
function getPackagedCliPath(appDir) {
  if (process.platform === 'darwin' || appDir.endsWith('.app')) {
    return join(appDir, 'Contents', 'Resources', 'bin', 'dorka')
  }
  if (process.platform === 'win32') {
    return join(appDir, 'resources', 'bin', 'dorka.exe')
  }
  return join(appDir, 'resources', 'bin', 'dorka-ide')
}

const appDir = resolve(readAppDirArg(process.argv.slice(2)))
const tempRoot = await mkdtemp(join(tmpdir(), 'dorka-packaged-cli-smoke-'))
...
try {
  ...
  assert(list.topics.some((topic) => topic.name === 'dorka-cli'))
  assert.match((await run(['skills', 'get', 'dorka-cli'])).stdout, /name: dorka-cli/)
  ...
  const update = JSON.parse(
    (await run(['skills', 'update', '--skill', 'dorka-cli', '--dry-run', '--json'])).stdout
  )
...
// Why: on Windows the launcher above spawns the copied Dorka.exe (and its crashpad/utility children)
