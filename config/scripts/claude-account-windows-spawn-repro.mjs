...
const tempRoot = await mkdtemp(join(tmpdir(), 'dorka-claude-spawn-'))
...
const fixtureDir = join(tempRoot, 'Profile space & ^ (paren) %DORKA_PATH_TRAP% !bang! 한글')
...
const fixtureEnv = {
  ...process.env,
  CLAUDE_CONFIG_DIR: join(fixtureDir, 'config space & ^ (paren) %DORKA_ENV_LITERAL% !bang! 한글'),
  DORKA_ARG_TRAP: 'EXPANDED_ARG',
  DORKA_PATH_TRAP: 'EXPANDED_PATH',
  DORKA_FIXTURE_CAPTURE: capturePath,
  DORKA_FIXTURE_PIDS: pidPath,
  DORKA_FIXTURE_NODE: process.execPath
}
const reportedEnv = {
  ...fixtureEnv,
  CLAUDE_CONFIG_DIR: join(reportedDir, 'config with spaces 한글'),
  DORKA_FIXTURE_CAPTURE: reportedCapturePath,
  DORKA_FIXTURE_PIDS: reportedPidPath
}
...
const fixtureSource =
  ...
  `  writeFileSync(process.env.DORKA_FIXTURE_PIDS, JSON.stringify({ child: process.pid, grandchild: grandchild.pid }))\n` +
  `  setInterval(() => {}, 1000)\n` +
  `} else {\n` +
  `  writeFileSync(process.env.DORKA_FIXTURE_CAPTURE, JSON.stringify({ argv: process.argv.slice(2), configDir: process.env.CLAUDE_CONFIG_DIR }))\n` +
  `}\n`
const shimSource = '@echo off\r\n"%DORKA_FIXTURE_NODE%" "%~dp0capture-child.cjs" %*\r\n'
...
try {
  ...
  for (const [name, segment] of Object.entries({
    ...
    percent: 'profile%DORKA_PATH_TRAP%',
    bang: 'profile!name',
    ...
    const env = {
      ...
      DORKA_FIXTURE_CAPTURE: captureFile
    }
