 ...
 * graceful-then-immediate kill pair emitted when Dorka closes a workspace.
...
const iterations = Number(process.env.DORKA_WINDOWS_DAEMON_CLOSE_ITERATIONS ?? 25)
...
async function main() {
  ...
  const scratch = mkdtempSync(join(tmpdir(), 'dorka-windows-daemon-close-'))
  const socketPath = `\\\\.\\pipe\\dorka-daemon-close-${process.pid}-${randomUUID()}`
  ...
  const child = fork(
    ...
    ['--socket', socketPath, '--token', tokenPath, '--log-file', daemonLogPath],
    {
      ...
      windowsHide: true,
      env: { ...process.env, DORKA_USER_DATA_PATH: scratch }
    }
  )
