...
export const KO_VALUE_OVERRIDES = {
  ...
  'Dorka uses the GitHub CLI (gh) to show pull requests, issues, and checks.':
    'Dorka는 GitHub CLI(gh)를 사용하여 PR, 이슈 및 검사를 표시합니다.',
  ...
  'Review inactive workspaces before deleting their local files and Dorka state.':
    '로컬 파일과 Dorka 상태를 삭제하기 전에 비활성 워크스페이스를 확인하세요.',
  ...
  'Install the Dorka skill so agents know to use the Dorka CLI.':
    '에이전트가 Dorka CLI를 사용하도록 Dorka 스킬을 설치하세요.',
  ...
  "Control Dorka from your phone. Check on agents, review changes, and kick off tasks while you're away from your desk.":
    '휴대폰에서 Dorka를 제어하세요. 자리를 비운 동안 에이전트를 확인하고, 변경 사항을 리뷰하고, 작업을 시작하세요.',
