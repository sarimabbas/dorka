...
function psLine(dir: string, appName = 'Dorka: some-branch') {
...
describe('dev-electron-bundle-cache', () => {
  ...
  it('protects a live bundle whose path contains spaces', () => {
    ...
    const spaced = '/Users/me/My Projects/dorka/out/electron-dev/aaaaaaaaaaaa'
    const other = '/Users/me/My Projects/dorka/out/electron-dev/bbbbbbbbbbbb'
    expect(
      selectStaleDevBundleDirs({
        bundles: [settled(spaced), settled(other)],
        currentDir: C,
        processTable: psLine(spaced, 'Dorka: my branch'),
        nowMs: NOW
      })
    ).toEqual([other])
  })
