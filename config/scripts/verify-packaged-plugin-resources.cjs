...
function hashPackagedPluginTree(root) {
  ...
  visit(root)
  const hash = createHash('sha256').update('dorka-plugin-tree-v1\0')
...
function verifyPackagedPluginResources(resourcesDir) {
  ...
  readJsonFile(join(launchRoot, 'dorka-marketplace.json'), 'marketplace index')
  ...
  for (const entry of index.plugins) {
    ...
    const manifest = readJsonFile(join(pluginRoot, 'dorka-plugin.json'), 'plugin manifest')
