#!/usr/bin/env bash
#
# Creates stablyai/dorka-adhoc and grants the existing release App write access to
# it, so adhoc-mac-build.yml can publish there.
#
# Why a separate repo rather than reusing dorka-hourly: an adhoc build is somebody's
...
ORG="stablyai"
ADHOC_REPO="$ORG/dorka-adhoc"
MAIN_REPO="$ORG/dorka"
APP_SLUG="dorka-hourly-release"
...
if gh api "repos/$ADHOC_REPO" --jq '.full_name' >/dev/null 2>&1; then
  ...
  # as it does for dorka-hourly. A private repo would 404 for every client.
  #
  ...
  gh repo create "$ADHOC_REPO" \
    --public \
    --description "Adhoc macOS dev builds of Dorka, cut from unlanded branches. Not a source repo." \
    --add-readme \
    --disable-issues \
    --disable-wiki ||
...
Could not do it from here${INSTALL_ID:+ (needs an Organization Owner)}. Do it in the browser:
  ...
  3. Repository access  ->  Only select repositories  ->  add $ADHOC_REPO
     (keep dorka-hourly/dorka-daily selected; all dev channels use this one App)
  4. Save.
EOF
fi
