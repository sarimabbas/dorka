...
const ITERATIONS = Number(process.env.DORKA_BRANCH_COMPARE_BENCH_ITERATIONS ?? '8')
const WARMUP = Number(process.env.DORKA_BRANCH_COMPARE_BENCH_WARMUP ?? '2')
const ROUNDS = 6

for (const [name, value] of [
  ['DORKA_BRANCH_COMPARE_BENCH_ITERATIONS', ITERATIONS],
  ['DORKA_BRANCH_COMPARE_BENCH_WARMUP', WARMUP]
]) {
...
// A short remote label is the common case (Dorka's base picker emits `origin/main`); the
