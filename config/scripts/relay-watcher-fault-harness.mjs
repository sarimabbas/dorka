...
async function main() {
  ...
  let relay
  try {
    tempRoot = await mkdtemp(join(tmpdir(), 'dorka-relay-watcher-fault-'))
    ...
    const socketPath =
      process.platform === 'win32'
        ? `\\\\.\\pipe\\dorka-relay-watcher-fault-${process.pid}-${Date.now()}`
    ...
    daemon = spawn(
      ...
      ],
      {
        ...
        env: { ...process.env, DORKA_WATCHER_CHILD_PID_FILE: pidFile },
    ...
    const beforePtyMarker = `DORKA_PTY_BEFORE_${Date.now()}`
    ...
    const afterPtyMarker = `DORKA_PTY_AFTER_${Date.now()}`
    startIndex = relay.messageCount()
