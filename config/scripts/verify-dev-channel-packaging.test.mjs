...
const WIN_ADHOC_ENV = {
  DORKA_WIN_ADHOC: '1',
  DORKA_ADHOC_BUILD_VERSION: '1.4.178-adhoc.20260819010203'
}
...
describe('electron-builder dev-channel identity', () => {
  it('keeps the SignPath publisherName on stable Windows builds', () => {
    ...
    expect(config.publish.repo).toBe('dorka')
  ...
  it.each([
    ['hourly', { DORKA_WIN_HOURLY: '1' }, 'dorka-hourly'],
    ['daily', { DORKA_WIN_DAILY: '1' }, 'dorka-daily'],
    ['adhoc', { DORKA_WIN_ADHOC: '1' }, 'dorka-adhoc']
  ...
  // Why: DORKA_MAC_* gates hardened runtime, notarization, and root-level
  ...
  it('still notarizes mac dev builds', () => {
    ...
    expect(config.publish.repo).toBe('dorka-adhoc')
  })
})

describe('collectDevChannelPackagingProblems', () => {
  const goodWinConfig = {
    publish: { repo: 'dorka-adhoc', releaseType: 'prerelease' },
    extraMetadata: { version: '1.4.178-adhoc.20260819010203' },
    win: { verifyUpdateCodeSignature: false }
  }
  const env = { DORKA_ADHOC_BUILD_VERSION: '1.4.178-adhoc.20260819010203' }
  ...
  it('rejects a config that resolved the main repo', () => {
    const problems = collectDevChannelPackagingProblems({
      channel: 'adhoc',
      platform: 'win32',
      config: { ...goodWinConfig, publish: { repo: 'dorka', releaseType: 'release' } },
      env
    })

    expect(problems.join('\n')).toContain('must publish to "dorka-adhoc"')
