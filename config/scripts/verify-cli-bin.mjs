...
function buildOutPackageJson(version) {
  return `${JSON.stringify(
    { name: 'dorka-compiled-output', type: 'commonjs', private: true, version },
    null,
    2
  )}\n`
}
...
export function verifyPackageCliBin({
  ...
  const binTarget = packageJson.bin?.dorka
  if (typeof binTarget !== 'string' || binTarget.length === 0) {
    throw new Error('package.json must declare bin.dorka')
  }

  const binPath = path.resolve(projectDir, binTarget)
  const stats = statSync(binPath)
  if (!stats.isFile()) {
    throw new Error(`bin.dorka target is not a file: ${binTarget}`)
  }
  if (stats.size === 0) {
    throw new Error(`bin.dorka target is empty: ${binTarget}`)
  }

  const content = readFileSync(binPath, 'utf8')
  if (!content.startsWith('#!/usr/bin/env node\n')) {
    throw new Error(`bin.dorka target must start with a Node shebang: ${binTarget}`)
  }
  ...
  if (process.platform !== 'win32' && (stats.mode & 0o111) === 0) {
    if (!fixExecutable) {
      throw new Error(`bin.dorka target is not executable: ${binTarget}`)
    }
