...
describe('locale-translation-policy zh round 5', () => {
  it('fixes brand spacing, hosted review, and Dorka Mobile regressions', () => {
    expect(
      repairTranslatedValue({
        key: 'auto.components.stats.ShareUsageCard.0eb31e79ee',
        enValue: 'Dorka IDE',
        localeValue: 'Dorka集成开发环境',
        locale: 'zh'
      })
    ).toBe('Dorka IDE')
    expect(
    ...
    expect(
      repairTranslatedValue({
        ...
        enValue: 'Show Dorka Mobile Button',
        localeValue: '显示 Dorka 移动按钮',
        locale: 'zh'
      })
    ).toBe('显示 Dorka Mobile 按钮')
    expect(
      repairTranslatedValue({
  ...
  it('keeps brand names English and generic workflow terms translated', () => {
    ...
    expect(
      repairTranslatedValue({
        ...
        enValue:
          'Git could not safely delete branch "{{value0}}"{{value1}}, so Dorka kept it to avoid losing local commits.',
        localeValue:
          'Git 无法安全删除分支“{{value0}}”{{value1}}，因此 Dorka 保留它以避免丢失本地提交。',
        locale: 'zh'
      })
    ).toBe('Git 无法安全删除分支“{{value0}}”{{value1}}，因此 Dorka 保留它以避免丢失本地提交。')
  })
