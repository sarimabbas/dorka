#!/usr/bin/env node
/**
 * Terminal E2E helpers for agent-browser + CDP testing against a running Dorka
 ...
 *
 * Usage:
 *   import { DorkaTerminal } from './terminal-e2e-helpers.mjs'
 *
 *   const term = new DorkaTerminal(9444)       // CDP port
 *   await term.connect()
...
export class DorkaTerminal {
  /** @param {number} cdpPort — the --remote-debugging-port used when launching Dorka */
  constructor(cdpPort) {
    this.port = cdpPort
  }
   ...
   */
  probePtyIdWithScreenshot(maxId = 10, screenshotPath = tempScreenshotPath('dorka-pty-probe.png')) {
    for (let i = 1; i <= maxId; i++) {
  ...
  /**
   * Take a screenshot of the Dorka window.
   *
   * @param {string} path — output file path
   * @returns {string} the screenshot path
   */
  screenshot(path = tempScreenshotPath('dorka-terminal.png')) {
    ab(this.port, ['screenshot', path])
    return path
  }

  /**
   * Open a new terminal tab in Dorka.
   * @returns {void}
   */
  newTerminal() {
  ...
  readLang(ptyId) {
    ...
    return this.screenshot(tempScreenshotPath('dorka-lang-check.png'))
  }
}
...
if (process.argv[1]?.endsWith('terminal-e2e-helpers.mjs')) {
  ...
  const term = new DorkaTerminal(port)
  console.log('Connecting to Dorka on CDP port', port, '...')
  term.connect()
