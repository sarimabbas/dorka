#!/usr/bin/env bash
# Capture one-shot macOS launch diagnostics for a published Dorka release.
#
# Usage:
#   DORKA_DIAGNOSTIC_TAG=v1.4.42-rc.1 bash config/scripts/macos-launch-diagnostics.sh
#   bash config/scripts/macos-launch-diagnostics.sh --tag v1.4.42-rc.1
set -euo pipefail

REPO="${DORKA_DIAGNOSTIC_REPO:-stablyai/orca}"
TAG="${DORKA_DIAGNOSTIC_TAG:-}"
KEEP=0
...
if [[ -z "$TAG" ]]; then
  echo "Set DORKA_DIAGNOSTIC_TAG or pass --tag vX.Y.Z-rc.N." >&2
  exit 2
fi

ARCH="$(uname -m)"
case "$ARCH" in
  arm64) ASSET="dorka-macos-arm64.dmg" ;;
  x86_64) ASSET="dorka-macos-x64.dmg" ;;
  *)
...
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/dorka-launch-diagnostics.XXXXXXXX")"
OUT_DIR="$WORK_DIR/output"
MOUNT_DIR="$WORK_DIR/mount"
APP_DIR="$WORK_DIR/Dorka.app"
DMG_PATH="$WORK_DIR/$ASSET"
mkdir -p "$OUT_DIR" "$MOUNT_DIR"

if [[ -d "$HOME/Desktop" ]]; then
  ZIP_PATH="$HOME/Desktop/dorka-launch-diagnostics-${TAG}-${TIMESTAMP}.zip"
else
  ZIP_PATH="$PWD/dorka-launch-diagnostics-${TAG}-${TIMESTAMP}.zip"
fi

diag_log() {
  printf '[dorka-diagnostics] %s\n' "$*"
}
...
write_environment_report() {
  {
    ...
    echo "$SHELL"
    echo
    echo "## current Dorka processes before diagnostics"
    pgrep -fl 'Dorka|dorka' || true
  } >"$OUT_DIR/environment.txt"
}

ensure_no_existing_dorka() {
  local existing
  existing="$(pgrep -x Dorka || true)"
  if [[ -z "$existing" ]]; then
    return 0
  fi

  {
    echo "An Dorka process is already running. Close Dorka and run this script again."
    echo
    ps -p "$(printf '%s' "$existing" | paste -sd, -)" -o pid=,ppid=,command= || true
  } | tee "$OUT_DIR/existing-dorka-process.txt" >&2
  exit 2
}
...
start_log_stream() {
  local file="$1"
  local predicate='process == "Dorka" OR eventMessage CONTAINS[c] "Dorka" OR eventMessage CONTAINS[c] "com.stablyai.dorka"'
...
latest_dorka_pid_for_app() {
  pgrep -nf "$APP_DIR/Contents/MacOS/Dorka" || true
}
...
wait_for_probe() {
  ...
  for i in $(seq 1 200); do
    local app_pid
    app_pid="$(latest_dorka_pid_for_app)"
...
run_launchservices_probe() {
  ...
  if [[ -n "$extra_env_name" ]]; then
    open -n -W \
      ...
      --env DORKA_STARTUP_DIAGNOSTICS=trace \
      --env DORKA_STARTUP_DIAGNOSTICS_TRACE_LIMIT=30000 \
      --env DORKA_STARTUP_DIAGNOSTICS_FILE="$bootstrap_file" \
...
run_direct_exec_probe() {
  ...
  ELECTRON_ENABLE_LOGGING=1 \
    DORKA_STARTUP_DIAGNOSTICS=trace \
    DORKA_STARTUP_DIAGNOSTICS_TRACE_LIMIT=30000 \
    DORKA_STARTUP_DIAGNOSTICS_FILE="$bootstrap_file" \
    "$APP_DIR/Contents/MacOS/Dorka" >"$stdout_file" 2>"$stderr_file" &
  local runner_pid="$!"
  wait_for_probe "$runner_pid" "$label"
  echo "ended_utc=$(date -u '+%Y-%m-%dT%H:%M:%SZ')" >>"$OUT_DIR/$label.meta"
  stop_log_stream "$stream_pid"
...
run_launchservices_probe "launchservices-bypass-lock" "DORKA_BYPASS_SINGLE_INSTANCE_LOCK" "1"
run_direct_exec_probe
write_system_log_snapshot
package_results
