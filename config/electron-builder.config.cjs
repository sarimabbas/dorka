...
// swap them over an installed Dorka and macOS treats each build as a new app.
const isMacHourly = process.env.DORKA_MAC_HOURLY === '1'
const isMacDaily = process.env.DORKA_MAC_DAILY === '1'
const isMacAdhoc = process.env.DORKA_MAC_ADHOC === '1'
...
const isWinHourly = process.env.DORKA_WIN_HOURLY === '1'
const isWinDaily = process.env.DORKA_WIN_DAILY === '1'
const isWinAdhoc = process.env.DORKA_WIN_ADHOC === '1'
const isWinDevChannel = isWinHourly || isWinDaily || isWinAdhoc
const isMacRelease = process.env.DORKA_MAC_RELEASE === '1' || isMacHourly || isMacDaily || isMacAdhoc
const isLinuxArm64Release = process.env.DORKA_LINUX_ARM64_RELEASE === '1'
const localBuildVersion =
  isMacRelease || isWinDevChannel ? undefined : process.env.DORKA_LOCAL_BUILD_VERSION
...
const devChannelBuildVersion = isHourlyChannel
  ? process.env.DORKA_HOURLY_BUILD_VERSION
  : isDailyChannel
    ? process.env.DORKA_DAILY_BUILD_VERSION
    : isAdhocChannel
      ? process.env.DORKA_ADHOC_BUILD_VERSION
      : undefined
...
const devChannelRepo = isHourlyChannel
  ...
  : isDailyChannel
    ...
    : isAdhocChannel
      ? 'dorka-adhoc'
      : null
const appId = 'com.stablyai.dorka'
...
// Electron's loader requirements alongside Dorka's headless-host dependencies.
...
// config/nsis/dorka-installer-hooks.nsh, which registers the same set on Windows.
...
// @dorka/windows-registry is a workspace link present on every host, so its presence proves nothing.
...
module.exports = {
  appId,
  productName: 'Dorka',
  protocols: [{ name: 'Dorka', schemes: ['dorka'] }],
  ...
  files: [
    ...
    '!examples{,/**/*}',
    // Why: pr-evidence/ is a local e2e screenshot output (DORKA_CAPTURE_EVIDENCE);
    ...
    // Why: the Windows CLI shim ships via extraResources to resources/bin/dorka.cmd
    // (beside the native resources/bin/dorka.exe). Packing the source tree into
    // app.asar too lets asarUnpack:['resources/**'] extract a second copy at
    // app.asar.unpacked/resources/win32/bin/dorka.cmd with no adjacent dorka.exe,
  ...
  afterPack: async (context) => {
    ...
    if (context.electronPlatformName === 'darwin') {
      ...
      let commit = process.env.DORKA_BUILD_COMMIT || process.env.GITHUB_SHA || 'unknown'
      if (commit === 'unknown') {
        try {
    ...
    if (context.electronPlatformName === 'darwin') {
      await signMacComputerUseHelper(join(resourcesDir, 'Dorka Computer Use.app'), context.packager)
      await signMacStandaloneHelper(
        join(resourcesDir, '..', 'MacOS', 'dorka-notification-status'),
        'dorka-notification-status',
        context.packager
      )
      await signMacStandaloneHelper(
        join(resourcesDir, '..', 'MacOS', 'dorka-keyboard-layout'),
        'dorka-keyboard-layout',
        context.packager
      )
    }
  },
  win: {
    executableName: 'Dorka',
  ...
  nsis: {
    artifactName: 'dorka-windows-setup.${ext}',
    ...
    include: resolve(__dirname, 'nsis', 'dorka-installer-hooks.nsh')
  },
  mac: {
    // Why rank Alternate: Dorka joins Finder's "Open With" list for Markdown without claiming
    ...
    extendInfo: {
      NSAppleEventsUsageDescription:
        'Dorka allows terminal-launched developer tools to automate local apps when you request it.',
      NSBluetoothAlwaysUsageDescription:
        'Dorka allows terminal-launched developer tools to access Bluetooth devices when you request it.',
      NSBluetoothPeripheralUsageDescription:
        'Dorka allows terminal-launched developer tools to access Bluetooth devices when you request it.',
      NSCameraUsageDescription: "Application requests access to the device's camera.",
      NSLocationUsageDescription:
        'Dorka allows terminal-launched developer tools to access location when you request it.',
      NSLocalNetworkUsageDescription:
        'Dorka allows terminal-launched developer tools to discover and connect to local development servers when you request it.',
      NSMicrophoneUsageDescription: "Application requests access to the device's microphone.",
      NSAudioCaptureUsageDescription:
        'Dorka allows terminal-launched developer tools to capture desktop audio when you request it.',
    ...
    extraResources: [
      ...
      {
        from: 'resources/darwin/bin/dorka',
        to: 'bin/dorka'
      },
      {
  ...
  dmg: {
    artifactName: 'dorka-macos-${arch}.${ext}'
  },
  linux: {
    // Why mimeTypes and not fileAssociations: shared-mime-info already maps *.md/*.markdown to
    // text/markdown, so reusing that type puts Dorka in the Open With list without shipping a glob
    ...
    // Why: Ubuntu desktop ships GNOME Dorka as the `dorka` package and /usr/bin/dorka.
    // The Linux installer should not claim those system package/file names.
    executableName: 'dorka-ide',
    ...
    desktop: {
      entry: {
        // Why: Electron reports WM_CLASS=dorka for the visible Linux window;
        // GNOME docks need an exact match to group it with dorka-ide.desktop.
        StartupWMClass: 'dorka'
      }
    },
    extraResources: [
      ...commonExtraResources,
      ...
      {
        from: 'resources/linux/bin/dorka-ide',
        to: 'bin/dorka-ide'
      },
      {
  ...
  appImage: {
    artifactName: isLinuxArm64Release ? 'dorka-linux-arm64.${ext}' : 'dorka-linux.${ext}'
  },
  deb: {
    packageName: 'dorka-ide',
    artifactName: 'dorka-ide_${version}_${arch}.${ext}',
    // Why: xvfb lets the bundled `dorka serve` CLI run browser panes on a headless
    ...
    // Why: symlink the bundled CLI onto PATH at install time so `dorka-ide serve`
  ...
  // on Intel Macs. The beforeBuild hook performs Dorka's targeted rebuild and
  // returns false so electron-builder does not rebuild optional cpu-features.
  npmRebuild: true,
  publish: {
    provider: 'github',
    owner: 'stablyai',
    repo: devChannelRepo ?? 'dorka',
...
function chmodUnixCliLaunchers(resourcesDir, electronPlatformName) {
  ...
  for (const launcherName of ['dorka', 'dorka-ide']) {
...
async function signMacComputerUseHelper(helperAppPath, packager) {
  if (!existsSync(helperAppPath)) {
    if (isMacRelease) {
      throw new Error(`Missing Dorka Computer Use helper app at ${helperAppPath}`)
    }
    return
  }
  const codeSigningInfo =
    isMacRelease && process.env.CSC_LINK && packager?.codeSigningInfo?.value
      ...
      : null
  const identity =
    process.env.DORKA_COMPUTER_MACOS_SIGN_IDENTITY ??
    process.env.CSC_NAME ??
    findInstalledMacSigningIdentity(codeSigningInfo?.keychainFile) ??
    (isMacRelease ? null : '-')
  if (!identity) {
    throw new Error('Missing signing identity for Dorka Computer Use helper app')
  }
  // Why: TCC grants attach to this nested app's code identity. Sign it before
  // the outer Dorka.app is sealed so production builds preserve that identity.
