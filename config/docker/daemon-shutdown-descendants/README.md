...
```sh
DORKA_BACKGROUND_LAUNCH=1 node config/scripts/run-daemon-shutdown-descendants-docker.mjs
```
...
`DORKA_DOCKER` overrides the Docker executable. `DORKA_DOCKER_PLATFORM` overrides the Linux architecture. The test creates a temporary image and removes it afterward.
