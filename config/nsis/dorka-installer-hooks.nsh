; electron-builder NSIS hooks for the Dorka Windows installer.
;
; electron-builder accepts exactly ONE `nsis.include` file, so every customInstall /
; customUnInstall hook Dorka needs lives here.

; ---------------------------------------------------------------------------
; Markdown "Open with Dorka" (issue #10138)
;
...
; uninstalling Dorka would leave .md pointing at a deleted ProgID.
;
; These writes are additive only. Registering a ProgID plus an OpenWithProgids
; hint and an Applications\<exe>\SupportedTypes entry puts Dorka in Explorer's
...
; ---------------------------------------------------------------------------
!define MARKDOWN_PROGID "Dorka.Markdown"

!macro DORKA_REGISTER_MARKDOWN_OPEN_WITH EXT
...
!macro DORKA_UNREGISTER_MARKDOWN_OPEN_WITH EXT
...
!macro customInstall
  ...
  !insertmacro DORKA_REGISTER_MARKDOWN_OPEN_WITH ".md"
  !insertmacro DORKA_REGISTER_MARKDOWN_OPEN_WITH ".markdown"
  !insertmacro DORKA_REGISTER_MARKDOWN_OPEN_WITH ".mdx"
...
; %LOCALAPPDATA%\Dorka\daemon-host so that app UPDATES cannot kill it —
...
!macro customUnInstall
  ${ifNot} ${isUpdated}
    ...
    Pop $0
    nsExec::Exec 'taskkill /F /IM "dorka-terminal-daemon.exe" $2'
    Pop $0
    Pop $2
    Pop $1
    Pop $0
    ...
    Sleep 500
    RMDir /r "$LOCALAPPDATA\Dorka\daemon-host"
  ${endIf}
  ...
  !insertmacro DORKA_UNREGISTER_MARKDOWN_OPEN_WITH ".md"
  !insertmacro DORKA_UNREGISTER_MARKDOWN_OPEN_WITH ".markdown"
  !insertmacro DORKA_UNREGISTER_MARKDOWN_OPEN_WITH ".mdx"
  System::Call "shell32::SHChangeNotify(i,i,i,i) (0x08000000, 0x1000, 0, 0)"
!macroend
