...
Reduce the recurring engineering and human work required to localize Dorka
...
Use gettext PO as Dorka's canonical bilingual translation source, under a
constrained profile:

- `msgctxt` — the stable Dorka message ID;
...
dependency. Dorka must not require network access or a translation service to
...
Dorka will support only the subset needed for its message model. PR B must
define and validate that subset rather than accepting arbitrary PO documents.

Each supported entry must carry:

- one stable Dorka message ID (`msgctxt`), unique within the file;
...
Plurals use Dorka's existing per-key convention: each i18next plural-suffixed
...
- XML is the format most likely to let a hand-edited community PR fail on a
  ...
  external editors produce whole-file diffs; Dorka's translation contributions
  arrive overwhelmingly as direct catalog PRs.

If a translation provider requires XLIFF, Dorka should add a deterministic
...
translator context, or review state. Meeting Dorka's requirements would require
...
own platform but are not a suitable authority for Dorka's macOS, Linux,
...
not become Dorka's sole source of truth or a runtime dependency. Selecting one
...
to establish a format decision. Dorka therefore chooses based on its own
