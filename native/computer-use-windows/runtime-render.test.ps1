...
public sealed class DorkaRenderTestCounter {
    public int FindAll;
}

public sealed class DorkaRenderTestBounds {
...
public sealed class DorkaRenderTestControlType {
    public string ProgrammaticName = "";
}

public sealed class DorkaRenderTestCurrent {
    public string AutomationId = "";
    public DorkaRenderTestBounds BoundingRectangle = new DorkaRenderTestBounds();
    public string ClassName = "fake";
    public DorkaRenderTestControlType ControlType = new DorkaRenderTestControlType();
...
public sealed class DorkaRenderTestPatternCurrent {
    public bool IsSelected;
    public string Value = "";
}

public sealed class DorkaRenderTestPattern {
    public DorkaRenderTestPatternCurrent Current = new DorkaRenderTestPatternCurrent();
}

public sealed class DorkaRenderTestCollection : IEnumerable {
    private readonly DorkaRenderTestElement[] values;

    public DorkaRenderTestCollection(DorkaRenderTestElement[] values) {
        this.values = values;
    }
    ...
    public DorkaRenderTestElement Item(int index) {
        return values[index];
    }
...
public sealed class DorkaRenderTestElement {
    public readonly List<DorkaRenderTestElement> Children =
        new List<DorkaRenderTestElement>();
    public DorkaRenderTestCounter Counter = new DorkaRenderTestCounter();
    public readonly DorkaRenderTestCurrent Current = new DorkaRenderTestCurrent();
    public bool FailFindAll;
    public int RuntimeIdValue;
    public string ValueText = "";

    public DorkaRenderTestCollection FindAll(object scope, object condition) {
        Counter.FindAll++;
        if (FailFindAll) {
            throw new System.InvalidOperationException("defunct node");
        }
        return new DorkaRenderTestCollection(Children.ToArray());
    }

    public DorkaRenderTestPattern GetCurrentPattern(object pattern) {
        DorkaRenderTestPattern result = new DorkaRenderTestPattern();
...
function New-TestCounter {
    New-Object -TypeName DorkaRenderTestCounter
}
...
function New-TestElement {
    ...
    $element = New-Object -TypeName DorkaRenderTestElement
...
$operationPath = Join-Path ([IO.Path]::GetTempPath()) ("dorka-runtime-render-test-" + [guid]::NewGuid() + ".json")
try {
    ...
    $tree = Render-DorkaTree $root $null
