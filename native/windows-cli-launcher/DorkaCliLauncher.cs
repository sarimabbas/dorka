...
internal static class DorkaCliLauncher
{
    private static int Main(string[] args)
    {
        try
        {
            string launcherDirectory = Path.GetDirectoryName(typeof(DorkaCliLauncher).Assembly.Location);
            ...
            string electronPath = Path.Combine(appDirectory, "Dorka.exe");
            ...
            {
                Console.Error.WriteLine("Unable to locate Dorka.exe next to \"{0}\"", resourcesDirectory);
                return 1;
            }

            if (!File.Exists(cliPath))
            {
                Console.Error.WriteLine("Unable to locate the Dorka CLI entrypoint at \"{0}\"", cliPath);
                return 1;
            }
            ...
            MoveEnvironmentVariable("NODE_OPTIONS", "DORKA_NODE_OPTIONS");
            MoveEnvironmentVariable("NODE_REPL_EXTERNAL_MODULE", "DORKA_NODE_REPL_EXTERNAL_MODULE");
            ...
            Environment.SetEnvironmentVariable(
                "DORKA_CLI_COMMAND",
                requestedCliCommand == "dorka-ide" ? "dorka-ide" : "dorka"
            );
        ...
        {
            Console.Error.WriteLine("Unable to start the Dorka CLI: {0}", error.Message);
            return 1;
        }
    }
